...
**PROCESS HYGIENE — BUILD AGENTS KILL BY PID, NEVER BY PATTERN (s48, 2026-08-17, self-disclosed).** A build agent ran an unscoped `pkill -f "fcode run"` on the shared box mid-session. **Blast radius was CHECKED and empty** — queue census flat, no tape files touched, only its own abandoned grid died — **but the check is what made it harmless, not the command.** This box runs several lanes' batteries concurrently, and a pattern kill cannot tell one lane's runaway from another lane's shard: it silently truncates that shard into a SHORT, BIASED sample that still looks like a completed tape and produces NO signal that anything was cut. **The rule: record the PID when you launch, kill that PID. If you have lost the PID, scope the match (`pgrep -f` first, read the list, kill the specific pids you recognise) — never `pkill -f` on a pattern that another lane could also match.** Same family as `git add -A` above: a wildcard is only safe on a box with one occupant.
