...
| **12** | ⭐ **THE POST-STOP WINDOW** — act on the 2-9 rounds after a raider stops moving | **GREP: PASS** — nothing in the live tree keys on "just stopped moving": no `since_move`, `arrived`, `stationary` or equivalent state. The window is currently invisible to the bot. | forward builder deaths occurring 2-9 rounds after last move, per forward builder-round | archive-measurable now (`fwd_death_attrib_s31.py` already computes the bucket) + local self-play | **MEASURED TODAY AND NOTHING IS AIMED AT IT: 40.6% of our forward builder deaths fall in 11.5% of forward rounds, at 4x mean hazard.** **Moving is BELOW-average hazard (0.90x) and parked 30+ rounds is the SAFEST state in the game (0.19x) — the danger is the transition, not the traverse and not the camp.** TOP shows the same non-monotone shape, so this is a universal hazard our bot simply does not react to. ⚠ the causal reading is soft — *moved last round* is partly a RESPONSE to danger — **but the bucket shape is robust and the plank only needs the shape.** |
| **13** | ⭐⭐ **AMBUSH THE REBUILD** — after we destroy an enemy turret, their builder must come to that tile | **GREP: PASS** — we keep no memory of a tile we destroyed and never revisit one; no rebuild/replace/revisit state exists in the live tree. | enemy builder deaths within d²≤2 of a tile we destroyed a turret on, within 25 rounds | live unrated; archive gives the base rate now | **MEASURED TODAY, AND IT TURNS MY OWN NEGATIVE INTO A WEAPON: destroying an enemy turret RAISES P(they build one within 25 rounds) from a 32.9% baseline to 65.0% — +32.1pp, and the top tier provokes the same (+29.5pp).** That replacement is why clearing nets only −0.68 turrets instead of −1 (#3). **But a provoked rebuild is a PREDICTED ARRIVAL at a KNOWN TILE, ~2/3 of the time** — the one thing this game almost never gives you. ⇒ **the destruction is the bait; the builder walking to the rubble is the target.** On-programme (kills units), and it costs nothing extra since we are already destroying the turret. |
