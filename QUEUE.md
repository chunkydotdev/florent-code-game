...
| **11** | ⭐ **ATTRIBUTE THE 46,858 CHANGED ENGINE BYTES** — the only item on this board resting on nobody's inference | **GREP: PASS** — nothing in `tools/` decodes or diffs engine `__text`; the 2.3.7 comparison was a one-off disassembly of six named functions | a named list of what changed between 2.3.6 and 2.3.7, function by function | the shipped `.so` itself — **it is not a sample, so there is no power question at all** | **The builder established that `__text` differs in 47,404 of 572,364 bytes (8.3%) while `can_launch` (390 instrs), `destroy_entity` (2,443), `GameRunner::run` (3,144), `runner::run` (1,643), `Watchdog::arm` (2,685) and `validate_bot_ast` (1,119) are byte-identical.** That is ~46.8k bytes of changed engine that **nobody has attributed to any function.** **Every exploit we hold is a guard-asymmetry read off this binary, so a silent change in the unattributed region is the one thing that could retire a weapon without warning** — and the retained Rust symbols make the attribution mechanical rather than speculative. **⛔ ENGINE PROBES ARE BUILDER-RESERVED, so research cannot run this; it is queued, not claimed.** Raised by the side lane, who is right that it rests on no one's inference. |
