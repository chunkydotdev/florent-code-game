...
# --- LOKI-HOMEEARLY (v250): LIFT THE ROUND GATE ON THE HOME LAUNCHER -------
# The gate above is priced entirely on the COST side (a launcher adds +10% to
# the one global additive scale factor, so buying it at r10 levies that
# surcharge on the whole match).  It was never priced against the OPPORTUNITY
# it excludes, and the opportunity is measured:
#   * 53.7% of our games have an enemy builder planting a turret within
#     d^2 <= 8 of OUR core;
#   * 73.3% of near-core enemy builds happen BEFORE round 160;
#   * our MEDIAN actual launcher build is round 318.
# So the r160 gate throws away roughly three quarters of the home pickup
# window, and the median build throws away more than that again.
# ⚠ THE PRICE IS KNOWN AND IT IS NOT ZERO: the early-levy penalty was measured
# at roughly 2.8pp on a different chassis (LATE160 51.42 vs RES0 48.63).  This
# variant exists to test whether the pickup opportunity outweighs that levy;
# it is not a claim that the levy is imaginary.
# LOKI_HOMEEARLY_RND = 1 rather than 0: a launcher bought before the first
# harvesters exist competes with the economy for the opening bank, and
# _try_build_launcher already refuses below SLOT_HARVESTERS < 1, so r24 is the
# earliest round at which that refusal is normally already satisfied.
...
# carried from _v251pinaim: pin the victim in the pickup ring instead of ejecting it
LOKI_PINAIM_ON = True
