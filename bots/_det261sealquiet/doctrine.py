...
# --- LOKI-COLLARSEAL (v254): AN ARRIVED BODY BUILDS, IT DOES NOT ATTACK ----
# MEASURED over 157,816 INSERT throws, bucketed by LANDING distance to their
# core (the reached/any_atk/core_atk columns are populated for INSERT):
#   d^2 <= 8   n=32,628  reached 61.4%  any_atk 3.0%  CORE_ATK 1.55%  med life 5
#   9-26       n=25,596  reached 14.7%  any_atk 3.3%  CORE_ATK 1.34%  med life 9
#   27-50      n=14,306  reached  8.0%  any_atk 0.9%  CORE_ATK 0.13%  med life 9
#   >200       n=36,119  reached  3.2%  any_atk 0.3%  CORE_ATK 0.05%  med life 7
# DELIVERY WORKS; CONVERSION DOES NOT.  A delivered builder attacks the core
# 1.55% of the time and lives a median of FIVE rounds, and a lone builder does
# 2 dmg a turn against 500 HP -- it cannot kill anything.  ⇒ AN ARRIVING BODY
# THAT TRIES TO ATTACK IS WASTED.  Five rounds is two or three BUILD actions.
# WHAT TO BUILD: the enemy core's 12-tile SPAWN RING.  CORE_SPAWNING_RADIUS_SQ
# = 2 -- the 8 orthogonal seats plus the 4 diagonal corners -- and can_spawn
# calls is_bot_passable (verified by disassembly), so ANY building on one of
# those tiles is permanent spawn denial: destroy is TEAM-CHECKED, so they
# cannot clear our barrier off their own ring, and 3 Ti costs them 15
# builder-actions to chew through.
# The archive's maximum hostile occupancy of an enemy spawn ring is 6 of 12,
# reached four times in 2,710 sides -- nobody has ever sustained delivery long
# enough to close a ring.
# THIS PLANK IS TWO EDITS AND NO NEW ROUTINE: the parent's step-2 seal is
# (i) lifted ABOVE the core peck and (ii) widened from the 8 heal seats to all
# 12 spawn tiles.  Ablates alone; stacks with a delivery plank.
LOKI_COLLARSEAL_ON = True
...
LOKI_QUIET_ON = False     # no builder melee: no core peck, no siphon hit, no counterbattery
