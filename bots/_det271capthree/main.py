        ...
        for eid in ct.get_nearby_buildings():
            try:
                if ct.get_team(eid) == self.team and ct.get_entity_type(eid) == EntityType.LAUNCHER:
                    # LOKI_LAUNCHER_CAP: this was a BOOLEAN ("any launcher -> refuse"),
                    # i.e. a hard cap of 1 that has never been tested at any other
                    # value. Counting instead lets the cap be swept.
                    _n_lnch += 1
                    if _n_lnch >= LOKI_LAUNCHER_CAP:
                        seen_launcher = True
                        break
            except Exception:
                continue
