...
class RaidMixin:
    ...
    def _launcher_turn(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            if LOKI_PINAIM_ON:
                # PIN, do not eject.  Keep the victim inside our own pickup
                # ring (d^2 <= 2) so this launcher can pick it up again next
                # round, free, indefinitely.  d^2 == 2 first: that is a
                # DIAGONAL of the launcher, and a builder bot's attack is
                # orthogonal-only, so a victim pinned there cannot answer.
                # d^2 == 0 is the launcher's own tile and is excluded here as
                # well as being refused by can_launch.
                pin = [t for t in sites if 1 <= t.distance_squared(lp) <= 2]
                pin.sort(key=lambda t: -t.distance_squared(lp))
                far = pin + far
            for site in far:
                try:
