...
# --- LOKI-SPAWNLOCK (v252): SITE THE LAUNCHER ON AN ENEMY SPAWN TILE -------
# Two mechanisms in one 20 Ti building, and the second one has never been
# scored on this line:
#  (1) EVICTION.  Its _launcher_turn EXILE step throws every enemy builder
#      that walks up to heal or rebuild.  That is the Jython/LingLing40
#      mechanism (56-78 evictions of US per game, field-measured) pointed the
#      other way, and it costs 0 ammo and fires EVERY round.
#  (2) SPAWN DENIAL.  Verified by disassembly: Controller::can_spawn calls
#      Tile::is_bot_passable, so ANY building on a spawn tile denies the enemy
#      core spawning there.  The spawn envelope is CORE_SPAWNING_RADIUS_SQ = 2,
#      i.e. EXACTLY 12 tiles -- the 8 orthogonal seats plus the 4 diagonal
#      corners of the 2x2 footprint -- NOT the sqrt(8) action radius.  Our
#      existing barrier seal already covers up to 8 of those 12 and nobody has
#      ever scored it as spawn denial; siting the launcher ON one of the 12
#      makes the launcher itself a 13th denial that also throws.
# THREE FIXES vs the _v233evict58 ancestor this is ported from:
#  (a) the siege prerequisite (a live forward sentinel) is DELETED -- it held
#      that plank's coverage to 16% of games;
#  (b) the call site moves ABOVE the barrier-seal step: the seal returns
#      before the raider ever reaches the launcher branch, AND the seal
#      removes the very tiles a victim would stand on, so the two planks were
#      fighting each other;
#  (c) the build tile is preferred to be ITSELF one of the 12 spawn tiles,
#      scored by how many of the REMAINING spawn tiles fall inside the
#      resulting launcher's d^2 <= 2 pickup ring.
LOKI_SPAWNLOCK_ON = True
LOKI_SPAWNLOCK_TI_FLOOR = 40    # bank left after paying for the launcher
LOKI_SPAWNLOCK_NEAR_DSQ = 8     # build tile must be this close to a Core tile
LOKI_SPAWNLOCK_CENSUS_DSQ = 32  # a friendly launcher inside this == cap reached
...
# carried from _v259rnd1: lift the launcher round gate to 1
LOKI_HOMEEARLY_ON = True
LOKI_HOMEEARLY_RND = 1

# ---- merged from _v262bodyblock: constants only, extracted as WHOLE
# STATEMENTS via ast (a line-based grep split a multi-line tuple and
# produced an unparseable file). Nothing already defined is redefined. ----
LOKI_BODYBLOCK_ON = True
...
LOKI_BB_STALL_MAX = 3
LOKI_BB_BAN_RNDS = 80
LOKI_BB_HYSTERESIS = 3
LOKI_BB_ORE_SAMPLE = 6
LOKI_BB_LOG = False
