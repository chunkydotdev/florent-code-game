...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-RENTGUN (v313) rental state, per raider.  Declared here
        # rather than created lazily: this file already carries two lazily-made
        # attributes (self.spawn_salt, self.nav_fail) and they are a hazard, not
        # a convention.  rent_seen is the HOP memory -- ore tiles this raider
        # has finished with -- and is bounded by LOKI_RENT_HOP_MAX. ---
        self.rent_harv = None
        self.rent_ore = None
        ...
        self.rent_done = 0
        self.rent_stranded = 0
        self.rent_seen = set()
        self.rent_prospect = None
        self.rent_prospect_rnd = -1
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # LOKI-RENTGUN.  A rental is 20 ammo for the two shots that kill a
            # 30 HP harvester, and it is bought BEFORE the Core can possibly
            # know about it: the rent path deliberately does NOT write
            # SLOT_FWD_GUN (that slot is the forward-sentinel CAP, it never
            # decrements, and rentals would close that arm for the match -- the
            # LOKI-2b rubble bug).  So the magazine floor has to carry the
            # rental unconditionally.  Without this the base floor is
            # AMMO_FLOOR = 16, 16 < 20, AND THE PLANK SILENTLY NEVER FIRES --
            # which is exactly the autopsy that found us parked at 24 ammo for
            # rounds 100-900 while a plank needed 30.
            if LOKI_RENTGUN_ON:
                ammo_target = max(ammo_target, LOKI_RENT_AMMO_BANK)
    ...
    def _turret(self, ct):
        ...
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                # LOKI-RENTGUN.  A rented Sentinel exists to kill ONE harvester
                # and be handed back.  The base table ranks HARVESTER 5th --
                # BELOW BUILDER_BOT -- so a defender's bot wandering onto the
                # line would soak the two shots the rental is budgeted for, the
                # harvester would live, and we would hold the +20% until the
                # hold clock expired.  Promoted to just under CORE (the Core is
                # still the win condition and still outranks everything), and
                # ONLY at the rental's own geometry: the turret is built from a
                # tile orthogonally adjacent to the ore, so the real separation
                # is 1, 2 or 4 and no home sentinel is ever re-aimed by this.
                if (LOKI_RENTGUN_ON and et == EntityType.HARVESTER
                        and turret_type == EntityType.SENTINEL
                        and p.distance_squared(t) <= LOKI_RENT_LOCK_DSQ):
                    prio = -1 if LOKI_RENT_HARV_FIRST else 0.5
                if prio < best_prio:
                    best_prio, best = prio, t
            if best is not None:
                # can_fire() returns True at 0 ammo on this engine and fire()
                # RAISES -- the base survives that only because of the enclosing
                # try and run()'s blanket catch, i.e. by accident rather than by
                # check.  Read the pool.  Falling through rather than returning
                # keeps the parent's control flow (the nearby-entity sweep and
                # then _idle_rotate) exactly as it was.
                shot = (LOKI_RENT_SENT_SHOT if turret_type == EntityType.SENTINEL
                        else LOKI_RENT_GUN_SHOT)
                if not (LOKI_RENTGUN_ON and ct.get_global_ammo() < shot):
                    ct.fire(best)
                    return
        except Exception:
            pass
