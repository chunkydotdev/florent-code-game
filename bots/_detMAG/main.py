...
# PIECE MAG -- retire the hive-only 256-ammo magazine.  See the block in _core.
# False = the map uses the same ammo ladder as every other map (24 under siege,
# AMMO_FLOOR otherwise).  True restores the old constant for A/B.
HIVE_MAGAZINE_ON = False
...
class Player:
    ...
    def _core(self, ct):
        ...
        if sporks_ammo:
            ...
            # PIECE MAG -- THE HIVE MAGAZINE IS RETIRED (see HIVE_MAGAZINE_ON).
            #
            # This branch used to read `256 if hive_magazine`, on a map where
            # the block's OWN comment three lines up says to keep the magazine
            # small because "a 60-round stockpile merely starves harvesters and
            # counter-gunners".  256 is 16x AMMO_FLOOR and 8x the atoll burst,
            # and it starved exactly what that sentence predicts.
            #
            # MEASURED (research's Lunds decode, 3/3 hive games): we convert
            # 286-341 Ti into ammunition and fire 3-16 shots ALL GAME, while
            # delivering 170 / 320 / 600 Ti against their 3,130 / 3,590 / 1,720.
            # Bank trace: r0 470 Ti -> r50 FOUR Ti and 73 ammo -> pinned at
            # 1-46 Ti from r150 with the magazine held at 256.  Harvesters ours
            # 2-3, theirs 8-11, off mirror-image geometry.  **We spend more
            # titanium filling a magazine we never fire than we mine all game.**
            #
            # And its arming is the SLOT_HOME_GUN monotone counter (`weapons`),
            # which this file documents elsewhere as counting rubble and the
            # saboteur's FORWARD gun at the enemy Core, and never decrementing.
            # So it latches on for the rest of the match the moment we build any
            # turret anywhere -- it is not even gated on home defence existing.
            #
            # Retired rather than retuned: there is no measured case for ANY
            # hive-specific magazine, only a constant nobody has defended. With
            # the flag off the map falls back to the standard ladder every other
            # map already uses, which fired 3-16 shots' worth of demand.
            hive_magazine = (
                HIVE_MAGAZINE_ON
                and weapons and w == 25 and h == 25
