...
LAUNCHER_RESERVE = 80   # DEAD: defined here, referenced nowhere (verified s20).

# PIECE NL -- BUILD NO LAUNCHER (2026-08-09, s20). Flag so the removal stays
# ablatable and det-testable in both directions.
#
# THE CASE, from two independent decodes neither of which was looking for this:
#   - 0 of 20 decoded TOP-TIER games contain a launcher AT ALL. We build one in
#     69% of ours. We field a unit the strong field has entirely abandoned.
#   - Across 45 Lunds games our launcher threw 759 times. Split by class:
#     705 (92.9%) were ENEMY builders thrown away = defensive disposal;
#     only 54 (7.1%) were our own builder thrown toward their core = offensive
#     insertion. Lunds uses the same unit offensively 4.4x more often (31.5%).
#
# So ~93% of what this removes is defensive disposal plus the build cost, which
# is an ECONOMY/DENIAL question — measurable on delivered titanium and locally
# gateable. Only the 7% insertion half is aggression-dependent and therefore
# inherits the backfire constraint (a dominated pool reports "aggression is
# free" by construction, so that half cannot be gated on opp_v63).
#
# WHAT THE EVIDENCE DOES NOT SAY, kept here so the leg is not over-sold: our
# 705 disposals vs Lunds' 250 is a statement about how much enemy labour arrives
# adjacent to our structures, NOT about how aggressive we are. Whether that
# means we are being infiltrated or successfully catching infiltrators is
# undetermined. And 189 of those disposals threw an enemy builder AWAY from
# their core, which could in principle be pulling them into a kill zone rather
# than disposal — untested, and it would move at most those 189 to a third class.
BUILD_LAUNCHER_ON = False
...
class Player:
    ...
    def _try_build_launcher(self, ct):
        ...
        # PIECE NL: gate the BUILD only. Every other launcher path (throwing
        # from one that already exists, the launcher turret branch, the stall
        # and give-up timers) is left untouched, so a launcher inherited from
        # any other source still behaves exactly as before. This isolates
        # "should we pay for one" from "how should one be used".
        if not BUILD_LAUNCHER_ON:
            return False
        if ct.read_store(SLOT_LAUNCHER):
            return False
        for eid in ct.get_nearby_buildings():
