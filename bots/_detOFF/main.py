...
# PIECE HV -- THE HIVE FREEZE, RESCOPED FROM "STOP" TO "STAY HOME".
#
# _expand carried a hive-only clause that returned OUTRIGHT, for the rest of
# the match, once a live home gun stood past r42.  Piece J narrowed WHEN it
# arms (live scan instead of the monotone SLOT_HOME_GUN counter, which read
# 1 for rubble and for our own forward siege gun alike) but left WHAT it does
# untouched, and what it does is stop the entire expander: harvesters, link
# building, the chain medic and the move phase all die together.  The file's
# own comment already called it "the confirmed economy self-freeze".
#
# MEASURED (tape row hive-freeze-live-defect, det NOISE_OFF both arms, hive
# only, 6 seeds x 2 seats vs opp_v63): seat A, all 6 seeds, 5,260 Ti / 28
# buildings frozen vs 11,030 Ti / 155 buildings unfrozen -- 2.1x delivered
# titanium, 5.5x standing buildings, 0 outcome flips.  Delivered titanium is
# tiebreak LEVEL 1, and research's census has it deciding 219/219 of the
# 26.2% of ladder games that reach r1000 (36.7% under v80).  Halving it on a
# live pool map is not a rounding error.
#
# WHY RESCOPE RATHER THAN DELETE.  The clause names a picket class, and the
# defensible half of that intent is real: on hive both cores sit on the same
# diagonal and the expander's ore walk runs straight down the core-core lane,
# so an unfrozen worker marches into gunner fire.  That argues against
# WALKING FORWARD.  It does not argue against building a harvester on the ore
# you are already standing next to, wiring the link you already queued, or
# patching the conveyor beside you -- none of which move the unit an inch.
# So the freeze now suppresses exactly the forward half:
#   - the action phase runs unchanged (harvester / link / chain medic),
#   - paving is off,
#   - and a move target on the ENEMY side of the core-core midline is
#     dropped rather than walked to.
# Home-side ore, which is what 155-vs-28 buildings was made of, is reachable
# again; the picket lane is still not entered.  Set False to restore the
# outright return (identity control).
HIVE_FREEZE_FIX_ON = False
...
class Player:
    ...
    def _forward_of_midline(self, p, tgt):
        """PIECE HV -- would walking to `tgt` cross onto the enemy's side?

        The picket lane the hive freeze was defending against runs down the
        core-core diagonal, so "forward" is measured against the two Cores and
        nothing else: a target strictly closer to the enemy Core than to ours
        is on their side of the midline.  Pure arithmetic on two cached
        Positions -- no engine calls, so it costs nothing on the CPU budget
        that already peaks at 93% of the limit.

        Two exemptions, both deliberate.  An adjacent target (d^2 <= 2, i.e.
        one step including diagonals) is never forward -- refusing the tile
        under the builder's nose would re-create the freeze in miniature, and
        a single step is not a march.  And an unrecognised map, where
        enemy_core_for falls back to a reflection rather than the CORE_PAIRS
        table, returns False: this clause only ever arms on hive, whose pair
        IS in the table, so a fallback answer here means the caller is
        somewhere the piece was never scoped for and the conservative reply is
        "not forward".
        """
        if tgt is None or self.core is None:
            return False
        if p is not None and p.distance_squared(tgt) <= 2:
            return False
        if not core_anchor_exact(self.mw, self.mh, self.core):
            return False
        ec = enemy_core_for(self.mw, self.mh, self.core)
        return tgt.distance_squared(ec) < tgt.distance_squared(self.core)
    ...
    def _expand(self, ct):
        ...
        # PIECE HV (see HIVE_FREEZE_FIX_ON): armed, the freeze no longer stops
        # the expander -- it pins it to our own side.  allow_forward rides down
        # to the pave flag and the move-target clamp at the bottom of _expand;
        # everything between here and there (harvester, link, medic) is
        # deliberately left running, because none of it moves the unit.
        allow_forward = True
        if hive_freeze:
            if not HIVE_FREEZE_FIX_ON:
                return
            allow_forward = False
        ...
        harv = ct.read_store(SLOT_HARVESTERS)
        allow_pave = (has_launch or harv >= 2) and allow_forward
        ...
        for d in DIRECTIONS:
            ...
            if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh:
                if ct.get_tile_env(bp) == Environment.ORE_TITANIUM and ct.get_tile_building_id(bp) is None:
                    self.tgt = bp
                    break
        # PIECE HV, the forward clamp.  Placed AFTER the adjacent-ore override
        # above deliberately: a tile one step away is inside the picket lane by
        # nobody's definition, and refusing it would re-create the freeze in
        # miniature.  Only a genuine walk toward the enemy Core is dropped.
        if not allow_forward and self._forward_of_midline(p, self.tgt):
            return
