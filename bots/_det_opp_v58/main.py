"""v82_highx_asym_d26 OFFLINE — overnight promote on v79 (+ live CPU wrapper).

Verified NEW 80 / OLD 77 / 9436 20 / LR 20.
Lever: high-x asym build-only delay26 + UNDER abort (hive-B Ti without hive-A 2397 bleed).
"""
import math
...
class Player:
    ...
    def _builder(self, ct):
        ...
        if (
            ...
            self.role = "saboteur"
        # Non-square compact band: second attacker (9436 antler lever).
        if (
            self.role == "expand" and self.role_n == 2 and not self.link_queue
            and 240 <= self.mw * self.mh <= 280 and self.mw != self.mh
            and ct.read_store(SLOT_HARVESTERS) >= 2 and rnd >= 8
