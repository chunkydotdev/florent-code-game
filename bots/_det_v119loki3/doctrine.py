...
# ============================================================================
# LOKI-3 (v119) -- THE KIDNAP PLANK.  Forward launcher PLACEMENT, plus a
# throw-target scorer.  Set LOKI_KIDNAP_ON = False and this bot is LOKI-2b
# exactly: that is the ablation control, and it is the only control that can
# attribute anything, because every other difference is zero by construction.
#
# WHY THIS SHAPE AND NOT THE ONE THE QUEUE ASKED FOR.  The brief was "throw an
# enemy builder onto a tile covered by our own gunner's ray".  Two cheap checks
# reshaped it before a line was written:
#
#   CODE FACT -- our turrets ALREADY fire at enemy builder bots (main.py:660
#   for the Gunner, and the Sentinel priority map has BUILDER_BOT at prio 3).
#   So the our-side half of the conversion needs no new targeting logic and no
#   behavioural bet on either team.  That half of the brief survives intact.
#
#   SPEC ARITHMETIC -- free, and it cannot be confounded.  A Gunner is 7 dmg on
#   a 1-round reload: SIX shots and ~11 rounds to kill a 40 HP builder, at 4
#   ammo each.  A Sentinel is 18 dmg on a 2-round reload: three shots, ~7
#   rounds, 30 ammo.  A thrown builder whose move cooldown is clear steps off
#   the ray in ONE round.  So kidnap-into-our-ray is worth ~7-18 HP per throw
#   unless the target is pinned -- HARASSMENT ARITHMETIC, not a kill.  s22
#   already refuted imprisonment, so pinning is not re-opened here.
#
# What survives is the half the corpus work itself ranked FIRST: PLACEMENT.
# 20.65% of enemy-builder-rounds before r250 sit beside a tile we could have
# built a launcher on, 16.03% persist a round (a turret cannot fire the round
# it is built, so persistence is the binding filter), and 0 of 1,355 games have
# zero opportunity.  We build 0.64 launchers/game and NONE of them forward.
#
# And the mechanism that connects a launcher to a dead Core is not a dead
# builder -- it is LOKI-1's own collar doctrine.  The defender's HEAL LINE is
# why Cores survive: net HP to kill is a stable 500-512 but raw hits landed
# range 28 -> 1206 across 14 decoded games, and that 43x spread IS the heal
# line.  A builder thrown OFF the 8 collar seats stops healing for as long as
# it takes to walk back.  THAT is the currency.  The ray bonus is taken when
# the geometry is free; it is never the reason to build.
#
# HONEST LIMIT, STATED BEFORE THE NUMBERS EXIST: how long a thrown builder
# stays on its landing tile is UNMEASURED.  It is the single number that
# decides whether the ray bonus is worth one shot or six, and it is out with
# research.  If the answer is "leaves next round", the ray bonus is a ~7-18 HP
# harassment term and the plank rests entirely on displacement.  Nothing here
# claims otherwise.
# ============================================================================
...
# The cap is enforced by a LIVE CENSUS, never by a monotone store counter.
# This is LOKI-2b's defect, and it is not being reintroduced: SLOT_FWD_GUN was
# written only as read+1 and counted rubble, so three destroyed forward
# sentinels closed the arm PERMANENTLY.  There is no store slot for forward
# launchers at all -- all 16 are in use, and none is needed, because the build
# condition requires SEEING a live enemy builder beside the site, so the
# census can never be blind at the moment it is consulted.

# --- throw scoring ---------------------------------------------------------
# A launcher near OUR Core is doing home defence and throws intruders as far
# from OUR Core as it can reach -- that is LOKI-2b's behaviour and it is kept
...
LOKI_KIDNAP_RAY_CONFIRMS = 6 # can_fire_from confirmations per turn (CPU bound).
                             # get_attackable_tiles_from ignores OCCUPANCY by
                             # contract, so for a Gunner it over-reports through
                             # blockers; the cheap set is a prefilter and
                             # can_fire_from is the instrument that decides.
