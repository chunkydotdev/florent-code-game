...
class Player(EcoMixin, RaidMixin):
    ...
    def _try_counterbattery(self, ct):
        ...
        # LOKI-9: order the facing search enemy-core-ward. Computed once per
        # call, not per tile -- the 10ms budget is per unit and this path can
        # run inside a defend turn that has already spent most of it.
        facings = DIRECTIONS
        if LOKI9_FACE_CORE:
            ecore = unpack_pos(ct.read_store(SLOT_ENEMY_CORE))
            if ecore is not None:
                # Rank by alignment with the core-ward vector (cosine, via a
                # normalised dot product). Stable sort, so DIRECTIONS order
                # survives as the tie-break and the change is a REORDER of the
                # existing search rather than a new rule.
                ex, ey = ecore.x - p.x, ecore.y - p.y
                emag = math.hypot(ex, ey) or 1.0

                def _coreward(f):
                    dx, dy = f.delta()
                    dmag = math.hypot(dx, dy) or 1.0
                    return -((dx * ex + dy * ey) / (dmag * emag))

                facings = sorted(DIRECTIONS, key=_coreward)
