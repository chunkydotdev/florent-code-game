...
# ============================================================================
# LOKI-9 -- TURRET FACING. Prereg: docs/prereg/PREREG-loki9-facing.md
#
# THE DEFECT, read out of THIS TREE rather than off the archive. The home
# counterbattery path picks facing as the FIRST direction in DIRECTIONS order
# that satisfies can_fire_from(bp, facing, kind, SLOT_THREAT) -- and a turret's
# facing is then frozen for the rest of the match. SLOT_THREAT is a TRANSIENT
# enemy position at build time. If the reported threat is a raider inside our
# own collar, the turret is permanently aimed INWARD.
#
# The archive says our home sentinels have a target on their line 13.2% of the
# time against 14.2% for a RANDOM draw over 8 facings with position held fixed
# -- we aim 0.9pp WORSE THAN CHANCE, 11.4% point at our own core, and 13.7%
# never saw one enemy on their line across a median 175-round life. A choice
# cannot be reliably worse than random unless it is not being made on the
# relevant axis, which is exactly what the code above does.
#
# SUBJECT WARNING, because this bit it once already tonight: those archive
# numbers are OUR TEAM OVER TIME and are Eir-dominated -- LOKI-8 has ~70 ladder
# games in that corpus. They motivate the plank; they do not size it. The
# licence for this change is the code reading, and every bar in the prereg is
# paired WITHIN the leg.
#
# THE TREATMENT: among facings the existing gate ALREADY PERMITS, prefer the one
# pointing at the known enemy core. Build incidence, siting, cost and timing are
# untouched -- this reorders a search, it does not widen or narrow it. Nothing
# relocates, so no new exposure and no collar interaction.
# ============================================================================

LOKI9_FACE_CORE = False     # prefer enemy-core-ward facing among legal facings
