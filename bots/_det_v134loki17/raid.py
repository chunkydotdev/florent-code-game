...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        # LOKI-17: BEST-FIT, NOT FIRST-FIT.
        # The guard below (range + can_fire_from + buildable) was already here
        # and is unchanged. What was missing was a CHOICE: the loop accepted the
        # FIRST tile that could fire and returned, so the site was decided by
        # wherever this builder happened to be standing when the predicate first
        # passed. Measured consequence across three opponents: median nearest
        # d^2 to the enemy core = 32, EXACTLY the Sentinel's range limit, and
        # only 52.1% of our sentinels could fire on the round they were built.
        # A team we beat comfortably plants at d^2 = 18 with 77.9% shootable.
        #
        # A Sentinel at the boundary is one enemy step from being useless and
        # has no margin for the core drifting behind cover; a closer plant keeps
        # more of the footprint on-ray and survives longer (median lifetime 27
        # rounds vs 74). So: score every legal (tile, facing) pair and take the
        # CLOSEST, tie-broken by how many core tiles it covers.
        #
        # Cost is bounded and small: <=4 cardinals x <=4 core tiles = 16 checks,
        # the same ceiling the first-fit loop already had in its worst case --
        # it simply no longer exits early. The CPU guard above still applies.
        best = None                      # (d2, -coverage, bp, facing)
        for d in CARDINALS:
            bp = p.add(d)
            ...
            for target in tiles:
                ...
                try:
                    ...
                    if not ct.can_build_sentinel(bp, facing):
                        continue
                    # COVERAGE: how many core tiles this exact (tile, facing)
                    # can hit. A pair that covers two footprint tiles keeps
                    # firing when the first is blocked.
                    cover = 0
                    for t2 in tiles:
                        if bp.distance_squared(t2) <= 32 and \
                                ct.can_fire_from(bp, facing, EntityType.SENTINEL, t2):
                            cover += 1
                except Exception:
                    continue
                key = (bp.distance_squared(target), -cover)
