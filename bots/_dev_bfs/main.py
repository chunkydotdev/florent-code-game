"""_dev_bfs — aug7 + bounded-BFS pathing to replace the greedy walker.

aug7's `_move_toward_target` picks a productive cardinal at random, tries
the two perpendiculars, then the reverse, and gives up on the target after
3 stuck rounds. That's a local hill-climb with no memory of what it already
tried, so in a concave wall pocket — common on the wallier maps in our
rotation (archipelago 30.8% walls, saga 28.5%, lighthouse 25.0%) — it
wanders the inside of the pocket instead of routing around it.

This version replaces *only* step selection: a bounded BFS over a per-unit
memo of walls we've actually seen, with the resulting path cached and
walked one step per round, falling back to aug7's exact greedy behaviour
whenever BFS can't produce a path (target unreachable within the node
budget, or the goal tile is a known wall). Target selection, stuck
detection, build/heal priority, and _try_move's trail-laying are all
unchanged — see aug7's original docstring below for the rest of the
strategy.

Original aug7 docstring follows.
...
# Bounded-BFS node cap for builder pathfinding (see _find_path). The search
# is pure Python over a plain dict -- no engine calls per node -- and is
# capped by number of tiles *expanded* (popped off the frontier), not tiles
# merely discovered, so worst-case work per call is a fixed, small multiple
# of this number regardless of map size.
#
# Measured (process-time instrumentation, ~55k builder-rounds across
# archipelago/saga/drumlin/snowflake, 3 seeds each, 8x8-26x26): cost is
# ~8us/node expanded, plus a fixed tail of up to ~3.5ms whenever BFS
# exhausts the cap and falls back to the greedy walk (that tail comes from
# the greedy fallback itself -- up to 4 directions, each an engine call --
# not from BFS, and is there regardless of the cap). At the 250-400 range
# this file started from, the exhausted-search worst case measured 5.8ms --
# only ~27% headroom under CPU_BUDGET_US, uncomfortably close given this is
# dev-machine timing, not ladder hardware. 200 measured a 3.8ms worst case
# (~53% headroom) at the cost of a somewhat higher fallback rate (~16% of
# recomputes vs ~7% at 300) -- a small, mostly long-range/unexplored-target
# slice, since the median successful search only expands ~20 nodes. That
# trade favours CPU safety without giving up much reach.
BFS_NODE_CAP = 200
...
class Player:
    def __init__(self):
        ...
        # Per-unit wall memo: (x, y) -> is_wall, filled from tiles we've
        # actually seen (see _update_wall_memo). Walls never change, so this
        # is exact forever and never needs invalidating -- only growing.
        self.wall_memo: dict[Position, bool] = {}

        # Cached BFS path toward self.target, as [target, ..., next_step] so
        # the next step to take is always path[-1] (see _find_path). Reused
        # across rounds until it's consumed, the target changes, or a step
        # in it turns out to be blocked -- see _move_toward_target.
        self.path: list[Position] = []
        self.path_target: Position | None = None  # target self.path was computed for
    ...
    def _move_toward_target(self, ct: Controller) -> None:
        ...
        Target priority and stuck detection are unchanged (see _pick_target
        and _run_builder) -- what changed is *how* we walk to the target.
        Instead of a greedy hill-climb, we BFS a path over remembered walls
        (_find_path), cache it, and walk it one step per round. We only
        recompute when the cache can't be trusted: no path, the target
        changed, or the path is fully consumed. Recomputing every round is
        the direct way to blow the CPU budget on the walliest maps.
        """
        # Grow the wall memo every round, even on a move-cooldown round --
        # vision doesn't care whether we can act, so this is a free chance
        # to learn more terrain for whenever we next get to plan.
        self._update_wall_memo(ct)
        ...
        if self.target is None or pos == self.target or self.stuck >= 3:
            ...
            self.stuck = 0
            self.path = []  # target changed -- old path is stale regardless of contents
        if self.target is None:
            return

        # Recompute only if the cache can't be trusted for this target.
        if not self.path or self.path_target != self.target:
            self.path = self._find_path(ct, pos, self.target)
            self.path_target = self.target
            if self.path is None:
                # BFS couldn't reach the target within the node budget --
                # behind unmapped/walled terrain, or simply farther than the
                # cap reaches (e.g. a fresh random exploration target on an
                # early round, before much of the map is memoised). Fall
                # back to today's greedy walk rather than sit idle.
                self._move_greedy(ct, self.target)
                return

        if not self.path:
            return  # start == target already; nothing to walk this round

        next_pos = self.path[-1]
        # BFS only ever expands cardinal neighbours (see _find_path), so this
        # is always a legal single-tile cardinal step -- never diagonal.
        direction = pos.cardinal_direction_to(next_pos)
        if self._try_move(ct, direction):
            self.path.pop()
        else:
            # The next tile turned out to be occupied -- a builder bot,
            # harvester, barrier or turret. None of those are walls, so
            # they're deliberately not in wall_memo (see _update_wall_memo)
            # and BFS had no way to see this coming. Drop the stale path;
            # next round's recompute plans around whatever's there now.
            self.path = []

    def _move_greedy(self, ct: Controller, target: Position) -> None:
        """aug7's original step selection, verbatim: try the productive
        cardinal (random when both axes are productive), then the two
        perpendiculars in random order, then the reverse. Never diagonal.

        This is the fallback for whenever _find_path can't produce a path
        -- see _move_toward_target. It's also, not coincidentally, exactly
        what every builder did before this change, so it's a known-good
        safety net rather than new untested code.
        """
        pos = ct.get_position()
        dx = target.x - pos.x
        dy = target.y - pos.y
    ...
    def _update_wall_memo(self, ct: Controller) -> None:
        """Record wall/not-wall for every currently visible tile we haven't
        already memoised.

        Walls never change, so once we've actually seen a tile this is exact
        forever -- no need to ever re-query it, which is why _find_path can
        afford to run a search every round it needs to without touching the
        engine at all. Tiles never recorded default to "passable" wherever
        they're read (see _find_path) -- optimistic, but exact in the sense
        that this dict only ever contains *confirmed* walls, never guesses.

        Buildings deliberately don't go in here (see _find_path's docstring
        and _move_toward_target) -- only Environment.WALL, which is terrain,
        not occupancy.

        Runs every round regardless of move cooldown, so a cooldown round
        still grows what we remember for whenever we next get to plan.
        """
        for tile in ct.get_nearby_tiles():
            if tile not in self.wall_memo:
                self.wall_memo[tile] = ct.get_tile_env(tile) == Environment.WALL

    def _find_path(self, ct: Controller, start: Position, goal: Position) -> list[Position] | None:
        """Bounded BFS from start to goal over self.wall_memo only.

        Never queries the engine -- every check here is a dict lookup against
        our own memo, so cost is pure Python and independent of vision or map
        size, bounded by BFS_NODE_CAP tile expansions. Returns the path as
        [goal, ..., first_step] so the caller can consume it with .pop() in
        O(1) (the next step to take is always the last element), or None if
        goal isn't reached within the node budget.

        Buildings are never treated as obstacles here, only memoised walls --
        harvesters, barriers, turrets and Cores block movement too, but they
        appear and disappear during a match, so baking them into a plan would
        go stale. The actual step is validated for real by _try_move's
        can_move() check; if that fails, _move_toward_target drops the cached
        path and we replan next round. That division of labour is also why
        this function is safe to call every round the cache is invalid: it
        can never be more wrong than "the terrain memo", which is exact.

        Tiles outside the memo are treated as passable (optimistic) -- see
        _update_wall_memo -- so the search can still plan through ground we
        simply haven't looked at yet, which matters a lot early in a match or
        when a target is off in fog we haven't explored.
        """
        if start == goal:
            return []
        if self.wall_memo.get(goal, False):
            return None  # confirmed wall -- no path will ever reach it

        w, h = ct.get_map_width(), ct.get_map_height()
        frontier = deque([start])
        came_from: dict[Position, Position] = {}
        expanded = 0
        while frontier and expanded < BFS_NODE_CAP:
            current = frontier.popleft()
            expanded += 1

            # Shuffle expansion order every time. An unshuffled N/E/S/W scan
            # means every tie between equally-short routes resolves the same
            # absolute way for every unit on the team -- exactly the
            # direction-bias bug class that handed whole maps to one seat
            # before (see the module docstring / aug7's v3-v4 history).
            neighbours = [current.add(d) for d in CARDINALS]
            random.shuffle(neighbours)
            for nxt in neighbours:
                if nxt == start or nxt in came_from:
                    continue
                if not (0 <= nxt.x < w and 0 <= nxt.y < h):
                    continue
                if self.wall_memo.get(nxt, False):
                    continue
                ...
                if nxt == goal:
                    ...
                    return path
                frontier.append(nxt)
        return None  # exhausted the node budget without reaching goal
