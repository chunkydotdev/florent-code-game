...
class Player:
    ...
    def _core_dist_sq4(self, pos: Position) -> int:
        """4x the squared distance from pos to the Core's 2x2 footprint CENTRE.

        get_position() returns the footprint's NW corner, which is not a
        rotation-equivariant reference point: the centre sits half a tile SE of
        it, so using the corner shifts every core-relative gate NW by a fixed
        offset that does not rotate with the map. The centre lands on a half
        tile, so we work in doubled coordinates and compare against 4x the
        threshold; this keeps the arithmetic exact.
        """
        dx = 2 * pos.x - (2 * self.core_pos.x + 1)
        dy = 2 * pos.y - (2 * self.core_pos.y + 1)
        return dx * dx + dy * dy
    ...
    def _try_build_sentinel(self, ct: Controller) -> bool:
        ...
        if self.core_pos is None or self._core_dist_sq4(pos) > 18 * 4:
            return False
    ...
    def _pick_target(self, ct: Controller) -> Position:
        ...
        if harvester_count >= TARGET_HARVESTERS and self.core_pos is not None:
            if self._core_dist_sq4(pos) > 8 * 4:
