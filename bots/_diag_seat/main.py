...
# --- DIAGSEAT instrumentation ---------------------------------------------
# Measurement-only additions for the seat-bias investigation (2026-08-06).
# Every added line either (a) increments a local Python counter that no
# other logic reads, or (b) prints to stderr with the "DIAGSEAT" tag. Never
# writes a new store slot, never branches a game decision on a new counter.
# Grep stderr for "DIAGSEAT" and aggregate with a script rather than reading
# raw. Sampling cadence for periodic snapshots, in rounds:
DIAG_SNAPSHOT_EVERY = 50
# Facing classes for the row-major get_attackable_tiles() enumeration bug
# (game-model.md, measured 2026-08-08): N/NE/NW/W scan farthest-first,
# E/SE/S/SW scan nearest-first. Recorded per sentinel to test whether the
# two seats' sentinels systematically face different classes.
DIAG_NEAR_CLASS = (Direction.EAST, Direction.SOUTHEAST, Direction.SOUTH, Direction.SOUTHWEST)
...
class Player:
    def __init__(self):
        ...
        # --- DIAGSEAT counters (measurement only, never read by game logic) ---
        self.diag_conveyors_built = 0
        self.diag_try_move_calls = 0     # rounds where a move was attempted
        self.diag_try_move_fail = 0      # rounds where all 4 alternatives failed
        self.diag_pick_target_calls = 0
        self.diag_pick_target_ore = 0
        self.diag_pick_target_gohome = 0
        self.diag_pick_target_shared = 0
        self.diag_pick_target_random = 0
        self.diag_stuck_repicks = 0      # times self.stuck >= 3 forced a re-pick
        self.diag_harvesters_built = 0
        self.diag_first_harvester_round: int | None = None
        self.diag_sentinels_built = 0
        self.diag_last_snapshot_round = -1
    ...
    def _run_builder(self, ct: Controller) -> None:
        ...
        # DIAGSEAT: periodic cumulative snapshot, sampled so volume stays
        # manageable. Read-only — no game action taken here.
        round_now = ct.get_current_round()
        if round_now - self.diag_last_snapshot_round >= DIAG_SNAPSHOT_EVERY:
            self.diag_last_snapshot_round = round_now
            print(
                f"DIAGSEAT SNAPSHOT team={ct.get_team().name} unit={ct.get_id()} round={round_now} "
                f"pos=({pos.x},{pos.y}) stuck={self.stuck} "
                f"conv_built={self.diag_conveyors_built} "
                f"move_calls={self.diag_try_move_calls} move_fail={self.diag_try_move_fail} "
                f"pick_calls={self.diag_pick_target_calls} pick_ore={self.diag_pick_target_ore} "
                f"pick_gohome={self.diag_pick_target_gohome} pick_shared={self.diag_pick_target_shared} "
                f"pick_random={self.diag_pick_target_random} stuck_repicks={self.diag_stuck_repicks} "
                f"harv_built={self.diag_harvesters_built} first_harv_round={self.diag_first_harvester_round} "
                f"sent_built={self.diag_sentinels_built}",
                file=sys.stderr,
            )
    ...
    def _try_build_harvester(self, ct: Controller) -> bool:
        ...
        for d in dirs:
            ...
            if ct.can_build_harvester(build_pos):
                ...
                # DIAGSEAT: log every harvester build, and flag this unit's
                # first one (the "time to reach first ore" metric).
                self.diag_harvesters_built += 1
                round_now = ct.get_current_round()
                is_first = self.diag_first_harvester_round is None
                if is_first:
                    self.diag_first_harvester_round = round_now
                print(
                    f"DIAGSEAT HARVESTER_BUILT team={ct.get_team().name} unit={ct.get_id()} "
                    f"round={round_now} pos=({build_pos.x},{build_pos.y}) "
                    f"seq={self.diag_harvesters_built} is_first={is_first}",
                    file=sys.stderr,
                )
                return True
        return False
    ...
    def _try_build_sentinel(self, ct: Controller) -> bool:
        ...
        for d in dirs:
            ...
            if ct.can_build_sentinel(build_pos, facing):
                ...
                # DIAGSEAT: log the round, position and facing class. Facing
                # class tests whether the row-major get_attackable_tiles()
                # enumeration bug (near-first for E/SE/S/SW, far-first for
                # N/NE/NW/W -- game-model.md) systematically differs by seat.
                self.diag_sentinels_built += 1
                near_class = facing in DIAG_NEAR_CLASS
                print(
                    f"DIAGSEAT SENTINEL_BUILT team={ct.get_team().name} unit={ct.get_id()} "
                    f"round={ct.get_current_round()} pos=({build_pos.x},{build_pos.y}) "
                    f"facing={facing.name} near_class={near_class}",
                    file=sys.stderr,
                )
                return True
        return False
    ...
    def _move_toward_target(self, ct: Controller) -> None:
        ...
        if self.target is None or pos == self.target or self.stuck >= 3:
            if self.stuck >= 3:  # DIAGSEAT: isolate the stuck-specific re-pick
                self.diag_stuck_repicks += 1
        ...
        # DIAGSEAT: preserves original semantics exactly (first success wins,
        # no further directions tried) -- only adds counting around the loop.
        moved = False
        for d in alternatives:
            if self._try_move(ct, d):
                moved = True
                break
        self.diag_try_move_calls += 1
        if not moved:
            self.diag_try_move_fail += 1
    ...
    def _try_move(self, ct: Controller, d: Direction) -> bool:
        ...
        if ct.is_tile_empty(next_pos) and self.core_pos is not None:
            ...
            if ct.can_build_conveyor(next_pos, cardinal):
                ...
                self.diag_conveyors_built += 1  # DIAGSEAT
    ...
    def _pick_target(self, ct: Controller) -> Position:
        ...
        """
        pos = ct.get_position()
        self.diag_pick_target_calls += 1  # DIAGSEAT
        ...
        if best_ore:
            self.diag_pick_target_ore += 1  # DIAGSEAT
        ...
        if harvester_count >= TARGET_HARVESTERS and self.core_pos is not None:
            if pos.distance_squared(self.core_pos) > 8:
                self.diag_pick_target_gohome += 1  # DIAGSEAT
        ...
        if shared is not None and pos.distance_squared(shared) > 4:
            self.diag_pick_target_shared += 1  # DIAGSEAT
            return shared

        # 4. No known ore — pick a random position to explore
        self.diag_pick_target_random += 1  # DIAGSEAT
    ...
    def _run_sentinel(self, ct: Controller) -> None:
        ...
        chosen_tile = None
        # DIAGSEAT: collect every enemy-occupied tile on the ray, read-only,
        # to test in practice whether the row-major get_attackable_tiles()
        # order (game-model.md) picks something other than the geometrically
        # nearest target. The engine call sequence and the fire decision
        # below are unchanged from the original -- this only adds queries
        # for diagnostics after the first hit, which the original never
        # reached because it returned immediately.
        enemy_dists = []
        for tile in ct.get_attackable_tiles():
            ...
            if ct.get_team(target_id) == my_team:
                continue
            enemy_dists.append(my_pos.distance_squared(tile))
            if chosen_tile is None:
                chosen_tile = tile

        if enemy_dists:
            chosen_dist = my_pos.distance_squared(chosen_tile)
            nearest_dist = min(enemy_dists)
            print(
                f"DIAGSEAT SENTINEL_FIRE team={my_team.name} unit={ct.get_id()} "
                f"round={ct.get_current_round()} n_enemies_on_ray={len(enemy_dists)} "
                f"chosen_dist_sq={chosen_dist} nearest_dist_sq={nearest_dist} "
                f"suboptimal={chosen_dist > nearest_dist}",
                file=sys.stderr,
            )

        if chosen_tile is not None and ct.can_fire(chosen_tile):
            ct.fire(chosen_tile)
