...
class Player:
    ...
    def _try_build_conveyor_toward_core(self, ct: Controller, harvester_pos: Position) -> None:
        ...
        import sys
        _ok = in_bounds(ct, conv_pos) and ct.can_build_conveyor(conv_pos, facing)
        _mydist = ct.get_position().distance_squared(conv_pos)
        print(f"CONVPROBE ok={_ok} builder={tuple(ct.get_position())} harv={tuple(harvester_pos)} conv={tuple(conv_pos)} dist_sq={_mydist}", file=sys.stderr)
        if _ok:
            ct.build_conveyor(conv_pos, facing)
            print("CONVPROBE BUILT", file=sys.stderr)
