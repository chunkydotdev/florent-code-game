...
class Player(EcoMixin, RaidMixin):
    ...
    def run(self, ct):
        import sys as _s
        try:
            _r = ct.get_current_round(); _i = ct.get_id()
            _k = str(ct.get_entity_type()).split(".")[-1]
            print("CPUSTART %d %d %s" % (_r, _i, _k), file=_s.stderr)
        except Exception:
            _r = _i = -1; _k = "?"
        try:
            _ret = self._probe_inner(ct)
            try:
                print("CPUEND %d %d %s %d" % (_r, _i, _k, ct.get_cpu_time_elapsed()), file=_s.stderr)
            except Exception:
                pass
            return _ret
        except Exception:
            try:
                print("CPURAISE %d %d %s" % (_r, _i, _k), file=_s.stderr)
            except Exception:
                pass
            raise

    def _probe_inner(self, ct):
