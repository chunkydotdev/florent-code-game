...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 7. LOKI-SALT (v178).  Cut their belt, then salt the tile.  LAST in
        # the ranking on purpose: the Sentinel (3) and the seat seal (2) both
        # outrank it, so this can only spend a round the parent spent idle.
        if LOKI_SALT_ON and self._salt_turn(ct, E, p, ti):
            return True
        return False
