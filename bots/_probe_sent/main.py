...
class Player:
    ...
    def _run_sentinel(self, ct: Controller) -> None:
        ...
        PROBE (_probe_sent, 2026-08-06): measures how often >=2 enemy tiles
        sit on the ray at once, and how the current first-hit choice differs
        from picking the nearest one instead. The probe call below is
        read-only (never touches ct.fire/can_fire/write_store/etc.) and is
        wrapped in its own try/except so it cannot change the firing
        decision below or escape into run()'s outer exception guard -- that
        guard would otherwise cost this unit its whole turn for the round,
        which is exactly the behaviour-change this measurement must avoid.
        get_attackable_tiles() is called once and the same list is reused
        for both the probe and the firing loop, so this is not two live
        queries racing -- it's one snapshot read twice.
        """
        my_team = ct.get_team()
        tiles = ct.get_attackable_tiles()

        try:
            self._probe_sentinel_targets(ct, my_team, tiles)
        except Exception:
            pass
    ...
    def _probe_sentinel_targets(self, ct: Controller, my_team, tiles) -> None:
        """PROBE ONLY -- not part of the strategy, safe to delete wholesale.

        Called from _run_sentinel under its own try/except; must never raise
        and must never call any state-changing Controller method (fire,
        can_fire, rotate, write_store, ...). Purely reads and prints.

        Tags (stderr, grep-able):
          SENTPROBE_BUILT -- once per sentinel, on its first active round.
                             id, round. Denominator: sentinels built/match.
          SENTPROBE       -- once per round this sentinel has >=1 enemy
                             tile on its raw attack ray. id, round, active
                             (this sentinel's Nth active round -- running
                             counter, denominator: sentinel firing rounds),
                             count (enemy tiles on the ray), facing (if
                             obtainable), first_hit_sq (squared distance to
                             the tile the unmodified loop above will target),
                             nearest_sq (squared distance to the nearest of
                             the enemy tiles).
        """
        import sys

        active = getattr(self, "_sentinel_probe_active_rounds", 0) + 1
        self._sentinel_probe_active_rounds = active
        if active == 1:
            print(
                f"SENTPROBE_BUILT id={ct.get_id()} round={ct.get_current_round()}",
                file=sys.stderr,
            )

        # Same detection order/team check as the real loop, but collecting
        # every enemy tile on the ray instead of stopping at the first.
        enemy_tiles = []
        for tile in tiles:
            target_id = ct.get_tile_builder_bot_id(tile)
            if target_id is None:
        ...
        if len(enemy_tiles) < 1:
            return

        my_pos = ct.get_position()
        # enemy_tiles[0] is, by construction (same scan order/filter as the
        # firing loop), exactly the tile the current first-hit logic picks.
        first_hit_sq = my_pos.distance_squared(enemy_tiles[0])
        nearest_sq = min(my_pos.distance_squared(t) for t in enemy_tiles)

        facing_field = ""
        try:
            facing_field = f" facing={ct.get_direction().name}"
        except Exception:
            pass  # omit rather than guess, per instructions

        print(
            f"SENTPROBE id={ct.get_id()} round={ct.get_current_round()} "
            f"active={active} count={len(enemy_tiles)}{facing_field} "
            f"first_hit_sq={first_hit_sq} nearest_sq={nearest_sq}",
            file=sys.stderr,
        )
