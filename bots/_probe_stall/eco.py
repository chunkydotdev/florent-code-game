...
class EcoMixin:
    ...
    def _stallprobe_pop(self, ct, tile):
        """STALLPROBE instrumentation only -- stderr, no behaviour change.

        Reports who owns the building occupying `tile` right before it is
        popped off link_queue as 'already built'.  Wrapped by the caller in
        try/except so any failure here cannot alter game behaviour or
        propagate to destroy the unit.
        """
        import sys
        owner = "UNKNOWN"
        etype = "?"
        ...
        if bid is not None:
            try:
                owner = "OURS" if ct.get_team(bid) == ct.get_team() else "ENEMY"
            except Exception:
                owner = "UNKNOWN"
            try:
                etype = ct.get_entity_type(bid).name
            except Exception:
                etype = "?"
        else:
            owner = "EMPTY"
        print(
            f"STALLPROBE POP r={ct.get_current_round()} unit={ct.get_id()} "
            f"tile=({tile.x},{tile.y}) owner={owner} etype={etype} "
            f"qlen_before={len(self.link_queue)}",
            file=sys.stderr,
        )
        if owner == "ENEMY" and not getattr(self, "_stallprobe_enemypop_seen", False):
            self._stallprobe_enemypop_seen = True
            print(
                f"STALLPROBE ENEMYPOP r={ct.get_current_round()} unit={ct.get_id()} "
                f"tile=({tile.x},{tile.y}) etype={etype}",
                file=sys.stderr,
            )

    def _stallprobe_round(self, ct, p):
        """STALLPROBE instrumentation only -- stderr, no behaviour change.

        One line per round for this builder: queue length, head tile, and
        whether the head tile is occupied (and by whom).  Called from
        main.py's _builder(), itself wrapped in try/except there.
        """
        import sys
        head = self.link_queue[0] if self.link_queue else None
        occ = "n/a"
        if head is not None:
            bid = None
            try:
                bid = ct.get_tile_building_id(head)
            except Exception:
                occ = "ERR"
            if occ != "ERR":
                if bid is None:
                    ...
                    try:
                        occ = "OURS" if ct.get_team(bid) == ct.get_team() else "ENEMY"
                    except Exception:
                        occ = "UNKNOWN"
        head_str = "None" if head is None else f"({head.x},{head.y})"
        print(
            f"STALLPROBE ROUND r={ct.get_current_round()} unit={ct.get_id()} "
            f"role={self.role} pos=({p.x},{p.y}) qlen={len(self.link_queue)} "
            f"head={head_str} occ={occ}",
            file=sys.stderr,
        )
