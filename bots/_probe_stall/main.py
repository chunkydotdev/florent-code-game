...
class Player(EcoMixin, RaidMixin):
    ...
    def _builder(self, ct):
        ...
        # STALLPROBE: per-round instrumentation, stderr only, no behaviour
        # change.  Restricted to roles that ever touch link_queue (expand,
        # and defend which falls back to _expand) to keep volume small; the
        # raid majority never builds trunk links so is uninformative here.
        if self.role in ("expand", "defend"):
            try:
                self._stallprobe_round(ct, p)
            except Exception:
                pass
