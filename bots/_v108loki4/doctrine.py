...
# ============================================================================
# LOKI-4 -- ORE DENIAL  (new in _v108loki4; logic lives in denial.py)
# ============================================================================
#
# THE MECHANIC, engine-probed 2026-08-09 with bots/_probe_denial on fjordgate:
#
...
#
# Harvesters may ONLY be built on ore, so a 3 Ti barrier permanently removes a
# 24 Ti harvester site.  The exchange is lopsided because ct.destroy() is
# ALLIED-ONLY: the victim cannot remove our barrier cheaply, they must chew
# 30 HP at 2 dmg / 2 Ti a swing -- 15 attacks, 30 Ti and 15 builder-turns to
# clear 3 Ti and 1 turn of ours.  ~10:1 in titanium, ~15:1 in builder-turns.
# And the denial is REVERSIBLE FOR US ONLY: our own builder standing beside it
# destroys it for free (no cooldown, no titanium) and the site comes back.
#
# WHAT THE CENSUS SAYS, AND IT NEARLY KILLS THE DOCTRINE
# ------------------------------------------------------
# Pool-map ore census (15 maps, parsed from maps/*.map26, 2026-08-09):
# 314 ore tiles, mean 20.9 per map, mean 9.4 per SIDE.  Replay census of 250
# archived games / 3,073 harvester builds: a team consumes a MEDIAN OF 4
# DISTINCT ORE SITES (p75 7, p90 13) out of the ~9.4 on its own side.
#
#   ==> On the median map, denying 2-3 sites denies nothing: they had five
#       spares.  Denial only binds where SITES ARE SCARCE.
#
# Enemy-side ore per map (the exact quantity DENY_MAX_ENEMY_ORE gates on):
...
# The gap between meander (7) and nordkap (10) is the only clean break, and it
# splits the pool 8/15 deniable, 7/15 noise.  DENY_MAX_ENEMY_ORE sits in it.
DENY_MAX_ENEMY_ORE = 7
#
# TIMING, and why ranks 1-2 are hopeless.  Median round at which the Nth-
# nearest ore to a core is claimed by a harvester (same 250-replay census,
...
# is wasted builder-turns.  can_build_barrier() would refuse an occupied tile
# anyway -- this only stops us WALKING to a lost cause.  Values are the census
# medians above, rounded up to the next 10.
DENY_RANK_DEADLINE = (0, 10, 20, 30, 30, 50, 60, 90, 120)
# Hard stop.  Nothing is planted after this round: the corpus read (3,791
# replays, 11,895 throws) puts median raider life after r150 at 6 rounds, and
# by then only rank 9+ ore is unclaimed, which they use 3% of the time.
DENY_MAX_RND = 150
#
...
# every cost = floor(scale * base)).  A barrier is +1%.  Four denial barriers
# = +4 points on the price of every harvester, gunner, sentinel and builder we
# buy while they stand -- about +0.8 Ti on a 20-base harvester.  Small, but not
# zero, and it is the reason this is capped rather than opportunistic-unbounded.
DENY_MAX_BARRIERS = 4
#
# COST 2: BUILDER-TURNS, which is the real currency (thor_r1 shipped zero
# harvesters, delivered zero titanium, went 2/60).  This plank buys ZERO
# DEDICATED TRIPS.  Only units already walking into the enemy half place
# barriers -- the role_n == 0 saboteur and Launcher-dropped raiders -- and only
# for an ore tile they are already orthogonally adjacent to, or one reachable
# by a detour of at most DENY_DETOUR_DSQ (d^2 <= 8, i.e. two steps) that is
# taken only while no siege approach has been planned yet.  Total cost is
# bounded at DENY_MAX_BARRIERS * (2 detour steps + 1 build turn) = 12
# builder-turns per unit, spent by the one unit whose measured contribution to
# core kills after r150 is ~zero.  The economy is at field parity (452.9 vs
# 444.0 stacks delivered) and no expander turn is touched.
DENY_DETOUR_DSQ = 8
# Titanium floor.  A barrier is 3 Ti, but the opening bank also has to buy
# five builders and the first harvesters; never trade a harvester for a
# barrier.  Same shape as MEDIC_TI_FLOOR.
DENY_TI_FLOOR = 60
# A barrier BLOCKS MOVEMENT and LOS.  Planting one in a corridor pinch can wall
# our own saboteur out of the enemy half.  Require the ore tile to have at
# least this many non-wall cardinal neighbours in the decoded grid.
DENY_MIN_OPEN_NBRS = 3
#
# WHAT IS DELIBERATELY NOT TARGETED
# ---------------------------------
# MIDLINE / CONTESTED ore (tiles equidistant from both Cores) is excluded.
# The census puts almost all of it on two maps -- meander 10 of 24, heart 6 of
# 28 -- and both of those maps carry 7 and 11 own-side ore respectively, so a
# team denied a midline tile simply takes a spare.  We would pay a builder-turn
# and a permanent +1% to deny a site they never needed, and lose the site
# ourselves in the bargain.  Ore is symmetric: a midline denial is a mutual
# denial, and we have no measured edge that makes a mutual trade good for us.
#
# THE MASTER FLAG.  This is the ablation unit: OFF reproduces _v103split
# exactly (denial.py is imported but every entry point returns immediately).
ORE_DENIAL_ON = True
#
# LANE B -- HOME-SIDE DENIAL, OFF BY DEFAULT, NOT MEASURED.
# 448 of 3,073 harvesters in the census (14.6%) were built on the OPPONENT's
# side of the midline, at median round r189 (only 9.2% by r50).  So the field
# does come and take our ore, but late.  Barriering our own far ore from
# inside our own half is far cheaper in builder-turns than the forward trip,
# and it is reversible for us (destroy() is free) and not for them.
#
# It is OFF because it has a concrete backfire path that Lane A does not:
# _pick() only skips an ore tile it can SEE is occupied, so our own barriers
# on out-of-vision ore stay live expansion targets and send our expanders on
# wasted walks -- we would be paying builder-turns from the economy arm, which
# is the exact asset this build is supposed to protect.  Left in place, wired,
# and defaulted off so a follow-up leg can measure it as its own arm without
# another build cycle.
DENY_HOME_ON = False
# Only ore beyond this rank in OUR OWN distance ladder is eligible -- the tail
# we would only ever reach past r150 anyway (census: we consume a median of 4
# sites; rank 9+ is claimed at r100+ and 3% of the time).
DENY_HOME_MIN_RANK = 9
DENY_HOME_MAX_BARRIERS = 3
DENY_HOME_MIN_RND = 40
#
