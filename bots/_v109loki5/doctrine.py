...
# ============================================================================
# PIECE KIDNAP (LOKI-5) -- invert the launcher's enemy-bot exile.
#
# WHAT THE PARENT DOES, AND WHY IT IS BACKWARDS.  _launcher's enemy branch
# sorts the legal landing tiles by ``distance_squared(our core)`` DESCENDING and
# throws the captured hostile to the first one that takes.  Inside a throw
# radius of sqrt(26) the tile farthest from OUR core is, on every symmetric map
# in the pool, the tile nearest THEIRS -- so we catch a raider that walked to us
# and pay its fare home.  Replay census: 1,909 such throws against a single
# archived opponent, ~22 per game, every one of them a free relocation for the
# victim in the direction it wanted to go.
#
# WHY THE LAUNCHER SPECIFICALLY.  Per-titanium conversion rates, straight from
# the spec: builder heal 4.00 HP/Ti, sentinel shot 1.80, gunner shot 1.75,
# builder attack 1.00.  Healing beats every damage source by 2.22x, so a
# titanium-symmetric attrition race is unwinnable by the attacker; and a builder
# bot's attack damages BUILDINGS ONLY, so the only two ways to remove an enemy
# HEALER are a turret shot or a launcher throw.  The launcher costs no
# ammunition.  It is the only free counter to the best conversion in the game.
# That is why the victim-selection rule below ranks probable healers first.
#
# THE RISK THIS TRADES AGAINST, stated plainly: a hostile thrown deeper into our
# own half is an IMPORTED SABOTEUR unless it dies or is contained.  The rule
# therefore vetoes any landing tile that is both uncovered and inside our home
# band, and falls back to the parent's exile sort whenever nothing scores.
#
...
# tempo term for an ordinary raider (the brief's ordering), while for a probable
# healer KIDNAP_W_SEP_HEAL * a typical 6-7 tile separation lands at ~90-100 and
# therefore competes with coverage on equal terms (the coordinator's ordering).
KIDNAP_W_KILLZONE = 100
KIDNAP_W_WALKBACK = 10        # per tile of extra Chebyshev walk to THEIR core
KIDNAP_W_SEP = 4              # per tile of separation from where it was caught
KIDNAP_W_SEP_HEAL = 14        # ...for a probable healer: separation IS the kill
KIDNAP_WALK_CLAMP = 6
KIDNAP_SEP_CLAMP = 7
# Per adjacent friendly non-barrier building on the landing tile's 8-ring,
# capped at KIDNAP_NEAR_CAP of them.  A hostile that lands next to our conveyor
# line can start attacking it the turn it lands (2 Ti -> 2 dmg, no cooldown
# beyond its own), so proximity to our economy is a real cost even on a covered
# tile.
KIDNAP_W_NEAR_OURS = 30
KIDNAP_NEAR_CAP = 2

# HARD VETO: never leave a hostile uncovered inside our own home band.  20 is
# the same radius the chassis already calls "at our Core" (INTRUDER_CORE_DSQ)
# and the band our home turrets measurably sit on (median d^2 20-22), so a tile
# inside it that NO turret line covers is the exact importing-a-saboteur case.
KIDNAP_HOME_VETO_DSQ = 20

# A turret line only counts as coverage if the team can actually pay for a shot
# (gunner 4 ammo, sentinel 10).  Coverage with an empty bank is decoration.
KIDNAP_MIN_AMMO_GUNNER = 4
KIDNAP_MIN_AMMO_SENTINEL = 10

# Victim-selection bonuses, applied when more than one hostile sits in the
# pickup ring (d^2 <= 2, the full 8-neighbourhood -- wild-measured 1,471/1,472
# throw events, game-model.md).  A "probable healer" is a hostile orthogonally
# adjacent to a DAMAGED entity of its own team: heal requires exactly that
# adjacency, and hp < max_hp is the only observable that says the heal has
# somewhere to go.
KIDNAP_V_HEALER = 100
KIDNAP_V_HEALER_HIGHVALUE = 40   # ...and the damaged thing is their core/turret
