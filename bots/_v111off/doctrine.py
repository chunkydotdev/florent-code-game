...
# ============================================================================
# PLANK "HOME" (s23).  Three defects from docs/research/machinery-audit-
# 2026-08-09.md, all sitting in the HOME BAND -- the one band today's corpus
# work says we are large and consistently better than the field in
# (fixed-horizon 50-round turret survival: +11.4 / +16.6 / +22.3pp over the
# field at r0-150 / r150-300 / r300+).  The audit's own recommended sequencing
# is "4 -> 3 -> exile is ONE leg with three ablatable flags"; these are flags
# 4 and 3, built together because fixing what the guns shoot without fixing
# what they can afford just wastes shots.
#
# WHY A COMPOSITE AND NOT THREE LEGS: builder-method.md 5.  LOKI-3 measured
# HEALER_FOCUS alone at an EXACT null (0.17, byte-identical to parent) and the
# same five flags together at 2.82.  Targeting is the LAST link in a chain
# (turret -> ammunition -> targeting); measured alone it reports the absence of
# its prerequisite.
#
# SCOPE NOTE, because a successor will otherwise read LOKI-3 as closing this:
# LOKI-3's healer-focus null was measured in the r200-300 FORWARD band, where
# we had ~0 turrets alive for it to aim.  This plank fires in the HOME band
# under siege, where we demonstrably do have turrets.  Different population,
# so "refuted alone" does not transfer.

# --- flag 4: the magazine is priced in GUNNER shots -------------------------
# main.py:620 sizes the magazine as min(48, 4 * weapons).  The 4 is one GUNNER
# shot and the comment says so.  MEASURED against the replay corpus (1,225
# joined files, our team only), our HOME gun mix is:
#     gunner 1,133 (34.1%)   sentinel 2,191 (65.9%)   blended 8.0 ammo/shot
# The field's home mix is the mirror image -- 89.0% gunner, 4.7 blended -- so
# the constant is right for THEM and wrong for US.  doctrine.py:1025 already
# contains this exact criticism, written about the disabled SPORKS block and
# never applied to the live one.
#
# 8 is the measured blended cost, not a guess.  The cap moves 48 -> 96 to keep
# the original comment's intent ("a dozen shots of reserve") true at the real
# price rather than at half of it.
#
# HONEST RISK, stated before the numbers exist: a fixed ammo floor has been
# refuted TWICE here (45.3%, 46.1%) and both times the failure mode was raising
# the target with ZERO turrets alive. This raise is multiplied by `weapons`, so
# it is exactly 0 when we have no guns -- which is the case both refutations
# actually measured. If it still loses, the lesson is that ammunition is not
# the binding constraint and the magazine should go back to 4.
MAG_PRICE_ON = False
MAG_SHOT_PRICE = 8
MAG_SHOT_CAP = 96

# --- flag 3: the anti-healer target table is map-locked ---------------------
# main.py:3635 enables builder-first targeting only on a 26x26 map whose enemy
# anchor is (5,5) -- snowflake seat B, one seat of one map.  Everywhere else our
# primary weapon shoots enemy TURRETS before enemy BUILDERS, and the audit's
# chain says only a turret or a launcher can remove a healer at all (builders
# cannot attack builders).  heal is 4.00 HP/Ti against best damage 1.80, so a
# healing screen we never shoot at wins attrition 2.2:1 by default.
#
# Generalised to: under siege, in our own home band. NOT made unconditional --
# shooting builders before turrets is wrong when nothing is threatening us, and
# the existing snowflake clause is preserved so that seat's measured behaviour
# is bit-identical to today.
HEALER_FOCUS_HOME_ON = False
HEALER_FOCUS_HOME_DSQ = 41          # HUNT_BAND_DSQ, this file's twice-validated home band

# --- flag 3b: first-wins tie-break resolved by our own FACING ---------------
# `if prio < best_prio` is a strict first-wins scan over get_attackable_tiles(),
# whose enumeration docs/game-model.md:283-290 measured as ABSOLUTELY ORIENTED:
# N/NE/NW/W turrets engage the farthest enemy on the line, E/SE/S/SW the
# nearest. So which of two equal-priority enemies we shoot is decided by which
# way the turret happens to point. Documented-and-unfixed in the shipped file.
# The fix is a (prio, dsq) tuple compare -- nearest first within a priority.
TARGET_TIEBREAK_ON = False
