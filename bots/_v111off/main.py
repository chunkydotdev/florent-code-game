...
class Player:
    ...
    def _core(self, ct):
        ...
        if sporks_ammo:
            ...
            if weapons:
                if MAG_PRICE_ON:
                    ammo_target = max(ammo_target,
                                      min(MAG_SHOT_CAP, MAG_SHOT_PRICE * weapons))
                else:
    ...
    def _turret(self, ct):
        ...
        # PLANK HOME flag 3.  The clause above is snowflake seat B and stays
        # bit-identical; this adds the general case the audit found missing --
        # under siege, on our own doorstep, shoot the healers first.
        if HEALER_FOCUS_HOME_ON and not healer_focus and self.core is not None:
            try:
                healer_focus = (
                    ct.read_store(SLOT_UNDER) != 0
                    and p.distance_squared(self.core) <= HEALER_FOCUS_HOME_DSQ
                )
            except Exception:
                pass
        ...
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                if TARGET_TIEBREAK_ON:
                    # Nearest first WITHIN a priority; first-wins otherwise
                    # resolves ties by our own facing (game-model.md:283-290).
                    key = (prio, t.distance_squared(p))
