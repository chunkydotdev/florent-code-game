...
def launcher_live(ct):
    """True when a friendly Launcher was observed within LAUNCHER_STALE_RNDS.

    LOKI-0b.  SLOT_LAUNCHER now stores `round + 1` (never 0 for a live
    launcher, so every existing truthiness read still behaves) instead of a
    bare 1.  A Launcher rewrites the slot every round it runs, so the value
    going stale IS the death signal.  Reads that mean "do we have a Launcher"
    must use this helper; reads that mean "has one ever been claimed" must not.
    """
    v = ct.read_store(SLOT_LAUNCHER)
    if not v:
        return False
    return ct.get_current_round() - (v - 1) <= LAUNCHER_STALE_RNDS
...
class Player:
    ...
    def _live_builders(self, ct, units, weapons):
        ...
        by_census = units - 1 - weapons - (1 if launcher_live(ct) else 0)
    ...
    def _builder(self, ct):
        ...
        if self.role == "launchwait":
            if self.dropped:
                self.role = "saboteur"
            elif rnd >= 70 and not launcher_live(ct) and self.role_n != 5:
                self.role = "saboteur"
        ...
        if (
            ...
            and rnd < LAUNCH_GIVEUP_RND and launcher_live(ct)
            and ct.read_store(SLOT_DROPPED) < RAID_DROP_CAP
    ...
    def _live_home_gun_count(self, ct):
        """How many friendly turrets stand in the home band RIGHT NOW.

        Counting sibling of _live_home_gun, same band constant and same
        vision-bounded caveat: get_nearby_buildings only returns what THIS unit
        sees, so an undercount from a caller far from home is the conservative
        answer (it builds more, and every build is still gated on titanium).
        """
        if self.core is None:
            return 0
        tiles = core_tiles(self.core)
    ...
    def _try_home_floor(self, ct):
        """PLANK FLOOR -- build a home Sentinel because we are SHORT, not because
        something is shooting at us.  Runs only after _try_counterbattery has
        already declined, so the reactive path is untouched."""
        import sys as _s
        if not HOME_FLOOR_ON or self.core is None:
            return False
        rnd = ct.get_current_round()
        if rnd < HOME_FLOOR_MIN_RND:
            return False
        print(f"FLOOR reached r{rnd}", file=_s.stderr)
        # Opening and its cost-scale profile stay untouched: no floor until the
        # economy this pays for actually exists.
        if ct.read_store(SLOT_HARVESTERS) < ECO_NEED:
            print(f"FLOOR blocked: harv {ct.read_store(SLOT_HARVESTERS)}<{ECO_NEED}", file=_s.stderr)
            return False
        want = min(HOME_FLOOR_MAX, 1 + (rnd - HOME_FLOOR_MIN_RND) // HOME_FLOOR_EVERY)
        _lc = self._live_home_gun_count(ct)
        if _lc >= want:
            print(f"FLOOR blocked: live {_lc}>=want {want}", file=_s.stderr)
            return False
        cost = ct.get_sentinel_cost()
        if ct.get_global_resources() < cost + HOME_FLOOR_RESERVE_TI:
            print(f"FLOOR blocked: ti {ct.get_global_resources()}<{cost+HOME_FLOOR_RESERVE_TI}", file=_s.stderr)
            return False
        print(f"FLOOR SCANNING r{rnd} want{want} live{_lc}", file=_s.stderr)
        p = ct.get_position()
        tiles = core_tiles(self.core)
        ban = self._seat_ban()
        ...
        for d in CARDINALS:
            ...
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            # A turret is impassable; never take a reserved heal/delivery seat.
            if ban is not None and (bp.x, bp.y) in ban:
                continue
            if min(t.distance_squared(bp) for t in tiles) > HUNT_BAND_DSQ:
                continue
            facing = bp.direction_to(enemy) if enemy is not None else Direction.NORTH
    ...
    def _defend(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            # PLANK FLOOR.  THE HOOK PLACEMENT IS THE WHOLE POINT.  The
            # block above is wrapped in `if under:` (main.py:2412), so every
            # home-turret build the chassis owns is gated on the SIEGE LATCH --
            # on top of the `threat is None` gate and the `aligned` gate inside
            # _try_counterbattery. Three reactive gates, and a floor placed
            # inside any of them is not proactive. Measured: wired inside the
            # under-block, the floor was reached only between r62 and r244 of a
            # 1000-round game and its target never even climbed past 2.
            # Here it sits OUTSIDE the latch, so it runs whenever the defender
            # has a free action, which is what "proactive" has to mean.
            if not defended and self._try_home_floor(ct):
                defended = True
            if not defended:
