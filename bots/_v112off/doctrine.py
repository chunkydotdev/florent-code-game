...
# ============================================================================
# PLANK "FLOOR" (s23) -- A PROACTIVE HOME-TURRET PRODUCTION PATH.
#
# THE DIAGNOSIS THIS IMPLEMENTS (coordination.md 09:57, verified in source):
# home turrets have exactly ONE build path, _try_counterbattery, and its first
# live statement is `if threat is None: return False`.  So the bot can only
# build a home gun as a REFLEX to a published threat.  Nothing in 5,100 lines
# builds a turret because turrets are good.
#
# (The HUNT_BAND_DSQ reach test at main.py:2250 is NOT the binding gate in v90 --
# it is wrapped in `if B8_ON` and B8_ON = False.  The binding gate is the
# unconditional `threat is None` test above it.  Checked, because the research
# arm's read quoted the band test as live and it is not.)
#
# THE MEASUREMENTS:
#   live turret count   US median 2 from r200 and NEVER MOVES  |  field 2->3->4->5->5
#   turret survival     US 50.6%                               |  field 50.8%   (attrition FINE)
#   shots per turret    US 21.85                               |  field 24.13   (productivity FINE)
#   first turret round  US r12                                 |  field r17     (we START EARLIER)
# We start earlier, our turrets last as long and work as hard, and we build 4.48
# per game against 10.72.
#
# AND THE WARNING LABEL ON THAT LIST, which matters more than the list.  These
# are NOT four independent votes.  They are four VIEWS OF ONE NUMBER -- we build
# ~4.5 turrets and the field builds ~10.7.  Survival-identical, shots-identical,
# count-flatlining and magazine-inert are all restatements of that single fact,
# so "four instruments agree" should carry the weight of ONE well-measured fact.
# And that fact is DESCRIPTIVE: it says what we do, not that doing more wins.
# (Written at the research arm's insistence, onto a sentence I had already
# overstated in this same comment block an hour earlier.)
#
# THE REACH I AM PRE-BANNING, because it is predictable and it has a name: if
# this reads null, the tempting move is "the mechanism fired but the composite
# masked it" -- which is exactly the LOKI-3 rescue, and s22 already established
# that a perfect mechanism result predicts nothing about wins.  A null on
# production PLUS a null on placement PLUS a null on price is not three unlucky
# experiments.  It is evidence that turrets are not our lever, and I will say so.
#
# WHAT THIS PLANK DOES: adds a floor -- "by this round, have this many live guns
# in the home band, threat or no threat" -- as a STRICT FALLBACK that runs only
# after _try_counterbattery has already declined.  The reactive path is
# byte-untouched, so with HOME_FLOOR_ON = False this build is the parent exactly.
#
# TYPE IS SENTINEL, AND IT IS A DELIBERATE CHOICE, NOT A DEFAULT.  Probed by the
# research arm this session: a gunner's line is BLOCKED by friendly builder bots
# and friendly buildings; a sentinel's line passes through both (-18 HP through
# two friendly entities, full SENTINEL_DAMAGE).  We run ~59 conveyors per game
# against the field's ~34, so our home is the cluttered case, where a gunner is
# the wrong instrument and is also the one our get_attackable_tiles siting would
# mis-score hardest.  Our own mix is already sentinel-leaning (g:s 0.89 vs the
# field's 4.88) -- this keeps that, on purpose.
#
# THE HONEST PRIOR, stated before the battery: LOKI-3's late home arm moved
# r200-300 turrets 0.00 -> 2.67 and its mechanism metric only 0.17 -> 0.32
# against a field reference of 2.79, inside a composite that returned +0.0pp on
# n=360.  That is the closest existing measurement and it is NOT encouraging.
# The case for trying anyway: it was gated to r200+ on surplus only, while the
# field's separation is already 1.6x by r100.
HOME_FLOOR_ON = False
# No floor before the economy exists. ECO_NEED harvesters must be standing, so
# the opening and its cost-scale profile are untouched.
HOME_FLOOR_MIN_RND = 60
# Target live in-band guns, stepped to approximate the field's own trajectory
# (2 by r200, 4 by r300) but deliberately SLOWER, because every gun is +20%
# on the turret cost scale and a first gate should under-reach.
#   r60-179: 1   r180-299: 2   r300-419: 3   r420+: 4
HOME_FLOOR_EVERY = 120
HOME_FLOOR_MAX = 4
# Never spend the economy's seed corn: this is surplus-only, above the cost of
# the sentinel itself.
HOME_FLOOR_RESERVE_TI = 60
