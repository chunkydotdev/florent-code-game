...
# ============================================================================
# PLANK "SITE" (s23) -- A SUBTRACTION.  Gate forward siting on core separation.
#
# THE MEASUREMENT (corpus, turrets built before r150, alive at r150; derived
# independently by both arms, numbers agree to rounding):
#
#   band          side      US n   US alive   FLD n   FLD alive      gap
#   narrow<=81    HOME       609     52.5%      838     48.7%     +3.9pp
#   narrow<=81    FORWARD    196     47.4%      540     28.9%    +18.6pp   <-- WE WIN HERE
#   mid 128-144   HOME       589     67.4%      800     56.1%    +11.3pp
#   mid 128-144   FORWARD    329     26.4%      642     40.2%    -13.7pp
#   wide 288-392  HOME       548     86.9%     1284     72.1%    +14.7pp
#   wide 288-392  FORWARD    697     18.9%      806     49.0%    -30.1pp   <-- AND LOSE HERE
#   hive 650      HOME        52     92.3%      151     60.9%    +31.4pp
#   hive 650      FORWARD     69     24.6%      109     51.4%    -26.7pp
#
# OUR HOME TURRETS ARE THE BEST-SURVIVING TURRETS IN THE CORPUS, IN EVERY BAND,
# and the advantage GROWS with map width (+3.9 -> +11.3 -> +14.7 -> +31.4pp).
# Our FORWARD turrets on wide maps are the worst in the corpus (18.9%).
#
# AND THE POLICY RUNS EXACTLY BACKWARDS.  Share of early turrets sited FORWARD:
#   band          US       FIELD
#   narrow<=81    24.3%    39.2%
#   mid           35.8%    44.5%
#   wide          56.0%    38.6%     <-- ours doubles, theirs is FLAT
#   hive          57.0%    41.9%
# As maps widen we move the MAJORITY of early turret investment out of the
# bucket where we survive 86.9% and into the one where we survive 18.9%.
# Nobody chose that: it is emergent from siting anchored on a FRACTION of the
# distance to the enemy, which necessarily walks outward as the map widens.
#
# THE SHARPENING BOTH ARMS INITIALLY MISSED: forward siting is NOT universally
# bad for us.  On narrow maps our forward turrets BEAT the field's by +18.6pp.
# So this is not "stop going forward" -- it is "stop going forward as the map
# widens", and the crossover sits between d^2 81 and 128.  A blanket ban would
# throw away a band we win.
#
# WHY A SUBTRACTION AND NOT A SUBSYSTEM: today refuted turret PLACEMENT (LOKI-3,
# +0.0pp), PRICE (plank HOME, -2.0pp) and PRODUCTION (plank FLOOR, -0.7pp with
# the mechanism firing).  Three nulls on three knobs.  This adds nothing and
# builds nothing new -- it moves titanium from a 18.9% bucket to an 86.9% one by
# declining to spend it.
#
# HONEST RISK, PRE-STATED: forward guns may earn their deaths.  Survival is not
# damage, and this measurement does not price what a forward gun does before it
# dies.  Our core-kill production is 29.8% and band-invariant; if forward siege
# is how that happens, this trade could cost offence to buy durability.  THE
# GATE MUST THEREFORE BE READ ON WIN RATE, NOT ON TURRET SURVIVAL -- moving the
# survival metric is guaranteed by construction and proves nothing.
SITE_WIDTH_GATE_ON = False
# No forward siege planning once the Cores are further apart than this.  81 is
# the measured crossover (the top of the band where our forward turrets still
# beat the field), not a round number.
SITE_MAX_SEP_DSQ = 81
