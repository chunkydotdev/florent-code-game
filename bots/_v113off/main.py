...
class Player:
    ...
    def _plan_siege(self, ct):
        ...
        if self.map_grid is None or self.enemy is None:
            return False
        # PLANK SITE.  Forward siting is measured GOOD on narrow maps (+18.6pp
        # over the field) and catastrophic as the map widens (-30.1pp on wide),
        # while our HOME turrets are the best-surviving in the corpus in every
        # band.  The anchor is a FRACTION of the distance to the enemy, so it
        # walks outward as separation grows and nobody chose the result.
        if SITE_WIDTH_GATE_ON and self.core is not None and self.enemy is not None:
            if self.core.distance_squared(self.enemy) > SITE_MAX_SEP_DSQ:
                return False
