...
# PLANK DODGE (s24).  _bfs_direction puts a visible enemy turret's OWN TILE into
# `blocked` -- it treats a gun as an obstacle to walk around and carries no
# line-of-fire cost anywhere in the search.  So a builder routes straight
# through a live firing line, every round, at no perceived cost.  That is the
# only mechanism anyone has produced that explains a single enemy tile killing
# 45 of our builders in one game without the tile needing to be special: 45
# kills is not 45 clever placements, it is us re-entering the same line 45
# times.  It also predicts the research arm's independent negative -- if the
# lethality is OUR PATHING rather than THEIR SITING, plant tiles should look
# unremarkable and unpredictable, which is exactly what both grains measured
# (plant grain +3.8pp lift, kill grain -3.9pp at k=8; COVER is dead).
#
# SIZE, MEASURED, AND IT IS SMALL -- do not let this read as a home-defence
# fix.  Classifying every US home builder death against the band's own
# composition (n=4,169, kill grain): HEAL_SEAT lift 2.14, HARVESTER 1.41,
# CONVEYOR 1.37, TRANSIT 0.07.  Counting only buildings that existed BEFORE the
# death round -- the research arm broke its own first number here -- transit is
# 5.9% of home deaths overall and 18.4% of deaths by r100.  So this is an
# OPENING fix worth ~18% of early home deaths, not a defence subsystem.  The
# read is pre-stated accordingly: early-game builder attrition, never pooled
# home deaths, which would guarantee a null.
#
# WHAT IT MUST NOT DO, and this is the hard constraint rather than a nicety.
# The exposure is concentrated at WORK SEATS, and the heal seat is both the
# highest-lift class (2.14) and SYMMETRIC with the field (26.1%, lift 2.19 --
# nobody has solved it; it is the price of healing, not our defect).  Declining
# it means declining 4.00 HP/Ti, our best exchange rate in the game, which is
# PLANK ESCALATE in a new coat and that is refuted at -7.8pp.  So the cost is
# applied to TILES WE PASS THROUGH ONLY: the goal set is exempt by
# construction, and when no safe route exists the search falls through to the
# unmodified one.  A builder is never stranded and never abandons a seat --
# the worst case is the old behaviour.
DODGE_TRANSIT_ON = True
# Node ceiling for the danger-avoiding first pass.  Bounds BOTH the CPU (the
# search runs at most twice) and the detour (a safe route that needs more than
# this many nodes is a long way round, and a long way round is an economy cost
# paid every round it persists).  Past the ceiling we take the direct route.
DODGE_NODE_CAP = 220
...
LATE_TURRET_ON = False
FORWARD_PLACEMENT_ON = False
LATE_AMMO_ON = False
...
class Player:
    def __init__(self):
        ...
        self.pave_dir = None
        self.pave_rnd = -2
        # PLANK DODGE per-round danger cache (see DODGE_TRANSIT_ON).  Per unit
        # instance and keyed on the round, because the set is derived from THIS
        # unit's vision and _bfs_direction can run more than once in a turn.
        self.danger = set()
        self.danger_rnd = -1
    ...
    # ------------------------------------------------------------------
    # LOKI-3 -- THE LATE FORWARD GUNLINE (see LATE_TURRET_ON above)
    # ------------------------------------------------------------------
    ...
    def _bfs_direction(self, ct, target):
        ...
        if desired in CARDINALS:
            ...
            order = CARDINALS
        # PLANK DODGE first pass: the same search over the same graph with the
        # firing lines removed.  Two properties make this safe to try first --
        # `goals` is subtracted out, so a work seat is never made unreachable
        # by its own danger, and a None result falls through to the unmodified
        # search below, so the worst case is exactly the old behaviour.
        if DODGE_TRANSIT_ON:
            ...
            if danger:
                ...
                if avoid:
                    step = self._bfs_scan(
                        ct, p, target, goals, blocked | avoid, order,
                        cap=DODGE_NODE_CAP,
                    )
                    if step is not None:
                        return step
        step = self._bfs_scan(ct, p, target, goals, blocked, order, cap=0)
        if step is not None:
            return step
        return p.cardinal_direction_to(target)

    def _bfs_scan(self, ct, p, target, goals, blocked, order, cap=0):
        """One BFS over static terrain. Returns the first step, or None.

        Split out of _bfs_direction so PLANK DODGE can run it twice over two
        different blocked sets.  Returning None rather than a fallback
        direction is the whole point: the caller distinguishes "no route"
        from "here is a route", which a direction cannot express.

        `cap` > 0 abandons the search after that many expansions and returns
        None, which is how the dodge pass bounds both its CPU and its detour
        length; cap == 0 means no ceiling, matching the original search.
        """
        start = (p.x, p.y)
        seen = {start}
        ...
        while q:
            x, y, first = q.popleft()
            nav_bfs_steps += 1
            if cap and nav_bfs_steps > cap:
                return None
            if nav_bfs_steps % 64 == 0 and self._cpu_exhausted(ct):
                # Same fallback this function's caller already returns when
                # goals is empty and when the search is exhausted: one direct
                # cardinal step toward the target. Pure function, no instance
                # state, so bailing here is trivially safe.
                return p.cardinal_direction_to(target)
            for d in order:
                n = Position(x, y).add(d)
                key = (n.x, n.y)
    ...
    def _danger_tiles(self, ct):
        """Tiles a VISIBLE enemy gunner or sentinel can already fire on.

        Cached per round per unit: every builder recomputes its own set from
        its own vision, and the set is read once per _bfs_direction call, of
        which there can be several in a turn.
        ...
        """
        rnd = ct.get_current_round()
        if self.danger_rnd == rnd:
            return self.danger
        tiles = set()
        try:
            me = ct.get_id()
            # Read the team from the controller rather than self.team: this
            # runs before some of the paths that populate the cached team, and
            # a None there would classify our OWN turrets as hostile and route
            # builders around our own firing lines -- a much larger behaviour
            # change than this plank is scoped to make.
            mine = ct.get_team()
            for eid in ct.get_nearby_entities():
                if eid == me or ct.get_team(eid) == mine:
                    continue
                et = ct.get_entity_type(eid)
                if et != EntityType.GUNNER and et != EntityType.SENTINEL:
                    continue
                for t in ct.get_attackable_tiles_from(
                    ct.get_position(eid), ct.get_direction(eid), et
                ):
                    tiles.add((t.x, t.y))
        except Exception:
            tiles = set()
