...
# ============================================================================
# LOKI-QUIET (v121) -- A DISCRIMINATING PROBE, NOT A PLANK. It is not meant to
# win and its win rate is not a result.
#
# WHAT IT SEPARATES. Measured over 225 CAD games / 88,066 rounds: after early
# core damage, CAD's off-collar builders holding >=30 Ti build on 0.94% of turns
# against 8.25% in undamaged games -- with an IDENTICAL move rate, 100% of the
# idle turns cooldown-free, deaths flat and zero TLEs. They are solvent,
# unblocked, running, and doing nothing, for ~25 rounds. Poverty, rebuilding,
# walking and blocked/dying are all refuted, each inside its own strata.
#
# But DAMAGE and INTRUDER-PRESENCE are the same event in that corpus, so it
# cannot say which one CAD is reacting to. That distinction sets the price of
# the exploit and nothing else does:
#   * if the trigger is a BODY IN THEIR BASE, we never have to land a hit --
#     arriving is the whole exploit, and arriving is the thing we are already
#     bad at but which costs no damage race;
#   * if the trigger is DAMAGE, we must actually connect, which is far dearer.
#
# LOKI_QUIET_ON makes the raider arrive and DO NOTHING HOSTILE: no core peck,
# no melee on their buildings. It still walks, still seals barriers, still
# heals, still plants a forward sentinel -- those are not attacks. Against the
# unmodified LOKI-4 arm, which attacks normally, the contrast is exactly the
# one the corpus cannot make.
#
# READ IT ON THE OPPONENT, NOT ON US: the outcome is CAD'S BUILD RATE in
# r14-40, not our win rate. A quiet raider that loses every game while CAD
# still goes idle has ANSWERED THE QUESTION and is a success.
# ============================================================================

LOKI_QUIET_ON = True     # probe only; False == LOKI-4 exactly
