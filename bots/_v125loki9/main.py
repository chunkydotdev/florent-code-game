...
class Player(EcoMixin, RaidMixin):
    ...
    def _core_hurt_enough(self, ct):
        """Is our Core actually in danger, or merely touched?

        SLOT_UNDER latches 50 rounds off a single point of chip damage, so
        without this gate every nearby builder spends 50 rounds healing -- 1 Ti
        and one MOVE each, because acting and moving are mutually exclusive.
        That is our 66.5% collar occupancy against the top tier's 40.6%, paid
        for with the rounds the incidence cut says we need for ARRIVING.

        Returns True (i.e. heal, unchanged behaviour) whenever the flag is off
        or anything is unreadable, so this is a pure gate and can only ever
        REMOVE heals, never add them.
        """
        if not LOKI9_GARRISON_LESS:
            return True
        try:
            if self.core is None:
                return True
            cid = ct.get_tile_building_id(self.core)
            if cid is None:
                return True
            hp, mx = ct.get_hp(cid), ct.get_max_hp(cid)
            if not mx:
                return True
            return hp < mx * LOKI9_HEAL_HP_FRAC
        except Exception:
            return True
