...
class RaidMixin:
    ...
    # LOKI-14 split-throw alternation counter, declared on the MIXIN rather
    # than in Player.__init__ so main.py stays byte-identical to _v124loki8
    # (v102) -- the tree must differ only in what this plank needs.  Read as a
    # class attribute until the first throw, then shadowed per instance by
    # `self.kidnap_n += 1`, so it is per-launcher state exactly like every
    # other `self.raid_*` field.  Counts only throws that landed on their
    # intended arm; see the LOKI-14 block in doctrine.py.
    kidnap_n = 0
    ...
    # --- LOKI-14: the kidnap destination ------------------------------------

    def _kidnap_plan(self, ct, bp, sites):
        """Ordered (tag, site) candidates for throwing a kidnapped ENEMY builder.

        `sites` is the launcher's own reachable set, already filtered to
        1 <= d^2 <= 26 and in-bounds by the caller; nothing here recomputes it
        and nothing here calls get_nearby_* at all, so the "dist_sq exceeds
        vision radius" self-delete surface is not touched.

        Tags: "B" = border arm (the crash bid), "I" = interior arm (the
        within-leg placebo that must read ~0), "F" = fallback, belonging to
        NEITHER arm.  The fallback is the incumbent v102 ordering and it is
        appended to every plan, so LOKI-14 can only ever change WHERE a throw
        lands -- never whether one happens.  Losing exiles would make this bot
        worse at home defence than the thing it is being measured against.

        Ordering inside each arm:
          * BORDER first by how many map edges the tile touches (a CORNER puts
            five of the victim's eight neighbours off-map against an edge
            tile's three, so it is the stronger form of the same bid), then by
            distance from our own Core.
          * INTERIOR first by margin -- min distance to any edge -- so the
            placebo lands as far from the treated property as this launcher's
            reach allows.
        Both arms then share the SAME secondary key, distance from our Core,
        so the two arms differ as little as possible in everything except the
        border property.  Ties break on (x, y): no random, ever.

        The victim's own tile is dropped from both lists.  can_launch would
        accept it (d^2 from the launcher is in range and the tile is where the
        victim already stands), and it would spend the launcher's cooldown to
        move the victim nowhere.
        """
        incumbent = [t for t in sites if t.x != bp.x or t.y != bp.y]
        incumbent.sort(key=lambda t: (-t.distance_squared(self.core), t.x, t.y))
        fallback = [("F", t) for t in incumbent]
        if not LOKI14_KIDNAP_ON:
            return fallback
        w, h = self.mw, self.mh
        if not w or not h:
            return fallback
        try:
            if self._cpu_exhausted(ct):
                # Out of budget: take the cheap single-sort incumbent answer
                # rather than skipping the exile.  Locally this guard is a
                # no-op (get_cpu_time_elapsed is hardcoded to 0 in the shipped
                # .so, docs/research/engine-source-crash-and-launcher-
                # 2026-08-10.md section 4), so it cannot be exercised here.
                return fallback
        except Exception:
            return fallback
        if self.kidnap_n % 2 == 0:
            arm = "B"
            picked = [t for t in incumbent
                      if t.x == 0 or t.y == 0 or t.x == w - 1 or t.y == h - 1]
            picked.sort(key=lambda t: (
                -((t.x == 0) + (t.x == w - 1) + (t.y == 0) + (t.y == h - 1)),
                -t.distance_squared(self.core), t.x, t.y))
        else:
            arm = "I"
            picked = [t for t in incumbent
                      if 0 < t.x < w - 1 and 0 < t.y < h - 1]
            picked.sort(key=lambda t: (
                -min(t.x, t.y, w - 1 - t.x, h - 1 - t.y),
                -t.distance_squared(self.core), t.x, t.y))
        if not picked:
            return fallback
        keys = set()
        for t in picked:
            keys.add((t.x, t.y))
        plan = [(arm, t) for t in picked]
        for t in incumbent:
            if (t.x, t.y) not in keys:
                plan.append(("F", t))
        return plan

    def _kidnap_done(self, ct, tag, eid, site):
        """Book one completed kidnap throw.

        The counter advances ONLY on an intended-arm throw, so a fallback can
        never desynchronise the B/I alternation.  The print() is the entire
        ledger: all 16 store slots are allocated (doctrine.py), so there is no
        slot to spend on this and the arms are countable from the replay's
        captured output rather than from the store.
        """
        if tag != "F":
            self.kidnap_n += 1
        if not LOKI14_KIDNAP_LOG:
            return
        try:
            print(
                f"LOKI14 KIDNAP arm={tag} n={self.kidnap_n} "
                f"rnd={ct.get_current_round()} victim={eid} "
                f"to={site.x},{site.y}"
            )
        except Exception:
            return
    ...
    def _launcher_turn(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            try:
                ...
                continue
            # LOKI-14.  Everything above this line is v102 unchanged: the
            # victim is an ENEMY builder standing in our launcher's pickup
            # ring, and v102 threw it at the reachable site farthest from our
            # own Core.  LOKI-14 changes only the DESTINATION, alternating a
            # MAP-BORDER site (the crash bid) with an INTERIOR site (the
            # within-leg placebo).  `_kidnap_plan` returns the incumbent order
            # verbatim when the flag is off, so an off-variant is one constant.
            # The plan call is itself wrapped: it is evaluated by the `for`
            # statement, which would put it OUTSIDE the per-site try below,
            # and an exception escaping _launcher_turn costs this launcher its
            # turn (main.py's run() catch is the only thing between it and
            # Game::destroy_entity).  On any failure fall back to v102's order.
            try:
                plan = self._kidnap_plan(ct, bp, sites)
            except Exception:
                plan = [("F", t) for t in sites]
            for tag, site in plan:
                try:
                    if ct.can_launch(bp, site):
                        ct.launch(bp, site)
                        self._kidnap_done(ct, tag, eid, site)
                        return
                except Exception:
                    continue
