...
# ============================================================================
# LOKI-14 (v131) -- LAUNCHER KIDNAP AS CRASH-INDUCTION.  One flag,
# LOKI14_KIDNAP_ON.  This is a DESTINATION change, not a new behaviour: the
# launcher already throws adjacent enemy builders (raid.py `_launcher_turn`,
# step 1 EXILE).  LOKI-14 only changes WHERE they land, and splits that choice
# into two arms so the leg reads its own control.
#
# THE MECHANISM, from the ENGINE BINARY (docs/research/
# engine-source-crash-and-launcher-2026-08-10.md -- read by disassembly with
# two independent toolchains, results intersected; it is the primary and it is
# ours).  Four facts do the work:
#
#   1. AN UNCAUGHT EXCEPTION ESCAPING run() ROUTES TO Game::destroy_entity --
#      the unit is PERMANENTLY destroyed for the match, indistinguishable in
#      the replay from a normal death except that no damage precedes it.  The
#      exemptions are exactly two, SystemExit and KeyboardInterrupt; an
#      escaping GameError KILLS the unit.  A CPU timeout does NOT (the
#      "interrupted" flag is written to the replay and never routed to
#      destroy).
#   2. can_launch / launch have ZERO vision guards and NO team check.  Pickup
#      ring is d^2 <= 2 from the launcher (orthogonal AND diagonal); throw
#      target range is 1 <= d^2 <= 26 measured FROM THE LAUNCHER; the throw
#      costs 0 ammo and sets launcher cooldown += 1.
#   3. THE THROW MUTATES POSITION ONLY.  The victim's own cached state (its
#      previous position, its path, "the tile I was about to build on")
#      silently goes stale, and the engine tells it nothing.
#   4. Several Controller getters RAISE off-map: get_tile_env,
#      get_tile_building_id, get_tile_builder_bot_id, is_tile_empty,
#      is_tile_passable.  A bot that reads the eight neighbours of its own
#      position without a bounds check is fine everywhere except a border
#      tile.
#
# So (3) + (4) is a crash recipe: teleport the victim to a MAP-BORDER tile and
# its next turn queries neighbours that are off-map.  Three independent
# measurements in this repo say the field is exposed and we are not:
#
#   * A census over the replay archive found 224.06 undamaged-removal events
#     per 10,000 builder-rounds ON BORDER TILES against 0 in 2,334,017
#     non-border builder-rounds.  Border standing is where opponents' builders
#     vanish without damage.
#   * tools/crash_census.py measures 0 crash-candidates for us across 1,855 of
#     our games against 2,451 for opponents.
#   * OUR OWN eco.py:760 carries a guard whose comment says a launcher throw
#     teleported OUR builder and made is_tile_empty raise.  That is direct
#     evidence that a cached-position bot does not survive being thrown -- we
#     were the bot, once.
#
# WHAT IS NOT ESTABLISHED, and the leg exists to answer it: the engine only
# guarantees the position changes.  Whether a throw makes the VICTIM's bot
# raise depends on the victim's own code.  The 2,451 is a prior, not proof, and
# the border census is over NATURALLY-occurring border standing, which need not
# behave like INDUCED border standing (a bot that walks to a border tile chose
# to be there; a bot teleported there did not).
#
# THE SPLIT-THROW, AND WHY THE SECOND ARM IS THE POINT.  Half the throws go to
# a MAP-BORDER tile (x == 0 or y == 0 or x == w-1 or y == h-1), half to an
# INTERIOR tile as far from any border as the d^2 <= 26 reach allows.  The
# interior arm is a WITHIN-LEG PLACEBO and it MUST READ ~0.  It holds "being
# thrown" constant -- same launcher, same pickup, same stale-cache shock, same
# lost turn -- and varies ONLY the border property.  If both arms produce
# undamaged enemy removals, the effect is the throw and not the border, and
# the leg has refuted its own mechanism without needing an off-variant window.
#
# THE SPLIT RULE IS A PER-LAUNCHER ALTERNATION COUNTER (self.kidnap_n),
# NOT RANDOM AND NOT VICTIM-ID PARITY.
#   * Not random: rush_probe is the only bot in this repo that calls random.,
#     and it is excluded from paired runs for exactly that reason.  A leg whose
#     treatment assignment is nondeterministic cannot be replayed.
#   * Not victim-id parity: entity ids are issued sequentially across ALL
#     entities of both teams, so their parity is a function of the OPPONENT's
#     build order.  An opponent that spawns a builder every other build biases
#     the parity, and the split would silently stop being a split.
#   * The counter alternates B, I, B, I, ... over the throws THIS launcher
#     actually made, so the arms are balanced to within 1 by construction.
#     Only a throw that landed on its intended arm increments it, so a
#     fallback throw (below) cannot desynchronise the alternation.
#   Multiple launchers would each keep their own counter, which bounds the
#   whole-team imbalance at one throw per launcher; SLOT_LAUNCHER is a one-way
#   latch and we build one.
#
# WHAT THE ALTERNATION DOES WHEN AN ARM IS UNREACHABLE, and it matters for
# reading the leg.  The throw range is d^2 <= 26 FROM THE LAUNCHER, so a border
# site exists only when the launcher stands within 5 tiles of a map edge.  Our
# launcher is built near our own Core (`_try_build_launcher`), so on the big
# maps (26x26 archipelago/snowflake, 25x25 drumlin/hive) there may be NO
# reachable border tile at all.  Because the counter advances only on an
# intended-arm throw, an unreachable B arm does not silently promote the next
# throw to I: the plan falls back, the counter stays even, and the arm stays B.
# A map that cannot reach a border therefore contributes ZERO throws to EITHER
# arm rather than a pile of unmatched placebos.  That is the intended failure
# mode -- but it means the leg's usable n is throws made from launchers near an
# edge, NOT all kidnaps, and a fixture of large maps can return n = 0.
#
# THE FALLBACK IS DELIBERATE.  If no site of the chosen arm is legal, the
# launcher throws to the incumbent's choice (farthest reachable site from our
# own Core) rather than declining.  Declining would make LOKI-14 strictly worse
# than v102 at home defence -- the exile is the cheapest defence we own -- and
# a leg that loses on a plank it did not intend to touch measures nothing.
# Fallback throws are logged under their own tag and belong to NEITHER arm.
#
# TELEMETRY: NOT IN THE COMMS STORE.  All 16 slots are allocated (0-15, see
# the SLOT_ table above; slots 8/10/11/12/15 are already second-hand).  A
# kidnap ledger would have to evict a live plank's slot, which is a bigger
# change than this plank is entitled to.  Instead each throw emits ONE print()
# line -- print() output is captured to the replay -- tagged LOKI14 with the
# arm, the round, the victim id and the landing tile, so both arms are
# countable from a replay by grepping the captured output.  Stated plainly:
# THE ARMS ARE NOT READABLE FROM THE STORE, only from the captured prints.
#
# SAFETY, and it is the whole premise inverted.  This plank's mechanism is
# "an escaping exception deletes a unit"; the one way to lose the leg outright
# is to delete OUR launcher building it.  Every new call site is can_*-gated
# and wrapped in `except Exception:` (`finally:` and bare `except:` are banned
# by the sandbox AST validator), and no new get_nearby_* call passes an
# explicit dist_sq -- the launcher's own radius is r^2 = 26 and passing more
# than the CALLER's radius raises "dist_sq exceeds vision radius", which is a
# self-delete, not an attack.
# ============================================================================

LOKI14_KIDNAP_ON = False        # split-throw destinations for kidnapped enemies
LOKI14_KIDNAP_LOG = True       # one print() per throw -- the only arm ledger
