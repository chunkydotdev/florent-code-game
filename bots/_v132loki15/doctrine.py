...
# ============================================================================
# LOKI-15 (v132) -- ECONOMY SUPPRESSION AT A DOSE THAT ACTUALLY BINDS:
# A HARD PER-BUILDER CONVEYOR CAP, CHARGED AT EVERY CONVEYOR SITE.
#
# WHY THIS EXISTS: LOKI-13 MISSED ITS OWN TREATMENT-OCCURRENCE BAR.
# LOKI-13 (docs/research/RESULT-loki13-economy-suppression-2026-08-10.md) turned
# PAVE_TRAIL_ON off and pre-registered <= 27 conveyors/game (<= 0.70x control).
# Live: 33.32 against a control of 38.66, ratio 0.86. BAR NOT MET; per its own
# pre-registration THE LEG ANSWERED NOTHING. The bar had been derived from the
# flag's own paired-seed LOCAL action (0.65x / 0.67x / 0.43x) and that action
# did not transfer. The lesson taken here is not "measure the ratio better" --
# it is that A TREATMENT WHOSE SIZE IS AN EMERGENT PROPERTY OF THE GAME CANNOT
# BE PRE-REGISTERED. This flag is built so the cut is arithmetic instead.
#
# THE FIRST DESIGN WAS THE OBVIOUS ONE AND IT WAS MEASURED DEAD.  Reading
# LOKI-13's two numbers together says the pave trail is worth only ~5.3
# conveyors/game live (38.66 - 33.32) and that ~86% of our conveyors come from
# `_build_next_link` draining a `link_queue` that `_link_path` planned -- so the
# first LOKI-15 was a cap on PLAN LENGTH: refuse any trunk plan longer than N
# tiles, complete every plan we do accept. Swept N = 3/4/5/6 on the fixture
# below, 54 games per arm, against a 54-game `_v124loki8` control:
#
#     N            3      4      5      6   |  control
#     conv/game 36.83  37.09  32.00  34.50  |  34.07
#     ratio      1.08   1.09   0.94   1.01  |  1.00
#     ratio, length-matched r0-150 window
#                0.85   0.83   0.86   0.88  |  1.00
#     harvesters BUILT / game
#                7.17   6.81   5.98   6.46  |  4.78
#
# NOT ONE DOSE BINDS, INCLUDING THE MOST AGGRESSIVE. The refusal is COMPENSATED,
# and the fixture names both compensators: harvesters built rise 25-50% (a
# builder that is not wiring is free to go dig instead), and the pave trail --
# which needs no plan at all -- relays the ground the refused plans would have
# covered. Cutting one SOURCE of conveyors does not cut CONVEYORS while another
# source is standing next to it with spare capacity. That is the same class of
# error as LOKI-13's and it is why this flag is not a source-level refusal.
#
# WHAT SHIPS: A CAP ON THE QUANTITY ITSELF. Each builder bot may build at most
# LOKI15_CONV_MAX conveyors in its whole lifetime, counted across BOTH sites
# that can lay one -- `_build_next_link` (the trunk chain) and the PIECE F pave
# in `_move`. Source-agnostic, so there is nothing left to compensate WITH: the
# link cannot be re-routed through the pave, and an extra harvester cannot buy
# itself a road. Same fixture, 54 games per arm:
#
#     cap            1      2      3      5   |  control
#     conv/game   3.78   7.00  10.24  15.81   |  34.07
#     ratio       0.11   0.21   0.30   0.46   |  1.00
#     ratio, SHORT games (<= r250)
#                 0.12   0.22   0.31   0.48   |  1.00
#     ratio, LONG games (> r250)
#                 0.09   0.17   0.26   0.40   |  1.00
#     titanium_collected ratio
#                 0.02   0.47   0.69   0.99   |  1.00
#     win rate    63.0%  75.9%  88.9%  96.3%  |  94.4%
#
# Monotone in the dose, and -- THE PROPERTY THIS WHOLE FLAG EXISTS TO HAVE --
# THE RATIO IS THE SAME IN SHORT GAMES AND LONG ONES at every dose (0.31 vs
# 0.26 at the shipped cap). LOKI-13 died of a treatment whose size grew with
# game length; the link-plan cap above is worse still (0.78 short, 1.36 long at
# N = 3). A per-body quota is spent in the opening, when a builder's first acts
# are to dig and wire, so it has already bound by ~r40 in a game that ends at
# r150 and it does not keep biting harder for 850 more rounds.
#
# WHY THE BAR IS NOW ARITHMETIC. Conveyors/game <= LOKI15_CONV_MAX x builder
# bots SPAWNED, by construction. Spawns are the most stable number either corpus
# has: 6.92/game in LOKI-13's live control, 6.68 in its treatment, and 5.65-6.00
# across all ten arms of the fixture below. At cap 3 the live ceiling is
# 3 x ~6.9 = ~20.7, i.e. ~0.54x of 38.66 -- under the 0.60x target BEFORE any
# game is played, and the local arm came in at 0.30x because raiders, the
# defender and short-lived bodies never spend their quota. HONEST LIMIT: this
# is a ceiling CONDITIONAL ON THE SPAWN COUNT, not an unconditional guarantee.
# A live game that runs long enough to exercise REPLACEMENT_MAX (8) plus the
# SURGE_EXTRA (5) on top of POP_FLOOR (5) could spawn 15+ bodies and lift the
# ceiling with it -- so the spawn count must be read off the leg's own replays
# alongside the conveyor count, not assumed.
#
# THE DOSE IS 3, and it is chosen as the largest cap whose ceiling still clears
# the bar. Cap 5's local numbers are seductive (0.46x conveyors at 0.99x
# titanium and a HIGHER win rate than control -- i.e. more than half our
# conveyors are locally pure waste, which is worth a leg of its own) but its
# live ceiling is 5 x 6.9 = ~34.5 = 0.89x and it could miss the bar exactly the
# way LOKI-13 did. Cap 1 is not a bot: titanium_collected 0.02x, win 63.0%.
# Cap 2 is coherent but halves delivery. Cap 3 keeps 0.69x of delivery, 0.77x of
# forward sentinels and 88.9% of the fixture's wins while cutting conveyors to
# 0.30x, and it is one constant away from any of the others.
#
# NO STORE SLOT, AND THE APPROXIMATION IS NAMED. All 16 comms slots are
# allocated and none is touched. The counter is `self.loki15_conv`, plain
# per-unit instance state on the builder that spent it -- the same pattern
# PIECE F's pave_prev and PIECE K's heal ledger already use, and for the same
# reason K states: buffered last-write-wins store traffic would advance one
# shared counter by 1 rather than by N when several builders build in the same
# round, i.e. it would fail OPEN, in the direction of the cap not binding.
# SO THIS IS A PER-UNIT APPROXIMATION OF A TEAM-WIDE QUANTITY, said plainly:
# the team-wide cut is CAP x spawns and the per-body quota does not reallocate
# unspent quota from a raider to an expander. It errs toward cutting MORE than a
# true team cap of the same size, which is the safe direction for a treatment
# that has to be seen to occur.
#
# HOW THIS LOSES. `titanium_collected` decides 94.1% of our r1000 games and the
# local read already shows delivery at 0.69x and forward sentinels at 0.77x, so
# any game that reaches the bell is a straightforward loss and the fixture's win
# rate is 5.5pp under control. The bet is the one LOKI-4/5/7/8 all won: rounds
# and titanium not spent on road buy ARRIVAL. If core-kill share does not move,
# the plank is dead and LOKI15_CONV_CAP_ON goes back to False -- which restores
# `_v124loki8` byte-for-byte behaviour, both gates being pure early-outs.
# ============================================================================

LOKI15_CONV_CAP_ON = True    # per-builder lifetime conveyor quota, both sites
LOKI15_CONV_MAX = 3          # dose: conveyors one builder bot may ever build
