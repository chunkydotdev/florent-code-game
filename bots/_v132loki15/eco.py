...
class EcoMixin:
    ...
    def _loki15_conv_spent(self):
        """Conveyors THIS builder has already laid, both sites, whole lifetime.

        LOKI-15's whole mechanism is this counter plus the two early-outs that
        read it.  `getattr` rather than an `__init__` line so main.py stays
        byte-identical to `_v124loki8`: a body that has never built one has no
        attribute and reads 0, which is the same thing.

        PER-UNIT ON PURPOSE, AND IT IS AN APPROXIMATION OF A TEAM QUANTITY.
        The thing worth capping is the team's conveyor spend; what is capped
        here is one builder's share of it, and no store slot is spent because
        all 16 are allocated and because a shared slot would be WRONG here
        anyway -- store writes are buffered and last-write-wins within a round,
        so N builders incrementing one counter would advance it by 1, i.e. the
        cap would fail OPEN exactly when the most conveyors are being laid.
        Per-body quota errs the other way (a raider's unspent quota is never
        handed to an expander), which is the safe direction for a treatment
        that must be seen to occur.  See the LOKI-15 block in doctrine.py.
        """
        return getattr(self, "loki15_conv", 0)

    def _loki15_capped(self):
        return LOKI15_CONV_CAP_ON and self._loki15_conv_spent() >= LOKI15_CONV_MAX

    def _loki15_charge(self):
        self.loki15_conv = self._loki15_conv_spent() + 1
    ...
    def _build_next_link(self, ct):
        ...
        if f == Direction.CENTRE:
            f = Direction.NORTH
        # LOKI-15 SITE 1 of 2: the trunk chain.  Dropping the queue rather than
        # merely skipping the build is deliberate -- a queue this body can never
        # drain would otherwise pin it beside the next tile every round (see the
        # `link_queue` branch at the bottom of `_expand`, which returns without
        # moving whenever the head is adjacent).  Clearing it hands the body
        # back to ordinary ore navigation, and the harvester keeps whatever
        # route the pave trail or another builder gives it.
        if self._loki15_capped():
            self.link_queue = []
            return False
            ...
            if readable:
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 \
                    ...
                    if dist_core(pp, self.core) > 0:
                        ...
                        # LOKI-15 SITE 2 of 2: the PIECE F pave.  Charging BOTH
                        # sites to one quota is the whole point -- the swept
                        # link-only cap (doctrine.py, LOKI-15 block) measured
                        # 1.08x conveyors at its most aggressive dose because
                        # the pave silently relaid what the linker refused.
                        # The MOVE is never skipped, only the pave, exactly as
                        # the E2B ore ban already does one line above.
                        try:
                            if not self._loki15_capped() and coreward_ok \
