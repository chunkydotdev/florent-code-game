...
# ============================================================================
# LOKI-16 -- RING-BODY DENIAL.  One body, on the ring, kept.
# ============================================================================
#
# THE MEASUREMENT.  The enemy Core is 2x2 and spawns builder bots onto the
# twelve tiles inside its action radius sqrt(8) -- the 8 orthogonal heal seats
# plus the 4 diagonal corners (eco.heal_seats + eco.core_corners).  A BODY
# standing on a ring tile makes that tile unspawnable.  Over 539,000 exposed
# builder-rounds with round < 250, ONE hostile body on the ring DOUBLES the
# enemy's 25-round core-death hazard: 2.24% -> 4.77%, confidence intervals
# disjoint.
#
# THE PRESCRIPTION IS EXACTLY THAT AND NOT ONE STEP FURTHER: ONE body, EARLY,
# and KEPT.  Bodies 2-12 have NO supporting evidence -- the archive's maximum
# hostile ring occupancy is 6 of 12, seen four times in 2,710 sides, so the
# multi-body case is UNMEASURED, not proven.  This block therefore pins exactly
# one raider and leaves every other raider's behaviour byte-for-byte as it was.
#
# WHAT THE PARENT ALREADY DOES, so the delta is honest.  _v130loki13's raid
# layer already walks raiders onto ring tiles: _raid_station picks a station
# from corners + seats every round, and _raid_act seals free SEATS with 3 Ti
# barriers.  So the parent already denies seats -- but it denies them with
# BUILDINGS, and it does not keep a BODY anywhere:
#
#   * a barrier on a seat denies the spawn permanently and costs no body, but
#     the hazard measurement above is about a BODY, and whether a barrier
#     reproduces it is UNMEASURED;
#   * the four CORNERS are never barriered at all -- _raid_act step 2 only
#     builds on tiles in `raid_seatkeys` -- so a corner is deniable only by
#     standing on it;
#   * and the parent actively LEAVES corners: _raid_station adds +12 to a
#     corner's score once `_open_seats_by` reaches 0, which is precisely the
#     moment the corner has become pure body-denial and nothing else.
#
# So this plank is a REFUSAL, not a mechanism: the pinned raider stops
# re-scoring stations and stops walking.  It keeps acting normally (it still
# seals adjacent seats, still buddy-heals, still holds the collar); it simply
# never chooses to be somewhere else.
#
# WHICH raider, with the comms store FULL (all 16 slots allocated).  There is
# no slot to elect a holder in, so the election is derived PER UNIT FROM
# VISION and is therefore AN APPROXIMATION, stated plainly: a raider standing
# on a ring tile claims the pin only if no friendly builder bot with a LOWER
# entity id is visible on another ring tile.  Every ring tile is within r^2=18
# of every other, inside a builder's r^2=20 vision, so in the ordinary case all
# candidates see the same set and exactly one claims.  It can transiently pin
# two bodies when a lower-id raider arrives in the same round it is first seen,
# or when a body is out of vision behind nothing (vision here is radius-only,
# so that case is arrival timing, not occlusion).  That is the known failure
# mode and it fails toward the UNMEASURED multi-body case, not away from it.
#
# THE CLAIM WINDOW is the "EARLY" half of the prescription and the one number
# this plank adds: the hazard was measured on rounds < 250, so a NEW pin may
# only be claimed before LOKI16_HOLD_CLAIM_RND.  An existing pin has no expiry
# at all -- "KEEP it" is the whole point, and a raider that has held a ring
# tile since r60 is exactly the state the measurement describes.  A raider
# thrown off the ring by a launcher drops the pin (its position is no longer a
# ring tile) and may re-claim only if the window is still open.
#
# HOW THIS LOSES.  A pinned body occupies a tile that another raider could
# otherwise have BARRIERED (can_build_barrier refuses an occupied tile), so the
# ceiling on permanent seat seals drops by one while the pin is a seat.  If the
# barrier is in fact the thing that was working, this trades a permanent denial
# for a mortal one and the leg goes negative.  Off is one constant away.
LOKI16_RING_HOLD_ON = True    # pin ONE established raider to its ring tile
LOKI16_HOLD_CLAIM_RND = 250   # a NEW pin may only be claimed before this round
