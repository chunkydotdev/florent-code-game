...
# ============================================================================
# LOKI-19 (v136) -- THE CORE PECK COMES BACK, AND ONLY THE CORE PECK.
#
# THIS IS NOT A CLAIM THAT LOKI-QUIET WAS WRONG. It measured well and its
# reasoning was sound. Its stated premise, verbatim from the LOKI-7 block above:
#
#     "the ladder says ARRIVAL is the scarce quantity, not damage"
#
# ARRIVAL IS NO LONGER SCARCE. Measured 2026-08-11 on our own corpus, self-inserts
# (`kind=INSERT`, ours, rated): reach 18.6% pre-quiet -> 38.1% now. v104 is the
# highest arrival rate in our entire history. The flag was correct when it was set
# and its world moved -- a conditional optimisation outliving its condition, which
# is the hardest kind to notice because nothing ever fails and no test goes red.
#
# WHAT THE SILENCE COSTS, measured: 0 builder attacks across 2,247 self-inserts
# over SIX consecutive versions (v102-v107), v104 alone 0/1,490. Pre-quiet we
# landed any_atk 80.7% and core_atk 17.5%. And 28.1% of night inserts stood
# ORTHOGONALLY ADJACENT TO THE ENEMY CORE and never swung -- which also kills the
# innocent reading that we simply ferry builders for economy now: a bot ferried
# for eco is not standing on their doorstep.
#
# THREE ALTERNATIVE EXPLANATIONS, ALL CLOSED BY CONTROLS BEFORE THIS WAS BUILT:
#   "the attack column broke"  -> opponents' inserts in the SAME v104 files
#                                 attack at 21.1% (124/587). Column alive.
#   "tracking window too short"-> our ferried bots hold 57,625 life-rounds vs
#                                 their 34,298. MORE observed time, not less.
#   "we ferry for eco now"     -> 28.1% stood adjacent to the ENEMY CORE.
#
# WHY ONLY THE PECK. Flipping LOKI_QUIET_ON itself would also restore the
# COUNTERBATTERY melee (main.py) and the SIPHON hit (eco.py). Counterbattery is a
# defensive mechanism and PLAY_DEFENCE: never puts it off-programme; the siphon is
# economic and R1000_IS_DEFEAT demotes it. Re-enabling all three would smuggle two
# off-programme changes in beside one on-programme one and make the leg
# uninterpretable. THE CORE PECK IS DIRECT DAMAGE TO THE ENEMY CORE, which is the
# only thing this programme buys.
#
# THE COUNTER-ARGUMENT, CARRIED RATHER THAN BURIED. The per-round trade is
# UNCHANGED: acting and moving are mutually exclusive, so a peck still costs that
# raider its move, and the LOKI-7 block records LOKI-QUIET going 3-2 against CAD
# while landing ZERO builder attacks -- the melee was not load-bearing at 18%
# arrival. THE QUESTION THIS PLANK ASKS IS NOT "does melee help". IT IS: at 38%
# arrival, is an arrived-round worth more spent on DAMAGE than on HOLDING
# POSITION? That is a different question from the one v96 answered, and the old
# result does not settle it.
#
# ⚠ AND THE ADOPTION WAS NEVER MONOTONE, WHICH IS WHY THIS IS A FAIR QUESTION TO
# REOPEN: _v121lokiquiet carries it as "probe only; False == LOKI-4 exactly",
# _v122loki6 DOES NOT CARRY IT AT ALL, and _v123loki7 restored it and every tree
# since has kept it. Somebody switched it on as a probe, shipped the next
# iteration without it, then switched it back on. THREE DELIBERATE ACTS, and the
# reason for the middle one is not written down anywhere in this repo.
# ============================================================================

LOKI19_CORE_PECK_ON = True   # peck the enemy Core from a seat, even under QUIET
