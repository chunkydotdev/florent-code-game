...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # LOKI-19: the ONE gate this plank opens. `_raid_peck` (step 6 below),
        # the counterbattery melee in main.py and the siphon hit in eco.py all
        # stay silenced -- see the LOKI-19 block in doctrine.py for why only this
        # one. Reverting the plank is `LOKI19_CORE_PECK_ON = False`, which
        # restores the expression to `not LOKI_QUIET_ON` exactly.
        if on_seat and ti >= LOKI_PECK_TI_FLOOR and (
                LOKI19_CORE_PECK_ON or not LOKI_QUIET_ON):
