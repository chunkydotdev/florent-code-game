...
# ============================================================================
# LOKI-20 -- GUNS NOT WALLS.  One reorder, no new mechanism.
# ============================================================================
#
# THE MEASUREMENT (field census, 71 teams).  A forward turret standing in the
# ENEMY HALF by ~r20-25 is the strongest correlate of fast kills we have found:
# rho = +0.784, and it DOMINATES generic forward building (partial +0.452 vs
# +0.124), so it is the turret that matters and not the act of building forward.
# Within-team p < 0.0001, within-team x opponent p = 0.0018, 8 independent
# teams, and with rating partialled out it still reads +0.685.  Reverse
# causality was tested: opponent losses by r30 are flat-to-LOWER in fast games
# (p = 0.0005), so it is not a turret following an already-collapsing enemy.
#
# WHERE WE SIT.  First forward turret median r49 (v104) / r73 (v107) against
# r23 for the league's fast killers; 0.34 forward turrets per game by r40
# against their 1.62.  And the composition is the sharp part: the fast killers'
# forward builds are 64% TURRET / 16% barrier.  v104's are 81% BARRIER / 13%
# turret -- while we lay down 2.62 forward builds per game, MORE than they do.
# We are already doing the work and buying the wrong thing with it.
#
# WHAT THIS FLAG DOES.  `_raid_act` currently tries, in order:
#     1 peck (silenced)  2 SEAL A SEAT WITH A BARRIER  3 forward sentinel  4 heal
# With the flag on, 3 is tried BEFORE 2.  A forward raider reaches for the gun
# first and falls back to the wall.  Nothing is added, removed or re-costed --
# it is a preference order, which is why the diff is two lines and why
# `inert_check` can see straight through it.
#
...
#
# THE FALSIFIER, AND IT IS OUR OWN HISTORY CONTRADICTING THE FIELD (it belongs
# in the prereg and it is recorded here so the tree carries it too):
#     version   fwd turrets by r40   fast-kill rate
#     v92              0.70               11.1%
#     v104             0.34               22.3%
#     v107             0.15               31.3%
# Our siting got WORSE and our speed got BETTER, monotonically.  Either the
# forward-barrier plank delivers the same tempo through another channel, or the
# league correlation does not transfer to this chassis.  A leg that clears the
# mechanism bar and moves no kill-speed has found the second.
LOKI20_GUNS_NOT_WALLS = True
