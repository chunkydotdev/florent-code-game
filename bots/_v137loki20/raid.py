...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # LOKI-20: GUNS NOT WALLS.  Reach for the gun before the wall.  The
        # forward sentinel below is step 3 in the incumbent; with the flag on it
        # is tried FIRST, and the barrier seal becomes the fallback.  Ordering
        # only -- no build is added, removed or re-costed.
        if LOKI20_GUNS_NOT_WALLS and self._try_forward_sentinel(ct, E):
            return True
        ...
        # LOKI-20 already tried this above; retrying is harmless (it returns
        # False on a second call in the same turn once the action is spent) but
        # the guard makes the single-attempt intent explicit.
        if not LOKI20_GUNS_NOT_WALLS and self._try_forward_sentinel(ct, E):
            return True
