...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        for d in CARDINALS:
            ...
            for target in tiles:
                ...
                try:
                    ...
                    continue
                # LOKI-24: BEST-FIT, NOT FIRST-FIT.  The incumbent builds on the
                # FIRST (tile, facing) whose ray can hit ANY core tile.  A
                # sentinel's line is single-tile-wide and IGNORES OBSTACLES, so
                # coverage is pure geometry and free to evaluate --
                # `get_attackable_tiles_from` is never called anywhere in this
                # tree.  Score each candidate by how many ENEMY CORE FOOTPRINT
                # tiles its ray actually covers and keep the best; a ray over two
                # core tiles delivers twice the HP/round at the same turret cost.
                try:
                    cov = len(set(ct.get_attackable_tiles_from(
                        bp, facing, EntityType.SENTINEL)) & set(tiles))
                except Exception:
                    cov = 1
        ...
        if best is not None:
            try:
                if ct.can_build_sentinel(best[1], best[2]):
                    ct.build_sentinel(best[1], best[2])
                    if LOKI2B_LIVE_CAP_ON:
                        ct.write_store(SLOT_FWD_GUN, (live or 0) + 1)
                    else:
                        ct.write_store(SLOT_FWD_GUN,
                                       ct.read_store(SLOT_FWD_GUN) + 1)
                    return True
            except Exception:
                pass
        return False
