...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 7. LOKI-29B COUNTERBATTERY. LAST in the ranking on purpose: every
        # branch above is either arrival or a deposit that outlives the body,
        # and this must not outbid them. It only ever fires on a round where the
        # raider had no productive action at all -- which research measured at
        # 25.76% of builder-rounds.
        if LOKI29_COUNTERBATTERY_ON and ti >= LOKI_PECK_TI_FLOOR:
            if self._raid_counterbattery(ct):
                return True
        return False

    def _raid_counterbattery(self, ct):
        """Melee an orthogonally adjacent ENEMY TURRET. Turrets only.

        Deliberately NOT _raid_peck: that ranks the enemy CORE first and grinds
        conveyors, which is the behaviour LOKI-QUIET removed for good reason
        (it bought damage with arrival). This buys the one thing measured to
        matter -- fewer enemy turrets alive per forward builder-round.
        """
        p = ct.get_position()
        best, best_hp = None, 10 ** 9
        for d in CARDINALS:
            t = p.add(d)
            if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                continue
            try:
                ...
                continue
            # lowest HP first: finish a turret rather than spread damage.
            if hp < best_hp:
                best, best_hp = t, hp
        if best is None:
            return False
