...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        # ARM 2 ONLY: the spawn-candidate sort below hashes ABSOLUTE map
        # coordinates, so it is a second seat-asymmetric site that
        # orient_cardinals does not reach.  See doctrine.seat_flip_for.
        self.seat_flip = seat_flip_for(self.core, enemy_core_for(w, h, self.core))
        ...
        # ARM 2: the SAME hash, on coordinates relative to our own Core and
        # negated on one seat, so both seats sort an identical relative ring.
        # The constants, the modulus and the tie-break ORDER are untouched --
        # only the frame the coordinates are expressed in changes.
        _f = -1 if getattr(self, "seat_flip", False) else 1
        _cx, _cy = self.core.x, self.core.y
        cands.sort(key=lambda sp: (((sp.x - _cx) * _f * 17 + (sp.y - _cy) * _f * 31
                                    + self.n * 13 + self.spawn_salt) % 97,
                                   (sp.y - _cy) * _f, (sp.x - _cx) * _f))
        for sp in cands:
