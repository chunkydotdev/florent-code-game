...
# SEAT-RELATIVE SPAWN SORT (LOKI-29 arm 2, s32).  The SECOND absolute-geometry
# site, found by the side lane while auditing arm 1's prereg -- an OMISSION in
# arm 1's scope, not an error in it.  `main.py` sorts the Core's spawn-candidate
# ring on `(sp.x*17 + sp.y*31 + n*13 + salt) % 97` with `sp.y, sp.x` as
# tie-breaks: a hash of ABSOLUTE map coordinates, deciding the spawn tile on
# every spawn from round 0.  The two seats' Cores sit at different (x, y), so it
# orders their candidate rings differently, PER MAP.
#
# WHY IT MATTERS TO THE MEASUREMENT AND NOT ONLY TO THE BOT: arm 1's mechanism
# bar asks whether canonicalising CARDINALS drops a 6.8pp seat gap below 3.0pp --
# a bar set against a total whose DECOMPOSITION IS UNMEASURED.  If this site
# carries ~4pp, a perfectly executed arm 1 misses a bar it could never meet; if
# it carries ~2pp, arm 1 passes partly on a component it never touched.
# ⇒ arm 2 exists so the partition is MEASURED rather than argued: arm 1 = the
# CARDINALS component, arm 2 = CARDINALS + spawn sort, and the difference is the
# spawn sort's share.
#
# `seat_flip` is the canonicalisation, and it is exact under point reflection:
# hash on coordinates RELATIVE TO OUR OWN CORE, negated on exactly one seat.  For
# seat B, core_B and every candidate tile are the reflections of seat A's, so
# rel_B = -rel_A; negating on the seat whose enemy lies WEST or NORTH makes
# rel_B == rel_A and both seats hash the identical relative ring.  The selector
# is itself reflection-invariant because the two seats' enemy directions are
# always exact opposites, so exactly one of them flips.
def seat_flip_for(core, enemy):
    """True on the ONE seat whose relative frame must be negated to canonicalise."""
    if core is None or enemy is None:
        return False
