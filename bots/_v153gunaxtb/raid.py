...
class RaidMixin:
    ...
    def _raid_station(self, ct, E, near):
        ...
        for i, s in enumerate(stations):
            ...
            # ⭐ LOKI-31 (s32): THE SAME AVOIDANCE AS A TIEBREAK, NOT A SCORE TERM.
            # `_v146gunaxis` adds LOKI_GUNAXIS_PENALTY to `score`, so a ray tile
            # can be outranked by a tile that is WORSE on every other axis --
            # including a tile further back. That is why its s30 mechanism read
            # showed forward presence falling 23% alongside deaths falling 24%:
            # presence loss is a SIDE EFFECT of a score term, not the design.
            #
            # THIS ARM EXISTS TO SEPARATE TWO READINGS OF THAT RESULT:
            #   SITING -- the win comes from the stations that remain, better placed
            #   VOLUME -- the win comes from simply going forward less
            # Moving the avoidance into the SORT KEY, after `score`, means a ray
            # tile can only lose to an EQUALLY-SCORED alternative. Forwardness is
            # decided entirely by `score` and is therefore held at the control's
            # level BY CONSTRUCTION rather than by tuning.
            # ⇒ currency SURVIVES here  => SITING, and volume was incidental.
            # ⇒ currency VANISHES here  => VOLUME, and the penalty's real channel
            #    was sending fewer builders forward.
            # (Design: side lane, s32 -- the polarity move that killed the ring
            # candidate, i.e. an arm where the proposed cause is present and the
            # rival is absent.)
            if standing_here:
                score -= 2  # hysteresis: do not shuffle between equal tiles
            on_ray = 1 if (s.x, s.y) in gun_axis else 0
            k = (score, on_ray,
                 (s.x * 17 + s.y * 31 + self.raid_slot * 7) % 97, s.y, s.x)
