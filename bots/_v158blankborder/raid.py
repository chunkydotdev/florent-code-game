...
class RaidMixin:
    ...
    def _launcher_turn(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            # escapes run(), and the ENGINE PERMANENTLY DESTROYS THAT UNIT for
            # the rest of the match (0x1ac5c -> Game::destroy_entity).
            #
            # ⭐ THE MECHANISM IS MEASURED, NOT ASSUMED -- 2026-08-12, 72 local
            # games, tools/crash_cells.py:  EVERY border arrival killed the
            # unguarded probe (13/13 and 16/16), while the GUARDED probe took
            # 16 border arrivals and lost nothing (0/16).  The kill needs both
            # our throw and their missing guard, which is the mechanism exactly.
            #
            # ⛔ AND THIS IS WHY IT IS A *REORDER* AND NOT THE LOKI-14 TREE.
            # `_v131loki14` alternates a border arm and an interior placebo on
            # `kidnap_n % 2` -- a MEASUREMENT DEVICE that shipped inside the
            # firing path -- so it spends half its throws on the interior and
            # measured FEWER border arrivals (13) than the plain incumbent
            # ordering it was meant to improve on (16).  The incumbent already
            # lands border throws by accident, because "farthest from our core"
            # is disproportionately an edge tile.  This makes that deliberate.
            #
            # Keys, outermost first:
            #   1. corner/edge count -- a CORNER puts five of the victim's eight
            #      neighbours off-map against an edge tile's three, so it is the
            #      stronger form of the same bid;
            #   2. distance from our own core, the incumbent key, retained so a
            #      tie inside the border set still throws the intruder AWAY.
            # Every site remains reachable and legal; nothing is filtered out,
            # so this can never lose an exile the incumbent would have made.
            def _edges(t):
                return ((t.x == 0) + (t.x == w - 1)
                        + (t.y == 0) + (t.y == h - 1))

            far = sorted(sites, key=lambda t: (_edges(t),
