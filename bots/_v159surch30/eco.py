...
class EcoMixin:
    ...
    def _eco_spendable(self, ct, cost):
        ...
        spend is made self-limiting by arithmetic rather than by a metric read
        after the leg.  Source: `confused`, BC2020, verified 1 hit/fragment --
        *"if it's still before round 250 and the rush isn't over ... then
        everything non-rush gets an additional rush cost of 250"*.

        TWO DESIGN POINTS, both from the source and both load-bearing:
          * THE SOURCE TERMINATES ON AN EVENT, NOT A ROUND NUMBER.  Their rush
            miner signals when the rush is over; round 250 is only an outer
            bound.  `SLOT_RAID_LIVE` (slot 15) is already exactly that signal --
            a HEARTBEAT written `rnd + 1` by each reporting raider, so
            `read_store(SLOT_RAID_LIVE) >= rnd` reads "a raider reported last
            round".  Store writes are buffered one round, which is why the
            comparison is `>=` and not `==`.
          * ZERO NEW STORE SLOTS.  All 16 are bound (`doctrine.py:931-961`,
            remapped `:1166-1170`), so the plank as first written -- which asked
            for a new `SLOT_STRIKE_ALIVE` -- was NOT BUILDABLE.  Both halves it
            needs already shipped separately and had simply never been connected.

        The reserve is ADDITIVE with the siege reserve rather than replacing it:
        under attack AND with a raider live is a real state and both reasons to
        hold titanium apply.  At `NON_STRIKE_SURCHARGE = 0` this function is
        byte-for-byte the incumbent, which is what makes the 0-arm a true null.
        """
        ...
        if (
            ...
            reserve += SIEGE_HEAL_RESERVE_TI
        if (
            NON_STRIKE_SURCHARGE
            and rnd < STRIKE_SURCHARGE_MAX_RND
            and ct.read_store(SLOT_RAID_LIVE) >= rnd
        ):
            reserve += NON_STRIKE_SURCHARGE
        return ti >= cost + reserve
