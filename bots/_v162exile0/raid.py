    ...
    def _launcher_turn(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            try:
                ...
                continue
            # LOKI-37 EXILE ABLATION.  Gates ONLY the exile branch; the FERRY
            # branch below (raid.py "2. FERRY") is untouched.
            # ⛔ WHY THIS ARM EXISTS AND LAUNCH0 IS NOT ENOUGH: the launcher does
            # TWO STRATEGICALLY OPPOSITE JOBS, as its own docstring says --
            # "exile intruders first, then ferry raiders forward".  EXILE is the
            # kidnap weapon; FERRY/INSERT is how the assault reaches their core
            # inside KILL_WINDOW_RND, and uncapping it IS LOKI-27, which we
            # SHIPPED as v112.  `LAUNCHER_CAP = 0` removes both, so a positive
            # LAUNCH0 result has two OPPOSITE readings -- "retire the kidnap
            # line" and "retire our fastest route to their core" -- and cannot
            # choose between them.  LAUNCH0 PRICES the launcher premium;
            # this arm ATTRIBUTES it.
            if not EXILE_ON:
                continue
