        ...
        for d in CARDINALS:
            ...
            for target in tiles:
                ...
                if facing == Direction.CENTRE:
                    continue
                try:
                    # LOKI-38 GUNNER-WHERE-CLEAR.  ADDITIVE, NOT A SWAP.
                    # The docstring above forbids substituting Gunner for
                    # Sentinel, and it is RIGHT: the collar blocks LOS, so a
                    # Gunner aimed THROUGH our own barriers would be shooting
                    # them.  But that argument is about OCCLUDED lines, and it
                    # was generalised to all lines.  `can_fire_from` with
                    # GUNNER is the exact test for "this particular line is
                    # already clear" -- a Gunner ray IS blocked by obstacles
                    # while a Sentinel's is not, so a TRUE here means no
                    # barrier stands in it.  Where it is true we lose nothing
                    # and pay 20 Ti instead of 30 at the same +20% scale, so
                    # the same titanium buys ~1.5x the guns.  Where it is
                    # false we fall through to Sentinel exactly as before, so
                    # this can never remove a turret the incumbent would build.
                    # THE FIELD GAP THIS ADDRESSES: gunners/game Pivot 10.77 ·
                    # O(1) 7.02 · ph 5.79 · US 1.86 -- the largest in the set.
                    # Pivot builds 5.72 barriers/game against our 6.17, so
                    # barriers and gunners demonstrably coexist; the binding
                    # constraint is our COLLAR GEOMETRY, not barriers existing.
                    if (LOKI38_GUNADD_ON
                            and bp.distance_squared(target) <= 13   # gunner r^2
                            and ct.can_fire_from(bp, facing, EntityType.GUNNER, target)
                            and ct.can_build_gunner(bp, facing)):
