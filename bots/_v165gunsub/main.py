...
class Player(EcoMixin, RaidMixin):
    ...
    def _try_counterbattery(self, ct):
        ...
        # LOKI-40 SUBSTITUTION-ONLY. Gunner-first, but ONLY when we could also
        # have afforded the SENTINEL the incumbent would have built.
        # ⛔ WHY THIS ARM EXISTS: LOKI-39 changes MIX, COUNT and SCALE together.
        # Measured on 6 paired games, treatment vs control INSIDE THE SAME FILE:
        # 39 turrets vs 30, and scale 354% vs 276% at r150 -- +78pp, so every
        # subsequent build of EVERY type costs the treatment ~28% more. A
        # negative currency read on LOKI-39 could mean "gunners are worse than
        # sentinels" OR "we bought 30% more turrets and inflated our own costs",
        # and those imply OPPOSITE actions: prefer sentinels, versus cap the
        # count and prefer the cheap one.
        # HOLDING COUNT AT THE SOURCE rather than with a cap: the extra builds
        # come from the gunner being reachable at 20 <= Ti < 30, where the
        # incumbent could afford nothing. Requiring sentinel-affordability
        # removes exactly that channel, so every build this arm makes is one the
        # incumbent would also have made -- only the TYPE differs.
        # ⚠ IT IS NOT PERFECTLY COUNT-NEUTRAL AND I AM NOT CLAIMING IT IS: a
        # cheaper build leaves more titanium banked, so a LATER trigger can
        # still fire that the incumbent could not afford. This removes the
        # first-order channel, not the second. The turret-count control below
        # is what says how much of it survived -- measured, not assumed.
        gun_ok = LOKI39_GUN_FIRST and (
            not LOKI40_SUB_ONLY
            or ct.get_global_resources() >= ct.get_sentinel_cost())
        choices = (
