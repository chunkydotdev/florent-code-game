...
class Player(EcoMixin, RaidMixin):
    ...
    def _try_counterbattery(self, ct):
        ...
        p = ct.get_position()
        ban = self._seat_ban()
        # LOKI-39: TRY THE CHEAPER TURRET FIRST.
        # ⛔ THE ORDER WAS THE WHOLE CONSTRAINT, AND IT MADE THE GUNNER BRANCH
        # NEARLY DEAD CODE BY CONSTRUCTION. A SENTINEL's line IGNORES obstacles
        # at r^2=32; a GUNNER's is BLOCKED at r^2=13. So `can_fire_from` is TRUE
        # for a sentinel EVERYWHERE it is true for a gunner, and strictly more
        # often -- sentinel DOMINATES gunner on the exact predicate this loop
        # selects on. Tried first, it therefore wins every contested tile, and
        # the gunner branch is reachable ONLY when we cannot AFFORD a sentinel
        # (20 <= Ti < 30). That is not a preference anyone expressed; it is a
        # side effect of tuple order meeting a dominance relation.
        # MEASURED CONSEQUENCE: 4,629 home gunners across 5,618 games are very
        # largely "could not afford a sentinel" builds, and our gunners/game run
        # 1.86 against Pivot 10.77, O(1) 7.02, ph 5.79.
        # ⚠ AND THE STATS DO NOT OBVIOUSLY FAVOUR THE GUNNER, WHICH IS WHY THIS
        # IS A TEST AND NOT A FIX: per titanium, sentinel is 6.0 dmg/rnd for
        # 30 Ti (0.200) against gunner 3.5 for 20 (0.175); per ammo 1.8 vs 1.75;
        # and sentinel has 40 HP to gunner's 25 and 2.5x the range. On paper the
        # incumbent's order is DEFENSIBLE. What contradicts it is the FIELD:
        # every top team fields far more gunners than we do. One of those two
        # is wrong and only a battery can say which.
        # The swap is otherwise inert: the affordability `continue` below still
        # falls through to the other type, so this can never build NOTHING where
        # the incumbent built something.
        # LOKI-41 GUNNERS ARE NOT BETTER TURRETS, THEY ARE EARLIER ONES.
        # The sentinel wins EVERY paper metric except price: 0.200 dmg/rnd/Ti to
        # 0.175, 1.8 dmg/ammo to 1.75, 40 HP to 25, r^2=32 to 13, and its line
        # ignores obstacles. So the field's 10.77 gunners/game cannot be a
        # steady-state efficiency story -- and the paper comparison prices NO
        # CLOCK AT ALL, which is the one thing this programme is made of.
        # THE ARITHMETIC, and note what it does NOT say. Gunner and sentinel BOTH
        # add +20% to the ONE GLOBAL scale factor, so N gunners inflate the whole
        # economy exactly as much as N sentinels: on 500 Ti you get 11 gunners
        # against 9 sentinels, NOT 15 against 10. "Spam the cheap turret" is
        # arithmetically false and the scale factor is why.
        # What you DO get is TIME. The opening three cost 20+24+28 = 72 Ti against
        # 30+36+42 = 108 -- 36 Ti, more than a harvester -- and against a spend
        # gate of the form `ti >= cost + reserve` at 2.5 Ti/round passive income
        # the first gunner clears FOUR ROUNDS SOONER at every reserve floor.
        # WHY FOUR ROUNDS IS NOT A ROUNDING ERROR: our median kill is r174 and our
        # median death r187. A THIRTEEN-ROUND RACE. Three turrets ~12 rounds
        # sooner is most of that gap, and `raid.py`'s own rush recipe is "three
        # turrets planted by r22".
        # ⇒ Prefer the cheap turret while TITANIUM is the binding constraint, and
        # hand back to the sentinel's range and obstacle-penetration once it is
        # not. GUN_EARLY_RND is the handover, and it is SWEPT, not guessed --
        # a knob chosen after seeing the result is a fit, not a mechanism.
        early = ct.get_current_round() < GUN_EARLY_RND
        choices = (
            (EntityType.GUNNER, ct.get_gunner_cost()),
            (EntityType.SENTINEL, ct.get_sentinel_cost()),
        ) if (LOKI39_GUN_FIRST and early) else (
            (EntityType.SENTINEL, ct.get_sentinel_cost()),
            (EntityType.GUNNER, ct.get_gunner_cost()),
        )
