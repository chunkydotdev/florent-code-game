            ...
            for site in far:
                try:
                    if ct.can_launch(bp, site):
                        ct.launch(bp, site)
                        # LOKI-43 RENT, DO NOT OWN.
                        # `CLAUDE.md:23`: every build adds to the ONE GLOBAL
                        # additive scale factor -- launcher +10% -- and
                        # **DESTRUCTION REMOVES THE CONTRIBUTION**. So a launcher
                        # that throws and then dies costs us 20 Ti ONCE, while a
                        # launcher that stands costs +10% on EVERY subsequent
                        # build of EVERY type for the rest of the match.
                        # WHY THIS ARM EXISTS, and it is measured rather than
                        # reasoned: the 2x2 prices the launcher at ~7.0pp TO OWN
                        # (BOTH0 45.79% vs LAUNCH0 52.77%, identical mechanism,
                        # differing only in whether it is built and paid for)
                        # against mechanisms returning ~4.2pp. Deleting it wins
                        # +2.77pp. But a launcher with FULL mechanism and ZERO
                        # ownership cost would read ~57%, so there is ~4pp of
                        # headroom above LAUNCH0 that deletion cannot reach.
                        # LOKI-42 reaches for it by DEFERRING the purchase.
                        # This reaches for it by REFUNDING it after every use.
                        # `LOKI6_LAUNCHER_RELEASE` already rebuilds a launcher
                        # once the Core can no longer see one, so the rent cycle
                        # needs no new machinery -- only the destruction.
                        # ⚠ NOT FREE: we re-pay 20 Ti per throw, we lose the
                        # FERRY between cycles, and a rebuild costs a builder
                        # action. Ferry measured at ~0 (FERRY0 49.84%), which is
                        # what makes the trade plausibly worth taking.
                        if LOKI43_RENT:
                            try:
                                ct.self_destruct()
                            except Exception:
