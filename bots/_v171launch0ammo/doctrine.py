...
# LOKI-35 LAUNCHER CAP -- see main.py `_try_build_launcher`.
# THE INCUMBENT BUILDS EXACTLY ONE LAUNCHER, EVER, and that ceiling was never a
# decision: `SLOT_LAUNCHER` is a BINARY LATCH and the cap is a side effect of
# using a flag where a count belonged.  The docstring justifies ONE launcher as
# home defence; nothing in it argues against a second.
# WHY THIS IS THE TREE THAT DID NOT EXIST: the archive's SmartFridge effect is a
# THROWN vs NOT-THROWN contrast (+3.71pp, z=4.28) -- it is dosed by how OFTEN we
# throw.  LOKI-33 border-first changes only WHERE a throw lands and leaves throw
# COUNT untouched, so it doses the CRASH channel and not the DISPLACEMENT one.
# This constant is the only knob that moves throw frequency.
# COST, stated because it is real: each launcher is 20 Ti base and adds +10 pct
# to the ONE GLOBAL scale factor, inflating every subsequent build of EVERY
# type.  That is why it is swept rather than set.
LAUNCHER_CAP = 0
