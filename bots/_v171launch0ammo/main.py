...
class Player(EcoMixin, RaidMixin):
    ...
    def _try_build_launcher(self, ct):
        # LOKI-36 ABLATION GATE. Checked FIRST and unconditionally.
        # Without this, LAUNCHER_CAP = 0 does not disable launchers: the cap
        # test `read_store(SLOT_LAUNCHER) >= LAUNCHER_CAP` is ALWAYS TRUE at 0,
        # so with no launcher in sight the LOKI6 release branch zeroes the slot
        # and execution falls THROUGH to the build. The ablation arm would have
        # quietly built launchers and read as a null. Verified by counting
        # launcher builds in a live game, not by reading the branch.
        if LAUNCHER_CAP <= 0:
            return False
        ...
        for eid in ct.get_nearby_buildings():
            try:
                ...
                continue
        if ct.read_store(SLOT_LAUNCHER) >= LAUNCHER_CAP:
            if seen_launcher:
                return False
        ...
        elif seen_launcher and ct.read_store(SLOT_LAUNCHER) >= LAUNCHER_CAP:
            ct.write_store(SLOT_LAUNCHER, LAUNCHER_CAP)
            return False
        if ct.read_store(SLOT_HARVESTERS) < 1:
            return False
        if not self._eco_spendable(ct, ct.get_launcher_cost() + LAUNCHER_RESERVE):
            return False
        # LOKI-35: claim the NEXT index, not the flag.  Store writes are
        # buffered one round, so every builder acting this round reads the SAME
        # count and the cap can overshoot by at most the number of builders
        # simultaneously adjacent to a legal launcher tile -- bounded, small,
        # and self-correcting next round.  A per-round arbiter would cost a
        # store slot and all 16 are bound.
        ct.write_store(SLOT_LAUNCHER, ct.read_store(SLOT_LAUNCHER) + 1)
        p = ct.get_position()
