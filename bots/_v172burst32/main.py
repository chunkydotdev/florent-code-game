    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            if (under or weapons or harv >= 2) and ammo < ammo_target and ti > ti_floor:
                # LOKI-44 MAGAZINE BURST. Same titanium, spent at the moment
                # the gun exists instead of dribbled 16 at a time.
                # MEASURED ON 25 LIVE GAMES: our first gun is a FORWARD SENTINEL
                # at median r13 (forward in 14/25), and a sentinel is 10 ammo a
                # shot. The incumbent already knows the right magazine --
                # `if fwd_guns: ammo_target = max(..., min(120, 40 + 20*fwd_guns))`
                # = 60 for one forward gun -- but this line fills it at 16/turn,
                # and `fwd_guns` is a STORE read, buffered one round. So the
                # sentinel lands r13, we notice r14, and the magazine reaches 60
                # around r17: FOUR ROUNDS of a forward sentinel sitting in the
                # enemy half with one volley in it.
                # x3r0's r0 pre-buy of 17 is AMMO_FLOOR+1 -- the same 16 we get
                # anyway at the harv>=2 gate (median r8), moved earlier, and it
                # is 1.7 SENTINEL shots. It funds one volley; it does not fund
                # the magazine.
                # ⇒ Raise the per-turn cap so the magazine loads in ONE turn once
                # a forward gun is up. No earlier spend, no new constant beyond
                # the cap, and it CANNOT convert more than `ammo_target` wants.
                cap = AMMO_BURST_CAP if fwd_guns else 16
                amt = min(cap, ammo_target - ammo, ti - ti_floor)
