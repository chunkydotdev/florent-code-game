...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 3. THE FORWARD SENTINEL.
        # LOKI-45 SEAL FIRST, SHOOT SECOND.
        # MEASURED TODAY, 50 live games: our first sentinel lands r13 and our
        # first barrier r18, so the gun fires FIVE ROUNDS before the collar
        # exists. That ordering is not a priority bug -- the seal at step 2 can
        # only fire when the raider is ADJACENT TO A HEAL SEAT, i.e. already at
        # the ring, while the sentinel is built on the APPROACH. The raider
        # passes the sentinel band on its way, so the gun always comes first.
        # WHY THAT IS WORTH INVERTING, in this file's own arithmetic: an enemy
        # core has a median 4 builders alive at r25, each healing +4 HP/round,
        # so ~16 HP/round of repair against a sentinel's 18 damage on a 2-round
        # reload = 6 HP/round. A SENTINEL FIRING BEFORE THE COLLAR MAKES NO NET
        # PROGRESS AT ALL -- every point is refunded, and we paid 30 Ti and +20%
        # global scale for it. Sealed, the same gun is +6 HP/round permanent.
        # And the ordering is safe because a SENTINEL LINE IGNORES OBSTACLES
        # (r^2=32) -- it shoots THROUGH our own collar, which is exactly why
        # this file chose Sentinel over Gunner in the first place.
        # THE GATE: hold the approach sentinel until a foothold is live, i.e.
        # some raider is established at the ring and the sealing has begun.
        # `_foothold_live` is already the published heartbeat for that state and
        # costs no new slot.
        # ⚠ COST, stated: the gun arrives later, and if we never establish we
        # never build it at all. That is the trade this arm measures.
        if (not LOKI45_SEAL_FIRST) or self._foothold_live(ct, ct.get_current_round()):
