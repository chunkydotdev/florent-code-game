...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        # --- ammunition ----------------------------------------------------
        # x3r0's v115 OPENING AMMO PRE-BUY, ported verbatim so the stack differs
        # from `bots/_x3r0_v115` in the LAUNCHER treatment and NOTHING ELSE.
        # Ammo starts at 0, there is NO passive ammo income, and convert_ammo is
        # the only source -- so without this the first turret we build stands
        # loaded with nothing. 17 buys four gunner shots or one sentinel volley
        # plus change.
        if rnd == 0 and ammo < 17:
            amt = min(17 - ammo, ti)
    ...
    def _turret(self, ct):
        ...
        # forward = this turret sits closer to THEIR core than to ours.
        fwd = False
        try:
            if self.core is not None and self.enemy is not None:
                _p = ct.get_position()
                fwd = _p.distance_squared(self.enemy) < _p.distance_squared(self.core)
        except Exception:
            fwd = False
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                if et is None or not ct.can_fire(t):
                    continue
                # LOKI-46 SHOOT THE HEALERS, NOT THE CORE.
                # MEASURED, 75 live games:
                #  * their core has a median 4 builders alive at r25 = up to
                #    16 HP/round of repair, against our sentinel's 18 damage on
                #    a 2-round reload = 6 HP/round. FIRING AT THE CORE IS NET
                #    NEGATIVE -- every point is refunded.
                #  * those same builders attack our sentinel for 2 damage each,
                #    so 4 of them are 8/round against its 40 HP: five rounds.
                #    41% OF OUR SENTINELS ARE DESTROYED, median life 12 rounds.
                #    One group of bots does BOTH jobs.
                #  * a builder is 40 HP = 3 sentinel shots, and their median
                #    REPLACEMENT LATENCY after a death is 12 ROUNDS (only 32%
                #    inside 3), capped at one spawn per core turn.
                # ⇒ each kill buys ~12 rounds of -4 HP/round repair AND
                #   -2 damage/round against our own gun. The core does not
                #   dodge, does not shoot back, and will still be there later.
                # ⚠ WHAT THIS IS NOT: attrition. Destruction REFUNDS their scale
                #   contribution (CLAUDE.md:23), so a kill makes their next
                #   builder CHEAPER. This buys a repeatable 12-round tempo
                #   window, not a grind -- and we currently kill a MEDIAN OF
                #   ZERO of their builders per game.
                # Only reordered for a FORWARD gun: a home turret defending our
                # own core still wants the thing attacking it.
                prio = {
                    EntityType.CORE: (4 if LOKI46_HEALERS_FIRST and fwd else 0),
                    EntityType.SENTINEL: 1,
                    EntityType.GUNNER: 2,
                    EntityType.BUILDER_BOT: (0 if LOKI46_HEALERS_FIRST and fwd else 3),
