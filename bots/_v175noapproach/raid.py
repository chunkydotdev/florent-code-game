...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # LOKI-47 NO APPROACH SENTINEL.
        # Measured, 75 live games: 41% of our sentinels are destroyed, median
        # life 12 rounds when killed -- about 6 volleys, 108 damage -- into a
        # core with up to 16 HP/round of repair against the gun's 6. The
        # approach sentinel may therefore be worth ~ZERO net, at 30 Ti and +20%
        # global scale. This arm deletes it and lets the titanium go elsewhere.
        # ⚠ NOT a claim that it IS zero -- that is the arithmetic, and the
        # arithmetic assumes they heal, which events.tsv cannot show (no heal
        # verb). This measures the claim instead of asserting it.
        if not LOKI47_NO_APPROACH:
