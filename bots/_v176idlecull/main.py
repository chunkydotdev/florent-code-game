...
class Player(EcoMixin, RaidMixin):
    ...
    def _builder(self, ct):
        # LOKI-48 IDLE CULL -- is a permanently idle builder worth more DEAD?
        # MEASURED, our own scale over 75 live games: we run at ~377% and
        # BUILDER BOTS ARE 44% OF IT -- 6.2/game x +20pp = 123pp, more than
        # every turret combined (sentinel 70 + gunner 30). Destruction REFUNDS
        # the contribution (CLAUDE.md:23), and a builder cannot be destroy()ed
        # (that is buildings only) but CAN self_destruct().
        # 25.76% of our builder-rounds are IDLE with both cooldowns at zero and
        # no verb emitted -- and idle share is the SAME in wins (27.59%), so it
        # is what our bot always does, not a losing-position artefact.
        # ⇒ two culled builders = -40pp of scale = every later build ~11%
        #   cheaper for the rest of the game.
        # ⛔ NOBODY IN THIS FIELD DOES THIS: across 25 live games there are 86
        #   undamaged GUNNER removals (turret walking, routine) and just TWO
        #   undamaged builder_bot removals. Untested by anyone, which cuts both
        #   ways -- novel lever, or an idea everyone else already rejected.
        # ⚠ AND THE OBVIOUS COST: a dead builder cannot heal, peck, seal or
        #   rebuild, and our raid IS builders. The floor keeps a working crew.
        if LOKI48_IDLE_CULL:
            try:
                if (ct.get_action_cooldown() == 0 and ct.get_move_cooldown() == 0):
                    self.idle_run = getattr(self, "idle_run", 0) + 1
                else:
                    self.idle_run = 0
                if (self.idle_run >= IDLE_CULL_RNDS
                        and ct.get_current_round() >= IDLE_CULL_MIN_RND
                        and ct.get_unit_count() > IDLE_CULL_FLOOR):
                    ct.self_destruct()
                    return
            except Exception:
                pass
