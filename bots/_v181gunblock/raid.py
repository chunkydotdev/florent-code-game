...
class RaidMixin:
    ...
    def _gunblock(self, ct, E):
        """Put a barrier ON a gunner's ray, in front of something of ours.

        The trigger is deliberately narrow: an orthogonally adjacent EMPTY tile
        that (a) is in the enemy half tested ON THE TILE, (b) sits on a visible
        enemy gunner's firing line, and (c) is strictly NEARER that gunner than
        some friendly entity further along the same line.  (c) is what makes
        this cover rather than decoration -- a gunner's shot stops on the first
        obstacle, so 30 HP of barrier absorbs what would have hit the asset.
        """
