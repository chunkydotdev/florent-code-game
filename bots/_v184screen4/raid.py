...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 3b. LOKI-44: THE APPROACH SCREEN.  Strictly BELOW the sentinel
        # attempt above -- a round that could plant a sentinel must still plant
        # it -- and below sealing, so the collar is never outbid by a screen.
        if LOKI_SCREEN_ON and self._screen(ct, E):
            return True
    ...
    # --- LOKI-44: the approach screen ---------------------------------------

    def _screen(self, ct, E):
        """Drop ONE barrier beside the route, in the approach band.

        The collar's only admissible tile is a heal seat (`seatkeys`); this is
        the second admissible class and nothing else changes.  A tile qualifies
        when it is orthogonally adjacent, empty, in the enemy half TESTED ON
        THE TILE, at d^2 LOKI_SCREEN_LO..HI from the enemy anchor -- the band
        ph (d^2~17) and not adgato (d^2~25) actually build in and we do not --
        and is NOT the tile this raider is about to walk onto or the one it
        just came from.  Barriers are impassable to us too; screening our own
        route would be paying 3 Ti to lengthen our own walk.
        """
        if E is None or self.core is None:
            return False
        if self.scr_spent >= LOKI_SCREEN_MAX:
            return False
        rnd = ct.get_current_round()
        if rnd - self.scr_rnd < LOKI_SCREEN_GAP:
            return False
        if ct.get_global_resources() < ct.get_barrier_cost() + LOKI_SCREEN_TI_FLOOR:
            return False
        p = ct.get_position()
        dp = p.distance_squared(E)
        if dp < LOKI_SCREEN_LO or dp > LOKI_SCREEN_HI + 8:
            return False
        # The route axis.  `tgt` is last round's station (this runs in the
        # ACTION phase, before the move phase resets it); the anchor is the
        # honest fallback and points the same way at this range.
        skip = set()
        try:
            tgt = self.tgt if self.tgt is not None else E
            sd = p.cardinal_direction_to(tgt)
            if sd != Direction.CENTRE:
                fwd = p.add(sd)
                back = p.add(sd.opposite())
                skip = {(fwd.x, fwd.y), (back.x, back.y)}
        except Exception:
            skip = set()
        for d in CARDINALS:
            t = p.add(d)
            ...
            if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                continue
            de = t.distance_squared(E)
            if de < LOKI_SCREEN_LO or de > LOKI_SCREEN_HI:
                continue
            # ENEMY HALF, tested on the TILE and not on the body.
            if de >= t.distance_squared(self.core):
                continue
            try:
            ...
            self.scr_rnd = rnd
            if LOKI_SCREEN_LOG:
                print("LOKI44SCR", t.x, t.y, rnd, de, t.distance_squared(self.core))
            return True
        return False
