...
# ============================================================================
# LOKI-44 -- THE APPROACH SCREEN  (arm _v182screen)
#
# THE GAP, measured across the top ten teams (events.tsv, per-team via
# meta_join names, d2_own/d2_enemy for placement):
#
#   team           barriers/g  forward   on their core ring (d2<=12)  median d2
#   us                    4.2      98%                          99%           4
#   Pantheon             15.1      87%                          66%           5
#   ph                   11.0      96%                          39%          17
#   not adgato            9.4      77%                          29%          25
#   Clankers             18.2      60%                          62%           5
#
# We run ONE barrier doctrine -- the collar, 99% of forward barriers on the
# enemy core's twelve-tile ring -- and we buy a quarter of what our own economy
# supports (34 Ti/game against Clankers' 110 on an identical median income of
# 880).  The use we do not have is the APPROACH SCREEN: forward barriers at
# d^2 ~ 17-25 that ph and not adgato build and we build essentially none of.
#
# WHAT IT IS FOR.  A barrier out on the approach is bot-impassable and blocks a
# gunner's line (not a sentinel's).  It costs 3 Ti base and 15 builder pecks at
# 2 Ti each to remove -- a 10:1 exchange, the same arithmetic the collar rests
# on, applied one band further out where the walk in actually happens.
#
# WHAT IT MUST NOT DO, and the constraint shapes the whole trigger:
#   * NOT slow the kill.  It sits BELOW the forward-sentinel attempt in
#     _raid_act.  Each placement costs the raider one round of walking (act and
#     move are mutually exclusive), so the cap is per BODY and small, and there
#     is a cooldown between placements.
#   * NOT wall ourselves off.  Barriers are impassable to our own bots too, so
#     the tile the raider is about to step onto -- and the one behind it -- are
#     excluded.  This is a screen BESIDE the route, never on it.
#   * NOT touch the collar.  The ring is d^2 <= 8 from the enemy anchor;
#     LOKI_SCREEN_LO = 13 excludes it by construction.
#   * NOT inflate scale.  Every barrier is +1% on the one global additive cost
#     factor, permanently, and that factor is what prices our launcher out.
# ============================================================================

LOKI_SCREEN_ON = True         # master flag -- False is the parent exactly
LOKI_SCREEN_MAX = 4           # LOKI-45: 2 -> 4.  THE DOSE KNOB.
LOKI_SCREEN_TI_FLOOR = 20     # bank left standing AFTER the barrier is paid
LOKI_SCREEN_LO = 13           # d^2 to the enemy anchor: off their ring...
LOKI_SCREEN_HI = 32           # ...and still well inside their half
LOKI_SCREEN_GAP = 5           # LOKI-45: 10 -> 5.  THE DOSE KNOB.
LOKI_SCREEN_LOG = False       # replay tag "LOKI44SCR x y rnd" -- dose readout


# ============================================================================
# LOKI-45 -- APPROACH SCREEN, DOUBLE DOSE  (arm _v184screen4)
#
# _v182screen is the MECHANISM leg and it dosed at 2.57 approach barriers per
# game.  This arm changes NOTHING about the trigger, the band, the route-tile
# exclusion or the bank floor -- only how often one raider is allowed to pull
# it: the per-body cap goes 2 -> 4 and the inter-placement gap 10 -> 5 rounds.
# Both halves are needed: doubling the cap alone would still be rate-limited by
# the gap over a raider's measured six-round post-throw life.
#
# WHY THIS BAND AND NOT MORE COLLAR.  Measured over the top ten teams, 99% of
# OUR forward barriers sit on the enemy core's twelve-tile ring at 4.2/game,
# while ph (11.0/game, median d^2 17) and not adgato (9.4/game, median d^2 25)
# run an APPROACH doctrine we have none of.  The budget is unspent rather than
# unavailable -- Clankers collects the same median 880 Ti and spends 110 Ti a
# game on barriers against our 34, at the same 7-9 Ti unit price.
#
# WHAT WOULD FALSIFY IT.  Volume is not obviously good: sporks leads the ladder
# at 2172 on 3.02 barriers/game, and every barrier is +1% on the ONE global
# additive cost factor, permanently -- the factor that already prices our
# launcher out (LAUNCHER_RESERVE 80 against a median r150-200 bank of 54).  A
# rise in median kill round is disqualifying on its own
# (DEFENCE_ADMISSION_BAR: kill_round_non_regression), whatever it does to the
# win rate.
# ============================================================================
