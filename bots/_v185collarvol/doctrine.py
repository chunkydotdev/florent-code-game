...
# ============================================================================
# LOKI-47 -- COLLAR VOLUME  (arm _v185collarvol)
#
# PLACEMENT IS UNCHANGED.  This arm does not invent a barrier doctrine; it buys
# more of the one we already run.  The admissible tile set is still exactly
# `raid_seatkeys` -- the eight heal seats of the enemy footprint -- and the seal
# still only fires from a body orthogonally adjacent to a free one.  The collar
# IS our doctrine: 99% of our forward barriers sit inside d^2 <= 12 of the enemy
# core, and the only barrier build site anywhere in this tree is that seal.
#
# WHAT IS BOUGHT.  4.2 barriers/game and 34 Ti/game of barrier spend, against
# Clankers' 18.2 and 110 Ti on an identical median income of 880 and the same
# 7-9 Ti unit price.  The budget is UNSPENT, not unavailable.  Two throttles
# stand between us and it, and this arm lifts both under one flag:
#
#   1. THE BANK FLOOR.  `LOKI_SEAL_TI_FLOOR = 12` on top of a scaled barrier
#      cost is a ~20 Ti gate against a median r150-200 bank of 54 -- a raider
#      standing beside a free seat with 15 Ti in the bank walks past it.  The
#      floor's stated purpose is "keep a peck's worth of change", and a peck is
#      2 Ti (LOKI_PECK_TI_FLOOR = 4).  4 is that purpose, priced honestly.
#
#   2. PECK-BEFORE-SEAL.  A raider standing ON a seat is orthogonally adjacent
#      to exactly one other seat, and step 1 of _raid_act spends its action on
#      2 damage to the core instead of sealing that neighbour.  The whole thesis
#      of the collar says this ordering is backwards: a heal is +4 HP for 1 Ti
#      and our peck is 2 HP for 2 Ti, so ONE round of their healing erases two
#      of our pecks, and the seal is what stops the healing.  Sealing first
#      costs at most a handful of pecks and only while a neighbouring seat is
#      still open -- after that step 1 runs exactly as before, forever.
#
# THE PRICE, and it is why this is an arm and not a patch.  Every barrier is
# +1% on the ONE global additive cost factor, PERMANENTLY -- doubling the
# collar adds ~4-6% of scale to every subsequent build of every type, and that
# factor is what already prices the launcher out.  Volume is also not obviously
# good on the field: sporks tops the ladder at 2172 on 3.02 barriers/game.
#
# THE BAR.  `DEFENCE_ADMISSION_BAR: kill_round_non_regression`.  Throttle 2
# trades core damage for seal tempo directly, so median kill round is the
# reading that decides this arm, not the barrier count.  A rise disqualifies it
# whatever it does to the win rate.
#
# ---------------------------------------------------------------------------
# ⛔ LOCAL PRE-FLIGHT: NEITHER THROTTLE IS THE THROTTLE.  120 games/arm,
# 6 maps x 10 seeds x 2 seats vs bots/_det_opp_v56 at --tle 0, collar barriers
# per game with the standard error of the mean:
#
#     parent (floor 12, peck-first, abandon +12)      3.90 +- 0.38
#     floor 4 + seal-first + hold                     4.37 +- 0.49
#     floor 4 + seal-first (HOLD off)                 4.82 +- 0.82
#     hold only (floor 12, peck-first)                3.88 +- 0.30
#
# Every arm sits inside ~1 SE of the parent and none is within a factor of two
# of the 8-10/game target.  A separate fully deterministic run (NOISE_ON=False,
# 12 independent cells) put the floor-only arm BELOW the parent, 3.92 vs 4.42.
# The bank floor is not binding, the peck ordering is a no-op, and the +12
# abandonment penalty is a measured null on its own (3.88 vs 3.90).
#
# THE DIAGNOSIS, and it is what a future collar-volume plank has to answer:
# THE COLLAR IS OPPORTUNITY-LIMITED, NOT BUDGET-LIMITED.  There are exactly
# eight seats, our own collar barriers are destroyed ~1.2 times a game, and the
# median game ends at r135-146 -- so the ceiling on this doctrine is roughly
# "seats a surviving raider is adjacent to before the game ends", and titanium
# was never the binding constraint.  Buying the field's 110 Ti/game of barrier
# spend REQUIRES A SECOND DOCTRINE (the approach band, _v182screen/_v184screen4)
# and cannot be reached by loosening the collar's purse.
#
# HOLD is therefore shipped OFF: it measured null on dose and it is the change
# most likely to cost kill round, since it keeps bodies on corners instead of
# on the seats where they peck the Core.
# ============================================================================

LOKI_COLLARVOL_ON = True      # master flag -- False is the parent exactly
LOKI_COLLARVOL_TI_FLOOR = 4   # replaces LOKI_SEAL_TI_FLOOR (12) while ON
LOKI_COLLARVOL_SEAL_FIRST = True   # seal an open seat before pecking from one
LOKI_COLLARVOL_HOLD = False   # MEASURED NULL, see the block: 3.88/g vs parent 3.90
LOKI_COLLARVOL_ABANDON = 0    # the +12 the parent charges a finished corner
LOKI_COLLARVOL_LOG = False    # replay tag "LOKI47SEAL x y rnd"
