...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 0. LOKI-47: SEAL BEFORE PECKING.  Off by default in the parent and ON
        # in this arm.  See the doctrine block: the seat-peck at 1 below is the
        # ONE thing that outranks a seal, and the two seats flanking a seat are
        # exactly what a seat-standing raider can reach, so this ordering is the
        # second throttle on collar volume after the bank floor.
        if LOKI_COLLARVOL_ON and LOKI_COLLARVOL_SEAL_FIRST:
            if self._collar_seal(ct, p, ti, seatkeys):
                return True
    ...
    def _collar_seal(self, ct, p, ti, seatkeys):
        """SEAL A FREE SEAT.  Verbatim the parent's step 2, plus one constant.

        can_build_barrier enforces adjacency, emptiness and occupancy, so a seat
        one of our own raiders is standing on is refused by the engine and stays
        a peck station.

        LOKI-47 changes the BANK FLOOR and nothing about placement: the set of
        admissible tiles is still `seatkeys` -- the eight heal seats of the
        enemy footprint -- and still only the ones orthogonally adjacent to this
        body.  With LOKI_COLLARVOL_ON False this is the parent, constant for
        constant, in the parent's call order.
        """
        if not LOKI_BARRIER_SEAL_ON:
            return False
        floor = (LOKI_COLLARVOL_TI_FLOOR if LOKI_COLLARVOL_ON
                 else LOKI_SEAL_TI_FLOOR)
        if ti < ct.get_barrier_cost() + floor:
            return False
        for d in CARDINALS:
            t = p.add(d)
            ...
            try:
                if ct.can_build_barrier(t):
                    ct.build_barrier(t)
                    if LOKI_COLLARVOL_LOG:
                        print("LOKI47SEAL", t.x, t.y, ct.get_current_round())
                    return True
            except Exception:
                continue
        return False
        ...
        for i, s in enumerate(stations):
            ...
            if i < ncorner:
                ...
                if self._open_seats_by(ct, s) == 0:
                    # LOKI-47 RETENTION.  +12 walks the body OFF a finished
                    # corner, and a finished corner does not stay finished:
                    # our collar barriers are destroyed 1.75 times a game.  The
                    # body that walks away is the body that would have re-sealed
                    # them.  This is the throttle on REPEAT placement -- the
                    # bank floor is measurably not (see the doctrine block).
                    score += (LOKI_COLLARVOL_ABANDON
                              if (LOKI_COLLARVOL_ON and LOKI_COLLARVOL_HOLD)
                              else 12)
                else:
                    score -= 6
            else:
