...
#
# ---------------------------------------------------------------------------
# ⚠ LOCAL PRE-FLIGHT SAYS THE GATE IS ON THE WRONG HALF.  120 games/arm,
# 6 maps x 10 seeds x 2 seats vs bots/_det_opp_v56 at --tle 0.  Self-play, so
# it prices the MECHANISM, not the field -- but the funnel is a property of our
# own code and reads the same anywhere.  Funnel over 12 logged games:
#
#     reached step 7 (all of 1-6 declined)  249.8 /game
#     idle gate OPENED                      161.3 /game   64.6% of reached
#     salt ACTED                             14.8 /game    9.2% of opened
#       of which CUTS (pecks)                13.58/game    92%
#       of which corpse-salts                 0.50/game
#       of which rebuild-blocks               0.75/game
#
# The gate refuses only 35% of the rounds it is asked about, and 92% of what it
# then buys is the CUT -- which is the ~10-action item that killed _v178salt.
# Barriers outside the collar band (d^2>12) are arm-attributable exactly,
# because the parent builds ZERO of them: v187 0.11/game against _v178salt's
# 1.96/game, i.e. 0.06x the mechanism.  And the kill round did not move:
# v187 154.0, _v178salt 154.5, parent 141.5 -- v187 vs the UNGATED arm is
# -0.5 rounds, permutation p=0.9998.  THE GATE REMOVED THE MECHANISM AND KEPT
# THE COST.  (v187 vs parent is +12.5 at p=0.091, and the parent's own median
# moved 141.5 -> 146.0 between two 120-game runs of this same design, so the
# local fixture cannot RESOLVE the regression -- it can only show that gating
# did not relieve it.)
#
# WHAT DOES CLEAR THE BAR, and it is one constant: LOKI_SALT_CUT_MAX = 0.
# A barrier is ONE action and a cut is ten, so forbidding the cut is the
# literal reading of the revival condition ("buy the denial WITHOUT spending
# raider actions").  Measured: kill round +1.0 vs parent, permutation p=0.939 --
# clean.  But it is also nearly INERT here at 0.10 salt-barriers/game, because
# with no cut of our own almost nothing dies adjacent to a raider to salt.  On
# a live opponent our forward Sentinel kills belt pieces that a self-play
# fixture does not present, so the inertness may be a fixture artefact; the
# kill-round cleanliness is not.  The default below is left at the briefed
# value so this tree IS the arm that was asked for.
# ============================================================================
