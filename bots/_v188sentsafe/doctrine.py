...
# The eight facings a turret can hold. CENTRE is not one of them.  Used by
# SENTSAFE (v188) to expand a gunner's rotated coverage.
ROT_DIRS = [d for d in Direction if d != Direction.CENTRE]
...
# ============================================================================
# SENTSAFE (v188) -- SITE THE FORWARD SENTINEL OFF ENEMY TURRET COVERAGE
# ============================================================================
#
# THE GAP THIS EXISTS FOR, measured on 25 live games against 5 opponents with
# BUILD/DEATH paired on tile:
#
#            built  destroyed  median life  sentinel-rounds
#     us      107    76 = 71%    12 rounds       5,143
#     them     68    23 = 34%    34 rounds       6,618
#
# We build 57% MORE sentinels and get 22% FEWER sentinel-rounds -- about 2x the
# damage per build for them, at the same 30 Ti and the same +20% scale.
#
# Three explanations are already dead: not COUNT (we build more), not TIMING
# (ours land earlier, r16 vs r23), and not SITING-FOR-OFFENCE (Magnus read the
# replay: our sentinel fired on their core and was well placed).
#
# The surviving mechanism, from a 75-game decode and independently reproduced
# to the round: "100% of the destroyed sat under ENEMY TURRET COVERAGE against
# 64% of survivors."  ==> we plant sentinels where enemy turrets can shoot them.
#
# THE ARM.  raid.py `_try_forward_sentinel` picked its site by `can_fire_from`
# alone -- PURE OFFENCE, consulting no threat model at all, and returning the
# FIRST site that could hit the core.  This adds one: among the sites that
# ALREADY satisfy `can_fire_from` (the parent's own admissible set, unchanged),
# prefer one that is not inside the union of VISIBLE enemy turret coverage.
#
# ⛔ IT IS A RANKING, NOT A FILTER, AND THAT IS A HARD REQUIREMENT.  A site
# filter that plants fewer turrets has cured the symptom by removing the
# patient.  The candidate set is byte-identical to the parent's; only the CHOICE
# within it moves.  So forward-sentinel COUNT is invariant by construction, not
# by hope -- if the parent would have built, this builds, on some site.
#
# ⚠ THE ALTERNATIVES THIS DOES NOT TEST, and they would each produce the same
# signature: ammo starvation, THEIR healing of THEIR turrets, and enemy target
# priority.  This arm tests the coverage hypothesis only.
#
# GUNNER vs SENTINEL, and they are NOT equally avoidable:
#   * GUNNER   r^2=13, ray BLOCKED by obstacles, can rotate for 10 Ti.
#   * SENTINEL r^2=32, ray IGNORES obstacles, cannot rotate.
# The worry was that near their core a sentinel's 32-radius line blankets every
# site that can fire on the core, so avoiding it would mean never planting.
# ⭐ MEASURED, 409 forward-sentinel builds over 192 local games: every admissible
# site was covered in 12.5% of builds -- and because this is a SOFT SCORE those
# builds still happen, on the least-covered site.  Plant count is untouched.
# ⭐ AND THE MEASUREMENT DECIDES THE CLASS QUESTION THE OTHER WAY ROUND FROM THE
# WORRY: all 14 redirects in that run were driven by SENTINEL coverage and ZERO
# by gunner coverage (74 admissible sites sat on a gunner ray, but when one did,
# so did its alternatives).  ⇒ GATING ON GUNNERS ONLY WOULD HAVE PRODUCED A DOSE
# OF EXACTLY ZERO.  Both classes are scored.
#
# Units are the same as LOKI_GUNAXIS_PENALTY / LOKI_EXILE_PENALTY -- an
# additive site-score penalty, lower is better.
LOKI_SENTSAFE_ON = True
LOKI_SENTSAFE_GUN_PENALTY = 8     # site sits on a visible enemy GUNNER's ray
LOKI_SENTSAFE_SENT_PENALTY = 3    # ...on a visible enemy SENTINEL's ray
# ROTATION.  A gunner's CURRENT ray is only its current ray: rotate() costs
# 10 Ti and one cooldown and buys any of the other seven facings, so a site
# that is off the ray but inside the rotated union is covered ON DEMAND, one
# round late.  A sentinel CANNOT rotate, so there is deliberately no sentinel
# twin of this term -- an off-ray tile is genuinely safe from that sentinel.
# Weighted below the live ray because it costs them a turn to collect.
#
# ⛔ DEFAULT OFF, AND THE MEASUREMENT IS WHY.  Over 409 forward-sentinel builds
# in 192 local games (4 opponents x 6 maps x 4 seeds x 2 seats) it moved the
# redirect rate 3.4% -> 4.4% -- FOUR extra redirects -- while classifying 42
# more admissible sites as risky and raising the fully-covered (forced) share
# 12.5% -> 17.4%.  It costs 8 `get_attackable_tiles_from` calls per gunner
# instead of 1.  One point of dose is not worth eight times the scan, and the
# term also makes the arm test two mechanisms at once.  Left in, flagged off,
# with its number written down so nobody re-derives it.
LOKI_SENTSAFE_GUNROT_ON = False
LOKI_SENTSAFE_GUNROT_PENALTY = 4
LOKI_SENTSAFE_GUNROT_MAX = 4      # enemy gunners expanded per round (CPU cap)
LOKI_SENTSAFE_LOG = False         # per-build funnel line into the replay
# DIAGNOSTIC ONLY.  The scan stops at the first UNCOVERED site because nothing
# can beat score 0 -- which is right for play and wrong for measurement, since
# it makes the logged `sites=` a lower bound rather than the funnel.  Turn this
# on to score every admissible site and log the true denominator.  It changes
# no decision, only CPU: with it off the chosen site is identical.
LOKI_SENTSAFE_FULLSCAN = False
