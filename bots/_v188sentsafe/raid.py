...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        # SENTSAFE (v188).  The admissible set below is EXACTLY the parent's:
        # every site that satisfies can_fire_from AND can_build_sentinel, in
        # the parent's own iteration order.  The only thing that changes is
        # WHICH member of that set gets built -- so the number of forward
        # sentinels planted cannot move.  With the flag off the loop
        # short-circuits on the first admissible site, which is the parent line
        # for line (including the number of predicate calls, so the shared
        # `_cpu_exhausted` budget is untouched too).
        cover = None
        best = None          # (bp, facing)
        best_score = None
        seen = 0             # admissible sites examined -- the funnel denominator
        penalised = 0        # ...of which sat under visible enemy turret cover
        n_gun = n_sent = n_rot = 0   # split by class: the gunner-vs-both read
        first = None         # the site the PARENT would have taken
        stop0 = not LOKI_SENTSAFE_FULLSCAN
        for d in CARDINALS:
            if stop0 and best is not None and best_score == 0:
                break
            bp = p.add(d)
            ...
            for target in tiles:
                ...
                try:
                    ...
                    continue
                if not LOKI_SENTSAFE_ON:
                    best, best_score = (bp, facing), 0
                    break
                seen += 1
                if cover is None:
                    # Built lazily: a turn with no admissible site pays nothing.
                    cover = self._turret_cover(ct)
                gun_ax, sent_ax, rot_ax = cover
                key = (bp.x, bp.y)
                score = 0
                if key in gun_ax:
                    score += LOKI_SENTSAFE_GUN_PENALTY
                    n_gun += 1
                elif key in rot_ax:
                    # Off the live ray but one 10 Ti rotate away from it.
                    score += LOKI_SENTSAFE_GUNROT_PENALTY
                    n_rot += 1
                if key in sent_ax:
                    score += LOKI_SENTSAFE_SENT_PENALTY
                    n_sent += 1
                if score:
                    penalised += 1
                if first is None:
                    # The PARENT's choice: it took the first admissible site.
                    # Kept so the log can report the REDIRECT RATE -- the share
                    # of builds this arm actually moved -- rather than only the
                    # share of covered sites.  One comparison per site.
                    first = (score, bp.x, bp.y)
                if best_score is None or score < best_score:
                    best, best_score = (bp, facing), score
                if stop0 and best_score == 0:
                    break   # cannot do better than uncovered
            if best is not None and ((stop0 and best_score == 0)
                                     or not LOKI_SENTSAFE_ON):
                break
        if best is None:
            return False
        bp, facing = best
        # NB: NOT gated on LOKI_SENTSAFE_ON, so a flag-off tree logs the same
        # line at the same point.  That is what makes the funnel columns
        # comparable between the two arms instead of existing only on one side
        # -- a log that only the treatment emits cannot show a null.
        if LOKI_SENTSAFE_LOG:
            try:
                gtiles = len(cover[0]) if cover else -1
                stiles = len(cover[1]) if cover else -1
                rtiles = len(cover[2]) if cover else -1
                f = first or (-1, -1, -1)
                print("SENTSAFE r=%d sites=%d cov=%d ongun=%d onrot=%d "
                      "onsent=%d raytiles=%d/%d/%d first=%d,%d fscore=%d "
                      "chosen=%d,%d score=%d moved=%d"
                      # NO cpu= COLUMN HERE ON PURPOSE.  It was tried and
                      # removed: `get_cpu_time_elapsed()` returns 0 in local
                      # `fcode run` even after a deliberate ~400k-iteration
                      # burn in the same turn (positive control, s33), so the
                      # column was constant and would have "verified" anything.
                      # Local CPU cannot be read from inside the bot; time the
                      # process instead.
                      % (ct.get_current_round(), seen, penalised, n_gun, n_rot,
                         n_sent, gtiles, stiles, rtiles, f[1], f[2], f[0],
                         bp.x, bp.y, best_score or 0,
                         int((f[1], f[2]) != (bp.x, bp.y))))
            except Exception:
                pass
    ...
    def _turret_cover(self, ct):
        """(gunner_rays, sentinel_rays, gunner_ROTATED_rays) as tile-key sets.

        `get_attackable_tiles_from` is the hypothetical-turret pattern -- the
        same predicate `_raid_station` already uses for gunner axes (LOKI-25),
        pointed at a second entity type.  Cached for the round because two
        raiders sharing a Player instance never happens (state here is per
        unit), but one unit can reach this twice in a turn through the retry
        paths.

        ⚠ SCOPE, and it bounds what this arm can possibly buy: this sees only
        what THIS BUILDER can see.  A builder's vision is r^2=20 and a
        SENTINEL's reach is r^2=32, so a sentinel can cover a site from outside
        the vision of the bot choosing it.  Coverage read here is a LOWER BOUND
        on real coverage; the arm cannot dodge a threat it cannot see.
        """
        rnd = ct.get_current_round()
        if self.sentsafe_rnd == rnd and self.sentsafe_cover is not None:
            return self.sentsafe_cover
        gun, sent, rot = set(), set(), set()
        nrot = 0
        try:
            for bid in ct.get_nearby_buildings():
                try:
                    ...
                    for t in ct.get_attackable_tiles_from(
                            gp, ct.get_direction(bid), et):
                        acc.add((t.x, t.y))
                    if (et == EntityType.GUNNER and LOKI_SENTSAFE_GUNROT_ON
                            and nrot < LOKI_SENTSAFE_GUNROT_MAX):
                        nrot += 1
                        for rd in ROT_DIRS:
                            for t in ct.get_attackable_tiles_from(
                                    gp, rd, EntityType.GUNNER):
                                rot.add((t.x, t.y))
                except Exception:
                    continue
        except Exception:
            pass
        self.sentsafe_rnd, self.sentsafe_cover = rnd, (gun, sent, rot)
        return self.sentsafe_cover
