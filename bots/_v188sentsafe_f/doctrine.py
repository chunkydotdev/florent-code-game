...
# Near their core a sentinel's 32-radius line can blanket every site that can
# fire on the core, in which case avoiding it means never planting -- worse
# than dying.  Both classes are scored, both weights live below, and the
# measured funnel decides them (see the leg note in the wrap).  Because it is a
# soft score it degrades gracefully: when every candidate is covered the least
# covered one is still built.
#
...
# Measured to be the term that carries the dose: with it off the arm redirects
# 3.4% of forward-sentinel builds, with it on 26.9% (409 builds, 192 games).
LOKI_SENTSAFE_GUNROT_ON = True
