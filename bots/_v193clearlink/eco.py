    ...
    def _build_next_link(self, ct):
        ...
        while self.link_queue:
            ...
            if occupied:
                ...
                if mine or not LOKI_LINKCLEAR_ON:
                    self.link_queue.pop(0)
                    continue
                # LOKI-LINKCLEAR: an enemy building is squatting on our own
                # trunk tile.  The old code popped and walked past it,
                # leaving a permanent hole (the L4 defect).  Peck it instead
                # of skipping it -- do NOT pop the tile off the queue.
                dist = abs(p.x - tile.x) + abs(p.y - tile.y)
                if (
                    dist == 1
                    and ct.get_action_cooldown() == 0
                    and ct.get_global_resources() >= LOKI_LINKCLEAR_TI_FLOOR
                ):
                    try:
                        if ct.can_fire(tile):
                            ct.fire(tile)
