...
class EcoMixin:
    ...
    def _link_path(self, ct, hpos):
        ...
        ban = self._pave_ban()
        if LOKI_LINKROUTE_ON and self.link_ban:
            # LOKI-LINKROUTE: fold in trunk tiles banned by an enemy
            # blocker, on top of whatever the heal-seat ban already
            # excludes.  Both downstream uses of `ban` (the blocked set in
            # the grid BFS and the inline check in the fallback BFS) then
            # treat these tiles as impassable exactly like a pave-banned
            # seat.
            ban = (set(ban) if ban is not None else set()) | self.link_ban
        if ban is not None:
            raw_goals -= ban
        start = (hpos.x, hpos.y)
    ...
    def _build_next_link(self, ct):
        ...
        while self.link_queue:
            ...
            try:
                ...
                return False
            occupied = bid is not None
            if occupied:
                mine = True
                try:
                    mine = (ct.get_team(bid) == ct.get_team())
                except Exception:
                    mine = True  # UNKNOWN -> behave exactly as today. Fail closed.
                if mine or not LOKI_LINKROUTE_ON:
                    self.link_queue.pop(0)
                    continue
                # LOKI-LINKROUTE: an enemy building is squatting on our own
                # trunk tile.  The old code popped and walked past it,
                # leaving a permanent hole (the L4 defect).  Ban the tile
                # and re-plan the whole chain around it next turn instead.
                self.link_ban.add((tile.x, tile.y))
                self.link_queue = []
                return False
