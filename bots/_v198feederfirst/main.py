...
class Player(EcoMixin, RaidMixin):
    ...
    def _turret(self, ct):
        ...
        try:
            # LOKI-FEEDER (#45): positions of enemy turrets in this turret's
            # vision, so an enemy builder standing beside one (the feeder that
            # rebuilds it in 1-2 rounds) can outrank the turret itself.
            enemy_turrets = []
            if LOKI_FEEDER_FIRST:
                for bid2 in ct.get_nearby_buildings():
                    try:
                        if (ct.get_team(bid2) != self.team
                                and ct.get_entity_type(bid2) in
                                (EntityType.GUNNER, EntityType.SENTINEL)):
                            enemy_turrets.append(ct.get_position(bid2))
                    except Exception:
                        continue
            ...
            for t in ct.get_attackable_tiles():
                ...
                if (LOKI_FEEDER_FIRST and et == EntityType.BUILDER_BOT
                        and any(t.distance_squared(q) <= LOKI_FEEDER_NEAR_DSQ
                                for q in enemy_turrets)):
                    prio = 0.5    # feeder: above turrets, below the core
                if prio < best_prio:
                    best_prio, best = prio, t
            if best is not None:
                if LOKI_FEEDER_LOG and best_prio == 0.5:
                    print(f"FEEDER45 r={ct.get_current_round()} "
                          f"t=({best.x},{best.y})")
                ct.fire(best)
                return
        except Exception:
