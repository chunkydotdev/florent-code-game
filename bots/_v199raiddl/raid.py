...
class RaidMixin:
    ...
    def _raid_open(self, ct, rnd, established):
        ...
        if established:
            return True
        # s36 arm: the deadline is an absolute round fitted when the longest
        # walk was ~26 tiles (25x25). Scale it by the map diagonal so a 30x30
        # keeps the same TIME-PAST-ARRIVAL budget instead of the same clock:
        # deadline = 150 * (mw+mh)/50, i.e. exactly 150 on a 25x25.
        deadline = LOKI_COLD_INSERT_RND * (self.mw + self.mh) / 50.0
        if rnd < deadline:
            return True
