...
class EcoMixin:
    ...
    def _expand(self, ct):
        ...
        try:
            ...
            on_ore = False
        # ORE_STEPOFF_MIN_WALLS gate dropped (s36 arm): a builder standing on
        # ore is the one unit that can never mine that tile, regardless of how
        # many walls the map has. The old >=80 condition was dead on maps with
        # a populated grid but fewer walls (midgard: 66) and always-dead
        # pre-MAPCODE (map_walls empty). Step-off on on_ore alone.
        if on_ore:
            for d in CARDINALS:
                n = p.add(d)
                if not (0 <= n.x < self.mw and 0 <= n.y < self.mh):
