...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        self.idx = 0
        self.role = "expand"
        # UNDERECO arm (s36): round the current siege latch FIRST engaged, or
        # None. A permanent ring camp (Leviathan) latches `under` for entire
        # games; after ~100 rounds a siege is CHRONIC and the harvester-rebuild
        # reserve must return — 750 rounds at a 12 Ti floor rebuilt nothing and
        # fed nothing (autopsy 18f30710, all four losses).
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            if under and self.under_since is None:
                ...
                self.under_since = None
            # Chronic siege (>100 rounds latched): restore the harvester
            # reserve. The skip exists for the ACUTE phase — spend on ammo,
            # fight — but a camp that latches `under` permanently otherwise
            # disables eco recovery forever (bank pinned at exactly 12 Ti for
            # 750 rounds vs Leviathan).
