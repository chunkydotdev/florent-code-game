...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        # STANDOFF (s36, #41): collect ALL legal (site, target) pairs, then
        # prefer the FARTHEST site with d^2 >= 14 to the nearest core tile.
        # Basis (both autopsies): 30/31 of our forward-turret deaths involve a
        # counter-GUNNER built next door (d^2=1 in 31/52 pairs, reach r^2=13,
        # blocked by obstacles); our sentinel reaches r^2=32 THROUGH obstacles.
        # A site at d^2 14-32 forces them to WALK a builder out (+3..+29-round
        # new-build latency) instead of planting the next-door answer.
        # Flag off = original first-fit scan, byte-for-byte behaviour.
        cands = []
        for d in CARDINALS:
            bp = p.add(d)
            ...
            for target in tiles:
                ...
                if not LOKI_STANDOFF_SITING:
                    ...
                    return True
                cands.append((min(bp.distance_squared(c) for c in tiles), bp, facing))
        if not cands:
            return False
        far = [c for c in cands if c[0] >= LOKI_STANDOFF_MIN_DSQ]
        _d2, bp, facing = max(far) if far else max(cands)
        ct.build_sentinel(bp, facing)
        if LOKI2B_LIVE_CAP_ON:
