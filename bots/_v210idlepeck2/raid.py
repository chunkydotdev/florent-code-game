...
class RaidMixin:
    ...
    def _raid(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            self._was_idlepeck = False
            if self._raid_act(ct, E, near):
                # LOKI-IDLEPECK (#48): the idle clock resets only on REAL
                # ladder actions — an idle-peck must not reset it, or the
                # peck would fire once per threshold instead of every round.
                if not self._was_idlepeck:
                    self._prog_rnd = ct.get_current_round()
                return
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 8. TERMINAL-IDLE CORE PECK (QUEUE #48). A raider whose whole ladder
        # has been empty for LOKI_IDLE_PECK_RNDS consecutive rounds, standing
        # beside the enemy core, pecks it. The quiet doctrine (LOKI_QUIET_ON)
        # governs ACTIVE raiders and is untouched; this reopens fire only for
        # a body that has proven it has nothing else to spend the round on.
        # The autopsy arithmetic (4b039a9c): parked bots stood at d²≤2 for
        # 300-850 rounds — at 2 dmg/round that is 600-1700 into a 500 HP core.
        if LOKI_IDLE_PECK and ti >= LOKI_PECK_TI_FLOOR:
            rnd = ct.get_current_round()
            last = getattr(self, "_prog_rnd", None)
            if last is None:
                self._prog_rnd = rnd          # first empty round starts the clock
            elif rnd - last >= LOKI_IDLE_PECK_RNDS:
                # Iteration 2: core-adjacency is starved when enemy bodies
                # or barriers hold the core seats (camper dose: 1/12 fired),
                # so the idle round is spent on the best adjacent enemy
                # BUILDING instead — core first, else the barrier/whatever
                # standing between this body and the core. Chewing the ring
                # at 2 dmg/round reopens the seat this bot will then use.
                targets = []
                for c in core_tiles(E):
                    if abs(p.x - c.x) + abs(p.y - c.y) == 1:
                        targets.append((0, c))
                for d in CARDINALS:
                    t = Position(p.x + d.delta()[0], p.y + d.delta()[1])
                    try:
                        bid = ct.get_tile_building_id(t)
                        ...
                        continue
                for _, t in sorted(targets, key=lambda x: x[0]):
                    try:
                        if ct.can_fire(t):
                            self._was_idlepeck = True
                            if LOKI_IDLE_PECK_LOG:
                                print(f"IDLEPECK48 r={rnd}")
                            ct.fire(t)
                            return True
                    except Exception:
                        continue
