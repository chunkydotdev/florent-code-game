...
# ============================================================================
# LOKI-43 -- BARRIER THE ENEMY GUNNER'S RAY  (arm _v181gunblock)
#
# WHAT IS ALREADY HERE.  raid.py's station scorer already computes, from
# `get_attackable_tiles_from`, the set of tiles on every visible enemy GUNNER's
# firing line (LOKI-25) -- and uses it for exactly one thing: pushing raiders
# OFF those tiles (LOKI_GUNAXIS_PENALTY).  This is the other half of the same
# read.  The gunner is the AVOIDABLE turret: its shot is a straight line that
# obstacles BLOCK, where a sentinel's ignores them.  92% of our forward builder
# deaths are enemy gunners against 42.8% for the rest of the league.
#
# WHY A BARRIER AND NOT A HEAL.  A heal is 0.25 Ti/HP and a barrier ~0.37, so
# on titanium the heal wins -- but the heal costs a BUILDER ACTION EVERY ROUND
# and the barrier costs ONE.  Actions, not titanium, are the scarce good on a
# raider: it can act or move, never both.  A barrier is ~7.5x the absorption
# per action spent.
#
# AND IT IS ONE ACTION, WHICH IS WHY IT IS NOT _v178salt.  Salt failed its
# kill-round bar (median 179 against a pooled 129, Mann-Whitney p=0.008)
# because pecking a conveyor to death costs ~10 raider actions.  Everything
# below is bounded so that the same failure cannot recur: at most
# LOKI_GUNBLOCK_MAX placements per raider for the whole match, and the whole
# behaviour sits BELOW the forward-sentinel attempt in _raid_act so a round
# that could plant a sentinel still plants it (DEFENCE_ADMISSION_BAR:
# kill_round_non_regression).
#
# THE SCALE PRICE IS REAL.  Every barrier is +1% on the ONE global additive
# cost factor, permanently, and our scale already runs 313-380%.  That factor
# is what prices the launcher out (LAUNCHER_RESERVE 80 + a scaled cost against
# a median r150-200 bank of 54).  Hence a per-unit cap AND a bank floor, not
# just one of the two.
# ============================================================================

LOKI_GUNBLOCK_ON = True       # master flag -- False is the parent exactly
LOKI_GUNBLOCK_MAX = 2         # ray barriers per raider, whole match
LOKI_GUNBLOCK_TI_FLOOR = 20   # bank left standing AFTER the barrier is paid
LOKI_GUNBLOCK_STALE = 8       # rounds a cached ray read stays usable
LOKI_GUNBLOCK_SHIELD_REQ = False   # LOKI-46: THE ARM.  See the block below.
LOKI_GUNBLOCK_LOG = True      # replay tag "GBNS45 LOKI43GB x y rnd" -- dose readout


# ============================================================================
# LOKI-46 -- THE SAME BLOCK WITHOUT THE SHIELD CLAUSE  (arm _v186gunblocknoshield)
#
# ONE LINE OF DIFFERENCE against _v181gunblock: `LOKI_GUNBLOCK_SHIELD_REQ`.
# LOKI-43 would only place a barrier on an enemy gunner's ray when something of
# OURS already stood further along that same ray -- provably cover.  This arm
# places on the ray unconditionally.
#
# WHY THAT IS NOT OBVIOUSLY WORSE.  The shield test is evaluated on the one
# round a raider happens to be orthogonally adjacent to the ray, and it asks a
# question about the PRESENT.  A raid is a stream of bodies down the same
# approach: a barrier in front of nothing this round is in front of the next
# raider ten rounds later, and it never stops being an obstacle a 3 Ti purchase
# forces them to spend 15 builder pecks to remove.  Measured, dropping the
# clause raises the dose 4.2x, 0.20 -> 0.83 barriers/game.
#
# WHY IT MIGHT STILL BE WORSE, and this is what the leg has to separate.  The
# dose figure for LOKI-43 is a FLOOR measured in self-play, and self-play is
# the wrong fixture for it in a known direction: we are the least gunner-heavy
# team in the field by 4-11x, so our own bots barely present the rays this
# reads.  Against a real gunner team the shielded version's dose rises too, and
# the precision it buys may come back.  A same-or-better read here retires
# `_gb_asset` and the per-ray rear scan as complexity we pay for nothing; a
# clearly worse read is a direct measurement that the clause is load-bearing.
#
# The kill-round bar is unchanged and binding: the block sits BELOW the
# forward-sentinel attempt in _raid_act, and a rise in median kill round is
# disqualifying whatever it does to the win rate.
# ============================================================================
