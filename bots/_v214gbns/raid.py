...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 3b. LOKI-43/46: BARRIER THE GUNNER'S RAY.  Strictly BELOW the
        # sentinel attempt above -- a round that could plant a sentinel must
        # still plant it -- and below sealing, so the collar is never outbid
        # by cover.
        if LOKI_GUNBLOCK_ON and self._gunblock(ct, E):
            return True
    ...
    # --- LOKI-43: barrier the gunner's ray ---------------------------------

    def _gb_rays(self, ct, rnd):
        """Visible enemy GUNNER firing lines, as (gunner_pos, ray_tiles).

        _raid_station already makes exactly this read once per rescan and
        publishes it above; this returns that cache.  It only rescans when the
        cache has gone stale, which it can, because _raid_station runs in the
        MOVE phase and _raid_act runs before it -- a raider that acts every
        round would otherwise never see a ray at all.  A gunner's facing is
        near-static (rotate() costs 10 Ti AND a full action cooldown), so a
        few rounds of staleness costs at worst one misplaced 3 Ti barrier.
        """
        if rnd - self.gb_rnd <= LOKI_GUNBLOCK_STALE:
            return self.gb_rays
        rays = []
        try:
            for bid in ct.get_nearby_buildings():
                ...
                rays.append((gp, ct.get_attackable_tiles_from(
                    gp, gd, EntityType.GUNNER)))
        except Exception:
            pass
        self.gb_rays, self.gb_rnd = rays, rnd
        return rays

    def _gb_asset(self, ct, q, me):
        """Is something of OURS standing on q for that ray to hit?

        Buildings first (a forward Sentinel is a 40 HP asset that keeps firing
        after every body is dead), then builder bots -- which includes THIS
        raider when it is itself on the line.
        """
        if not (0 <= q.x < self.mw and 0 <= q.y < self.mh):
            return False
        try:
            ...
            if uid is not None and ct.get_team(uid) == me:
                return True
        except Exception:
            return False
        return False

    def _gunblock(self, ct, E):
        """Put a barrier ON a gunner's ray.

        LOKI-46 -- THE SHIELD CLAUSE IS OFF IN THIS TREE.  _v181gunblock's
        trigger was (a) orthogonally adjacent EMPTY tile, (b) in the enemy half
        tested ON THE TILE, (c) on a visible enemy gunner's firing line, and
        (d) strictly NEARER that gunner than some friendly entity further along
        the same line.  `LOKI_GUNBLOCK_SHIELD_REQ` drops (d): the barrier goes
        down on the ray whether or not anything of ours is behind it yet.

        THE TRADE, and it is the whole point of the arm.  (d) is what made the
        placement provably cover rather than decoration -- a gunner's shot
        stops on the first obstacle, so 30 HP of barrier absorbs what would
        have hit the asset.  But it is also evaluated at the ONE ROUND the
        raider happens to be adjacent to the ray, and a raid is a stream of
        bodies down that same ray: cover placed in front of nothing now is
        cover in front of the next raider ten rounds later.  Measured 4.2x the
        dose (0.20 -> 0.83/game) at the price of some barriers that shield
        nobody.  If the cheap version measures the same or better, `_gb_asset`
        and the per-ray rear scan are complexity we are paying for nothing.
        """
        if E is None or self.core is None:
            return False
        if self.gb_spent >= LOKI_GUNBLOCK_MAX:
            return False
        cost = ct.get_barrier_cost()
        if ct.get_global_resources() < cost + LOKI_GUNBLOCK_TI_FLOOR:
            return False
        if self._cpu_exhausted(ct):
            return False
        rnd = ct.get_current_round()
        rays = self._gb_rays(ct, rnd)
        if not rays:
            return False
        p = ct.get_position()
        me = self.team
        need_shield = LOKI_GUNBLOCK_SHIELD_REQ
        for d in CARDINALS:
            t = p.add(d)
            if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                continue
            # ENEMY HALF, tested on the TILE and not on the body.
            if t.distance_squared(E) >= t.distance_squared(self.core):
                continue
            for gp, ray in rays:
                dt = gp.distance_squared(t)
                if dt == 0:
                    continue
                on_ray = False
                shields = not need_shield
                for q in ray:
                    if q.x == t.x and q.y == t.y:
                        on_ray = True
                        if not need_shield:
                            break     # nothing else to learn from this ray
                        continue
                    if gp.distance_squared(q) <= dt:
                        continue          # in front of the barrier, not behind
                    if self._gb_asset(ct, q, me):
                        shields = True
                if not (on_ray and shields):
                    continue
                try:
                    if not ct.can_build_barrier(t):
                        break
                    ...
                    break
                self.gb_spent += 1
                if LOKI_GUNBLOCK_LOG:
                    print("GBNS45", "LOKI43GB", t.x, t.y, rnd,
                          t.distance_squared(E), t.distance_squared(self.core))
                return True
        return False
            ...
            for bid in ct.get_nearby_buildings():
                ...
                # never once invoked.
                    ...
                    try:
                        gp = ct.get_position(bid)
                        gd = ct.get_direction(bid)
                        ray = ct.get_attackable_tiles_from(
                            gp, gd, EntityType.GUNNER)
                        for t in ray:
                            gun_axis.add((t.x, t.y))
                        if LOKI_GUNBLOCK_ON:
                            # LOKI-43 reuses THIS read rather than making its
                            # own: same predicate, same round, one scan.
                            rays.append((gp, ray))
                    except Exception:
                        continue
        except Exception:
