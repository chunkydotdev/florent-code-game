    ...
    def _launcher_turn(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            try:
                ...
                continue
            ray = set()
            if LOKI_AIMEDTHROW_ON:
                for sid in ct.get_nearby_buildings():
                    try:
                        if ct.get_entity_type(sid) != EntityType.SENTINEL \
                                or ct.get_team(sid) != self.team:
                            continue
                        sp2 = ct.get_position(sid)
                        ddx, ddy = ct.get_direction(sid).delta()
                        for k in range(1, 7):
                            rx, ry = sp2.x + k * ddx, sp2.y + k * ddy
                            if (rx - sp2.x) ** 2 + (ry - sp2.y) ** 2 > 32:
                                break
                            ray.add((rx, ry))
                    except Exception:
                        continue
            far = sorted(
                sites,
                key=lambda t: (((t.x, t.y) in ray), t.distance_squared(self.core)),
                reverse=True,
            )
            for site in far:
                try:
