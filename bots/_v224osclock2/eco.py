...
from doctrine import LOKI_OSC_DETECT_ON, LOKI_OSC_K, LOKI_OSC_TGT_BAN_RNDS, LOKI_OSC_TILE_BAN_RNDS
...
class EcoMixin:
        ...
        if self.map_ores and self.role != "defend":
            ...
            for _ in range(len(assigned)):
                ...
                try:
                    ...
                    pass
                bans = getattr(self, "osc_bans", None)
                if bans and bans.get((t.x, t.y), 0) > ct.get_current_round():
                    continue
                return t
    ...
    def _bfs_direction(self, ct, target):
        ...
        if bans:
            ...
            blocked.update(k for k, until in bans.items() if until > rnd)
    ...
    def _nav(self, ct, pave=True):
        if self.tgt is None or ct.get_move_cooldown() != 0:
            return
        if LOKI_OSC_DETECT_ON:
            p_now = ct.get_position()
            h = getattr(self, "osc_hist", None)
            if h is None:
                h = self.osc_hist = []
            h.append((p_now.x, p_now.y))
            if len(h) > LOKI_OSC_K:
                del h[0]
            if len(h) == LOKI_OSC_K:
                a, b = h[-1], h[-2]
                if a != b and all(
                    h[i] == (a if (LOKI_OSC_K - 1 - i) % 2 == 0 else b)
                    for i in range(LOKI_OSC_K)
                ):
                    del h[:]
                    rnd = ct.get_current_round()
                    bans = getattr(self, "osc_bans", None)
                    ...
                    if self.tgt is not None:
                        bans[(self.tgt.x, self.tgt.y)] = rnd + LOKI_OSC_TGT_BAN_RNDS
                    bans[a] = rnd + LOKI_OSC_TILE_BAN_RNDS
                    bans[b] = rnd + LOKI_OSC_TILE_BAN_RNDS
                    self.tgt = None
                    self.stuck = 5
                    return
