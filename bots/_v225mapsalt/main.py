...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not hasattr(self, "spawn_salt"):
            # x3r0's per-map salt table (v141 "Counter Router", grafted with
            # credit): terrain-fingerprinted constants from two independent
            # colour-swapped 8-salt sweeps. glacierkeep RE-KEYED to OUR
            # corrected table's grid (his 446389448 hashes the stale grid);
            # valkyrie's tuned value was 0 = the default, no re-key needed.
            # Unknown maps keep the old OS-entropy salt (and its NOISE_ON
            # battery-honesty role).
            if getattr(self, "map_grid", None):
                sig = 2166136261
                ...
                self.spawn_salt = {
                    ...
                    2948331259: 7,   # glacierkeep (corrected-table grid)
