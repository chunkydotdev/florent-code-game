    ...
    def _core(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            if (et in (EntityType.GUNNER, EntityType.SENTINEL) and d <= 64) or (
                ...
                # 0033 book: their first attack structure is a BARRIER inside
                # our spawn ring (median r43), the gunner creep follows at
                # r50-150. A barrier at d2<=16 IS the attack starting.
                LOKI_NESTSHOT_ON and et == EntityType.BARRIER and d <= 16
            ):
                under = True
        ...
        if ct.read_store(SLOT_HARVESTERS) < ECO_NEED and self._live_home_gun(ct):
            ...
            if LOKI_NESTSHOT_ON:
                try:
                    ...
                    nest = False
            if not nest and not self._core_shelled(ct):
                return False
        p = ct.get_position()
        ban = self._seat_ban()
        choices = (
