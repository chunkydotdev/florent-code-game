...
# --- #14 LOKI-DEST: the idle builder gets a DESTINATION, not a stand (s39) ---
# Bucket A (idle-and-free: both cooldowns 0, no verb -- idle_split_s31, read
# off the wire) is 25.76% of our builder-rounds before core death (27.59% in
# wins, so not a losing-position artefact).  OSCLOCK2 (46.39 @ n=1800, DROP)
# freed loop-locked builders back into _pick, which re-picks the same failing
# plan; freed-into-idleness is the strengthened hypothesis this A/B pair
# tests.  Detector: LOKI14_IDLE_K consecutive movement-tail entries with both
# cooldowns 0 and no position change (a free tail entry proves a no-action
# round: acting sets the cooldown to 1 immediately, engine-probed s39).
# Registered directional prediction: arms A ("dest") and B ("recall") move
# median kill round in OPPOSITE signs from the same idle pool.  Arm B
# additionally carries the DEFENCE_ADMISSION_BAR kill-round non-regression
# rider at read time.  Scope: EXPAND-role builders only -- the parked-raider
# slice of bucket A is #48(c)'s road, deliberately not raced here.
LOKI14_IDLE_DEST_ON = True
LOKI14_MODE = "recall"        # "dest" (arm A) | "recall" (arm B)
LOKI14_IDLE_K = 4           # consecutive measured bucket-A rounds to trigger
LOKI14_NEAR_DSQ = 8         # inside this of the destination: stand, no churn
