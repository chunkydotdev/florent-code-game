    ...
    def _expand(self, ct):
        ...
        if self.link_queue:
            ...
            if p.x == nxt.x and p.y == nxt.y:
                ...
                self._nav(ct, pave=False)
            return
        if LOKI14_IDLE_DEST_ON:
            # LOKI-14 detector, denominated in the WIRE's own idle definition
            # (idle_split_s31: bucket A = both cooldowns 0, no verb emitted).
            # Reaching this movement tail with action cooldown 0 PROVES no
            # action was taken this round (acting sets it to 1 immediately,
            # engine-probed s39); two consecutive free tail entries without a
            # position change = one measured bucket-A round.  _pick is never
            # None in practice (probed: 0 sinks in 4 games incl. a 443-round
            # one), so idleness here means a plan that is not progressing --
            # the OSCLOCK2 lesson is that re-picking the same failing plan
            # frees nobody; the DESTINATION is what changes.
            rnd14 = ct.get_current_round()
            free14 = ct.get_action_cooldown() == 0 and ct.get_move_cooldown() == 0
            if (free14 and getattr(self, "d14_free_rnd", -9) == rnd14 - 1
                    and getattr(self, "d14_pos", None) == (p.x, p.y)):
                self.d14_run = getattr(self, "d14_run", 0) + 1
            elif getattr(self, "d14_pos", None) != (p.x, p.y):
                self.d14_run = 0
            if free14:
                self.d14_free_rnd = rnd14
            self.d14_pos = (p.x, p.y)
            if getattr(self, "d14_run", 0) >= LOKI14_IDLE_K and self.core is not None:
                dest = (enemy_core_for(self.mw, self.mh, self.core)
                        if LOKI14_MODE == "dest" else self.core)
                if dest is not None and p.distance_squared(dest) > LOKI14_NEAR_DSQ:
                    self.tgt = dest
                    self.stuck = 0
                    self.wall = None
                    self.d14_run = 0
