...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 2b. LOKI-41: a fresh forward sentinel's pending denial barrier
        # completes before any next plant -- one 3-Ti barrier on the core-side
        # adjacent seat (sentinel fire ignores obstacles; our own line is safe).
        if self._try_fwd_barrier(ct, E):
            return True
    ...
    def _try_fwd_barrier(self, ct, E):
        """LOKI-41: deny the point-blank counter-build seat beside a fresh
        forward sentinel with one 3-Ti barrier on its core-side orthogonal
        neighbour.  A Sentinel's shot ignores obstacles so the barrier cannot
        block our own line; the enemy's reactive Gunner (r^2=13, obstacle-
        blocked) cannot say the same, which is what makes the seat worth 3 Ti.
        """
        if not LOKI41_BAR_ON:
            return False
        due = getattr(self, "fwd_bar_due", None)
        if due is None:
            return False
        rnd = ct.get_current_round()
        if rnd - getattr(self, "fwd_bar_rnd", rnd) > LOKI41_BAR_WINDOW:
            self.fwd_bar_due = None
            return False
        try:
            if ct.get_global_resources() < ct.get_barrier_cost():
                return False
            s = Position(due[0], due[1])
            bid = ct.get_tile_building_id(s)
            if bid is None or ct.get_entity_type(bid) != EntityType.SENTINEL \
                    or ct.get_team(bid) != self.team:
                self.fwd_bar_due = None
                return False
            anchor = min(core_tiles(E), key=lambda c: s.distance_squared(c))
            sd = s.distance_squared(anchor)
            p = ct.get_position()
            for d in CARDINALS:
                t = p.add(d)
                if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                    continue
                if abs(t.x - s.x) + abs(t.y - s.y) != 1:
                    continue
                if t.distance_squared(anchor) >= sd:
                    continue
                if ct.can_build_barrier(t):
                    ct.build_barrier(t)
                    self.fwd_bar_due = None
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        for d in CARDINALS:
            ...
            for target in tiles:
                ...
                if d2t > 32:
                    continue
                if LOKI41_BAND_ON and d2t < LOKI41_MIN_DSQ:
                    continue
                ...
                if LOKI41_BAR_ON:
                    self.fwd_bar_due = (bp.x, bp.y)
                    self.fwd_bar_rnd = ct.get_current_round()
