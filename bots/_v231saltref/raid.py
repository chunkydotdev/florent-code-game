...
class RaidMixin:
    ...
    def _salt_turn(self, ct, E, p, ti):
        ...
        if LOKI_SALT_CUT_REFRESH_RNDS:
            # LOKI-48b: the peck budget is an ALLOWANCE, not a lifetime cap --
            # forgiven every LOKI_SALT_CUT_REFRESH_RNDS rounds so a parked
            # raider's cutting pressure is sustained instead of exhausting at
            # exactly LOKI_SALT_CUT_MAX pecks and going silent forever.
            _ep = rnd // LOKI_SALT_CUT_REFRESH_RNDS
            if _ep != getattr(self, "salt_epoch", _ep):
                self.salt_pecks = 0
            self.salt_epoch = _ep
