...
class EcoMixin:
    ...
    # --- LOKI-52: COLLAR MEDIC ---------------------------------------------
    # Three methods, all additive.  `_collar_patient` picks and claims,
    # `_collar_medic_heal` is the ACTION half, `_collar_medic_move` is the
    # DISPATCH/STAY half.  Doctrine block: LOKI-52 in doctrine.py.
    #
    # THE EXCHANGE, at 4:1 (NOT the 8:1 the older comment above claims):
    # our heal is 1 Ti for +4 HP = 0.25 Ti/HP; their peck is 2 Ti for 2 dmg =
    # 1.00 Ti/HP; undoing one peck costs us 0.5 Ti against their 2 Ti.

    def _collar_eligible(self, ct):
        """Is this body allowed to medic at all?

        Home-band ECO bodies only.  A raider is excluded BY ROLE, not by
        position, so a stood-down raider standing on our own ore (raid.py
        falls through to `_expand` with role still == "raid") is never
        recruited -- arrival forward is the scarce quantity on this line.
        """
        if not LOKI_COLLAR_MEDIC_ON or self.core is None:
            return False
        if self.role == "raid":
            return False
        if ct.get_global_resources() < LOKI_CMEDIC_TI_FLOOR:
            return False
        try:
            return ct.get_position().distance_squared(self.core) <= LOKI_CMEDIC_HOME_DSQ
        except Exception:
            return False

    def _collar_rivals(self, ct, patient):
        """How many home builders are strictly better placed than us.

        Local staffing cap: no store slot (all 16 are taken) and no round of
        write-buffer latency.  Every builder computes the same ordering by
        (distance_squared to patient, entity id), so the two nearest claim and
        the rest keep working.  Fails CLOSED -- an unreadable neighbour list
        returns the cap, which declines the job.
        """
        try:
            p = ct.get_position()
            me = ct.get_id()
            mine = p.distance_squared(patient)
            n = 0
            for uid in ct.get_nearby_units():
                if uid == me:
                    continue
                ...
                if ct.get_entity_type(uid) != EntityType.BUILDER_BOT:
                    continue
                up = ct.get_position(uid)
                if self.core.distance_squared(up) > LOKI_CMEDIC_HOME_DSQ:
                    continue
                d = up.distance_squared(patient)
                if d < mine or (d == mine and uid < me):
                    n += 1
                    if n >= LOKI_CMEDIC_MAX:
                        return n
            return n
        except Exception:
            return LOKI_CMEDIC_MAX

    def _collar_patient(self, ct):
        """The damaged friendly home building this builder should staff."""
        if not self._collar_eligible(ct):
            return None
        p = ct.get_position()
        ...
        for bid in buildings:
            try:
                if ct.get_team(bid) != self.team:
                    continue
                if ct.get_entity_type(bid) not in LOKI_CMEDIC_TYPES:
                    continue
                bp = ct.get_position(bid)
                if self.core.distance_squared(bp) > LOKI_CMEDIC_HOME_DSQ:
                    continue
                d = p.distance_squared(bp)
                if d > LOKI_CMEDIC_REACH_DSQ:
                    continue
                hp, mx = ct.get_hp(bid), ct.get_max_hp(bid)
            except Exception:
                continue
            if hp > mx - LOKI_CMEDIC_MIN_DMG:
                continue
            # Nearest first, then the most wounded, then id -- every builder
            # must break the tie the same way or the cap does not hold.
            key = (d, hp, bid)
            if best_key is None or key < best_key:
                best_key, best = key, bp
        if best is None:
            return None
        if self._collar_rivals(ct, best) >= LOKI_CMEDIC_MAX:
            return None
        return best

    def _collar_medic_heal(self, ct):
        """ACTION half: repair the worst wounded building we are beside."""
        if not self._collar_eligible(ct):
            return False
        p = ct.get_position()
        best, best_key = None, None
    ...
    def _collar_seat(self, ct, patient):
        """A tile orthogonally beside the patient that we could stand on.

        Target SELECTION only -- the walking is still `_nav`/`_bfs_direction`.
        Picked explicitly because a CONVEYOR/SPLITTER is not in the BFS blocked
        set, so handing the patient tile itself to `_nav` would aim the walk at
        a tile no builder can occupy.
        """
        p = ct.get_position()
        ...
        for d in CARDINALS:
            ...
            if best_d is None or (dd, t.x, t.y) < (best_d, best.x, best.y):
                best, best_d = t, dd
        return best

    def _collar_medic_move(self, ct):
        """DISPATCH/STAY half.  True == this builder's turn is spent here.

        STAY is the half the incumbent is missing as much as dispatch: a
        builder that healed a barrier this round and then walked off to ore
        hands the tile straight back to the pecker.
        """
        patient = self._collar_patient(ct)
        if patient is None:
            if self.cmedic:
                self.cmedic = False
                self.tgt = None
        ...
        self.cmedic = True
        p = ct.get_position()
        if abs(p.x - patient.x) + abs(p.y - patient.y) == 1:
            return True                      # STAY: already on station
        if ct.get_move_cooldown() != 0:
            return True
        seat = self._collar_seat(ct, patient)
        if seat is None:
            # No standable tile beside the patient -- hand the turn back to the
            # ore walk, and clear the medic's target with it so a stale seat
            # cannot steer the next `_nav`.
            self.cmedic = False
            self.tgt = None
            self.stuck = 0
            self.wall = None
        ...
        for d in CARDINALS:
            ...
            try:
                ...
                if bid is None or ct.get_team(bid) != self.team:
                    continue
                et = ct.get_entity_type(bid)
                if et in (EntityType.CORE, EntityType.SPLITTER):
                    return False
                if et == EntityType.CONVEYOR:
                    out = t.add(ct.get_direction(bid))
                    if (out.x, out.y) != (hpos.x, hpos.y):
                        return False
            except Exception:
                continue
        return True
    ...
    def _l4_repair(self, ct):
        ...
        for d in CARDINALS:
            g = p.add(d)
            if not (0 <= g.x < self.mw and 0 <= g.y < self.mh):
                continue
            if (
    ...
    def _expand(self, ct):
            ...
            if not endgame and ct.get_global_resources() >= MEDIC_TI_FLOOR and (
                ...
                for d in CARDINALS:
                    ...
                    try:
                        ...
                        continue
            # COLLAR MEDIC (LOKI-52), ACTION HALF.  Ranked LAST in the action
            # block so nothing above it changes: everything the CHAIN MEDIC
            # would have healed, it still heals, on the same round, with the
            # same gates.  What this adds is the rounds the chain medic
            # declines -- before MEDIC_EARLY_MIN_RND, under MEDIC_TI_FLOOR, or
            # on a BARRIER / turret / launcher, none of which is in MEDIC_TYPES.
            if not endgame and self._collar_medic_heal(ct):
                return
        ...
        # COLLAR MEDIC (LOKI-52), DISPATCH/STAY HALF.  Below the Core
        # convergence (a shelled Core outranks a pecked barrier) and below
        # siphon hygiene, so it displaces no ACTION the incumbent would have
        # taken -- it only outranks the ORE WALK, which is the whole leak: the
        # ore walk wins by default and carries the only body that could heal
        # away from the thing being pecked.
        # Short by construction: patient inside builder vision, builder inside
        # the Core's -- QUEUE #54's nav oscillation makes a long dispatch a
        # builder that never arrives.
        if self._collar_medic_move(ct):
            return
