...
# ============================================================================
# LOKI-52: COLLAR MEDIC -- STAFF THE HEAL EXCHANGE WE ALREADY OWN
# ============================================================================
# THE ARITHMETIC, STATED CORRECTLY. `eco.py:_heal_adjacent` prices this "eight
# to one" and that number is WRONG; the exchange is FOUR to one and the fix
# belongs in that comment, not here (a separate change). Worked:
#   * our heal  : 1 Ti buys +4 HP  => 0.25 Ti per HP restored
#   * their peck: 2 Ti buys  2 dmg => 1.00 Ti per HP removed
#   * undoing one peck therefore costs us 0.5 Ti against their 2 Ti = 4:1.
# Four to one is still decisive, but it is NOT cosmetic here: the correction
# moved a threshold. LOKI_CMEDIC_MIN_DMG was 1 under the eight and is 4 under
# the four -- see the note on that constant below for the worked clipping
# arithmetic. Nothing else in this plank was sized off the exchange.
#
# THE LEAK. The exchange exists in this tree and is collected only BY ACCIDENT.
# `_heal_adjacent` (main.py `_home_defend`) and the CHAIN MEDIC (eco.py
# `_expand`) both require a builder to ALREADY be standing beside the wounded
# building. Nothing in the file ever WALKS a builder to one. A pecked home
# building therefore dies unattended whenever no builder happens to be next to
# it -- which is most of the time, because the expander's own logic is pulling
# it toward ore.
#
# THE CHANGE, and it is one condition, not a role: an ECO builder already
# inside the home band that can see a damaged friendly building claims it,
# walks to an adjacent tile using the EXISTING nav (`_nav`/`_bfs_direction`,
# which already resolves a blocked target to its orthogonal neighbours), and
# STAYS there healing until the building is whole again.
#
# WHY THE RADII ARE SMALL. QUEUE #54: the nav has a known oscillation defect,
# so a long dispatch is a builder that never arrives. Every distance here is
# inside builder vision (r^2=20), so a dispatch is at most a few steps and the
# patient is observed, not remembered.
LOKI_COLLAR_MEDIC_ON = False  # MUTATION ARM: the one-line ablation
# The builder AND the patient must both be within this of our own Core. 36 is
# the Core's own vision radius^2 -- "home" is exactly what the Core can see.
LOKI_CMEDIC_HOME_DSQ = 36
# ...and the patient must be within this of the BUILDER: builder vision is
# r^2=20, so anything further is not actually observed this round.
LOKI_CMEDIC_REACH_DSQ = 20
# At most this many medics on one patient at once. The cap is enforced locally
# (count visible home builders strictly nearer the patient, id as tie-break)
# rather than through a store slot -- all 16 slots are already spoken for, and
# a local count needs no round of write-buffer latency.
LOKI_CMEDIC_MAX = 2
# ⭐ THIS IS THE THRESHOLD THE 8:1 -> 4:1 CORRECTION MOVES, AND IT MOVED IT.
# A heal restores min(4, missing) HP for a flat 1 Ti, so the exchange DEGRADES
# as the wound gets shallower:
#     missing 1 -> 1 HP per Ti vs their 1 HP per Ti  = 1:1   (a wash)
#     missing 2 -> 2:1        missing 3 -> 3:1
#     missing 4+ -> 4:1       <- the full rate, nothing clipped
# Under the file's old "eight to one" even a 1-HP wound looked like 2:1 and
# "heal anything" was defensible. At the true 4:1 it is not: topping up a
# 1-HP scratch spends a builder's whole action for a Ti-neutral trade. So the
# floor is the depth at which the +4 is not clipped. Survival costs nothing by
# waiting -- a pecker removes 2 HP/round against our 4, so we win the race from
# any depth, and a 30 HP barrier is in no danger at 26.
LOKI_CMEDIC_MIN_DMG = 4
# Heal costs 1 Ti. This is a "can we pay" check, NOT a reserve: QUEUE #53 owns
# the question of whether a bank floor belongs on the heal budget, and
# LOKI_SEAL_TI_FLOOR is 0 in this tree, so adding one here would be that
# experiment smuggled into this one.
LOKI_CMEDIC_TI_FLOOR = 1
# Everything a builder can heal that is worth walking for. BARRIER is first
# because it is the asset the queue row names; the eco chain and the home
# turrets ride along because the dispatch is the same condition for all of them.
LOKI_CMEDIC_TYPES = (
    EntityType.BARRIER, EntityType.CONVEYOR, EntityType.SPLITTER,
    EntityType.HARVESTER, EntityType.GUNNER, EntityType.SENTINEL,
    EntityType.LAUNCHER,
)
# Instrumentation only: one print per collar-medic heal, into the replay.
# LOCAL ONLY -- platform replays strip stdout (see CLAUDE.md), so nothing may
# be read off a live leg with this.
LOKI_CMEDIC_LOG = True


# --- LOKI-52 heal instrument ------------------------------------------------
# ⛔ INSTRUMENT, NOT BEHAVIOUR. One line per heal LANDED, tagged by call site,
# into the LOCAL replay only -- platform-downloaded replays strip stdout
# (CLAUDE.md), so nothing may ever be read off a live leg with this.
#
# WHY IT EXISTS: "heals delivered per game" is the dose metric for this plank
# and NOTHING in tools/ can decode it. Checked, not assumed: the replay stream
# emits placeEntity ONCE per entity and never re-emits on HP change (0
# re-emissions and 0 HP deltas across a decoded local game), so there is no
# HP-restoration channel to read. stdout IS captured locally (the L4R tag from
# this same tree decodes out of the replay bytes), so a tagged print is the
# only instrument available.
#
# WHY IT IS ON IN BOTH ARMS: the control for the heal count is THIS TREE with
# LOKI_COLLAR_MEDIC_ON flipped to False, which is behaviour-identical to
# _v223sealrepair. Instrumenting one arm and not the other would make the
# comparison uninterpretable.
LOKI_HEALLOG_ON = True


def heal_log(ct, tag):
    try:
        if LOKI_HEALLOG_ON:
            print("HEALLOG52 %s r=%d" % (tag, ct.get_current_round()))
    except Exception:
        pass
