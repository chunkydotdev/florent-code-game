    ...
    def _raid_station(self, ct, E, near):
        ...
        for i, s in enumerate(stations):
            ...
            if i < ncorner:
                ...
                # it still has an unsealed seat beside it.  LOKI-53 TEMPO: the
                # pull toward an open corner is a doctrine constant -- the
                # incumbent's literal 6 loses to plain distance after a few
                # tiles, which is one mechanism behind our r51-median seal
                # against Juusto's r32 on the same open-at-r6 ring.
                if self._open_seats_by(ct, s) == 0:
                    score += LOKI_SEAL_CORNER_DONE_PUSH
                else:
                    score -= LOKI_SEAL_CORNER_OPEN_PULL
            else:
