...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 3b. LOKI-58: the FORWARD EVICTION LAUNCHER -- one, conditional on an
        # established siege (a live forward sentinel), planted beside their
        # core to evict the heal/repair staff during our kill window.  #47's
        # discipline inherited: cap 1, state-gated, never a standing build.
    ...
    def _try_evict_launcher(self, ct, E):
        """LOKI-58: plant ONE launcher beside the enemy Core once the siege is
        established.  Its _launcher_turn EXILE step then evicts every enemy
        builder that walks up to heal or rebuild -- the Jython/LingLing40
        mechanism (56-78 evictions of us per game, field-measured) pointed the
        other way.  State-gated on a live forward sentinel, cap 1 via a live
        census of the siege band plus a per-unit latch, Ti-floored so it never
        starves the kill."""
        if not LOKI58_ON or getattr(self, "l58_done", False):
            return False
        live = self._live_fwd_guns(ct, E)
        if not live:
            return False
        if ct.get_global_resources() < ct.get_launcher_cost() + LOKI58_TI_FLOOR:
            return False
        p = ct.get_position()
        tiles = core_tiles(E)
            ...
            for bid in ct.get_nearby_buildings():
                if (ct.get_team(bid) == me
                        ...
                        and min(ct.get_position(bid).distance_squared(c)
                                for c in tiles) <= LOKI58_CENSUS_DSQ):
                    self.l58_done = True
                    return False
        except Exception:
            return False
