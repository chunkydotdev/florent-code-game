...
# --- #60 x #54 LOKI-RETIRE: the unemployable builder retires its +20% (s39) --
# Registered against ~5.1% of the self-levied scale integral (research's
# deflated sizing: 11.58% locked builder-rounds x 44% builder share of a
# ~308-point/game tax), NOT the 44% headline.  Definition fixed pre-probe on
# the coordination tape.  Bars: kill-round non-regression + the scale curve
# read BESIDE the build-count curve from replay-kept batches (flat scale
# bought by building less is not a win).  Conservative floor by design: a
# never-acted, position-locked bot far from their ring is unambiguously
# finished; false retirement ~ 0.
LOKI60_RETIRE_ON = True
LOKI60_K = 4                # A-B alternations (2K positions) before locked
LOKI60_SAFE_DSQ = 8         # never retire within this of THEIR core anchor
LOKI60_LOG = False          # probe batches flip this; screens stay silent
