...
class EcoMixin:
    ...
    def _expand(self, ct):
        ...
        if self.link_queue:
            ...
            return
        if LOKI60_RETIRE_ON:
            # LOKI-60 x #54 (s39): THE UNEMPLOYABLE BUILDER RETIRES ITS OWN
            # +20%.  Definition FIXED PRE-PROBE on the tape (fitted-knob
            # guard): retire iff (a) the two-tile limit-cycle detector fires
            # (K consecutive A-B alternations -- OSCLOCK's field-calibrated
            # instrument; the census: 11.58% of builder-rounds locked, 39.8%
            # of locked bots never acted), AND (b) zero lifetime actions
            # (ever_acted set at every eco action site + the universal core
            # heal), AND (c) not within d^2<=8 of THEIR core anchor (a body
            # at their ring denies seats by standing -- working while locked).
            # OSCLOCK2 freed such bots into idleness and left the +20%
            # standing (46.39, DROP); retirement reclaims it (engine-probed:
            # self_destruct removes the scale contribution, exact, next round).
            h = getattr(self, "osc60_hist", None)
            if h is None:
                h = self.osc60_hist = []
            h.append((p.x, p.y))
            if len(h) > 2 * LOKI60_K:
                del h[0]
            if len(h) == 2 * LOKI60_K and not getattr(self, "ever_acted", False):
                a, b = h[-1], h[-2]
                locked = a != b and all(
                    h[i] == (a if (2 * LOKI60_K - 1 - i) % 2 == 0 else b)
                    for i in range(2 * LOKI60_K)
                )
                if locked:
                    ec60 = enemy_core_for(self.mw, self.mh, self.core) if self.core is not None else None
                    if ec60 is None or p.distance_squared(ec60) > LOKI60_SAFE_DSQ:
                        try:
                            if LOKI60_LOG:
                                import sys
                                print("RETIRE60 retire r=%d at %d,%d"
                                      % (ct.get_current_round(), p.x, p.y), file=sys.stderr)
                            ct.self_destruct()
                            return
                        except Exception:
                            pass
                    elif not getattr(self, "c60_blocked", False):
                        # (c) BLOCKED a retirement -- counted ONCE per bot so the
                        # first probe MEASURES the carve-out's cost instead of
                        # assuming it small (research's ask: without this, a null
                        # has two indistinguishable explanations).
                        self.c60_blocked = True
                        if LOKI60_LOG:
                            import sys
                            print("RETIRE60 c-blocked r=%d d2=%d"
                                  % (ct.get_current_round(), p.distance_squared(ec60)), file=sys.stderr)
