...
# --- #62 LOKI-TINYECO: the tiny-map economy floor (s39; prereg b045e7ea) ---
# One constant, one gate: on maps with area < 300, cap builders spawned
# before the first harvester at 3 (from the unconditional opening 5).
# 3 is a first dose, not an optimum (the 2/3/4 sweep is a second leg if it
# pays).  Off-segment non-regression is BY CONSTRUCTION (the gate cannot
# fire at area >= 300) and is asserted from this diff, not from rows.
# Screen: TINY SET ONLY {antler, fjordgate, moonrise}; a pooled number must
# never be quoted as this arm's effect.  Dose gates everything: median
# builders-before-first-harvester must move 5.0 -> ~3.0 on the tiny set.
LOKI62_TINY_ECO_ON = True
LOKI62_TINY_AREA = 300
LOKI62_PRE_HARV_CAP = 3
