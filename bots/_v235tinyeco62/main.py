...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if ct.get_action_cooldown() != 0:
            return
        # LOKI-62 TINYECO (prereg b045e7ea): on TINY maps the map-blind
        # opening starves the economy -- 5 builders before the first
        # harvester on a 100-tile, 6-ore map (BOOK-worst-maps).  Hold the
        # spawn queue at LOKI62_PRE_HARV_CAP until the first harvester
        # exists; byte-identical behaviour on area >= LOKI62_TINY_AREA.
        if (LOKI62_TINY_ECO_ON and self.mw and self.mh
                and self.mw * self.mh < LOKI62_TINY_AREA
                and self.n >= LOKI62_PRE_HARV_CAP and harv < 1):
            return
