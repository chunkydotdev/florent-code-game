...
# --- #64 LOKI-SPAWNPOCKET (s39; prereg a22fd953, home-lock report §5.1) -----
# Two halves, DISJOINT map sets, opposite standing -- registered accordingly:
# (a) TERRAIN: free-region size leads the spawn sort (valkyrie's two mirrored
#     spawn-ring pockets, 37/37 of terrain class-P) -- DESCRIPTIVE ONLY.
# (b) PAVE GUARD (THE PRIMARY, segment {midgard, fjordgate}, direction
#     POSITIVE): never place a pave-behind conveyor that seals an adjacent
#     region smaller than the floor (62 class-P bots were self-sealed).
# Per-half dose dials (LOKI64_LOG): pocket-spawn events -> ~0 on valkyrie;
# pave-refusals > 0 on midgard/fjordgate.  A half whose dose does not move
# is inert and its map's outcome uninterpretable -- report and stop.
# Rider: kill-round non-regression as an EXCLUSION.  Overlap: shares bots
# with RETIRE60's reclaim if both ever run -- neither claims the full effect.
LOKI64_SPAWNPOCKET_ON = True
LOKI64_REGION_CAP = 40      # flood cap for the spawn key (>= cap = open)
LOKI64_POCKET_MAX = 12      # spawn region below this = a pocket spawn (dial a)
LOKI64_PAVE_MIN_REGION = 12 # paving may not seal a region smaller than this
LOKI64_LOG = False          # probe batches flip; screens stay silent
