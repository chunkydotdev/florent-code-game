...
class EcoMixin:
    ...
    def _would_seal64(self, ct, tile):
        """LOKI-64 half (b): True if building on `tile` would seal an adjacent
        region smaller than LOKI64_PAVE_MIN_REGION (62 class-P bots were
        sealed by OUR OWN conveyors -- midgard 30, fjordgate 10; the probe's
        mutation showed the pave-behind site alone is too rare to carry the
        dose, so BOTH conveyor sites call this).  Unknown tiles count
        passable: the guard under-fires rather than over-fires."""
        if not LOKI64_SPAWNPOCKET_ON:
            return False
        try:
            for d64 in CARDINALS:
                t0 = tile.add(d64)
                if not (0 <= t0.x < self.mw and 0 <= t0.y < self.mh):
                    continue
                if (t0.x, t0.y) in self.map_walls:
                    continue
                try:
                    if ct.is_in_vision(t0) and ct.get_tile_building_id(t0) is not None:
                        continue
                except Exception:
                    pass
                seen = {(tile.x, tile.y), (t0.x, t0.y)}
                stack = [(t0.x, t0.y)]
                small = True
                while stack:
                    if len(seen) - 1 >= LOKI64_PAVE_MIN_REGION:
                        small = False
                        break
                    x, y = stack.pop()
                    for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                        if not (0 <= nx < self.mw and 0 <= ny < self.mh):
                            continue
                        if (nx, ny) in seen or (nx, ny) in self.map_walls:
                            continue
                        tt = Position(nx, ny)
                        try:
                            if ct.is_in_vision(tt) and ct.get_tile_building_id(tt) is not None:
                                continue
                        except Exception:
                            pass
                        seen.add((nx, ny))
                ...
                if small:
                    if LOKI64_LOG:
                        import sys
                        print("SPKT64 seal-refusal r=%d at %d,%d region=%d"
                              % (ct.get_current_round(), tile.x, tile.y, len(seen) - 1),
                              file=sys.stderr)
                    return True
        except Exception:
            return False
    ...
    def _build_next_link(self, ct):
        ...
        if f == Direction.CENTRE:
            f = Direction.NORTH
        if self._would_seal64(ct, tile):
            self.link_queue.pop(0)   # skip the sealing link, keep the chain alive
            return False
            ...
            if readable:
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 \
                    ...
                    if dist_core(pp, self.core) > 0:
                        ...
                        try:
                            if not self._would_seal64(ct, pp) \
                                    and coreward_ok and facing is not None \
