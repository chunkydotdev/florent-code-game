...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        # LOKI-64 SPAWNPOCKET half (a), TERRAIN (prereg a22fd953): free-region
        # size leads the sort so the core never spawns into a sealed terrain
        # pocket (37/37 class-P terrain pockets are valkyrie's two mirrored
        # spawn-ring pockets, fixed tiles).  Static walls only, cached once;
        # valkyrie is DESCRIPTIVE in the read -- the primary is half (b).
        if LOKI64_SPAWNPOCKET_ON and self.map_grid is not None:
            # map_walls inits as an EMPTY SET and only the BUILDER path fills
            # it -- the probe's mutation caught this half running on a
            # wall-less map (every region tied at the cap).  The core fills
            # it from its own grid, once.
            if not self.map_walls and self.map_grid is not None:
                self.map_walls = {
                    (x, y) for y, row in enumerate(self.map_grid)
                    for x, cell in enumerate(row) if cell == "#"
                }
            if not hasattr(self, "spkt_region"):
                self.spkt_region = {}
            def _region64(sp):
                k = (sp.x, sp.y)
                if k in self.spkt_region:
                    return self.spkt_region[k]
                if k in self.map_walls or not (0 <= sp.x < w and 0 <= sp.y < h):
                    self.spkt_region[k] = 0
                    return 0
                seen = {k}; stack = [k]
                while stack and len(seen) < LOKI64_REGION_CAP:
                    x, y = stack.pop()
                    for nx, ny in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):
                        if (0 <= nx < w and 0 <= ny < h
                                and (nx, ny) not in seen
                                and (nx, ny) not in self.map_walls):
                            seen.add((nx, ny)); stack.append((nx, ny))
                self.spkt_region[k] = len(seen)
                return len(seen)
            cands.sort(key=lambda sp: (-min(_region64(sp), LOKI64_REGION_CAP),
        ...
        for sp in cands:
            ...
            if ok:
                ct.spawn_builder(sp)
                if LOKI64_SPAWNPOCKET_ON and LOKI64_LOG and self.map_walls is not None:
                    try:
                        if self.spkt_region.get((sp.x, sp.y), LOKI64_REGION_CAP) < LOKI64_POCKET_MAX:
                            import sys
                            print("SPKT64 pocket-spawn r=%d at %d,%d region=%d"
                                  % (ct.get_current_round(), sp.x, sp.y,
                                     self.spkt_region.get((sp.x, sp.y), -1)), file=sys.stderr)
                    except Exception:
                        pass
                self.n += 1
                return
