...
# ============================================================================
# LOKI-BODYBLOCK (v262) -- THE TERMINAL FALLBACK.  A builder that has declined
# every verb in the ladder does something anyway.
#
# THE MEASUREMENT THAT ORDERED THIS (corpus, 8,338 games, 894k builder-rounds).
# "Idle AND free" -- no cooldown, no verb emitted -- is a DECAY CURVE, not a
# constant: 3.31% of builder-rounds at r10, 17.77% at r60, 29.01% in the 20
# rounds before our Core dies.  The bot does not start broken; IT RUNS OUT OF
# THINGS TO DO.  There is no engine excuse: every SetActionCooldown /
# SetMoveCooldown ever written for a builder is 1 and decrements at end of
# round, so a builder may act or move EVERY round and 0% is reachable.
#
# WHY THE TERMINAL IS FREE OR NEARLY FREE.  eco.py `_eco_spendable` gates every
# PAID verb on the bank (plus SIEGE_HEAL_RESERVE_TI = 16 under siege), and our
# global titanium at T-1 is median 26 on a loss / 53 on a win, q1 = 12 -- at the
# first quartile build AND heal AND attack are ALL unaffordable at once.  A MOVE
# costs 0 Ti and is never bank-gated.  That is the whole case for the body as
# the last resort.
#
# ⛔ THE TRAP THIS FILE EXISTS TO NOT FALL INTO.  A bot oscillating in a
# two-tile navigation lock IS EMITTING A MOVE, so it scores as ACTIVE while
# doing nothing -- 11.58% of all v125 builder-rounds are in permanent two-tile
# locks and 39.8% of locked bots never act in their entire lives.  A naive
# "always emit a move" implementation MEETS THE IDLE METRIC IN FULL WHILE DOING
# NOTHING.  So the fallback commits to a NAMED DESTINATION and makes progress
# toward it (`_bfs_direction`, exact BFS on static terrain, already in eco.py),
# it does not merely move; and it carries an explicit trail memory
# (LOKI_BB_TRAIL) that refuses a step back onto a recently-occupied tile.  When
# no fresh legal step exists it STANDS STILL and counts a stall rather than
# taking the stale step -- the stale step is the lock.
#
# ⛔ AND THE HAZARD THAT DECIDES WHERE THE BODY GOES.  `is_bot_passable` is
# TEAM-BLIND: our own body blocks our own builders just as well as theirs.  So
# every relocation destination must sit on THEIR side of the two Core anchors,
# tested with the raid layer's existing `_salt_forward`.  The one exemption is
# the gunner shield (class 0/a below), which is by construction standing in
# front of an asset of ours.
# ============================================================================

LOKI_BODYBLOCK_ON = False      # MASTER.  False == _v223sealrepair, exactly.

# ----------------------------------------------------------------------------
# (0) THE TERMINAL PECK -- an idle-and-free builder beside an enemy CONVEYOR or
# SPLITTER melees it.  2 Ti for 2 damage, 20 HP, so the tenth peck severs a
# delivery chain.
#
# ⭐ TARGET SUPPLY, measured over our own 9,231 games: 585,331 enemy conveyors
# built, mean 63.4 per game, present in 99.9% of games, 126,807 of them inside
# d^2 <= 8 of their own Core.  AND A CONVEYOR CANNOT WALK AWAY -- every
# launcher/eviction plank on this line died on DELIVERY (EVICT58 got 0.04
# evictions/game because 0033's builders never entered the envelope).  A fixed
# 20 HP building in 99.9% of games does not have that failure mode.
#
# ⛔ WHAT IS ALREADY SHIPPED, so nobody re-measures it as new.  LOKI-SALT /
# LOKI-48 (`RaidMixin._salt_turn` step 2) ALREADY pecks enemy belt, already
# carved out of LOKI_QUIET_ON, already enemy-half-only, already idle-gated on
# the MOVE (`_salt_idle_ok`), capped at LOKI_SALT_CUT_MAX per unit.  What is
# NEW here is only the POPULATION: `_salt_turn` is reachable exclusively from
# `_raid_act`, so EXPANDERS, the home DEFENDER and STOOD-DOWN RAIDERS (raid.py
# hands those to `_expand` at :149 and :163) have never pecked anything at all.
# This terminal gives the belt peck to every builder, not just the raiders.
#
# ⛔ THE QUIET CARVE-OUT, scoped exactly.  LOKI_QUIET_ON stays True and keeps
# silencing everything it silences today -- core peck (`_raid_act` step 1),
# siphon hit (eco.py :1074), counterbattery melee (`_sabotage_prio`).  The
# terminal below never reads LOKI_QUIET_ON and never touches those paths; it
# can only ever fire on a CONVEYOR or SPLITTER, on the enemy side of the map,
# on a round the ladder had already given up on.  It is the same one-entity
# exemption LOKI-SALT already holds, extended to non-raiders.
#
# ⛔ THE Ti GATE, and it is the diagnosed cause of the last refutation.  A
# WIDENED PECK arm (any adjacent enemy building) fired in 12/12 games at 22-462
# pecks and came out median +44.5 rounds SLOWER to the kill; the diagnosis was
# 2 Ti per peck diverted from the ammunition pipeline, and ammunition is what
# kills cores.  THE DIFFERENCE HERE, stated so this arm is not re-refuted by
# association: that arm pecked with builders THAT HAD OTHER THINGS TO DO and
# spent titanium THAT HAD OTHER USES.  This one fires only after the ladder has
# closed AND only out of surplus -- the gate is eco.py's existing
# `_eco_spendable`, which subtracts SIEGE_HEAL_RESERVE_TI under siege, and the
# floor below is its argument.
#
# ⚠ LOKI_PECK_TI_FLOOR (= 4, above) is NOT reused: it is the RAID peck's floor
# and moving it would silently re-tune a shipped plank.  This is a separate
# named constant so it can be swept on its own.  20 is chosen, not measured:
# it is above one sentinel shot of ammunition (10) plus change, so a terminal
# peck can never be the reason a convert_ammo did not happen.  SWEEP ME.
LOKI_BB_PECK_ON = False
# ⛔⛔ LOWERED 20 -> 6, s44 2026-08-15, and the reason is that 20 CONTRADICTED THE
# PLANK'S OWN PREMISE. The entire case for a terminal fallback is Magnus's: at the
# first quartile our bank at T-1 is 12 Ti, so every PAID verb is unaffordable and
# a move is not. A 2 Ti peck gated behind a 20 Ti floor therefore SWITCHES ITSELF
# OFF IN EXACTLY THE BANK STATE THE PLANK EXISTS TO SERVE — the fallback's first
# reach would be for the one thing the evidence says is not there.
# 6 = three pecks' worth, affordable at the q1 bank of 12 while still leaving
# change. The ammo argument the 20 was built on is NOT discarded: it is already
# enforced upstream and independently by `_eco_spendable` + SIEGE_HEAL_RESERVE_TI
# (16), so a terminal peck still cannot be the reason a convert_ammo did not
# happen. This constant only decides how poor is too poor to peck at all.
# STILL CHOSEN, NOT MEASURED. SWEEP ME.
LOKI_BB_PECK_TI_FLOOR = 6

# ----------------------------------------------------------------------------
# (0b)/(a)-(d) THE BODY AS AN OBSTACLE.  Destination classes, in priority order:
#   0b  the tile a VISIBLE enemy builder is about to step into (one-step only;
#       see LOKI_BB_OPP_ON)
#   a   a tile in a live enemy GUNNER's fire line, in front of an asset of ours
#   b   an unoccupied tile of the enemy Core's 12-tile spawn envelope
#       (CORE_SPAWNING_RADIUS_SQ = 2; `can_spawn` calls Tile::is_bot_passable
#       and a bot makes a tile impassable, so the body denies that spawn)
#   c   an ore tile in their half (occupancy makes can_build_harvester refuse)
#   d   an empty tile pinched between two live enemy belt pieces
LOKI_BB_BLOCK_ON = False

# (0b) ONE STEP ONLY, ON PURPOSE.  A committed walk to a MOVING tile is the
# oscillation trap by construction, and chasing a mobile builder is the exact
# delivery failure EVICT58 died of.  So this fires only when the interception
# tile is one legal cardinal step away and the enemy builder moved last round;
# otherwise the chooser falls through to the static classes.
LOKI_BB_OPP_ON = False
# HARD BUDGET, per unit lifetime.  The displaced body is walked BACK to its own
# job by the ladder next round -- which is correct, that job is real -- so an
# enemy builder that keeps offering a heading could in principle drive a
# two-round shuttle.  Observed once in the stub harness and bounded here rather
# than reasoned away.
LOKI_BB_ICEPT_MAX = 8

# (a) ⚠ THIS ONE CONTRADICTS A MEASURED PLANK IN THIS TREE AND IS SHIPPED OFF.
# LOKI-25 (`_raid_station`, LOKI_GUNAXIS_PENALTY = 8) SCORES A STATION DOWN for
# sitting on a gunner ray, because 92% of our forward builder deaths are enemy
# gunners (4,379 of ours vs 6,830 for the rest of the league).  Standing in the
# ray on purpose is the opposite instruction.  It is only ever right when the
# ray CURRENTLY lands on an asset of ours worth more than a 40 HP builder --
# a gunner facing empty ground is not blanked by our body, it is handed a free
# kill.  So the code requires a live friendly first-hit of a type in
# LOKI_BB_SHIELD_TYPES and confirms exposure with `can_fire_from`, and the flag
# still defaults False: turn it on as its own arm, never inside this one.
LOKI_BB_SHIELD_ON = False
LOKI_BB_SHIELD_TYPES = (
    EntityType.CORE, EntityType.SENTINEL, EntityType.GUNNER,
    EntityType.LAUNCHER, EntityType.HARVESTER, EntityType.BUILDER_BOT,
)

# HOW LONG A BODY MUST BE USELESS BEFORE IT IS RELOCATED.  The peck has no such
# gate (it costs no step and abandons no post); RELOCATION does, because the
# largest idle bucket in `_expand` is a builder standing beside an ore tile it
# cannot yet afford, and walking that body away is a real economy cost.  Six
# consecutive idle-and-free rounds is ~15 rounds of passive income; a builder
# waiting on one harvester's 20 Ti does not idle that long, a builder at
# ECO_CAP with nothing left to build idles forever.  THIS IS THE TUNING KNOB.
LOKI_BB_IDLE_RNDS = 6
LOKI_BB_TRAIL = 6         # anti-oscillation memory, in tiles (>= 4 required)
LOKI_BB_RESCAN = 20       # rounds between re-validations of a chosen destination
LOKI_BB_STALL_MAX = 3     # blocked steps before a destination is abandoned
LOKI_BB_BAN_RNDS = 80     # how long an abandoned destination stays banned
LOKI_BB_HYSTERESIS = 3    # score bonus for the tile we are already standing on
LOKI_BB_ORE_SAMPLE = 6    # of their ore, only the N nearest their Core are offered
LOKI_BB_LOG = False       # one line per engagement; LOCAL only (platform strips stdout)
