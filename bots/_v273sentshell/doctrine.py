...
# ============================================================================
# LOKI-SENTSHELL (#73) -- THE FORWARD SENTINEL'S BARRIER SHELL
# ============================================================================
# THE DEFICIT THIS IS AIMED AT, and it is the largest one we have measured:
# OUR forward sentinels have a Kaplan-Meier median lifetime of 17 rounds
# against the field's 104 -- a 6x gap (us-only, n=18,762 of ours vs 86,096 of
# the field's).  A forward sentinel is the ONLY sustained damage the collar
# has once the raiders are dead (18 dmg / 2-round reload = 6 HP per round
# against a sealed Core), so 17 rounds is ~100 HP of a 500 HP Core and then
# nothing.  Every earlier increment on this line bought MORE sentinels; this
# one buys LONGER ones.
#
# WHY A BARRIER SHELL IS THE RIGHT PURCHASE, from the engine rather than from
# taste (disassembly, 2026-08-15):
#   * `turret_target_valid_from@0x276f8`, sentinel branch 0x27764-0x27800, is a
#     PURE ARITHMETIC RAY WALK with ZERO tile loads: range check d^2<=32 then
#     dx^2+dy^2.  Obstacle-blindness is UNREACHABLE, not merely unchecked.  A
#     sentinel that is COMPLETELY walled in still fires.  So the shell costs
#     the sentinel exactly nothing.
#   * The GUNNER branch (0x27804-0x278f0) loads bot slot / building slot / env
#     byte for every tile before the target and hard-fails on any hit.  Gunner
#     fire IS blocked -- and 67.1% of final blows on our forward sentinels are
#     GUNNERS.  A shell tile on a live gunner's ray deletes that gunner's
#     contribution outright.
#   * A builder's attack is MANHATTAN-1 EXACTLY (four `cneg` pairs at
#     `can_fire@0x16604`; own tile and diagonals excluded), so barriers on the
#     orthogonal neighbours make the sentinel unreachable by enemy melee.  At
#     2 dmg per builder action a 30 HP barrier is 15 actions to chew.
#   * `can_launch` tests `is_tile_bot_passable` on the DESTINATION and only
#     kinds 1 (conveyor) and 2 (splitter) are bot-passable ==> a BARRIER shell
#     is launcher-proof.  A conveyor/splitter shell is NOT.  Barriers only.
# Price: 3 barriers = 9 Ti and +3pp scale against the sentinel's 30 Ti and
# +20pp, i.e. ~30% on top, scale-invariant.  The real cost is raider TURNS.
#
# THE MASTER TOGGLE.  False == `_v223sealrepair` exactly: every call site
# below tests LOKI_SENTSHELL_ON first and falls through to the parent's code.
LOKI_SENTSHELL_ON = True

# --- 1. THE BAND ------------------------------------------------------------
# Ported from `_v230sent41` (LOKI41_MIN_DSQ), which built and dose-verified it
# and was deferred for fixture reasons, not for a measurement.  The parent's
# siting scan accepts ANY candidate with d^2(enemy core tile) <= 32 and NO
# minimum, so nothing stops a point-blank plant INSIDE the reactive
# counter-gunner's r^2=13.  14..32 is outside a gunner's reach and inside our
# own r^2=32.
LOKI_SHELL_BAND_ON = True
LOKI_SHELL_MIN_DSQ = 14

# --- 2. SITE SCORING (a TIEBREAK, never a filter) ---------------------------
# 67.3% of in-band tiles have ZERO free orthogonal sides, so a HARD terrain
# requirement finds nothing and silently stops building sentinels altogether.
# Instead: enumerate the legal candidates the parent would have accepted and
# prefer the one whose orthogonal neighbours are ALREADY impassable (map edge,
# wall, any building) -- each such side is a barrier we never have to buy.
# Ties break on the parent's own scan order, so with no terrain to prefer this
# picks exactly what the parent picked.
LOKI_SHELL_SITE_SCORE_ON = True

# --- 3. THE SHELL ITSELF ----------------------------------------------------
# THREE SIDES, NEVER FOUR.  `can_heal` is Manhattan-1 (`can_heal@0x16804`) and
# this tree's buddy-heal (raid.py, step 4/5 of _raid_act) is live and shipped:
# a 4-tile shell makes the sentinel PERMANENTLY UNHEALABLE by our own raiders,
# trading +4 HP/round for a melee channel that is only 13.2% of final blows
# (and 12.7% at d^2<=8, i.e. LOWEST exactly where we shell).  The open side is
# the one facing OUR approach -- it doubles as the heal seat.  There is
# precedent in this tree: `_seat_ban` (main.py) already refuses to plant
# impassables on our OWN Core's heal seats for the same reason.
LOKI_SHELL_MAX = 3          # barrier BUDGET per sentinel; the 4th side is the heal seat
LOKI_SHELL_TI_FLOOR = 0     # bank left after paying for one barrier
# How long after the plant the planter will WALK to finish its own shell.  A
# raider that has not closed it by then is worth more at the ring: the walk is
# bounded so a shell that cannot be reached can never strand a body.
LOKI_SHELL_WINDOW = 14
# ...and a SECOND bound inside it, because the window alone is not enough.
# _shell_station returning the tile the body is already standing on means
# "stand still and build next turn", which is right while an action cooldown
# runs down and WRONG once the build has been declined on a zero cooldown --
# then the body idles at the seat for the rest of the window.  Three declined
# rounds at the same seat and the obligation is dropped: this line wins on
# ARRIVAL and a parked raider is the one cost it cannot pay.
LOKI_SHELL_STALL = 3
# Any raider (not only the planter) fills a needed shell tile it happens to be
# standing beside.  Costs no walk at all and is what makes the shell a TEAM
# obligation rather than one body's errand -- all 16 store slots are already
# allocated, so this is derived from VISION and needs no coordination.
LOKI_SHELL_OPPORTUNIST_ON = True

# --- 4. PRIORITISE BY LIVE GUNNER RAYS, NOT A UNIFORM BOX -------------------
# This is where the value is.  Melee is 13.2% of final blows; GUNNERS are
# 67.1%.  `can_fire_from(gunner_pos, dir, GUNNER, sentinel_pos)` is the free
# hypothetical predicate (ignores ammo and cooldown by contract) and this tree
# already calls its sibling `get_attackable_tiles_from` in _raid_station.
# A plus-shell covers only the 4 cardinals and 4 of a gunner's 8 approach rays
# are DIAGONAL, so the candidate set is all 8 neighbours and a DIAGONAL that a
# gunner actually holds outranks a cardinal that nobody threatens.
# Score: +2 live enemy gunner ray, +1 cardinal (melee channel).  So
#   cardinal+ray 3  >  diagonal+ray 2  >  cardinal 1  >  diagonal 0.
LOKI_SHELL_RAY_PRIORITY_ON = True
LOKI_SHELL_RAY_W = 2
LOKI_SHELL_CARD_W = 1

# --- 5. SELF-HARM GUARDS ----------------------------------------------------
# Our own barriers are TEAM-BLIND: they block our own gunners' fire and our own
# raiders' movement.  Three rules, each answering a defect this repo has hit:
#  (a) SELF-TRAP.  Ported verbatim in spirit from `_v70sb/main.py:1508-1522`,
#      an O(4) `escapes > 0` check written for exactly this and then dropped
#      from every later tree.  We have a field observation of a bot
#      self-immuring for 221 rounds, and `ct.destroy(`/`ct.can_destroy(` appear
#      ZERO times in all four modules of this tree -- there is NO recovery
#      path, so the guard is the only thing standing between us and it.
#  (b) FRIENDLY GUNNER RAYS.  A shell tile inside a friendly gunner's raw
#      attack pattern is skipped: a sentinel ignores obstacles but a gunner
#      does not, and the barrier would be shooting our own turret in the foot.
#  (c) RING STATIONS.  Never seal a tile our own raiders stand on to work the
#      collar.  (Geometrically these cannot overlap while the band holds --
#      a shell tile is d^2 >= ~5 from the nearest Core tile and a ring tile is
#      d^2 <= 2 -- but the check is a frozenset lookup and the band is a flag.)
LOKI_SHELL_ESCAPE_GUARD_ON = True
LOKI_SHELL_FRIENDLY_RAY_GUARD_ON = True
