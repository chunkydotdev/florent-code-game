...
class EcoMixin:
    ...
    def _expand(self, ct):
        ...
        # LOKI-BODYBLOCK (v262).  A body that has ALREADY committed to a
        # blocking tile owns its own movement until it gets there or gives up.
        # Without this hook the eco navigator would walk it back toward its ore
        # every other round and the pair would be a two-tile lock that EMITS A
        # MOVE -- i.e. the exact fake-activity failure this plank must not be.
        # Engagement still happens only in `_bb_terminal`, after the full
        # ladder has declined; this is the follow-through, not the decision.
