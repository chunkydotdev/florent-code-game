...
# ============================================================================
# LOKI-AIMSITE (v314) -- PURPOSEFUL FORWARD-SENTINEL SITING.  Isolated arm:
# this tree carries the siting change and NOTHING else, so it can be measured
# against the parent on its own.  The rent/hop machinery lives in
# bots/_v313rentgun, which also contains this change.
#
# Magnus, 2026-08-15: "Dont we put down an early sentinel usually? What if we
# change that to be behind a harvestor if there's a spot to kill both a
# harvestor and then shoot the core?"
#
# THE DEFECT.  `_try_forward_sentinel` took THE FIRST (neighbour, core-tile)
# pair that passed `can_fire_from` + `can_build_sentinel` and RETURNED -- no
# score, no argmax, no tiebreak.  Siting was therefore a function of where the
# raider's walk happened to stop.  AND A SENTINEL CANNOT ROTATE (`rotate()` is
# gunner-only), so that accidental facing is PERMANENT for the rest of the
# match.  We were already paying 30 Ti and +20% scale and discarding the aim
# for free -- this costs nothing extra to fix.
#
# THE FIX.  A scorer with exactly one bonus: +1 for a site whose ray runs
# through an enemy HARVESTER *and* an enemy CORE tile.  Such a sentinel kills
# the harvester on the way in (18 dmg x 2 against 30 HP) and then keeps firing
# into the Core for the rest of the match, off the same 30 Ti we were spending
# anyway.
#
# THE ALIGNMENT IS MAP-CONDITIONAL AND THIN.  Aligned sites per pool map:
# frostgate 12, midgard 8, antler 6, archipelago 4, drumlin 4, fjordgate 4,
# nordkap 4, ragnarok 2; auroraveil, drakkarfjord, glacierkeep, icefloe,
# royale, valkyrie, yulerune ZERO.  44 sites on 8 of 15 maps -- SO ON SEVEN
# MAPS THIS MUST BE A NO-OP RATHER THAN A STALL.
#
# ⭐ AND IT DEGRADES TO THE PARENT EXACTLY, BY CONSTRUCTION, WHICH IS WHY THE
# SEVEN-MAP CASE IS SAFE.  The scan keeps the FIRST site of the best score
# (`score > best[0]`, strictly), so when every candidate scores 1 the winner is
# the pair the parent's loop would have returned on, in the same iteration
# order.  Identical choice, not merely a similar one.  The only residue on
# those maps is CPU: the scan no longer short-circuits, which is why there is a
# `_cpu_exhausted` check inside the site loop.
#
# ONE GUARD THE PARENT DID NOT HAVE: a sentinel is a building, and our OWN
# gunners' rays are prefix-blocked and TEAM-BLIND, so planting one on a
# friendly firing line switches that gunner off for the match.  Vetoed.
# ============================================================================

LOKI_AIMSITE_ON = True          # the whole plank; False == the parent exactly
LOKI_AIMSITE_LOG = True         # one line when an ALIGNED site is taken; LOCAL
                                # ONLY -- platform replays strip stdout (s28).
