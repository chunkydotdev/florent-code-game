...
# ============================================================================
# ARM B (v316) -- SIPHON HUNT CULLED.  The COMPLEMENT of arm A (v315), and both
# directions are legitimate; what cannot be right is the shipped state, which
# pays the cost and takes none of the benefit.
#
# THE SHIPPED STATE (test audit, s44).  `_siphon_deny` navigates: at d > 1 it
# sets `self.tgt` and calls `_nav`, spending the builder's MOVE to walk it to an
# enemy belt beside our harvester.  At d == 1 the LOKI_QUIET_ON branch returns
# False, the belt survives, SIPHON_MAX_RNDS (24) bans the tile, and the hunt
# picks the next belt and walks to that one too.
#
# ARM A re-arms the payload so the walk buys something.  ARM B does the other
# thing: if we are not going to attack, stop spending the move -- and stop
# paying SIPHON_SCAN_EVERY's get_nearby_buildings sweep for a target list that
# is never acted on.  `_siphon_deny` returning False lets `_expand` fall through
# to its own move (eco.py:1188), so the round is returned to the expander.
#
# ⭐ THE MEASURED POPULATION (research s43, commit 549b3e3f): **30.2% of ALL
# bucket-A rounds -- 133 per game -- are spent cardinally adjacent to an enemy
# belt emitting nothing.**  A located population counted on our own games.
#
# ⛔ THE MECHANISM IS OPEN AND NOTHING HERE MAY ASSERT ONE.  Commit 86473148
# killed both competing accounts -- the side lane's "the walk does nothing" and
# research's "`_salt_turn` picks it up" -- because roles are mutually exclusive
# per `run()`.  What those 133 rounds/game are actually buying is UNMEASURED.
# This arm does not claim they buy nothing; it claims the question is decidable
# by removing them and reading the number, which is the only claim available.
#
# Gated on LOKI_QUIET_ON deliberately: if the quiet flag is ever lifted, the
# payload lives again and this cull switches itself off rather than deleting a
# working behaviour.
#
# HOW THIS LOSES.  If the WALK itself is doing work we have not credited -- a
# body parked beside our harvester denying the tile, or an expander pulled to
# the contested edge of the map where it happens to lay useful chain -- then
# removing it costs more than the move it refunds.  Given the mechanism is open,
# this is a live branch and not a formality.
# ============================================================================

LOKI_SIPHON_NAV_CULL_ON = True   # arm B: no payload, no hunt
