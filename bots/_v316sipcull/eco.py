...
class EcoMixin:
    ...
    def _siphon_deny(self, ct):
        if not SIPHON_DENY_ON or self.core is None:
            return False
        # ARM B (v316): the payload at d == 1 below is silenced by LOKI_QUIET_ON,
        # so the whole hunt spends this builder's MOVE walking to a belt it will
        # never attack, then bans the tile at SIPHON_MAX_RNDS and walks to the
        # next one.  Cull the hunt rather than pay for a dead arrival.  The
        # complement of v315, which re-arms the payload instead.
        if LOKI_SIPHON_NAV_CULL_ON and LOKI_QUIET_ON:
