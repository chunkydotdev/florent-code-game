...
# ============================================================================
# ARM C (v317) -- _raid_peck's ONLY CALL SITE RE-OPENED.  ⛔ READ THE TARGET
# LADDER BELOW BEFORE LABELLING THIS ARM: IT IS A MIXED ARM, NOT A CLEAN ONE,
# AND ITS TOP RANK IS A MEASURED-AND-REJECTED PLANK.
#
# WHAT THE TEST AUDIT FOUND (143-test pure-predicate sweep, s44).  `_raid_peck`
# (raid.py:589, 46 lines) has exactly ONE call site -- step 6 of `_raid_act` --
# behind `not LOKI_QUIET_ON`, so the function never runs in a game.  That much
# is a fact about reachability.  It is NOT a defect claim: LOKI_QUIET_ON is a
# deliberate measured decision (see the LOKI-SALT block below, verbatim).
#
# THE TARGET LADDER, READ OFF THE CODE, AND IT SPLITS THREE WAYS:
#   rank 0  ENEMY CORE          -> this is ARM D's class, the plank the
#                                  doctrine explicitly measured and REJECTED
#                                  ("2 damage a round against a 500 HP Core
#                                  ... is not progress").  It is the FIRST
#                                  thing this function reaches for.
#   rank 1  seat-standing belt  -> the SHIPPED conveyor-melee carve-out class.
#   rank 5  any other belt         `_salt_turn` (LOKI_SALTIDLE_ON = True) is
#                                  LIVE in this tree and already cuts these.
#   rank 2  launcher            -> genuinely not covered anywhere today.
#   rank 3  gunner / sentinel
#   rank 4  harvester
#
# ⇒ TWO CONFOUNDS THIS ARM CARRIES INTO ITS OWN RESULT, both to be written into
# the read rather than discovered afterwards:
#   (a) A WIN COULD BE RANK 0 -- i.e. arm D's rejected plank arriving under a
#       different name.  The arms are separately toggled so the pair can be
#       read together, but this one arm cannot attribute its own win.
#   (b) RANKS 1/5 ARE NOT NEW BEHAVIOUR, THEY ARE SHIPPED BEHAVIOUR ON A
#       REMOVED GATE.  `_salt_turn` sits at step 7 behind `_salt_idle_ok`,
#       which permits a belt peck ONLY on a round the raider would not have
#       moved.  Step 6 has NO such gate, so this arm fires belt pecks on
#       rounds the raider WAS going to walk -- and cancelling that walk is
#       precisely what killed _v178salt (kill round r179 vs pooled r129,
#       Mann-Whitney p=0.008).  Step 6 also runs BEFORE step 7, so it preempts
#       salt rather than adding to it.
#
# THIS ARM RE-OPENS THE CALL SITE ONLY, UNMODIFIED.  LOKI_QUIET_ON stays True,
# so the seat peck (arm D / v318), the siphon hit (arm A / v315) and main.py's
# counterbattery all remain silenced.
#
# THE CLEANER SUCCESSOR IF THIS IS AMBIGUOUS: the same call site with ranks 0,
# 1 and 5 dropped, leaving launcher/turret/harvester -- the only ranks no
# shipped path already reaches and the only ones outside the rejected class.
#
# HOW THIS LOSES.  The trade QUIET and _salt_idle_ok were both bought to stop:
# a raider that pecks is a raider that did not step, and ARRIVAL is the scarce
# quantity on this line.
# ============================================================================

LOKI_RAIDPECK_ON = True   # arm C: re-open _raid_act step 6's call into _raid_peck
