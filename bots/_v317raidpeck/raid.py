...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # ARM C (v317): this is _raid_peck's ONLY call site and `not
        # LOKI_QUIET_ON` makes it unreachable as shipped -- 46 lines of ranked
        # target selection that never runs.  LOKI_RAIDPECK_ON re-opens THIS SITE
        # ONLY; LOKI_QUIET_ON stays True for the seat peck (v318), the siphon
        # (v315/v316) and the counterbattery in main.py.
        if (ti >= LOKI_PECK_TI_FLOOR and (LOKI_RAIDPECK_ON or not LOKI_QUIET_ON)
