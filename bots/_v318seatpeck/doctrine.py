...
# ============================================================================
# ARM D (v318) -- RE-TEST OF A MEASURED-AND-REJECTED PLANK.  Read that label
# first; it is the whole framing and it was corrected mid-build.
#
# ⛔ THIS IS NOT A DEFECT FIX AND MUST NEVER BE WRITTEN UP AS ONE.  Step 1 of
# `_raid_act` is a raider standing on a heal seat pecking the ENEMY CORE, and
# `LOKI_QUIET_ON` is a DELIBERATE, MEASURED decision that shut it.  The
# rejection is recorded verbatim two blocks down (LOKI-SALT, this file, the
# CONVEYOR-MELEE CARVE-OUT paragraph):
#
#     "LOKI_QUIET_ON silences all builder melee and stays on: 2 damage a round
#      against a 500 HP Core that heals +4 for 1 Ti is not progress, and every
#      peck costs the step that arrival is made of."
#
# "2 damage a round against a 500 HP Core at the price of a step" is EXACTLY
# this site.  Calling it an unreachable branch would launder a rejected idea
# back in as a bug report.  It is re-tested because Magnus's standing rule is
# to test rather than reason -- not because the prior measurement is doubted.
#
# WHAT IS GENUINELY NEW SINCE THAT MEASUREMENT, and it is the only reason this
# is worth a shard: the COLLAR.  The rejection priced 2 damage/round against a
# defender healing +4 HP for 1 Ti.  Under a sealed collar the defender's heal
# rate is zero, so the same 2 damage/round is PERMANENT rather than undone --
# and the raider spending the action is one already parked on a seat doing
# denial with its body.  That is an ARITHMETIC that has changed, not a defect
# that was found.  It is still only an argument; the shard is the answer.
#
# The audit did check the GEOMETRY, which could have made the site dead on its
# own merits, and it is SOUND: every heal seat is Manhattan-1 from exactly one
# Core tile and every ring CORNER is Manhattan-1 from zero, so the loop finds
# its target whenever on_seat is True and correctly finds nothing on a corner.
#
# THIS ARM RE-OPENS STEP 1 ONLY.  LOKI_QUIET_ON stays True, so step 6's
# _raid_peck (arm C / v317), the siphon hit (arm A / v315) and main.py's
# counterbattery stay silenced.
#
# HOW THIS LOSES -- and the prior measurement says this is the likely branch.
# If seat-sitting raiders are in fact moving on many of those rounds
# (re-seating, chasing a broken seal, stood down into _expand) the action is
# NOT free and this is the old grind wearing a better argument.  It also
# outranks step 2 (seal a free seat), so a raider that pecks is a raider that
# did not lay the barrier that makes the peck permanent -- if it drops, that
# inversion is the first place to look.
# ============================================================================

LOKI_SEATPECK_ON = True   # arm D: re-open _raid_act step 1, the seat peck of the enemy Core
