...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # ARM D (v318): unreachable as shipped behind `not LOKI_QUIET_ON`.  The
        # audit checked the GEOMETRY separately and it is sound -- every heal
        # seat is Manhattan-1 from exactly one Core tile and every ring CORNER
        # is Manhattan-1 from zero, so this loop finds its tile whenever
        # on_seat is True and finds nothing on a corner.  It is shut by the
        # flag, not by a _try_fwd_barrier defect.  LOKI_SEATPECK_ON re-opens
        # THIS SITE ONLY.
        if on_seat and ti >= LOKI_PECK_TI_FLOOR and (LOKI_SEATPECK_ON or not LOKI_QUIET_ON):
