...
class RaidMixin:
        ...
        if LOKI_SALTIDLE_ON:
            ...
            if self._salt_idle_ok(ct, E, p, near):
                ...
                if self._salt_turn(ct, E, p, ti):
                    ...
                    return True
                # 8. ARM E (v319) -- THE RESTRICTED PECK, LAST AND IDLE-GATED.
                # ⛔ THIS POSITION IS THE WHOLE SAFETY PROPERTY, NOT A STYLE
                # CHOICE.  raid.py:193 is `if action_cooldown == 0 and
                # _raid_act(...): return` -- ANY True return from _raid_act
                # cancels this raider's MOVE for the round.  That is the
                # defect that cost _v178salt 50 rounds of kill time (median
                # kill r179 vs pooled r129, Mann-Whitney p=0.008), and it is
                # NOT fixed by dropping ranks 1/5: it attaches to the peck
                # ACTION, not to the target class.  Sitting here buys both
                # halves from ONE gate evaluation:
                #   * AFTER _salt_turn, so salt keeps first refusal and this
                #     arm cannot preempt a behaviour we already ship; and
                #   * INSIDE _salt_idle_ok, so it fires only on a round this
                #     raider was not going to walk anyway.
                # It deliberately reuses the existing _salt_idle_ok call
                # rather than adding a second one -- the gate calls
                # _raid_station, and one evaluation per raider per round is
                # what that function's cache is documented for.
                if (LOKI_TURRPECK_ON and ti >= LOKI_PECK_TI_FLOOR
                        and self._raid_peck(ct, seatkeys, LOKI_TURRPECK_RANKS)):
                    return True
            if LOKI_SALTIDLE_LOG:
                print("S48", ct.get_id(), self.si_reach,
                      self.si_open, self.si_fire)
    ...
    def _raid_peck(self, ct, seatkeys, ranks=None):
        ...
        `ranks` (v319, ARM E): an optional whitelist of priority ranks.  None
        preserves the parent's full ladder EXACTLY -- the original step 6 call
        site passes nothing, so with LOKI_TURRPECK_ON off this tree is the
        parent.  The v319 call site passes LOKI_TURRPECK_RANKS = {2, 3, 4} so
        that only launcher / gunner-sentinel / harvester are eligible, which
        drops rank 0 (enemy CORE -- arm D's separately-labelled rejected class)
        and ranks 1 and 5 (conveyor/splitter -- the carve-out this tree ALREADY
        ships live via _salt_turn, LOKI_SALTIDLE_ON = True at doctrine.py:1575).
        Rank 6 (everything else, barriers included) is excluded with them.
        """
        p = ct.get_position()
        ...
        for d in CARDINALS:
            ...
            if et == EntityType.CORE:
                ...
                pr = 6
            # ARM E (v319): the rank whitelist.  `ranks is None` is the parent.
            if ranks is not None and pr not in ranks:
                continue
            if pr >= best_pr:
                continue
            try:
