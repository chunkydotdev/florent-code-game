    ...
    def _move(self, ct, d, pave=True):
        ...
        if PAVE_TRAIL_ON:
            ...
            if readable:
                ...
                # LOKI-PAVEFIRST (#50, Magnus's conditional 2026-08-13): the
                # top of the field paves BEFORE the first harvester (100%/
                # 99.6%/100% of the three strongest teams; we were 99.7%
                # harvester-first, the outlier). A FORWARD spot is a claim
                # race and keeps harvester-first; a SAFE spot (our half, by
                # the symmetry anchor) rewards the belt: pave-first there so
                # harvester #1's first stack is a delivery, not waste.
                pave_ok = ct.read_store(SLOT_HARVESTERS) >= 1
                if (not pave_ok and LOKI_PAVE_FIRST and self.core is not None
                        and self.mw):
                    _E = enemy_core_for(self.mw, self.mh, self.core)
                    if (_E is not None and self.tgt is not None
                            and self.tgt.distance_squared(self.core)
                            < self.tgt.distance_squared(_E)):
                        pave_ok = True
                        if LOKI_PAVE_FIRST_LOG:
                            print(f"PAVE50 r={ct.get_current_round()}")
                if not ore_ban and pave_ok \
