...
# LOKI-SIEGE-LAUNCH (QUEUE #45, iteration 2): the deferral above was swept on
# UNSIEGED games and buys the +10% scale surcharge out of the game's tail.
# But the field's point-blank ladders arrive r89-112 (OPP-lingling40/team-lazy
# profiles) and the launcher's EXILE path is our feeder-eviction tool — built
# at r160 it misses the siege entirely. Carve-out: the deferral is waived the
# moment THIS unit can see an enemy turret within LOKI_SIEGE_DSQ of our core.
# The premium is then paid only in games that are already under siege, where
# EXILE0's ~+3pp mechanism is worth the most.
LOKI_SIEGE_LAUNCHER = True
LOKI_SIEGE_DSQ = 32
LOKI_SIEGE_LOG = True      # dose tags SIEGE45 (gate waived) / EVICT45 (enemy
                           # builder thrown); stdout is platform-stripped, so
                           # local-only by nature

# ============================================================================
# LOKI-SIEGE-FULL (#45 iteration 4) -- THE WAIVER v320 DOES NOT ACTUALLY GRANT.
#
# THE DEFECT, and it is DOSE DELIVERY, not a prediction about the result.
# `_v320siegelaunch` waives the eco gate by passing `reserve=0` into
# `_eco_spendable`. But `_eco_spendable` (eco.py) adds a reserve OF ITS OWN:
#
#     if SIEGE_RESERVE_ON and read_store(SLOT_UNDER) != 0
#            and round >= HUNT_MIN_RND:      return ti >= cost + SIEGE_HEAL_RESERVE_TI
#
# with SIEGE_RESERVE_ON=True, SIEGE_HEAL_RESERVE_TI=16, HUNT_MIN_RND=120.
# That condition OVERLAPS the plank's own `sieged` condition, so the caller
# waives a gate the callee immediately re-applies. Waiving a gate the callee
# re-applies is not a waiver.
#
# ⭐ THE EFFECTIVE TITANIUM BAR, WRITTEN OUT SO THE LADDER IS LEGIBLE WITHOUT
# RE-DERIVING IT. `cost` = get_launcher_cost() (52-61 observed at live scale);
# LAUNCHER_RESERVE=80; SIEGE_HEAL_RESERVE_TI=16.
#
#   condition                                   v320 bar        v326 bar
#   -------------------------------------------------------------------------
#   sieged, round < 120                         cost            cost
#   sieged, round >= 120, SLOT_UNDER == 0       cost            cost
#   sieged, round >= 120, SLOT_UNDER != 0       cost + 16       cost
#   not sieged (so round >= 160), UNDER == 0    cost + 80       cost + 80
#   not sieged (so round >= 160), UNDER != 0    cost + 96       cost + 96
#
# ⇒ THE TWO ARMS DIFFER IN EXACTLY ONE CELL: sieged AND round >= 120 AND the
# core under attack (SLOT_UNDER latches for 50 rounds off core HP loss). That
# is the honest size of this plank -- NOT "the whole siege window". A siege
# arriving in the field's observed r89-112 band is already fully waived in
# v320 for its first 8-31 rounds. v326 is a 16 Ti dose delta on one cell, and
# the pair is a DOSE LADDER: v320 keeps the original dose exactly, v326 grants
# the waiver the original prose CLAIMS.
#
# ⛔⛔ WHAT THIS DOES NOT FIX, AND ON THE PRIMARY EVIDENCE IT IS THE WHOLE
# BINDING CONSTRAINT. Read off the 16 committed replays of the original dose
# (`scratchpad/feeder45_dose/siege_*.json`, commit d012f04e) rather than off
# the summary in LOKI-EVICT below, by `strings` on the local stdout the
# platform would have stripped:
#
#     EVICT45                 0  in all 16 games   (the falsifier FIRED)
#     GATE45 gate=eco       170  refusals
#     GATE45 gate=harvester   0
#     bank < cost           170 of 170 rows, gap 1-54 Ti
#     bank  observed range  0-73    cost observed range 47-75
#
# ⇒ THE BANK NEVER REACHED THE BARE COST ONCE. Removing 16 Ti from the bar can
# only convert rows where cost <= bank < cost+16, and there are ZERO such rows.
# **On the v197 chassis this arm is a no-op and so is v320.** The 16 Ti is
# real -- it is measured, see the table above -- but it was never what held the
# gate shut.
# (Note the secondary figures "bank 1-44 / cost 52-61" quoted in LOKI-EVICT
# below are a majority description, not bounds: they cover 92% / 73% of the
# 170 rows. The qualitative claim "gate=eco 100% of refusals" is exact.)
#
# ⭐ SO WHY BUILD IT AT ALL: THE CHASSIS MOVED, AND IT MOVED THE ECONOMY.
# v197 -> v223 is 173 lines of eco.py, most of it LOKI-L4 trunk repair, whose
# own doctrine block says our severed-trunk repair rate was 6.8% against a
# field 40.5% and that "a trunk that delivers nothing from r80 onward is a raid
# that runs out of titanium at r200". A siege in the r89-160 window with a cut
# trunk is EXACTLY that scenario. If L4 lifts the sieged bank over `cost`, the
# gate opens and the 16 Ti becomes the binding constraint for the first time --
# which is when this arm starts to mean something. That is a HYPOTHESIS about
# the new chassis, not a result.
#
# ⇒ AND IT IS ANSWERABLE FOR 16 GAMES, NOT 5400. The question "does the bank
# now reach cost" is read straight off GATE45's `bank=`/`cost=`/`bar=` fields
# and EVICT45's count, at the same n=16 the original dose used. A 5400-game
# win-share screen does not answer it and cannot see it. Run the cheap dose
# replication on this chassis FIRST; only a non-zero EVICT45 makes the screen
# worth its cores.
#
# WHY BYPASSING THE SHARED GATE IS SAFE, verified rather than assumed. The
# reserve is INDIRECT: no heal call site consults `_eco_spendable` at all --
# `_heal_adjacent` (eco.py) and every other heal path gate on `ct.can_heal()`,
# which the engine enforces at 1 Ti. SIEGE_HEAL_RESERVE_TI works by stopping
# OTHER eco spends from draining the last 16 Ti, leaving ~16 heals' worth
# banked. So this bypass costs at most 16 heals (64 HP) ONCE -- the launcher
# latches SLOT_LAUNCHER, so it is a single purchase -- recovering at passive
# income 10 Ti / 4 rounds (~6.4 rounds). It does NOT disable healing and it
# does NOT weaken the reserve for any other spend. Trading up to 64 HP of heal
# for the EXILE path is precisely this plank's thesis.
# ============================================================================
LOKI_SIEGE_FULLWAIVE = True
