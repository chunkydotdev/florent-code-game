...
class Player(EcoMixin, RaidMixin):
    ...
    def _try_build_launcher(self, ct):
        ...
        # LOKI-SIEGE-LAUNCH: an enemy turret parked on our core (the
        # point-blank ladder) waives the round deferral AND the eco reserve.
        # The unit deciding this stands at home, so its own vision is the
        # right census for a siege of our own core. TWO gates guard the
        # launcher (research, 2026-08-13): LAUNCHER_MIN_RND and
        # LAUNCHER_RESERVE — the Leviathan autopsy shows the bank pinned at
        # ~12 Ti in exactly the sieged games, so waiving only the round gate
        # can null with the mechanism sound. Every later no-fire under siege
        # logs GATE45 with the gate's own arithmetic.
        sieged = False
        if LOKI_SIEGE_LAUNCHER and self.core is not None:
            for eid in ct.get_nearby_buildings():
                try:
                    if (ct.get_team(eid) != self.team
                            and ct.get_entity_type(eid) in
                            (EntityType.GUNNER, EntityType.SENTINEL)
                            and ct.get_position(eid).distance_squared(
                                self.core) <= LOKI_SIEGE_DSQ):
                        sieged = True
                        break
                except Exception:
                    continue
        if ct.get_current_round() < LAUNCHER_MIN_RND:
            if not sieged:
                return False
            if LOKI_SIEGE_LOG:
                print(f"SIEGE45 r={ct.get_current_round()} gate waived")
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            if sieged and LOKI_SIEGE_LOG:
                print(f"GATE45 r={ct.get_current_round()} gate=harvester "
                      f"harv={ct.read_store(SLOT_HARVESTERS)}")
            return False
        reserve = 0 if sieged else LAUNCHER_RESERVE
        # LOKI-SIEGE-FULL (#45 iteration 4) -- MAKE THE WAIVER A REAL WAIVER.
        # v320 passes reserve=0 and STILL faces cost + SIEGE_HEAL_RESERVE_TI,
        # because `_eco_spendable` adds that reserve ITSELF under a condition
        # that overlaps this one (SIEGE_RESERVE_ON and SLOT_UNDER != 0 and
        # round >= HUNT_MIN_RND). Waiving a gate the callee re-applies is not a
        # waiver. Bypass the shared gate for THIS ONE DECISION rather than
        # weakening `_eco_spendable`, so every OTHER eco spend keeps the heal
        # reserve intact -- see the doctrine block for why that is safe.
        cost = ct.get_launcher_cost()
        heal_res = (
            SIEGE_HEAL_RESERVE_TI
            if (SIEGE_RESERVE_ON and ct.read_store(SLOT_UNDER) != 0
                and ct.get_current_round() >= HUNT_MIN_RND)
            else 0
        )
        if sieged and LOKI_SIEGE_FULLWAIVE:
            bar = cost
            affordable = ct.get_global_resources() >= bar
        else:
            bar = cost + reserve + heal_res
            affordable = self._eco_spendable(ct, cost + reserve)
        if not affordable:
            if sieged and LOKI_SIEGE_LOG:
                # `bar` is the EFFECTIVE requirement. v320 logs `reserve=0`
                # while actually needing cost+16, i.e. its own dose-delivery
                # instrument understates its own gate by 16 Ti; printing the
                # bar is what makes a no-fire readable.
                print(f"GATE45 r={ct.get_current_round()} gate=eco "
                      f"bank={ct.get_global_resources()} "
                      f"cost={cost} reserve={reserve} heal_res={heal_res} "
                      f"bar={bar}")
            return False
