...
# --- QUEUE #76: PER-TILE ATTRITION MEMORY ON THE FORWARD PLANT --------------
# Against a defence that holds, `_try_forward_sentinel` replants the same tile
# until the game ends (measured: 50 sentinels on one tile in one rated game;
# 73% of plants land on a tile that already ate one; median forward-sentinel
# life vs kladde = 6 rounds; ~21 rebuilds/game). LOKI2B_LIVE_CAP_ON counts
# ALIVE turrets, so a cap on things that die in 6 rounds frees itself forever.
# This is the EMPIRICAL half of the threat model (the tile already ate K of
# ours — cannot be wrong about what happened, no ray math in the 10ms budget).
# A site with >= K recorded plants is refused outright: if the K-th is still
# standing the build would fail anyway (tile occupied), and if it is dead the
# refusal is the point. Kept per-tile in Player state (shared across raiders,
# like raid_ban), no store slots consumed.
LOKI_SENTBAN_ON = True
LOKI_SENTBAN_K = 2          # plants a single tile may consume before it is dead to us
