...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        if LOKI_SENTBAN_ON and not hasattr(self, "fwd_plants"):
            self.fwd_plants = {}
        for d in CARDINALS:
            bp = p.add(d)
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            # #76 attrition memory: a tile that already consumed K of our
            # sentinels is an attrition sink, not a build site (doctrine.py,
            # LOKI_SENTBAN block).
            if LOKI_SENTBAN_ON and self.fwd_plants.get((bp.x, bp.y), 0) >= LOKI_SENTBAN_K:
                continue
            for target in tiles:
                ...
                if LOKI_SENTBAN_ON:
                    self.fwd_plants[(bp.x, bp.y)] = self.fwd_plants.get((bp.x, bp.y), 0) + 1
