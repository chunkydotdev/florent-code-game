...
# ---- ARM-A: turbo4 doctrine fixes ported from bots/_x3r0v152, WITHOUT the
# LOKI-TURBO CPU rewrite (Direction-delta caches, flat-grid helpers). Ported
# by hand against this chassis (_v223sealrepair), not by tools/stack.py.
# ============================================================================
...
# seat. ⛔ T4_SEAT_FIRST_ON (the seat-first reorder in _defend, taking a free
# seat before chasing an untouchable intruder) is OMITTED FROM THIS TREE: its
# "already on a seat, HOLD" check calls `self._home_seat_keys_set()`, which is
# LOKI-TURBO CPU-cache infrastructure (`self._home_seat_key` / `_home_seat_keys`,
# declared only in the Family-A "LOKI-TURBO caches" __init__ block) that does
# not exist in this chassis. STOPPED per build instructions rather than shimmed
# — see the build report for build 2.
