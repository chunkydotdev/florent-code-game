...
class Player(EcoMixin, RaidMixin):
    ...
    def _defend(self, ct):
        ...
        if under and ct.get_move_cooldown() == 0:
            # T4_SEAT_FIRST_ON (seat-first reorder ahead of this branch) is
            # OMITTED from this tree -- see doctrine.py's note at section 2 and
            # the build report. Only the chase-break half of the fix ports.
            intruder = self._nearest_home_intruder(ct)
