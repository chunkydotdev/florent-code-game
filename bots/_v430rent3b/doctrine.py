...
# --- MAGNUS s46 (2026-08-16): "Test that with a combination of less builders."
# Staged opening composed with LOKI-RENTGUN: open with THREE builders, not
# five, and let the EXISTING income-gated expansion (POP_EXPAND_TI_RATE
# below) grow the population as delivered Ti/round clears the meter. The
# three constants move TOGETHER or the floor machinery fights the cap.
# Currency: 2 fewer openers = -40% scale before the first turret, so the
# rented sentinels and the rush turrets are bought at near-base prices.
MAX_BUILDERS = 3
EARLY_BUILDERS = 3
ECO_CAP = 18
ECO_NEED = 3
...
# ============================================================================
# LOKI-RENTGUN (v313) -- THE RENTED TURRET.  Magnus's design, verbatim
# (2026-08-15):
#
#   "could we try something for fun? Place a turret and kill enemy harvestor,
#    then place a barrier on the titanium, try this instead of the salt-ring
#    around enemy core, so it's a rush tactic i guess"
#   "Also, turret should be rented, destroy it when it has killed the
#    harvestor. This is our primary rush tactic, then kill the core."
#
# THE SEQUENCE, one raider, one tile:
#   1. walk to a tile orthogonally adjacent to a live ENEMY HARVESTER
#   2. plant a SENTINEL on another tile adjacent to us, aligned on it
#   3. it fires twice and the harvester dies
#   4. BARRIER the freed ore tile so no harvester can be rebuilt there
#   5. DESTROY our own sentinel -- the rent
#   6. carry on to the Core
#
# ---------------------------------------------------------------------------
# WHY "RENTING" WORKS, AND IT IS ENGINE-MEASURED, NOT INFERRED
# ---------------------------------------------------------------------------
# The cost scale is ONE GLOBAL ADDITIVE team factor: every build adds to it and
# inflates ALL later builds of every type.  CLAUDE.md and the guard-matrix
# disassembly (get_scale_percent@0x11fb8) both assert that DESTRUCTION REMOVES
# THE CONTRIBUTION, but neither is an OBSERVATION of the removal --
# `bots/_probe_scale` only ever proved the ADDITION half.  So it was probed.
#
#   bots/_probe_rentscale, `fcode run`, 2026-08-15 12:3xZ:
#     r1  S0 pre-build     scale%=120.0000  gun=24 sent=36 harv=24 conv=3 bar=3
#     r2  S1 post-build    scale%=140.0000  gun=28 sent=42 harv=28 conv=4 bar=4
#     r3  S1b hold         scale%=140.0000  (held a full round: not transient)
#     r4  S2 post-destroy  scale%=120.0000  gun=24 sent=36 harv=24 conv=3 bar=3
#     PROBE VERDICT S0=120.0000 S1=140.0000 S2=120.0000 -> REMOVAL CONFIRMED
#
# ⇒ A TURRET WE HAND BACK COSTS ITS TITANIUM BUT NOT ITS PERMANENT +20% TAX ON
# EVERY LATER BUILD OF EVERY TYPE.  That is the whole reason this is a rush
# tool and not an expensive one, and the removal is INSTANT -- S2 was read in
# the same turn as the destroy, and every cost getter had already fallen back.
#
# THE SAME PROBE ANSWERED THE SECOND LOAD-BEARING QUESTION: destroy() really is
# free AND cooldown-free.  The probe destroyed the sentinel and then BUILT A
# BARRIER in the same run(), printing
#     PROBE same-turn-act-after-destroy=YES(barrier @Position(x=6, y=2)) cd=1
# -- the cd=1 came from the barrier BUILD, not from the destroy.  So the rent
# is returned at ZERO tempo, and steps 4 and 5 above are ONE turn, not two.
#
# NO REFUND, though: titanium read 446 before and 456 after, and +10 is exactly
# the passive tick (r4, PASSIVE_TITANIUM_INTERVAL=4).  Assume the Ti is spent.
#
# ---------------------------------------------------------------------------
# SENTINEL, NOT GUNNER -- AND THE ARITHMETIC IS NOT CLOSE
# ---------------------------------------------------------------------------
# A harvester is 30 HP.
#   GUNNER    7 dmg, reload 1, 4 ammo/shot, 20 Ti, +20%, r^2=13
#             -> 5 shots = 20 ammo, one shot every 2 rounds = ~9 rounds
#   SENTINEL 18 dmg, reload 2, 10 ammo/shot, 30 Ti, +20%, r^2=32
#             -> 2 shots = 20 ammo, one shot every 3 rounds = ~4 rounds
# IDENTICAL AMMO.  IDENTICAL SCALE STEP (and it is refunded on the destroy
# anyway).  The sentinel costs 10 Ti more and kills in LESS THAN HALF THE
# ROUNDS -- in a rush, five rounds of a parked raider is the expensive thing,
# not 10 Ti.  Third and decisive: a GUNNER ray is prefix-blocked and TEAM-BLIND
# (this file already says so at LOKI_FWD_SENTINEL_ON, and `_probe_rayempty`
# measured it), and step 4 of this very sequence drops a BARRIER on the target
# tile -- so from the second rental onward a gunner would be shooting our own
# salt, with our own raider's body standing beside the muzzle besides.  The
# sentinel line is pure arithmetic and ignores obstacles.
#
# ---------------------------------------------------------------------------
# AMMO IS THE GATE, AND IT IS THIS PROJECT'S MEASURED WEAK POINT
# ---------------------------------------------------------------------------
# Turrets fire from a team-global pool that starts at 0 with no passive income;
# the only source is the Core's convert_ammo.  A prior autopsy found us sitting
# at 24 ammo for rounds 100-900 while a plank needed 30, and the plank silently
# never fired.  Two answers, both required:
#   * the BUILD is gated on `get_global_ammo() >= LOKI_RENT_AMMO_MIN` (20).  A
#     turret that cannot shoot is 30 Ti of building plus a +20% tax we then
#     have to walk back for -- we simply do not plant it.
#   * the Core's magazine floor is raised to LOKI_RENT_AMMO_BANK.  It has to
#     be, because the rent path deliberately does NOT write SLOT_FWD_GUN (that
#     slot is the forward-sentinel CAP counter, is never decremented, and three
#     rentals would close the forward-sentinel arm for the rest of the match --
#     the LOKI-2b rubble bug, in this file at line 1239).  Without the raise,
#     `AMMO_FLOOR` is 16, 16 < 20, and the plank NEVER FIRES.
# AND IF THE POOL EMPTIES MID-RENTAL: the sentinel just stops, the harvester
# lives, and `LOKI_RENT_HOLD_MAX` is what turns that into a BOUNDED loss --
# the clock expires, we destroy the turret, take the Ti loss and keep the
# scale.  There is no path where a rental holds the +20% indefinitely.
#
# ---------------------------------------------------------------------------
# IT RUNS BESIDE THE SEAL, NOT INSTEAD OF IT -- A ROLE SPLIT
# ---------------------------------------------------------------------------
# Magnus's first framing was "instead of the salt-ring around enemy core" and
# the first draft of this arm duly made the two mutually exclusive.  HIS
# AMENDMENT REVERSED THAT: "While another builder works on sealing their core
# with barriers?"  So `LOKI_BARRIER_SEAL_ON` is UNTOUCHED and the two planks
# run on DIFFERENT BODIES, split by `raid_slot` (the monotone seat every raider
# is issued at birth, already the source `_raid_station` uses to spread the
# ring without a store write).  `LOKI_RENT_SEAT_MOD`/`LOKI_RENT_SEAT` set the
# ratio; at 2/0 it is every other raider hunting harvesters while the rest run
# the untouched ladder.  A hunter that has spent its hop budget falls through
# to that same ladder and seals like anybody else.
# ⚠ THE SEAL IS MEASURED AND SHIPPED: s30 ablated it at `barrier-seal-off`
# 399/1024, a REAL NEGATIVE -- removing it COST us.  Which is the second reason
# not to swap it out on a first pass.  Note `LOKI_SALT_*` (belt salting) is a
# different mechanism again and is also left alone.
#
# ---------------------------------------------------------------------------
# ⭐ THE AIM WAS ALREADY BEING THROWN AWAY -- LOKI-AIMSITE
# ---------------------------------------------------------------------------
# Magnus: "Dont we put down an early sentinel usually? What if we change that
# to be behind a harvestor if there's a spot to kill both a harvestor and then
# shoot the core?"  We do, and we were siting it by accident.
# `_try_forward_sentinel` took THE FIRST (neighbour, core-tile) pair that
# passed `can_fire_from` + `can_build_sentinel` and RETURNED -- no score, no
# argmax, no tiebreak -- so the facing was a function of where the raider's
# walk happened to stop.  AND A SENTINEL CANNOT ROTATE (`rotate()` is
# gunner-only), so that accidental facing is PERMANENT for the match.  We were
# already paying the 30 Ti and the +20% and discarding the aim for free.
# ⇒ The loop is now a SCORER with exactly one bonus: +1 for a site whose ray
# runs through an enemy HARVESTER *and* an enemy CORE tile.
#
# THE ALIGNMENT IS MAP-CONDITIONAL AND THIN, so the plank must degrade rather
# than stall.  Aligned sites per pool map: frostgate 12, midgard 8, antler 6,
# archipelago 4, drumlin 4, fjordgate 4, nordkap 4, ragnarok 2; auroraveil,
# drakkarfjord, glacierkeep, icefloe, royale, valkyrie, yulerune ZERO.  44
# sites on 8 of 15 maps -- SO ON SEVEN MAPS THIS MUST BE A NO-OP.  It is, by
# construction: the scan keeps the FIRST site of the best score, so when every
# candidate scores 1 the winner is the pair the parent's loop would have
# returned on, in the same iteration order.  Identical choice, not merely a
# similar one.
#
# ⇒ TWO BRANCHES, SELECTED BY GEOMETRY, AND THIS REVERSES THE ORIGINAL SPEC:
#   ALIGNED site  -> build it, kill the harvester, KEEP IT.  It then fires into
#                    the Core for free for the rest of the match, and tearing
#                    that down to recover +20% scale is a bad trade.  A keeper
#                    takes a SLOT_FWD_GUN cap slot, because that is what it is.
#   NO aligned site -> build, kill, BARRIER, DESTROY (the rent), and HOP to the
#                    next harvester and repeat.  The hop is what makes this a
#                    rush rather than a one-shot; `rent_seen` is what stops the
#                    raider re-targeting the tile it just walled.
#
# ⚠ ISOLATED AS ITS OWN ARM.  The siting change costs nothing extra and is the
# effect most worth measuring alone, so `bots/_v314aimsite` carries it WITHOUT
# any of the rent/hop machinery.  The ladder is base -> v314 (siting only) ->
# v313 (siting + rent/hop/role-split), so v314-vs-base isolates the aim and
# v313-vs-v314 isolates the rental.
#
# ---------------------------------------------------------------------------
# THE GEOMETRY, because it is why the whole sequence fits in one tile
# ---------------------------------------------------------------------------
# The raider stands ORTHOGONALLY ADJACENT to the ore.  A build site is any tile
# orthogonally adjacent to the RAIDER, so from that one standing tile it can
# reach (a) the ore tile itself, for the barrier, and (b) the turret tile, for
# the destroy -- both without moving.  Alignment is never a problem: with the
# raider east of the ore, the tile east of the raider aims WEST straight
# through our own body (the sentinel line ignores obstacles), and the tiles
# north/south of the raider aim diagonally at d^2=2.  Legality is not assumed
# anywhere -- `can_fire_from(site, facing, SENTINEL, ore)` is the engine's own
# hypothetical-turret predicate and it is asked every time, exactly as
# `_try_forward_sentinel` does.
#
# THE MAIN FAILURE MODE IS A STRANDED TURRET -- a raider that walked (or was
# THROWN: a launcher moves any adjacent builder from either team, which is what
# the exile detector at raid.py:167 exists for) away from a live rental never
# returns the rent and we keep the +20% for the match.  So the raider PINS
# itself beside the rental for the duration, and if it is displaced anyway it
# walks back on a bounded clock and logs `RENT313 STRANDED` when it gives up.
# The state is validated BY TILE (`get_tile_building_id(pos) == id`) rather
# than by `get_hp(id)`, and an unreadable tile returns None rather than False,
# because reading "unknown" as "gone" abandons live turrets -- the flattering
# direction, and the expensive one.
# ============================================================================

LOKI_RENTGUN_ON = True          # the whole plank; False == the parent exactly
# 2 sentinel shots = the 30 HP harvester.  Do not plant a turret we cannot
# fire; see the ammo section above.
LOKI_RENT_AMMO_MIN = 20
# Core magazine floor while the plank is on.  20 for the rental + one shot of
# margin, since a home turret shares the pool.
LOKI_RENT_AMMO_BANK = 30
LOKI_RENT_TI_FLOOR = 30         # bank left after paying for the rental
# THE HOP: harvesters one raider will chain through before it goes back to
# being an ordinary raider.  Magnus: "The builder should still continue to
# their next harvestor and kill it with another sentinel."
LOKI_RENT_HOP_MAX = 3
# Detour allowed even after the raider is established at the ring, but only
# this far: a defender's harvesters usually sit INSIDE the establish band
# (d^2 <= 40), so refusing every detour there threw away the commonest target.
LOKI_RENT_NEAR_DSQ = 8
# Rounds a hunter will walk at one prospected ore tile before retiring it and
# re-picking.  Stops an unreachable ore holding a body for the match.
LOKI_RENT_PROSPECT_MAX = 40
# THE ROLE SPLIT: hunters are the raiders whose seat satisfies
# raid_slot % MOD == SEAT.  Everyone else runs the untouched ladder, seal
# included.  MOD 1 makes every raider a hunter; MOD 0/1 disables the split.
LOKI_RENT_SEAT_MOD = 2
LOKI_RENT_SEAT = 0
# KEEP an aligned sentinel instead of renting it back.  False reverts to the
# original brief (always destroy) and is the ablation for Magnus's amendment.
LOKI_RENT_AIMED_KEEP_ON = True
# Rounds a raider will hold station beside a live rental before writing it off.
# 2 shots land at roughly +1 and +4; 14 leaves room for one enemy heal cycle
# and for the ammo pool to refill once.
LOKI_RENT_HOLD_MAX = 14
# Detour radius.  A builder bot's vision is r^2=20, so this is exactly "a
# harvester I can actually see" -- the raider never walks at a remembered one.
LOKI_RENT_SEEK_DSQ = 20
# Firing promotion radius in main.py._turret.  The rental is built from a tile
# adjacent to the ore, so the real separation is 1, 2 or 4; nothing wider gets
# its target priority rewritten, which keeps home sentinels untouched.
LOKI_RENT_LOCK_DSQ = 4
# Extra rounds spent walking back to a rental we were displaced from.
LOKI_RENT_RECOVER_MAX = 10
# ⚠ DOES THE HARVESTER OUTRANK THE ENEMY CORE, FOR THIS ONE TURRET?
# Found by the stub drive, not by reading: on an ALIGNED (keeper) site the
# enemy Core sits on the same ray as the harvester, and `_turret`'s table ranks
# CORE 0 against HARVESTER 5 -- so the keeper shot straight past the harvester
# at the Core and the harvester NEVER DIED.  The ore was never freed and never
# salted, i.e. half of Magnus's sequence silently did not happen.
# TRUE  = harvester first, which is what he asked for ("kill both a harvestor
#         and then shoot the core").  Costs 2 shots / ~4 rounds = 36 HP of
#         forgone Core damage, once, and buys their income plus the permanent
#         ore barrier.
# FALSE = Core first, the parent's ordering.  Defensible under R1000_IS_DEFEAT
#         -- our median kill is r174 against a median death of r187, a
#         thirteen-round race -- which is exactly why this is a FLAG and not a
#         decision buried in a priority table.
# Scoped by LOKI_RENT_LOCK_DSQ either way, so no home sentinel is re-aimed.
LOKI_RENT_HARV_FIRST = True
LOKI_RENT_SENT_SHOT = 10        # ammo per sentinel shot
LOKI_RENT_GUN_SHOT = 4          # ammo per gunner shot
# Instrumentation: one line per rental open / return / strand, into the replay.
# ⛔ LOCAL ONLY -- platform-downloaded replays strip stdout in 30,664 of 30,664
# BotOutput events (CLAUDE.md, s28), so nothing may be read off a live leg with
# this.  It is how the sequence is verified under `fcode run`.
LOKI_RENT_LOG = True

# --- LOKI-AIMSITE: purposeful forward-sentinel siting ------------------------
# Independent of LOKI_RENTGUN_ON on purpose -- this is the half that is carried
# alone by bots/_v314aimsite.  False restores `_try_forward_sentinel`'s
# first-match loop exactly.
LOKI_AIMSITE_ON = True
LOKI_AIMSITE_LOG = True         # one line when an ALIGNED site is taken; local only
