...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if LOKI_SEATCANON_SPAWN:
            # Seat-canonical: fold each candidate offset through the same
            # reflection that maps this core's own position toward the
            # board's low corner before hashing, so a mirrored/rotated seat
            # sorts the SAME relative ring by the SAME permutation instead of
            # an unrelated one (the absolute-coordinate hash below is not
            # equivariant under the map symmetry -- see LOKI_SEATCANON_SPAWN).
            fx = 1 if p.x <= (w - 1) / 2 else -1
            fy = 1 if p.y <= (h - 1) / 2 else -1

            def _seat_key(sp):
                dx = (sp.x - p.x) * fx
                dy = (sp.y - p.y) * fy
                key = ((dx * 17 + dy * 31 + self.n * 13 + self.spawn_salt) % 97, dy, dx)
                return key

            cands.sort(key=_seat_key)
        else:
