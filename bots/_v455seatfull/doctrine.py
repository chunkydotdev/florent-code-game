...
# LOKI_SEATCANON_FULL -- one toggle for the WHOLE equivariance family
# (docs/research/EQUIVARIANCE-SWEEP-2026-08-16.md sites 1,2,3,4,5,7).  Every
# site below hashes/sorts/orders on coordinates that are ABSOLUTE (x, y) or on
# a fixed compass/iteration order, none of which are equivariant under the
# map's mirror/rotation symmetry -- so a mirrored seat sorts an unrelated
# permutation of the same relative candidate set.  This tree does NOT declare
# LOKI_SEATCANON_SPAWN (see _v450seatspawn); site 1 is folded in here under
# this one flag instead, so SEATFULL is a strict superset covering the rest of
# the family (rung 2, queued as SEATFULL against the SEATSPAWN contrast).
LOKI_SEATCANON_FULL = True
