...
def delivery_seats(o, mw, mh, walls, ores):
    ...
    if LOKI_SEATCANON_FULL:
        # Seat-canonical tiebreak order: heal_seats enumerates N,NE,E,... from
        # literal North regardless of which map half `o` sits in, so its
        # index is not equivariant under the map's mirror/rotation symmetry.
        # Re-rank the same seat LIST (heal_seats itself is untouched -- other
        # callers depend on its absolute compass order) by the folded (dx,dy)
        # relative to `o`, same fold rule as LOKI_SEATCANON_FULL elsewhere.
        _fx = 1 if o.x <= (mw - 1) / 2 else -1
        _fy = 1 if o.y <= (mh - 1) / 2 else -1
        canon = sorted(seats, key=lambda s: ((s.x - o.x) * _fx, (s.y - o.y) * _fy))
        order = {(s.x, s.y): i for i, s in enumerate(canon)}
    else:
...
class EcoMixin:
    ...
    def _link_path(self, ct, hpos):
        ...
        if LOKI_SEATCANON_FULL:
            # Seat-canonical seed order for the multi-source BFS below: a raw
            # set's CPython iteration order is a function of absolute-
            # coordinate hashes, not equivariant under the map's symmetry, so
            # on a tie it silently favours an unrelated core-side per seat.
            # Fold relative to our own Core (same rule as LOKI_SEATCANON_FULL
            # elsewhere) and feed the queue from an explicit list instead.
            _fx = 1 if self.core.x <= (self.mw - 1) / 2 else -1
            _fy = 1 if self.core.y <= (self.mh - 1) / 2 else -1
            goal_order = sorted(
                raw_goals,
                key=lambda g: ((g[0] - self.core.x) * _fx, (g[1] - self.core.y) * _fy),
            )
        else:
            goal_order = None
        ...
        if self.map_grid is not None:
            ...
            try:
                ...
                pass
            if LOKI_SEATCANON_FULL:
                goals = [g for g in goal_order if g not in blocked]
    ...
    def _pick(self, ct):
        ...
        if self.map_ores and self.role != "defend":
            ...
            if LOKI_SEATCANON_FULL:
                # Seat-canonical: same fold as the spawn-ring hash, relative
                # to our own Core -- see LOKI_SEATCANON_FULL.
                _fx = 1 if self.core.x <= (self.mw - 1) / 2 else -1
                _fy = 1 if self.core.y <= (self.mh - 1) / 2 else -1
                ordered = sorted(
                    self.map_ores,
                    key=lambda t: (
                        ...
                        (((t.x - self.core.x) * _fx) * 17
                         + ((t.y - self.core.y) * _fy) * 31
                         + worker * 7) % 97,
                    ),
                )
            else:
