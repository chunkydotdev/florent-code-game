...
class RaidMixin:
    ...
    def _raid_station(self, ct, E, near):
        ...
        if LOKI_SEATCANON_FULL and self.core is not None:
            # Seat-canonical: same fold as the spawn-ring hash (main.py), but
            # relative to the ENEMY anchor E, since these tiles orbit their
            # Core, not ours -- the fold itself is always keyed on which map
            # half OUR core owns, so both frames stay consistent (see
            # LOKI_SEATCANON_FULL).
        ...
        for i, s in enumerate(stations):
            ...
            if LOKI_SEATCANON_FULL and self.core is not None:
                dsx = (s.x - E.x) * _fx
                dsy = (s.y - E.y) * _fy
                k = (score, (dsx * 17 + dsy * 31 + self.raid_slot * 7) % 97, dsy, dsx)
            else:
    ...
    def _launcher_turn(self, ct):
        ...
        if LOKI_SEATCANON_FULL:
            # Both throw-site sorts below break distance ties on the ORIGINAL
            # list order, which was built by a fixed dx-outer/dy-outer sweep
            # -- not equivariant under the map's mirror/rotation symmetry.
            # Add an explicit final tiebreak: the folded (dx, dy) offset from
            # the launcher, same own-half fold rule as LOKI_SEATCANON_FULL
            # elsewhere (keyed on our own Core regardless of which anchor --
            # own or enemy -- the primary distance key uses).
            _fx = 1 if self.core.x <= (w - 1) / 2 else -1
            _fy = 1 if self.core.y <= (h - 1) / 2 else -1

            def _site_fold(t):
                return ((t.x - lp.x) * _fx, (t.y - lp.y) * _fy)
        ...
        for eid in ct.get_nearby_entities():
            ...
            if LOKI_SEATCANON_FULL:
                far = sorted(
                    sites,
                    key=lambda t: (t.distance_squared(self.core), _site_fold(t)),
                    reverse=True,
                )
            else:
        ...
        if LOKI_SEATCANON_FULL:
            near_first = sorted(
                sites, key=lambda t: (t.distance_squared(dest), _site_fold(t))
            )
        else:
