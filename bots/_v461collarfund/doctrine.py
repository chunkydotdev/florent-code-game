...
# the r180-300 band only: never a bare round constant on its own, always the
# same SURGE_TI_FLOOR bank test AND an actual contact/siege signal (see
# _eco_cap in eco.py for the two comms-store reads it OR's together).  The
# opening and the r300+ branch are unaffected -- this only lets a bank that
# has already reached surplus convert into hands while we are demonstrably
# fighting, instead of waiting on the clock alone.
