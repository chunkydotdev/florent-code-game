...
class EcoMixin:
    ...
    def _eco_cap(self, ct):
        # QUEUE #71 -- FUND THE COLLAR BEFORE THE KILL WINDOW.  The bank test
        # stays the outer AND for BOTH ways in, so the r300+ branch below
        # (bank >= SURGE_TI_FLOOR and round >= SURGE_MIN_RND) is preserved
        # verbatim.  The new r180-300 branch never fires on the round alone:
        # it additionally requires the same surplus floor AND a live
        # contact/siege signal -- a raid foothold established at the enemy
        # Core (SLOT_RAID_LIVE via _foothold_live, raid.py), OR our own Core
        # under attack (SLOT_UNDER, main.py's threat latch) -- so a quiet
        # r200 economy race still waits for r300 exactly as before.
        rnd = ct.get_current_round()
        if ct.get_global_resources() >= SURGE_TI_FLOOR and (
            rnd >= SURGE_MIN_RND
            or (
