...
SURGE_EXTRA = 5
SURGE_ECO_CAP = 24
# QUEUE #71 -- FUND THE COLLAR BEFORE THE KILL WINDOW.  The round-300 branch
# above is preserved verbatim -- a bare surplus bank still opens the surge at
# SURGE_MIN_RND, untouched.  This adds a second, KILL-CONDITIONAL way in for
# the r180-300 band only: never a bare round constant on its own, always a
# bank-surplus test AND an actual contact/siege signal (see _eco_cap in eco.py
# for the two comms-store reads it OR's together).  The opening and the r300+
# branch are unaffected -- this only lets a bank that has already reached
# surplus convert into hands while we are demonstrably fighting, instead of
# waiting on the clock alone.
SURGE_CONTACT_MIN_RND = 180
# QUEUE #80 -- REACHABLE FLOOR FOR THE CONTACT BRANCH (research #80 read,
# 2026-08-16).  SURGE_TI_FLOOR (1500) is a fossil fitted to v80/v90-era
# banking and never fires in the kill window: 0 of 1,243 v140 game-sides ever
# hold 1500 Ti before r250.  Median bank falls below 260 by r8 -- the 470
# opening endowment is spent early -- so a 200-Ti level test AT r180+
# (SURGE_CONTACT_MIN_RND, past the opening builder curve) is genuine mid-game
# surplus, not endowment.  This floor is used ONLY by the CONTACT branch
# (_eco_cap's second disjunct and main.py's spawn-budget surge, both via
# _surge_contact_live in eco.py); the LEGACY r300+ branch keeps SURGE_TI_FLOOR
# verbatim, so a quiet economy race is completely unaffected by this constant.
SURGE_CONTACT_TI = 200
