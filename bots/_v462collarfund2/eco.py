...
class EcoMixin:
    ...
    def _surge_contact_live(self, ct, rnd):
        """Is the r180-300 CONTACT branch of the surge live this round?

        QUEUE #80 (research #80 read, 2026-08-16): reachable-floor version of
        the kill-window surge -- SURGE_CONTACT_TI (200) replaces the
        SURGE_TI_FLOOR (1500) fossil for this branch only, because 0 of 1,243
        v140 game-sides ever hold 1500 Ti before r250 (see SURGE_CONTACT_TI in
        doctrine.py).  Gated on a live contact/siege signal -- a raid foothold
        established at the enemy Core (SLOT_RAID_LIVE via _foothold_live,
        raid.py), OR our own Core under attack (SLOT_UNDER, main.py's threat
        latch) -- so a quiet r200 economy race with a merely-surplus bank
        still does not fire.

        SINGLE SOURCE OF TRUTH: this predicate is consumed by BOTH _eco_cap's
        contact branch below and main.py's Core spawn-budget surge, so the
        two consumers cannot drift out of step with each other.
        """
        return (
            rnd >= SURGE_CONTACT_MIN_RND
            and ct.get_global_resources() >= SURGE_CONTACT_TI
            and (self._foothold_live(ct, rnd) or ct.read_store(SLOT_UNDER) != 0)
        )

    def _eco_cap(self, ct):
        # QUEUE #71 -- FUND THE COLLAR BEFORE THE KILL WINDOW.  The LEGACY
        # r300+ branch (bank >= SURGE_TI_FLOOR and round >= SURGE_MIN_RND) is
        # preserved verbatim.  The CONTACT branch (r180-300) is factored into
        # _surge_contact_live above -- QUEUE #80 gave it its own, reachable
        # bank floor (SURGE_CONTACT_TI) instead of sharing SURGE_TI_FLOOR, so
        # the two branches now use two different Ti tests by design.
        rnd = ct.get_current_round()
        legacy_surge = (
            ct.get_global_resources() >= SURGE_TI_FLOOR and rnd >= SURGE_MIN_RND
        )
        if legacy_surge or self._surge_contact_live(ct, rnd):
    ...
    def _bfs_direction(self, ct, target):
        ...
        bodies.discard(start)                          # BODYAWARE (#63)

        # BODYAWARE (#63): pure code motion -- desired/side/order depend only on
        # p, target, self.idx and CARDINALS, never on blocked/bodies/goals.
                ...
                for c in core_tiles(target):
                    for d in CARDINALS:
                        ...
                        if (
                            ...
                            and dist_core(qpos, target) > 0 and key not in blk
                        ):
                            goals.add(key)
            else:
                goals = {
