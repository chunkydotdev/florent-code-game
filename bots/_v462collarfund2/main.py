...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        # QUEUE #80 -- RESTORE SURGE_EXTRA'S BODY CONSUMER (research #80 read,
        # 2026-08-16).  The monolith (_v115dodge/main.py:1804) added
        # SURGE_EXTRA straight onto spawn_budget once the surge bank/round
        # test passed; the modular refactor kept the eco-cap consumer
        # (_eco_cap, eco.py) but dropped this one, leaving SURGE_EXTRA dead
        # code in this lineage.  Reuses _surge_contact_live (eco.py) -- the
        # SAME predicate _eco_cap's contact branch reads -- so the two
        # consumers cannot drift.  The round gate inside that predicate
        # (rnd >= SURGE_CONTACT_MIN_RND == 180) guarantees this never fires
        # before r180, so the opening curve above (LOKI_BASE_BUILDERS,
        # LOKI_SURPLUS_EXTRA, LOKI_RICH_EXTRA, all capped by LOKI_MAX_BUILDERS)
        # is untouched; the hard population ceiling is still enforced below
        # via get_unit_count() against GameConstants.MAX_TEAM_UNITS.
        if self._surge_contact_live(ct, rnd):
            budget += SURGE_EXTRA
