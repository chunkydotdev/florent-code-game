...
# QUEUE #83 (research #83 read): a gunner's shot is blocked by obstacles, so
# can_fire yields at most the nearest occupied tile in its line -- when that
# tile is an enemy BARRIER the gunner fires into it forever (measured: 34.1%
# of all our gunner shots land on barriers, one ate 243 shots) and never
# reaches ROTATE_DISCIPLINE_ON's _idle_rotate re-aim below it, because the
# fire paths RETURN before that branch is ever reached. Fix (b), shipped
# here: fall through instead of firing on a barrier, so the gunner re-faces
# via _idle_rotate (or simply idles, if _idle_rotate's own discipline
# declines to rotate -- still strictly better than feeding the rock).
# Fix (a), hold-fire-only with no re-aim, was the rejected confounded
# alternative -- it stops the leak but leaves the turret pinned blind.
# Sentinels are untouched: their shots pierce, so a barrier target for a
# sentinel is a deliberate choice, gated out below by turret_type check.
LOKI_GUNNER_UNPIN = True
