        ...
        if turret_type == EntityType.GUNNER:
            try:
                ...
                if tgt is not None and ct.can_fire(tgt):
                    ...
                    # QUEUE #83 (research #83 read): a gunner's shot is blocked
                    # by obstacles, so this is at most the nearest occupied
                    # tile in the line -- if that's an enemy BARRIER, don't
                    # fire and don't return; fall through toward _idle_rotate
                    # instead of pinning the facing on the rock (measured:
                    # 34.1% of our gunner shots land on barriers, one ate 243
                    # shots). Fix (b); fix (a) hold-fire-only was rejected as
                    # confounded (stops the leak, never re-aims).
                    barrier = (
                        hostile and bid is not None
                        and ct.get_entity_type(bid) == EntityType.BARRIER
                    )
                    if hostile and not (LOKI_GUNNER_UNPIN and barrier):
                        ct.fire(tgt)
                        return
            except Exception:
                pass
        ...
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                if et is None or not ct.can_fire(t):
                    continue
                # QUEUE #83 (research #83 read): shared with Sentinel, so
                # gate on turret_type -- a Gunner must never let a BARRIER
                # win this ladder (it pins the facing forever, see the fire
                # path above); a Sentinel's barrier shot is a deliberate
                # choice and stays untouched.
                if (
                    turret_type == EntityType.GUNNER
                    and LOKI_GUNNER_UNPIN
        ...
        try:
            for eid in ct.get_nearby_entities():
                if ct.get_team(eid) == self.team or not ct.can_fire(ct.get_position(eid)):
                    continue
                # QUEUE #83 (research #83 read): shared with Sentinel, same
                # gate as the ladder above -- a Gunner skips a BARRIER here
                # too instead of firing-and-returning, so it can keep
                # falling through to _idle_rotate; Sentinel is untouched.
                if (
                    turret_type == EntityType.GUNNER
                    and LOKI_GUNNER_UNPIN
