...
# LOKI-KILLTILE (QUEUE #87) -- TURRET KILL-TILE MEMORY, both turret-build call
# sites: _try_counterbattery's home/eco placement (main.py) and
# _try_forward_sentinel's forward plant (raid.py).
#
# MEASURED BASIS (research #87 sizing,
# docs/research/REPLAY-STUDY-TOPTEAM-MICRO-2026-08-16.md): we rebuild turrets
# on tiles where our own turret just died at 2.5-4x the field's rate (8.6% of
# turret builds on the v140 control, up to 25.4% on v152); 82.9% of such
# rebuilds are THE SAME BUILDER repeating itself [CI 75.4-88.6], exactly one
# builder touches the kill tile in >92% of cases (the canonical case: builder
# id 13 fed six builds to one tile), median rebuild gap 6 rounds, 87% within
# 40 rounds.
#
# THE ELEGANCE: no death detection is needed.  A still-standing turret makes
# its tile unbuildable anyway, so "in this builder's own memory + buildable
# again + recently built" IS the death signal -- can_build_gunner/
# can_build_sentinel already re-validates occupancy for free.
#
# Player instances are PER-UNIT and persist across rounds (official-docs:1725
# + disassembly), so this builder's own self.* is the right home for the
# memory -- zero comms-store cost, and it is the same builder repeating itself
# 82.9% of the time anyway, so a per-unit memory catches the mechanism
# directly rather than needing a team-wide ledger.
#
# THE VETO IS A PREFERENCE ORDERING, NOT A BUILD BAN: if every candidate site
# is in-memory-recent, the un-vetoed (i.e. first-found) choice is still taken
# -- building somewhere beats not building, and refusing to build at all would
# turn a wasteful rebuild into a worse outcome (no turret at all).
LOKI_KILLTILE_ON = True
LOKI_KILLTILE_ROUNDS = 45
