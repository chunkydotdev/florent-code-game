...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-KILLTILE (QUEUE #87), per unit: {(x, y): round_built} for
        # every gunner/sentinel THIS builder has planted, home or forward.
        # Pruned to the memory window on touch in _mark_kill_tile, so it
        # cannot grow with the match (see doctrine.py, LOKI_KILLTILE_ROUNDS). ---
        self.kill_tiles = {}
    ...
    # --- LOKI-KILLTILE (QUEUE #87): shared turret-site memory ---------------
    #
    # Shared by BOTH turret-build call sites in this file's mixin chain --
    # _try_counterbattery below (home/eco) and RaidMixin._try_forward_sentinel
    # (raid.py, forward plant) -- since Player(EcoMixin, RaidMixin) is one
    # class and self.kill_tiles is one per-unit dict.  See doctrine.py for the
    # sizing measurement and rationale.

    def _kill_tile_recent(self, ct, x, y):
        """True if THIS builder planted a turret on (x, y) within the window.

        Combined with the caller's own can_build_gunner/can_build_sentinel
        check, this alone reconstructs "my last turret here died": a still-
        standing turret makes the tile unbuildable regardless of memory, so
        buildable-again + recently-built-by-me is the death signal, no
        separate death detection needed.
        """
        if not LOKI_KILLTILE_ON:
            return False
        m = self.kill_tiles.get((x, y))
        if m is None:
            return False
        return ct.get_current_round() - m <= LOKI_KILLTILE_ROUNDS

    def _mark_kill_tile(self, ct, x, y):
        """Record a successful gunner/sentinel build at (x, y) by this unit."""
        if not LOKI_KILLTILE_ON:
            return
        rnd = ct.get_current_round()
        kt = self.kill_tiles
        kt[(x, y)] = rnd
        if len(kt) > 24:
            cut = rnd - LOKI_KILLTILE_ROUNDS
            self.kill_tiles = {k: v for k, v in kt.items() if v >= cut}

    def _build_turret_at(self, ct, turret_type, bp, facing):
        """Build the given turret type at bp/facing and mark the kill-tile
        memory.  Single choke point so every turret build -- home or
        forward -- feeds the same ledger."""
        if turret_type == EntityType.SENTINEL:
            ct.build_sentinel(bp, facing)
        else:
            ct.build_gunner(bp, facing)
        self._mark_kill_tile(ct, bp.x, bp.y)
    ...
    def _try_counterbattery(self, ct):
        ...
        # LOKI-KILLTILE fallback: the first candidate that validates but sits
        # on a recent kill tile.  Used only if the whole scan finds no
        # kill-tile-free candidate -- the veto is a preference ordering, not a
        # build ban (see doctrine.py).
        fallback = None
        for turret_type, cost in choices:
            ...
            for d in CARDINALS:
                ...
                for facing in DIRECTIONS:
                    ...
                    if not ok:
                        continue
                    if LOKI_KILLTILE_ON and self._kill_tile_recent(ct, bp.x, bp.y):
                        if fallback is None:
                            fallback = (turret_type, bp, facing)
                        continue
                    self._build_turret_at(ct, turret_type, bp, facing)
                    ct.write_store(SLOT_HOME_GUN, ct.read_store(SLOT_HOME_GUN) + 1)
                    return True
        if fallback is not None:
            turret_type, bp, facing = fallback
            self._build_turret_at(ct, turret_type, bp, facing)
            ct.write_store(SLOT_HOME_GUN, ct.read_store(SLOT_HOME_GUN) + 1)
            return True
        return False
