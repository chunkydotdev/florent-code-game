...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        # LOKI-KILLTILE fallback (QUEUE #87, see doctrine.py and
        # main.py._build_turret_at): the first candidate that validates but
        # sits on a tile THIS builder's own turret died on recently.  Used
        # only if the whole scan finds no kill-tile-free candidate -- the
        # veto is a preference ordering, not a build ban.
        fallback = None
        for d in CARDINALS:
            bp = p.add(d)
            ...
            for target in tiles:
                ...
                if LOKI_KILLTILE_ON and self._kill_tile_recent(ct, bp.x, bp.y):
                    if fallback is None:
                        fallback = (bp, facing)
                    continue
                self._build_turret_at(ct, EntityType.SENTINEL, bp, facing)
                self._publish_fwd_gun(ct, live)
                return True
    ...
    def _publish_fwd_gun(self, ct, live):
        """Advance SLOT_FWD_GUN after a forward Sentinel build (factored out
        of _try_forward_sentinel so both the primary and kill-tile-fallback
        build sites share one write path)."""
