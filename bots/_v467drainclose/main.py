...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # regardless of what the turret-count computation above landed
            # on (doctrine.py, LOKI-CLOSE block). max()-only, so this can
            # only ever RAISE ammo_target -- it never competes with or
            # narrows the `under` branch above, it only takes over once that
            # branch's own ceiling would be lower.
