...
# --- LOKI-KLADKILL: HARVESTER CAP -------------------------------------------
# Magnus strategy test 2026-08-16 s47, directed verbatim: "kladde eco start to
# at most 3 harvesters, then go for the kill with at least 2 sentinels, defend
# those."  Tighter than the general ECO_CAP (=18) above -- this stops the
# opening at 3 rather than letting the normal economy grow toward it, so the
# bank goes to sentinels/defence instead.  Read off SLOT_HARVESTERS
# (eco.py _sync_harvesters), the team-shared count -- NOT a second counter.
# KNOWN LIMITATION carried from that sync, unfixed here: it only updates from
# a builder standing within d^2<=64 of the Core (eco.py:375), roughly a
# sqrt(64)=8-tile radius -- about 6% of tiles on a 30x30 map -- so the count
# can lag on a large map with no builder near home. Used anyway: it is the one
# team-shared source, and inventing a second counter would just disagree with
# it under exactly the conditions that make either one hard to trust.
LOKI_HARV_CAP = 3
...
# --- LOKI-KLADKILL: ECO-GATED FORWARD-SENTINEL COMMITMENT -------------------
# Magnus strategy test 2026-08-16 s47, refined same day, quoted verbatim:
# "If we have 3 harvesters it should build 2 sentinels and peck and salt,
# otherwise it builds 1 sentinel and pecks and salts until we have 3
# harvesters then we build the second sentinel and third."
#
# Two thresholds, not one flat minimum, because the COMMITTED target moves
# with the eco phase (LOKI_HARV_CAP above): while the team harvester count is
# still below LOKI_HARV_CAP, a raider that can plant treats reaching
# LOKI_FWD_SENT_MIN_PRE (1) as the priority -- the plant attempt moves ahead
# of that raider's other discretionary actions (peck, salt) in raid.py
# _raid_act, though never ahead of survival/movement toward the Core. Once
# the harvester count reaches LOKI_HARV_CAP, the committed target rises to
# LOKI_FWD_SENT_MIN_POST (2) and, AT OR ABOVE whichever target currently
# applies, planting drops back to its ordinary (opportunistic) position --
# peck and salt proceed as the discretionary actions they already are.
#
# LOKI_FWD_GUN_CAP (=3, unchanged, NOT raised here) remains the hard monotone
# ceiling on forward sentinels ever built; a third sentinel can still arrive
# once the POST target (2) is already met, but only opportunistically, under
# that existing cap -- this plank does not touch the cap or how it counts.
LOKI_FWD_SENT_MIN_PRE = 1   # committed target while team harvesters < LOKI_HARV_CAP
LOKI_FWD_SENT_MIN_POST = 2  # committed target once team harvesters >= LOKI_HARV_CAP
...
# ---- composed by tools/stack.py from: turbo (_x3r0v152), bodyaware (_v242bodyaware), samestop (_v464samestop), kladkill (_v466kladkill)
