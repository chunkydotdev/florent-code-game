...
# QUEUE #83 fix (a), HOLD-FIRE ONLY -- shipped FIRST, ahead of fix (b).
# _v463unpin (fix (b): fall through to _idle_rotate on a barrier target) was
# fired and came back a real negative at 44.3, rotate-thrash suspected --
# the fall-through re-aims at 10 Ti per rotation while the blocking barrier
# persists, so a gunner pinned behind a rock pays to spin in place forever.
# Fix (a) costs ZERO: it only stops the 4-ammo-per-shot leak into a 3-Ti
# barrier (measured 34.1% of all our gunner shots / 11.9% of all ammo) and
# does NOT re-aim -- the gunner holds fire and stays aimed exactly where it
# was. No rotation, so no thrash is possible from this fix by construction.
# Sentinels are untouched everywhere this flag is read: their shots pierce,
# so a barrier target for a Sentinel is a deliberate choice.
LOKI_GUNNER_HOLDFIRE = True
