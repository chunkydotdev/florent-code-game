...
class Player(EcoMixin, RaidMixin):
    ...
    def _turret(self, ct):
        ...
        # QUEUE #83 fix (a), HOLD-FIRE ONLY (doctrine.LOKI_GUNNER_HOLDFIRE):
        # set True for this turn the moment a GUNNER's only fireable
        # candidate turns out to be an enemy BARRIER, so the rotate branch
        # at the bottom of this method can skip _idle_rotate -- hold fire
        # AND stay aimed, never re-aim. See doctrine.py for why fix (b)'s
        # fall-through-to-rotate was rejected (44.3, rotate-thrash).
        gunner_barrier_only = False
        ...
        if turret_type == EntityType.GUNNER:
            try:
                ...
                if tgt is not None and ct.can_fire(tgt):
                    ...
                    barrier = (
                        hostile and bid is not None
                        and ct.get_entity_type(bid) == EntityType.BARRIER
                    )
                    if hostile and LOKI_GUNNER_HOLDFIRE and barrier:
                        # Hold fire, stay aimed: do not feed the rock, and
                        # do not fall through to a re-aim either -- return
                        # ends this unit's turn with the facing untouched.
                        return
                    if hostile:
                        ct.fire(tgt)
                        return
        ...
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                if et is None or not ct.can_fire(t):
                    continue
                # QUEUE #83 fix (a): a BARRIER must never win selection for a
                # GUNNER -- skip it here, before prio is even computed, and
                # remember that a barrier was the reason this tile didn't
                # count, in case it turns out to be the only candidate this
                # turn. Sentinel is untouched (gated by turret_type): its
                # pierce-through shot on a barrier is a deliberate choice.
                if (
                    turret_type == EntityType.GUNNER
                    and LOKI_GUNNER_HOLDFIRE
                    and et == EntityType.BARRIER
                ):
                    gunner_barrier_only = True
                    continue
                prio = {
        ...
        try:
            for eid in ct.get_nearby_entities():
                if ct.get_team(eid) == self.team or not ct.can_fire(ct.get_position(eid)):
                    continue
                # QUEUE #83 fix (a): same gate as the ladder above -- a
                # GUNNER must not proceed to fire at a BARRIER here either
                # (this is the fallback the ladder falls through to when its
                # only candidate was a barrier). Remember it so the rotate
                # branch below stays silent too. Sentinel untouched.
                if (
                    turret_type == EntityType.GUNNER
                    and LOKI_GUNNER_HOLDFIRE
                    and ct.get_entity_type(eid) == EntityType.BARRIER
                ):
                    gunner_barrier_only = True
                    continue
            ...
            pass
        if gunner_barrier_only and LOKI_GUNNER_HOLDFIRE:
            # Hold fire, stay aimed: this turn's only fireable target was a
            # barrier, so skip the 10-Ti _idle_rotate re-aim entirely --
            # that fall-through is fix (b), the rejected rotate-thrash path.
            return
