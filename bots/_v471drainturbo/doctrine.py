...
# ---- composed by tools/stack.py from: turbo (_x3r0v152), bodyaware (_v242bodyaware)
...
# ============================================================================
# LOKI-CLOSE: THE WEALTH-AWARE CLOSE (v467, Magnus directive 2026-08-16 s47 --
# "if we don't do anything with a better eco then a better eco won't help").
#
# MEASURED BASIS (0033 study + research #89 + #80 bank series, 2026-08-16):
# we lose sieges on ammo and turret uptime WHILE HOLDING MONEY -- median 1,256
# Ti / 1.81 shots per 100r in farmed games against a field median of 14.38
# shots per 100r. That is not a poor bot, it is a funded one that never spent
# the fund. The ammo target in _core (main.py) already keys to LIVE TURRET
# COUNT (`ammo_target = max(24 if under else AMMO_FLOOR, min(48, 4 *
# weapons))`) -- a closed loop where a suppressed turret throttles its own
# magazine. And the forward-sentinel plant (raid.py, _try_forward_sentinel)
# gates on a bank floor (LOKI_FWD_TI_FLOOR) sized for a bot that is normally
# eco-poor, not for one sitting on a 1,256 Ti median.
#
# THE CONDITION. A siege is CLOSE-FUNDED when three things are simultaneously
# true: (1) a foothold at the enemy ring is actually live right now --
# `RaidMixin._foothold_live`, the same heartbeat that already gates
# reinforcement in `_raid_open`, so this plank never fires against an
# established siege that does not exist; (2) the bank holds a genuine
# post-endowment surplus, `get_global_resources() >= LOKI_CLOSE_TI` -- median
# bank dips below 260 Ti by r8, so 200 Ti is never opening float; (3) the
# round is past LOKI_CLOSE_MIN_RND. That round gate is NOT a claim that r60
# footholds are unsafe to fund -- a live foothold that early is rare by
# construction (LOKI_COLD_INSERT_RND=150 gates cold insertion, and an
# established raider needs time to reach the ring and report in) -- it exists
# only so the plank cannot touch the opening even in the tail case where a
# foothold heartbeat somehow fires early. See RaidMixin._close_funded
# (raid.py) for the single shared predicate both spends below read.
#
# SPEND 1 -- AMMO CEILING (main.py, _core). When close-funded, the ammo
# target is lifted to at least LOKI_CLOSE_AMMO = 96: two forward sentinels
# firing continuously cost 10 ammo/shot on a 2-round reload, so sustaining
# both takes ~10 ammo every reload tick: 96 buys roughly 28 net shots before
# the magazine needs a refill mid-grind, enough to put 500 Core HP through a
# heal line that is already sealed by the collar. This does not touch HOW
# ammo is bought -- the existing up-to-16-Ti/round conversion drip (the E1
# path) is the pump and is unmodified -- only the target level it fills
# toward, and only while the condition holds.
#
# SPEND 2 -- SENTINEL REPLANT (raid.py, _try_forward_sentinel). When
# close-funded, the bank floor guarding a forward-sentinel plant
# (LOKI_FWD_TI_FLOOR / LOKI2_RUSH_TI_FLOOR) is waived to 0. Same inversion
# LOKI-KLADKILL proved on its own gate -- a reserve that exists to guard
# LESSER spends must not be allowed to block the spend that outranks them --
# but applied here on the CLOSE condition alone. This tree carries no
# escalation ladder and must not grow one: LOKI-KLADKILL's staged floors are
# a different plank on a different base and are out of scope for v467.
# Everything else in the plant path -- the live-turret cap (LOKI_FWD_GUN_CAP),
# min_harv gate, CPU guard, siting scan -- is untouched.
LOKI_CLOSE_ON = True
LOKI_CLOSE_TI = 200         # bank floor that counts as genuine surplus, not opening float
LOKI_CLOSE_MIN_RND = 60     # foothold before this is rare; keeps the opening untouched by construction
LOKI_CLOSE_AMMO = 96        # two sentinels sustained: 10 ammo/shot, 2-round reload, ~28 net shots
