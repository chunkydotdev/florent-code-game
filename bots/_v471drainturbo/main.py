...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # LOKI-CLOSE: a live, funded siege raises the magazine ceiling
            # regardless of what the turret-count computation above (T4 BURN
            # CAP included) landed on (doctrine.py, LOKI-CLOSE block).
            # max()-only, so this can only ever RAISE ammo_target -- placed
            # AFTER the burn cap on purpose: that cap clamps DOWN to
            # T4_BURN_RNDS * T4_AMMO_PER_RND * weapons_top (e.g. 40 on one
            # live turret), well under LOKI_CLOSE_AMMO, so bumping before the
            # cap would let the cap immediately undo it.
            if self._close_funded(ct, rnd):
                ammo_target = max(ammo_target, LOKI_CLOSE_AMMO)
