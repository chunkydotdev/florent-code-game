...
class RaidMixin:
    ...
    def _close_funded(self, ct, rnd):
        """LOKI-CLOSE: is this the moment a wealth-aware close should spend?

        Three conditions, all load-bearing (see doctrine.py, LOKI-CLOSE
        block, for the measured basis): a foothold must actually be LIVE at
        the ring right now (reuses `_foothold_live` -- the same heartbeat
        that already gates reinforcement, so this never fires against a
        siege that does not exist), the bank must hold a genuine
        post-endowment surplus rather than opening float, and the round must
        be past the opening. Shared by both spends (main.py's ammo ceiling,
        raid.py's forward-sentinel floor waiver) so they read one predicate
        rather than two copies that could drift.
        """
        # v2 (demo-diagnosed, s47): the bank >= LOKI_CLOSE_TI term is REMOVED.
        # Circular for the funding waiver (a bank clearing 200 already clears
        # cost+40, so the waiver could never matter) and self-defeating for
        # the ammo ceiling (the close DRAINS the bank, switching the drain
        # off exactly while the siege spends). Wealth is now each spend's own
        # arithmetic: the ammo pump's reserve floor throttles when poor, the
        # sentinel cost check still charges full price. The condition is
        # simply THE CLOSE IS ON: a live foothold, past the opening.
        return (
            LOKI_CLOSE_ON
            and self._foothold_live(ct, rnd)
            and rnd >= LOKI_CLOSE_MIN_RND
        )
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        # LOKI-CLOSE: a live, funded siege outranks the reserve that guards
        # ordinary (non-close) replants -- same inversion LOKI-KLADKILL
        # proved on its own gate, applied here on the close condition alone
        # (doctrine.py, LOKI-CLOSE block). Gated on the CLOSE predicate, not
        # on an escalation ladder -- this tree has none and must not gain one.
        if self._close_funded(ct, rnd):
            ti_floor = 0
