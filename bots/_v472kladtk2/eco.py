...
class EcoMixin:
    ...
    def _sync_harvesters(self, ct):
        # LOKI-KLADKILL (coordinator defect report, 2026-08-16): left
        # UNTOUCHED, deliberately.  This function's own `live` is scoped to a
        # builder within d^2<=64 of the Core AND that builder's own vision --
        # a live-SIGHTING count, not a build count, and it plateaus below the
        # true total whenever the harvesters spread (they always do).  It is
        # safe anyway, because it can only ever RAISE the stored value
        # (`if live > stored`, never the reverse) and the harvester-build site
        # in `_expand` above already writes an unconditional build-counter +1
        # on every successful build, with no distance restriction.  With that
        # counter reaching and holding LOKI_HARV_CAP (=3), this function's
        # writes can never exceed it (live is bounded by the same 3 that were
        # ever built) and it degrades to a harmless no-op for this tree.
        if self.core is None:
            return
        p = ct.get_position()
    ...
    def _expand(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if not endgame and self.link_queue and self._build_next_link(ct):
                return
            # LOKI-KLADKILL (Magnus strategy test 2026-08-16 s47): "kladde eco
            # start to at most 3 harvesters, then go for the kill."  harv is
            # SLOT_HARVESTERS, the same team-shared count CHANGE 2 reads in
            # raid.py -- capped here BEFORE the existing endgame/eco_cap gate
            # ever runs, so it applies to the r960+ ENDGAME_SWITCH bootstrap
            # bypass too, not only the normal-phase branch below. That bypass
            # exists to buy the r1000 harvesters-alive tiebreak, a road this
            # project's own CLAUDE.md retires (R1000_IS_DEFEAT) -- a kill-
            # focused strategy has no reason to want harvester #4 built at
            # r970 either, so folding the cap over both branches is the
            # consistent reading, not a carve-out that needed inventing.
            # RACE FIX v2 (demo-diagnosed twice on this base, s47): a
            # same-round double-build both overshoots AND corrupts the
            # buffered count (two writes of read+1 record ONE build), so any
            # gate reading the store can sit one below actual forever -- the
            # v1 last-slot-owner fix still overshot through the lagged count.
            # THE CAP THEREFORE USES NO SHARED STATE AT ALL: one owner (the
            # lowest seat) builds every harvester, counting in its own
            # per-unit memory, which per-unit Player instances make exact.
            # The store keeps being written (single writer now = race-free
            # and accurate) for raid.py's eco-gated sentinel ladder. Failure
            # direction is SAFE: a dead seat-1 under-builds, never over.
            # Eco-phase cost: serialized builds, a few rounds slower than
            # parallel -- correctness of Magnus's "at most 3" budget
            # outranks them; the shard prices the trade.
            # v3 (shard-diagnosed, s47): the v2 single-owner serialization
            # STRUCTURALLY UNDER-BUILT on spread-ore maps (yulerune demo: 2
            # harvesters forever, ladder never armed, 31.9% collapse) -- the
            # "safe" under-direction is safe for the BUDGET, fatal for the
            # STRATEGY whose escalation gates on reaching the cap. v3 restores
            # PARALLEL builds and accepts the measured rare +1 overshoot
            # (same-round double-completion, ~20 Ti of noise); the LADDER is
            # made robust to the count's known undercount instead (raid.py
            # arms at CAP-1). Registered as "at most 3, occasionally 4".
            if harv < LOKI_HARV_CAP and (
                ...
                for ddx, ddy in DIR_DELTAS:
                    ...
                    if ok:
                        ...
                        # RACE FIX v2: the owner's own exact count (per-unit
                        # instance memory, no shared-state race possible).
                        self._my_harv = getattr(self, "_my_harv", 0) + 1
                        # LOKI-KLADKILL BUDGET SEMANTICS (coordinator defect
                        # report, 2026-08-16, demo drumlin/4711): LOKI_HARV_CAP
                        # means "at most 3 harvesters EVER", a start-phase
                        # BUDGET per Magnus's directive -- so the count this
                        # cap and CHANGE 2's fwd_target read must be a BUILD
                        # counter, not a live-sighting count (a died harvester
                        # is not replaced, and a budget that refilled on death
                        # would not be a cap at all). This unconditional +1 on
                        # every successful build (pre-existing in this economy
                        # layer, inherited unchanged from _v464samestop, NOT
                        # added for CHANGE 1/2) is exactly that build counter:
                        # it has no distance-to-Core restriction, unlike
                        # _sync_harvesters below, so it is the correct single
                        # source and nothing here needed adding.
                        # BENIGN RACE: store writes are buffered to next round
                        # and every unit sees one consistent per-round
                        # snapshot, so if two builders each successfully build
                        # a harvester in the SAME round, both read the same
                        # stale pre-round value and both write value+1 --
                        # whichever write lands last for the round wins, so
                        # the stored count can undercount by at most 1 for one
                        # round (self-corrects next build) and, symmetrically,
                        # the harv<LOKI_HARV_CAP gate above can let both
                        # builders through in that same round, overshooting
                        # the 3-harvester budget by at most 1. Not fixed here:
                        # narrow, self-bounded, and the alternative (a lock)
                        # is not worth it for a +/-1 budget slip.
