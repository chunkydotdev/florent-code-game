...
# LOKI-KLADKILL: private default for _try_forward_sentinel's `live` param --
...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # LOKI-KLADKILL (Magnus strategy test 2026-08-16 s47, refined same
        # day): the COMMITTED forward-sentinel target moves with the eco
        # phase -- LOKI_FWD_SENT_MIN_PRE (1) while the team harvester count is
        # still below LOKI_HARV_CAP (the same 3-harvester cap CHANGE 1 reads
        # off SLOT_HARVESTERS), LOKI_FWD_SENT_MIN_POST (2) once it reaches it.
        # `fwd_live` is computed once here (the same live-census
        # _try_forward_sentinel would otherwise recompute internally) and
        # threaded through both call sites below so the two don't scan twice
        # in the same turn. `fwd_priority` says which of those two positions
        # in the ranking below fires this round: below target, planting jumps
        # ahead of peck/salt (step 3); at or above target, it drops to
        # opportunistic -- after peck AND salt have had their turn -- so a
        # third sentinel can still arrive under the unchanged LOKI_FWD_GUN_CAP
        # (=3) but no longer preempts the discretionary actions.
        ...
        fwd_target = (
            LOKI_FWD_SENT_MIN_POST
            # v3: arm at CAP-1 -- the store's same-round double-write
            # UNDERCOUNTS by design (two writes of read+1 record one build),
            # so >= CAP could sit unarmed forever with 3 harvesters standing.
            # Arming one early is a mild dose drift; unarmed-forever was the
            # 31.9% collapse.
            if ct.read_store(SLOT_HARVESTERS) >= LOKI_HARV_CAP - 1
            else LOKI_FWD_SENT_MIN_PRE
        )
        ...
        # LOKI-KLADKILL: only while this raider's committed target (see
        # fwd_priority above) is not yet met -- at or above it, the attempt
        # moves to the opportunistic slot below, after peck and salt.
        if fwd_priority and self._try_forward_sentinel(ct, E, live=fwd_live, priority=True):
            return True
        ...
        # 8. THE FORWARD SENTINEL, OPPORTUNISTIC.  LOKI-KLADKILL: mirrors step
        # 3 but only fires when fwd_priority was False this turn -- the
        # committed target (LOKI_FWD_SENT_MIN_PRE/_POST) is already met, so
        # this raider let peck and salt go first, in their ordinary ranked
        # positions above, and only reaches for a further plant (up to the
        # unchanged LOKI_FWD_GUN_CAP=3) if neither of them found an action.
    ...
    def _try_forward_sentinel(self, ct, E, live=_FWD_LIVE_UNSET, priority=False):
        ...
        LOKI-KLADKILL: `live` is optional and defaults to _FWD_LIVE_UNSET (a
        ...
        fwd_priority decision and passes it through here so the two call
        sites (steps 3 and 8) don't each re-scan get_nearby_buildings().
        """
        ...
        # Magnus strategy test 2026-08-16 s47 (demo-diagnosed): while the
        # sentinel commitment is the raider's PRIORITY (live < eco-gated
        # target), the reserve floor is WAIVED — it exists to keep change for
        # lesser spends, and gating the priority spend behind a reserve for
        # the spends it outranks is inverted. Demo evidence: 389 rounds at
        # ~3 Ti/round income never reached cost+40, sentinel #2 never landed.
        # Outside priority the floor applies exactly as before.
        if priority:
            ti_floor = 0
