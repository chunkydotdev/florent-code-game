...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # target, planting jumps ahead of peck/salt (step 3, the base's own
        # slot) and the bank floor is waived; at or above target it drops to
        # opportunistic (step 8, after peck AND salt), so a third sentinel can
        # still arrive under the unchanged LOKI_FWD_GUN_CAP (=3) without
        # preempting the discretionary actions.
        #
        ...
        # priority and the waiver, and the raid replants.  A raider blind to
        # the band gets `None` and falls back to the MONOTONE store, which
        # never decrements -- so blind raiders do not replant.  That is
        ...
        if LOKI_LADDER_ON and LOKI_FWD_SENTINEL_ON:
            ...
            fwd_waive = fwd_priority
        else:
            # FLAG OFF == THE BASE, EXACTLY: the census is not paid for, step 3
            # keeps its unconditional attempt, the reserve floor is NOT waived,
            # and step 8 never fires.
        ...
        if fwd_priority and self._try_forward_sentinel(
            ct, E, live=fwd_live, waive_floor=fwd_waive
        ):
            return True
        ...
        # of them found an action.  The reserve floor is NOT waived here: this
        # spend no longer outranks the spends the floor exists to protect.
    ...
    def _try_forward_sentinel(self, ct, E, live=_FWD_LIVE_UNSET, waive_floor=False):
        ...
        `waive_floor` is set only on the PRIORITIZED call site; see the waiver
        below.
        """
        ...
        # LOKI-KLADLADDER (demo-diagnosed on KLADTK2, s47): while the sentinel
        # commitment is this raider's PRIORITY (live < the eco-gated target),
        # the reserve floor is WAIVED -- it exists to keep change for LESSER
        # spends, and gating the priority spend behind a reserve for the spends
        # it outranks is inverted.  Demo evidence from that build: 389 rounds
        # at ~3 Ti/round income never reached cost+40 and sentinel #2 never
        # landed.  Outside priority (step 8) the floor applies exactly as
        # before, so the waiver is bounded by the same live<target condition
        # that ends it -- the belt planner is never starved permanently.
        if waive_floor:
            ti_floor = 0
