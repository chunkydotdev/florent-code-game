...
# ⭐ THIS TREE IS THE DOSE ARM.  It differs from _v474sealsentA in ONE
# CONSTANT -- LOKI_SEALSENT_MAX, 1 -> 2 -- and in nothing else; every other
# byte of doctrine.py, raid.py, eco.py and main.py is identical to that tree.
# So A vs B measures DOSE and only dose: is the second substituted seat worth
# a second 30 Ti and a second +20% on the global cost factor, or does the
# first one carry the whole effect?  Priced: A costs at most +20% scale and
# +27 Ti over the base, B at most +40% and +54 Ti; both are 0% and 0 Ti in a
# game that would have filled LOKI_FWD_GUN_CAP (=3) anyway, since the shared
# cap is unchanged in both and a seat Sentinel counts against it.
LOKI_SEALSENT_MAX = 2          # N-CAP: seat Sentinels live in the band at once
