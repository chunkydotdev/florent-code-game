...
LOKI_SEALSENT_FUND_ON = True   # the eco-deferral contention guard
# ⭐ THIS TREE IS THE ENFILADE ARM, AND IT IS A DIFFERENT MECHANISM, NOT A
# DIFFERENT DOSE.  A and B SUBSTITUTE: the Sentinel takes the seat the barrier
# would have taken, so the seal is bought once and shoots.  C ADDS: **every
# barrier still goes down exactly as in _v468kladturbo** -- the barrier loop is
# never asked to skip a tile, because `_sealsent_site` in "enfilade" mode
# refuses every tile that is on the twelve-tile ring at all -- and one Sentinel
# is planted one tile OUTSIDE the ring, sited so its line RAKES AT LEAST TWO OF
# THEIR HEAL SEATS.
#
# WHAT IT IS FOR: the barrier's failure mode.  A barrier does not punish the
# builder that comes to remove it; it only makes the removal slow (15 pecks, 30
# Ti, 15 builder-turns).  The whole time, that builder is standing ON a seat --
# and a seat is exactly what this Sentinel's line covers.  So the barrier
# becomes the bait and the Sentinel the answer: 18 damage on a 2-round reload
# kills a 40 HP builder in 6 rounds while it is 14 pecks from finishing.  It is
# the GUNBLOCK inverse -- our own collar would blind a Gunner, and a Sentinel's
# line ignores obstacles, so it fires THROUGH the barrier it is protecting.
#
# WHY OFF-RING AND NOT ON A CORNER.  A corner is a raider BUILD STATION (it is
# orthogonally adjacent to the two seats flanking it and to no Core tile), so a
# building there costs the raid a station permanently.  One tile further out
# costs nothing and still rakes two seats: from (-2,-1) facing EAST the line
# runs (-1,-1) corner, (0,-1) seat, (1,-1) seat, (2,-1) corner.
#
# COST.  C is strictly ADDITIVE -- +30 Ti and at most +20% scale over the base,
# with none of it recovered from a displaced barrier (A/B recover 3 Ti).  It is
# 0% again whenever the shared LOKI_FWD_GUN_CAP would have been filled anyway.
LOKI_SEALSENT_MODE = "enfilade"   # "seat" == substitute on the ring;
                               # "enfilade" == add off-ring, barrier untouched
LOKI_SEALSENT_MAX = 1          # N-CAP: band Sentinels live at once
LOKI_SEALSENT_CENSUS_MIN = 3   # N-cap census band, INCLUSIVE, dsq_core:
LOKI_SEALSENT_CENSUS_MAX = 8   # the one-tile-out enfilade band ONLY (4-5).
                               # It starts at 3 so a SEAT (1) or a CORNER (2)
                               # Sentinel -- including one the base's own
                               # `_try_forward_sentinel` planted -- does not
                               # close this arm's cap.
