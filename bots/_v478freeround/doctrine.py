...
# ============================================================================
# LOKI-FREEROUND (s48) -- KILL THE BLIND BACKSTEP.  Study mechanism M2,
# docs/research/ECO-STUDY-fast-connected-harvesters-2026-08-17.md §M2.
#
# ⭐⭐ THIS IS A GENERALISATION OF MACHINERY THIS TREE ALREADY SHIPS, NOT A NEW
# MECHANISM.  `_t4_chase_ok` (main.py, T4_CHASE_* above) is ALREADY a
# last-K-position oscillation detector: it keeps the positions this unit held
# over T4_CHASE_MAX_RNDS consecutive rounds and trips on
# `len(hist) >= T4_CHASE_MAX_RNDS and len(set(hist)) <= 2` -- "the test is on
# the SET of tiles held over the window, not on standing still", from its own
# docstring, written for a decoded case that alternated (0,8)/(0,9) forever.
# It exists for ONE role (the home defender) and ONE target class (an intruding
# builder).  THE DETECTOR IS RIGHT AND ITS SCOPE IS TOO NARROW.  This plank
# lifts the same window mechanics -- same history list, same contiguity reset,
# same `<= 2` set test, same banked cooldown -- into `_nav`, where every
# builder in the tree walks.
#
# THE DEFECT IT IS AIMED AT, in the parent's own three lines (eco.py `_nav`).
# When the BFS step is illegal, `_nav` falls through to (left, right, REVERSE)
# and takes whichever moves.  `_move` returning True is `_nav`'s only notion of
# success, so a REVERSE counts as a successful move, and `self.stuck`
# increments only when ALL FOUR cardinals fail.  Consequence: inside an A->B->A
# oscillation every round "succeeds", `stuck` stays at 0 forever, and the
# escape hatch (`self.stuck >= 5` -> re-pick, eco.py `_expand`) CANNOT FIRE --
# the one mechanism in the file built to break exactly this is unreachable from
# inside it.  The other exit, `p == self.tgt`, is unreachable too: an
# oscillating builder is by definition not standing on its target.
#
# BACKING.  12.0% of our v155 rated builder-rounds are A->B->A oscillation
# (58,154 builder-rounds over 110 sides; 11.2% unrated).  Field bounds on the
# same metric: team lazy 2.1%, Torsko 5.2% -- i.e. 7-10pp of ALL builder-rounds
# is reachable headroom.  Seat 4 is the worst offender at 20.4% of its moves,
# while standing at d^2 = 12.5 from our OWN Core.  And it cashes out in builds:
# Torsko builds on 13.7% of its builder-rounds against our 8.2%.
#
# ⛔ WHAT IT DOES WITH THE DETECTION IS WHERE IT DIFFERS FROM THE TWO ARMS THAT
# DIED.  OSCLOCK (48.53) and OSCLOCK2 detected and RE-PICKED -- they changed
# the TARGET.  `_t4_chase_ok` also bans a target, which is right for a chase
# because a chase has an alternative (seats, launcher, expansion) and an ore
# walk does not.  QUEUE #54 is explicit that the fix must change NAVIGATION.
# So the detector is `_t4_chase_ok`'s and the CONSEQUENCE is a move-legality
# ban: for FREEROUND_COOLDOWN rounds after a trip, `_nav` will not step BACK
# ONTO the tiles of the two-cycle it just detected, and takes any other legal
# cardinal in the parent's own preference order instead.  The target is never
# touched.
#
# THREE PROPERTIES THAT KEEP IT SAFE, all three carried from the template:
#   * CONTIGUITY.  `nav_osc_since != rnd - 1` clears the history, exactly as
#     `t4_chase_since != rnd - 1` does -- so a launcher throw, a build round or
#     a cooldown gap resets rather than convicting on stale evidence.
#   * THE HISTORY IS OF ROUNDS THIS BUILDER NAVIGATED, and it is sampled inside
#     `_nav` where the parent already decided to move, so a builder parked on
#     purpose (converging, stationed, wiring) contributes nothing to it.
#   * THE BANNED STEP IS STILL TAKEN WHEN IT IS THE ONLY LEGAL ONE -- a
#     corridor or a dead end must not lock a builder where the parent at least
#     walked back out -- but `self.stuck` increments when that happens, which
#     is the second half of the measured defect: it re-arms the escape hatch
#     that could not fire before.  One forced reverse changes nothing; five
#     re-picks.
#
# ⚠ SEAM.  `_nav` is the sole consumer of `_bfs_direction`, the hand-merged
# TURBO x BODYAWARE block (eco.py:1236-1456) and the highest-risk region in
# this tree.  NOT ONE LINE OF THAT BLOCK IS TOUCHED, and the call to it is
# unchanged in position, arguments and handling.  Everything here is what
# `_nav` does with the answer.
# ============================================================================

LOKI_FREEROUND_ON = True        # the whole plank; False == the parent exactly
# Window and set test: the template's own shape. SHORTER than T4_CHASE_MAX_RNDS
# (6) on purpose -- a chase can afford six rounds of evidence because it has
# somewhere else to be, while a walking builder spending six rounds on two
# tiles has already lost the rounds this plank exists to recover. Three is the
# shortest window in which A->B->A is distinguishable from a legitimate
# there-and-back at all.
FREEROUND_WINDOW = 3
FREEROUND_SET_MAX = 2           # `len(set(hist)) <= 2` -- the template's test
# Rounds the two-cycle's tiles stay refused after a trip. The template banks
# T4_CHASE_COOLDOWN = 20 rounds for a chase; a nav ban is far more intrusive
# per round (it constrains every step, not one optional behaviour), so this is
# deliberately short -- long enough to break the cycle and let the flood find
# another way round, short enough that a genuine corridor costs a handful of
# rounds and not twenty.
FREEROUND_COOLDOWN = 4
# Instrumentation only: one print per trip, per redirected step and per forced
# step, into the replay. LOCAL instrument only -- platform-downloaded replays
# strip stdout (30,664 of 30,664). Dose tags "FR2TRIP", "FR2SKIP", "FR2FORCE".
LOKI_FREEROUND_LOG = True
