...
class EcoMixin:
    ...
    def _freeround_tick(self, ct, p):
        """`_t4_chase_ok`'s detector, generalised from one role to every walk.

        Line for line the same shape as main.py `_t4_chase_ok`: a history of
        the positions this unit held over a window of CONSECUTIVE rounds, reset
        the moment the rounds stop being consecutive, tripped on the SET of
        tiles rather than on standing still, and banked for a cooldown once it
        trips.  What differs is only the scope (every navigating builder, not
        the home defender) and the consequence (a move ban, set up by the
        caller below, not a target ban).

        Returns the set of tiles this builder may not step ONTO this round --
        empty when the plank is off, when nothing has tripped, or when the
        cooldown has lapsed.  `p` is passed in because `_nav`'s caller already
        holds it and the detector must sample the SAME position the move is
        decided from.
        """
        if not LOKI_FREEROUND_ON:
            return ()
        rnd = ct.get_current_round()
        if rnd < self.nav_osc_until:
            return self.nav_osc_ban
        hist = self.nav_osc_pos
        if hist is None or self.nav_osc_since != rnd - 1:
            hist = []
            self.nav_osc_pos = hist
        self.nav_osc_since = rnd
        hist.append((p.x, p.y))
        if len(hist) > FREEROUND_WINDOW:
            del hist[0]
        if len(hist) >= FREEROUND_WINDOW and len(set(hist)) <= FREEROUND_SET_MAX:
            # The two-cycle, caught. Ban its tiles EXCEPT the one we are
            # standing on -- a move onto our own tile is not a move, and
            # leaving it out keeps the ban a statement about where we may go
            # rather than about where we are.
            ban = frozenset(t for t in set(hist) if t != (p.x, p.y))
            self.nav_osc_until = rnd + FREEROUND_COOLDOWN
            self.nav_osc_ban = ban
            self.nav_osc_pos = None
            self.nav_osc_since = None
            # ⭐ AND THIS LINE IS THE OTHER HALF OF THE MEASURED DEFECT, not a
            # second mechanism.  The study's sentence is that `_nav` "counts a
            # backstep as a successful move ... so the escape hatch can never
            # fire inside an oscillation": `self.stuck` increments only when
            # all four cardinals fail, which never happens while the builder is
            # happily bouncing.  A trip is the evidence the parent's own
            # counter was built to accumulate and never got, so it is fed here.
            # ⛔ NOTE WHAT THIS IS NOT: it is not detect-and-repick.  OSCLOCK
            # (48.53) and OSCLOCK2 died re-picking on the FIRST detection.
            # This feeds the PARENT'S OWN hatch at its PARENT'S OWN threshold
            # -- five accumulated trips, through the unchanged `stuck >= 5`
            # test in `_expand` -- so the tile ban is still what breaks the
            # cycle and the re-pick is a last resort that was always in the
            # file.
            self.stuck += 1
            if LOKI_FREEROUND_LOG:
                print(f"FR2TRIP rnd={rnd} at={p.x},{p.y} "
                      f"ban={sorted(ban)} id={ct.get_id()}")
            return ban
        self.nav_osc_ban = ()
        return ()
    ...
    def _nav(self, ct, pave=True):
        ...
        if desired == Direction.CENTRE:
            return
        # LOKI-FREEROUND (s48).  The candidate order below -- BFS step, then
        # left, right, reverse -- is the parent's, unchanged.  What changes is
        # that a step landing on a tile of a DETECTED two-cycle is skipped on
        # this pass and retried only if nothing else was legal.  With the plank
        # off `ban` is the empty tuple, the membership test can never match,
        # `held` stays empty and the second pass is unreachable, so this loop
        # performs exactly the parent's sequence of `_move` calls.
        p = ct.get_position()
        ban = self._freeround_tick(ct, p)
        idx = CARDINALS.index(desired) if desired in CARDINALS else 0
        order = (desired, CARDINALS[(idx + 1) % 4],
                 CARDINALS[(idx + 3) % 4], desired.opposite())
        held = []
        for d in order:
            if ban:
                ndx, ndy = DELTA[d]
                if (p.x + ndx, p.y + ndy) in ban:
                    held.append(d)
                    continue
            if self._move(ct, d, pave):
                # Logged HERE and only here, so the tag marks the rounds where
                # the ban actually CHANGED the outcome: a direction earlier in
                # the parent's own preference order was refused and a later one
                # carried the move.  A round where the ban never came up is
                # silent, so the count is a dose and not a heartbeat.
                if held and LOKI_FREEROUND_LOG:
                    print(f"FR2SKIP rnd={ct.get_current_round()} "
                          f"held={held[0].name} took={d.name} id={ct.get_id()}")
                return
        if held:
            # THE NEGATIVE CONTROL, in the code rather than in the report: on a
            # corridor or a dead end the banned tile is the only way out, and
            # refusing it outright would lock the builder where the parent at
            # least walked back.  So we take it -- and count it as stuck, which
            # is the half of the defect the study names: `self.stuck` was
            # unreachable from inside an oscillation, so the escape hatch
            # (`stuck >= 5` -> re-pick a different ore) could never fire.
            if LOKI_FREEROUND_LOG:
                print(f"FR2FORCE rnd={ct.get_current_round()} "
                      f"dir={held[0].name} id={ct.get_id()}")
            self.stuck += 1
            for d in held:
                if self._move(ct, d, pave):
                    return
