...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-FREEROUND (s48), per unit.  Deliberately the SAME THREE
        # FIELDS as the t4_chase trio directly above, with the same meanings
        # and the same sentinels, because this plank is that detector
        # generalised from the defender's chase to every builder's walk: the
        # position history over a window of consecutive rounds, the round it
        # was last sampled, and the round its cooldown lapses.  `nav_osc_ban`
        # is the one addition -- the tiles of the two-cycle that tripped it,
        # which is what a NAVIGATION ban needs and a target ban did not. ---
        self.nav_osc_pos = None         # builder: positions held in the window
        self.nav_osc_since = None       # builder: the round it was last sampled
        self.nav_osc_until = -10 ** 9   # builder: ban lapses at this round
        self.nav_osc_ban = ()           # builder: tiles refused while banned
