...
# ============================================================================
# LOKI-ROUTESCORE (s48) -- SCORE ORE BY THE ROUTE, NOT BY THE CROW.  Study
# mechanism M3, docs/research/ECO-STUDY-fast-connected-harvesters-2026-08-17.md
#
# THE DEFECT.  `_pick` (eco.py) sorts every ore on the map by
# `abs(t.x - core.x) + abs(t.y - core.y)` -- straight-line Manhattan distance
# to our Core -- and that is the ONLY geometry term in the file.  The length of
# the belt the builder will then have to BUILD is never scored.  On any map
# with a wall between the Core and a near deposit, the two numbers disagree,
# and the parent always takes the crow's answer.
#
# WHY IT IS WORTH A TREE.  Connect latency is ~2.00 rounds per tile of BFS
# route length (Theil-Sen plus binned medians to L = 6 over 1,730 harvesters),
# and it degrades WORSE than linearly past L ~ 8: at L = 10 the measured median
# is 32 rounds against the linear model's 20.  Route length is exactly what
# deteriorates by ordinal -- d^2 to our Core runs 18 -> 25 -> 37 -> 50 -> 61 ->
# 85 for harvesters #1-#6 while the ever-connected share falls 92% -> 91% ->
# 81% -> 71% -> 66% -> 64%.  Route length is therefore the DIRECT lever on
# both halves of the eco clock, and it is the one the parent does not pull.
#
# ⭐ NO ORDINAL SPECIAL-CASING, AND THAT IS DELIBERATE.  The obvious "cheap"
# variant scores only the later harvesters, since #1 and #2 connect at 92%/91%
# anyway.  The study says the opposite: the connect rate falls to 64% at
# harvesters #3-#6, so route scoring MATTERS MOST THERE -- and a builder's
# ordinal is not knowable to it (SLOT_HARVESTERS is a team-wide store with a
# known same-round undercount).  Scoring every pick is both simpler and the
# only form that reaches the ordinals the measurement is about.
#
# ⛔ THE PARTITION IS NOT TOUCHED.  `_pick`'s Manhattan sort and its
# `ordered[worker::workers]` stripe are what keep four builders off one
# deposit.  Both are computed EXACTLY as in the parent and the stripe is
# preserved verbatim; the change is only WHICH member of this seat's own
# stripe the cursor hands back.  The study's own warning is that a
# route-length score pulls all seats toward the same near cluster -- keeping
# the stripe is what prevents that, so it is load-bearing, not inherited.
#
# THE CPU OBJECTION IS DEAD AND MEASURED: p99 execution time is 550 us against
# a 10,000 us budget, 94.5% free (study §6.5b).  At v140, when p90 on big maps
# was 8,150 us, this change would have been reckless.  It is not now.  The
# budget is kept anyway and in three forms -- a candidate cap, a TTL cache, and
# the up-front CPU probe -- because "measured free" is a fact about today's
# tree and the guard costs nothing.
#
# WHERE THE FLOODS COME FROM.  `_link_path`, the same router SAMESTOP and the
# trunk planner already use, under the same LINK_NODE_BUDGET (= NAV_NODE_BUDGET
# = 4096) node cap.  No parallel router (research #50 cut, step 3c).  `_pick`
# is called only when the builder has no target, has arrived, or is stuck --
# not every round -- so the candidate floods are paid a handful of times per
# builder per game, not per turn.
# ============================================================================

LOKI_ROUTESCORE_ON = True       # the whole plank; False == the parent exactly
# How many members of this seat's stripe to score, starting at the cursor.
# Bounds the floods per `_pick` call. Four is the study's "bound the candidate
# list" with room to actually change an answer: on the pool maps a seat's
# stripe holds ~10 tiles, so four is a real choice and not a rubber stamp.
ROUTESCORE_CANDIDATES = 4
# Rounds a scored route length stays cached for an ore tile. The route only
# changes when belt or blockers move, and a stale length costs at most one
# suboptimal pick -- against re-flooding four tiles every time a builder loses
# its target.
ROUTESCORE_TTL_RNDS = 25
# Routes longer than this are scored as unreachable-for-ranking rather than
# compared on length: past the knee the corpus measures (see ECOMMIT's
# MAX_LINK_TILES note -- L = 10 medians 32 rounds against a linear 20) the
# difference between 11 and 14 tiles is not a difference worth a pick.
ROUTESCORE_FAR_TILES = 12
# Instrumentation only: one print per pick where the route-length winner is
# NOT the tile the parent's cursor would have returned. LOCAL instrument only
# -- platform-downloaded replays strip stdout (30,664 of 30,664). Dose tag
# "RS3PICK".
