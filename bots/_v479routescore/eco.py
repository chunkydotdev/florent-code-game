...
class EcoMixin:
    ...
    # --- LOKI-ROUTESCORE (s48): rank a seat's own ore candidates by the
    # length of the belt we would have to BUILD, not by the crow's distance
    # to the Core.  See doctrine.py for the 2.00-rounds-per-tile measurement
    # and for why the stripe partition is preserved verbatim. -------------

    def _routescore_len(self, ct, ore):
        """BFS route length home for `ore`, in tiles, with a TTL cache.

        Returns 0 for a site that is already home -- one in the Core ring, or
        one with an acceptor beside it -- because that is a route of length
        zero and not a missing answer; `_link_path` returns [] for both a
        Core-ring start and an unroutable one, and conflating those would rank
        the best possible site with the worst.  Unroutable is scored
        ROUTESCORE_FAR_TILES + 1, i.e. below every real route but still
        ORDERED, so a seat whose whole stripe is unreachable still gets a tile
        back rather than falling through to the wander.
        """
        key = (ore.x, ore.y)
        rnd = ct.get_current_round()
        hit = self.rs_len_cache.get(key)
        if hit is not None and rnd - hit[1] < ROUTESCORE_TTL_RNDS:
            return hit[0]
        try:
            if key in self._link_goals(self._pave_ban()) or self._has_acceptor(ct, ore):
                out = 0
            else:
                plan = self._link_path(ct, ore)
                out = len(plan) if plan else ROUTESCORE_FAR_TILES + 1
        except Exception:
            out = ROUTESCORE_FAR_TILES + 1
        if out > ROUTESCORE_FAR_TILES:
            out = ROUTESCORE_FAR_TILES + 1     # past the knee, all the same
        self.rs_len_cache[key] = (out, rnd)
        return out

    def _routescore_best(self, ct, cands):
        """The candidate with the shortest route home; `cands[0]` on a tie.

        `cands[0]` is precisely the tile the parent's cursor would have
        returned, so the tie-break is "change nothing unless the route says
        so" -- and the strict `<` below means an equal-length route never
        displaces it.  That keeps this deterministic and keeps every pick the
        route cannot separate byte-identical to the parent's.

        The CPU probe is asked ONCE, up front, and degrades to `cands[0]`:
        a unit that arrives here already spent gets the parent's answer rather
        than a truncated scan.  This mirrors `_bfs_direction`'s own up-front
        probe rather than inventing a second policy.
        """
        if not LOKI_ROUTESCORE_ON or len(cands) == 1:
            return cands[0]
        ...
        if self._cpu_exhausted(ct):
            return cands[0]
        best = cands[0]
        best_l = self._routescore_len(ct, best)
        for t in cands[1:]:
            if best_l == 0:
                break                          # cannot be beaten
            l = self._routescore_len(ct, t)
            if l < best_l:
                best, best_l = t, l
        if LOKI_ROUTESCORE_LOG and (best.x, best.y) != (cands[0].x, cands[0].y):
            print(f"RS3PICK rnd={ct.get_current_round()} "
                  f"took={best.x},{best.y} L={best_l} "
                  f"over={cands[0].x},{cands[0].y} "
                  f"L0={self._routescore_len(ct, cands[0])} id={ct.get_id()}")
        return best
    ...
    def _pick(self, ct):
        ...
        if self.map_ores and self.role != "defend":
            ...
            if self._pick_key != key or assigned is None:
                ...
                self._pick_key = key
            # LOKI-ROUTESCORE (s48): the parent's cursor walk, kept whole,
            # with one insertion. It still walks THIS SEAT'S OWN STRIPE in
            # cursor order and still skips a tile it can see is taken; the
            # only change is that instead of returning the FIRST such tile it
            # collects up to ROUTESCORE_CANDIDATES of them and returns the one
            # whose route home is shortest. With the plank off the candidate
            # list is capped at one and this is the parent's loop exactly.
            n_cand = ROUTESCORE_CANDIDATES if LOKI_ROUTESCORE_ON else 1
            cands = []
            for _ in range(len(assigned)):
                ...
                if len(cands) >= n_cand:
                    break
            if cands:
                return self._routescore_best(ct, cands)
