...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-ROUTESCORE (s48), per unit. (x, y) -> (route_len, round) for
        # the ore tiles this builder has scored. Kept SEPARATE from the
        # samestop one-slot cache on purpose: that one holds a PLAN for the
        # tile a builder is walking to and is evicted every time the target
        # moves, while this holds only LENGTHS for several candidates and must
        # survive across picks. It is bounded by the map's ore count. ---
        self.rs_len_cache = {}
