...
# LOKI-KLADLADDER: private default for _try_forward_sentinel's `live` param --
# distinct from a real `None`, which _live_fwd_guns already uses to mean
# "this raider is blind to the siege band, fall back to the monotone store".
_FWD_LIVE_UNSET = object()
...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # --- LOKI-KLADLADDER: the eco-gated sentinel commitment -------------
        # Doctrine block "LOKI-KLADLADDER" carries the full rationale and the
        # arming-trigger decision; this is the mechanism.
        #
        # `fwd_live` is the live forward-sentinel census, computed ONCE here
        # (the same one `_try_forward_sentinel` would otherwise recompute) and
        # threaded through both call sites below so no raider scans
        # get_nearby_buildings() twice in a turn.  Guarded by
        # LOKI_FWD_SENTINEL_ON so a disabled arm pays no census -- in the base
        # the census lived AFTER that flag check inside the function.
        #
        # `fwd_target` is the committed number of LIVE forward sentinels:
        #   0  before a plant is even legal (below the effective min_harv),
        #   PRE (1) from there,
        #   POST (2) once LOKI_LADDER_POST_HARV harvesters have been BUILT.
        # The PRE arm mirrors `_try_forward_sentinel`'s own `min_harv` --
        # including the LOKI-2 rush relaxation -- rather than restating a
        # constant, so the "arm exactly when a plant becomes legal" seam
        # cannot drift if that gate is ever retuned.
        #
        # `fwd_priority` decides WHICH of the two slots below fires: below
        # target, planting takes step 3 (the base's own slot, ahead of salt);
        # at or above target it drops to opportunistic (step 8, after peck AND
        # salt), so a third sentinel can still arrive under the unchanged
        # LOKI_FWD_GUN_CAP (=3) without preempting the discretionary actions.
        #
        # ⛔ KLADLADDER2 (s48, iteration 2): `fwd_waive` IS GONE.  In
        # _v473kladladder this same flag ALSO waived the bank reserve floor for
        # the priority plant (ti_floor -> 0), and that waiver is the plank the
        # Sleipnir screen killed at 41.86% [40.20,43.52] n=3,404 while the dose
        # showed builder deaths at 1.91x.  Here the floor ALWAYS binds: a plant
        # waits for a bank that covers cost + LOKI_FWD_TI_FLOOR, i.e. it is
        # funded out of SURPLUS and never out of another unit's spending.
        # `fwd_priority` is now a pure ORDERING of THIS raider's own act ladder
        # and is read at exactly the two call sites below; nothing in eco.py or
        # main.py sees it (both are byte-identical to _v468kladturbo).
        #
        # SELF-HEALING falls out of comparing against the LIVE census: a
        # sentinel dying drops fwd_live_n below fwd_target, which re-arms
        # priority and the raid replants (once the surplus is there).  A raider
        # blind to the band gets `None` and falls back to the MONOTONE store,
        # which never decrements -- so blind raiders do not replant.  That is
        # inherited behaviour, not new here.
        if LOKI_LADDER_ON and LOKI_FWD_SENTINEL_ON:
            fwd_live = self._live_fwd_guns(ct, E) if LOKI2B_LIVE_CAP_ON else None
            fwd_live_n = fwd_live if fwd_live is not None else ct.read_store(SLOT_FWD_GUN)
            _rush = LOKI2_RUSH_ON and ct.get_current_round() < LOKI2_RUSH_RND
            _pre_arm = LOKI2_RUSH_MIN_HARV if _rush else LOKI_FWD_MIN_HARV
            _harv = ct.read_store(SLOT_HARVESTERS)
            if _harv >= LOKI_LADDER_POST_HARV:
                fwd_target = LOKI_FWD_SENT_MIN_POST
            elif _harv >= _pre_arm:
                fwd_target = LOKI_FWD_SENT_MIN_PRE
            else:
                fwd_target = 0
            fwd_priority = fwd_live_n < fwd_target
        else:
            # FLAG OFF == THE BASE, EXACTLY: the census is not paid for, step 3
            # keeps its unconditional attempt, and step 8 never fires.  (The
            # reserve floor needs no mention here any more -- it binds on every
            # path in this tree, flag on or off.)
            fwd_live = _FWD_LIVE_UNSET
            fwd_priority = True
        ...
        # 3. THE FORWARD SENTINEL, PRIORITIZED.  Built on the approach, before
        # the ring is reached, because that is where the 5-tile band is.
        # LOKI-KLADLADDER: only while this raider's committed target is not yet
        # met (see fwd_priority above) -- at or above it, the attempt moves to
        # the opportunistic slot below, after peck and salt.
        # KLADLADDER2: no `waive_floor` argument exists any more; this call is
        # bank-for-bank identical to the base's step 3.
        if fwd_priority and self._try_forward_sentinel(ct, E, live=fwd_live):
            return True
        ...
        # 8. THE FORWARD SENTINEL, OPPORTUNISTIC.  LOKI-KLADLADDER: mirrors
        # step 3 but only fires when fwd_priority was False this turn -- the
        # committed target is already met, so this raider let peck and salt go
        # first, in their ordinary ranked positions above, and only reaches for
        # a further plant (up to the unchanged LOKI_FWD_GUN_CAP=3) if neither
        # of them found an action.  KLADLADDER2: the reserve floor binds here
        # exactly as it does at step 3 -- the two call sites are now identical
        # in funding and differ only in WHERE in the act ladder they sit.
        if not fwd_priority and self._try_forward_sentinel(ct, E, live=fwd_live):
            return True
        return False
    ...
    def _try_forward_sentinel(self, ct, E, live=_FWD_LIVE_UNSET):
        ...
        LOKI-KLADLADDER: `live` is optional and defaults to _FWD_LIVE_UNSET (a
        private sentinel distinct from a real `None`, which already means
        "blind, fall back to the monotone store" -- see _live_fwd_guns).
        _raid_act computes the live census once per turn for its own
        fwd_priority decision and passes it through here so the two call sites
        (steps 3 and 8) don't each re-scan get_nearby_buildings().

        ⛔ KLADLADDER2 (s48, iteration 2): the `waive_floor` parameter of
        _v473kladladder IS DELETED, not defaulted-off -- there is no dormant
        path to flip back on.  The reserve floor below binds on EVERY call from
        EVERY slot, so a forward plant is funded strictly out of surplus.  See
        the KLADLADDER2 doctrine block for what that costs and why we want to
        measure it anyway.
        """
        if not LOKI_FWD_SENTINEL_ON:
            return False
        if live is _FWD_LIVE_UNSET:
        ...
        # ⛔ WHERE THE WAIVER USED TO BE.  _v473kladladder set `ti_floor = 0`
        # here whenever the plant was the priority spend, on the argument that
        # a reserve held for LESSER spends should not gate the spend that
        # outranks them.  The argument was clean and the SCREEN KILLED IT:
        # 41.86% [40.20,43.52] over n=3,404 vs Sleipnir, with the dose showing
        # the mechanism firing (2.11x forward sentinels) AND builder deaths at
        # 1.91x.  Buying the turret out of the bank the rest of the team is
        # standing in is what the 1.91x is made of.  The floor now binds here
        # unconditionally.
