...
# --- LOKI-SEALSENT: A SENTINEL WHERE A BARRIER WOULD HAVE GONE (s48) --------
# Magnus, s48, verbatim: "Can we test a few builds where we place sentinels on
# some spots instead of barriers around their core?  Be aware that if we allow
# our builders to build unlimited harvestors they will take the titanium from
# our offensive builders trying to set up turrets."
#
# WHICH BARRIER SITE THIS TOUCHES, AND WHICH TWO IT DOES NOT.  raid.py builds
# barriers in exactly three places:
#   (1) SEAT SEAL, `_raid_act` step 2 -- a heal seat of the ENEMY Core;
#   (2) SALT THE CORPSE, `_salt_turn` step 1 -- the tile a cut enemy belt
#       piece stood on;
#   (3) DENY THE REBUILD SEAT, `_salt_turn` step 3 -- an empty tile beside a
#       still-live enemy belt.
# Only (1) is "around their core"; (2) and (3) are belt denial anywhere on the
# enemy half.  **THIS PLANK TOUCHES SITE (1) ONLY.  Sites (2) and (3) are
# byte-identical to _v468kladturbo in every arm of this family**, so a later
# ablation cell can name all three without this plank contaminating two of
# them.
#
# WHY A SENTINEL CAN STAND WHERE A BARRIER STOOD, AT ALL.  A seat is sealed by
# being OCCUPIED: `can_build_barrier` refuses an occupied tile, an enemy
# builder cannot stand on a tile holding a building, `can_spawn` is false
# there, and a conveyor cannot be laid into it.  Every one of those is a
# property of "a building is here", not of "a barrier is here".  A Sentinel is
# a building.  So the seal is retained IN FULL, and 40 HP replaces 30 -- the
# seat costs them 20 builder pecks to reopen instead of 15.
#
# THE EXCHANGE ARITHMETIC, from the entity tables (every price read live
# through the get_*_cost getters at the call site, never hardcoded).
#   * PRICE.  Barrier 3 Ti / +1% scale.  Sentinel 30 Ti / +20% scale.  Ten
#     times the titanium and twenty times the scale contribution.
#   * WHAT THE EXTRA 27 Ti BUYS.  18 damage on a 2-round reload = 9 HP/round,
#     r^2=32, and the line IGNORES OBSTACLES -- so it shoots THROUGH our own
#     collar, which is the whole reason this line uses Sentinels and not
#     Gunners forward.  Against the defender's heal (+4 HP for 1 Ti) a single
#     seat Sentinel is already a net -5 HP/round on a Core being fully
#     repaired, and the seat it occupies is one of the eight that repair had
#     to come from.
#   * AGAINST THE BUILDER THAT COMES TO REMOVE IT.  A builder peck is 2 dmg
#     for 2 Ti, one per turn: 20 turns and 40 Ti to break a 40 HP Sentinel.
#     The Sentinel kills a 40 HP builder in 3 shots -- 6 rounds at reload 2 --
#     for 30 ammunition.  **20 rounds against 6.**  A barrier loses this
#     exchange by construction: it cannot shoot back at all, it only makes the
#     wait expensive.
#   * AMMUNITION IS NOT FREE AND IS NOT PHYSICAL.  10 ammo/shot from the team
#     global pool, and the ONLY source is the Core converting titanium 1:1.
#     Sustained fire is therefore 5 Ti/round for 9 HP/round = 0.56 Ti/HP
#     against the defender's 0.25 Ti/HP heal -- **we lose the pure titanium
#     exchange 2.2:1 and win anyway, because the seal means they cannot spend
#     into it.**  A seat Sentinel is A SEAL THAT SHOOTS, priced as a seal.
#   * SO THE MAGAZINE MUST FOLLOW.  A seat Sentinel increments SLOT_FWD_GUN
#     exactly as `_try_forward_sentinel` does, which is what raises the Core's
#     `ammo_target` to `min(120, 40 + 20*fwd_guns)` in main.py.  Without that
#     write the turret is a 30 Ti barrier with a worse scale bill -- and worse
#     than that, `can_fire` returns TRUE at 0 ammo while the engine RAISES
#     inside `finish_firing_turret`, which destroys our own turret.
#
# THE BIND IS SCALE, NOT TITANIUM, SO THE SUBSTITUTION IS N-CAPPED.
# (research census, s48: we build 9.49 barriers/game on the ladder, v155,
# n=115, 100% enemy-half.)  A FULL swap would be 9.49 x 30 Ti = ~285 Ti AND
# ~+190% on the ONE GLOBAL ADDITIVE cost factor -- roughly tripling every
# subsequent build of every type for the rest of the match.  That is economy
# suicide and it is not what is being tested.  **LOKI_SEALSENT_MAX caps the
# substitution at N seats; every seal past N is the ordinary 3 Ti barrier.**
#
# AND THE SCALE DELTA IS SMALLER THAN N x 20% IN THE COMMON CASE, because a
# seat Sentinel counts against the UNCHANGED LOKI_FWD_GUN_CAP (=3) through the
# same live census `_try_forward_sentinel` already uses.  In a game that would
# have filled the forward cap anyway the plank RELOCATES a Sentinel from the
# approach band to the seat ring and the scale delta is **0%**; in a game that
# would not, it is **+20% x N**.  Both bounds are reported, because which one
# applies is a property of the game and not of the arm.
#
# CONTEXT (research, s48 directive-3 census): WE ARE ALREADY A FORWARD-SENTINEL
# BOT -- 3.72 forward sentinels/game against a field median of 2.61, with
# placement at the field median.  This family is therefore "spend the seal
# budget smarter at the margin", NOT "add forward turrets".  That is exactly
# why the shared cap is left alone.
#
# THE SITING FILTER, AND WHY THE INHERITED ONE WAS NOT ENOUGH.  All three
# barrier sites choose their tile with NO facing term, NO firing-line term and
# NO survivability term -- correct for a 3 Ti wall, wrong for a 30 Ti turret.
# So a seat is only substituted when a build there has a facing whose line
# CONTAINS AN ENEMY CORE TILE (`can_fire_from`, the hypothetical-turret
# predicate, which ignores ammo and cooldown by contract), and among the
# qualifying (tile, facing) pairs the best is chosen by how much of the
# enemy's own ring that same line rakes (`get_attackable_tiles_from` -- the
# predicate raid.py already materialises for ENEMY gunner rays in
# `_raid_station`, used here for a hypothetical turret of OURS).  A Sentinel
# that cannot see their Core is not bought at any price.
#
# THE HOLD, AND WHY IT IS BOUNDED.  A seat that has been barriered can never
# afterwards carry a Sentinel -- `can_build_sentinel` refuses an occupied
# tile.  So when the site qualifies but the bank does not yet cover the
# Sentinel, the arm HOLDS that one seat open (the barrier loop skips exactly
# that tile) rather than spending it for 3 Ti and losing the option forever.
# Holding a seat open is precisely the thing the collar exists to prevent, so
# the hold is capped at LOKI_SEALSENT_HOLD_MAX rounds PER RAIDER, after which
# the ordinary barrier lands and this raider stops asking.  Every OTHER
# adjacent seat is barriered normally throughout, on the unchanged path.
#
# THE FUNDING CONTENTION GUARD -- Magnus's second sentence, and it is the half
# that does NOT cap harvesters.  `_v469kladtkill`/`_v472kladtk2` capped the
# harvester count and the s47 shard read that cap at ~-8.5pp; that road is
# dead and is not re-opened here.  Nothing is capped: a raider that has a
# qualifying seat it cannot yet pay for publishes a WANT BEAT in SLOT_SEALSENT
# (round+1), and while that beat is fresh `_eco_spendable` in eco.py requires
# `ti >= cost + sentinel_cost + LOKI_SEALSENT_FUND_MARGIN` instead of
# `ti >= cost`.  Harvesters and belt links therefore **DEFER ABOVE A RESERVE
# FLOOR** -- they are not forbidden, they yield the top ~30 Ti of the bank to
# the turret and take it back the moment the turret is bought or the beat goes
# stale (LOKI_SEALSENT_FUND_STALE rounds; and the beat is only ever written
# while a raider is actually standing at a qualifying seat, under the cap,
# unable to pay).  Because `_eco_spendable` is the single choke point, the
# launcher build (main.py, which already carries its own LAUNCHER_RESERVE)
# defers with them; that is deliberate -- while the siege has a turret
# pending, the turret is first.  The bootstrap is never touched: the beat
# cannot be written before LOKI_SEALSENT_MIN_HARV harvesters exist.
#
# FLAGS.  Two, split so the mechanism and its funding are separately
# ablatable, and BOTH OFF == _v468kladturbo EXACTLY (the plant becomes a
# constant-false branch, the reserve becomes a constant-false branch, and no
# store slot is written).
# ⭐⭐ THIS TREE IS THE FUNDING ABLATION OF _v474sealsentA, AND IT EXISTS
# BECAUSE THE LOCAL BATTERY SAID THE FUNDING GUARD IS THE EXPENSIVE HALF.
# It differs from _v474sealsentA in ONE CONSTANT -- LOKI_SEALSENT_FUND_ON,
# True -> False -- and in nothing else.  The seat Sentinel is still planted,
# still N=1, still sited by firing line; what is removed is the eco-deferral:
# `_eco_spendable` no longer reserves the top `sentinel_cost + margin` of the
# bank while a raider is waiting, so harvesters, belts and the launcher spend
# exactly as _v468kladturbo spends.  The raider simply buys the turret when the
# bank happens to cover it, and the bounded seat HOLD still gives it up to
# LOKI_SEALSENT_HOLD_MAX rounds to happen.
#
# THE MEASUREMENT, local self-play against the base, 48 matches per arm
# (4 maps x 6 seeds x both orderings, `tools/arena.py`, 0 crashes in all of
# them).  ⚠ LOCAL SELF-PLAY IS NOT THE FIXTURE OF RECORD -- a live-team
# unrated leg is -- so these are a PRIORITY signal, not a verdict:
#
#     arm                          deferral pressure     win rate  95% CI
#     _v481sealsentAnofund (this)  NONE                   54.2%   [40.3, 67.4]
#     _v474sealsentA               beat until 1 planted    37.5%   [25.2, 51.6]
#     _v475sealsentB (N=2)         beat until 2 planted    22.9%   [13.3, 36.5]
#
# The B arm is the one that clears: its interval excludes 50% against us.  And
# the three arms line up MONOTONICALLY on how long the want-beat stays armed --
# B holds it armed longest because its cap of 2 is almost never met, A drops it
# the moment the first Sentinel lands, this tree never raises it at all.
# ⇒ **THE PLANT IS NOT WHAT COSTS; THE DEFERRAL IS.**  Magnus's contention
# worry is real in DIRECTION -- the eco does outbid the turret -- but paying
# for the turret by making the economy stand still costs more locally than the
# contention it prevents.  Read the ordering as a dose-response on the FUNDING
# axis, not as three unrelated arms.
# ⚠ The A-vs-this contrast is 26/48 against 18/48, z ~ 1.64, p ~ 0.10: the
# DIRECTION is consistent across three arms, the individual gap is not
# separately significant, and neither this arm's interval nor A's excludes 50%.
LOKI_SEALSENT_ON = True        # the plant itself
LOKI_SEALSENT_FUND_ON = False  # the eco-deferral contention guard -- OFF here
LOKI_SEALSENT_MODE = "seat"    # "seat" == substitute on the ring;
                               # "enfilade" == add off-ring, barrier untouched
LOKI_SEALSENT_MAX = 1          # N-CAP: seat Sentinels live in the band at once
LOKI_SEALSENT_CENSUS_MIN = 0   # N-cap census band, INCLUSIVE, dsq_core.
LOKI_SEALSENT_CENSUS_MAX = 1   # seat=1, corner=2, one-tile-out=4-5, so
                               # 0..1 is SEATS ONLY and does not count the
                               # base's own corner-sited forward Sentinels.
LOKI_SEALSENT_MIN_HARV = 2     # never before the economy exists
LOKI_SEALSENT_TI_FLOOR = 0     # reserve waived: this IS the priority spend
LOKI_SEALSENT_HOLD_MAX = 12    # rounds one raider may hold a seat unbarriered
LOKI_SEALSENT_FUND_MAX = 60    # rounds one raider may publish the want-beat,
                               # shared by the build leg and the magazine leg
LOKI_SEALSENT_AMMO_ON = True   # the magazine leg of the want-beat
LOKI_SEALSENT_AMMO_FLOOR = 20  # two Sentinel shots.  Below this a planted
                               # seal Sentinel is a 70 Ti barrier: measured
                               # on drumlin s3, planted r24, alive to r303,
                               # fired ZERO times on a 2-14 magazine.
LOKI_SEALSENT_FUND_STALE = 3   # rounds a want-beat stays credible
LOKI_SEALSENT_FUND_MARGIN = 6  # kept beyond the Sentinel cost, so the eco
                               # deferral cannot round down into "never"
LOKI_SEALSENT_ENF_MIN_SEATS = 2   # "enfilade" only: seats the line must rake
LOKI_SEALSENT_LOG = False      # local demo tag "SEALSENT r=.. t=x,y f=.."
SLOT_SEALSENT = 13             # was SLOT_DEFEND_BEAT, dead in this tree
                               # (0 references outside doctrine.py, verified)
...
# ============================================================================
# LOKI-SEALPIERCE (s48) -- THE SEAT SENTINEL SHOOTS PAST THE CORE IT SITS ON
# ============================================================================
# Magnus, s48, verbatim: "what if the sentinels placed around their core also
# shoot whatever is on the other side of the core?  If it's a conveyor it can
# break the enemy eco by just keeping one killed and then keep hammering the
# core.  Also if it can shoot a builder that is healing the core, would it be
# beneficial to kill it?"
#
# ⭐ STEP 0 IS MEASURED, NOT ASSUMED, AND IT IS THE WHOLE PLANK'S LICENCE.
# `bots/_probe_pierce` on `maps/invented/pierce16.map26` (both cores on row
# y=7; our Sentinel seated at (9,7) facing EAST, two ENEMY CORE tiles at
# (10,7),(11,7) on its ray, an enemy core-mouth CONVEYOR at (12,7) behind
# them).  One local game, --tle 0, seed 1, 100+ real shots:
#
#   ATTACKABLE(seat, EAST) = [(10,7),(11,7),(12,7),(13,7),(14,7)]
#       -> the raw pattern DOES contain the far-side tiles; the ray is 5 tiles
#          long from the seat (d^2 <= 32) so TWO core tiles cost two of them
#          and THREE far-side tiles remain.
#   can_fire(belt behind the core) = True   (with can_fire(far)=True too)
#   LEDGER, every shot:  belt hp 20 -> 2  |  enemy core hp 500 -> 500
#
# ⇒ (a) the engine ACCEPTS a target behind an enemy CORE, (b) the damage lands
# ON THE TARGET TILE ONLY -- the intervening enemy core took 0 across every
# shot in the probe, so this is a pierce, not a beam, and (c) the far-side
# tiles are in `get_attackable_tiles()`.  18 dmg vs a 20 HP conveyor is TWO
# shots per kill (never one), at 10 ammo a shot.
#
# THE PRICE ARGUMENT, because it is the reason the default ladder is wrong
# here.  The base ladder (`TURRET_PRIO` in main.py) ranks an enemy CORE first,
# so a seat Sentinel with the core on its ray hammers 500 HP at 18 a shot --
# 28 shots, 280 ammo, and only the last one scores anything.  A core-mouth
# belt is 20 HP: 2 shots, 20 ammo, and it severs the delivery route that
# `titanium_collected` is defined on.  The rebuild is the FARM: they re-lay
# the same tile, we kill it again, and the ladder falls back to the core
# whenever the tile is empty.  Nothing is given up -- the core is still the
# default rung, it is just no longer the ONLY rung.
#
# THE LADDER, exactly as implemented in `_turret` (seat Sentinels only):
#   0  enemy CONVEYOR/SPLITTER on the ray, orthogonally adjacent to their core
#      (a delivery mouth -- the tile `titanium_collected` is credited through)
#   1  any other enemy CONVEYOR/SPLITTER on the ray within
#      LOKI_SEALPIERCE_BELT_DSQ of their core (their local trunk)
#   2  an enemy BUILDER_BOT on the ray, adjacent to their core, that HAS NOT
#      MOVED for LOKI_SEALPIERCE_HEAL_DWELL rounds (the healer rung).  ⛔ The
#      dwell WATCH runs before the `can_fire` filter, because `can_fire` is
#      False on a reload round: a watch behind it observes only every other
#      round and a consecutive-rounds test can never pass.  Measured: 807
#      opportunity rounds produced 2 healer shots before that was fixed.
#   3  their CORE (the base default, unchanged)
#   4+ everything else, in the base's own order, strictly below the core
# Flag off, `_pierce_prio` is never called and the ladder is `TURRET_PRIO`
# byte-for-byte.
#
# ⛔ THE DWELL GUARD ON THE HEALER RUNG EXISTS BECAUSE 3 SHOTS IS 6 ROUNDS.
# A builder is 40 HP and a Sentinel does 18 on a 2-round reload, so a kill is
# THREE shots across six rounds -- longer than most raiders stand still.
# Chasing a mover spends 30 ammo for nothing AND drops the belt/core rungs
# while it does so.  The rung therefore requires the same builder id to have
# been seen on the same tile for LOKI_SEALPIERCE_HEAL_DWELL consecutive
# rounds before it outranks the core.  This is a cheap per-unit memory, not a
# store slot: the turret is the only reader.
#
# THE SITING TERM.  `_sealsent_site` already scores a candidate (tile, facing)
# by what its ray covers -- core tiles 3, ring seats 1.  Under this plank the
# score gains what the pierce made worth having: LOKI_SEALPIERCE_W_BELT per
# enemy belt ALREADY STANDING on a far-side tile, and LOKI_SEALPIERCE_W_FAR
# per far-side tile at all (the pierce DEPTH -- a facing that leaves three
# tiles of their delivery apron under the gun is worth more than one that
# clips the core corner and stops).  "Far side" is defined off the ray itself:
# the tiles whose d^2 from the seat EXCEEDS that of the farthest enemy core
# tile on the same ray.  With the flag off the key is the parent's exactly.
#
# AMMO IS DELIBERATELY UNTOUCHED.  A seat Sentinel already increments
# SLOT_FWD_GUN (raid.py), which raises the Core's magazine target, and the T4
# BURN CAP then holds it at T4_BURN_RNDS * T4_AMMO_PER_RND per weapon = 40,
# i.e. four Sentinel shots in hand.  Firing MORE (which this plank does)
# makes the magazine FALL, which is exactly what clears the T4 idle brake.
# Any magazine change belongs in its own leg -- bundled here it would confound
# the ladder with the funding.
# ----------------------------------------------------------------------------

# ============================================================================
# WHAT THE DEMOS MEASURED (s48, 72 local games, three opponents, 14-map pool)
# ============================================================================
# Fixtures: `_tmp_pierceon` vs kladde_probe (32), opp_v78 (24) and the parent
# (16), rotating the whole new-maps pool over both seats.  Tape:
# `scratchpad/sweep_kladde3.txt`, `sweep_v78c.txt`, `sweep_base2.txt`;
# instrument `scratchpad/sp_shots.py` (shot-level, with a jitter control).
#
#   3,652 live seat-Sentinel rounds
#     447 (12.2%) had an enemy BELT behind their Core ON OUR FACING
#   1,550 (42.4%) had one on SOME legal facing  <- the facing term captures
#                                                  about 29% of the exposure,
#                                                  and cannot do better while
#                                                  it reads the board only at
#                                                  BUILD time (no rotate())
#     258 ( 7.1%) had an enemy BUILDER on a far heal seat
#
#   shots: MOUTH 33 · APRON 24 · HEALER 19 · CORE 1,333
#   kills: 24 belts · 7 builders (removal within one round of a seat shot,
#          assists included -- our raiders peck the same targets)
#   hygiene: 0 tracebacks, 0 TLE bot-turns in 72 games
#
# ⇒ THE LADDER'S TOP RUNG IS REAL BUT NARROW (57 of 1,409 seat shots, 4.0%).
# The Core rung remains what a seat Sentinel does almost all of the time, and
# that is by design: the plank adds a rung, it does not replace the hammer.
# ⚠ THE RE-KILL FARM WAS NOT OBSERVED IN THE FIELD.  Zero tiles were killed
# TWICE in 72 games -- the farm is demonstrated only on `bots/_probe_pierce`,
# where a scripted victim relaid the same mouth tile ten times and the seat
# Sentinel killed it ten times (two shots each, 18 dmg vs 20 HP).  Report it as
# an engine capability, not as a field behaviour.
# ⚠ THE JITTER CONTROL IS DECISIVE ONLY OFF A DENSE LATTICE.  Displacing every
# target by (+1,0) takes the probe's MOUTH count 20 -> 0 and the kladde/parent
# fixtures' 22 -> 5, but on opp_v78 it goes UP (11 -> 28): their belts are dense
# enough near the Core that a displaced ray lands on another one.  Off a dense
# lattice the through-core geometry check is the stronger control.
#
# ⚠ ONE COST THE HEALER RUNG PAYS AND THE BELT RUNG DOES NOT: destroying an
# enemy UNIT removes its contribution to THEIR cost scale (+20% for a builder
# bot), so a killed builder makes their next builds cheaper.  The rung is still
# worth it while the builder is HEALING -- 4 HP/round at 1 Ti against our 18
# per two rounds -- but it is a real term, and it is why the rung is armed by
# DWELL rather than by mere presence.
# ============================================================================

LOKI_SEALPIERCE_ON = True       # the plank; False == _v481sealsentAnofund exactly
LOKI_SEALPIERCE_BAND_MAX = 2    # a "seat" Sentinel: dsq_core <= this (seat 1,
                                # corner 2).  Beyond that the turret is not
                                # sitting on their ring and the base ladder --
                                # which is tuned for the open field -- stands.
LOKI_SEALPIERCE_BELT_DSQ = 8    # a belt further than this from their core is
                                # not their delivery apron; it does not outrank
                                # 18 damage on the core.
LOKI_SEALPIERCE_HEALER_ON = True
LOKI_SEALPIERCE_HEAL_DWELL = 1  # consecutive rounds the same builder id must
                                # hold the same tile before the healer rung
                                # outranks the core.  1 == "seen twice".
LOKI_SEALPIERCE_SITE_ON = True  # the build-time facing term
LOKI_SEALPIERCE_W_BELT = 4      # per live enemy belt already on a far-side tile
                                # (ADDITIVE -- evidence).  Far-side DEPTH is a
                                # TIE-BREAK slot, not a weight: see the key in
                                # `_sealsent_site`.
LOKI_SEALPIERCE_LOG = False     # local demo instrument only; platform replays
                                # strip stdout (see the Controller note above)
