...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-SEALSENT (s48) raid state, per unit.  Bounded by
        # LOKI_SEALSENT_HOLD_MAX, so an unaffordable Sentinel cannot leave an
        # enemy heal seat open for the rest of the match. ---
        self.sealsent_hold_n = 0
        self.sealsent_want_n = 0

        # --- LOKI-SEALPIERCE (s48) turret state, per TURRET.  `pierce_seat`
        # is tri-state on purpose: 0 means "not resolved yet", and the anchor
        # read can legitimately fail on the turret's first turn (the store
        # slot is written by the Core, one round buffered), so an unresolved
        # read must RETRY rather than latch "not a seat" forever. ---
        self.pierce_seat = 0            # 0 unresolved, 1 seat Sentinel, -1 no
        self.pierce_anchor = None
        self.pierce_dwell_id = None     # healer rung: id last seen dwelling
        self.pierce_dwell_key = None    # ...and the tile it was seen on
        self.pierce_dwell_n = 0         # consecutive rounds on that tile
        self.pierce_dwell_rnd = -10 ** 9
    ...
    def _turret(self, ct):
        ...
        # LOKI-SEALPIERCE: a Sentinel SEATED on their ring reads a different
        # ladder, because its ray runs THROUGH their Core (measured: the shot
        # lands on the target tile only, the Core on the ray takes 0) and the
        # far side of that ray is their delivery apron.  Resolved once per
        # turret and then cached; None everywhere else, including with the
        # flag off, where `_pierce_prio` is never reached at all.
        pierce = None
        if LOKI_SEALPIERCE_ON and turret_type == EntityType.SENTINEL:
            pierce = self._pierce_anchor(ct, p)
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                if et is None:
                    continue
                # ⛔ THE DWELL WATCH RUNS BEFORE THE can_fire FILTER, AND THAT
                # ORDER IS A DEMO FINDING.  `can_fire` is False on a reload
                # round, so a watch placed after it only ever sees the turret's
                # OWN firing cadence -- one observation every 2 rounds -- and a
                # "seen on consecutive rounds" test can then never pass.
                # Measured on the kladde demo before the fix: 807 rounds with an
                # enemy builder on a far heal seat produced 2 HEALER shots.
                if pierce is not None and et == EntityType.BUILDER_BOT:
                    self._pierce_watch(ct, pierce, t)
                if not ct.can_fire(t):
                    continue
                prio = TURRET_PRIO.get(et, 8)
                if pierce is not None:
                    prio = self._pierce_prio(pierce, t, et, prio)
                if prio < best_prio:
                    best_prio, best = prio, t
            if best is not None:
                if pierce is not None and LOKI_SEALPIERCE_LOG and best_prio < 3:
                    print("SEALPIERCE r=%d seat=(%d,%d) rung=%d t=(%d,%d)"
                          % (ct.get_current_round(), p.x, p.y, best_prio,
                             best.x, best.y))
                ct.fire(best)
                return
        except Exception:
    ...
    def _pierce_anchor(self, ct, p):
        """Their Core anchor iff this Sentinel is seated on their ring, else None.

        Read once and cached: a turret cannot move and their Core cannot move,
        so the verdict is a property of the pair.  An unresolved read (slot 0
        still empty on the turret's first turn) leaves the tri-state at 0 so
        the next round tries again -- latching -1 there would silently disarm
        the plank on exactly the turrets planted earliest.
        """
        if self.pierce_seat == 0:
            try:
                E = unpack_pos(ct.read_store(SLOT_ENEMY_CORE))
                if E is None:
                    E = self.enemy
                if E is not None:
                    if dsq_core(p, E) <= LOKI_SEALPIERCE_BAND_MAX:
                        self.pierce_anchor, self.pierce_seat = E, 1
                    else:
                        self.pierce_seat = -1
            except Exception:
                return None
        return self.pierce_anchor if self.pierce_seat == 1 else None

    def _pierce_prio(self, E, t, et, base):
        """The seat-Sentinel firing ladder.  Lower is fired first.

        0 belt on a delivery mouth · 1 belt in their apron · 2 a DWELLING
        builder on a heal seat · 3 their Core (the base default) · base+4
        everything else, which keeps the parent's relative order intact below
        the Core instead of inventing a new one.
        """
        if et == EntityType.CONVEYOR or et == EntityType.SPLITTER:
            d = dsq_core(t, E)
            if d <= 1:              # orthogonal to the footprint == a mouth
                return 0
            if d <= LOKI_SEALPIERCE_BELT_DSQ:
                return 1
            return base + 4
        if et == EntityType.BUILDER_BOT:
            if LOKI_SEALPIERCE_HEALER_ON and dsq_core(t, E) <= 1 \
                    and self._pierce_dwelt(t):
                return 2
            return base + 4
        if et == EntityType.CORE:
            return 3
    ...
    def _pierce_watch(self, ct, E, t):
        """Book-keeping for the healer rung.  Runs every round, fires nothing.

        A builder is 40 HP; a Sentinel lands 18 on a 2-round reload, so a kill
        is three shots across six rounds.  Chasing a bot that steps away after
        the first one spends 30 ammo for nothing AND suppresses the belt and
        Core rungs while it does.  So the rung is armed only by PERSISTENCE:
        the same entity id on the same tile for LOKI_SEALPIERCE_HEAL_DWELL
        consecutive rounds.

        Exactly one tile is tracked per round (the first in
        `get_attackable_tiles()` order, which is fixed) -- otherwise two
        adjacent builders would reset each other's counter every round and the
        rung could never arm.  Only heal seats are watched: a builder passing
        through the apron is not the target this rung exists for.
        """
        rnd = ct.get_current_round()
        if self.pierce_dwell_rnd == rnd:
            return                       # already tracked a tile this round
        if dsq_core(t, E) > 1:
            return
        try:
            bot = ct.get_tile_builder_bot_id(t)
        ...
        if bot is None:
            return
        key = (t.x, t.y)
        if bot == self.pierce_dwell_id and key == self.pierce_dwell_key \
                and rnd - self.pierce_dwell_rnd == 1:
            self.pierce_dwell_n += 1
        else:
            self.pierce_dwell_id = bot
            self.pierce_dwell_key = key
            self.pierce_dwell_n = 0
        self.pierce_dwell_rnd = rnd

    def _pierce_dwelt(self, t):
        """Pure read of the watch above -- no state, so the ladder can ask it
        for several tiles in one round without disturbing the count."""
        return (self.pierce_dwell_key == (t.x, t.y)
                and self.pierce_dwell_n >= LOKI_SEALPIERCE_HEAL_DWELL)
