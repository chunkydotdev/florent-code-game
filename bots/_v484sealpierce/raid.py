...
class RaidMixin:
        ...
        for t in cands:
            ...
            for f in Direction:
                ...
                if enf:
                    ...
                    if not ok:
                        continue
                # LOKI-SEALPIERCE: what the ray does BEHIND their Core is now
                # worth something, so the facing choice has to price it.  Only
                # in "seat" mode, and only after the facing has already
                # qualified on the parent's own terms -- this term can reorder
                # the qualifying facings, never admit a new one.
                pbelt = pfar = 0
                if LOKI_SEALPIERCE_ON and LOKI_SEALPIERCE_SITE_ON and not enf:
                    pbelt, pfar = self._pierce_site_term(ct, t, ray, corekeys)
                # Deterministic: score first, then a fixed tile/facing order, so
                # two raiders that see the same board choose the same site.
                # ⛔ THE TWO PIERCE TERMS SIT IN DIFFERENT SLOTS ON PURPOSE.  A
                # live belt behind their Core is EVIDENCE and is additive; raw
                # far-side DEPTH is only option value and is a TIE-BREAK, so it
                # can never outbid a Core tile.  (Additive depth was the first
                # cut: at W_FAR=1 and three far tiles it exactly equals one Core
                # tile at weight 3, i.e. it could have swapped a two-Core-tile
                # facing for a one-Core-tile facing on a coin-flip tie.)
                key = (-(3 * ncore + nseat + LOKI_SEALPIERCE_W_BELT * pbelt),
                       -pfar, t.y, t.x, fi)
    ...
    def _pierce_site_term(self, ct, t, ray, corekeys):
        """Score what a candidate (tile, facing) puts BEHIND their Core.

        "Behind" is defined off the ray itself and needs no geometry: the
        far-side tiles are the ones whose d^2 from the seat exceeds that of
        the FARTHEST enemy Core tile on the same ray.  (Measured in
        `bots/_probe_pierce`: a seat ray is five tiles at r^2=32, two of them
        eaten by the 2x2 footprint, so three remain -- the depth is small
        enough that a facing which wastes it is a real loss.)

        Returns (n_belt, n_far):
          * n_belt -- enemy CONVEYOR/SPLITTERs ALREADY STANDING out there.  The
            thing the pierce is for, and the only part of this that is evidence
            rather than option value; the caller weights it and ADDS it.
          * n_far  -- far-side tiles at all, i.e. the pierce DEPTH.  Option
            value only, so the caller uses it as a TIE-BREAK.
        Returns (0, 0) on any read failure or when no Core tile is on the ray,
        i.e. it can only ever reorder facings the parent already accepted.

        ⚠ MEASURED LIMIT, and it is the honest bound on this whole term: it can
        only see belts that EXIST AT BUILD TIME.  A Sentinel cannot rotate
        (rotate() is Gunner-only), so the facing is permanent, while their
        delivery apron is laid, cut and re-laid all game.  Demo, 32 games vs the
        parent: 0 of 1,190 live seat-Sentinel rounds had a belt behind the Core
        on the CHOSEN facing, against 218 rounds where SOME legal facing would
        have -- and almost none of those belts existed when the seat was bought.
        """
        if not ray:
            return (0, 0)
        try:
