...
LOKI_FWD_TI_FLOOR = 16      # bank left after paying for one
LOKI_FWD_MIN_HARV = 2       # do not open the siege before the economy exists

# --- LOKI-KLADLADDER: ECO-GATED FORWARD-SENTINEL COMMITMENT (s48) -----------
# CHERRY-PICK, NOT A REBUILD.  This is the SENTINEL LADDER half of KLADTK2
# (_v472kladtk2), lifted onto the untouched _v468kladturbo "Sleipnir v1" base.
# The other half of KLADTK2 -- LOKI_HARV_CAP, the 3-harvester budget in
# eco.py -- is DELIBERATELY NOT PORTED: the s47 shard read the four-plank at
# 51.8-53.1 against the base's 61.57 vs v140, i.e. the CAP subtracts ~8.5pp.
# The hypothesis this tree exists to test is that the LADDER alone adds.
# eco.py here is byte-identical to the base; there is no harvester budget.
#
# Magnus's original wording (s47), which the ladder still implements:
#   "If we have <enough> harvesters it should build 2 sentinels and peck and
#    salt, otherwise it builds 1 sentinel and pecks and salts until we have
#    <enough> harvesters, then we build the second sentinel and third."
#
# WHAT THE LADDER IS: a COMMITTED live-sentinel target that moves with the eco
# phase.  Below target, planting is a PRIORITY -- it takes the step-3 slot in
# raid.py `_raid_act`, ahead of peck and salt, and the bank reserve floor is
# waived (see LOKI_FWD_TI_FLOOR above and the waiver in
# `_try_forward_sentinel`).  At or above target, planting DEMOTES to an
# opportunistic slot after peck and salt, so a third sentinel can still arrive
# under the unchanged LOKI_FWD_GUN_CAP (=3) without preempting anything.
# Because the target is compared against the LIVE census (`_live_fwd_guns`),
# the ladder is SELF-HEALING: a sentinel dying drops live below target, which
# re-arms priority + the waiver and replants it.
#
# THE ARMING TRIGGER -- the one design decision this tree owns, because
# KLADTK2 armed off `LOKI_HARV_CAP - 1` and there is no cap here.
#   * PRE (target = LOKI_FWD_SENT_MIN_PRE = 1) arms at the harvester count
#     where a forward plant becomes LEGAL AT ALL -- i.e. the same `min_harv`
#     `_try_forward_sentinel` already enforces (LOKI_FWD_MIN_HARV = 2
#     normally, LOKI2_RUSH_MIN_HARV inside the LOKI-2 rush window).  It is
#     read from that same source in raid.py rather than duplicated as a
#     number, so the two can never drift apart.  WHY THIS SEAM: below it the
#     plant attempt returns False regardless of which slot it sits in, so
#     arming any earlier would be a distinction without a behaviour, and
#     arming any LATER would demote the FIRST sentinel below peck/salt --
#     which the base never did and which is not what "commit to sentinels"
#     means.  Consequence, and it is the flag-off equivalence this tree
#     claims: in the PRE-TRIGGER phase the build sequence is IDENTICAL to
#     _v468kladturbo.
#   * POST (target = LOKI_FWD_SENT_MIN_POST = 2) arms at
#     LOKI_LADDER_POST_HARV = 3 harvesters BUILT.  Three because that is
#     ECO_NEED, the count this codebase already calls "the economy exists"
#     (eco.py writes SLOT_ECO_READY at it), and because it is one above the
#     PRE arm so the ladder has a rung.  It is a BUILD counter, not a live
#     count, and it is UNBOUNDED here -- with no cap the eco keeps building
#     past 3, so once armed the ladder stays armed.
#   * ROBUSTNESS TO THE KNOWN UNDERCOUNT, carried from KLADTK2 v3: two
#     builders completing a harvester in the SAME round both write `read + 1`
#     off the same buffered snapshot, so SLOT_HARVESTERS can record one build
#     for two.  KLADTK2 v3 had to arm at CAP-1 to survive this, because with a
#     hard cap of 3 an undercounting store could sit at 2 FOREVER and never
#     arm (the yulerune 31.9% collapse).  THAT FAILURE MODE CANNOT OCCUR HERE:
#     the base's eco is uncapped and keeps building, so the counter is
#     monotonically rising past any fixed threshold -- an undercount delays
#     the POST rung by at most the time to build one more harvester.  No
#     CAP-1 fudge is needed and none is used; a plain `>= 3` is correct.
LOKI_LADDER_ON = True       # master switch for the whole plank (off == base)
LOKI_LADDER_POST_HARV = 3   # harvesters BUILT at which the target rises to POST
LOKI_FWD_SENT_MIN_PRE = 1   # committed live-sentinel target before that
LOKI_FWD_SENT_MIN_POST = 2  # committed live-sentinel target after it

# --- KLADLADDER2 (s48, ITERATION 2): THE LADDER WITHOUT THE COMMITMENT TAX ---
# Magnus: "iterate on kladladder until it works."  Iteration 1
# (_v473kladladder) shipped the ladder ABOVE together with a COMMITMENT TAX --
# the priority plant waived the bank reserve floor (`fwd_waive` ->
# `ti_floor = 0` in raid.py `_try_forward_sentinel`).  It died on the screen:
#
#     _v473kladladder vs Sleipnir: 41.86% [40.20, 43.52], n = 3,404  (Band-4)
#     dose: forward sentinels 1.58/game vs ctrl 0.75  (2.11x -- MECHANISM FIRED)
#           builder deaths     1.83/game vs ctrl 0.96  (1.91x)
#           forward deaths                             (2.25x)
#
# So the mechanism was NOT the problem -- the mechanism worked.  The suspicion
# this tree exists to test is that the FUNDING was: convergent unregistered
# demo evidence from the sealsent variants (_v474-_v476, _v481) reads the same
# plant at ~parity WITHOUT eco-deferral and craters WITH it.  (Hypothesis-grade.
# This tree is the registered test, not the confirmation.)
#
# WHAT IS SEVERED, exhaustively -- and it is one thing, because one thing is
# all _v473 ever added beyond the ordering:
#   * `fwd_waive` in `_raid_act`: GONE (the variable does not exist).
#   * `waive_floor=` parameter of `_try_forward_sentinel`: DELETED, not
#     defaulted-off.  There is no dormant path and nothing to flip.
#   * `if waive_floor: ti_floor = 0`: GONE.  Every plant, from EITHER slot,
#     now requires `bank >= cost + LOKI_FWD_TI_FLOOR` (40, or
#     LOKI2_RUSH_TI_FLOOR inside the rush window).  SURPLUS-ONLY FUNDING.
# WHAT IS KEPT, byte-for-byte: the targets (PRE=1 / POST=2), the arming
# triggers (`min_harv` mirror / LOKI_LADDER_POST_HARV=3), the live-census
# self-healing replant, and the step-8 opportunistic slot.
#
# NOTHING ELSE EVER CONSULTED THE PRIORITY FLAG.  Verified by grep on the
# _v473 tree: `fwd_priority` appears only in `_raid_act` and only as the
# selector between the step-3 and step-8 call sites; `eco.py` and `main.py`
# are byte-identical to _v468kladturbo in BOTH trees, so no other unit's
# spending was ever gated on it.  The waiver WAS the whole eco-deferral: it
# let one raider draw the global bank below the reserve every other consumer
# (belt planner, Core ammo conversion at its own ti_floor of 52/12) budgets
# against.  Severing it severs the deferral in full.
#
# ⛔⛔ THE SEAM THIS CREATES, STATED BEFORE THE DOSE RUNS, BECAUSE IT IS AN
# ARITHMETIC PROPERTY OF THE TREE AND NOT A RESULT.  With the waiver gone the
# ladder has NO funding path the base lacks, and its two remaining deviations
# from _v468kladturbo point in OPPOSITE directions:
#   * below target -> step 3 fires, with the floor.  The base's step 3 is
#     UNCONDITIONAL and carries the same floor.  ⇒ IDENTICAL.
#   * at/above target -> step 3 is SKIPPED and the attempt moves to step 8,
#     which only runs if peck and salt both declined.  ⇒ STRICTLY FEWER
#     opportunities than the base for sentinels #2 and #3.
# ⇒ PER TURN, GIVEN IDENTICAL STATE, this tree's plant-attempt set is a SUBSET
#   of the base's.  ⚠ AND THAT DOES **NOT** MAKE THE GAME-LEVEL COUNT A SUBSET,
#   which is the mistake to avoid: the two bots diverge on round 0 (our raiders
#   gate on `get_cpu_time_elapsed()`, so the fixture is not even deterministic
#   against ITSELF), and once the states differ a longer-surviving or
#   richer-banked arm plants more.  The s48 demo shows both directions inside
#   eight cells.  So the honest prior is delivery <= control IN EXPECTATION,
#   the addendum's criterion (1) (fwd-sentinel diff CI-LOWER > 0) is expected
#   to FAIL, and the per-game spread is wide.  Measured, not assumed.
# ⇒ AND THE ROAD THIS DOES CLOSE IS WORTH CLOSING: if delivery reads at
#   control while builder deaths read at control, then _v473's 2.11x AND its
#   1.91x were BOTH bought by the waiver alone, and "the ladder" as an
#   ordering plank is inert -- which retires the ordering and leaves the
#   funding rule as the only live design question.  Do not re-add a waiver to
#   rescue the number; a smaller floor is a DIFFERENT plank and needs its own
#   registration.
...
# ---- cherry-picked by builder s48 from: kladtk2 (_v472kladtk2) — the SENTINEL
#      LADDER only. NOT tools/stack.py: this is a partial lift of one of that
#      tree's two planks, which a whole-statement AST merge cannot express.
#      The harvester CAP (LOKI_HARV_CAP, eco.py) is deliberately left behind;
#      eco.py and main.py here are byte-identical to _v468kladturbo.
...
# ---- builder s48, ITERATION 2: derived from _v473kladladder by DELETING the
#      commitment tax (the fwd_waive / waive_floor reserve-floor waiver). Same
#      lineage marker as above -- provenance is still the KLADTK2 sentinel
#      ladder on the _v468kladturbo base, minus that waiver. eco.py and main.py
#      remain byte-identical to _v468kladturbo.
