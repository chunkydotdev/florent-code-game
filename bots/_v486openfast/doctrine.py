...
# ============================================================================
# LOKI-OPENFAST (builder s48, 2026-08-17) -- SPAWN THE ECONOMY ON THE ORE SIDE.
#
# TARGET METRIC (re-pointed mid-build by the coordinator, on a research
# within-game control the first version of this block did not have): against
# the opponents we ACTUALLY face, our first HARVESTER is at parity (+0.30
# rounds, we are earlier in 48.3% of games) -- the 4-round harvester gap was
# against ph / Leviathan / farming_200s, whom we rarely play. WHAT SURVIVES IS
# THE BELT: our first CONVEYOR is 4.42 rounds LATER (earlier in only 17.4% of
# games) and we hold 3.57 FEWER conveyors by r8 (fewer in 88% of games) while
# fielding +1.50 MORE builders (97.4%). ⇒ the plank is scored on FIRST-CONVEYOR
# ROUND and CONVEYORS BY r8, with harvester timing held at parity-or-better.
#
# WHY A SPAWN-PLACEMENT PLANK IS A BELT PLANK -- the structural fact, read off
# this tree and then measured:
#
#   `_wire_on_build` (eco.py:800) is the ONLY producer of `link_queue`, and it
#   is called from exactly one place: immediately after a harvester is built
#   (eco.py `_expand`). `_samestop_fire` and `_build_next_link` only CONSUME
#   that queue. ⇒ **the first conveyor cannot precede the first harvester by
#   construction.** MEASURED on 97 base cells of the demo below:
#   c1 - h1 == 1 in 90, == 2 in 5, == 3 in 2, and < 1 in ZERO.
#   ⇒ under the constraint "do not break harvester parity", the only admissible
#   way to move the belt earlier is to move the whole opening earlier. Pre-
#   laying belt on the walk in would move c1 at the COST of h1 (acting and
#   moving are mutually exclusive for a builder), which is exactly the trade
#   the parity constraint forbids.
#
# THE CONSTRAINT THAT MOVES THE WHOLE OPENING (scratchpad/s48_lb.py +
# s48_spawnring.py, 32 local cells = 8 maps x 2 seeds x both seats vs opp_v63,
# rounds 0-30):
#
#   * Our eco builders ALREADY walk optimally once they exist. Per cell,
#     `h1 - OPT_noS0` -- the actual first-harvester round minus the earliest
#     round any NON-seat-0 builder could have reached an ore-adjacent tile
#     from the tile it was actually spawned on -- is **0 in 18 of 32 cells,
#     median 0, mean 1.06**. So ore-choice, tasking order, bank and route
#     preconditions are NOT what costs the rounds: they are already tight.
#
#   * The SPAWN TILE is. `_core` sorts the ~21 legal ring(core, 2) candidates
#     by `(hash(x, y, n, salt) % 97, y, x)` -- **a pure dispersion key with no
#     geometry term at all**. Against the best legal ring tiles on the same
#     maps the opening five spawn a **median +10.5 (mean +10.6) tiles of extra
#     walking** from ore, and the worst cells are brutal: saga/s11/A best ring
#     tile is 1 step from ore-adjacent and we spawned the five at 9/8/4/8/7;
#     hive/s11/A has a ring tile ALREADY ore-adjacent (d=0) and we spawned at
#     5/4/3/7/6 and got our first harvester at r8.
#
# ⇒ ONE MECHANISM: for the ECO spawn ordinals only, sort the spawn candidates
#   by BFS distance to the nearest ore-adjacent tile FIRST, and keep the
#   existing hash key as the tie-break so dispersion among equal-distance
#   tiles is unchanged.
#
# WHAT IT DELIBERATELY DOES NOT TOUCH:
#   * LOKI_BASE_BUILDERS -- the opening count is QUEUE #62, a separate plank
#     with its own conditions. This arm spawns exactly as many bodies as the
#     base, at exactly the same rounds (measured: builders by r8 5.38 base vs
#     5.46 arm, and the difference is only the SIXTH body arriving earlier
#     because the eco gate `harv >= 1 and ti >= LOKI_SURPLUS_TI` opens sooner).
#   * Spawn ordinal 0 (the raider) and 4 (the home defender). Ordinal 0 sets
#     where the raid starts from and ordinal 4 is the core's own body; moving
#     either is a different plank. ⇒ **first-turret timing is unchanged**: no
#     turret build site, cost, ordering or gate is touched, so the TW
#     side-effect registration (delaying early visible turrets re-enables
#     Odin's TW against us) does not bind on this arm.
#   * The SAMESTOP machinery. This changes WHERE a builder starts walking,
#     never the harvester+first-link pattern it executes when it arrives --
#     `_link_path`, `_samestop_plan/_arm/_fire` and `_build_next_link` are
#     byte-identical to the base.
#   * `_pick` and its stripe. The partition is untouched; the eco seats still
#     target the same ores in the same order.
#
# COST: one multi-source BFS at the core, computed at most once per match,
# node-budgeted, wrapped in try/except, and only reached on the first eco
# spawn. Falls back to the base ordering whenever the ore set is unknown.
# ============================================================================

LOKI_OPENFAST_ON = True
LOKI_OPENFAST_SEATS = (1, 2, 3)   # spawn ordinals biased toward ore (== LOKI_ECO_SEATS)
LOKI_OPENFAST_NODES = 1200        # BFS node budget; a 30x30 map is 900 tiles
LOKI_OPENFAST_FAR = 99            # distance for a ring tile the flood never reached
