...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        self.map_ores = []
        self.ore_cursor = 0
        # LOKI-OPENFAST: ring-tile -> BFS steps to the nearest ore-adjacent
        # tile, computed at most once per match on the Core.  None = not yet
        # computed; {} = computed and unusable (no ore known), which is a
        # LATCH so the flood is never retried.
        self._of_dist = None
    ...
    # ------------------------------------------------------------------
    # LOKI-OPENFAST (s48): where the economy is born
    # ------------------------------------------------------------------

    def _openfast_dist(self, ct):
        """Tile -> steps to the nearest tile a harvester could be BUILT from.

        Multi-source BFS *outward* from the ore-adjacent tiles, so one flood
        answers the question for every spawn candidate at once.  Terrain comes
        from the known-map grid when we have one and from the Core's own
        vision (r^2 = 36) when we do not -- the Core sits still all game, so
        that vision never grows and there is nothing to re-sense later.

        Returns a dict.  An EMPTY dict is a LATCH, not a failure to be retried:
        if no ore is known on the first eco spawn, no later round will know
        more, and re-running a whole-map flood every spawn is exactly the kind
        of hot-path cost this tree removed elsewhere.  Callers treat {} as
        "keep the base ordering".
        """
        if self._of_dist is not None:
            return self._of_dist
        self._of_dist = {}
        try:
            ...
            if grid is not None:
                ...
                for tile in ct.get_nearby_tiles():
                    ...
                    if env == Environment.WALL:
                        ...
                        ores.append((tile.x, tile.y))
            if not ores:
                return self._of_dist

            # GOALS: a builder builds on an ORTHOGONALLY adjacent tile and
            # never its own, so the tile it must REACH is a non-wall neighbour
            # of an ore tile.  Ore itself is passable and is a legal stand
            # tile for mining the ore next to it, so it is not excluded.
            dist = {}
            frontier = []
            for ox, oy in ores:
                for dx, dy in ((0, -1), (1, 0), (0, 1), (-1, 0)):
                    g = (ox + dx, oy + dy)
                    if not (0 <= g[0] < mw and 0 <= g[1] < mh) or g in walls:
                        continue
                    if g not in dist:
                        dist[g] = 0
                        frontier.append(g)
            nodes = len(frontier)
            d = 0
            while frontier and nodes < LOKI_OPENFAST_NODES:
                d += 1
                nxt = []
                for x, y in frontier:
                    for dx, dy in ((0, -1), (1, 0), (0, 1), (-1, 0)):
            ...
            # Whatever went wrong, the base ordering is a correct answer.
            self._of_dist = {}
        return self._of_dist
    ...
    def _core(self, ct):
        ...
        # LOKI-OPENFAST (s48).  The base key is a pure dispersion hash with no
        # geometry term, and measured over 32 local cells it spends a median
        # +10.5 tiles of walking across the opening five.  For the ECO spawn
        # ordinals only, distance-to-ore leads and the hash stays as the
        # tie-break, so dispersion among equally-good tiles is unchanged and
        # the raider (ordinal 0) and the home defender (ordinal 4) are placed
        # exactly as the base places them.
        of = None
        if LOKI_OPENFAST_ON and self.n in LOKI_OPENFAST_SEATS:
            of = self._openfast_dist(ct) or None
        if of is None:
            cands.sort(key=lambda sp: ((sp.x * 17 + sp.y * 31 + self.n * 13 + self.spawn_salt) % 97,
                                       sp.y, sp.x))
        else:
            cands.sort(key=lambda sp: (of.get((sp.x, sp.y), LOKI_OPENFAST_FAR),
