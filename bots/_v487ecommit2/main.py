...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-ECOMMIT (s48), per unit. `ecommit_ban` maps a refused ore
        # tile (x, y) -> the round its deferral lapses; `adopt_next` is the
        # earliest round this builder will re-scan for an orphaned chain after
        # a scan that found nothing. Both are per-unit instance state with no
        # store slot, for the same reason the rest of this block is: a
        # launcher throw can move this body between turns and there is nothing
        # here that goes stale in a way that matters if it does. ---
        self.ecommit_ban = {}
        self.adopt_next = -1
