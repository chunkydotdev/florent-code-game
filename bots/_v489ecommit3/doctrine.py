...
# ============================================================================
# LOKI-ECOMMIT (s48) -- COMMIT BEFORE YOU DIG.  Study mechanism M1,
# docs/research/ECO-STUDY-fast-connected-harvesters-2026-08-17.md §M1.
#
# THE MEASUREMENT THIS EXISTS FOR.  435 of 1,730 v155 harvesters (25.1%) NEVER
# structurally connect to our Core, against the opponents' 13.7% in the same
# 245 games -- we strand at twice their rate.  0 of 1,730 were terrain-
# unroutable, and the failure is NOT attrition (74.9% connected in the 209
# games with zero builder deaths by r60).  It is monotone in distance and
# concentrated in harvesters #3-#6, where the ever-connected share falls
# 81% -> 71% -> 66% -> 64% -- exactly the ones `_expand` builds with no route
# check of any kind.  A harvester that never reaches the Core scores ZERO on
# titanium_collected forever AND leaves +5% permanent cost scale behind it.
#
# TWO HALVES, AND THE SECOND IS WHAT MAKES THE FIRST SAFE.
#
# 1. THE GATE (`_ecommit_ok`, eco.py).  Refuse a harvester site whose route
#    home does not exist, is longer than ECOMMIT_MAX_LINK_TILES, or which the
#    bank cannot pay for BELT AND ALL at current scale.  No new router and no
#    new BFS in the common case: it asks `_samestop_plan`, which is the SAME
#    per-ore-tile cache the stop-tile preference already fills one branch
#    later in the same turn (eco.py:852).  A site already in the Core ring, or
#    one with a live acceptor beside it, is connected the instant the
#    harvester lands and is admitted without any flood at all.
#
# 2. ORPHAN ADOPTION (`_adopt_orphan`, eco.py).  `link_queue` is per-builder
#    instance state with no store slot and no hand-off, so when a builder
#    dies, converges on the Core, diverts to siphon denial or is thrown by a
#    launcher, its route dies with it -- and `_l4_repair` explicitly REFUSES
#    the wreckage: its own docstring names them "DEAD HEADS: chains this bot
#    abandoned mid-walk, which `_build_next_link` never returns to because it
#    pops its queue as it lays it", and "a dead head with no acceptor in reach
#    ... is left alone".  So a dropped chain is unrecoverable by ANY machinery
#    in the parent tree.  Adoption is the hand-off: an eco builder with an
#    EMPTY link_queue that can see a friendly harvester with no acceptor at
#    all re-plans that harvester's route and lays the rest of it.
#    ⛔ Half 1 without half 2 would be a gate that leaves every existing dead
#    head on the map; the study says do not implement them apart.
#
# DEFERRAL, NEVER ABANDONMENT -- the mitigation the study names for the one
# real risk (a gate that fires too often and shrinks the economy; ECO_CAP=18
# is an absolute integer and a tile budget would bind below it on big maps).
# A refusal bans that ORE TILE for ECOMMIT_BAN_RNDS rounds and nothing else:
# the builder re-picks, which under the unchanged `_pick` cursor means a
# DIFFERENT tile from the same seat's stripe, and the ban expires.  No ore is
# ever struck off the map.
#
# WHAT IS NOT TOUCHED.  SAMESTOP is untouched -- it reads the same cached
# `plan` and, when the gate refuses, no harvester is built so nothing is armed.
# TURBO (`_bfs_direction`) is untouched.  BODYAWARE is untouched.  The
# `_pick` partition, its Manhattan sort and its stripe are untouched -- this
# plank changes WHETHER a site is dug, never WHICH sites a seat owns.
# ============================================================================

LOKI_ECOMMIT_ON = True          # the whole plank; False == the parent exactly
# Route budget in TILES, not in distance-squared -- the study's own mitigation
# wording ("budget the route in TILES").  Connect latency is ~2.00 rounds per
# tile of BFS route and degrades WORSE than linearly past L ~ 8 (at L = 10 the
# measured median is 32 rounds against the linear model's 20), so 8 is the
# knee the corpus actually shows and not a round number.
ECOMMIT_MAX_LINK_TILES = 8
# Refuse a site the bank cannot wire END TO END at current scale: harvester +
# len(route) x conveyor. Without this the gate admits a route we then cannot
# afford, which is the same never-connected harvester with extra steps.
ECOMMIT_FUND_BELT = True
# --- LOKI-ECOMMIT3 (s49) THE ONE PLANK OF THIS ARM: PREPAY A PREFIX, NOT THE
# WHOLE BELT.  How many of the route's UNPAID tiles the funding gate above
# demands cash for, at the instant the harvester is built.  `None` == every
# unpaid tile == `_v477ecommit` byte-for-byte (the guard: the arithmetic below
# reduces to the parent's `unpaid * conveyor_cost` when this is None).
#
# WHY THIS CONSTANT AND NOT `ECOMMIT_MAX_LINK_TILES`.  The parent's registered
# first suspect was over-refusal through the `long` gate; the only firings that
# exist for the 8->16 iteration (`_v487ecommit2`, 3 of 24 F-battery cells, ONE
# map, ONE unseeded seed -- a DIRECTION, not an effect size) read `long` = 1,
# `bank` = 6, `route` = 0.  If that survives replication the binding gate is
# the FUNDING one, and 8->16 does not touch it.
#
# WHY A PREFIX IS THE HONEST FORM AND NOT A LOOSENING FOR ITS OWN SAKE.  The
# belt is not bought at the harvester's instant -- it is bought ONE TILE PER
# BUILDER ACTION over the following rounds, and `_build_next_link` (eco.py)
# already re-checks `_eco_spendable(ct, conveyor_cost)` per tile and RETURNS
# FALSE while keeping `link_queue` intact.  So an underfunded TAIL stalls and
# resumes when the bank recovers; it does not strand the harvester.  The gate's
# whole-route prepayment is therefore a SECOND, stricter test applied to money
# that will have arrived before it is spent -- and passive income alone is
# 10 Ti / 4 rounds = 2.5 Ti/round against a conveyor at 3 x scale, i.e. it
# funds most of a tile per round while the builder can lay at most one.
# 3 tiles is the prefix that covers the builder's next ~3 actions, which is the
# window before that income lands.
#
# ⛔ WHAT IT COSTS, IN THE ARM'S OWN CURRENCY.  Admitting a site BUYS MORE BELT:
# every conveyor is +1% on the ONE GLOBAL ADDITIVE team cost scale and the
# harvester is +5%, so a flat or negative result here is a real price signal
# and not a null.  Economy is instrumental (R1000_IS_DEFEAT) -- this constant
# is only worth moving because a harvester that never delivers still leaves its
# +5% behind, and because titanium bought the kill.
ECOMMIT_FUND_TILES = 3
# Rounds an ore tile stays deferred after a refusal. Long enough that the
# builder walks somewhere else instead of re-refusing the same tile every
# round; short enough that a bank recovery or a new belt re-admits it.
ECOMMIT_BAN_RNDS = 40
# --- orphan adoption ---
ECOMMIT_ADOPT_ON = True         # half 2; False leaves the gate alone (do not)
# Rounds between adoption scans on a builder that found nothing. The scan is
# get_nearby_buildings() plus at most one _link_path, and it only runs at all
# when link_queue is EMPTY -- i.e. on a builder that is walking, not wiring.
ECOMMIT_ADOPT_EVERY = 6
# A route we would refuse to COMMIT to is a route we will not adopt either;
# same budget, so the two halves cannot disagree about what "wireable" means.
ECOMMIT_ADOPT_MAX_TILES = ECOMMIT_MAX_LINK_TILES
# Instrumentation only: one print per gate refusal and per adoption, into the
# replay. LOCAL instrument only -- platform-downloaded replays strip stdout
# (30,664 of 30,664), so nothing may be read off a live leg with it. Dose tags
# "EC1DEFER" and "EC1ADOPT".
LOKI_ECOMMIT_LOG = True
