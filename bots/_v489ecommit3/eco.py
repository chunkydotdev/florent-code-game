...
class EcoMixin:
    ...
    # --- LOKI-ECOMMIT (s48): commit before you dig, and adopt orphans.
    # See doctrine.py for the study cut (M1) and the 25.1% measurement.
    # ------------------------------------------------------------------

    def _ecommit_banned(self, ct, x, y):
        """True while ore tile (x, y) is deferred by a route refusal.

        A DEFERRAL, not an abandonment: the entry carries the round it lapses
        and is dropped on read, so a tile refused for want of bank comes back
        once the bank recovers, and one refused for want of a route comes back
        once somebody's belt has moved the frontier.
        """
        if not LOKI_ECOMMIT_ON or not self.ecommit_ban:
            return False
        until = self.ecommit_ban.get((x, y))
        if until is None:
            return False
        if ct.get_current_round() >= until:
            del self.ecommit_ban[(x, y)]
            return False
        return True

    def _ecommit_route(self, ct, ore):
        """The route home for `ore`, through the cache SAMESTOP already fills.

        `_samestop_plan` is cached per ore tile (eco.py above) and the stop-tile
        preference asks it for the SAME tile one branch later in the same turn,
        so in the common case -- gate a build on the ore this builder walked to
        -- this costs no flood at all.  With SAMESTOP off there is no cache and
        the router is called directly; the answer is the same list either way,
        because `_samestop_plan` is a cache in front of `_link_path` and
        nothing else (research #50 cut, step 3c: do not invent a parallel
        router).
        """
        if LOKI_SAMESTOP_ON:
            ...
            if plan is not None:
                return plan
        return self._link_path(ct, ore)

    def _ecommit_unpaid(self, ct, plan):
        """Tiles of `plan` we would actually have to BUY.

        `_link_path` routes THROUGH friendly belt (LINK_ROUTABLE_TYPES) and
        `_build_next_link` pops an occupied tile off the queue without paying
        for it, so `len(plan)` is the route's LENGTH and not its PRICE.  The
        first build of this gate funded the length and refused sites whose
        route was already half-laid -- which is the exact opposite of what the
        plank is for, since a route that reuses our belt is the cheapest kind
        there is.  A tile we cannot read is counted as unpaid, so an unknown
        is conservative rather than free.
        """
        n = 0
        for t in plan:
            try:
    ...
    def _ecommit_why(self, ct, bp):
        """THE GATE.  None when we may commit to wiring a harvester at `bp`,
        otherwise the REASON we may not.

        A reason and not a bool, because the three refusals below are three
        different mechanisms and a log that renders them identically is a log
        nobody can act on afterwards -- and a one-game demo has no other
        instrument.  `route` / `long` / `bank` are the three tags.

        THREE ADMISSIONS FIRST, each a site that is connected the moment the
        harvester lands, so asking the router would be paying for an answer we
        already hold:
          * the plank is off, or we have no anchor to route to;
          * `bp` is in the Core ring itself -- `_link_path` returns [] for a
            start already in its goal set, and [] from THAT branch means
            "already home", not "no route", so the two must not be conflated;
          * `bp` already has one of our acceptors beside it -- our belt, or a
            chain somebody else laid, already reaches this deposit.

        THEN THE FLOOD, AND THREE REFUSALS:
          * `route` -- NO ROUTE AT ALL.  0 of 1,730 measured harvesters were
            terrain-unroutable, so in practice this fires on a flood-budget or
            blocker edge rather than on geometry; refusing costs nothing by
            construction, because a site with no route delivers nothing ever.
          * `long`  -- past ECOMMIT_MAX_LINK_TILES the corpus puts us in the
            superlinear tail where the never-connect rate lives (at L = 10 the
            measured median connect latency is 32 rounds against the linear
            model's 20).
          * `bank`  -- harvester + the UNPAID tiles of the route x conveyor, at
            current scale, through the same `_eco_spendable` the rest of the
            eco spends through so the siege reserve is honoured here too.

        ⚠ EVERY EXCEPTION PATH ADMITS THE BUILD.  A gate that fails closed on a
        raising getter would silently stop the economy on exactly the maps
        where vision is tightest; this one can only ever refuse on an answer it
        actually got.
        """
        if not LOKI_ECOMMIT_ON or self.core is None:
            return None
        try:
            if (bp.x, bp.y) in self._link_goals(self._pave_ban()):
                return None
            if self._has_acceptor(ct, bp):
                return None
        except Exception:
            return None                      # never let the gate kill a build
        try:
            plan = self._ecommit_route(ct, bp)
        except Exception:
            return None
        if not plan:
            return "route"
        if len(plan) > ECOMMIT_MAX_LINK_TILES:
            return "long"
        if ECOMMIT_FUND_BELT:
            try:
                unpaid = self._ecommit_unpaid(ct, plan)
                # LOKI-ECOMMIT3 (s49): prepay a PREFIX of the unpaid route, not
                # all of it. `None` restores the parent's arithmetic exactly.
                if ECOMMIT_FUND_TILES is not None and unpaid > ECOMMIT_FUND_TILES:
                    unpaid = ECOMMIT_FUND_TILES
                need = (ct.get_harvester_cost()
                        + unpaid * ct.get_conveyor_cost())
            except Exception:
                return None
            if not self._eco_spendable(ct, need):
                return "bank"
        return None

    def _ecommit_defer(self, ct, bp, why):
        if not LOKI_ECOMMIT_ON:
            return
        self.ecommit_ban[(bp.x, bp.y)] = ct.get_current_round() + ECOMMIT_BAN_RNDS
        if LOKI_ECOMMIT_LOG:
            print(f"EC1DEFER rnd={ct.get_current_round()} "
                  f"tile={bp.x},{bp.y} why={why} id={ct.get_id()}")

    def _adopt_orphan(self, ct):
        """HALF 2: pick up a chain nobody owns any more.

        `link_queue` is per-builder instance state with no store slot and no
        hand-off, so a builder that dies, converges on the Core, diverts to
        siphon denial or gets thrown by a launcher takes its route with it --
        and `_l4_repair` refuses the wreckage by design ("a dead head with no
        acceptor in reach ... is left alone").  This is the hand-off the parent
        tree does not have.

        THE TEST IS THE HARVESTER, NOT THE BELT.  `_has_acceptor` is False only
        when a harvester has NO route home at all, which is exactly the state
        that scores zero on titanium_collected forever.  A harvester with a
        partial belt beside it already has an acceptor and is not orphaned by
        this definition, so this cannot fire on a chain that is merely SHORT.

        Only ever called with an EMPTY `link_queue` -- a builder mid-wiring is
        never diverted -- and only every ECOMMIT_ADOPT_EVERY rounds on a
        builder that found nothing, so the scan cost is bounded on the units
        that would otherwise pay it every round.
        """
        if not (LOKI_ECOMMIT_ON and ECOMMIT_ADOPT_ON) or self.core is None:
            return False
        if self.link_queue or self.map_grid is None:
            return False
        rnd = ct.get_current_round()
        if rnd < self.adopt_next:
            return False
        if self._cpu_exhausted(ct):
            return False
        if not self._eco_spendable(ct, ct.get_conveyor_cost()):
            self.adopt_next = rnd + ECOMMIT_ADOPT_EVERY
            return False
        p = ct.get_position()
        best = None
        best_d = None
        try:
            for eid in ct.get_nearby_buildings():
                ...
                if ct.get_team(eid) != self.team:
                    continue
                hp = ct.get_position(eid)
                if self._has_acceptor(ct, hp):
                    continue
                d = abs(hp.x - p.x) + abs(hp.y - p.y)
        ...
        if best is None:
            ...
            return False
        try:
            path = self._link_path(ct, best)
        except Exception:
            path = []
        if not path or len(path) > ECOMMIT_ADOPT_MAX_TILES:
            self.adopt_next = rnd + ECOMMIT_ADOPT_EVERY
            return False
        ...
        self.adopt_next = rnd + ECOMMIT_ADOPT_EVERY
        if LOKI_ECOMMIT_LOG:
            print(f"EC1ADOPT rnd={rnd} harv={best.x},{best.y} "
                  f"len={len(path)} id={ct.get_id()}")
        return True
    ...
    def _pick(self, ct):
        ...
        if self.map_ores and self.role != "defend":
            ...
            for _ in range(len(assigned)):
                ...
                self.ore_cursor += 1
                # LOKI-ECOMMIT (s48): a tile the gate just refused is skipped
                # here too, which is what makes the refusal a DEFERRAL rather
                # than a stall -- the cursor keeps walking this seat's own
                # stripe instead of re-offering the tile we cannot wire.
                if self._ecommit_banned(ct, t.x, t.y):
                    continue
                try:
    ...
    def _expand(self, ct):
            ...
            if (
                ...
                for ddx, ddy in DIR_DELTAS:
                    ...
                    if not (0 <= bx < self.mw and 0 <= by < self.mh):
                        continue
                    bp = Position(bx, by)
                    if self._ecommit_banned(ct, bx, by):
                        continue
                    try:
                        ok = ct.can_build_harvester(bp)
                    except Exception:
                        continue
                    if ok and LOKI_ECOMMIT_ON:
                        # LOKI-ECOMMIT (s48) THE GATE. A site we cannot commit
                        # to wiring is DEFERRED, not abandoned: the tile is
                        # banned for ECOMMIT_BAN_RNDS and the loop moves on to
                        # the next adjacent ore, then `_pick` to another tile
                        # of this seat's own stripe. No delivery is lost by
                        # refusing an unwireable site -- by construction it
                        # would have delivered nothing.
                        why = self._ecommit_why(ct, bp)
                        if why is not None:
                            self._ecommit_defer(ct, bp, why)
                            continue
                    if ok:
        ...
        if len(self.map_walls) >= ORE_STEPOFF_MIN_WALLS and on_ore:
            for si in (0, 1, 2, 3):
                ...
                if ok:
                    ...
                    self.stuck = 0
                    return
        # LOKI-ECOMMIT (s48) HALF 2: before this builder walks off to a new
        # ore, offer it the wreckage. Placed here, immediately above the
        # link_queue follow branch, because adoption's whole effect is to FILL
        # link_queue -- so a successful adoption falls straight into the
        # parent's own chain-following code below with nothing else changed.
        if not self.link_queue:
            self._adopt_orphan(ct)
        if self.link_queue:
        ...
        for odx, ody in DIR_DELTAS:
            ...
            if 0 <= bx < self.mw and 0 <= by < self.mh:
                bp = Position(bx, by)
                # LOKI-ECOMMIT (s48): the deferral has to hold here as well,
                # or this short-circuit re-targets the tile the gate just
                # refused and the builder loops on it until the ban lapses.
                if self._ecommit_banned(ct, bx, by):
                    continue
                try:
