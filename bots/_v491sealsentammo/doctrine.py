...
# ⭐⭐ _v491sealsentammo IS THIS ONE FLAG BECOMING REACHABLE.  In the parent
# _v481sealsentAnofund the magazine leg was DEAD CODE in both halves: `dry` was
# computed at raid.py:920-925 and its ONLY consumer was nested under
# LOKI_SEALSENT_FUND_ON (raid.py:926), which this lineage sets False; and the
# only reader of SLOT_SEALSENT (eco.py `_sealsent_reserve`) returned 0 on the
# same flag.  So the arm bought a Sentinel and never protected a single shot's
# worth of bank -- exactly the drumlin-s3 failure the FLOOR comment below
# records.  Both halves now read AMMO_ON as well, so the leg fires with
# FUND_ON=False.  AMMO_ON=False restores _v481sealsentAnofund byte-for-byte.
# ⚠ SIZING, STATED HONESTLY BECAUSE IT IS THE MOST LIKELY NULL: the reserve the
# beat buys is `get_sentinel_cost() + FUND_MARGIN`, i.e. the price of a SECOND
# turret, to fund 20 ammunition.  It is not sized to the magazine, it is reused.
# The Core needs bank > its own ti_floor (12, or up to
# min(harvester_cost, E1_RESERVE_CAP) + E1_HARV_RESERVE_MARGIN = 46 via
# E1_AMMO_FLOOR_ON) PLUS 4 before `amt >= 4` lets convert_ammo run, so a reserve
# of 48 (sentinel_cost 42 at scale 1.0 + 6) leaves only 2 Ti of conversion
# headroom and buys NOTHING.  It only works because the seat plant happens at
# 1.4-2.7x scale, where sentinel_cost is 59-87.  If the dose reads zero shots,
# SUSPECT THE RESERVE SIZE FIRST, not the mechanism.
LOKI_SEALSENT_AMMO_ON = True   # the magazine leg of the want-beat -- LIVE here
