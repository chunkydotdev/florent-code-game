...
class EcoMixin:
    ...
    def _sealsent_reserve(self, ct):
        """Titanium the economy must leave alone for a pending seal Sentinel.

        LOKI-SEALSENT, the funding-contention guard.  Magnus, s48: "if we allow
        our builders to build unlimited harvestors they will take the titanium
        from our offensive builders trying to set up turrets."

        NOTHING IS CAPPED HERE, and that is the design.  The harvester CAP that
        _v469kladtkill / _v472kladtk2 tried read at roughly -8.5pp on the s47
        shard; that road is dead.  This instead makes eco spending DEFER ABOVE A
        RESERVE FLOOR: while a raider standing at a qualifying enemy seat cannot
        pay for the Sentinel it wants, it publishes a want-beat and the economy
        stops spending the top `sentinel_cost + margin` of the bank.  The moment
        the turret is bought -- or the raider walks off, dies, or the cap
        closes -- the beat goes stale in LOKI_SEALSENT_FUND_STALE rounds and the
        economy takes the whole bank back.  Harvesters and belts are never
        forbidden, only outranked, and only while a turret is actually pending.

        Read as a BEAT rather than a flag for the same reason SLOT_RAID_LIVE is:
        store writes are buffered one round, so a level would need someone to
        clear it and the only unit that could is the one that just died.
        """
        # ⭐ READS FOR EITHER LEG.  The parent gated this on LOKI_SEALSENT_FUND_ON
        # alone, which is the SECOND half of the dead-code path: even if the
        # magazine leg had published its beat, the only consumer of SLOT_SEALSENT
        # in the whole tree is this function and it returned 0 unconditionally.
        # AMMO_ON=False restores the parent exactly (`not FUND_ON`).
        if not (LOKI_SEALSENT_FUND_ON or LOKI_SEALSENT_AMMO_ON):
            return 0
        try:
            beat = ct.read_store(SLOT_SEALSENT)
            if not beat:
                return 0
            if ct.get_current_round() - (beat - 1) > LOKI_SEALSENT_FUND_STALE:
                return 0
            return ct.get_sentinel_cost() + LOKI_SEALSENT_FUND_MARGIN
        except Exception:
            return 0

    def _eco_spendable(self, ct, cost):
        ti = ct.get_global_resources()
        # LOKI-SEALSENT: the single choke point for every eco spend in this
        # tree -- harvester (_expand), belt links (_build_next_link, _l4_repair,
        # _move's opportunistic lay) and, through main.py, the launcher, which
        # already carries its own LAUNCHER_RESERVE.  Deferring all of them is
        # deliberate: while the siege has a turret pending, the turret is first.
        res = self._sealsent_reserve(ct)
        if res and ti < cost + res:
            return False
        if (
            SIEGE_RESERVE_ON
