    ...
    def _sealsent_try(self, ct, E, p, seatkeys):
        ...
        if have and LOKI_SEALSENT_AMMO_ON:
            try:
                ...
                dry = False
        # ⭐⭐ THE TWO LEGS NOW CARRY THEIR OWN GATES, AND THIS IS THE WHOLE ARM.
        # In _v481sealsentAnofund this read `LOKI_SEALSENT_FUND_ON and (short or
        # dry)`, so with FUND_ON=False the MAGAZINE leg was DEAD CODE: `dry` was
        # computed three lines up and had no other consumer anywhere in the tree
        # (grepped: raid.py 920/921/923/925/926 only).  Turning off the eco
        # deferral therefore also turned off the one thing that let the Core's
        # own convert_ammo reach `ti > ti_floor` -- so the arm planted a
        # cost-scale-inflating Sentinel and never guaranteed it a shot.  The two
        # legs bill DIFFERENT things and so must gate separately:
        #   BUILD LEG    (short) -- FUND_ON.  Defers eco to buy the TURRET.
        #                Off here: the local dose-response says this leg is the
        #                expensive half and the parent's registered state is OFF.
        #   MAGAZINE LEG (dry)   -- AMMO_ON.  Defers eco to buy the SHOTS, and
        #                only ever while a seat Sentinel is ALREADY STANDING
        #                (`have`), which is a far narrower window than `short`.
        # ⛔ EQUIVALENCE: `dry` is False whenever AMMO_ON is False (line 921), so
        # with AMMO_ON=False this expression reduces to `FUND_ON and short` ==
        # the parent's `FUND_ON and (short or False)` for EVERY value of FUND_ON.
        # With BOTH on it reduces to `short or dry` == the parent again.  The
        # only cell that moves is FUND_ON=False, AMMO_ON=True -- this arm.
        # ⛔ NOTHING BELOW IS LOWERED.  The magazine leg only ever RAISES the
        # bank floor the economy must respect; it does not touch E1_AMMO_FLOOR,
        # LOKI_SPAWN_RESERVE or any harvester reserve.  The Core still refuses to
        # convert under its own `ti_floor`.
        if (((LOKI_SEALSENT_FUND_ON and short) or (LOKI_SEALSENT_AMMO_ON and dry))
