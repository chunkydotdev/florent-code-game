...
# ============================================================================
# SIEGEBREAK (2026-08-17) -- ONE PLANK, ONE FLAG.  THE DEFENDER'S ACTION MAY BE
# SPENT REMOVING A NEAR-CORE ENEMY TURRET INSTEAD OF HEALING UNDERNEATH IT.
#
# WHY THIS IS A RESTORATION, NOT AN INVENTION.  Everything from
# "TURRET-HUNTING UNDER SIEGE" (this file, ~line 169) through "DUEL DISCIPLINE
# (Piece D)" (~line 216) describes `_hunt_turret` and `_duel_safe`, two methods
# that DO NOT EXIST anywhere in this tree.  `HUNT_DESIGNATE_DSQ`,
# `HUNT_MIN_HEALERS`, `HUNT_FINISH_HP`, `HUNT_FIRE_TI` and `DUEL_DISCIPLINE_ON`
# are defined above and read by NOTHING in main.py, eco.py or raid.py.  The
# implementation lives in `bots/_v109loki5/main.py` (`_duel_safe` at 1326,
# `_hunt_turret` at 2054) and is absent from `_v122loki6` onward -- so the
# counter-battery capability was dropped on the loki lineage while ~80 lines of
# its rationale stayed behind as comments.  This plank restores the NARROW half
# of it: the duel-safety test, ported near-verbatim, wired to the defender only.
#
# THE ARITHMETIC, and it is the whole design.  A builder peck is 2 Ti for 2
# damage at action cooldown 1, i.e. one peck EVERY round (cooldowns decrement at
# end of round).  So 25 HP Gunner = 13 pecks / 13 rounds / 26 Ti; 40 HP Sentinel
# = 20 pecks / 20 rounds / 40 Ti.  Our builder has 40 HP.
#   * ON the turret's facing ray: Gunner 7/round kills us in 6 rounds (12 damage
#     dealt of 25 needed) and a Sentinel 18-per-2-rounds kills us in ~5 (10 of
#     40).  A LONE BUILDER LOSES BOTH DUELS OUTRIGHT.
#   * OFF the ray: BOTH turrets fire a SINGLE-TILE-WIDE line along their facing
#     ONLY (official-docs.md:239-256).  Of a turret's four orthogonal
#     neighbours, at most ONE is on that line -- and when the facing is DIAGONAL,
#     NONE are.  A besieging turret faces our Core, so a perpendicular or rear
#     seat takes ZERO damage from it and the peck is free.
# => The duel is not lost, it is lost FROM THE WRONG TILE.  This plank never
# opens a duel it cannot win: it only ever pecks from a seat `_duel_safe`
# clears, and it walks to such a seat rather than to the turret.
#
# KNOWN COST, REGISTERED UP FRONT: destroying an enemy building REMOVES that
# entity's contribution to THEIR single additive cost-scale factor, i.e. it
# LOWERS their prices (engine-read, docs/research/engine-guard-matrix-exploit-
# hunt-2026-08-10.md:157-158).  Turret removal here is a TEMPO/DEFENCE act and
# is not justified economically anywhere in this plank.
#
# OPPORTUNITY COST ACCEPTED: the role=="defend" seat only, and only while
# SLOT_UNDER is latched.  That seat's alternative use is measured in this file
# already -- the T4 lockstep chase (a builder cannot `can_fire` a builder, so
# the chase buys nothing) and a heal seat at +4 HP/round against a Sentinel's
# sustained -9.  Healing loses that race by construction; removal ends it.  No
# expander, no raider and no other builder's heal is touched.
# ============================================================================

SIEGEBREAK_ON = True
SIEGEBREAK_BAND_DSQ = 41      # turret must be this close to OUR core (== HUNT_BAND_DSQ)
SIEGEBREAK_REACH_DSQ = 36     # ...and its duel-safe seat this close to US
SIEGEBREAK_MIN_BANK = 12      # never peck the bank to zero
SIEGEBREAK_MAX_TI = 120       # per-builder lifetime peck budget (60 pecks ~= 3 turrets)

# ============================================================================
# SIEGEABORT (2026-08-17) -- ONE PLANK, ONE FLAG.  `_duel_safe` PROVES THE SEAT
# IS SAFE AND NOTHING PROVES THE TARGET IS FALLING.
#
# THE MEASUREMENT THIS PLANK ANSWERS (s49 SIEGEBREAK leg, 30 paired games on the
# live 15-map pool, decoded off retained replays against `BuilderAttack`):
#   builder attacks on an enemy gunner/sentinel  TREAT 10.07/game  CTRL 0.00
#   turret kills with melee on the tile          8 from 302 pecks = 37.8/kill
#   theory (doctrine above)                      gunner 13, sentinel 20
#   => ~45% EFFICIENCY: HALF OUR DAMAGE IS ABSORBED.
#   enemy turrets alive within d^2<=41 of our core   END 1.30 vs 1.30 UNCHANGED
#   two of thirty games hit the full 60-peck budget -- the cap is BINDING.
#
# THE CAUSE IS ALREADY NAMED IN THIS FILE, ~1.7k lines above, and it is a
# COMMENT (a claim about code, not code): the eider Sentinel counter-example,
# "healer-escorted, it absorbed 630 damage and had 630 healed back, net zero, a
# pure titanium sink at 2 Ti per 2 damage".  An enemy builder healing +4 per
# action against our 2 damage per action means WE LOSE THAT RACE 2:1 -- and the
# parent's only stopping condition is SIEGEBREAK_MAX_TI, i.e. it burns the whole
# budget discovering this, every time, per body.
#
# THE MECHANISM: the attacker remembers the target's HP at its FIRST peck and
# refuses to keep paying when the HP has not fallen by what its own pecks should
# have produced.  Our peck is 2 damage, so after N pecks the HP must be down 2N
# absent healing.  `drop*100 < FRAC * 2 * N` is the whole test.
#
# WHY A RATIO AND NOT A DEFICIT OR A WINDOW -- the three forms considered:
#   * ABSOLUTE DEFICIT (abort when 2N - drop > D) does not scale with N.  With
#     D = 4, ONE stray heal inside a twenty-peck grind that is otherwise 90%
#     efficient trips it; with D large enough not to, an escort gets 20 free
#     pecks.  There is no D that is right at both N = 4 and N = 20.
#   * PATIENCE WINDOW (last K pecks only) needs a second, independent constant
#     (K) and buys nothing: the cumulative ratio measured from the FIRST peck
#     already IS a window whose start is pinned at the engagement, and pinning
#     it is what makes a SUSTAINED escort separable from a one-off heal.
#   * RATIO (chosen) is scale-free.  An escort holds the ratio near zero however
#     long the grind runs; a single heal inside a clean grind washes out as N
#     grows.  It is the same quantity as the "~45% efficiency" the leg measured,
#     so the abort is denominated in the number that named it.
# The two constants are the SAME test, not two mechanisms: MIN_PECKS is the
# denominator's validity condition (at N = 1 the drop is 0 or 2 and the ratio
# can only read 0% or 100%, so every threshold fires spuriously on the first
# peck), MIN_DROP_FRAC is that ratio's threshold.  Neither is usable alone.
#
# HOW IT INTERACTS WITH FINISH-ALWAYS-PAYS, and the abort LOSES that argument:
# `_duel_safe` (main.py) already exempts a target at HUNT_FINISH_HP or fewer HP
# -- "finishing always pays" -- and `_sa_refuse` checks that exemption FIRST, so
# a nearly-dead turret is re-engaged even after this body banned it.  Reasons:
# (a) at <=8 HP the whole remaining bill is bounded at 4 pecks / 8 Ti, which is
# cheaper than the walk to another seat; (b) a healer that misses one action
# loses the target outright, so the variance is on our side only down there;
# (c) a NEW rule that contradicted an EXISTING one on the same tile would make
# the tree's behaviour depend on which check ran first, and that is how planks
# become unattributable.  The cost of the exemption is a heal-loop at <=8 HP,
# and its worst case is exactly the parent's: SIEGEBREAK_MAX_TI still caps it.
#
# ABORTING IS HALF A MECHANISM -- WHERE THE REFUSED ACTION GOES.  Measured on a
# sibling arm this session: refusing an action freed 340 of them and only 23%
# were substituted; the rest IDLED.  So the ban is honoured in BOTH siegebreak
# paths, and both already scan every candidate:
#   * `_sb_fire` filters banned targets out of its adjacent-tile loop, so a
#     SECOND adjacent turret is pecked instead; with none, it returns False and
#     the caller falls through to `_heal_core` (call site 1) or to
#     `_sabotage_prio` / `_try_counterbattery` / `_heal_core` (call site 2).
#   * `_sb_seat` filters them out of its in-band turret scan, so the body WALKS
#     TO the next duel-safe seat instead; with none, it returns None and the
#     parent's T4 seat / chase / heal block runs byte-for-byte.
# i.e. the redirect is 1 Ti of healing or another turret, never an idle turn.
#
# REGISTERED LIMITS.  (1) The ban is per BODY and permanent for that body: an
# escort that leaves is not noticed.  Deliberate -- a timer is a third constant
# and the body's whole budget is 120 Ti anyway.  (2) The baseline is never
# re-based, so a heal ABOVE the baseline reads as a negative drop and aborts
# faster; that is the intended direction.  (3) Damage from OUR OWN turrets on
# the same target inflates the drop above 2N and SUPPRESSES the abort -- also
# intended: a shared grind is winnable and is what `_duel_safe` clause (b)
# already says.  (4) Entity ids are reused after death, so a fresh turret could
# in principle inherit a banned id; the cost is one refused target and the type
# and team are re-checked every round regardless.
# SIEGEABORT_LOG is INSTRUMENTATION, not a plank constant: stderr only (print()
# goes to the replay, stderr does not), False in the shipped tree, and flipped
# to True only in throwaway measurement copies.
# ============================================================================

SIEGEABORT_ON = True
SIEGEABORT_MIN_PECKS = 4       # ratio is meaningless below this many pecks
SIEGEABORT_MIN_DROP_FRAC = 50  # percent of 2*pecks the HP must actually have fallen
SIEGEABORT_LOG = False         # stderr diagnostics; no effect on play
