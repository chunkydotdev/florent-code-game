...
# ============================================================================
# LOKI-HEALGATE (_v492healgate) -- ONE TURRET-COVERAGE GATE ON THE HEAL LINE.
#
# THE FINDING IS A PRICING ERROR IN A DOCSTRING, AND THE DOCSTRING IS SHIPPED
# DOCTRINE.  `EcoMixin._heal_adjacent`'s docstring (eco.py, a DOCSTRING not
# executable code) reads verbatim:
#
#     "1 Ti for +4 HP against an enemy peck's 2 Ti for 2 dmg -- eight to one
#      on titanium."
#
# and `_raid_turn`'s step-5 collar comment (raid.py, a COMMENT) repeats the
# frame: "+4 HP for 1 Ti against their 2 dmg for 2 Ti".  Both price the heal
# against a BUILDER PECK.  A peck is 2 damage per enemy ACTION.  It is not the
# threat that kills the things we heal.
#
# THE ENGINE'S ACTUAL THREATS, read off GameConstants (executable values, not
# prose) and confirmed by probe:
#
#     GUNNER_DAMAGE   = 7   GUNNER_FIRE_COOLDOWN   = 1  -> fires EVERY round
#     SENTINEL_DAMAGE = 18  SENTINEL_FIRE_COOLDOWN = 2  -> fires every 2nd round
#     HEAL_AMOUNT     = 4   heal leaves action cooldown 1 -> +4 EVERY round
#
# (cooldown decreases by 1 at end of round and an action needs cooldown == 0,
# so cooldown 1 means "act again next round".  The heal cadence has NO
# GameConstants entry and was measured on the engine: cd_after_heal = 1, and a
# single builder landed one +4 heal per round for six consecutive rounds.)
#
# So per ROUND, one healer against one shooter:
#
#     vs a GUNNER    +4 against -7   ->  net -3 HP/round.  Heal LOSES.
#     vs a SENTINEL  +4 against -9   ->  net -5 HP/round.  Heal LOSES worse.
#     vs a PECKER    +4 against -2   ->  net +2 HP/round.  Heal WINS.
#
# The docstring is CORRECT about pecks and is being applied where pecks are not
# the threat.  Two smaller errors in the same sentence, noted so they are not
# re-derived: the titanium ratio against a pecker is 4:1, not 8:1 (they spend
# 1.0 Ti per HP removed, we spend 0.25 Ti per HP added -- the "eight" appears
# to multiply the 2:1 HP ratio by the 4:1 Ti ratio); and raid.py's buddy-heal
# comment "a Gunner's 7 damage on a 1-round reload (3.5 HP/round)" is wrong by
# a factor of two -- GUNNER_FIRE_COOLDOWN = 1 means every round, so 7/round.
# doctrine.py's own "a Sentinel is -9 HP/round sustained and a healer is +4"
# has always been the right reading; the two comments disagreed with each other
# and only one of them was right.
#
# ⭐ AND THE TITANIUM RATIO IS A TRUE FACT ABOUT THE WRONG CURRENCY.  Against a
# gunner we still pay 0.25 Ti/HP against their 0.57 Ti/HP of ammo, so on
# TITANIUM the heal wins 2.3:1 even under turret fire.  That is exactly why the
# error survived review for so long.  But titanium is not what is scarce here
# and `R1000_IS_DEFEAT` says it is not what scores: a heal consumes a whole
# BUILDER ACTION, and a shot consumes nothing we own.  Measured on our own
# tape: 5,882 heals = 90.5 builder-actions/game = 29.7% of the entire action
# budget, and 4,677 of them went into entities dead within 10 rounds = 23.6% of
# the budget.  The comparable figure for kladde v119 is 369/12,530 = 2.9%.
#
# WHY THE CORE IS DIFFERENT, AND IT IS AN HP-POOL ARGUMENT, NOT A LOYALTY ONE.
# A losing rate only matters if the pool runs out before help arrives:
#
#     SENTINEL_MAX_HP  = 40 : one gunner kills it in 6 rounds unhealed, 14 with
#                             one healer.  Eight rounds bought for 14 actions.
#     HARVESTER_MAX_HP = 30 : 5 rounds unhealed, 10 with a healer.
#     CONVEYOR_MAX_HP  = 20 : 3 rounds unhealed.
#     CORE_MAX_HP      = 500: 56 rounds against a sentinel unhealed, 100 with
#                             one healer -- and THREE converged healers is +12
#                             against -9, which flips the sign outright.
#
# Only the Core's pool is deep enough for the multi-healer convergence this
# file already implements to arrive while the patient is still alive.  That is
# the mechanism behind the measured outcome table (research, 65 rated games vs
# kladde v119, control 458 rated games on the same our-versions vs all other
# opponents):
#
#     our SENTINELS taking >=1 turret hit: 0 heals -> 95.7% died, median life 7
#                                          3+ heals -> 97.4% died, median life 7
#     our HARVESTERS:                      98.9% -> 98.9%
#     our CORE with 3+ heals:              62.1% died over a MEDIAN of 279
#                                          rounds under 35 hits
#     heal replaces 70.9% of damage on the CORE vs 28.0% on sentinels
#
# ⛔ SO A BLANKET "HEAL LESS" ARM WOULD DESTROY A WORKING BEHAVIOUR.  The gate
# spares the Core explicitly, and it spares every heal that is NOT under a
# turret ray -- the pecker case, where the shipped docstring is right and the
# +2/round exchange is real.  It refuses exactly the hopeless cell.
#
# THE GATE.  `EcoMixin._hg_covered(ct, tile)` is True when a LIVE enemy GUNNER
# or SENTINEL, at its own tile with its own CURRENT facing, could fire on tile.
# It asks the ENGINE, via `ct.can_fire_from(pos, facing, type, tile)`, which is
# the documented hypothetical-turret predicate: it uses current map occupancy
# and walls and ignores ammo and cooldown -- ignoring ammo is what we want,
# because an unfunded turret is one convert_ammo() away from funded.
#
# BOTH GEOMETRIES ARE MODELLED, AND BY DELEGATION RATHER THAN BY HAND.  A
# gunner's ray is BLOCKED by obstacles and a sentinel's PIERCES them; both are
# already inside can_fire_from, so this file does not re-implement either and
# cannot get either wrong.  Probed on the engine before use:
#
#   * can_fire_from is TEAM-BLIND ON THE TARGET.  With our own barriers, our
#     own builder bots and our own Core as `target` it returned True from many
#     seats.  This was the make-or-break question: had it team-checked the
#     target, the gate would have been ALWAYS-FALSE and vacuous.
#   * IT ANSWERS FROM AN OCCUPIED SEAT.  Queried at a live enemy sentinel's own
#     tile with its own facing, it agreed with the engine's own
#     get_attackable_tiles_from 4/4 on that turret's pattern, and marked 2 of
#     14 friendly-occupied tiles in vision as covered -- so it DISCRIMINATES
#     rather than saturating.
#   * OFF-MAP IS SAFE: can_fire_from returned False (did not raise) for an
#     off-map target AND for an off-map seat.  Every call is inside a try
#     regardless, because an uncaught exception out of run() destroys the unit
#     permanently.
#   * A GUNNER query needs an OCCUPANT at the target: on EMPTY tiles gunner
#     returned 0 Trues from every seat and facing while sentinel returned 15+.
#     Harmless here -- a heal target always holds the damaged thing -- but it
#     means this predicate must never be reused to ask about an empty tile.
#
# COST.  The enemy-turret census is built ONCE PER ROUND PER UNIT and cached
# (self.hg_rnd / self.hg_turrets), so a builder scanning four cardinal
# neighbours pays one census, not four.  The census is O(nearby buildings) and
# bails through the house `_cpu_exhausted` helper.  A tile verdict is cached
# too (self.hg_seen), so the same tile asked twice in a round is free.
#
# ⚠ STRUCTURAL LIMIT, STATED NOT HIDDEN: the census can only see turrets inside
# the asking unit's vision (BUILDER_BOT_VISION_RADIUS_SQ = 20).  A sentinel
# reaches 32.  So a sentinel shelling us from outside our own vision is INVISIBLE
# to this gate and the heal proceeds.  The gate therefore UNDER-refuses, which
# fails toward the control -- the safe direction for a first arm, and the
# obvious second arm (publish seen turrets through the store) if this one pays.
#
# SCOPE -- WHICH HEAL SITES.  There are exactly FIVE `ct.heal()` call sites in
# this tree (grep, not memory):
#
#   1. eco.py `_heal_core`            -- CORE ONLY.  UNGATED, DELIBERATELY.
#      This is the measured carve-out and it is left byte-for-byte alone, which
#      is also why our own core-survival number is a clean check on leakage.
#      Reached from main.py `_builder` (universal adjacent heal), `_home_defend`
#      and `_defend`.
#   2. eco.py `_heal_adjacent`        -- GATED, with a Core carve-out inside it.
#      It scans cardinal neighbours blind and can land on a Core tile, so the
#      four Core footprint tiles are allowed unconditionally there.
#   3. eco.py `_expand` chain medic   -- GATED.  MEDIC_TYPES is conveyor /
#      splitter / harvester: 20-30 HP, the worst cell in the table.
#   4. raid.py step 4 BUDDY HEAL      -- NOT GATED.  Different target class: a
#      friendly BUILDER BOT, which is a mobile body rather than a building, has
#      no row in the measured table, and whose loss costs an action generator
#      and not just HP.  The arithmetic does look the same (40 HP, 6 rounds
#      unhealed against a gunner, 14 with a healer -- and raid.py's own comment
#      "median raider life after a throw is six rounds" matches the UNHEALED
#      figure exactly), so this is the next arm, not a claim that it is fine.
#      Left out on purpose to keep this arm one plank.
#   5. raid.py step 5 HOLD THE COLLAR -- GATED, BEHIND ITS OWN SUB-FLAG.
#
# ⚠ THE COLLAR IS THE UNRESOLVED CASE AND IT GETS ITS OWN SWITCH.  Its comment
# claims the seal barriers are "the raider's own LOS cover against Gunner rays",
# so healing a barrier under gunner fire might be buying COVER rather than HP.
# Justification from the code, both ways:
#   FOR the claim: a gunner's shot IS blocked by obstacles and a barrier IS an
#     obstacle -- and this tree already relies on that fact elsewhere, in
#     `_try_forward_sentinel`'s docstring: "the collar blocks LOS, so a Gunner
#     built to shoot the Core would be shooting our own barriers".
#   AGAINST it: SENTINELS PIERCE OBSTACLES, so a barrier is worth ZERO cover
#     against the 18-damage threat; BARRIER_MAX_HP is 30, which is four rounds
#     of gunner fire whether or not we heal it; and nothing in the collar block
#     checks that the barrier it heals is actually between this raider and any
#     shooter -- it heals any adjacent damaged friendly building.
# Not resolvable from the code alone, so it is SEPARATELY SWITCHED rather than
# silently included or excluded.  Same predicate, same refusal, same mechanism;
# the only reason it is not folded into the master flag is that this
# counterargument applies to this site and to no other.
# ============================================================================

# ============================================================================
# LOKI-HEALBREAK (_v495healbreak) -- THE COMPOSITION.  HEALGATE FREES A HAND;
# BELTBREAK IS THE ONLY PLACE MEASURED TO WANT ONE.
#
# WHY THIS IS ONE MECHANISM AND NOT TWO PLANKS BOLTED TOGETHER.  HEALGATE
# (_v492healgate) works and does not pay: its gate refused 210 of 1618 collar
# heals (13.0%), cut SENTINEL heals 39.5% with CORE heals unchanged (+3), and
# then the freed actions IDLED -- 340 heals refused against 78 substitute
# actions (23% substitution) and act_move FELL 421.  BELTBREAK (_v480beltbreak)
# pays -- 53.81% [52.49,55.14] share at n=5400, timely-kill-by-r300 30.54% vs
# 26.33% non-overlapping -- and its own measured limit is that shots/shredder is
# FLAT as dose rises while its refusal funnel reads TI 638x/game against CAP
# 6.6x/game: the BANK stops the next shredder, and the marginal shredder sits on
# a worse chain.  (Both figures cited from the s49 build reports in
# docs/coordination.md, not re-derived here.)
#
# ⛔⛔ AND THE FIRST THING THIS BLOCK HAS TO SAY IS THAT THE OBVIOUS REDIRECT IS
# DEAD BY CONSTRUCTION, WHICH IS AN ARCHITECTURAL FACT AND NOT A MEASUREMENT.
# In `RaidMixin._raid_act` the ranking is: 1 peck, 2 seal, 3 forward sentinel,
# **3b BELTBREAK PLANT**, 4 buddy heal, **5 COLLAR HEAL**, 6 peck, 7 salt.
# Step 3b is therefore ALREADY TRIED, THIS ROUND, BEFORE the collar heal the
# gate refuses -- and nothing between 3b and 5 mutates any state 3b reads (both
# intervening branches return True when they act).  ⇒ **"offer the refused
# raider a beltbreak plant" is a PROVABLE NO-OP: a re-call would recompute the
# same refusal from the same state.**  This tree therefore does NOT re-call it.
# It READS step 3b's own refusal reason for this round (`self.bb_last`) and
# reports it as the redirect's refusal reason, which costs nothing and answers
# the funding question directly instead of by simulation.
#
# ⭐ WHAT IS LEFT, AND IT IS ONE SENTENCE OF GEOMETRY: A RAIDER PARKED ON THE
# COLLAR CAN NEVER PLANT A BELTBREAK GUNNER, AT ANY BANK.  The collar seats are
# the eight tiles orthogonally adjacent to the 2x2 enemy footprint and the four
# diagonal ring tiles, i.e. dsq_core <= 2; a build tile is one orthogonal step
# from the raider, so its own dsq_core is at most ~8.  The productive annulus
# starts at LOKI_BELTBREAK_DSQ_LO = 20.  ⇒ the plant is refused there on
# GEOMETRY before the bank is ever consulted, and the one thing a refused body
# can change for free is WHERE IT IS STANDING.
#
# THE REDIRECT, THEREFORE: A BENCH IN THE ANNULUS.  On a gate-refused collar
# heal the raider latches the nearest passable tile whose dsq_core to the ENEMY
# core lands in [LOKI_BELTBREAK_DSQ_LO, +HB_BENCH_PAD], found by stepping
# radially outward from where it stands.  While the latch is live it is the
# raider's MOVE target instead of its ring station; on arrival the raider parks
# in band, where step 3b's plant scan runs every round -- so the instant the
# bank clears `get_gunner_cost()` the plant lands.  The inner annulus is chosen
# deliberately: the study's excess belt-kills per gunner PEAK at d^2 20-30.
#
# COST IS A MOVE, NOT AN ACTION, AND THAT IS THE WHOLE POINT.  The refused
# raider was measured PARKED -- act_move fell 421 in the parent, because a
# raider standing on its own station has `p == self.tgt` and returns without
# moving.  A bench target is a tile it is not standing on, so it spends a move
# it was not spending.  No titanium, no action, nothing taken from the collar's
# barrier budget.
#
# ⛔ THE COST THAT IS REAL AND MUST BE REPORTED: A BODY OFF THE COLLAR IS A SEAT
# UNSEALED.  `barrier-seal-off` read 399/1024 in s30 -- removing the seal is a
# measured NEGATIVE -- so this redirect is spending a known-good behaviour.  Two
# bounds on that: it fires ONLY for a raider whose collar heal the gate refused,
# i.e. one standing under a live enemy turret ray, which is the cell HEALGATE
# measured as hopeless anyway; and the latch expires (HB_BENCH_HOLD /
# HB_BAND_HOLD) so a raider cannot be benched for the match.
#
# ⛔ NOT OFFERED AT THE OTHER TWO GATED SITES, AND THE REASON IS THE FUNDING
# CONSTRAINT ITSELF.  `_hg_refuse` is asked at three sites: `_heal_adjacent`
# (VERIFIED DEAD -- its only caller `_home_defend` is reached only from
# main.py's melee-recall block, guarded `not LOKI8_RAIDERS_STAY_OUT`, and that
# constant is True), the `_expand` chain medic, and the collar.  The medic site
# belongs to ECONOMY builders standing in our own half: they are nowhere near
# the annulus, and they are the titanium source that BELTBREAK's TI refusals are
# waiting on.  Redirecting them would spend the funding to chase the symptom.
# ⇒ THE REDIRECT IS COLLAR-ONLY.
# ============================================================================
HEALBREAK_ON = True        # MASTER.  False == _v480beltbreak exactly: the gate
                           # goes silent (HG_* below all fold to False) and the
                           # redirect never runs.
# Each component stays separately switchable so the three can be split later.
# ⛔ THESE ARE `HEALBREAK_ON and ...` ON PURPOSE: eco.py in this tree is
# BYTE-IDENTICAL to _v492healgate/eco.py, so the master flag reaches the gate
# through its constants rather than through an edit to the ported code.
HG_COVER_GATE_ON = HEALBREAK_ON and True   # component 2: the coverage gate
# The collar heal (raid.py step 5) inside the gate?  Only meaningful while
# HG_COVER_GATE_ON; see the LOS-cover argument above.  False leaves the collar
# healing exactly as the parent does while the other two sites stay gated.
# ⚠ IT IS ALSO THE ONLY SITE THE REDIRECT HANGS OFF, so HG_GATE_COLLAR False
# makes component 3 inert as well.
HG_GATE_COLLAR = HEALBREAK_ON and True
# Instrumentation only, same shape as LOKI_SAMESTOP_LOG above: one stderr line
# per gate decision, tag "HG492".  stderr is console-only (print() goes to the
# replay, and platform replays strip stdout entirely), so this is a LOCAL
# instrument.  MUST stay False in anything fired -- it is a per-decision write
# inside the hot path.
HG_LOG = False

# --- component 3: THE REDIRECT --------------------------------------------
HB_REDIRECT_ON = HEALBREAK_ON and True
# Bench band: [LOKI_BELTBREAK_DSQ_LO, LOKI_BELTBREAK_DSQ_LO + HB_BENCH_PAD].
# 10 keeps the bench inside d^2 20-30, the study's peak excess-belt-kill shell,
# rather than anywhere in the 20-100 annulus.
# ⚠ 14, NOT 10, AND THE REASON IS THE STEP PATTERN.  A DIAGONAL outward step
# raises dsq_core much faster than a cardinal one (from d^2 12 a diagonal step
# lands on ~23 and the next on ~38), so a 20-30 window is jumped clean over on
# some seats and the bench comes back empty.  20-34 brackets the peak shell on
# both step patterns.  Verified by the NOSEAT counter, which must not dominate.
HB_BENCH_PAD = 14
HB_BENCH_STEPS = 6         # radial steps searched outward (CPU bound, O(steps))
HB_BENCH_HOLD = 12         # rounds a walk latch survives without a re-refusal
HB_BAND_HOLD = 40          # ONE in-band dwell, NOT refreshed by more refusals
HB_COOL = 20               # after a completed cycle, re-benching is banned for
                           # this many rounds -- the duty-cycle bound that stops
                           # a raider being removed from the collar for good
# Refusal reasons from step 3b that the redirect will NOT act on.  CAP means we
# already hold LOKI_BELTBREAK_CAP live shredders, so a body walking into the
# annulus cannot plant however long it waits; every other reason is resolvable
# by time (EARLY/HARV), by bank (TI) or by position (NOTGT/NORAY/OWNRAY).
HB_DEAD_REASONS = frozenset(("CAP",))
HB_LOG = False             # LOCAL stderr instrument, tag "HB495".  Never fired.
