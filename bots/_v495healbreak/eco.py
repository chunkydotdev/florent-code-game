...
class EcoMixin:
    ...
    # --- LOKI-HEALGATE: the turret-coverage gate ---------------------------

    def _hg_turrets(self, ct):
        """[(pos, facing, type)] for every LIVE enemy gunner/sentinel in vision.

        Built ONCE per round per unit and cached, so a builder asking about its
        four cardinal neighbours pays one census rather than four.  Vision-bound
        by construction (see the STRUCTURAL LIMIT note in doctrine.py): a
        sentinel reaches r^2 = 32 and a builder sees r^2 = 20, so this census
        can MISS a live shooter.  It therefore under-reports threat, which makes
        the gate under-refuse -- the direction that falls back to the parent.
        """
        rnd = ct.get_current_round()
        if self.hg_rnd == rnd:
            return self.hg_turrets
        self.hg_rnd = rnd
        self.hg_seen = {}
        out = []
        if not self._cpu_exhausted(ct):
            try:
                near = ct.get_nearby_buildings()
            except Exception:
                near = ()
            for eid in near:
                try:
                    ...
                    if et != EntityType.GUNNER and et != EntityType.SENTINEL:
                        continue
                    out.append((ct.get_position(eid), ct.get_direction(eid), et))
                except Exception:
                    continue
        self.hg_turrets = out
        return out

    def _hg_covered(self, ct, tile):
        """True if a live enemy gunner or sentinel could CURRENTLY fire on tile.

        Delegated wholly to `ct.can_fire_from`, the engine's own hypothetical-
        turret predicate: it uses live occupancy and walls (so a gunner's ray is
        blocked and a sentinel's is not, without this file modelling either) and
        ignores ammo and cooldown (so a turret that is merely unfunded still
        counts -- convert_ammo is one core turn away).

        Probed before use, because the failure that matters is a predicate that
        is accidentally always-False: can_fire_from is TEAM-BLIND on the target
        (it returns True for our own barriers, bots and Core) and it answers
        from an OCCUPIED seat (agreeing 4/4 with get_attackable_tiles_from on a
        live sentinel's own pattern).  Off-map returns False rather than raising,
        for both the seat and the target -- every call is inside a try anyway.
        """
        key = (tile.x, tile.y)
        seen = self.hg_seen
        if seen is not None and key in seen:
            return seen[key]
        verdict = False
        for tpos, facing, ttype in self._hg_turrets(ct):
            try:
                if ct.can_fire_from(tpos, facing, ttype, tile):
                    verdict = True
                    break
            except Exception:
                continue
        if self.hg_seen is not None:
            self.hg_seen[key] = verdict
        return verdict

    def _hg_refuse(self, ct, tile, site):
        """The gate, as one predicate every heal site can ask.

        True == do not spend the action healing `tile`.  False == heal exactly
        as the parent would.  Off (HG_COVER_GATE_ON False) it is False always,
        which is what makes the flag-off tree behaviourally identical.

        The Core is NEVER refused: CORE_MAX_HP is 500, deep enough for this
        file's own multi-healer convergence to arrive and flip the sign, and it
        is the one target the measurement says healing works on.
        """
        if not HG_COVER_GATE_ON:
            return False
        if self.core is not None and (tile.x, tile.y) in core_tiles_xy(self.core):
            return False
        if not self._hg_covered(ct, tile):
            return False
        if HG_LOG:
            import sys
            print("HG492 REFUSE", site, tile.x, tile.y, ct.get_current_round(),
                  file=sys.stderr)
        return True

    def _heal_core(self, ct):
        # UNGATED, DELIBERATELY.  The Core is the measured carve-out; see the
        # HP-pool argument in the LOKI-HEALGATE block in doctrine.py.
    ...
    def _heal_adjacent(self, ct):
        ...
        1 Ti for +4 HP against an enemy peck's 2 Ti for 2 dmg -- four to one on
        titanium (the "eight to one" this docstring used to claim multiplied the
        2:1 HP ratio by the 4:1 Ti ratio).  can_heal() enforces adjacency, cost
        and real damage, so this is free on a round when nothing is hurt.

        ⛔ AND THE SENTENCE ABOVE IS ONLY TRUE ABOUT PECKS.  The threats that
        actually kill what we heal are a gunner at 7 damage EVERY round
        (GUNNER_FIRE_COOLDOWN = 1) and a sentinel at 18 every second round, i.e.
        9/round -- against a healer's +4/round.  LOKI-HEALGATE refuses the heal
        on any tile a live enemy turret can currently fire on, EXCEPT a Core
        tile: this scan is blind to what it is standing next to, so it can land
        on the Core footprint, and that heal is the one the measurement defends.
        """
        p = ct.get_position()
        px, py = p.x, p.y
                ...
                for mdx, mdy in CARD_DELTAS:
                    ...
                    try:
                        ...
                        if (
                            ...
                            # LOKI-HEALGATE.  MEDIC_TYPES is conveyor (20 HP),
                            # splitter (20) and harvester (30): under a gunner's
                            # 7/round or a sentinel's 9/round a +4/round repair
                            # is 3-5 rounds of delay for one action per round.
                            # Measured: our harvesters died 98.9% of the time
                            # with heals and 98.9% without.
                            if self._hg_refuse(ct, bp, "medic"):
                                continue
                            ct.heal(bp)
                            return
                    except Exception:
