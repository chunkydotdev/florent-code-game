...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        self.bb_shots = 0
        self.bb_seen = False
        # LOKI-HEALBREAK reads this: (round, reason) of the LAST beltbreak
        # refusal.  Written in `_bb_refuse` under HEALBREAK_ON only, so the
        # flag-off tree does not execute it at all.  It is how the redirect
        # learns why step 3b said no THIS round without re-running step 3b --
        # which would be a provable no-op (see the doctrine block).
        self.bb_last = None
        self.bb_last_rnd = -10 ** 9

        # --- LOKI-HEALGATE turret-coverage cache, per unit.  hg_rnd is the
        # round the census was taken; hg_turrets is [(pos, facing, type)] of
        # enemy gunners/sentinels in vision then; hg_seen memoises per-tile
        # verdicts for that same round.  All three are rebuilt on the first
        # query of a new round, so nothing can go stale across rounds. ---
        self.hg_rnd = -1
        self.hg_turrets = []
        self.hg_seen = {}

        # --- LOKI-HEALBREAK, per RAIDER.  hb_bench is the latched annulus tile
        # (an (x, y) pair, not a Position, so nothing is allocated when it is
        # only being compared); hb_bench_rnd is when the latch was last
        # refreshed by a refusal; hb_band_until is the round the in-band park
        # expires.  The counters are the local instrument: refusals seen,
        # benches latched, moves redirected, arrivals in band, and the
        # inherited step-3b refusal histogram that answers "is the redirect
        # blocked by the same bank as everything else". ---
        self.hb_bench = None
        self.hb_phase = 0               # 0 idle / 1 walking to bench / 2 parked
        self.hb_until = -10 ** 9        # the live latch's expiry round
        self.hb_cool_until = -10 ** 9   # re-benching banned before this round
        self.hb_plants_at = 0           # bb_plants when the latch was taken
        self.hb_refused = 0
        self.hb_latched = 0
        self.hb_moves = 0
        self.hb_arrived = 0
