...
class RaidMixin:
    ...
    def _raid(self, ct):
        ...
        # ⭐ LOKI-HEALBREAK: THE BENCH OVERRIDES THE RING STATION.  This is the
        # entire cost of the redirect -- one move target, no action, no
        # titanium.  It is placed AFTER `_raid_station` so the parent's station
        # bookkeeping (bans, rescans, slot rotation) runs exactly as it does in
        # `_v480beltbreak`; only the destination changes, and only while a latch
        # is live.  In phase 2 the bench IS this raider's own tile, so the
        # parent's own `p == self.tgt` branch below turns it into a park with no
        # extra code -- and step 3b's plant scan then runs from inside the
        # annulus every round until the dwell expires.
        if HB_REDIRECT_ON and self.hb_phase:
            if rnd > self.hb_until or self.bb_plants > self.hb_plants_at:
                # Latch expired, or this raider actually planted.  Hand the body
                # back to the collar and ban re-benching for HB_COOL rounds --
                # the duty-cycle bound that keeps a refused raider from being
                # removed from the seal permanently.
                self.hb_phase = 0
                self.hb_bench = None
                self.hb_cool_until = rnd + HB_COOL
            elif self.hb_bench is not None:
                self.tgt = Position(self.hb_bench[0], self.hb_bench[1])
                self.hb_moves += 1
                if HB_LOG:
                    import sys
                    print("HB495 GO r%d phase%d (%d,%d)->(%d,%d)%s"
                          % (rnd, self.hb_phase, p.x, p.y, self.hb_bench[0],
                             self.hb_bench[1],
                             " PARKED" if (p.x, p.y) == self.hb_bench else ""),
                          file=sys.stderr)
        ...
        if stalled:
            # LOKI-HEALBREAK: A STALL WHILE WALKING TO A BENCH IS THE BENCH'S
            # FAULT, NOT THE STATION'S.  Drop the latch and return BEFORE the
            # parent's ban logic, or an unreachable bench would ban a perfectly
            # good ring station and rotate the far-phase slot -- the redirect
            # silently corrupting the parent's own station bookkeeping.  The
            # cooldown then keeps it from immediately re-latching the same tile.
            if HB_REDIRECT_ON and self.hb_phase:
                self.hb_phase = 0
                self.hb_bench = None
                self.hb_cool_until = rnd + HB_COOL
    ...
    def _raid_act(self, ct, E, near):
        ...
        #
        # ⚠ LOKI-HEALGATE, AND THIS IS THE ONE SITE WHERE THE GATE IS ARGUABLE.
        # The pecker pricing above is right about peckers and wrong about
        # turrets (7/round from a gunner, 9/round from a sentinel, against
        # +4/round of repair -- see the LOKI-HEALGATE block in doctrine.py).
        # But the LOS clause is a real counterargument HERE and nowhere else: a
        # gunner's ray IS blocked by a barrier, so keeping the barrier alive may
        # be buying COVER rather than HP.  Against it: a SENTINEL pierces
        # obstacles, so the cover is worth zero against the 18-damage threat;
        # BARRIER_MAX_HP is 30, four rounds of gunner fire healed or not; and
        # this block never checks that the barrier is actually between the
        # raider and any shooter -- it heals any adjacent damaged friendly
        # building.  Unresolvable from the code, so it is switched separately
        # (HG_GATE_COLLAR) instead of being silently included or excluded.
        #
        # ⭐ LOKI-HEALBREAK HANGS OFF THIS REFUSAL AND NOWHERE ELSE.  This is
        # the only gated site whose subject is a RAIDER, i.e. the only body that
        # is both (a) measured to idle when refused and (b) within walking
        # distance of the beltbreak annulus.  The redirect takes no action and
        # spends no titanium -- it latches a MOVE target and lets the parent's
        # own move phase walk there.
        if near:
            for dx, dy in CARD_DELTAS:
                tx, ty = p.x + dx, p.y + dy
                if not (0 <= tx < self.mw and 0 <= ty < self.mh):
                ...
                try:
                    ...
                    if (
                        ...
                        if HG_GATE_COLLAR and self._hg_refuse(ct, t, "collar"):
                            if HB_REDIRECT_ON:
                                self._hb_redirect(ct, E, p)
                            continue
                        ct.heal(t)
                        return True
                except Exception:
    ...
    def _try_beltbreak_gunner(self, ct, E):
        ...
        rnd = ct.get_current_round()
        # LOKI-HEALBREAK.  Stamp "step 3b RAN this round" and clear last
        # round's verdict, so the collar redirect can distinguish three cases
        # it must not confuse: a refusal this round (bb_last set), step 3b
        # passing every refusal and failing later (bb_last None, stamp
        # current), and step 3b never getting here at all because the silent
        # band pre-scan above bailed (stamp stale).  Two attribute writes; no
        # behaviour.
        if HEALBREAK_ON:
            self.bb_last = None
            self.bb_last_rnd = rnd
    ...
    def _bb_refuse(self, why, a=0, b=0):
        ...
        # LOKI-HEALBREAK: stamp the reason with the round, so the collar
        # redirect further down can ask "why did step 3b say no THIS round"
        # without re-running step 3b -- which is a provable no-op, because
        # nothing between step 3b and step 5 mutates any state it reads.
        # Guarded on the master flag so the flag-off tree executes no new code
        # at all; the write is a pure attribute set with no behavioural effect
        # either way, which the flag-off replay-hash check confirms.
        if HEALBREAK_ON:
            self.bb_last = why
        if LOKI_BELTBREAK_LOG and n % 25 == 1:
    ...
    # --- LOKI-HEALBREAK: the redirect --------------------------------------

    def _hb_redirect(self, ct, E, p):
        """A gate-refused collar heal, turned into a MOVE toward the annulus.

        Takes NO action, spends NO titanium and returns nothing.  It latches
        `self.hb_bench` and lets the parent's own move phase do the walking.
        That is the design, not a shortcut: the refused raider was measured
        PARKED (`act_move` fell 421 in `_v492healgate`, because a raider
        standing on its own station has `p == self.tgt` and returns without
        moving), so the resource this plank is trying to spend is a MOVE.  An
        ACTION taken here would come out of the collar's own budget, which is
        the budget the gate was trying to protect.

        THE ORDERING, AND THE FIRST RUNG IS A NO-OP ON PURPOSE:

          1. PLANT -- READ, NOT RETRIED.  `_raid_act` ranks the beltbreak plant
             at step 3b and the collar heal at step 5, so step 3b ALREADY RAN
             this round, and nothing between them mutates any state it reads
             (both intervening branches return True when they act).  A re-call
             would recompute the same refusal from the same state.  So the
             reason is INHERITED from `self.bb_last` and counted, which answers
             the question the composition actually has to answer -- is the
             redirect blocked by the same bank as everything else -- instead of
             simulating an answer.

          2. BENCH -- the only free lever left.  A raider on the collar sits at
             dsq_core <= 2, its build tiles at <= ~8, and the annulus starts at
             LOKI_BELTBREAK_DSQ_LO = 20: the plant is refused there on GEOMETRY
             before the bank is ever consulted, at any bank.  Standing in the
             annulus instead is worth exactly one move.

          3. FALL THROUGH -- nothing latched, and the parent's steps 6-7 and its
             own station choice run as in `_v480beltbreak`.  The composition can
             never REMOVE a parent behaviour.

        Bounded on purpose (see the seal cost in the doctrine block): phase 1 is
        a walk latch expiring HB_BENCH_HOLD rounds after the last refusal,
        phase 2 is a single in-band dwell of HB_BAND_HOLD rounds that is NOT
        refreshed, and a completed cycle bans re-benching for HB_COOL rounds.
        """
        rnd = ct.get_current_round()
        self.hb_refused += 1

        # 1. WHY step 3b said no, THIS round.  Three cases that must not be
        # confused: a counted refusal (bb_last set with a current stamp); step
        # 3b passing every refusal and then failing at the build or the CPU
        # guard (stamp current, bb_last None); and step 3b never reaching its
        # own counters because the silent band pre-scan bailed (stale stamp).
        if self.bb_last_rnd != rnd:
            why = "FARBAND"
        elif self.bb_last is None:
            why = "PASSED"
        else:
            why = self.bb_last
        self.hb_why[why] = self.hb_why.get(why, 0) + 1
        if HB_LOG:
            import sys
            print("HB495 WHY r%d %s phase%d" % (rnd, why, self.hb_phase),
                  file=sys.stderr)
        if why in HB_DEAD_REASONS:
            return
        ...
        try:
            ...
            return
        in_band = LOKI_BELTBREAK_DSQ_LO <= db <= LOKI_BELTBREAK_DSQ_HI

        if self.hb_phase == 2:
            # Already parked in band.  The dwell is deliberately NOT refreshed
            # by further refusals -- a bench that renewed itself would be a
            # raider removed from the collar for the match.
            return
        if self.hb_phase == 1:
            if in_band:
                self._hb_park(rnd, p)
            else:
                self.hb_until = rnd + HB_BENCH_HOLD     # refresh the walk
            return

        # phase 0 -- idle.
        if rnd < self.hb_cool_until:
            self.hb_why["COOL"] = self.hb_why.get("COOL", 0) + 1
            if HB_LOG:
                import sys
                print("HB495 COOL r%d until=%d" % (rnd, self.hb_cool_until),
                      file=sys.stderr)
            return
        if in_band:
            self._hb_park(rnd, p)
            return
        seat = self._hb_bench_seat(ct, E, p)
        if seat is None:
            self.hb_why["NOSEAT"] = self.hb_why.get("NOSEAT", 0) + 1
            if HB_LOG:
                import sys
                print("HB495 NOSEAT r%d at=(%d,%d) d2=%d" % (rnd, p.x, p.y, db),
                      file=sys.stderr)
            return
        self.hb_phase = 1
        self.hb_until = rnd + HB_BENCH_HOLD
        self.hb_bench = seat
        self.hb_plants_at = self.bb_plants
        self.hb_latched += 1
        if HB_LOG:
            import sys
            print("HB495 BENCH r%d from=(%d,%d) d2=%d -> (%d,%d) why=%s"
                  % (rnd, p.x, p.y, db, seat[0], seat[1], why), file=sys.stderr)

    def _hb_park(self, rnd, p):
        """Enter the in-band dwell: the bench becomes this raider's own tile.

        `_raid_turn`'s override then sets `self.tgt` to the tile the raider is
        standing on, which the parent's own `p == self.tgt` branch turns into a
        park -- so the plant scan at step 3b runs from here every round with no
        further code.  One dwell per cycle; it expires and hands the body back.
        """
        self.hb_phase = 2
        self.hb_until = rnd + HB_BAND_HOLD
        self.hb_bench = (p.x, p.y)
        self.hb_plants_at = self.bb_plants
        self.hb_arrived += 1
        if HB_LOG:
            import sys
            print("HB495 PARK r%d (%d,%d)" % (rnd, p.x, p.y), file=sys.stderr)

    def _hb_bench_seat(self, ct, E, p):
        """Nearest inner-annulus tile, found by stepping RADIALLY OUTWARD.

        O(HB_BENCH_STEPS) tile reads, no map scan and no BFS -- this runs on a
        hot path in a tree that already does whole-map BFS elsewhere, so it may
        not add a second one.  `E.direction_to(p)` is the outward direction (the
        only one that raises dsq_core), and the walk keeps the first tile in
        [LO, LO + HB_BENCH_PAD]: the d^2 20-34 shell, which brackets the d^2
        20-30 band where the study measures excess belt-kills per gunner to
        PEAK.  It is a MOVE target, not a build tile -- step 3b does the real
        siting on arrival, with its own live-target gate.

        Returns an (x, y) pair or None.  None is a real outcome and is counted:
        a raider boxed in by walls has no bench, and the honest answer is to
        fall through to the parent rather than invent an unreachable target.
        """
        try:
            d = E.direction_to(p)
        except Exception:
        ...
        if d == Direction.CENTRE:
            return None
        ddx, ddy = DELTA[d]
        lo = LOKI_BELTBREAK_DSQ_LO
        hi = lo + HB_BENCH_PAD
        x, y = p.x, p.y
        for _ in range(HB_BENCH_STEPS):
            x += ddx
            y += ddy
            if not (0 <= x < self.mw and 0 <= y < self.mh):
                return None                     # cannot go further outward
            t = Position(x, y)
            try:
                db = dsq_core(t, E)
            except Exception:
            ...
            if db < lo or db > hi:
                continue
            try:
                # is_in_vision does NOT raise off-map (it returns False), so it
                # is the safe pre-check for the passability query, which does.
                # Outside vision the tile is accepted unchecked: a move TARGET
                # need not be passable for `_nav` to approach it, and refusing
                # every unseen tile would empty the bench on exactly the maps
                # where the walk is longest.
                if ct.is_in_vision(t) and not ct.is_tile_passable(t):
                    continue
            except Exception:
                continue
            return (x, y)
