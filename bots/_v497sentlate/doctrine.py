...
# ===========================================================================
# LOKI-SENTLATE (s48) -- THE FORWARD SENTINEL IS DEFERRED, NOT REMOVED.
#
# ONE FLAG, `LOKI_SENTLATE_ON`.  False == `_v480beltbreak` exactly: the guard is
# `LOKI_SENTLATE_ON and ...`, so with the flag off the `and` short-circuits and
# not one Controller call is made on that path.
#
# WHAT IT DOES: `_try_forward_sentinel` refuses every plant before
# `LOKI_SENTLATE_RND`.  Nothing else changes -- no cap moves, no floor moves, no
# BELTBREAK constant is retuned, and after the gate the sentinel path is the
# parent's, byte for byte.
#
# THE PROBLEM IT ADDRESSES.  `raid.py:_raid_act` step 3 calls
# `_try_forward_sentinel` and RETURNS on success, so step 3b (BELTBREAK) never
# runs for that builder that round; and both paths draw the same global bank.
# So the two arms compete on two channels, an ACTION channel and a BANK channel.
# The bank channel is the bigger one: `_try_forward_sentinel` needs
# `sentinel_cost + LOKI_FWD_TI_FLOOR` (= 40) in hand, while BELTBREAK's
# first plant waives its reserve entirely and needs only `gunner_cost`.
#
# ⛔ WHY A DEFERRAL AND NOT A REMOVAL, and the reason is that the adverse
# evidence is STRATIFIED BY ROUND, not flat.  Rated read vs kladde v119, n=83:
# our r0-60 forward sentinels are 98.8% dead, median life 8 rounds, 5.19 shots
# fired, against a field control of 59.5% / 45 rounds / 22.25 shots.  It is the
# EARLY sentinel that is bad.  A full stand-down would also discard the late one
# -- and the late one is the only ranged channel we own that can damage a Core
# THROUGH our own barrier collar, because a Sentinel line ignores obstacles
# while a Gunner ray does not.  BELTBREAK cannot replace it: its own study text
# records the share of in-band gunner shots at the enemy core as 0.000, a
# gunner reaching only d^2 <= 13 from a band that starts at 20.
#
# ⛔ AND THE SCALE ARGUMENT FOR A REMOVAL DOES NOT SURVIVE ARITHMETIC.  The cost
# scale is ONE GLOBAL ADDITIVE factor and a GUNNER and a SENTINEL contribute the
# SAME +20% to it.  So a 1-for-1 sentinel->gunner substitution saves ZERO scale;
# the only saving is the titanium differential (30 vs 20 base, i.e. 10 x scale)
# plus the reserve-floor differential.  Any claim that "not buying the sentinel
# makes everything later cheaper" is only true where the sentinel is not
# replaced by another +20% building, and inside the deferral window that is
# exactly what a shredder is.  Stated here so nobody re-derives it as a win.
#
# THE THRESHOLD IS THE EVIDENCE WINDOW, NOT A GUESS.  r60 is the upper edge of
# the n=83 rated stratum above; a harvester count or a bank floor would not map
# onto it.  Note what the window contains: BELTBREAK's own gate opens at
# `LOKI_BELTBREAK_RND` = 25, so r25-60 is the interval where the two arms really
# bid against each other, and r0-25 is an interval where the sentinel has no
# competitor for the ACTION but still removes titanium from the stock BELTBREAK
# will draw on at r25.  Both are inside the gate on purpose.
#
# ⛔⛔ THE HAZARD THIS ARM CARRIES, RECORDED BEFORE MEASURING IT: x3r0's `TW`
# behaviour gates on the opponent having NEVER SEEN A TURRET.  Deferring our
# earliest turret can push `round of our first turret of any kind` LATER, and
# the home counter-battery is no backstop -- `main.py:_try_counterbattery` is
# reactive, gated on `SLOT_THREAT` landing inside `HUNT_BAND_DSQ` of our OWN
# core.  If the first-turret round moves later, this arm is disqualified on that
# alone, whatever the outcome column says.
LOKI_SENTLATE_ON = True     # master switch; False == _v480beltbreak exactly
LOKI_SENTLATE_RND = 60      # forward sentinel plants are refused before this round
