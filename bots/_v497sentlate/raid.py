...
class RaidMixin:
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if not LOKI_FWD_SENTINEL_ON:
            return False
        # LOKI-SENTLATE (s48) -- THE WHOLE TREATMENT, one line.  The forward
        # sentinel is DEFERRED past LOKI_SENTLATE_RND so that BELTBREAK owns
        # both the builder's action and the bank through the window where the
        # rated evidence says the early sentinel dies without firing (n=83:
        # 98.8% dead, median life 8, 5.19 shots).  Placed FIRST, before the
        # census and the store reads, so with the flag off the `and`
        # short-circuits and this path costs nothing -- not one Controller call,
        # not one changed decision.  See doctrine.py, LOKI-SENTLATE block, for
        # why this is a deferral and not the removal it could have been, and for
        # the TW first-turret hazard it carries.
        if LOKI_SENTLATE_ON and ct.get_current_round() < LOKI_SENTLATE_RND:
            return False
