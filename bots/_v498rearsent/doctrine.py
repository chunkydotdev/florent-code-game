...
# ============================================================================
# LOKI-REARSENT (v498) -- A SENTINEL BEHIND THE ENEMY CORE, INSTEAD OF A SEAL.
#
# Magnus, 2026-08-17, verbatim: "what if we place one [sentinel] behind enemy
# core? Instead of a barrier?"
#
# THE MECHANISM, and it is ENGINE-MEASURED, not read off the organisers' doc
# (probe _probe_rearlos, 2026-08-17, nordkap seed 900001, both cores):
#   get_attackable_tiles_from(A, EAST, GUNNER)   -> 3 tiles, dx = 1..3
#   get_attackable_tiles_from(A, EAST, SENTINEL) -> 5 tiles, dx = 1..5
#   with the 2x2 Core occupying dx = 1 and dx = 2 on that row:
#       can_fire_from(A, EAST, GUNNER,   dx=1 core tile) -> True
#       can_fire_from(A, EAST, GUNNER,   dx=2 core tile) -> False
# The far core tile is IN the gunner's pattern and still refused, so a Core
# body STOPS a gunner ray -- a gunner on the near side cannot reach anything
# the footprint hides.  A rear tile is therefore cover against their cheapest
# and most numerous turret.
#
# ⛔ WHAT THE PROBE DID **NOT** SHOW, and the plank does not depend on it:
# can_fire_from(..., SENTINEL, ...) returned True for EVERY in-pattern target
# including EMPTY tiles, so for a Sentinel that predicate certifies RANGE AND
# ALIGNMENT ONLY -- it is blind to occupancy and cannot be used as evidence
# that a Sentinel ray is unblocked.  The Sentinel half of the asymmetry is the
# organisers' doc (official-docs.md:257, PROSE) and is untested here.  The
# plank never needs it: its target is a Core tile ADJACENT to the plant, so
# nothing is interposed on our own ray.
#
# WHAT IT CHANGES, and it is one ordering.  _raid_act ranks (1) peck,
# (2) SEAL A FREE SEAT with a barrier, (3) forward Sentinel.  Step 3 already
# scans all four cardinal neighbours, rear ones included, so the control CAN
# plant behind the Core -- but step 2 pre-empts it whenever a free seat is
# adjacent.  This inserts the REAR plant between them: on a rear tile with a
# legal Core-bearing facing, the Sentinel outbids the barrier.
#
# ⛔ NOT A LIKE-FOR-LIKE SWAP, AND THE ARM MUST NOT BE PRESENTED AS ONE.
# Barrier 3 Ti / +1% scale; Sentinel 30 Ti / +20% on the ONE GLOBAL ADDITIVE
# factor, which inflates every later build of every type.  Plus 10 ammo a
# shot, from titanium the Core converts 1:1.  ~10x the price.
#
# SAME BUDGET, ON PURPOSE.  The rear plant counts against LOKI_FWD_GUN_CAP and
# writes SLOT_FWD_GUN exactly as the forward plant does, so the arm cannot win
# by buying MORE turrets -- it changes WHERE and WHEN, not how many.  The
# SLOT_FWD_GUN write also keeps the Core's magazine floor
# (min(120, 40 + 20*fwd_guns), main.py) funding the thing.
#
# WHAT IS GIVEN UP: raid.py's collar comment says the barriers are also the
# raider's own LOS cover against gunner rays.  Every round the rear plant wins
# is a round no seat was sealed.  The paired opportunity-cost read (barriers
# planted, raider survival) is the falsifier for that half.
# ============================================================================

# ⭐⭐ MEASURED 2026-08-17, 480 LOCAL SELF-PLAY GAMES (240 per arm; live 15-map
# pool x 2 seats x 8 seeds; --tle 10; opponent = a third copy of the parent).
# ⛔ THE FIXTURE IS OUR OWN BOT, so per CLAUDE.md point 6 this can PRIORITISE and
# cannot RETIRE anything.  Read the within-fixture control, not the live study.
#
# 1. THE ASYMMETRY IS REAL ON THE ENGINE.  can_fire_from table over 560 in-vision
#    non-WALL 3-step rays: (target occupied, path clear) -> GUNNER True n=46;
#    (target occupied, a body between) -> GUNNER False n=145; SENTINEL True in
#    560/560.  Zero contradictions either way.  And the damage probe settles the
#    Sentinel half that the predicate cannot: 78 real Sentinel shots fired through
#    an occupied tile -> the INTERPOSED body lost HP 0 times, the target behind it
#    lost exactly 18 in 65 and died in 7.  A Core body blocks a Gunner (11 cells,
#    all False); a Sentinel ray ignores it.
#
# 2. REAR TILES DO SURVIVE LONGER.  Rear plants n=96 vs same-arm approach-side
#    n=317: EVER HIT 21.9% vs 33.8%, median rounds to first incoming hit +29 vs
#    +4, DIED 20.8% vs 30.0%, median life of those that died 26 vs 9.
#
# 3. ⛔ AND THE MECHANISM'S OWN PREDICTION IS **REFUTED**.  The prediction was
#    that Gunner kills of a rear Sentinel go to ~zero because their own Core
#    blocks the ray.  Observed on the 93 damage events rear Sentinels took:
#    7-damage (a Gunner) 32, 3-damage (a Gunner minus our +4 heal) 40,
#    14 (two Gunners) 5, 18 (a Sentinel) 16.  Of 20 rear deaths, 8-10 carry a
#    Gunner signature.  WHY: a defender's Gunners are not confined to the
#    approach side -- one planted behind their own Core has no Core between it
#    and us.  The survival gain in (2) is real but it is NOT the shielding
#    mechanism; the far side is simply less crowded.
#
# 4. IT DOES NOT PAY AT GAME LEVEL.  Paired over 240 cells: timely kill by r300
#    (ITT, all games) 0.300 vs 0.308 (-0.8pp, 95% CI -8.9..+7.2), game win 0.504
#    vs 0.554 (-5.0pp, CI -13.2..+3.2), our builder deaths +0.36/game.  Nothing
#    significant, every point estimate slightly AGAINST the arm, and the interval
#    does NOT exclude a regression -- so this is UNDERPOWERED, not a clean null.
#    Dose is the reason: 0.40 rear plants/game in 34% of games.
#
# 5. THE AMMO BILL IS NOT FUNDED.  Over 8,278 rear-Sentinel unit-rounds the team
#    magazine was BELOW one shot (10) in 40.0% and empty in 5.8%; median 18, i.e.
#    under two shots banked.  Sustained rear fire is ammo-starved by design of the
#    parent's magazine floor, which was written for one forward turret.
LOKI_REARSENT_ON = False        # the whole plank; False == the parent exactly
# Half-plane margin, in HALF-TILES, past the Core centre along the axis from
# our Core to theirs.  The Core centre is (E.x+0.5, E.y+0.5), so the test is on
# doubled coordinates and 1 == "strictly past the centre"; 3 == past the far
# edge of the footprint on that axis.  Same mechanism as the flag (it IS the
# definition of "behind"), which is why it is the only companion constant.
LOKI_REARSENT_MIN_DOT = 3
# Bank floor left AFTER paying for the rear Sentinel.  ⛔ THE ARM'S SECOND
# CONSTANT, AND THE HONEST REASON IT EXISTS: with the parent's LOKI_FWD_TI_FLOOR
# (= 40) the plank is INERT.  Measured 2026-08-17, 16 self-play games (4 pool
# maps x 2 seeds x 2 seats, noise off): flag ON produced replay hashes IDENTICAL
# to the control in 16 of 16, and the per-round rejection census on one game read
# 296 x bank-floor, 9 x harvester-prerequisite, 2 x no-legal-rear-tile, 0 plants.
# A raider is rarely standing behind their Core on the same round the bank holds
# cost + 40, so a pure re-ordering never fires.  Lowering the floor is therefore
# not a tuning knob bolted on -- it IS what makes "spend it on the rear Sentinel
# instead of the seal" happen at all.  It is the SAME mechanism (how much we will
# pay for the rear plant) and it is why the arm is NOT a pure re-ordering: it
# also spends down further than the parent would.  Report it as such.
LOKI_REARSENT_TI_FLOOR = 8
# Instrumentation only, stderr, LOCAL runs only (platform replays strip stdout,
# and stderr never reaches them at all).  Dose tag "RS498".
LOKI_REARSENT_LOG = False
