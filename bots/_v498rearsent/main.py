...
class Player(EcoMixin, RaidMixin):
    ...
    def _turret(self, ct):
        ...
        # LOKI-REARSENT instrumentation, LOCAL stderr only, default OFF.  Logs
        # EVERY friendly Sentinel's per-round HP so the offline reader can
        # classify rear vs approach-side itself from the two Core anchors -- an
        # in-bot classification would be OUR claim about the tile, and the HP
        # series is the engine's.  Rounds-to-first-hit, lifespan and the killer
        # signature (gunner 7 / sentinel 18 / builder melee 2 per hit) all come
        # out of this series.
        if LOKI_REARSENT_LOG and turret_type == EntityType.SENTINEL:
            self._rs_tick(ct, p)
    ...
    # --- LOKI-REARSENT instrumentation (LOG flag only; no behaviour) --------

    def _rs_tick(self, ct, p):
        import sys
        try:
            E = unpack_pos(ct.read_store(SLOT_ENEMY_CORE))
            print("RS498 TICK r%d id=%d pos=%d,%d hp=%d ammo=%d ec=%s"
                  % (ct.get_current_round(), ct.get_id(), p.x, p.y,
                     ct.get_hp(), ct.get_global_ammo(),
                     "%d,%d" % (E.x, E.y) if E is not None else "?"),
                  file=sys.stderr)
        except Exception:
            pass

    def _rs_fire(self, ct, p, target):
        """One line per Sentinel shot.  The target detail is read AFTER the
        shot and is therefore MISSING when the shot killed the target -- so the
        detail is a separate try and the SHOT line always prints, or the count
        would be biased downwards by exactly the killing blows."""
        import sys
        detail = "kind=? thp=?"
        try:
            bid = ct.get_tile_building_id(target)
            bot = ct.get_tile_builder_bot_id(target)
            tid = bid if bid is not None else bot
            if tid is None:
                detail = "kind=GONE thp=-"
            else:
                et = ct.get_entity_type(tid)
                detail = "kind=%s thp=%d" % (et.name, ct.get_hp(tid))
        except Exception:
            pass
        try:
            print("RS498 FIRE r%d id=%d pos=%d,%d tgt=%d,%d %s"
                  % (ct.get_current_round(), ct.get_id(), p.x, p.y,
                     target.x, target.y, detail),
                  file=sys.stderr)
        except Exception:
            pass
