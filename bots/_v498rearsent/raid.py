...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 1b. LOKI-REARSENT: A SENTINEL BEHIND THEIR CORE INSTEAD OF THE SEAL.
        # Ranked ABOVE the barrier because that ordering IS the plank -- step 3
        # can already reach a rear tile and step 2 pre-empts it.  Doctrine block
        # LOKI-REARSENT carries the engine probe and the price.
        if LOKI_REARSENT_ON and self._try_rear_sentinel(ct, E):
            return True
    ...
    def _try_rear_sentinel(self, ct, E):
        """Plant a Sentinel BEHIND their Core -- the far side from ours.

        Every gate is `_try_forward_sentinel`'s, unchanged and deliberately so:
        the same cap (LOKI_FWD_GUN_CAP), the same store slot (SLOT_FWD_GUN, which
        is also what funds the Core's magazine floor), the same harvester
        prerequisite and bank floor.  The arm may not buy MORE turrets than the
        parent; it changes WHICH TILE and WHICH ROUND.

        "BEHIND" IS COMPUTED, NOT GUESSED.  s = the sign vector of (their anchor
        - our anchor); the Core centre in doubled coordinates is
        (2E.x+1, 2E.y+1); a candidate tile is rear when its doubled offset from
        that centre, projected on s, is at least LOKI_REARSENT_MIN_DOT.  At the
        default 3 that is "past the far edge of the 2x2 footprint" on an
        axis-aligned pair, and the rear QUADRANT on a diagonal pair (where both
        components of s are non-zero, so the two axes' contributions add).

        The target is a Core tile ADJACENT-OR-NEAR the plant, so this never
        relies on a Sentinel ray piercing anything: the engine probe in the
        doctrine block only established the GUNNER half of the asymmetry.
        `can_fire_from` is still the alignment check, because facing is fixed at
        build time and Sentinels cannot rotate.
        """
        ...
        rush = LOKI2_RUSH_ON and ct.get_current_round() < LOKI2_RUSH_RND
        min_harv = LOKI2_RUSH_MIN_HARV if rush else LOKI_FWD_MIN_HARV
        ti_floor = LOKI2_RUSH_TI_FLOOR if rush else LOKI_REARSENT_TI_FLOOR
        if ct.read_store(SLOT_HARVESTERS) < min_harv:
            return False
        cost = ct.get_sentinel_cost()
        if ct.get_global_resources() < cost + ti_floor:
            return False
        p = ct.get_position()
        # A Sentinel reaches r^2 = 32 and a build site is one step from here.
        if dsq_core(p, E) > 50:
            return False
        if self._cpu_exhausted(ct):
            return False
        ...
        for dx, dy in CARD_DELTAS:
            ...
            if not (0 <= bx < self.mw and 0 <= by < self.mh):
                continue
            if (2 * bx - cx2) * sx + (2 * by - cy2) * sy < LOKI_REARSENT_MIN_DOT:
                continue
            bp = Position(bx, by)
            # Nearest Core tile first: the shorter the ray the less of it can
            # ever be argued about, and a Sentinel out-ranges all four anyway.
            cands = sorted(
                ((bp.distance_squared(t), i, t) for i, t in enumerate(tiles)),
            )
            for d, _i, target in cands:
                if d > 32:
                    continue
                facing = bp.direction_to(target)
                if facing == Direction.CENTRE:
                ...
                if LOKI2B_LIVE_CAP_ON:
                    ...
                    ct.write_store(SLOT_FWD_GUN, ct.read_store(SLOT_FWD_GUN) + 1)
                if LOKI_REARSENT_LOG:
                    import sys
                    print("RS498 PLANT r%d id=%d bp=%d,%d face=%s tgt=%d,%d dsq=%d"
                          % (ct.get_current_round(), ct.get_id(), bx, by,
                             facing.name, target.x, target.y, d),
                          file=sys.stderr)
                return True
        return False
