...
# =====================================================================
# ECORESERVE (v499) -- THE OFFENSIVE MIRROR OF SIEGE_HEAL_RESERVE_TI.
# =====================================================================
# THE ASYMMETRY THIS FIXES.  `_eco_spendable` (eco.py) is the ONLY reserve in
# this tree and it is DEFENSIVE on both axes: it fires only when SLOT_UNDER != 0
# (we are being shelled) AND only from HUNT_MIN_RND (= 120).  Before r120 there
# is no reserve of any kind, and 16 Ti is not a reserve anyway.  Meanwhile the
# spender that wants that money -- `_try_forward_sentinel` (raid.py) -- needs
# `get_sentinel_cost() + LOKI_FWD_TI_FLOOR` and opens as soon as
# SLOT_HARVESTERS >= LOKI_FWD_MIN_HARV, i.e. INSIDE the unreserved window.
# Eco spends the bank down to a 3 Ti conveyor's price before the siege gets a
# look, and (cost scale being ONE GLOBAL ADDITIVE factor) each of those
# conveyors permanently raises the price of the Sentinel it outbid by +1%.
#
# DESIGN (c) -- DEMAND-INFERRED, AND IT IS NOT A HYPOTHETICAL.  Every
# precondition of the forward-Sentinel gate is ALREADY published in the comms
# store by raid.py: SLOT_HARVESTERS, SLOT_FWD_GUN, and the SLOT_RAID_LIVE
# foothold heartbeat.  So eco re-evaluates the raid's own gate locally, O(1),
# with NO new slot and NO added latency.
#   (b) demand-signalling is IMPOSSIBLE here without clobbering: all 16 slots
#       (GameConstants.STORE_SIZE) are allocated in this tree, and the two that
#       could host a packed flag (SLOT_FWD_GUN, SLOT_RAID_LIVE) are written as
#       `read + 1`, which bit-packing corrupts.  It also costs a buffered round.
#   (a) a round-windowed flat reserve taxes eco unconditionally in exactly the
#       window where harvesters #1 and #2 are built -- the window we are
#       already measured 2-4 rounds late in -- so it is the one design most
#       likely to DELAY the offence it is trying to fund.
#
# ⛔ HARVESTERS ARE NOT CONVEYORS, AND THE SEPARATION IS STRUCTURAL, NOT TUNED.
# `_try_forward_sentinel` is GATED on SLOT_HARVESTERS >= LOKI_FWD_MIN_HARV, so a
# reserve that starves harvesters delays the turret it is funding.  Two
# independent mechanisms keep that from happening:
#   1. The reserve is applied at the three CONVEYOR call sites ONLY.  The
#      harvester site (eco.py `_expand`) and the launcher site (main.py
#      `_try_build_launcher`) keep calling bare `_eco_spendable`.
#   2. The reserve is INERT below OFF_RESERVE_MIN_HARV, which is
#      LOKI_FWD_MIN_HARV.  It cannot exist before harvester #2 is standing, so
#      even the conveyor tax starts strictly AFTER the economy the offence
#      waits for.
OFF_RESERVE_ON = False       # MASTER FLAG.  False => `_off_reserve` returns 0
                             # at its first line and every belt call site is
                             # behaviourally identical to the control.
OFF_RESERVE_MIN_HARV = 2     # == LOKI_FWD_MIN_HARV.  Below this, no reserve.
OFF_RESERVE_MAX_RND = 400    # past this the siege is decided; stop taxing eco.
OFF_RESERVE_NEED_FOOTHOLD = True   # only reserve while a raider is actually at
                             # the enemy ring (SLOT_RAID_LIVE heartbeat, same
                             # LOKI_FOOTHOLD_STALE credibility window raid.py
                             # `_foothold_live` uses).  Without this the reserve
                             # holds cash against a raider that does not exist.
OFF_RESERVE_LOG = False      # dose channel, stderr, --tle 0 ONLY (hot path).
OFF_RESERVE_TI = 0           # 0 => reserve the LIVE price of what we are saving
                             # for, `get_sentinel_cost() + LOKI_FWD_TI_FLOOR`
                             # (a scaled getter, never a base cost).  A positive
                             # int overrides with a flat dose, for sweeps.
# ⚠ KNOWN CONSERVATISM, reported rather than hidden: SLOT_FWD_GUN is MONOTONE
# (doctrine "GHOST MAGAZINE" note) while the raid gate uses a LIVE count when
# LOKI2B_LIVE_CAP_ON.  So once 3 forward Sentinels have ever been built the
# reserve goes inert even though the raid may still build.  The reserve
# UNDER-fires rather than over-taxes; that is the direction we want to be wrong
# in, and it caps the economic downside of this plank.
