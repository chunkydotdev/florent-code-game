...
class EcoMixin:
    ...
    # --- ECORESERVE (v499): the OFFENSIVE mirror of the siege reserve -------

    def _off_reserve(self, ct):
        """Ti to hold back from BELT spends so the next forward Sentinel is payable.

        Design (c), demand-INFERRED from state raid.py already publishes; see the
        ECORESERVE block in doctrine.py for why (a) and (b) lose.  O(1): at most
        three `read_store`, one round read and one cost getter, no map scan, no
        allocation -- it sits on the same hot path as `_eco_spendable`.

        Returns 0 (never raises) whenever the offence does not currently want
        money, so the caller degrades exactly to the control's behaviour.
        """
        if not OFF_RESERVE_ON:
            return 0
        rnd = ct.get_current_round()
        if rnd > OFF_RESERVE_MAX_RND:
            return 0
        # 1. HARVESTER FIREWALL.  `_try_forward_sentinel` will not fire below
        #    this, so reserving below it could only delay harvester #2 -- the
        #    prerequisite of the very turret being saved for.
        if ct.read_store(SLOT_HARVESTERS) < OFF_RESERVE_MIN_HARV:
            return 0
        # 2. The magazine is full (monotone counter; see the conservatism note).
        if ct.read_store(SLOT_FWD_GUN) >= LOKI_FWD_GUN_CAP:
            return 0
        # 3. A raider is actually standing at the enemy ring.  Same heartbeat
        #    and same staleness window as raid.py `_foothold_live`.
        if OFF_RESERVE_NEED_FOOTHOLD:
            beat = ct.read_store(SLOT_RAID_LIVE)
            if not beat or rnd - (beat - 1) > LOKI_FOOTHOLD_STALE:
                return 0
        if OFF_RESERVE_TI:
            return OFF_RESERVE_TI
        # Scaled getter, never a base cost: by the time this fires, scale is
        # routinely 200%+ and a hardcoded 30 would under-reserve by half.
        return ct.get_sentinel_cost() + LOKI_FWD_TI_FLOOR

    def _eco_spendable_belt(self, ct, cost):
        """`_eco_spendable` for CONVEYOR spends only, plus the offensive reserve.

        Deliberately NOT used by the harvester path (`_expand`) or the launcher
        path (main.py `_try_build_launcher`) -- see the doctrine block.
        """
        r = self._off_reserve(ct)
        if not r:
            return self._eco_spendable(ct, cost)
        ok = self._eco_spendable(ct, cost + r)
        if OFF_RESERVE_LOG and not ok and self._eco_spendable(ct, cost):
            # A REFUSAL THE RESERVE CAUSED: affordable bare, refused with the
            # reserve.  stderr, not stdout -- stdout is captured into the replay
            # and platform-downloaded replays strip it anyway (CLAUDE.md s28).
            # ⛔ NEVER enable this under a WALL-CLOCK --tle: the print is on the
            # hot path.  Dose probes run --tle 0.
            import sys as _s
            print("ECORES BITE rnd=%d ti=%d cost=%d res=%d"
                  % (ct.get_current_round(), ct.get_global_resources(), cost, r),
                  file=_s.stderr)
        return ok
    ...
    def _build_next_link(self, ct):
        if not self.link_queue or not self._eco_spendable_belt(ct, ct.get_conveyor_cost()):
            return False
        p = ct.get_position()
    ...
    def _l4_repair(self, ct):
        ...
        if self._cpu_exhausted(ct):
            return False
        if not self._eco_spendable_belt(ct, ct.get_conveyor_cost()):
            return False
        p = ct.get_position()
        px, py = p.x, p.y
        ban = self._pave_ban()
        ...
        if PAVE_TRAIL_ON:
            ...
            if readable:
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 \
                        and self._eco_spendable_belt(ct, ct.get_conveyor_cost()):
