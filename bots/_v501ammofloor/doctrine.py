...
# ============================================================================
# LOKI-AMMOFLOOR (2026-08-17) -- THE MAGAZINE OUTBIDS THE TURRET.
#
# THE FINDING THIS PLANK RESTS ON (sibling arm, 638 games, self-validating
# titanium ledger closing to 0/+10 in 1,020/1,020 rows; relayed to this arm as
# a measurement, NOT re-derived here): ammunition conversion is the #1 titanium
# claimant in this bot -- 29.7% of r25-120 spend and 36.2% full-game, i.e.
# 2.3-3.0x more titanium spent on SHOTS than on the TURRETS that fire them.
#
# THE MECHANISM IS THE ONE LINE `ti_floor = 12 if (under or weapons_top) else 52`
# IN main.py's core ammunition block (verified by reading main.py:350 of the
# parent tree, _v468kladturbo):
#
#   * BEFORE the first weapon, `weapons_top` is 0 and `under` is normally False,
#     so the floor is 52 -- and E1_AMMO_FLOOR raises it no further (E1 asks for
#     min(harvester_cost, 23) + 23 = 46 <= 52).  A FORWARD SENTINEL, which is
#     the plank that actually kills cores, is refused by raid.py
#     `_try_forward_sentinel` (raid.py:679) unless the bank holds
#     `get_sentinel_cost() + LOKI_FWD_TI_FLOOR` -- ~121 Ti at the live scale
#     (~2.7).  So the drip is allowed to hold the bank at 52 while the thing we
#     are saving for costs 121: the FIRST turret is bought late, or on a
#     coincidence, because the magazine was served first.
#   * AFTER any weapon has ever existed the floor collapses to 12, and E1 lifts
#     it only back to 46.  That is why the bank reads pinned at ~40-46 Ti from
#     r25 on: 46 is E1's number, not an accident, and turrets 2 and 3 are priced
#     out of it permanently.  (`weapons` is MONOTONE -- doctrine's own T4 block
#     above documents that both gun slots are written only as `read + 1` -- so
#     rubble keeps the floor down even with nothing alive.)
#
# WHAT THIS PLANK DOES.  Raise the floor the ammunition drip may not spend
# through to the bank a TURRET PURCHASE needs, so the drip can only have the
# titanium the weapon does not want.  It changes NOTHING else: not the magazine
# target, not the endgame dump (which sets `endgame_dumped` upstream and
# switches the drip off entirely), not who plants what where.
#
# HOW THE RESERVE IS PRICED, and this is the part that is not a guess: each dose
# reserves EXACTLY the bank threshold that the corresponding plant path refuses
# below, read off that path's own source line.
#   D1  get_gunner_cost()                       <- main.py `_try_counterbattery`
#                                                  refuses at `ti < cost`, no
#                                                  margin (main.py:681)
#   D2  get_sentinel_cost() + LOKI_FWD_TI_FLOOR <- raid.py `_try_forward_sentinel`
#                                                  refuses at
#                                                  `ti < cost + ti_floor`
#                                                  (raid.py:679)
# and the dose ladder is monotone in reserve size at live scale (~54 -> ~121):
#   0/off  parent exactly.
#   1      D1 reserve, UNARMED branch only  (the pre-first-turret regime)
#   2      D2 reserve, UNARMED branch only  (the sibling's naive-probe value:
#          measured first-turret EARLIER, d = -1.13 +- 0.82, CI excludes 0,
#          0 never-built, economy intact; outcome unpowered)
#   3      D2 reserve, BOTH branches -- `under` and `weapons_top` included, so
#          the drip can never drain past a turret price at all.
#   4      D1 reserve, BOTH branches.        <- SHIPPED
#   5      get_sentinel_cost() with NO margin, BOTH branches -- the exact build
#          price, reserving the turret but not the raid's safety buffer.  Added
#          to LOCATE THE CROSSING between doses 4 (~54) and 3 (~121).
#
# ⛔ WHY THERE IS A FOURTH RUNG, and it was added from a MEASUREMENT rather than
# from taste.  The dose channel below was read on a single smoke game
# (nordkap, seat A, vs a deterministic opp_v63, dose 2, --tle 0) and the floor
# histogram came back 108 / 108 wanted-rounds at floor == 12, i.e. **not one of
# them in the unarmed branch**: the `under` latch (main.py, 50-round latch on
# SLOT_ATK_RND) closed at r9 and never reopened, and the first Sentinel went up
# at r11.  Doses 1 and 2, which are gated on the UNARMED branch, are therefore
# STRUCTURALLY INERT for almost the whole game -- so a ladder of 1/2/3 would have
# been a one-comparison sweep wearing three arms' clothing, and any difference
# between "reserve size" and "branch scope" would have been unidentifiable.
# Dose 4 completes the 2x2 (small/large reserve x unarmed-only/both-branches) and
# is the rung that makes the two factors separable.  The ladder is NOT monotone
# in reserve size across 3->4 on purpose; it is monotone within each branch
# scope (1<2 unarmed-only, 4<3 both-branches).
#
# ⚠ THE TENSION IS REAL AND IT IS WHY THIS IS SWEPT RATHER THAN SET.  Ammunition
# is not waste -- turrets fire from it, 4/shot gunner and 10/shot sentinel, with
# NO passive ammo income and convert_ammo the only source.  A floor set too high
# STARVES THE MAGAZINE, and dose 3 is the dangerous one on purpose: it overrides
# the 12 that E1 deliberately leaves alone under siege ("a gun that cannot fire
# during the assault does not get to spend the bank" -- E1's own note above), so
# it is the dose most likely to buy a turret we then cannot feed.  Doses 1 and 2
# leave the under-siege and armed floors bit-for-bit alone.
#
# COMPOSITION WITH E1_AMMO_FLOOR: max(), and it must be max().  Both clauses
# mean "leave at least this much in the bank", so the binding constraint is the
# larger, and taking the max is the only composition that cannot LOWER E1's
# harvester-rebuild reserve (the thing E1's 0/1190-conversion forensics
# validated).  This block sits BELOW E1's so it reads E1's value as its input;
# additive composition was rejected -- the two reserves are for the same
# titanium being spent by the same call, not two bills to pay.
# ============================================================================

# ============================================================================
# THE SWEEP, AND IT DOES NOT SAY WHAT THIS PLANK'S PREMISE SAID (s49, local,
# 2,160 games; NOISE_OFF paired on identical (map, seat, opponent) cells over
# the LIVE 15-map pool x 2 seats x 8 deterministic opponents = 240 cells/arm,
# --tle 0, plus a 960-game NOISE_ON unpaired spot-check).  Naive intervals:
# local balanced-by-construction fixtures read pair-weighted DEFF = 0.98
# (CLAUDE.md s39), so the platform constants are deliberately NOT applied.
#
# WHAT BINDS AT ALL.  Control floor histogram, per game, over rounds in which
# the drip WANTED a top-up (mean 107.8 such rounds): floor == 12 in 57.7,
# floor == 46 (E1) in 49.3, floor == 52 (the UNARMED branch) in **0.8**.
# ⇒ THE UNARMED BRANCH IS 0.8% OF THE DECISION.  Dose 1 changed 0 of 120 cells
# on EVERY metric -- exactly inert, because get_gunner_cost() never exceeds 52
# before the bot is armed.  Dose 2 (the sibling probe's value) reaches 1.7
# rounds/game and moves nothing downstream.
#
# THE COST CURVE IS MONOTONE IN THE RESERVE AND WELL IDENTIFIED
# (both-branches rungs; reserve at live scale in Ti):
#     reserve      ~54(d4)   ~81(d5)   ~121(d3)      control
#     mean ammo      25.3      17.7      10.2         30.9
#     dry rounds     40.0      82.1     181.4         26.2
#     median kill     161       169       251        151.5
#     kill-by-r300  0.854     0.846     0.608        0.883
# THE GAIN CURVE IS NOT.  Turret builds go 8.67 / 6.04 / 12.17 against a
# control 4.35 -- NON-monotone, wide intervals (d4 +4.33 +-3.61), and the d4
# turret gain **does not replicate in the NOISE_ON regime** (+0.47 +-0.59
# there).  The magazine cost DOES replicate (d4 mean ammo -5.57 +-1.42
# NOISE_OFF, -5.04 +-2.19 NOISE_ON; both exclude 0).
# ⇒ THERE IS NO CROSSING INSIDE THE MEASURED RANGE.  The loss starts at the
# first rung that binds and the gain is not reliably positive anywhere.
#
# DOSE 3 IS DISQUALIFIED, not merely unattractive: timely-kill-by-r300 falls
# -27.5pp +-6.5 (NOISE_OFF) and -19pp +-8 (NOISE_ON), both CIs excluding 0,
# against PROGRAMME.md's DEFENCE_ADMISSION_BAR (r300 crossing non-regression).
# It is the arm that buys the most turrets and it buys them by starving the
# magazine to 10 ammo mean and 181 dry rounds -- turrets it cannot feed.
#
# ⛔ AND THE FIRST-TURRET CLAIM DOES NOT SURVIVE THE REGIME CHECK.  NOISE_OFF
# reproduces the sibling probe's sign and magnitude (dose 2: -1.10 +-0.72
# rounds EARLIER, dose 3: -1.38 +-0.75).  NOISE_ON INVERTS the point estimate
# for every dose (+1.28 to +2.21) with intervals ~+-4 that resolve nothing.
# Per the standing fixture warning, the direction on first-turret round is
# therefore NOT established by this arm and must not be quoted as a finding.
#
# WHY DOSE 4 IS THE ONE SHIPPED, stated as what it is: it is the SMALLEST rung
# that delivers a measurable dose (af_bind +26.0 +-8.2, 237/240 cells; 120
# rounds/game at a raised floor) while its primary stays inside the bar
# (kill-by-r300 -2.9pp +-3.7, straddling 0; median kill +6.0 paired).  It is
# chosen by MINIMISING A MEASURED COST, not by maximising a measured gain --
# there was no reliable gain to maximise.  ⚠ RESTATED AS AN EXCLUSION, as the
# DEFF-direction rule requires of any fail-to-exclude claim: dose 4's interval
# excludes a regression of 6.6pp or more; it does NOT exclude a small one, and
# it does not exclude zero.  It also pays +3 r1000 games per 240 and +3.8pp of
# games running past r300 -- real costs under R1000_IS_DEFEAT.
#
# ⛔ THE FIXTURE'S OWN LIE, named because it bounds everything above: the eight
# opponents are OLD VERSIONS OF OUR OWN LINEAGE and the control wins 97.9% of
# 240 games.  Absolute shares are saturated and meaningless here; only the
# within-fixture mechanism reads (ammo, shots, dry rounds, turret counts, kill
# round) carry, and a BENEFIT could not have been detected on this panel at
# all.  Per LOKI directive point 6 this arm neither confirms nor CLOSES the
# road; a live-team unrated leg is the only instrument that can.
# ============================================================================

LOKI_AMMOFLOOR_ON = True        # False == the parent exactly (dose 0)
LOKI_AMMOFLOOR_DOSE = 4         # 1 | 2 | 3 | 4 | 5, see the ladder above
# Instrumentation only: one stdout print per round in which the drip WANTED a
# top-up, carrying the floor and whether it bound.  Local instrument only --
# platform replays strip stdout (CLAUDE.md s28), and the local harness must keep
# the replay (`--replay <file>`, not /dev/null) for it to be readable at all.
# Dose tag "AF501".  Never enable under a WALL-CLOCK --tle: this is the core's
# hot path.
LOKI_AMMOFLOOR_LOG = False
