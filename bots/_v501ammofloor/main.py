...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # LOKI-AMMOFLOOR.  Raise the bank floor the drip may not spend
            # through to the price of the WEAPON we are notionally saving for --
            # see doctrine.py LOKI-AMMOFLOOR for the ladder, the pricing (each
            # dose is the plant path's own refusal threshold) and the
            # magazine-starvation hazard dose 3 deliberately runs.
            # BELOW E1 on purpose: composition is max(), so E1's harvester
            # reserve is an input here and can never be lowered by this plank.
            if LOKI_AMMOFLOOR_ON:
                try:
                    if LOKI_AMMOFLOOR_DOSE in (2, 3):
                        reserve = ct.get_sentinel_cost() + LOKI_FWD_TI_FLOOR
                    elif LOKI_AMMOFLOOR_DOSE == 5:
                        reserve = ct.get_sentinel_cost()
                    else:
                        reserve = ct.get_gunner_cost()
                    if LOKI_AMMOFLOOR_DOSE in (3, 4, 5) or not (under or weapons_top):
                        ti_floor = max(ti_floor, reserve)
                except Exception:
                    pass
            # Dose channel.  Fires in the flags-OFF tree too, on purpose: the
            # baseline refusal count is the denominator this plank is judged on.
            if LOKI_AMMOFLOOR_LOG and (under or weapons_top or harv >= 2) \
                    and ammo < ammo_target:
                print("AF501 WANT rnd=%d ti=%d floor=%d ammo=%d tgt=%d bind=%d"
                      % (rnd, ti, ti_floor, ammo, ammo_target,
