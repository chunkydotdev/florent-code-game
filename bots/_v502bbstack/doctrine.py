...
# ===========================================================================
# LOKI-BBSTEP (_v500bbstep) -- NORAY BECOMES A STEP, NOT A REFUSAL.
# ===========================================================================
# THE TARGET, and it is the SECOND-largest refusal reason in the BELTBREAK
# funnel (31.8% of in-context refusals, behind the bank's 53.8%).  NORAY
# (`raid.py` `_try_beltbreak_gunner`, the `best is None` branch) means every
# earlier rung PASSED: the round gate, the harvester floor, the live cap, the
# funding, and the live-target scan -- and then no
#     (adjacent build tile in d^2 20-100) x (45-degree-snapped facing)
# combination put any of those live targets on a legal gunner ray.  A gunner
# reaches d^2 <= 13, about three tiles, so the build tile has to be nearly on
# top of their belt, and belts do not run past our ring stations by
# arrangement.  THE WASTE: a FUNDED raider stands in the productive annulus
# with targets in its own vision and plants nothing, when a tile two steps away
# might have a ray.
#
# THE PLANK: on a NORAY refusal, look 2-3 tiles further out for a build tile
# that WOULD pass the same ladder, and if one exists make it this raider's MOVE
# TARGET for a few rounds.  A step costs no titanium, no scale contribution and
# no action -- `_bbs_probe` mutates nothing but this unit's own latch, returns
# nothing, and `_raid_act` continues to its steps 4-7 exactly as in the parent.
# The ONLY thing spent is the move the parent would have used walking at a ring
# station, which is the same resource `_v495healbreak` spends and for the same
# reason.
#
# ⛔ SCOPED TO GEOMETRY, DELIBERATELY.  A step is offered ONLY on NORAY.  It is
# NOT offered on TI (the bank is empty -- walking does not create titanium),
# CAP (we already hold the maximum), EARLY (the round gate), HARV (the
# harvester floor) or NOTGT (nothing in vision to shoot -- a two-tile step does
# not extend r^2=20 vision meaningfully and NOTGT is a different plank).
# OWNRAY is also excluded: it means a ray EXISTS and our own collar is on it,
# which is a siting-order question, not a reachability one.
#
# ⛔⛔ AND NORAY IS TWO DIFFERENT POPULATIONS, WHICH THE PARENT'S COUNTER POOLS.
# MEASURED, 2026-08-17, 30 instrumented games vs the pinned control: of 1,724
# NORAY refusals, **1,693 (98.2%) came from a raider with NOT ONE build tile
# inside the annulus**, and 1,469 of those from a raider at d^2 = 1 or 2 --
# STANDING ON THE COLLAR.  Only **31 (1.8%)** were the refusal this plank is
# named for: band geometry actually scored, and no ray found.
#
# WHY THE PARENT'S COUNTER POOLS THEM, exactly, because it is an arithmetic slip
# and not a judgement call.  The band pre-scan is
#     if d < lo - 24 or d > hi + 24: return False
# and its own comment derives the 24 as "2*sqrt(d)+1, which is 21 AT THE OUTER
# EDGE".  That is right at the outer edge and wrong at the inner one: at d = 20
# the same expression is 2*sqrt(20)+1 = 9.9, so the correct inner guard is
# `d < lo - 10`, i.e. 10.  Using the outer bound on BOTH sides makes the inner
# guard `d < -4`, which admits EVERY raider on the map including one standing on
# the enemy core's collar at d^2 = 1.  The pre-scan docstring's claim that "every
# refusal COUNTED below is a refusal that happened with the raider actually
# standing in the productive annulus" is therefore false at the inner edge, and
# 98.2% of NORAY is that falsehood.
#
# ⇒ SO ELIGIBILITY IS NOT A d^2 THRESHOLD.  It is `saw_band`, set by the
# parent's own loop the moment one of its four build tiles passes the band
# filter -- the engine-exact statement of "the annulus was scored and produced
# no ray".  A raider at d^2 = 17 with a neighbour at d^2 = 22 IS this plank's
# business; a collar raider at d^2 = 1 is NOT, and belongs to the arm that
# already exists to walk it outward (`_v495healbreak`).  The `NOBAND` counter
# keeps the split visible instead of letting this arm bank a bench arm's work.
#
# WHAT IS *NOT* A HAZARD, and it is worth stating because it is the usual
# objection to acting on a stale predicate: the ray is checked NOW for a tile
# reached in one to three rounds, but every entity in BB_SITE_VALUE
# (HARVESTER / CONVEYOR / SPLITTER) is a BUILDING and buildings are IMMOVABLE.
# The target cannot walk out of the ray.  It can only be DESTROYED, or a new
# building can appear on the line.  Both are re-checked from scratch by the
# parent's own scan on arrival -- this plank never plants anything itself, it
# only decides where to stand -- so a stale probe costs a walk, never a bad
# gunner.
#
# THE BOUNDS, AND WHAT THEY GUARANTEE.  Three of them, and together they cap
# the total move-time this plank can take from the collar over a raider's whole
# life at BBS_MAX_CYCLES * (BBS_HOLD + BBS_DWELL) rounds:
#   * one latch at a time, on a FIXED tile, so a raider cannot oscillate
#     between two candidates -- a live latch suppresses further probing;
#   * BBS_DWELL is a single un-refreshed park so an arrived raider is handed
#     back to the ring even if it never plants;
#   * BBS_COOL after every completed cycle, plus a hard per-life cycle cap.
# And the displacement itself is bounded by construction: every candidate build
# tile is required to be IN BAND, so a stepping raider never leaves the annulus
# it was already in.  It cannot be walked away from the raid by this plank.
BBSTEP_ON = True           # MASTER.  False == _v480beltbreak exactly.
BBS_RINGS = (2, 3)         # Manhattan rings from the raider searched for build
                           # tiles.  Ring 1 IS the parent's own scan and has
                           # already failed by the time we are called, so it is
                           # excluded; ring 2 costs one step, ring 3 costs two.
                           # 4 is not searched: 3 steps is ~4 rounds of walking
                           # for a ray checked before the walk started, and the
                           # rings cost 4*md tiles each (8 + 12 = 20 here).
BBS_MAX_TILES = 8          # in-band candidate tiles taken past the cheap
                           # arithmetic filters (CPU bound, engine calls)
BBS_MAX_FIRE = 10          # can_fire_from calls per probe.  HARD.  The parent's
                           # own scan is 4 tiles x up to 12 targets; this stays
                           # inside the same order of magnitude.
BBS_HOLD = 6               # rounds a WALK latch lives.  The stand tile is 1-2
                           # steps away and a builder moves at most once per
                           # round, so 6 covers the walk with slack for a move
                           # cooldown and one blocked tile.
BBS_DWELL = 3              # rounds parked on arrival, NOT refreshed: enough for
                           # the action cooldown to clear and the parent's scan
                           # to run from the new tile twice.
BBS_COOL = 8               # rounds before this raider may latch again
BBS_MAX_CYCLES = 3         # step cycles per raider LIFE.  Hard ceiling: at most
                           # 3 * (6 + 3) = 27 rounds of this raider's movement
                           # can ever be diverted from the ring.
BBS_LOG = False            # LOCAL stderr instrument, tag "BBS500".  Never read
                           # off a live leg: print()/stderr is stripped from
                           # platform-downloaded replays (see CLAUDE.md).

# ===========================================================================
# LOKI-BBSTACK (_v502bbstack, s48) -- THE FOUR-LEG NEST.
# ===========================================================================
# WHY A STACK AND NOT FOUR LEGS.  Each of these measured NULL OR NEAR-NULL
# ALONE, and the funnel says why: they are NESTED, not parallel.  The parent
# BELTBREAK2 doctrine block above records the CAP null in its own words --
# `LOKI_BELTBREAK_CAP = 2 -> 3` moved shredders/game by 0.000 -- together with
# the reason: an instrumented parent refuses on `CAP` 6.6 times per game
# against `TI` 638.  Raising a cap that is not binding cannot buy a plant.
# The order the nest has to be relieved in is therefore
#       TI (the bank)  ->  CAP (the permission)  ->  SITING (the geometry)
# and the four legs below are exactly that chain, plus the instrument fix that
# makes the chain readable.  ⛔ NONE OF THEM IS CLAIMED TO PAY ALONE; the
# headline this tree exists to produce is whether plants become ADDITIVE once
# all three constraints are relieved together, and if they do not, WHICH
# refusal now dominates the funnel.  That second answer is the deliverable.
#
# ---------------------------------------------------------------------------
# LEG 0 -- THE BOUND FIX.  COUNTING, PLUS CPU.  NOT BEHAVIOUR.
# ---------------------------------------------------------------------------
# The pre-scan bail in `raid.py` `_try_beltbreak_gunner` reads, in the parent,
#     if d < lo - 24 or d > hi + 24: return False
# and its own comment (raid.py, the docstring immediately above it -- read, not
# assumed) derives the 24 as "2*sqrt(d)+1, which is 21 AT THE OUTER EDGE".
# That is correct at the outer edge and wrong at the inner one: at d = lo = 20
# the same expression is 2*sqrt(20)+1 = 9.94, so the inner guard should be
# `lo - 10` = 10.  Using the outer bound on both sides makes the inner guard
# `d < -4`, which is UNREACHABLE for a squared distance -- i.e. the inner bail
# is DEAD CODE and every raider on the map enters the scan, including one
# standing on the enemy core's collar at d^2 = 1.
# ⇒ The docstring's claim that "every refusal COUNTED below is a refusal that
#   happened with the raider actually standing in the productive annulus" is
#   FALSE at the inner edge, and that falsehood is most of the NORAY column.
# THE SIZE OF IT IS ALREADY MEASURED, by the LEG 2 arm and relayed here rather
# than re-derived (`_v500bbstep` doctrine, 30 instrumented games vs the pinned
# control): of 1,724 NORAY refusals, 1,693 (98.2%) came from a raider with NOT
# ONE build tile inside the annulus, and 1,469 of those from a raider at
# d^2 = 1 or 2.  Only 31 (1.8%) were the refusal NORAY names.  1,724/31 = 55.6x.
#
# ⭐ WHAT ELSE CHANGES, AND THE ANSWER IS NOTHING -- ARGUED FROM THE CODE PATH,
# NOT ASSERTED.  The fix tightens an early `return False`, so the question is
# what a raider at d^2 <= 9 did between the old bail and its eventual refusal.
# Traced by reading `_try_beltbreak_gunner` end to end:
#   * the band filter is `if db < lo or db > hi: continue`, applied to the four
#     CARDINAL BUILD TILES.  For d <= 9 every neighbour has db <= d+2*sqrt(d)+1
#     <= 16 < 20 = lo, so ALL FOUR `continue`, `best` stays None, and the call
#     ends at `_bb_refuse("NORAY")` -- it was ALREADY returning False.
#   * `OWNRAYHIT` is written INSIDE that loop, after the band filter, so it is
#     unreachable for d <= 9.
#   * `SLOT_BELTBREAK` (the heartbeat that opens the Core's magazine) is written
#     only after a SUCCESSFUL `build_gunner`, so it cannot be suppressed.
#   * nothing else in the skipped region mutates anything: the target scan is
#     pure reads, and `_bb_refuse` only increments a dict and may print.
# ⇒ At `--tle 0` this leg is BEHAVIOUR-NEUTRAL BY CONSTRUCTION and changes only
#   refusal COUNTS and CPU.  Under a WALL-CLOCK `--tle` it can only ADD
#   headroom (strictly less work per call), never remove it.
# AND THE BOUND IS SAFE, not merely smaller: the tightest correct bail solves
# d + 2*sqrt(d) + 1 < 20, i.e. d < 12.06.  `lo - 10` = 10 sits BELOW that, so
# no raider that could have reached the band is ever turned away.  10 is
# deliberately the conservative integer and not the tight one.
#
# ---------------------------------------------------------------------------
# LEG 1 -- CAP 2 -> 3, AND THE MAGAZINE DERIVED FROM IT RATHER THAN SET.
# ---------------------------------------------------------------------------
# The parent's own block above already states the arithmetic that makes the
# bare cap bump incoherent: a 20-HP conveyor is ceil(20/7) = 3 gunner shots at
# 4 ammo each = 12 ammo per belt kill, and `LOKI_BELTBREAK_AMMO = 24` is
# EXACTLY TWO of those -- sized for CAP = 2.  A third shredder under an
# unchanged magazine plants and then cannot complete a kill.
# ⇒ So the primitive here is ONE KILL CYCLE, and the fleet magazine is DERIVED:
#       LOKI_BELTBREAK_AMMO_EFF = LOKI_BELTBREAK_KILL_AMMO * CAP_EFF
#   which is 12*2 = 24 at CAP 2 (the parent's number, recovered exactly) and
#   12*3 = 36 at CAP 3.  Deriving it is what makes "fund what you allow" ONE
#   mechanism with one knob instead of two integers that can drift apart.
#
# ⚠⚠ THE HIDDEN THIRD EFFECT, STATED BECAUSE IT POINTS AGAINST THIS LEG.
# `LOKI_BELTBREAK_AMMO` is read at THREE sites, not one, and only one of them
# is the magazine:
#   1. main.py, the Core's `ammo_target`  -- the magazine.  Scales.  Intended.
#   2. raid.py, the funding `need` for the SECOND-and-later gunner
#      (`need += LOKI_BELTBREAK_TI_FLOOR + LOKI_BELTBREAK_AMMO`).  Scaling this
#      RAISES THE PLANT PRICE by 12 Ti -- and `TI` is the refusal that already
#      dominates the funnel 638-to-6.6.  ⇒ This leg can therefore SUPPRESS the
#      very plants LEG 1 exists to buy.  It is given its own sub-flag,
#      `BBSTACK_L1_FUND_EFF`, precisely so the sweep can separate "bigger
#      magazine" from "dearer plant"; they are not the same intervention and
#      pooling them would make a null unattributable.
#   3. main.py, the beltbreak gunner's ROTATION affordability gate
#      (`ti < 10 + LOKI_BELTBREAK_AMMO`).  DELIBERATELY NOT SCALED, and pinned
#      to the parent's 24 via the unscaled name.  This gate asks "can we afford
#      to re-aim without eating the magazine"; keying it to FLEET size would
#      make re-aiming progressively harder as the fleet grows, which is a
#      category error -- the plank's job is to add shredders, not to stop the
#      ones we have from turning round.  Left alone ON PURPOSE, recorded here
#      so the choice is visible rather than accidental.
#
# ---------------------------------------------------------------------------
# LEG 2 -- THE NORAY STEP.  PORTED, NOT RE-IMPLEMENTED.
# ---------------------------------------------------------------------------
# The whole LOKI-BBSTEP block above, and `raid.py`/`main.py` in this tree, are
# taken VERBATIM from `_v500bbstep`.  That is exact rather than approximate for
# a structural reason worth recording: `_v500bbstep` was built on
# `_v480beltbreak`, and this tree's chassis `_v488beltbreak2` differs from
# `_v480beltbreak` IN doctrine.py ONLY (its raid.py, main.py and eco.py are
# byte-identical -- verified by md5, 0 differing lines).  So both trees' step
# machinery sits on the same base file, and `raid.py`/`main.py` here are
# md5-identical to `_v500bbstep`'s before the LEG 0/1/3 edits below.
# PORT DIVERGENCE: ZERO, and it is checkable, not claimed.
#
# ⭐ COMPOSITION WITH LEG 0, WHICH IS THE ONE INTERACTION THAT HAD TO BE
# CHECKED.  The step's eligibility is `saw_band`, a flag set INSIDE the band
# filter the moment one build tile passes it -- chosen by that arm precisely so
# that eligibility is not a d^2 threshold.  LEG 0 turns raiders at d^2 <= 9
# away BEFORE that loop runs.  But those raiders would have reached `_bbs_probe`
# with `saw_band = False`, which the probe refuses immediately as `NOBAND`.
# ⇒ THE TWO LEGS ARE ORTHOGONAL BY CONSTRUCTION: same decision (no step), taken
#   earlier and cheaper.  What DOES change is the `NOBAND` counter, which loses
#   its ~1,469 collar rows and afterwards counts only what it was named for.
#   Read the two arms' NOBAND columns as DIFFERENT INSTRUMENTS, not as a dose.
#
# ---------------------------------------------------------------------------
# LEG 3 -- AMMOFLOOR.  PORTED FROM `_v501ammofloor`, WITH ONE ADDED RUNG.
# ---------------------------------------------------------------------------
# PROVENANCE, stated exactly because the port is cross-chassis.  Snapshotted
# 2026-08-17T12:12:58Z, doctrine md5 35a2cad3ddc2605460842c6dd2a5f1ad, verified
# unchanged 2.5 minutes later (that agent was still writing when this tree
# began; the snapshot is the version ported).  Its shipped values are
# `LOKI_AMMOFLOOR_ON = True`, `LOKI_AMMOFLOOR_DOSE = 4`, and BOTH ARE CARRIED
# HERE UNCHANGED.  The main.py block below is its code, structurally verbatim.
#
# ⛔ BUT `_v501ammofloor` IS BUILT ON `_v468kladturbo`, NOT ON THIS CHASSIS --
# its raid.py and eco.py are byte-identical to kladturbo's and it contains NO
# BELTBREAK AT ALL.  That matters for one reason: its dose ladder prices each
# reserve at THE PLANT PATH'S OWN REFUSAL THRESHOLD, read off that path's
# source line, and on kladturbo the plant path is `_try_forward_sentinel`.
# On THIS chassis the path being funded is `_try_beltbreak_gunner`, whose
# refusal line (raid.py, read directly) is
#     need = ct.get_gunner_cost();  if live > 0: need += TI_FLOOR + AMMO
# ⇒ Porting the METHOD faithfully means adding the rung that method implies on
#   this chassis, so DOSE 6 is that rung and it is THIS TREE'S DEFAULT.  Doses
#   1-5 keep `_v501ammofloor`'s numbering EXACTLY so a later sweep is comparable
#   across the two trees rather than merely across arms of this one.
#
# ⭐⭐ AND THE DOSE IS SET FROM ITS SWEEP, NOT FROM TASTE.  `_v501ammofloor`'s
# sweep completed while this tree was being assembled (2,897 games, control
# chassis, paired 240 cells/arm; relayed as a MEASUREMENT, not re-derived here).
# Three of its findings bind on the choice below:
#   1. ⛔ UNARMED-BRANCH-ONLY RESERVES ARE INERT.  Dose 1 changed 0 of 120 cells
#      on any metric; dose 2 raised the floor in 1.7 rounds/game with nothing
#      downstream, and the earlier probe's promising first-turret result DID NOT
#      REPLICATE (sign flips between NOISE regimes).  The unarmed phase is too
#      short to matter.  ⇒ DOSES 1 AND 2 ARE EFFECTIVELY DEAD CODE and are kept
#      only for numbering comparability.  A reserve that does anything must
#      cover the ARMED branch -- i.e. dose 4's SHAPE.
#   2. ⛔ THE COST CURVE IS MONOTONE AND REPLICATES; THE GAIN CURVE DOES NOT.
#      As the reserve rises: ammo held 30.9 -> 10.2, dry-turret rounds 26 -> 181,
#      shots -14/game.  Dose 3 (sentinel+40, both branches) is DISQUALIFIED at
#      -27.5pp timely-kill.  Turret gains appeared only in NOISE_OFF and vanished
#      in NOISE_ON.  ⇒ EVERY EXTRA TITANIUM OF RESERVE IS PAID IN SHOTS, and a
#      gunner eats 4 ammo a shot.  This is why the reserve below is the SHREDDER'S
#      price and not the SENTINEL'S, and why it stops there.
#   3. ⭐ THE SPLIT THAT MAKES THIS CHASSIS DIFFERENT: for turrets 2..n the floor
#      IS causal (+4.33 turrets at dose 4 -- money really had priced them out);
#      for the FIRST turret siting is the gate (121 Ti of reserve moved it only
#      -1.38 rounds).  OUR SHREDDERS ARE THE TURRETS-2..n CASE -- the funnel
#      refuses on `TI` 638x/game -- so the reserve has a cheap target here that
#      it did not have on the control, where the binding constraint was turret
#      THROUGHPUT (shots) rather than COUNT.  That is the open question this
#      stack exists to answer, and it is why the sweep's own solo verdict ("a
#      cost with no reliable gain") does not settle it on this base.
#
# ⇒ DOSE 6 = `get_gunner_cost() + LOKI_BELTBREAK_TI_FLOOR`, BOTH BRANCHES.
#   ~54-60 Ti at early scale: dose 4's SHAPE at the shredder's price.
# ⛔ `LOKI_BELTBREAK_AMMO_EFF` IS DELIBERATELY *NOT* IN THAT SUM, and this is a
#   correction to the value this tree was originally specified with.  Adding it
#   would reserve ~130 Ti at live scale and, by finding 2, would be paid for in
#   exactly the shots LEG 1 just bought a bigger magazine to fire -- the same
#   trade that disqualified dose 3.  Dose 7 below IS that variant; it exists so
#   the crossing can be located, and it is NOT a candidate.
# ⭐ THE READ THIS LEG IS JUDGED ON, therefore, is NOT plants alone:
#       PLANTS UP  *with*  SHOTS/SHREDDER FLAT.
#   Plants up and shots down is the disqualifying pattern, not a mixed result.
#
# ===========================================================================
# ⭐⭐ THE HEADLINE -- MEASURED, s48, 2026-08-17.  THE PLANTS ARE **NOT**
# ADDITIVE, AND THE FULL STACK IS WORSE THAN ITS OWN BEST SINGLE LEG.
# ===========================================================================
# FIXTURE.  LOCAL, 25 maps x 2 seats = 50 games PER ARM, `--tle 0`,
# NOISE_ON=False (so every arm replays the SAME 50 games and an outcome
# difference is causal, not variance), opponent = a NOISE-off copy of the pinned
# control `_v468kladturbo`.  Job list INTERLEAVED ACROSS ARMS AND SHUFFLED
# before a fixed-width pool, so a load swing cannot align with an arm.  Dose
# read off the bot's own unrate-limited stderr funnel (BBS_LOG), which is gated
# on BBS_LOG and NOT on any leg flag -- that is what makes the all-off arm
# usable as the chassis's own funnel.  ⚠ LOCAL INSTRUMENT ONLY: stdout AND
# stderr are stripped from platform-downloaded replays (CLAUDE.md s28), so
# nothing in this table could be read off a live leg.
#
#   arm            legs        plants/gm  fires  shots/shred  ITT300  medKill
#   S0 (chassis)   ----           1.54     1079      14.0      26.0%    206
#   L0             0---           1.54       =S0     14.0      26.0%    206
#   L1             -1--           1.34        --       --      26.0%     --
#   L2             --2-           1.64        --       --      26.0%     --
#   L3             ---3           2.20     1495      13.6      26.0%    256
#   L0+L3          0--3           2.20     1495      13.6      26.0%    256
#   L0+L2+L3       0-23           2.02     1411      14.0      22.0%    270
#   L0+L1+L3       01-3           2.04      614       6.0      28.0%    228
#   L0+L1+L2       012-           1.52     2075      27.3      26.0%    246
#   FULL           0123           1.86      579       6.2      28.0%    228
#
# ⇒ **STRICTLY SUB-ADDITIVE.**  Individually the legs sum to +0.56 plants/game
#   (0 - 0.20 + 0.10 + 0.66), predicting 2.10.  FULL delivers 1.86, and
#   L0+L3 alone delivers 2.20 -- MORE than the full stack.  The nest hypothesis
#   (TI -> CAP -> siting) is RIGHT that TI is first and WRONG that the other two
#   are waiting behind it: relieve TI and the other two become COSTS.
#
# ⇒ **THE FULL STACK FAILS THE SIBLING SWEEP'S OWN DISQUALIFYING TEST.**  Plants
#   +21% (1.54 -> 1.86) while shots/shredder fall 56% (14.0 -> 6.2) and TOTAL
#   FIRES fall 46% (1079 -> 579).  "Plants up, shots down" is exactly the trade
#   that disqualified that sweep's dose 3.  L0+L3 is the only cell that passes
#   the stated bar -- plants +43% with shots/shredder FLAT (14.0 -> 13.6) and
#   total fires +39%.  Hence the defaults below.
#
# ⇒ **WHERE THE COLLAPSE COMES FROM: LEG 1 *WITH* LEG 3, not either alone.**
#   Every cell containing both (L0+L1+L3 6.0, FULL 6.2) collapses; every cell
#   lacking LEG 1 (L0+L3 13.6, L0+L2+L3 14.0) does not; and LEG 1 WITHOUT LEG 3
#   goes the other way entirely (L0+L1+L2 reads 27.3 shots/shredder on 1.52
#   plants -- twice the shots each, spread over no extra shredders).  LEG 3
#   holds ~130 Ti out of the drip while LEG 1 raises the magazine TARGET to 36:
#   the target rises and the floor forbids paying for it, so three shredders
#   stand dry.  That is the magazine-starvation failure `_v501ammofloor`'s own
#   doctrine named, reached through LEG 1's raised target rather than through
#   the reserve.  Median magazine 8 (chassis) -> 2-3 (any LEG 3 cell); rounds in
#   which the drip wanted ammo, a shredder was live and the magazine held less
#   than one shot: 6.3/game (chassis) -> ~95-101/game (any LEG 3 cell).
#
# ⇒ **THE NEXT CONSTRAINT, WHICH IS THE MOST VALUABLE NUMBER HERE: STILL `TI`.**
#   ⛔ READ ONLY WITHIN AN L0 GROUP -- LEG 0 changes what gets COUNTED, so a
#   funnel from an L0-on arm and one from an L0-off arm are DIFFERENT
#   INSTRUMENTS and must never be differenced.  Within L0-ON, per game:
#       L0 alone   TI 54.8  NOTGT  4.5  CAP  6.4  NORAY 2.8   (total 71.6)
#       L0+L3      TI 55.7  NOTGT 12.6  CAP 10.4  NORAY 8.6   (total 90.5)
#       FULL       TI 64.0  NOTGT 10.4  CAP  1.2  NORAY 6.4   (total 85.1)
#   TI is 62-75% of every post-fix funnel.  AMMOFLOOR MOVED THE BANK WITHOUT
#   DETHRONING IT (bank at r95-105: 29 -> 54 Ti; TI refusals stay ~56/game).
#   ⚠ AND THE OBVIOUS INFERENCE FROM `CAP 10.4` IS WRONG, WHICH IS WHY THE
#   ABLATION WAS WORTH RUNNING: L0+L3 refuses on CAP 10.4 times a game, which
#   reads as "the cap now binds" -- yet raising it buys nothing (2.20 -> 2.04
#   with the fund bump, 2.00 -> 2.02 without).  Those CAP refusals are raiders
#   that would have refused on TI one rung later.  A refusal counter names the
#   rung that fired, NOT the constraint that binds.
#
# ⛔ WHAT THIS TABLE DOES *NOT* SAY.  n = 50 games/arm gives a +-12pp half-width
# on a single ITT300 share and ~+-17pp on a difference of two, so EVERY
# timely-kill and win figure above is UNRESOLVED -- not "non-regressing".  The
# median-kill drift (206 -> 256 for L0+L3) sits inside r200-300, which s45's
# re-pricing reports rather than penalises; the L0+L2+L3 cell's 270 and its
# ITT300 of 22.0% are the only readings pointing at the r300 backstop, and they
# are also inside noise.  The plant, fire and refusal columns are the resolvable
# ones, because NOISE_ON=False makes them a paired replay of one game set rather
# than a sample.  ⚠ AND THEY ARE 50 GAMES AGAINST ONE OPPONENT ON A LOCAL
# FIXTURE WE AUTHORED: per this repo's own directive a road is not closed by
# that, only prioritised.
#
# VERIFICATION LOG (every verdict driven both ways):
#   * FLAG-OFF IDENTITY: master off vs `_v488beltbreak2`, 16/16 games
#     byte-identical stdout.  POSITIVE CONTROL: master on vs the same chassis,
#     16/16 DIFFERING.  Both at `--tle 0` with NOISE off.
#   * LEG 0 BEHAVIOUR-NEUTRALITY, measured twice and not merely argued:
#     S0 vs L0 identical in 50/50 games, and L3 vs L0+L3 identical in 50/50 --
#     neutral alone AND under composition.  The comparison's normaliser was
#     itself checked against a case that MUST differ (S0 vs L1: 27 of 50
#     differ), because a normaliser that silently no-ops would have printed
#     "identical" for everything.  ⛔ It DID no-op on the first attempt (BSD
#     `sed` has no `\b`), which is why that control exists.
#   * LEG 2 CONVERSION FUNNEL fires on this chassis: 49 latches, 13 arrivals,
#     30 plants tagged phase 1-2 = 61% (the parent arm reported 57%).
#     ⛔⛔ AND THE FUNNEL IS AN ATTRIBUTION ILLUSION, WHICH IS THE MOST
#     IMPORTANT CAVEAT IN THIS BLOCK.  In the L2-alone cell the phase tags
#     claim 24 stepped plants, while TOTAL plants rise only 77 -> 82 and
#     phase-0 plants fall 77 -> 58.  The tag marks a plant made WHILE A LATCH
#     WAS LIVE, not a plant CAUSED by one, so ~19 of those 24 are re-labelled
#     parent plants.  A 61% "conversion rate" is therefore NOT a 61% yield, and
#     the parent arm's 57% carries the same defect.  Only the total moves.
#   * ZERO tracebacks, zero `GameError`, zero probe exceptions (`bbs_why`
#     RAISED = 0) in 700 games.
#   * CPU: `--tle 10` with all instruments off reproduced `--tle 0` EXACTLY on
#     all three shipped-shape arms (win/kill/ITT300/median identical), and no
#     engine timeout wording appears anywhere.  ⇒ TLE does not bind in this
#     fixture, so a TLE-zero column would be uninformative here -- and
#     structurally LEG 0 can only REMOVE work (an earlier `return False`), while
#     LEGS 1 and 3 add module-folded integer comparisons plus one
#     `get_gunner_cost()` call on the CORE's path only, never a raider's.
#   * NOWINNER: 0 of 850 games (350 + 350 + 150).
#
# ===========================================================================
# ⛔⛔⛔ THE REPLICATION, AND IT IS WHY LEG 3 DEFAULTS **OFF**.
# ===========================================================================
# THE THREAT THAT HAD TO BE TESTED.  Everything above is NOISE_ON=False -- a
# paired replay of one game set.  `_v501ammofloor`'s own sweep reported that its
# turret gains "appear in NOISE_OFF and vanish in NOISE_ON", so the entire table
# above was open to being that same artefact.  Re-run with NOISE_ON=True, a
# NOISE-ON opponent, 25 maps x 2 seats x 4 reps = 200 games PER ARM, unpaired
# and balanced by construction (`--seed` is inert in this repo, so a paired
# design is not available), chassis vs the SHIPPED-DEFAULT configuration:
#
#   arm                      n   plants/gm     win  kill  ITT300  medKill  r1000
#   chassis (master off)   200     1.365       112   103   28.0%     259      26
#   L0+L3 (shipped shape)  200     1.815       102    95   22.5%     303      14
#
# ⭐ **THE MECHANISM REPLICATES.**  plants/game +0.450, 95% CI [+0.134, +0.766],
#   EXCLUDES ZERO (sd 1.54 / 1.68, unpaired two-sample).  This is NOT a
#   NOISE_OFF artefact, and it is the one claim in this tree that survives its
#   own strongest test.  ⚠ shots/shredder is NOT re-read here (these arms carry
#   the shipped stdout-capped FIRE log, and bot stdout goes into the replay, not
#   the terminal), so the flat-shots finding rests on the paired battery alone.
#
# ⛔ **AND THE CURRENCY DOES NOT FOLLOW IT.  EVERY OUTCOME COLUMN MOVES AGAINST
#   THE PLANK.**  Wins 112 -> 102, kills 103 -> 95, timely-kill-by-r300
#   28.0% -> 22.5% (diff -5.5pp, 95% CI [-14.0, +3.0]pp), median kill round
#   259 -> 303.  No single column is resolvable at n=200 -- the win-share
#   difference alone carries +-9.8pp -- but they ALL point the same way while
#   the mechanism column points the other, and that is the shape of a plank that
#   buys the intermediate and not the goal.  (The one column that moves FOR it:
#   r1000 games 26 -> 14, i.e. fewer tiebreak defeats.)
#
# ⇒ **IT DOES NOT CLEAR `DEFENCE_ADMISSION_BAR`, SO IT DOES NOT SHIP ARMED.**
#   The bar's primary is the ITT timely-kill share, which must not FALL; it
#   fell.  The gross backstop is the median kill round crossing 300; it crossed,
#   at 303.  Both readings are inside noise and NEITHER CONVICTS -- but the
#   burden is on the plank to CLEAR a bar, not on the bar to convict the plank,
#   and a 33% gain in shredders that shows up as -5.5pp of timely kills is
#   exactly the trade `R1000_IS_DEFEAT` and the r300 re-pricing exist to refuse.
#   ⇒ `BBSTACK_L3_AMMOFLOOR = False`.  One flag arms it for a powered leg.
#
# ⇒ **CONSEQUENCE FOR WHAT THIS TREE IS.  Because LEG 0 is behaviour-neutral
#   (50/50, twice) and LEGS 1-3 all default off, THE SHIPPED DEFAULT OF THIS
#   TREE IS BEHAVIOURALLY THE CHASSIS.**  That is deliberate and it is stated
#   plainly rather than dressed up: what this tree delivers is (a) a corrected
#   refusal instrument, (b) three measured, individually-flagged legs with their
#   numbers attached, and (c) the finding that the funnel's dominant refusal is
#   STILL `TI` after all four legs.  It delivers no measured currency gain,
#   because there was none to deliver.
# ===========================================================================
BBSTACK_ON = True          # MASTER.  False == `_v488beltbreak2` EXACTLY, all
                           # four legs off, byte-identical behaviour (measured
                           # 16/16 below).
BBSTACK_L0_BOUND = True    # LEG 0: inner pre-scan bail `lo - 24` -> `lo - 10`
# ⛔⛔ LEGS 1 AND 2 DEFAULT **OFF**, AND THAT IS A MEASUREMENT, NOT A HEDGE.
# The ablation battery below found the FOUR-LEG STACK MEASURABLY WORSE THAN
# LEGS 0+3 ALONE on every instrument this plank is judged by, and worse than the
# CHASSIS on the one the sibling sweep named as disqualifying.  The flags stay
# because the cells must remain re-runnable; the defaults follow the numbers.
BBSTACK_L1_CAP = False     # LEG 1: live cap 2 -> 3, magazine derived from it.
                           # OFF: measured -0.20 plants/game ALONE, and in
                           # combination with LEG 3 it collapses shots/shredder
                           # 13.6 -> 6.0.  See THE HEADLINE below.
BBSTACK_L1_FUND_EFF = False # LEG 1 sub-flag: does the 2nd+ gunner's PLANT PRICE
                           # follow the bigger magazine?  See the hidden-third-
                           # effect warning above -- this is the half of LEG 1
                           # that can push the wrong way, and it MEASURED that
                           # way (TI refusals 453.5 -> 540.6/game).  Inert while
                           # BBSTACK_L1_CAP is False; kept for the sweep.
BBSTACK_L2_STEP = False    # LEG 2: the ported NORAY-step machinery.  OFF: mildly
                           # positive ALONE (+0.10 plants/game) but a DRAG once
                           # LEG 3 is present (2.20 -> 2.02), and the L0+L2+L3
                           # cell pushed the median kill round to 309, i.e.
                           # THROUGH the r300 gross backstop.
BBSTACK_L3_AMMOFLOOR = False # LEG 3: the ammunition-drip bank floor.  THE WHOLE
                           # MEASURED MECHANISM EFFECT OF THIS TREE IS THIS LEG
                           # (+0.450 plants/game, CI excludes 0, REPLICATED
                           # under NOISE_ON) -- AND IT DEFAULTS **OFF** because
                           # the currency moved against it and it does not clear
                           # DEFENCE_ADMISSION_BAR (ITT300 -5.5pp, median kill
                           # 303 > 300).  See THE REPLICATION above.  Flip this
                           # one flag to arm the plank for a powered leg.

# The primitive LEG 1 derives the magazine from: ONE belt kill.  A 20-HP
# conveyor is ceil(20/7) = 3 gunner shots at 4 ammo = 12 ammo.
LOKI_BELTBREAK_KILL_AMMO = 12

# ---- derived effective constants -------------------------------------------
# Module-level so they fold once at import and cost nothing in the hot path.
# Every one of them collapses to the chassis value when BBSTACK_ON is False;
# that is what makes the flag-off arm byte-identical rather than merely similar.
_BBST = BBSTACK_ON
# LEG 0.  Chassis: 20 - 24 = -4 (unreachable, dead).  Armed: 20 - 10 = 10.
LOKI_BELTBREAK_LO_BAIL = LOKI_BELTBREAK_DSQ_LO - (
    10 if (_BBST and BBSTACK_L0_BOUND) else 24)
# LEG 1.  CAP_EFF drives AMMO_EFF, so the two cannot drift apart.
LOKI_BELTBREAK_CAP_EFF = 3 if (_BBST and BBSTACK_L1_CAP) else LOKI_BELTBREAK_CAP
LOKI_BELTBREAK_AMMO_EFF = LOKI_BELTBREAK_KILL_AMMO * LOKI_BELTBREAK_CAP_EFF
# LEG 1 sub-flag.  The PLANT PRICE, which may or may not follow the magazine.
LOKI_BELTBREAK_FUND_AMMO = (
    LOKI_BELTBREAK_AMMO_EFF if (_BBST and BBSTACK_L1_FUND_EFF)
    else LOKI_BELTBREAK_AMMO)
# LEG 2.  The ported arm's own master survives, ANDed with ours.
BBSTEP_EFF = BBSTEP_ON and _BBST and BBSTACK_L2_STEP

# ---- LEG 3: LOKI-AMMOFLOOR, ported from `_v501ammofloor` --------------------
# Dose ladder, its numbering preserved verbatim (see its doctrine for the 2x2
# reasoning and for why doses 1-2 are structurally inert most of the game):
#   0/off  the chassis exactly
#   1      gunner_cost                          , UNARMED branch only  [INERT]
#   2      sentinel_cost + LOKI_FWD_TI_FLOOR    , UNARMED branch only  [INERT]
#   3      sentinel_cost + LOKI_FWD_TI_FLOOR    , BOTH branches  [DISQUALIFIED
#          on the control chassis at -27.5pp timely-kill]
#   4      gunner_cost                          , BOTH branches
#          (`_v501ammofloor`'s own shipped value)
#   5      sentinel_cost, no margin             , BOTH branches
#   6      gunner_cost + LOKI_BELTBREAK_TI_FLOOR, BOTH branches  <- THIS TREE.
#          ADDED HERE: the beltbreak plant path's own refusal threshold, i.e.
#          this chassis's answer to the question `_v501ammofloor` asked of
#          kladturbo's forward-sentinel path.  Dose 4's shape, shredder's price.
#   7      dose 6 + LOKI_BELTBREAK_AMMO_EFF     , BOTH branches -- the
#          magazine-inclusive variant.  ⛔ NOT A CANDIDATE (see finding 2); it
#          exists only so the sweep can locate the crossing between 6 and 3.
LOKI_AMMOFLOOR_ON = True   # mechanism carried from the sibling
LOKI_AMMOFLOOR_DOSE = 6    # ⭐ set from the sibling's SWEEP, not its default:
                           # dose 4's branch scope at the shredder's price.
                           # # DOSE PENDING SWEEP -- 4 (cheaper) and 7 (dearer)
                           # are the two rungs either side of this one.
LOKI_AMMOFLOOR_EFF = bool(_BBST and BBSTACK_L3_AMMOFLOOR and LOKI_AMMOFLOOR_ON)
LOKI_AMMOFLOOR_LOG = False # LOCAL instrument, tag "AF502", core hot path.
                           # NEVER enable under a wall-clock --tle, and nothing
                           # may be read off a live leg with it (platform
                           # replays strip stdout -- CLAUDE.md s28).
