...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-BBSTEP, per RAIDER.  The step latch and its funnel.
        # `bbs_step` is the tile this raider is walking to STAND on and
        # `bbs_aim` the build tile it was chosen for (diagnostic only -- the
        # parent's own scan re-sites from adjacency on arrival, so nothing reads
        # bbs_aim to decide anything).  `bbs_phase` 0 idle / 1 walking / 2
        # parked; `bbs_cycles` is the hard per-life ceiling counter, incremented
        # in `_bbs_release` and nowhere else. ---
        self.bbs_step = None
        self.bbs_aim = None
        self.bbs_phase = 0
        self.bbs_until = -10 ** 9       # the live latch's expiry round
        self.bbs_cool_until = -10 ** 9  # re-latching banned before this round
        self.bbs_plants_at = 0          # bb_plants when the latch was taken
        self.bbs_cycles = 0
        self.bbs_noray = 0              # NORAY refusals this raider saw
        self.bbs_latched = 0
        self.bbs_moves = 0
        self.bbs_arrived = 0
        self.bbs_planted = 0
        ...
        if not endgame_dumped:
            ...
            if LOKI_BELTBREAK_ON:
                ...
                if beat and rnd - (beat - 1) <= LOKI_BELTBREAK_STALE:
                    bb_live = True
                    # LEG 1 (BBSTACK).  The magazine is DERIVED from the live
                    # cap (KILL_AMMO * CAP_EFF), so it cannot drift away from
                    # the number of shredders we allow: 24 at CAP 2 (the
                    # parent's value, recovered exactly), 36 at CAP 3.
                    ammo_target = max(ammo_target, LOKI_BELTBREAK_AMMO_EFF)
                else:
                    bb_live = False
            else:
                bb_live = False
            ...
            # ---- LEG 3 (BBSTACK): LOKI-AMMOFLOOR ---------------------------
            # Ported from `_v501ammofloor/main.py` (snapshot md5
            # 35a2cad3ddc2605460842c6dd2a5f1ad, 2026-08-17T12:12:58Z).  Raise
            # the bank floor the ammunition drip may not spend through to the
            # price of the WEAPON we are notionally saving for, so the drip can
            # only have the titanium the weapon does not want.
            # BELOW E1 on purpose: the composition is max(), so E1's harvester
            # reserve is an INPUT here and can never be lowered by this plank.
            # Additive composition was rejected by the sibling -- the two
            # reserves are for the same titanium spent by the same call, not two
            # bills to pay.  Dose 6 (this tree's default) prices the reserve at
            # `_try_beltbreak_gunner`'s OWN refusal threshold; see doctrine
            # LOKI-BBSTACK LEG 3 for the sweep findings that set it, and for why
            # LOKI_BELTBREAK_AMMO_EFF is deliberately NOT in the sum.
            if LOKI_AMMOFLOOR_EFF:
                try:
                    # `_afd`, not `d`: `d` is a live squared-distance local
                    # earlier in this same method and must not be shadowed.
                    _afd = LOKI_AMMOFLOOR_DOSE
                    if _afd in (2, 3):
                        reserve = ct.get_sentinel_cost() + LOKI_FWD_TI_FLOOR
                    elif _afd == 5:
                        reserve = ct.get_sentinel_cost()
                    elif _afd == 6:
                        reserve = ct.get_gunner_cost() + LOKI_BELTBREAK_TI_FLOOR
                    elif _afd == 7:
                        reserve = (ct.get_gunner_cost() + LOKI_BELTBREAK_TI_FLOOR
                                   + LOKI_BELTBREAK_AMMO_EFF)
                    else:
                        reserve = ct.get_gunner_cost()
                    # Branch scope.  Doses 1-2 are UNARMED-branch only and the
                    # sibling's sweep measured them INERT (dose 1: 0 of 120
                    # cells moved); everything from 3 up covers both branches.
                    if _afd >= 3 or not (under or weapons_top):
                        ti_floor = max(ti_floor, reserve)
                except Exception:
                    pass
            if LOKI_AMMOFLOOR_LOG and ammo < ammo_target \
                    and (under or weapons_top or bb_live or harv >= 2):
                print("AF502 WANT rnd=%d ti=%d floor=%d ammo=%d tgt=%d bb=%d "
                      "bind=%d" % (rnd, ti, ti_floor, ammo, ammo_target,
                                   1 if bb_live else 0,
                                   1 if ti - ti_floor < 4 else 0))
