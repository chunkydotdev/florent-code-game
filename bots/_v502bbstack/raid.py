...
def _bbs_ring_offsets(rings):
    """Every (dx, dy) at Manhattan distance in `rings`, nearest ring first.

    Computed ONCE at import.  Ring md has exactly 4*md members, so (2, 3) is 20
    offsets -- and the point of the ordering is that a viable ring-2 tile (one
    step) is preferred to a ring-3 one (two steps) with no distance arithmetic
    in the hot loop.
    """
    out = []
    for md in rings:
...
BBS_OFFSETS = _bbs_ring_offsets(BBS_RINGS)


def _bbs_say(msg):
    """One telemetry line on STDERR, tag "BBS500".  Off in the shipped tree.

    stderr and not stdout on purpose: stdout is the replay channel and is
    STRIPPED from platform-downloaded replays, so a funnel read from stdout is
    a local-only instrument that LOOKS live.  stderr is local-only and says so.
    It is also the channel `_v495healbreak` used for the same job.
    """
    import sys
    print("BBS500 " + msg, file=sys.stderr)
...
class RaidMixin:
    ...
    def _raid(self, ct):
        ...
        if BBS_LOG:
            # RAIDER CHURN AND LIFETIME, and it is emitted from inside the bot
            # because the alternative does not exist: a unit that has been
            # destroyed does not get a turn in which to report its own death.
            # Distinct ids = raiders this game; max(r) - min(r) per id = the
            # lifetime.  One short line per raider per round, LOCAL only.
            _bbs_say("TICK %d %d" % (ct.get_id(), rnd))
        ...
        if self.raid_prev is not None and p.distance_squared(self.raid_prev) > LOKI_TELEPORT_DSQ:
            ...
            self.tgt = None
            self.stuck = 0
            # LOKI-BBSTEP: a throw moved this body, so a step tile latched from
            # the old position is now arbitrarily far and its ray was computed
            # from vision we no longer have.  Same rationale as the trail-memory
            # clear on the next line.
            if BBSTEP_EFF and self.bbs_phase:
                self._bbs_release(rnd, "EXILE")
            self.pave_prev = None
            self.pave_dir = None
            self.pave_rnd = -2
        self.raid_prev = p
        ...
        # ⭐ LOKI-BBSTEP: THE STEP OVERRIDES THE RING STATION.  Placed AFTER
        # `_raid_station` on purpose, so the parent's station bookkeeping -- bans,
        # rescan clock, far-phase slot rotation -- runs byte-for-byte as it does
        # in `_v480beltbreak`; only the destination changes, and only while a
        # latch is live.  On arrival the step tile IS this raider's own tile, so
        # the parent's `p == self.tgt` branch below turns it into a park and step
        # 3b's plant scan runs from the new position with no extra code.
        if BBSTEP_EFF and self.bbs_phase:
            if self.bb_plants > self.bbs_plants_at:
                # THE CONVERSION.  This raider planted while latched -- whether
                # from the stepped tile or from somewhere on the way to it, both
                # of which this plank caused.  Release immediately: the arm is
                # done and the collar gets the body back.
                self.bbs_planted += 1
                self._bbs_release(rnd, "PLANT")
            elif self.bbs_step is None or rnd > self.bbs_until:
                self._bbs_release(rnd, "EXPIRE%d" % self.bbs_phase)
            else:
                if self.bbs_phase == 1 and (p.x, p.y) == self.bbs_step:
                    # ARRIVED.  One un-refreshed dwell, long enough for the
                    # action cooldown to clear and the parent's scan to run from
                    # here -- and no longer, or a stepped raider that turns out
                    # to have a stale ray would be parked out of the raid.
                    self.bbs_phase = 2
                    self.bbs_until = rnd + BBS_DWELL
                    self.bbs_arrived += 1
                    if BBS_LOG:
                        _bbs_say("ARRIVE r%d (%d,%d)" % (rnd, p.x, p.y))
                self.tgt = Position(self.bbs_step[0], self.bbs_step[1])
                self.bbs_moves += 1
        ...
        if stalled:
            # LOKI-BBSTEP: A STALL WHILE WALKING TO A STEPPED TILE IS THE STEP'S
            # FAULT, NOT THE STATION'S.  Drop the latch and return BEFORE the
            # parent's ban logic, or an unreachable step tile would ban a
            # perfectly good ring station and rotate the far-phase slot -- the
            # plank silently corrupting the parent's own station bookkeeping.
            # The cooldown then stops it re-latching the same tile next round.
            if BBSTEP_EFF and self.bbs_phase:
                self._bbs_release(rnd, "STALL")
                self.stuck = 0
                self.nav_fail = 0
                return
            ...
            for c in core_tiles(E):
                ...
                try:
                    if ct.can_fire(c):
                        ct.fire(c)
                        if BBS_LOG:
                            _bbs_say("PECK r%d" % ct.get_current_round())
                        return True
                except Exception:
                    continue
            ...
            for dx, dy in CARD_DELTAS:
                ...
                try:
                    ...
                    if ct.can_build_barrier(t):
                        ct.build_barrier(t)
                        if BBS_LOG:
                            _bbs_say("SEAL r%d" % ct.get_current_round())
                        return True
                except Exception:
                    continue
        ...
        if LOKI_SALTIDLE_ON:
            ...
            if self._salt_idle_ok(ct, E, p, near):
                ...
                if self._salt_turn(ct, E, p, ti):
                    self.si_fire += 1
                    if BBS_LOG:
                        _bbs_say("SALT r%d" % ct.get_current_round())
    ...
    def _try_beltbreak_gunner(self, ct, E):
        ...
        # histogram.
        # LEG 0 (BBSTACK).  `lo - 24` was UNREACHABLE for a squared distance
        # (= -4), so the inner bail was dead and collar raiders at d^2 = 1
        # entered the scan and were counted as NORAY.  See doctrine LOKI-BBSTACK
        # LEG 0 for the derivation, the 55.6x mislabel measurement, and the
        # code-path argument that this changes COUNTING and CPU but not
        # behaviour.  `LOKI_BELTBREAK_LO_BAIL` folds to the parent's -4 when
        # the master flag is off.
        if d < LOKI_BELTBREAK_LO_BAIL or d > hi + 24:
            return False
        ...
        if live >= LOKI_BELTBREAK_CAP_EFF:   # LEG 1 (BBSTACK): 2 -> 3
        ...
        if live > 0:
            # LEG 1 (BBSTACK) sub-flag `BBSTACK_L1_FUND_EFF`.  This is the
            # site where a bigger magazine becomes a DEARER PLANT -- the
            # hidden third effect in the doctrine block.  Folds to the
            # parent's 24 when that sub-flag is off.
            need += LOKI_BELTBREAK_TI_FLOOR + LOKI_BELTBREAK_FUND_AMMO
        ti = ct.get_global_resources()
        if ti < need:
        ...
        blocked_own = False
        # LOKI-BBSTEP reads this and nothing else does.  See `_bbs_probe`: it is
        # the ENGINE-EXACT test for "this raider's NORAY is a RAY problem", i.e.
        # at least one of its own build tiles was actually inside the annulus and
        # got scored, as against "this raider is nowhere near the annulus".
        saw_band = False
        for dx, dy in CARD_DELTAS:
            bx, by = p.x + dx, p.y + dy
            if not (0 <= bx < self.mw and 0 <= by < self.mh):
        ...
        if best is None:
            if blocked_own:
                return self._bb_refuse("OWNRAY")
            # ⭐ LOKI-BBSTEP.  The refusal is counted FIRST and unchanged, so the
            # parent's funnel reads identically in both arms and the probe can
            # never be mistaken for a plant.  `_bbs_probe` takes no action,
            # spends nothing and returns nothing -- it latches a MOVE target and
            # this function still returns False, so `_raid_act` walks on to its
            # steps 4-7 exactly as in `_v480beltbreak`.
            self._bb_refuse("NORAY")
            if BBS_LOG:
                _bbs_say("NORAY r%d at=(%d,%d) d2=%d cands=%d band=%d"
                         % (rnd, p.x, p.y, d, len(cands), saw_band))
            if BBSTEP_EFF:
                try:
                    self._bbs_probe(ct, E, p, d, cands, saw_band)
                except Exception:
                    # A probe is a convenience, never worth a unit.  An escaping
                    # exception from run() destroys this builder PERMANENTLY
                    # (engine 0x1ac5c -> Game::destroy_entity), so the whole
                    # plank sits under one belt-and-braces catch on top of the
                    # per-call ones inside.
                    self.bbs_why["RAISED"] = self.bbs_why.get("RAISED", 0) + 1
            return False
        ...
        self.bb_plants += 1
        if BBS_LOG:
            # The conversion numerator.  `bbs_phase` is the state THIS raider was
            # in when it planted: 0 = the parent's own plant, 1 = planted while
            # walking to a stepped tile, 2 = planted from the stepped tile.
            # 1 and 2 are the only plants this plank can claim.
            _bbs_say("PLANT r%d at=(%d,%d) dsq=%d phase=%d cyc=%d"
                     % (rnd, bp.x, bp.y, dsq_core(bp, E), self.bbs_phase,
                        self.bbs_cycles))
    ...
    def _bb_refuse(self, why, a=0, b=0):
        ...
        if BBS_LOG:
            # UNRATE-LIMITED, unlike the stdout line below, because the refusal
            # funnel is the denominator of this arm's headline and a 1-in-25
            # sample of it is not a denominator.  Identical in both arms (it is
            # gated on BBS_LOG, not on BBSTEP_ON), which is what makes the
            # flag-off run usable as the parent's own funnel.
            _bbs_say("REFUSE " + why)
        if LOKI_BELTBREAK_LOG and n % 25 == 1:
            print("BB48 REFUSE %s n=%d %d/%d" % (why, n, a, b))
        return False

    # --- LOKI-BBSTEP: the step ---------------------------------------------

    def _bbs_probe(self, ct, E, p, d, cands, saw_band):
        """A NORAY refusal, turned into a MOVE toward a tile that HAS a ray.

        Takes NO action, spends NO titanium, builds nothing and returns nothing.
        It latches `self.bbs_step` and lets the parent's own move phase walk
        there.  That is the design and not a shortcut: the refused raider is
        FUNDED and standing still, so the only resource it has left to spend
        this round is its MOVE.  Spending its ACTION here would come out of the
        budget the collar and the sentinel already have first refusal on
        (`_raid_act` steps 1-3), and this plank must never be able to outbid
        them.

        `cands` is REUSED, not recomputed: the caller has already read this
        builder's vision for live enemy HARVESTER / CONVEYOR / SPLITTER, scored
        them off BB_SITE_VALUE and truncated to LOKI_BELTBREAK_MAX_TGT.  So the
        probe adds no vision scan at all -- its whole cost is geometry over a
        list that already exists.

        THE LADDER, and each rung is counted so a null is readable:
          NOBAND -- NOT ONE of this raider's own build tiles was inside
            [LO, HI], so the parent never scored band geometry at all and this
            refusal is not a ray problem.  MEASURED, and it is this arm's
            headline: 1,693 of 1,724 NORAY refusals over a 30-game pilot
            (98.2%), of which 1,469 came from a raider at d^2 = 1 or 2 -- i.e.
            STANDING ON THE COLLAR.  That population is `_v495healbreak`'s
            bench, and pooling the two is how a step arm gets credited with a
            bench arm's work.  The test is `saw_band`, computed by the caller
            from its own band filter, NOT a d^2 threshold here: a raider at
            d^2 = 17 with a neighbour at d^2 = 22 DID have band geometry scored
            and is this plank's business, while an in-band raider whose four
            neighbours all fall outside the band did not.
          LATCH / COOL / CYCLES -- the three bounds.  A live latch suppresses
            probing entirely (so a raider cannot re-aim every round and
            oscillate), a completed cycle bans re-latching for BBS_COOL, and
            BBS_MAX_CYCLES is a hard per-life ceiling.
          CPU -- the shared guard, checked on entry and again inside the loop.
          NOTILE -- no in-band candidate tile even had a target within d^2 13.
          NOSTAND -- a tile had a ray but no passable neighbour to stand on, so
            the plant could never be reached.  Counted, not silently dropped.
          NOFIRE -- tiles were reachable and empty and the engine still put no
            target on any snapped ray.  THIS is the residue that says NORAY is
            irreducible at this radius rather than merely unsearched.
        """
        rnd = ct.get_current_round()
        self.bbs_noray += 1
        lo, hi = LOKI_BELTBREAK_DSQ_LO, LOKI_BELTBREAK_DSQ_HI
        ...
        def why(tag):
            self.bbs_why[tag] = self.bbs_why.get(tag, 0) + 1
            if BBS_LOG:
                _bbs_say("WHY r%d %s d2=%d phase=%d" % (rnd, tag, d, self.bbs_phase))
        ...
        if self.bbs_phase:
            why("LATCH")
            return
        if rnd < self.bbs_cool_until:
            why("COOL")
            return
        if self.bbs_cycles >= BBS_MAX_CYCLES:
            why("CYCLES")
            return
        if self._cpu_exhausted(ct):
            why("CPU")
        ...
        # --- the scan.  Manhattan rings 2..3, nearest ring first.
        best = None                     # ((value, -md), build tile, stand tile)
        tiles = 0
        fires = 0
        nostand = 0
        seen_reach = False
        mw, mh = self.mw, self.mh
        for oi, (dx, dy) in enumerate(BBS_OFFSETS):
            if tiles >= BBS_MAX_TILES or fires >= BBS_MAX_FIRE:
                break
            bx, by = p.x + dx, p.y + dy
            if not (0 <= bx < mw and 0 <= by < mh):
                continue
            bp = Position(bx, by)
            db = dsq_core(bp, E)
            # THE BAND IS APPLIED TO THE BUILD TILE, exactly as the parent
            # applies it, and it is also this plank's displacement bound: a
            # raider that is already in band can only ever step to another
            # in-band tile, so the step cannot walk it out of the annulus.
            if db < lo or db > hi:
                continue
            # Pure arithmetic reach filter -- no engine call.  A target further
            # than the gunner's r^2 = 13 cannot be on any ray from bp.
            reach = []
            for v, tp in cands:
                if bp.distance_squared(tp) <= 13:
                    reach.append((-v, bp.distance_squared(tp), tp))
            if not reach:
                continue
            seen_reach = True
            tiles += 1
            if tiles % 4 == 0 and self._cpu_exhausted(ct):
                why("CPU")
                return
            # LEGALITY PROXY, AND IT IS A PROXY -- STATED.  `can_build_gunner`
            # cannot be used here: it enforces orthogonal adjacency to the
            # CALLING builder and bp is 2-3 tiles away, so it would return False
            # for every candidate and the plank would be inert.  The substitute
            # is the two conditions the engine's own check is made of that we can
            # read at range: the tile is empty (no building, not a wall) and no
            # builder bot is standing on it.  The REAL check still happens -- the
            # parent's scan re-runs `can_build_gunner` on arrival, from adjacency.
            try:
                if not ct.is_in_vision(bp):
                    continue
                if not ct.is_tile_empty(bp):
                    continue
                if ct.get_tile_builder_bot_id(bp) is not None:
                    continue
            except Exception:
                continue
            sp = self._bbs_stand(ct, bp, p)
            if sp is None:
                nostand += 1
                continue
            reach.sort()                # value DESC (stored negated), then near
            md = abs(dx) + abs(dy)
            for nv, dt, tp in reach:
                if fires >= BBS_MAX_FIRE:
                    break
                facing = bp.direction_to(tp)
                if facing == Direction.CENTRE:
                    continue
                ...
                if best is None or key > best[0]:
                    best = (key, bp, sp)
                # One viable facing per tile is enough: this is a MOVE decision,
                # and the parent's own ladder re-scores every facing on arrival.
                break
            if best is not None and best[0][0] >= 100:
                # A harvester ray on the nearest ring that works is the top of
                # the parent's own siting ladder; nothing further out beats it.
                break
        ...
        key, bp, sp = best
        self.bbs_phase = 1
        self.bbs_until = rnd + BBS_HOLD
        self.bbs_step = (sp.x, sp.y)
        self.bbs_aim = (bp.x, bp.y)
        self.bbs_plants_at = self.bb_plants
        self.bbs_latched += 1
        if BBS_LOG:
            _bbs_say("LATCH r%d from=(%d,%d) d2=%d -> stand=(%d,%d) build=(%d,%d)"
                     " v=%d md=%d tiles=%d fires=%d"
                     % (rnd, p.x, p.y, d, sp.x, sp.y, bp.x, bp.y,
                        key[0], -key[1], tiles, fires))

    def _bbs_stand(self, ct, bp, p):
        """The tile this raider must stand on to build on `bp`, or None.

        A gunner is built on an ORTHOGONALLY ADJACENT tile, so the stand tile is
        one of bp's four neighbours; the nearest to `p` is chosen because the
        walk is what this plank pays for.  `p` itself is never a candidate --
        BBS_RINGS starts at 2, so bp is never orthogonally adjacent to the
        raider, which is precisely why the parent's own four-tile scan could not
        see it.

        None is a real outcome and is counted by the caller: a build tile with a
        ray but no passable neighbour is a tile we could never plant from, and
        latching a walk to it would be a bounded waste with no upside.
        """
        me = ct.get_id()
        best, bk = None, None
        for dx, dy in CARD_DELTAS:
            sx, sy = bp.x + dx, bp.y + dy
            ...
            try:
                ...
                if not ct.is_tile_passable(s):
                    continue
                o = ct.get_tile_builder_bot_id(s)
                if o is not None and o != me:
                    continue
            except Exception:
                continue
            k = (abs(sx - p.x) + abs(sy - p.y), sy, sx)
            if bk is None or k < bk:
                best, bk = s, k
        return best

    def _bbs_release(self, rnd, tag):
        """Hand the body back to the ring and start the cooldown.

        Called on expiry, on a plant, on a navigation stall and on an exile.
        Every exit goes through here so the duty-cycle bound cannot be bypassed
        by adding a new exit later: the cooldown and the cycle count are
        incremented in exactly one place.
        """
        self.bbs_phase = 0
        self.bbs_step = None
        self.bbs_aim = None
        self.bbs_cool_until = rnd + BBS_COOL
        self.bbs_cycles += 1
        if BBS_LOG:
            _bbs_say("RELEASE r%d %s cyc=%d" % (rnd, tag, self.bbs_cycles))
