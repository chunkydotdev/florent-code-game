...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # V504 LOKI-BBAMMO: the ammo-floor leg at the SHREDDER'S price,
            # dose-4 SHAPE (BOTH branches) ported from _v501ammofloor/main.py:363-374.
            # Sweep findings honoured: unarmed-branch-only reserves are INERT
            # (dose 1: 0/120 cells changed), so this applies to the armed branch
            # too; the reserve is the CHEAP weapon's refusal threshold
            # (gunner + LOKI_BELTBREAK_TI_FLOOR), NOT the sentinel's 121 -- the
            # sweep disqualified sentinel-price-on-both-branches at -27.5pp
            # timely-kill, and every extra Ti of reserve is paid in shots.
            # Sits BELOW E1 and composes by max(): E1's harvester reserve can
            # never be lowered by this clause. LOKI_BELTBREAK_AMMO is NOT in
            # the reserve (the magazine bump downstream handles the magazine;
            # double-counting it here would starve the tap the bump opens).
            if LOKI_BBAMMO_ON:
                try:
                    ti_floor = max(
                        ti_floor,
