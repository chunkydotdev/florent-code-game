...
T4_BURN_RNDS = 5   # V505: 10->5. THE TARGET AXIS, NOT THE FLOOR AXIS -- the sweep
                   # and three fixtures refuted standing Ti RESERVES (floors), but the
                   # standing ammo TARGET was never manipulated. weapons_top is a
                   # MONOTONE counter that counts rubble (the tree's own ghost-magazine
                   # warning), so a 10-round burn target held for dead turrets converts
                   # titanium nobody will fire. 5 rounds still covers 2.5 sentinel
                   # volleys (reload 2) and 5 gunner shots per counted weapon; conversion
                   # is same-turn usable, so demand arriving late converts late instead
                   # of never. Replay basis: 1,142 Ti/game converted on the carrier at
                   # 36% of all income (the #1 ledger claimant).
T4_AMMO_PER_RND = 4
