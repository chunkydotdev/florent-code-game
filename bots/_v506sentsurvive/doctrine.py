...
# ============================================================================
# LOKI-SENTSURVIVE (v506, 2026-08-17) -- THE REPLANT TREADMILL.
#
# ⛔ EVERY NUMBER IN THIS BLOCK IS RELAYED, NOT MEASURED BY THIS FILE'S AUTHOR.
# The dose figures below arrived in the build brief (coordinator relay of the
# s49 census vs kladde) and the rear-tile figures are read off
# bots/_v498rearsent/doctrine.py:1946-1950, which measures a LOCAL SELF-PLAY
# fixture (480 games, our own bot as opponent).  Per CLAUDE.md point 6 nothing
# here retires anything; it PRIORITISES this plank.  Labelled as relay so a
# later reader does not promote it.
#
# THE PROBLEM.  The forward-sentinel cap is a LIVE census
# (raid.py `_live_fwd_guns`, LOKI2B_LIVE_CAP_ON = True), so a death FREES A SLOT
# and the next funded raider replants immediately -- same scan, same tile class,
# no memory of what just happened there.  Relayed census vs kladde: 94.7% of our
# siege sentinels take a hit within a MEDIAN OF 1 ROUND of being built, 93.2%
# die, median life 6 rounds (~3 shots).  Net we BUILD 11.71/game and HOLD 0.88
# alive; they build 8.11 and hold 7.68.  At late scale a rebuild is 71-102 Ti.
# The tile is pre-covered before the turret exists: we are paying ~85 Ti to feed
# a gunner that is already aimed at that square.
#
# WHAT IS **NOT** THE ANSWER, both closed before this plank was written:
#   * HEALING the sentinel.  Relayed: 28% damage replacement; 0-heal and 3+-heal
#     sentinels died alike.  No heal call is added here.
#   * REFUSING TO PLANT.  Magnus, direct, mid-build: *"we always need to get
#     those sentinels up somewhere."*  A raider sitting on sentinel money beside
#     a sieged core with no sentinel anywhere is a WORSE state than a sentinel
#     that dies in six rounds -- the dead one at least fired ~3 shots.  This is
#     why the mechanism is RE-SITE, NOT REFUSE, and why the primary dose metric
#     is ALIVE-STOCK AT r60/r150/END rather than titanium saved.  A treatment
#     that saves titanium by having fewer sentinels UP is a FAILURE here even if
#     every other column improves.
#
# THE MECHANISM, one sentence: a sentinel that died FAST marks its own tile's
# neighbourhood as covered, and the next plant is STEERED ELSEWHERE -- far side
# first -- while never being cancelled.
#
# THE THREE TERMS AND THEIR RANK (this ordering is Magnus's refinement, not the
# first draft's):
#   (1) THE REDIRECT IS THE PLANK.  On a live ban the four cardinal build
#       candidates are REORDERED far-side-first (SENT_FARSIDE_ON) and banned
#       tiles are skipped.  The plant happens the moment any legal non-banned
#       tile exists and the bank allows -- the parent's cap, harvester
#       prerequisite and bank floor are UNTOUCHED, so this arm can never buy
#       MORE sentinels than the carrier, only different tiles.
#   (2) THE BAN IS STEERING, NOT SUPPRESSION.  d^2 <= SENT_BAN_DSQ around a
#       fast-death tile, expiring after SENT_BAN_RNDS.
#   (3) THE GRACE PERIOD IS A LAST RESORT, NOT A COOLDOWN ON DEATH.  If EVERY
#       legal tile this turn is banned, the arm waits at most SENT_COOL_RNDS
#       rounds and then plants on a banned tile anyway (pass B) -- i.e. the ban
#       is SOFT and the worst case is a bounded DELAY versus the carrier, never
#       a refusal.  SENT_BAN_HARD = True removes pass B and is the FORCED-FAIL
#       variant: it is expected to make zero-sentinel rounds WORSE, which is how
#       the instrument is shown able to see Magnus's failure mode at all.
#
# ⛔ WHAT THIS PLANK DOES **NOT** CLAIM.  The far side is preferred because it is
# MEASURED LESS CROWDED (v498 doctrine item 2: rear plants 21.9% ever-hit vs
# 33.8% approach-side, median rounds to first hit +29 vs +4, died 20.8% vs
# 30.0%).  It is NOT gunner-shielding: v498 item 3 REFUTED that mechanism on 93
# damage events (Gunner signatures on 8-10 of 20 rear deaths -- a defender's
# gunners are not confined to the approach side).  Do not cite shielding.
#
# ⛔ AND THE ONE THING v498 LEARNED THAT THIS ARM DEPENDS ON NOT REPEATING:
# v498's rear plant was a SEPARATE, EARLIER step with its own bank floor, and at
# the parent's LOKI_FWD_TI_FLOOR (= 40) it was INERT -- 16/16 identical replays,
# 296 bank-floor rejections in one game, 0 plants.  It only fired after dropping
# the floor to 8, i.e. by SPENDING MORE than the parent.  This arm does NOT
# touch any floor: the far-side preference is a REORDERING OF THE FOUR TILES THE
# PARENT ALREADY SCANS, inside the parent's own budget, so it fires on exactly
# the rounds the parent would have planted.  Dose comes free; that is the design
# difference from v498 and the reason its 0.40 plants/game dose problem does not
# transfer.
# ============================================================================

# ⛔⛔ READ THIS BEFORE SPENDING A WINDOW ON THIS ARM -- THE LOCAL FIXTURE DOES
# NOT CONTAIN THE PROBLEM.  Measured 2026-08-17, 450 local self-play games
# (150/arm, live 15-map pool x both seats, --tle 10, opponent = a NOISE-off copy
# of _v468kladturbo): we build **1.01 forward sentinels per game**, against the
# **11.71/game** the relayed vs-kladde census reports.  A FAST forward-sentinel
# death -- the plank's only trigger -- occurs in **7 of 150 control games
# (4.7%)**, and the raider must additionally be alive and in vision to see it.
# ⇒ AT LEAST ~92% OF TREATED GAMES ARE THE CARRIER, UNCHANGED.  Every game-level
# difference the battery printed (rounds-with-a-sentinel +27.7, game share
# +6.0pp, timely kill +2.7pp, all point estimates in the success direction, NOT
# ONE interval excluding zero) is larger than that dose can arithmetically
# produce, so those numbers are NOISE_ON divergence and must not be read as the
# plank.  The treadmill this file is named for is a PLATFORM/vs-kladde
# phenomenon; local self-play does not reproduce it.  The next measurement of
# this arm belongs on `fcode match unrated`, not in the arena.
SENT_SURVIVE_ON = False         # the whole plank; False == the carrier exactly
# A death this fast is a PRE-COVERED TILE, not an unlucky turret.  Relayed
# median life is 6 rounds and 93.2% die, so the modal sentinel is inside this
# window by a wide margin.  The ceiling is set at 12 rather than at the median
# because 12 rounds is ~6 sentinel shots (reload 2) = ~108 damage, i.e. a fifth
# of a Core -- past that the turret has arguably returned its 71-102 Ti and its
# tile is not the thing to blame.  It also sits BELOW the rear-plant median life
# of 26 (v498 item 2), so a tile class that is genuinely working never bans
# itself.
SENT_FASTDEATH_RNDS = 12
# The banned neighbourhood.  d^2 <= 8 is exactly the 5x5 block centred on the
# death tile (max |dx|,|dy| = 2 gives d^2 = 8).  Chosen as the SMALLEST
# displacement that can change the answer: a Gunner covers r^2 = 13, so a tile
# one step from a covered tile is almost certainly covered by the same turret,
# while two steps can leave its pattern.  The siege band the parent scans is
# d^2 <= 50 from the enemy Core, ~30+ legal tiles, so one ban removes a small
# fraction of it -- deliberately, per term (2).
SENT_BAN_DSQ = 8
# A ban must EXPIRE: the covering turret can itself be shredded (the carrier's
# own beltbreak plank does that for a living) and a permanent ban would shrink
# the siege band monotonically for the rest of the match.  ⛔ HONEST PROVENANCE:
# 40 is NOT measured.  It is bounded above by "must be far shorter than a game"
# and below by "must outlast one raider's approach walk" (relayed median first
# hit on the approach side is +4 rounds, so anything under ~20 would expire
# before a replacement body has walked back).  A tuning candidate, flagged as
# such.
SENT_BAN_RNDS = 40
# THE GRACE PERIOD, and read term (3) before touching it: this is NOT a
# post-death cooldown.  It applies ONLY on a turn where every legal plant tile
# is banned, and it is the number of rounds the arm will look for an alternative
# before planting on a banned tile anyway.  10 rounds is ~2 sentinel reloads and
# roughly the time a raider needs to walk out of a 5x5 ban, so it is the
# shortest window in which the redirect could plausibly succeed on its own.
# Every round of it is a round with one fewer sentinel up, which is the metric
# this plank is judged on -- so it is a CEILING, not a target.
SENT_COOL_RNDS = 10
# Bounds on per-unit memory, so neither list can grow with the match.  A raider
# is capped at LOKI_FWD_GUN_CAP (= 3) live plants and bans expire, so both
# caps are slack in practice; they exist because an unbounded list on a
# 10 ms/turn budget is a CPU bug waiting for a long game.
SENT_MEM_MAX = 6
SENT_BAN_MAX = 8
# SUB-FLAG: the far-side reordering of the four candidate tiles.  Separable from
# the ban because a ban alone still changes behaviour (it skips tiles), so the
# two terms are NOT inseparable and are not pretended to be.
SENT_FARSIDE_ON = True
# SUB-FLAG: on a fast death, ALSO re-site the BODY -- ban the ring station this
# raider was working and force a station rescan, via the carrier's own exile
# machinery (raid.py:210-217).  ⛔ NOT A SECOND MECHANISM AND NOT SYMMETRY: at
# SENT_BAN_DSQ = 8 a raider standing beside its own dead turret has ALL FOUR
# cardinal candidates inside that turret's ban, so without this the redirect
# CANNOT FIRE and the arm degenerates into "wait SENT_COOL_RNDS, replant on the
# same tile".  Measured before it was added: 9 logged games at
# SENT_FASTDEATH_RNDS = 1000 produced bans, pass-B plants, and `redirected = 0`
# in every one.  A different tile requires a different station.
# ⛔ AND IT DID NOT FIX THE REDIRECT EITHER, stated here because the honest
# record is the point: with it ON, 90 further stderr-logged games (30 each at
# the default threshold, at threshold 1000, and in the forced-fail
# configuration) produced 86 plants, 10 observed deaths, ONE pass-B plant and
# STILL `redirected = 0`.  The redirect branch has never executed in a live
# game.  It is verified only by the stub-controller drive (scratchpad
# ss_unit.py, cell B, both verdicts), which is a code-path check and not
# evidence the plank pays.
SENT_RESITE_BODY_ON = True
# SUB-FLAG: remove pass B, making the ban ABSOLUTE.  This is the FORCED-FAIL
# variant (with SENT_FARSIDE_ON = False) whose job is to make the
# zero-sentinel-rounds metric go the WRONG way.  Never ship True.
SENT_BAN_HARD = False
# Instrumentation only, stderr, LOCAL runs only (platform replays strip stdout
# and stderr never reaches them at all).  Dose tag "SS506".
SENT_SURVIVE_LOG = False
