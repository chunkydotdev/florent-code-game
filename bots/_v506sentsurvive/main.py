...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-SENTSURVIVE (v506), per RAIDER.  Per-unit and not a store
        # slot because all 16 comms slots are already allocated in this tree
        # (see the SLOT_* block in doctrine.py); same precedent as bb_plants
        # above and self.siphon_ban.  Both lists are bounded by SENT_MEM_MAX /
        # SENT_BAN_MAX in raid.py, so neither grows with the match. ---
        self.ss_plants = []        # [(entity_id, Position, build_round)] mine
        self.ss_bans = []          # [(x, y, expiry_round)] fast-death tiles
        self.ss_starve = None      # first round every legal tile was banned
        self.ss_fast = 0           # fast deaths this raider has observed
        self.ss_slow = 0           # slow deaths (no response, by design)
        self.ss_redirect = 0       # plants that a live ban actually moved
        self.ss_late = 0           # pass-B plants (grace expired, ban ignored)
