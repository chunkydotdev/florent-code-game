...
class RaidMixin:
    ...
    def _raid(self, ct):
        ...
        if LOKI6_BEAT_BEFORE_PAUSE and E is not None:
            ...
            try:
                ...
                pass
        # LOKI-SENTSURVIVE: the plant census runs BEFORE every early return in
        # this method, because a raider whose navigation is paused or whose
        # anchor is momentarily unknown is exactly the raider standing next to
        # its own dying turret.  Free when this unit has planted nothing, which
        # is every raider that never reached the band.
        if SENT_SURVIVE_ON and self.ss_plants:
            self._sent_watch(ct, rnd)
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        if self._cpu_exhausted(ct):
            return False
        tiles = core_tiles(E)
        if not (SENT_SURVIVE_ON and self.ss_bans):
            # THE CARRIER PATH, byte for byte.  With no live ban there is
            # nothing to steer around, so the plank must not even reorder the
            # scan -- that is what keeps flag-off equivalence exact AND keeps
            # the flag-ON arm identical to the carrier until the first fast
            # death has actually been observed.
            return self._fwd_plant(ct, E, p, tiles, CARD_DELTAS, live, (), 'A')

        # --- LOKI-SENTSURVIVE: RE-SITE, DON'T REFUSE ------------------------
        bans = tuple((bx, by) for bx, by, _exp in self.ss_bans)
        deltas = self._sent_farside_order(p, E) if SENT_FARSIDE_ON else CARD_DELTAS
        built, ban_blocked = self._fwd_plant(ct, E, p, tiles, deltas, live, bans,
                                             'A', ret_blocked=True)
        if built:
            self.ss_starve = None
            return True
        if not ban_blocked:
            # Nothing was refused BY A BAN -- the parent would not have planted
            # either (wrong tile, occupied, no legal facing).  Not starvation,
            # so the grace clock is not started and pass B is not owed.
            return False
        rnd = ct.get_current_round()
        if self.ss_starve is None:
            self.ss_starve = rnd
        if SENT_BAN_HARD or rnd - self.ss_starve < SENT_COOL_RNDS:
            return False
        # PASS B -- THE ANTI-REFUSAL CLAUSE.  Every legal tile has been banned
        # for SENT_COOL_RNDS rounds and Magnus's rule binds: a sentinel up on a
        # bad tile beats no sentinel at all.  The ban is soft; the worst case
        # versus the carrier is a bounded DELAY.
        if self._fwd_plant(ct, E, p, tiles, CARD_DELTAS, live, (), 'B'):
            self.ss_starve = None
            self.ss_late += 1
            return True
        return False

    def _fwd_plant(self, ct, E, p, tiles, deltas, live, bans, tag,
                   ret_blocked=False):
        """The carrier's forward-sentinel scan, with a tile filter and an order.

        Extracted verbatim from `_try_forward_sentinel` (v488 raid.py:719-747):
        same four cardinal candidates, same d^2 <= 32 alignment test, same
        `can_fire_from` + `can_build_sentinel` pair, same SLOT_FWD_GUN write.
        The ONLY additions are `bans` (tiles skipped) and `deltas` (the order
        they are tried in), both empty/identity on the carrier path.

        Returns True/False, or (built, ban_blocked) when ret_blocked -- where
        ban_blocked means "a tile that would otherwise have been PLANTABLE was
        skipped because it was banned", which is the only condition that can
        owe pass B a plant.
        """
        blocked = False
        for dx, dy in deltas:
            ...
            banned = False
            for kx, ky in bans:
                if (bx - kx) * (bx - kx) + (by - ky) * (by - ky) <= SENT_BAN_DSQ:
                    banned = True
                    break
            bp = Position(bx, by)
            for target in tiles:
                ...
                try:
                    ...
                    continue
                if banned:
                    # A LEGAL TILE REFUSED BY A BAN.  Recorded, then the scan
                    # continues to the next candidate -- the redirect.
                    blocked = True
                    break
                sid = ct.build_sentinel(bp, facing)
                ...
                if SENT_SURVIVE_ON:
                    self._sent_record(ct, sid, bp, bans, blocked, tag)
                return (True, blocked) if ret_blocked else True
        return (False, blocked) if ret_blocked else False

    def _sent_record(self, ct, sid, bp, bans, blocked, tag):
        """Remember a plant so its death can be attributed to its tile.

        Wrapped whole: `build_sentinel` returns an int by contract, but this is
        bookkeeping and an exception escaping run() destroys the unit
        permanently -- there is no bookkeeping worth a body.
        """
        try:
            if sid is None:
                return
            self.ss_plants.append((sid, bp, ct.get_current_round()))
            if len(self.ss_plants) > SENT_MEM_MAX:
                del self.ss_plants[0]
            if blocked:
                self.ss_redirect += 1
            if SENT_SURVIVE_LOG:
                import sys
                print("SS506 PLANT r%d unit=%d id=%d bp=%d,%d pass=%s "
                      "bans=%d redirected=%d cost=%d"
                      % (ct.get_current_round(), ct.get_id(), sid, bp.x, bp.y,
                         tag, len(bans), int(blocked), ct.get_sentinel_cost()),
                      file=sys.stderr)
        except Exception:
            return

    def _sent_farside_order(self, p, E):
        """The four cardinal candidates, FAR SIDE OF THEIR CORE FIRST.

        THE GEOMETRY IS PORTED FROM bots/_v498rearsent/raid.py:767-779 (read
        line by line, not cited from its report): s is the SIGN VECTOR of
        (their anchor - our anchor); the Core centre in DOUBLED coordinates is
        (2E.x+1, 2E.y+1); a tile's rearness is its doubled offset from that
        centre projected on s.  v498 used that projection as a HARD FILTER
        (>= LOKI_REARSENT_MIN_DOT) in a separate, earlier step and needed its
        own lowered bank floor to fire at all; here it is only a SORT KEY on
        tiles the carrier already scans, so no tile becomes illegal, no floor
        moves, and a near tile is still planted when no far tile works.  That
        is the whole difference and it is why this arm is not v498.
        """
        ...
        if not (sx or sy):
            return CARD_DELTAS
        cx2, cy2 = 2 * E.x + 1, 2 * E.y + 1
        return tuple(sorted(
            CARD_DELTAS,
            key=lambda d: -((2 * (p.x + d[0]) - cx2) * sx
                            + (2 * (p.y + d[1]) - cy2) * sy),
        ))

    def _sent_watch(self, ct, rnd):
        """Did one of MY forward sentinels just die, and how fast?

        THE THREE-WAY DISCRIMINATION THIS NEEDS, because two of the three look
        identical from a builder's seat:
          ALIVE     -- get_hp(id) returns a positive number.
          BLIND     -- get_hp raised (or reads dead) but the tile is NOT in
                       vision.  Left recorded, NOT counted as a death: a raider
                       that walked away must not ban a turret that is still
                       shooting.  `is_in_vision` is the safe probe -- it returns
                       False off-map where get_tile_* RAISE.
          DEAD      -- in vision and the tile no longer holds that id.
        Every engine call is inside a try: an uncaught exception out of run()
        destroys this unit for the rest of the match.
        """
        keep = []
        for eid, pos, r0 in self.ss_plants:
            try:
                hp = ct.get_hp(eid)
                alive = hp is not None and hp > 0
            except Exception:
                alive = False
            if alive:
                keep.append((eid, pos, r0))
                continue
            try:
                if not ct.is_in_vision(pos):
                    keep.append((eid, pos, r0))          # BLIND, not dead
                    continue
                if ct.get_tile_building_id(pos) == eid:
                    keep.append((eid, pos, r0))          # still standing
                    continue
            except Exception:
                keep.append((eid, pos, r0))
                continue
            life = rnd - r0
            if life >= SENT_FASTDEATH_RNDS:
                # A SLOW DEATH CHANGES NOTHING, on purpose: the turret earned
                # its keep and its tile is not the thing to blame.
                self.ss_slow += 1
            else:
                self.ss_fast += 1
                self.ss_bans.append((pos.x, pos.y, rnd + SENT_BAN_RNDS))
                if len(self.ss_bans) > SENT_BAN_MAX:
                    del self.ss_bans[0]
                if SENT_RESITE_BODY_ON:
                    # ⛔ MEASURED NECESSITY, not symmetry.  With SENT_BAN_DSQ = 8
                    # every tile a STATIONARY raider can build on is inside the
                    # ban of its own dead turret (the raider stands adjacent to
                    # it, so all four of its cardinal candidates are within
                    # d^2 = 4 of the death tile).  Logged 9 games at
                    # SENT_FASTDEATH_RNDS = 1000: bans formed, pass B fired, and
                    # `redirected` was 0 EVERY TIME -- i.e. the redirect, which is
                    # the plank, could not fire at all while the body did not
                    # move.  A DIFFERENT TILE REQUIRES A DIFFERENT STATION.
                    # This is the carrier's OWN exile machinery, called with the
                    # same three writes it uses at raid.py:210-217 when a
                    # launcher throw proves a station is covered: ban the
                    # station, drop it, force a rescan.  Nothing new is invented
                    # and no navigation code is touched.
                    if self.raid_station is not None:
                        self.raid_ban[(self.raid_station.x,
                                       self.raid_station.y)] = rnd + SENT_BAN_RNDS
                    self.raid_station = None
                    self.raid_rescan = rnd
            if SENT_SURVIVE_LOG:
                try:
                    import sys
                    print("SS506 DEATH r%d unit=%d id=%d pos=%d,%d life=%d "
                          "fast=%d bans=%d"
                          % (rnd, ct.get_id(), eid, pos.x, pos.y, life,
                             int(life < SENT_FASTDEATH_RNDS), len(self.ss_bans)),
                          file=sys.stderr)
                except Exception:
                    pass
        self.ss_plants = keep
        if self.ss_bans:
            live_bans = [b for b in self.ss_bans if b[2] > rnd]
            if len(live_bans) != len(self.ss_bans):
                self.ss_bans = live_bans
                if not live_bans:
                    # Nothing left to steer around: the grace clock is void.
                    self.ss_starve = None
