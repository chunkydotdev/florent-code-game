...
# ============================================================================
# LOKI-RAYDISCIPLINE (v507, s48) -- RAY DISCIPLINE ON THE FORWARD SENTINEL.
#
# THE LEDGER FACT THIS ANSWERS (research, s48).  The ammo pipeline is 94%
# pass-through: `ammo_converted - ammo_end == 4*gunner_shots + 10*sentinel_shots`
# holds exactly over 240/240 series, overhead ratio 1.06.  So the "36% of
# titanium goes to ammunition" ledger line IS the firing bill, and the only way
# to spend less without firing less is to make each shot worth more.
#
# WHERE THE WASTE CONCENTRATES.  A sentinel core-plink is a 28-shot /
# 280-ammo proposition (core 500 HP, sentinel dmg 18).  A belt tile is 2 shots
# (harvester 30 HP) or 2 shots (conveyor/splitter 20 HP).  Kladde aims 19.2% of
# its sentinel shots at cores; we aim 94.0%.
#
# THE MECHANISM, AND IT NEEDS NO NEW ENGINE FACT.  A sentinel's shot is a
# single-tile-wide line along a FIXED facing that IGNORES obstacles
# (docs/reference/official-docs.md:257, quoted: "never blocked by walls or units
# in the way"; "Facing is fixed at build time; Sentinels cannot rotate").
# `_turret` (main.py) therefore ALREADY scans the whole ray -- it walks
# `get_attackable_tiles()` and takes the best `TURRET_PRIO`.  The 94% is not a
# targeting bug, it is one dict row: `TURRET_PRIO[CORE] = 0`, above
# HARVESTER = 5 and CONVEYOR/SPLITTER = 6.  This plank is a REORDER of that
# ladder for FORWARD sentinels only, not a new scan and not a new call.
#
# ⛔ THE HP GATE THE BRIEF ASKED FOR IS VACUOUS, AND SAYING SO IS THE POINT.
# "divert only to an economy building killable in <= 2 shots" cannot ever
# REFUSE: 2 sentinel shots = 36 damage, and harvester/conveyor/splitter MAX HP
# are 30/20/20.  Healing cannot exceed max HP, so no enemy economy building can
# ever sit above the bar.  The gate is kept (it is the correct expression of the
# intent, and it WOULD bind if the engine ever raised belt HP or lowered
# sentinel damage) but it is DEAD CODE today and must not be reported as a
# control that was driven both ways.  What actually decides is the LADDER plus
# the BUDGET below.
#
# WHY A VALUE LADDER AND NOT A BARE GATE.  With the gate vacuous, a bare gate
# would pick among candidates by scan order -- and `get_attackable_tiles()` is
# ROW-MAJOR IN ABSOLUTE COORDINATES, the exact hazard main.py:962-966 already
# warns about (a "first hit wins" scan engages the farthest enemy at four
# facings and the nearest at the other four).  So the choice among candidates is
# made by BB_SITE_VALUE -- HARVESTER 100 > CONVEYOR/SPLITTER 40, the s48
# beltbreak siting ladder, reused verbatim so the two arms cannot disagree about
# what a belt tile is worth -- then by LOWEST HP (finish the wounded one: a
# 20 HP conveyor left at 2 HP still delivers, and heal is +4/round), then by
# (x, y) so the pick is deterministic and facing-independent.
#
# THE TWO RAILS THAT PROTECT THE KILL CLOCK, and they are the whole safety
# argument.  Diversion is a REAL cost in the currency: `R1000_IS_DEFEAT` and
# `DEFENCE_ADMISSION_BAR: r300_crossing_non_regression` mean the core plink is
# what scores and the belt kill is instrumental.  Unbounded diversion is not a
# tuning risk, it is a LOSING policy -- study 3.4 measured the field rebuilding
# a killed belt tile in a median of 2 rounds, i.e. a belt on our ray is a
# PERPETUAL target, so "prefer economy" with no bound is "never shoot the core
# again".  Therefore:
#   1. BUDGET, PER UNIT PER LIFE (not per round, not a duty cycle).  A forward
#      sentinel may spend LOKI_RAYDISC_BUDGET shots on economy in its entire
#      life and then reverts to the parent ladder forever.  Worst case is
#      arithmetic, not a hope: 6 shots x reload 2 = 12 rounds of core delay per
#      forward sentinel, hard.  Precedent in this same tree:
#      LOKI_BELTBREAK_MAX_ROT = 1 per life, which is what makes an A->B->A
#      oscillation impossible BY CONSTRUCTION rather than by cooldown tuning.
#      Same shape, same reason -- a per-unit counter that never resets.
#   2. ENDGAME STOP.  Once the enemy core is within LOKI_RAYDISC_CORE_HP_STOP
#      of death, every shot goes to the core regardless of budget.  The last
#      five shots of a siege are the WIN and no belt tile prices against that.
#
# WHAT IS DELIBERATELY NOT TOUCHED, and each exclusion is enforced by a
# condition and not by intent:
#   * GUNNERS -- `turret_type == EntityType.SENTINEL`.  Beltbreak gunners
#     already leave `_turret` at main.py:941 before this code is reached.
#   * HOME SENTINELS -- `_rd_forward` requires d^2 <= LOKI_RAYDISC_FWD_DSQ of
#     the ENEMY anchor AND d^2 > HUNT_BAND_DSQ of OUR OWN anchor.  The second
#     half is not decoration: maps run from 8x8, so on a small board a home
#     defender satisfies the enemy-core band as pure geometry (the same seam
#     `_bb_in_band` documents at main.py:1021).
#   * ANY RAY WHOSE WINNER IS NOT A CORE.  The diversion fires only when the
#     parent scan already chose a CORE tile (best_prio == 0).  An enemy
#     SENTINEL (1), GUNNER (2), BUILDER_BOT (3) or LAUNCHER (4) on the ray
#     still outranks economy exactly as before -- untouched.
#
# ⛔ NOT A NEW ENGINE FACT, AND NOT A NEW COST.  Ammo is 10/shot flat and the
# reload is 2 rounds regardless of what the shot hits (official-docs.md:250-255
# table, quoted above) -- a belt shot and a core shot are the same purchase.
# The `can_fire(t)` predicate on every tile is the parent's own gate and is
# unchanged; nothing here fires at a tile the parent would have called illegal.
#
# THE NAMED FAILURE MODE, pre-registered: GEOMETRIC STARVATION.  A forward
# sentinel is built facing a CORE TILE (raid.py:719-738 iterates `core_tiles(E)`
# and takes the first facing that `can_fire_from` accepts), so its ray is a
# core-facing line and their belt has no reason to lie on it.  If a forward
# sentinel's ray essentially never crosses an economy building, the arm is
# starved by geometry and cannot dose no matter how the ladder is written.
# `LOKI_RAYDISC_LOG` exists to measure exactly that -- it stamps `es=<n>`, the
# number of ELIGIBLE economy tiles seen on the ray, on EVERY sentinel fire in
# BOTH arms, so the control measures the opportunity even though it never takes
# it.
# ============================================================================
...
LOKI_RAYDISC_FWD_DSQ = 50       # forward test (d^2 of the ENEMY anchor).  50 is
                                # not a new number: it is the same bound
                                # _try_forward_sentinel uses to decide a site
                                # could align at all (raid.py:715).
LOKI_RAYDISC_MAX_HP = 36        # 2 sentinel shots x 18 dmg.  VACUOUS today --
                                # see the block above.  Kept as the intent.
LOKI_RAYDISC_BUDGET = 6         # economy shots per forward sentinel PER LIFE.
                                # 6 = three belt kills = 12 rounds of core delay
                                # worst case.  Never resets.
LOKI_RAYDISC_CORE_HP_STOP = 90  # 5 sentinel shots.  At or below this the core
                                # takes every shot, budget or no budget.
LOKI_RAYDISC_LOG = False        # LOCAL instrument, stderr, one line per sentinel
                                # fire.  OFF in this tree: platform replays strip
                                # stdout and stderr is console-only, so this is a
                                # battery-copy switch, never a shipped one.
                                # Dose tag "RD508".

# ============================================================================
# LOKI-RAYDISC-FF (v508, s48) -- THE FRIENDLY-FIRE GUARD ON THE DIVERSION.
#
# ⛔ THE MEASURED DEFECT, not an inference.  n=630 noise-on battery of v507 vs
# the carrier, engine-verified on 133/133 confirmed diversion shots with a
# 0/133 placebo: the diversion picks an economy BUILDING tile, but THE ENGINE
# RESOLVES A TILE SHOT AGAINST THE BUILDER BOT STANDING ON THAT TILE -- either
# team -- AHEAD OF THE BUILDING.  Of those 133 shots:
#     69.2% hit the intended building
#      9.0% hit an ENEMY builder standing on it
#     21.8% put 18 damage into OUR OWN builder (40 max HP)
# Effective economy dose 0.736/game against 1.063 FIRED.  Worked case: g503
# r331-335, three -18s into our own id9 at (25,27) while the enemy conveyor on
# that tile sat untouched.  ⇒ better than a fifth of the plank's shots are a
# self-inflicted 18-damage hit on a 40 HP unit AND a wasted 10 ammo AND a
# forgone core plink.  This is a MECHANISM defect in v507's target selection,
# not a tuning question.
#
# THE GUARD, and it is one condition.  A tile is refused as a DIVERSION
# candidate when a builder bot of OUR OWN team stands on it.  The id is already
# in hand -- the parent ladder reads `get_tile_builder_bot_id(t)` for every
# attackable tile (main.py, the `bot` local) -- so the only new engine call is
# `get_team(bot)` on an id the engine just handed us.  It cannot be off-map and
# it cannot be off-vision: the tile came out of `get_attackable_tiles()` and the
# sentinel's vision r^2 == its attack r^2 == 32, so every tile the parent
# already queried twice is in vision by construction.  The read still sits in
# its own try, and A FAILED READ REFUSES THE CANDIDATE -- the conservative
# direction, because the failure we are removing is friendly fire.
#
# ⭐ THE ENEMY-BUILDER CASE IS ALLOWED, DELIBERATELY, and this is the ruling.
# 18 damage into a 40 HP enemy builder is not waste: two shots kill it, and
# TURRET_PRIO already ranks an enemy BUILDER_BOT (3) as a legitimate target
# ABOVE any economy building (5/6) -- so a shot that "misses" the conveyor and
# hits their builder has landed on something the parent ladder would have
# preferred anyway.  Refusing that class would cost real damage to buy a
# cleaner-looking dose number.  It IS counted separately (`eb=` in the log) so
# the economy dose is never inflated by it: an enemy-builder hit is a HIT, not
# an economy hit.
#
# WHAT IS NOT TOUCHED, enforced by structure and not by intent:
#   * THE PARENT LADDER.  The guard suppresses only the rd candidate's
#     SELECTION.  The refused tile still contributes its TURRET_PRIO to `best`
#     exactly as in v507 -- there is no `continue`, because skipping the tile
#     outright would silently drop an enemy conveyor from the parent's fallback.
#   * `es=` (eligible economy tiles on the ray), which still counts shielded
#     tiles.  It measures OPPORTUNITY, and a tile our own builder happens to be
#     standing on this round is opportunity that geometry gave us; hiding it
#     would make the starvation falsifier read as geometry when it was traffic.
#   * BUDGET / ENDGAME STOP / FORWARD TEST / GUNNERS -- unchanged.
# ============================================================================

LOKI_RAYDISC_FF_GUARD = True    # sub-flag.  False == _v507raydiscipline exactly
                                # (with LOG off, the occupancy read is not even
                                # made).  Refuses a diversion candidate whose
                                # tile carries one of OUR builder bots; enemy
                                # builders are ALLOWED and counted apart.

# ============================================================================
# LOKI-SALTRAY (v509, s50) -- THE SALT x RAY COORDINATION ARM.
#
# ⛔ THE COLLISION v508 MEASURED, not an inference.  The FF guard closed the
# friendly-fire defect cleanly (own-builder hits 0.429/game -> 0.000, 0/190,
# force-False restores 90/260 and 29 self-killed builders).  But it SUPPRESSES
# rather than REDIRECTS: effective economy dose fell 0.695 -> 0.517/game,
# because the shielded tile is usually the ONLY eligible tile on the ray
# (`es=1` throughout), so a refused diversion falls back to the CORE.
#
# ⭐ WHY THE TILE IS OCCUPIED, and it is the finding this arm exists for: THE
# BUILDER STANDING ON THE ENEMY BELT TILE IS OUR OWN SALT RAIDER.  Salting and
# pecking ARE "stand on their belt".  Two SHIPPED planks are interfering --
# LOKI-SALTIDLE puts our bodies exactly where LOKI-RAYDISCIPLINE wants to
# shoot.  The 21.8% (v507) and 34.6% (v508 refusal rate) figures are that same
# overlap measured twice, once as damage and once as refusals.
#
# ⇒ SO THE FIX IS NOT A RE-TUNE OF THE GUARD.  The guard is correct and stays
# byte-for-byte intact.  What is missing is a CHANNEL: the sentinel knows which
# tile it wants and cannot move; the raider can move and does not know.  This
# plank is one publish and one step-off.
#
# THE MECHANISM, and it needs no new engine fact.
#   PUBLISH (sentinel side, main.py `_turret`).  When the FF guard has refused
#   at least one candidate AND the diversion is therefore about to fall back to
#   the core (rd_arm on, best_prio == 0, no surviving rd_best, endgame stop not
#   reached), the sentinel writes the blocked tile plus its own FACING plus a
#   round stamp into SLOT_RAY_YIELD.  It still fires the core that round -- the
#   publish costs nothing and forfeits nothing.
#   YIELD (raider side, raid.py `_raid`).  A raider whose own position IS the
#   published tile steps off, preferring a neighbour that is NOT also on the
#   publishing ray.  Then it HOLDS (acts but does not move) for
#   LOKI_SALTRAY_HOLD rounds, which is what makes re-entry impossible BY
#   CONSTRUCTION rather than by hoping the navigator picks another route --
#   the same shape as LOKI_BELTBREAK_MAX_ROT = 1 per life.
#
# ⛔ THE ONE-ROUND WRITE BUFFER IS LOAD-BEARING AND IS RESPECTED.  A store write
# is visible only from the NEXT round, so a tile published on round R is read
# by the raider on R+1 and the tile is clear from R+1 (raider's turn) or R+2
# (if the sentinel's turn precedes the raider's).  Sentinel reload is 2, so the
# yield lands inside one reload cycle.  The stamp exists so a raider can tell a
# LIVE request from a stale one; LOKI_SALTRAY_STALE bounds it.
#
# THE SLOT, and it is reclaimed, not invented.  All sixteen slots are declared.
# SLOT_ECO_READY (5) is WRITTEN in three places (main.py, eco.py x2) and READ
# BY NOBODY -- grep `read_store(SLOT_ECO_READY)` returns nothing in any
# ancestor of this file.  It is provably dead, exactly like SLOT_LINKS_DONE
# (slot 9) and slot 13 before it, and it is reclaimed the same way: the three
# dead writes are gated OFF under this flag so the slot has ONE writer class
# (forward sentinels).  With the flag OFF the dead writes are restored and the
# tree is _v508raydisc2 exactly.
#
# THE ENCODING.  One int, because a tile plus a facing plus a stamp is 26 bits
# and maps are at most 30x30 (CLAUDE.md).  Deltas rather than a Direction enum
# index so no ordering assumption about the enum can ever go stale:
#     bits  0-5   x        (0..63)
#     bits  6-11  y        (0..63)
#     bits 12-13  dx + 1   (0..2)
#     bits 14-15  dy + 1   (0..2)
#     bits 16+    round+1  (1..1001)   0 == no request, and rnd+1 >= 1 always
# Max value ~65.6e6, well inside an i32.
#
# WHY THE STEP-OFF PREFERS AN OFF-RAY NEIGHBOUR, and why "off-ray" is
# computable at all.  A sentinel's shot is a single-tile-wide line along a
# FIXED facing, so the ray through the published tile is {tile + k*(dx,dy)}.
# For a CARDINAL facing exactly two of the four cardinal steps leave that line
# (dx == 0 -> leave the column; dy == 0 -> leave the row); for a DIAGONAL
# facing all four leave it.  Without the facing the test is impossible -- every
# cardinal neighbour of a tile shares its row or its column -- which is why the
# facing is in the packet and not just the tile.
#
# WHAT THIS COSTS THE SALT PLANK, stated before it is measured.  The yield
# outranks `_raid_act`, so a yielding raider forfeits that round's peck / seal /
# heal / salt, and the HOLD forfeits movement for a few rounds afterwards.  That
# is a REAL cost to a shipped plank and it is why salt occupancy is reported
# ON vs OFF rather than assumed harmless.
#
# WHAT IS DELIBERATELY NOT TOUCHED:
#   * THE FF GUARD.  Untouched.  A refusal is still a refusal; the sentinel
#     never fires into our own builder, this round or any round.
#   * THE DIVERSION CHOOSER.  Already prefers an unshielded tile by
#     construction -- a refused candidate is excluded from `rd_best` entirely,
#     so a lower-value clean tile wins over a higher-value shielded one
#     (main.py, `if not ff and (rd_key is None or key > rd_key)`).  The es>1
#     half of the brief is therefore ALREADY SHIPPED in v508 and needs no edit;
#     the measured reality is es=1, so the yield is the load-bearing part.
#   * BUDGET / ENDGAME STOP / FORWARD TEST / GUNNERS / the parent ladder.
# ============================================================================

SALTRAY_COORD_ON = True         # master switch; False == _v508raydisc2 exactly
                                # (no publish, no yield, and SLOT_ECO_READY's
                                # three dead writes restored).
SLOT_RAY_YIELD = SLOT_ECO_READY  # 5.  Reclaimed: written by three call sites in
                                # every ancestor and read by none.  Under this
                                # flag the sole writer class is the FORWARD
                                # SENTINEL, so last-write-wins across two
                                # sentinels in one round costs at most one
                                # round of one request, never a corrupt read.
LOKI_SALTRAY_STALE = 3          # a request older than this many rounds is
                                # ignored.  Covers the one-round write buffer
                                # plus a sentinel reload (2) with one round of
                                # slack, and bounds how long a raider can be
                                # displaced by a sentinel that has since died.
LOKI_SALTRAY_HOLD = 4           # rounds a raider refuses to MOVE after stepping
                                # off (it may still act).  Two sentinel reloads:
                                # long enough for the diversion to fire twice
                                # (2 shots kill any belt tile), short enough that
                                # a raider is not parked off-station.  Makes
                                # re-entry impossible by construction rather
                                # than by trusting the navigator.
LOKI_SALTRAY_MAX_BREAK = 2      # allied BARRIERS one raider may demolish to
                                # escape a cage, PER LIFE, never reset.  See
                                # the CAGE block below.  2 = the raider can
                                # swap seats twice in a whole match; a
                                # demolition loop against our own collar is
                                # impossible by construction, not by tuning.
LOKI_SALTRAY_LOG = False        # LOCAL instrument, stderr, tags "SR509PUB"
                                # (sentinel published) and "SR509YLD" (raider
                                # stepped off) / "SR509HOLD".  OFF in this tree:
                                # the platform strips stdout and stderr is
                                # console-only, so this is a battery-copy switch,
                                # never a shipped one.

# ============================================================================
# ⭐⭐ THE CAGE -- MEASURED AT BUILD TIME (s50, 54 games vs _v488beltbreak2,
# frostgate..yulerune rotation, NOISE off, --tle 10), AND IT OVERTURNED THIS
# ARM'S OWN DESIGN BEFORE THE ARM WAS FIRED.
#
# The plank as briefed was "the raider standing on the ray steps aside".  At
# every event where one of our raiders stood on a tile a forward Sentinel had
# just published -- 204 events over 54 games -- the raider's move cooldown was
# 0 and `can_move` was FALSE IN ALL FOUR CARDINAL DIRECTIONS in 202 of them
# (99.0%).  Two events had a legal step.  ⇒ A STEP-OFF PLANK WOULD HAVE FIRED
# TWICE IN 204 CHANCES AND MEASURED AS A NULL ABOUT COORDINATION, WHICH IT IS
# NOT ABOUT.
#
# WHAT THE CAGE IS, read off the tiles (the same probe dumped env / building
# team / building type / can_destroy for all four neighbours at every event):
#     561 tiles  OUR OWN BARRIER, can_destroy TRUE on every one
#     204 tiles  the ENEMY CORE  (exactly one per event -- 204 of 204)
#      47 tiles  WALL
#       4 tiles  passable / enemy conveyor
# i.e. the shield raider is parked on a HEAL SEAT of the enemy Core, walled in
# by OUR OWN LOKI_BARRIER_SEAL collar on the other seats and by the Core
# itself.  Nobody put it there by accident: seats are where the collar doctrine
# sends raiders and where the seal doctrine puts barriers.
#
# ⭐ AND THE TILE UNDER THE RAIDER IS THE ENEMY'S DELIVERY TILE.  A seat is
# orthogonally adjacent to their Core, so the enemy economy building our
# Sentinel wants to shoot there is the LAST conveyor of their trunk -- the tile
# their whole harvest passes through.  That is the highest-value belt kill on
# the board, and v508's guard was declining it every single reload for the rest
# of the match (the same tile republished at r187, 189, 191, 193, ... in the
# worked game).  The 0.695 -> 0.517 dose loss is not diffuse; it is this.
#
# ⇒ THE ACTUATOR HAD TO CHANGE, AND THE CHEAP ONE IS ALREADY IN THE API.
# `destroy` takes an ALLIED building on an orthogonally adjacent tile, is free,
# has no cooldown, and every one of the 561 caging tiles answered
# `can_destroy == True`.  So the raider SWAPS PLACES WITH ITS OWN BARRIER: it
# destroys one seat-barrier and steps into that seat.  What it keeps: a seat
# (denied by its body instead of by a barrier, which raid.py step 1 already
# treats as equivalent), adjacency to the Core, and its peck.  What it spends:
# one 3 Ti barrier, which the seal step rebuilds.  What we gain: the delivery
# tile is clear and the Sentinel kills it in 2 shots -- and the corpse is then
# a legal SALT target for the same raider, which is the two planks finally
# pulling the same way instead of against each other.
#
# ⭐ AN ENGINE FACT THIS ARM MEASURED, WORTH KEEPING: `destroy` DOES NOT SPEND
# THE MOVE.  The API says destroy is free, has no cooldown and is unlimited per
# turn, while acting and moving are mutually exclusive -- which leaves it open
# whether a builder that has just destroyed may still step.  It may: over the
# s50 battery the escape destroyed a barrier and MOVED INTO IT IN THE SAME TURN
# 18 times out of 18.  The code still handles the other world (it returns
# turn-consumed and takes the step next round), but that branch has never
# fired here and is not claimed as verified.
#
# BARRIERS ONLY, AND BOUNDED PER LIFE.  `EntityType.BARRIER` is checked
# explicitly -- a conveyor, splitter, harvester or turret of ours is never a
# candidate no matter what `can_destroy` says -- and LOKI_SALTRAY_MAX_BREAK
# caps demolitions per raider per LIFE with no reset.
# ============================================================================
