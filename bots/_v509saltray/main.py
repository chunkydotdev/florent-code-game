...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-RAYDISCIPLINE (v507), per SENTINEL.  rd_econ is the
        # per-life economy-shot budget and NEVER resets -- that is what bounds
        # the kill-clock cost arithmetically instead of by hope (doctrine
        # LOKI-RAYDISCIPLINE block, rail 1).  rd_fwd latches TRUE only, exactly
        # as bb_seen does: the anchor in SLOT_ENEMY_CORE can be a GUESS early
        # (main.py:246 seeds it by reflection before anyone has SEEN a core),
        # so a False must stay revisable while a True cannot go stale -- a
        # turret is a building and neither it nor the anchor can move. ---
        self.rd_econ = 0
        self.rd_fwd = False

        # --- LOKI-RAYDISC-FF (v508), per SENTINEL.  rd_ff counts diversion
        # candidates REFUSED because one of our own builder bots stood on the
        # tile (the engine resolves a tile shot against the bot ahead of the
        # building -- 21.8% of v507's 133 confirmed diversions were an 18-damage
        # self-hit); rd_eb counts candidates ALLOWED with an ENEMY builder on
        # the tile, which is a hit and not waste but must never be counted as an
        # ECONOMY hit.  See the LOKI-RAYDISC-FF block in doctrine.py. ---
        self.rd_ff = 0
        self.rd_eb = 0

        # --- LOKI-SALTRAY (v509).  Per SENTINEL: sr_pub counts yield requests
        # published (a refusal that actually cost a diversion).  Per RAIDER:
        # sr_yld counts step-offs taken and sr_hold_until is the round the
        # movement freeze expires -- the freeze is what makes re-entry
        # impossible BY CONSTRUCTION rather than by trusting the navigator.
        # One Player instance per unit, so the same two names never collide. ---
        self.sr_pub = 0
        self.sr_yld = 0
        self.sr_hold_until = -1
        # sr_break: allied BARRIERS this raider has demolished to get out of a
        # cage, PER LIFE, never reset -- bounded by LOKI_SALTRAY_MAX_BREAK.
        self.sr_break = 0
    ...
    def _core(self, ct):
        ...
        # LOKI-SALTRAY: slot 5 is reclaimed as SLOT_RAY_YIELD.  This write is
        # one of three that no ancestor of this file ever READ (see the
        # LOKI-SALTRAY doctrine block); it is preserved verbatim under the flag
        # so SALTRAY_COORD_ON = False is _v508raydisc2 exactly.
        if harv >= ECO_NEED and not SALTRAY_COORD_ON:
        ...
        if not endgame_dumped:
            ...
            if (under or weapons_top or bb_live or harv >= 2) \
    ...
    def _turret(self, ct):
        ...
        try:
            # LOKI-RAYDISCIPLINE.  `rd_scan` decides whether the ladder below
            # ALSO collects economy candidates; `rd_arm` decides whether it may
            # ACT on them.  They are separate on purpose: with LOG on and the
            # master flag OFF the control arm still measures the OPPORTUNITY it
            # declines, which is the only way the pre-registered
            # geometric-starvation failure is falsifiable from the control.
            rd_arm = rd_scan = False
            if (LOKI_RAYDISC_ON or LOKI_RAYDISC_LOG) \
                    and turret_type == EntityType.SENTINEL \
                    and self._rd_forward(ct, p):
                rd_scan = True
                rd_arm = LOKI_RAYDISC_ON and self.rd_econ < LOKI_RAYDISC_BUDGET
            rd_best, rd_key, rd_n, rd_core_hp = None, None, 0, None
            rd_occ = 0
            # LOKI-SALTRAY (v509).  The highest-value candidate the FF guard
            # REFUSED this round because one of OUR builders stands on it, kept
            # by the same (value, -hp, x, y) key the chooser uses so the tile we
            # ask a raider to vacate is the tile we would actually have shot.
            # Nothing is published unless the refusal is what cost us the
            # diversion -- see the publish site below the loop.
            sr_blk, sr_key = None, None
            ...
            for t in ct.get_attackable_tiles():
                ...
                if et is None or not ct.can_fire(t):
                    continue
                if rd_scan:
                    v = BB_SITE_VALUE.get(et)
                    if v is not None:
                        # HP gate: a diversion must be a KILL, not a chip.
                        # VACUOUS today (max HP 30/20/20 vs 36) -- kept because
                        # it is the correct statement of intent, NOT reported as
                        # a control.  See the doctrine block.
                        hp = ct.get_hp(bid)
                        if hp <= LOKI_RAYDISC_MAX_HP:
                            rd_n += 1
                            # LOKI-RAYDISC-FF (v508).  The engine resolves a
                            # tile shot against the BUILDER BOT on the tile,
                            # either team, AHEAD of the building -- so `bot`
                            # (already read above, no new scan) decides whether
                            # this candidate is really an economy shot.  occ: 0
                            # nobody, 1 OURS (refuse -- 21.8% of v507's
                            # diversions were an 18-damage self-hit), 2 ENEMY
                            # (allowed and counted apart: 2 shots kill a 40 HP
                            # builder and TURRET_PRIO already ranks one above
                            # any belt tile).  A FAILED read refuses, which is
                            # the conservative direction.  `es=` (rd_n) still
                            # counts shielded tiles: it measures OPPORTUNITY.
                            occ, ff = 0, False
                            if (LOKI_RAYDISC_FF_GUARD or LOKI_RAYDISC_LOG) \
                                    and bot is not None:
                                try:
                                    ...
                                    occ = 1
                                if LOKI_RAYDISC_FF_GUARD:
                                    if occ == 1:
                                        ff = True
                                        self.rd_ff += 1
                                    else:
                                        self.rd_eb += 1
                            # -hp: finish the wounded one.  (t.x, t.y) last so
                            # the pick is deterministic and facing-independent
                            # -- get_attackable_tiles() is row-major in ABSOLUTE
                            # coordinates (see the comment above), so scan order
                            # must never be the tie-break.
                            key = (v, -hp, t.x, t.y)
                            # No `continue` on a refusal: the tile must still
                            # reach the TURRET_PRIO block below, or the guard
                            # would quietly drop an enemy belt tile out of the
                            # PARENT ladder's fallback too.
                            if not ff and (rd_key is None or key > rd_key):
                                rd_best, rd_key, rd_occ = t, key, occ
                            elif ff and SALTRAY_COORD_ON \
                                    and (sr_key is None or key > sr_key):
                                # LOKI-SALTRAY: remember it, publish later.
                                sr_blk, sr_key = t, key
                    elif et == EntityType.CORE and rd_core_hp is None:
                        rd_core_hp = ct.get_hp(bid)
                prio = TURRET_PRIO.get(et, 8)
                if prio < best_prio:
                    best_prio, best = prio, t
            # LOKI-SALTRAY (v509): PUBLISH THE BLOCKED TILE.  Fires only in the
            # exact state v508 measured as the dose loss -- the guard refused a
            # candidate, NO clean candidate survived (es=1 is the measured norm),
            # and the shot is therefore about to fall back to the core.  The
            # sentinel still fires the core below: publishing costs no shot, no
            # ammo and no cooldown, and the one-round write buffer means the
            # earliest a raider can act on it is next round anyway.
            if SALTRAY_COORD_ON and sr_blk is not None and rd_best is None \
                    and rd_arm and best_prio == 0 \
                    and rd_core_hp is not None \
                    and rd_core_hp > LOKI_RAYDISC_CORE_HP_STOP:
                self._sr_publish(ct, sr_blk)
            # The diversion.  ONLY when the parent ladder already chose a CORE
            # tile (TURRET_PRIO[CORE] == 0 and nothing else scores 0), so every
            # enemy turret / builder / launcher on the ray keeps its existing
            # precedence.  Rail 2: the endgame stop outranks the budget.
            if rd_arm and rd_best is not None and best_prio == 0 \
                    and rd_core_hp is not None \
                    and rd_core_hp > LOKI_RAYDISC_CORE_HP_STOP:
                ct.fire(rd_best)
                self.rd_econ += 1
                # -rd_key[1] is the target's PRE-shot hp (the ladder stored it
                # as -hp).  Passed explicitly because _rd_log runs AFTER the
                # shot, where a re-read would see hp-18 or a dead tile.
                self._rd_log(ct, "ECON", rd_best, rd_n, rd_core_hp, -rd_key[1],
                             rd_occ)
                return
            if best is not None:
                ct.fire(best)
                if rd_scan:
                    self._rd_log(ct, "CORE" if best_prio == 0 else "OTHER",
                                 best, rd_n, rd_core_hp,
                                 rd_core_hp if best_prio == 0 else None)
                return
        except Exception:
            pass
        try:
            for eid in ct.get_nearby_entities():
                if ct.get_team(eid) != self.team:
                    ...
                    if ct.can_fire(ep):
                        ct.fire(ep)
                        # Logged too, or the instrument would silently
                        # UNDERCOUNT sentinel shots and the CORE/ECON split
                        # would have an unmeasured denominator.  rd_fwd is
                        # already latched by the ray scan above.
                        if turret_type == EntityType.SENTINEL and self.rd_fwd:
                            self._rd_log(ct, "TAIL", ep, 0, None)
                        return
        except Exception:
            pass
    ...
    # --- LOKI-RAYDISCIPLINE: the forward test and the instrument -----------

    def _rd_forward(self, ct, p):
        """Is this SENTINEL a FORWARD sentinel (siege) rather than a home one?

        Two conditions, and the second is not decoration.  d^2 <=
        LOKI_RAYDISC_FWD_DSQ of the ENEMY anchor is the same bound
        `_try_forward_sentinel` uses to decide a site could align at all
        (raid.py:715).  d^2 > HUNT_BAND_DSQ of OUR OWN anchor excludes home
        defence: maps run from 8x8 up, so on a small board a home sentinel
        satisfies the enemy-core band as PURE GEOMETRY -- the identical seam
        `_bb_in_band` documents above.  `enemy_core_for` is an involution
        (table lookup or point reflection, both self-inverse), so applying it to
        the ENEMY anchor returns OUR anchor with no store slot and no extra
        state.

        Latches TRUE only, exactly as `bb_seen` does.  A turret cannot move and
        the anchor cannot move, so a True can never go stale; a False must stay
        revisable because SLOT_ENEMY_CORE is seeded by REFLECTION at
        main.py:246 before anyone has actually seen a core and is corrected
        later at main.py:551.
        """
        if self.rd_fwd:
            return True
        try:
            ...
            if E is None:
                return False
            self.enemy = E
            if dsq_core(p, E) > LOKI_RAYDISC_FWD_DSQ:
                return False
    ...
    # --- LOKI-SALTRAY: the sentinel half (publish) -------------------------

    def _sr_publish(self, ct, t):
        """Ask whoever is standing on `t` to step off, and say which way we aim.

        ONE write_store, no scan, no extra engine read except `get_direction()`
        on ourselves.  The packet is (x, y, dx+1, dy+1, round+1) bit-packed --
        see the LOKI-SALTRAY doctrine block for the field layout and for why the
        FACING has to be in it (every cardinal neighbour of a tile shares its
        row or its column, so without the facing "step somewhere off the ray" is
        not computable at all).

        The whole body sits in one try: a store write is a mutating call and an
        escaping exception permanently destroys this sentinel, which would cost
        far more than the diversion this is trying to recover.  A failed publish
        is simply a round with no request -- the same state as v508.
        """
        try:
            d = ct.get_direction()
            dx, dy = d.delta()
            if dx < -1 or dx > 1 or dy < -1 or dy > 1:
                return
            if not (0 <= t.x < 64 and 0 <= t.y < 64):
                return
            rnd = ct.get_current_round()
            val = (((rnd + 1) << 16) | ((dy + 1) << 14) | ((dx + 1) << 12)
                   | (t.y << 6) | t.x)
            ct.write_store(SLOT_RAY_YIELD, val)
            self.sr_pub += 1
            if LOKI_SALTRAY_LOG:
                import sys
                print("SR509PUB r%d id%d tile=%d,%d dir=%d,%d n=%d"
                      % (rnd, ct.get_id(), t.x, t.y, dx, dy, self.sr_pub),
                      file=sys.stderr)
        except Exception:
            return

    def _rd_log(self, ct, arm, t, n_econ, core_hp, tgt_hp=None, occ=0):
        """One stderr line per FORWARD-sentinel fire.  LOCAL instrument only.

        stderr, not print(): print() goes to the replay and the platform strips
        it (CLAUDE.md, measured 30,664/30,664 empty stdout fields), while stderr
        is console-only and survives a local `fcode run`.  Fires in BOTH arms so
        the CORE/ECON split and `es=` (eligible economy tiles on this ray) are
        measured on the same instrument in treatment and control.
        """
        if not LOKI_RAYDISC_LOG:
            return
        try:
            import sys
            # thp is the target's hp BEFORE this shot: a diversion KILLED when
            # thp <= 18 (one sentinel shot).  Without it "economy kills" and
            # "ammo per economy kill" have no honest estimator -- a repeat-fire
            # proxy conflates FARMING one rebuilt tile with failing to kill it.
            if tgt_hp is None:
                bid = ct.get_tile_building_id(t)
                tgt_hp = ct.get_hp(bid) if bid is not None else -1
            p = ct.get_position()
            # occ is the OCCUPANCY OF THE TILE FIRED AT (0 none / 1 ours /
            # 2 enemy builder) and it is the whole point of v508: with the guard
            # ON, `ECON ... occ=1` must never appear, and that is a per-shot
            # assertion rather than a battery average.  ff/eb are this unit's
            # running refusal and enemy-builder-allowed counts.
            print("RD508 %s r%d id%d at%d,%d (%d,%d) es=%d chp=%s thp=%s b=%d"
                  " occ=%d ff=%d eb=%d"
                  % (arm, ct.get_current_round(), ct.get_id(), p.x, p.y,
                     t.x, t.y, n_econ, core_hp, tgt_hp, self.rd_econ,
                     occ, self.rd_ff, self.rd_eb),
                  file=sys.stderr)
        except Exception:
            pass
