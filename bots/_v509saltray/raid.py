...
class RaidMixin:
    ...
    def _raid(self, ct):
        ...
        # LOKI-SALTRAY (v509): the raider half of the SALT x RAY coordination.
        # Placed AFTER exile detection -- a thrown body must reconcile its own
        # position before it decides where to step -- and BEFORE the action
        # ladder, because clearing the tile is the whole point: a raider that
        # pecks this round is a raider still standing on the forward Sentinel's
        # ray next round.  Costs one read_store on a raider turn.
        #   2 = stepped off this round (the move is spent, nothing else to do)
        #   1 = inside the post-yield freeze: may ACT, must not MOVE
        #   0 = no live request for this body
        sr = self._sr_yield(ct, p, rnd) if SALTRAY_COORD_ON else 0
        if sr == 2:
            return
        ...
        if ct.get_move_cooldown() != 0:
            return
        if sr == 1:
            # The freeze.  Re-entry onto a tile we were just asked to vacate is
            # impossible BY CONSTRUCTION for LOKI_SALTRAY_HOLD rounds, rather
            # than by hoping `_raid_station` picks a route around it.  Same
            # shape as LOKI_BELTBREAK_MAX_ROT = 1 per life, and the reason an
            # A->B->A oscillation cannot be written here.
            return
    ...
    # --- LOKI-SALTRAY: the raider half (yield) ------------------------------

    def _sr_yield(self, ct, p, rnd):
        """Step off a tile a friendly forward Sentinel has asked to shoot.

        Returns 2 (stepped off, move spent), 1 (frozen: act but do not move) or
        0 (nothing live for this body).

        WHY THE FACING IS IN THE PACKET.  A Sentinel's shot is a single-tile
        line along a fixed facing, so the ray through the published tile is
        {tile + k*(dx, dy)}.  Every CARDINAL neighbour of a tile shares its row
        or its column, so "step somewhere that is not also on the ray" is not
        computable from the tile alone -- with the facing it is one comparison:
        a cardinal ray is a column (dx == 0) or a row (dy == 0) and exactly two
        of the four steps leave it; a DIAGONAL ray is left by all four.

        FAILURE IS ALWAYS "STAY".  No legal step, a cooldown, a raised
        can_move, a raised move: every one of them falls through to a hold.
        Never self_destruct, never let anything escape -- an exception out of
        run() permanently destroys this raider for the rest of the match, which
        is a far worse outcome than one unfired diversion.
        """
        try:
            v = ct.read_store(SLOT_RAY_YIELD)
        except Exception:
            return 0
        live = False
        yx = yy = 0
        dx = dy = 0
        if v:
            # stamp is round+1 at publish time, so 0 is unambiguously "never
            # published" and a live request is at least one round old by
            # construction (store writes are buffered one round).
            stamp = (v >> 16) - 1
            age = rnd - stamp
            if stamp >= 0 and 0 <= age <= LOKI_SALTRAY_STALE:
                yx, yy = v & 63, (v >> 6) & 63
                dx, dy = ((v >> 12) & 3) - 1, ((v >> 14) & 3) - 1
                live = True

        if live and p.x == yx and p.y == yy:
            if ct.get_move_cooldown() != 0:
                return 1
            best = fallback = None
            for i in range(4):
                ...
                if fallback is None:
                    fallback = d
                if best is None and not ((dx == 0 and tx == yx)
                                         or (dy == 0 and ty == yy)):
                    best = d
            d = best if best is not None else fallback
            if d is None:
                # ⛔ THE MEASURED CASE, and it is not the exception -- it is
                # 202 of 204 shield events (s50 probe, 54 games vs the
                # incumbent).  The raider has NO legal cardinal step: all four
                # neighbours are occupied, and 561 of 561 friendly ones are OUR
                # OWN BARRIERS, every one of them can_destroy == True.  See the
                # LOKI-SALTRAY doctrine block.  Swap places with one.
                if self._sr_break_cage(ct, p, yx, yy, dx, dy):
                    # A barrier came down; the step into it may or may not have
                    # landed on the same turn (MEASURED: it does, 18/18 -- see
                    # the doctrine block).  Either way the turn is spent, and
                    # the return must be 2 and NOT 1: on 1 the caller would run
                    # `_raid_act`, whose seal step would re-barrier the seat we
                    # just opened and undo the escape on the very turn it was
                    # made.  If the step did not land, it is taken next round,
                    # where `_sr_yield` again runs BEFORE `_raid_act`.
                    return 2
                return 1
            try:
                ct.move(d)
                ...
                return 1
            self.sr_yld += 1
            self.sr_hold_until = rnd + LOKI_SALTRAY_HOLD
            if LOKI_SALTRAY_LOG:
                import sys
                print("SR509YLD r%d id%d off=%d,%d dir=%d,%d off_ray=%d n=%d"
                      % (rnd, ct.get_id(), yx, yy, dx, dy,
                         1 if best is not None else 0, self.sr_yld),
                      file=sys.stderr)
            return 2

        if rnd < self.sr_hold_until:
            return 1
        return 0

    def _sr_break_cage(self, ct, p, yx, yy, dx, dy):
        """Swap places with one of our OWN barriers when boxed in.

        Returns True when the turn has been spent (a barrier was destroyed,
        with or without the step landing in the same turn), False when nothing
        was done.

        ⛔ WHY THIS EXISTS AT ALL, and it is the s50 surprise: the step-off this
        plank was designed around FAILS 202 times in 204.  Measured over 54
        games vs the incumbent, at every event where one of our raiders stood
        on a tile a forward Sentinel had published: `can_move` was False in ALL
        FOUR cardinal directions with move cooldown 0.  The cage is
        563 buildings, and 561 of them are OUR OWN BARRIERS.

        WHY THE SWAP IS NEARLY FREE, and this is what makes it worth doing
        rather than a reason to abandon the arm.  The barriers are the
        LOKI_BARRIER_SEAL collar and they sit on HEAL SEATS.  Destroying one
        and stepping into it leaves the raider on a seat -- still denying it
        with its own body (raid.py, step 1: "the seat itself is denied by our
        body"), still adjacent to the enemy Core, still pecking.  What is
        actually spent is one 3 Ti barrier, and the seal step rebuilds it.

        BOUNDED PER LIFE, not per round: LOKI_SALTRAY_MAX_BREAK, never reset --
        the same shape as LOKI_BELTBREAK_MAX_ROT = 1 and LOKI_RAYDISC_BUDGET.
        A raider cannot turn into a demolition loop against our own collar.

        BARRIERS ONLY.  `EntityType.BARRIER` is checked explicitly: a conveyor,
        splitter, harvester or turret of ours is never a candidate, whatever
        `can_destroy` says about it.
        """
        if self.sr_break >= LOKI_SALTRAY_MAX_BREAK:
            return False
        pick = pick_fb = None
        for i in range(4):
            mx, my = CARD_DELTAS[i]
            ...
            if pick_fb is None:
                pick_fb = i
            # Same off-ray preference as the plain step: do not vacate one tile
            # of the Sentinel's line onto another one.
            if pick is None and not ((dx == 0 and tx == yx)
                                     or (dy == 0 and ty == yy)):
                pick = i
        i = pick if pick is not None else pick_fb
        if i is None:
            return False
        mx, my = CARD_DELTAS[i]
        try:
            ct.destroy(Position(p.x + mx, p.y + my))
        except Exception:
            return False
        self.sr_break += 1
        d = CARDINALS[i]
        ...
        if moved:
            self.sr_yld += 1
            self.sr_hold_until = ct.get_current_round() + LOKI_SALTRAY_HOLD
        if LOKI_SALTRAY_LOG:
            import sys
            print("SR509BRK r%d id%d off=%d,%d moved=%d n=%d"
                  % (ct.get_current_round(), ct.get_id(), yx, yy,
                     1 if moved else 0, self.sr_break), file=sys.stderr)
        return True
