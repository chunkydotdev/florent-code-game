        ...
        if not endgame_dumped:
            ...
            if fs_live:
                ...
                if fs_ph == FS_PH_KILL:
                    ...
                    # ⛔ THIS RESERVE MUST STRICTLY EXCEED the raider's own
                    # seal gate (`_fs_seal_ok`), or the two reserves deadlock
                    # against each other on the same bank -- measured on the
                    # first integrated run, where the seal sat a few titanium
                    # short of its bar for the whole match and placed nothing.
