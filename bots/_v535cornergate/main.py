...
class Player(EcoMixin, RaidMixin, SiegeMixin):
    ...
    def __init__(self):
        ...
        self.wire_pending = []
        # --- v530 P1 MOUTH-FIRST BELT, per builder.  Every one of these is
        # read only inside a `LOKI_FS_V530 and FS_V530_MOUTH` branch; they are
        # initialised unconditionally because an attribute that exists but is
        # never read cannot change behaviour, while a `getattr` default in the
        # hot path can (and the v515 finding-3 hazard is exactly that shape).
        self.mouth_queue = []           # chain tiles, CORE -> ORE order
        self.mouth_ore = None           # the ore the chain terminates at
        self.mouth_prev = None          # link laid last; the facing anchor
        self.mouth_done = False         # finished, abandoned, or never armed
        self.mouth_since = 0            # round the chain was armed (TTL clock)
        self.mouth_links = 0            # INSTRUMENT: links laid mouth-first
        self.mouth_sock_rnd = -1        # INSTRUMENT: round our socket landed
        # --- v530 P2 CORNER BARRIERS, per builder.
        self._v530_corner_keys = None   # cached {(x, y)} of our 4 diagonals
        self.v530_corners = 0           # INSTRUMENT: corner barriers built
        # --- v535 CORNER GATE, per unit.  `v535_refuse` is this unit's cached
        # copy of `SiegeMixin._fs_map_gated`'s verdict; None = not yet asked or
        # not yet computable (no core / no dims), which is the "ask again next
        # round" state, NOT "runs".  Deliberately SEPARATE from `fs_gate_ok`
        # (siege.py `_v535_map_refuses` says why).
        self.v535_refuse = None
        self.v535_corner_gated = 0      # INSTRUMENT: corner branches stood down
        # --- v530 P1b THE CONVEYOR RING (candidate, ships OFF), per builder.
        self._v530_ring_seats = None    # cached our 8 orthogonal sockets
        self.v530_ring = 0              # INSTRUMENT: ring sockets paved
        # --- v530 P3 DOOR-LAUNCHER KILL, per turret.
        self._v530_home = None          # our own core anchor, recovered by the
                                        # enemy_core_for involution (a turret's
                                        # self.core is None BY DESIGN, main.py
                                        # ~2054 -- do not fill that one in)
        self.v530_doorkill = 0          # INSTRUMENT: shots taken under the rule
    ...
    # ------------------------------------------------------------------
    # v530 P3: DOOR-LAUNCHER KILL.  See the doctrine.py v530 block.
    #
    # ⛔ THIS IS A TARGETING RULE, NOT THE v513 DOOR RESPONSE.  It reuses the
    # DETECTION SHAPE of `_door_turret` (above, dead behind FS_V515_DOOR_OFF)
    # and nothing else: no builder is dispatched, no peck is issued, and both
    # `FS_HOME_TURRET_RESPONSE` and `FS_V515_DOOR_OFF` keep the values the
    # parent ships.  The pecker stands inside the pickup envelope, so a peck
    # response IS the farm; the whole point of the plank is that a turret
    # shoots from outside it.
    # ------------------------------------------------------------------

    def _v530_home_anchor(self, ct):
        """OUR core anchor, for a unit whose `self.core` is None BY DESIGN.

        `enemy_core_for` is an involution on a symmetric board, so it recovers
        our anchor from the published enemy one with no extra store slot --
        the same trick `_bb_in_band` (main.py) already uses.  ⛔ It writes
        `self._v530_home`, NEVER `self.core`: `_door_turret_turn` returns early
        on that null and filling it in would silently re-open the door path.
        """
        if self._v530_home is not None:
            return self._v530_home
        ...
        if E is None:
            return None
        self._v530_home = enemy_core_for(self.mw, self.mh, E)
        return self._v530_home

    def _v530_door_launcher_at(self, ct, t, ours):
        """Does tile `t` hold an ENEMY LAUNCHER inside our own door band?"""
        try:
            bid = ct.get_tile_building_id(t)
            if bid is None or ct.get_team(bid) == self.team:
                return False
            if ct.get_entity_type(bid) != EntityType.LAUNCHER:
                return False
            return dsq_core(t, ours) <= V530_DOOR_DSQ
        except Exception:
            return False

    def _v530_doorkill_fire(self, ct):
        """The GUNNER pre-empt.  True when this gunner spent its shot here.

        ⛔ WITHOUT THIS CLAUSE THE PLANK IS SENTINEL-ONLY.  A gunner never
        reaches the typed priority ladder: the fast path below fires whatever
        `get_gunner_target()` returns and RETURNS.  Promoting LAUNCHER in
        `TURRET_PRIO` alone would therefore change sentinel behaviour and
        nothing else, while reading as a both-turrets rule.
        """
        ours = self._v530_home_anchor(ct)
        if ours is None:
            return False
        try:
            for t in ct.get_attackable_tiles():
                if not self._v530_door_launcher_at(ct, t, ours):
                    continue
                if not ct.can_fire(t):
                    continue
                ct.fire(t)
                self.v530_doorkill += 1
                if FS_V530_LOG:
                    print("V530 DOORKILL gunner rnd=%d at=%d,%d"
                          % (ct.get_current_round(), t.x, t.y), file=sys.stderr)
                return True
        except Exception:
            return False
        return False
    ...
    def _turret(self, ct):
        ...
        # ⭐ v530 P3 -- THE GUNNER PRE-EMPT, and it has to sit HERE, above the
        # fast path, because that path ends in `return`.
        if LOKI_FS_V530 and FS_V530_DOORKILL and V530_DOOR_GUNNER \
                and turret_type == EntityType.GUNNER \
                and self._v530_doorkill_fire(ct):
            return
        ...
        _v517_skipped = False
        # ⭐ v530 P3 -- resolved ONCE, outside the tile loop: the anchor is a
        # pure function of the published enemy anchor and is cached per unit.
        _v530_ours = None
        if LOKI_FS_V530 and FS_V530_DOORKILL:
            _v530_ours = self._v530_home_anchor(ct)
        try:
            ...
            for t in ct.get_attackable_tiles():
                ...
                # ⭐ v530 P3 -- THE LADDER PROMOTION, and the value is NEGATIVE
                # by necessity: `best_prio == 0` is used ~20 lines below as the
                # proxy for "that shot was at their core", so reusing 0 here
                # would make the v517 core-shot accounting miscount silently.
                if _v530_ours is not None and et == EntityType.LAUNCHER \
                        and dsq_core(t, _v530_ours) <= V530_DOOR_DSQ:
                    prio = V530_DOOR_PRIO
                if prio < best_prio:
            ...
            if best is not None:
                ct.fire(best)
                # v530 P3 INSTRUMENT: counted at the FIRE, never at the
                # promotion -- a promotion that loses the scan is not a shot.
                if _v530_ours is not None and best_prio == V530_DOOR_PRIO:
                    self.v530_doorkill += 1
                    if FS_V530_LOG:
                        print("V530 DOORKILL %s rnd=%d at=%d,%d"
                              % (turret_type.name, ct.get_current_round(),
                                 best.x, best.y), file=sys.stderr)
