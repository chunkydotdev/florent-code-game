...
# ⭐ v534 MAPTRUST F2 -- the same confirmation for `FS_MAP_SKIP`.  Decoded
# LAZILY, one signature at a time, rather than at import like the two v524
# grids above: the skip set has six grids across five signatures and at most
# ONE of them can ever be the board we are playing, so decoding all six on the
# first unit's first turn would be five wasted expansions inside the 10 ms
# budget.  `_decode_grid` is itself memoised in eco.py, so a repeat ask is a
# dict hit.
_FS_V534_SKIP_GRIDS = {}


def _fs_v534_skip_grids(sig):
    grids = _FS_V534_SKIP_GRIDS.get(sig)
    if grids is None:
        mw, mh = sig[0], sig[1]
        grids = frozenset(
            _decode_grid(code, mw, mh) for code in FS_V534_SKIP_CODES.get(sig, ())
        )
        _FS_V534_SKIP_GRIDS[sig] = grids
    return grids
...
class SiegeMixin:
    ...
    def _fs_eco_publish(self, ct, rnd, harv):
        ...
        # ⭐ v539 -- THE FAMINE BITS RIDE THIS WRITE, THEY DO NOT GET THEIR OWN.
        # `read_store` returns the PRE-round value, so a second Core write to
        # FS_ECO_SLOT in the same turn would read a stale word and silently
        # drop this one (the r197 lost-update defect, same shape).  One writer,
        # one word, one write.  The cost of folding it in is that the latched
        # early-return below can no longer fire while the plank is on: the
        # famine detector needs `held` EVERY round, and `held` is exactly what
        # the early return was skipping.  Flag off, the return is untouched.
        _v539 = LOKI_FS_V539 and FS_V539_REEST
        if conn and deliv and not _v539:
            return                           # latched: nothing left to say
        mouth, held = self._fs_eco_mouth(ct)
        now_conn = (harv >= FS_SENT_HARV_MIN) and mouth is not None
        new = cur
        if not (conn and deliv):
        ...
        if _v539:
            new = self._v539_famine_bits(ct, rnd, new, held)
        if new == cur:
            return
        try:
    ...
    # ---- LOKI-REESTABLISH (v539) ------------------------------------------
    #
    # ⛔ THREE FUNCTIONS AND A BIT.  `_v539_famine_bits` is CORE-ONLY and runs
    # inside the single FS_ECO_SLOT write above; `_v539_famine_state` and
    # `_v539_famine` are pure READERS and are what every other body uses.  No
    # body except the Core ever writes a famine bit.

    def _v539_famine_bits(self, ct, rnd, word, held):
        """CORE ONLY.  Fold the famine bit + episode round into `word`.

        THE DETECTOR IS A DELIVERY DROUGHT, not a head count -- see the
        LOKI-REESTABLISH block in doctrine.py for why a head count cannot be
        read honestly by anyone on this team.  A stack sitting on our own
        core-adjacent mouth is the one economic fact the Core can read exactly
        and every round.
        """
        try:
            deliv = bool(word & FS_ECO_BIT_DELIV)
            fam = bool(word & FS_ECO_BIT_FAMINE)
            if held:
                self.v539_last_flow = rnd
            if not deliv:
                # NEVER DELIVERED = THE OPENING, NOT A FAMINE.  This is one of
                # the two guards that make the opening curve untouchable.
                return word
            if self.v539_last_flow is None:
                # deliv is latched but this Core has not seen a stack itself
                # (it cannot happen -- the Core is the only writer and sets
                # deliv from its own sighting -- but a missing seed would make
                # the drought infinite, so seed from the conn latch round).
                self.v539_last_flow = max(
                    0, ((word >> FS_ECO_RND_SHIFT) & FS_ECO_RND_MASK) - 1)
            if fam:
                started = ((word >> FS_ECO_FAM_RND_SHIFT)
                           & FS_ECO_FAM_RND_MASK) - 1
                # CLEAR ON DELIVERY.  This is the plank's stop condition and
                # the reason its cap is not an arbitrary number: the episode
                # ends when titanium arrives again, i.e. when the kill is
                # funded.  The hard stop is the backstop, not the rule.
                if held or (started >= 0 and rnd - started >= FS_V539_MAX_RNDS):
                    word &= ~FS_ECO_BIT_FAMINE
                    word &= ~(FS_ECO_FAM_RND_MASK << FS_ECO_FAM_RND_SHIFT)
                    if FS_V539_LOG:
                        self._v539_log("CLEAR", rnd, held, started)
                return word
            if rnd < FS_V539_MIN_RND:
                return word
            if self.v539_eps >= FS_V539_MAX_EPISODES:
                return word
            if rnd - self.v539_last_flow < FS_V539_DROUGHT:
                return word
            self.v539_eps += 1
            word |= FS_ECO_BIT_FAMINE
            word &= ~(FS_ECO_FAM_RND_MASK << FS_ECO_FAM_RND_SHIFT)
            word |= (((rnd + 1) & FS_ECO_FAM_RND_MASK) << FS_ECO_FAM_RND_SHIFT)
            if FS_V539_LOG:
                self._v539_log("DECLARE", rnd, held, rnd)
            return word
        except Exception:
            return word

    def _v539_log(self, what, rnd, held, started):
        try:
            print("V539", what, "rnd", rnd, "held", 1 if held else 0,
                  "started", started, "eps", getattr(self, "v539_eps", -1),
                  "lastflow", getattr(self, "v539_last_flow", -1),
                  file=sys.stderr)
        except Exception:
            return

    def _v539_famine_state(self, ct):
        """(famine_now, episode_start_round).  ANY body.  Read-only.

        One round stale by construction -- store writes are buffered -- which
        is the same staleness every other slot read in this file carries.
        """
        if not (LOKI_FS_V539 and FS_V539_REEST):
            return (False, -1)
        try:
            v = ct.read_store(FS_ECO_SLOT)
        except Exception:
            return (False, -1)
        if not (v & FS_ECO_BIT_FAMINE):
            return (False, -1)
        return (True,
                ((v >> FS_ECO_FAM_RND_SHIFT) & FS_ECO_FAM_RND_MASK) - 1)
    ...
    def _v539_lifeline(self, ct, cost):
        ...
        if not (LOKI_FS_V539 and FS_V539_REEST and FS_V539_LIFELINE):
            return False
        if getattr(self, "role", None) != "expand":
            return False
        fam, started = self._v539_famine_state(ct)
    ...
    def _fs_enemy_anchor(self, ct):
        ...
        ⭐ PORTED FROM `bots/_v535cornergate` FOR v538 -- BEHAVIOUR-PRESERVING,
        and it exists so that the v538 claim gate CALLS this resolution instead
        of COPYING it.  These exact eight lines stood inline in `_fs_gate`; the
        v535 build's first draft duplicated them into its reader and the copy
        was refactored away rather than defended, because a copy inherits none
        of the original's verification (D21d).  There is now ONE anchor
        resolution in this tree, as there is ONE `_fs_map_gated`.
        ...
        two paths differ under `FS_V538_CLAIM_GATE = False`, which is exactly
        the property flag-off exists to rule out.  Identity is proved instead
        the way this repo proves it -- structurally (AST), against the frozen
        parent's `_fs_gate` over the whole pool in the harness, and on the
        ENGINE game-row for game-row (build report §5).
        """
        E = self.enemy
        if E is None:
            try:
    ...
    def _v535_map_refuses(self, ct):
        ...
        ⭐ PORTED FROM `bots/_v535cornergate` FOR v538, NAME AND SEMANTICS
        UNCHANGED.  It is kept under its v535 name because it is the SAME
        object doing the SAME job -- publishing the siege's own map verdict to
        a non-siege plank -- and renaming it per consumer would be the first
        step back toward two predicates.  v538's consumer is `EcoMixin.
        _v538_claim_on` (the socket claim); v535's was the corner barriers,
        which are not gated on this lineage.

        This is a READER, not a second predicate: `_fs_map_gated` stays the
        single definition of "this board refuses", and any future correction to
        it moves every gated plank with it.
        ...
        ⛔ WHY NOT `_fs_gate` ITSELF, which looks like the obvious call:
          1. `_fs_gate` returns False for `LOKI_FERRY_SIEGE_ON = False`, which
             ...
             the BOARD.  Conflating the two would silently kill the gated plank
             in every siege-off arm and make the master-off control mean
          ...
          2. It CACHES into `self.fs_gate_ok`.  Being called from the eco path
             ...
             and the enemy-anchor resolution falls back to a MIRROR GUESS
             before `SLOT_ENEMY_CORE` is published -- so an early eco call
             could freeze a siege verdict computed off the guess.  This reader
             keeps its own cache and never touches `fs_gate_ok`.
        ...
        the anchor from `_fs_enemy_anchor` and the verdict from
        `_fs_map_gated`.  What is local to this method is only the CACHE and
        the SIGN FLIP.
        """
        ...
        if FS_V538_LOG:
            try:
                print("V538GATE rnd=%d id=%d refuse=%d"
    ...
    def _fs_map_gated(self, mw, mh, ours, E, ct=None):
        ...
        if max(mw, mh) < min_dim:
            ...
            if sig in FS_MAP_SKIP:
                skip = True
                # ⭐ v534 MAPTRUST F2 -- GRID-CONFIRM THE SKIP.  `FS_MAP_SKIP`
                # was a bare-signature test: a board merely SHARING
                # (w, h, both anchors) with one of the five surveyed maps lost
                # ferry-siege for the whole match with no tell, and the whole
                # plank is what the programme is spending its kill on.  The
                # confirmation is the SAME mechanism v524 CHANGE 1 already
                # applies to the cripple list one clause below -- a coarse hit
                # is only a CANDIDATE until the actual tile grid agrees.
                # ⛔ NO MATCH => RUN.  A signature hit whose terrain does not
                # confirm is treated as a map we have never surveyed, and the
                # registered default for an unsurveyed map is that ferry-siege
                # plays normally (an unseen board is not evidence its ring
                # does not close).
                # ⛔ AND `self.map_grid` IS NOT WRITTEN HERE, ON ANY PATH --
                # the v524 lesson verbatim: `main._builder`'s own map_grid
                # init is guarded by `if self.map_grid is None:` and this gate
                # can be asked FIRST on the same round, which would silently
                # lose map_walls/map_ores for that unit for the rest of the
                # match.
                if FS_V534_MAPTRUST:
                    sgrid = self.map_grid
                    ...
                    if FS_V534_LOG and ct is not None:
                        try:
                            print("V534SKIP", ct.get_current_round(),
                                  "sig", sig, "grid_known", sgrid is not None,
                                  "confirmed", skip, file=sys.stderr)
                        except Exception:
                            pass
                if skip:
                    ok = False
    ...
    # ======================================================================
    # LOKI-FS-V527 -- THE COLLAR.  s51, 2026-08-19.  Three mechanisms:
    #   M1  the BUNKER SWAP + the DEFENDED-TILE preference   (FS_V527_BUNKER)
    #   M2  the PURCHASE SURVIVES THE RAIDER                 (FS_V527_PSURV)
    #   M3  the SEAL-PATH ORDER                              (FS_V527_SEALPATH)
    # Every predicate below is a RUN-time read; nothing is derived at module
    # level.  `LOKI_FS_V527 = False` restores the RDV-only parent.
    # ======================================================================

    def _v527_on(self):
        return LOKI_FS_V527

    def _v527_log(self, *a):
        """One stderr line per EVENT.  Off in the fired build; the two-tape
        discipline is v521's (`_v521_log` state / `_v521_rung` event)."""
        if not (LOKI_FS_V527 and FS_V527_LOG):
            return
        try:
            print("V527", *a, file=sys.stderr)
        except Exception:
            return

    # --- M1: the trapped test ---------------------------------------------

    def _v527_trapped(self, ct, p):
        """NO LEGAL CARDINAL MOVE THIS ROUND.

        ⛔ A MOVE COOLDOWN IS NOT A TRAP, and conflating them is how this
        predicate would have fired on every second round of an ordinary walk:
        with `get_move_cooldown() != 0` the engine's `can_move` is False in all
        four directions for a body that is merely resting.  Cooldown therefore
        reads NOT TRAPPED -- the fail-closed direction, since the swap is a
        one-way conversion of a barrier we paid for.
        """
        try:
            if ct.get_move_cooldown() != 0:
                return False
    ...
    def _v527_mag_ok(self, ct):
        """CAN THE MAGAZINE SUSTAIN FIRE?  Magnus's ruling on the archipelago
        r28 state, encoded: banked ammunition, never a promise to convert.

        A sentinel is 10 ammo a shot (GameConstants / the entity table).  With
        an empty magazine the swap trades a 3 Ti barrier for a 30 Ti building
        that cannot shoot -- strictly worse than holding the seat as denial,
        which is the alternative this gate selects.
        """
        try:
            return ct.get_global_ammo() >= FS_V527_MAG_SHOTS * 10
        except Exception:
            return False

    def _v527_bunker_state(self, ct, p, orth_open):
        """(armed, why).  `why` is carried to the tape so a NO-FIRE round is
        readable -- an alarm that only speaks when it fires cannot be seen to
        check."""
        if self._v527_trapped(ct, p):
            return True, "trapped"
        if orth_open <= FS_V527_BUNKER_NEAR:
            return True, "collar"
        return False, "open"

    def _v527_bunker_swap(self, ct, E, p, rnd, needed, orth_open, ti):
        """⭐ M1 -- CONVERT OUR OWN BARRIER INTO A CORE-AIMED SENTINEL.

        THE MEASURED HOLE THIS FILLS (AUTOPSY-rush-top3 #2): `_fs_try_sentinel`
        can only score the <=4 tiles orthogonally adjacent to the body, MINUS
        the ring (`FS_SENTINEL_OFFRING`) -- and a ring raider stands ON the
        ring, so its own hand excludes its own candidates.  7 of 30 games never
        bought a forward sentinel at all.  v516's `SENTREACH` answered that by
        WALKING the body to a site; this answers it without moving, which is
        the only answer available to a body that cannot move.

        ORDER OF OPERATIONS, and it is the whole safety argument:
          1. every gate is checked BEFORE anything is destroyed --
             `_fs_sentinel_ok` (ruling 2's economy gate whole: salt/eco
             disjunction, collar reserve, purchase cap, ti floor), the
             magazine, the per-body cap, and `can_fire_from` on a REAL core
             tile for the tile we are about to free;
          2. destroy (free, no cooldown, so it composes with a same-round
             build);
          3. build the sentinel;
          4. if the build is still refused, REBUILD THE BARRIER THIS TURN.
        ⇒ The seat is never open across a round boundary.  Step 4 is counted
        (`v527_bunker_reseal`) because a silent fallback is indistinguishable
        from one that never happens.
        """
        if not (self._v527_on() and FS_V527_BUNKER):
            return False
        if getattr(self, "v527_bunker_n", 0) >= FS_V527_BUNKER_MAX:
            return False
        armed, why = self._v527_bunker_state(ct, p, orth_open)
        if not armed:
            return False
        # ⛔ FAIL CLOSED, IN THIS ORDER.  The economy gate first: it is the one
        # Magnus ruled on and the cheapest to fail.
        if not self._fs_sentinel_ok(ct, ti, needed, orth_open):
            self._v527_log("BUNKER HOLD", rnd, "why", why, "blk", "gate")
            return False
        if not self._v527_mag_ok(ct):
            self._v527_log("BUNKER HOLD", rnd, "why", why, "blk", "mag")
            return False
        try:
            cost = ct.get_sentinel_cost()
            bar = ct.get_barrier_cost()
            ...
            return False
        _floor, _rebuy = self._v517_sent_floor(ct)
        if ti < cost + _floor:
            self._v527_log("BUNKER HOLD", rnd, "why", why, "blk", "ti")
            return False
        try:
            if ct.read_store(SLOT_FWD_GUN) >= FS_SENT_BUY_MAX:
                return False
        except Exception:
            return False
        tiles = core_tiles(E)
        ...
        for dx, dy in CARD_DELTAS:
            ...
            if not (0 <= bx < self.mw and 0 <= by < self.mh):
                continue
            # OUR OWN BARRIER, and nothing else.  A harvester or a launcher of
            # ours is doing a job; a barrier on a seat is doing the job a
            # sentinel standing there also does (the census counts any
            # blocking building of ours as denial -- `_fs_denied`).
            try:
                bid = ct.get_tile_building_id(Position(bx, by))
                if bid is None:
                    continue
                if ct.get_team(bid) != self.team:
                    continue
            ...
            for target in tiles:
                ...
                if facing == Direction.CENTRE:
                    continue
                try:
                    if not ct.can_fire_from(bp, facing,
                ...
                if FS_V527_DEFENDED and self._v527_defended(ct, bx, by):
                    score += FS_V527_DEFENDED_BONUS
                if best_k is None or score > best_k:
                    best, best_k, best_ray = (bp, facing), score, target
        if best is None:
            self._v527_log("BUNKER HOLD", rnd, "why", why, "blk", "nosite")
            return False
        bp, facing = best
        try:
            if not ct.can_destroy(bp):
                self._v527_log("BUNKER HOLD", rnd, "why", why, "blk", "nodes")
                return False
            ct.destroy(bp)
        except Exception:
            return False
        ...
        try:
            ...
            built = False
        if not built:
            # STEP 4 -- THE RESEAL.  Same turn, so the seat never spends a
            # round open.  If even this fails the tape says so and the census
            # will re-queue the tile next round as ordinary rung-1 work.
            self.v527_bunker_reseal = getattr(self, "v527_bunker_reseal", 0) + 1
            back = False
            try:
                if ti >= bar and ct.can_build_barrier(bp):
                    ct.build_barrier(bp)
                    back = True
            except Exception:
                back = False
            self._v527_log("BUNKER RESEAL", rnd, "at", (bp.x, bp.y),
                           "back", 1 if back else 0)
            return back
        self.v527_bunker_n = getattr(self, "v527_bunker_n", 0) + 1
        self.fs_sentinels += 1
        try:
        ...
        self._fs_draw_dot(ct, bp, 255, 0, 255)
        self._fs_draw_line(ct, bp, best_ray, 255, 0, 255)
        self._v527_log("BUNKER FIRE", rnd, "at", (bp.x, bp.y),
                       "why", why, "orth", orth_open,
                       "ammo", self._v527_ammo(ct), "ti", ti, "cost", cost,
                       "n", self.v527_bunker_n)
        return True

    def _v527_ammo(self, ct):
        try:
            return ct.get_global_ammo()
        except Exception:
            return -1

    # --- M1b: the defended-tile preference --------------------------------

    def _v527_defended(self, ct, bx, by):
        """Is this tile inside the pickup envelope of one of OUR launchers?

        Magnus, marker 5: "perfect spot for a Sentinel, defended behind the
        Launcher already there".  A launcher picks up an ADJACENT builder from
        EITHER team (`can_launch` has no team check -- engine-read), so a
        turret site our launcher covers is a site whose attacker gets thrown
        off it before it finishes pecking.  ORDERING TERM ONLY.

        Cached per round: the scorer asks it up to 4 times a turn and
        `get_nearby_buildings` is the expensive half.
        """
        if not (self._v527_on() and FS_V527_DEFENDED):
            return False
        try:
            rnd = ct.get_current_round()
        except Exception:
            return False
        if getattr(self, "v527_def_rnd", None) != rnd:
            self.v527_def_rnd = rnd
            pts = []
            try:
                for bid in ct.get_nearby_buildings():
                    if ct.get_entity_type(bid) != EntityType.LAUNCHER:
                ...
                pts = []
            self.v527_def_pts = tuple(pts)
        for lx, ly in getattr(self, "v527_def_pts", ()):
            if (lx - bx) ** 2 + (ly - by) ** 2 <= FS_V527_DEFENDED_DSQ:
                return True
        return False

    # --- M2: the purchase survives the raider ------------------------------

    def _v527_no_turret(self, ct):
        """No forward turret has EVER been bought, and none is standing.

        `SLOT_FWD_GUN` is the team-wide MONOTONE purchase count and survives
        the body that bought them, so this is not a vision question -- which
        matters, because AUTOPSY-rush-top3 #1 measured the vision read blind in
        63.4% of the rounds a sentinel of ours was actually hitting their core.
        """
        try:
            if ct.read_store(SLOT_FWD_GUN) > 0:
                return False
        except Exception:
            return False
        return not self._fs_sent_beat_live(ct)

    def _v527_psurv_state(self, ct, rnd):
        """⭐ M2 -- IS THE COLLAR PAID FOR AND UNCONVERTED?

        The signature from Magnus's two marker games: a SEALED enemy core with
        NO turret on it, held for hundreds of rounds.  Every input is already
        published, so no new channel is opened (all 8 phase codes are taken and
        a channel nobody needs is the v523 arc-merge defect).

        Returns True when the published crew phase says the collar is
        sealed-or-better AND nothing was ever bought to shoot through it.
        """
        if not (self._v527_on() and FS_V527_PSURV):
            return False
        if rnd > FS_V527_PSURV_MAXRND:
            return False
        try:
            _b, ph, _r = self._fs_state(ct)
        except Exception:
            return False
        if not (FS_PH_SEALED <= ph <= FS_PH_KILL_NEAR):
            return False
        return self._v527_no_turret(ct)

    def _v527_lastseat_buy(self, ct, E, p, rnd, needed, ti, orth_open):
        """⭐ M2(b) -- BUY BEFORE THE FINAL SEAL.  7 of 8 seats, no turret ever
        bought, gate open ⇒ THE TURRET OUTRANKS THE LAST BARRIER.

        Magnus, marker 6: "(3,16) r51 seal-all-before-sentinel".  The last
        barrier is worth nothing until something is shooting through the collar
        it completes, and `orth_open` flickers, so a strict seal-first ladder
        can defer the purchase indefinitely -- which is exactly the 780-round
        sealed-and-turretless stretch in the marker game.

        ⛔ THE GATE IS NOT RELAXED, ONLY THE RANK.  `_fs_sentinel_ok` still
        decides (and at `orth_open >= 1` the salt disjunct is shut, so this
        fires only on the ECO disjunct -- ruling 2's own condition) and
        `_fs_try_sentinel` still decides the siting.
        ⛔ AND IT IS NOT "FIRE EARLIER".  The autopsy is explicit that opening
        sooner is NOT the carrier (failed sieges open at r82 against r111 for
        wins): this clause changes games that would have bought NOTHING, not
        games that would have bought later.  `_v527_no_turret` is what makes
        that true and it is the clause's first test.
        """
        if not (self._v527_on() and FS_V527_PSURV and FS_V527_PSURV_LASTSEAT):
            return False
        if not (1 <= orth_open <= FS_V527_PSURV_NEAR):
            return False
        if not self._v527_no_turret(ct):
            return False
        if not self._fs_sentinel_ok(ct, ti, needed, orth_open):
            return False
        if not self._fs_try_sentinel(ct, E, p):
            return False
        self.v527_lastseat_n = getattr(self, "v527_lastseat_n", 0) + 1
        self._v527_log("LASTSEAT", rnd, "orth", orth_open,
                       "need", len(needed), "ti", ti,
                       "n", self.v527_lastseat_n)
        return True

    def _v527_turret_first(self, ct, E, p, rnd, needed, ti, orth_open):
        """⭐ M2(a) -- A DISPATCHED BODY BUYS THE TURRET FIRST.

        Set on the body at appointment (`main.py`), never re-derived here: a
        body appointed under the PSURV state exists BECAUSE the collar has no
        turret, so rung 4 outranks rung 1 for it until one is standing.  The
        tag clears itself the moment a turret exists, so this cannot become a
        permanent second ladder.
        """
        if not (self._v527_on() and FS_V527_PSURV
                and FS_V527_PSURV_TFIRST):
            return False
        if not getattr(self, "v527_tfirst", False):
            return False
        if not self._v527_no_turret(ct):
            self.v527_tfirst = False         # the job is done; ordinary ladder
            return False
        if not self._fs_sentinel_ok(ct, ti, needed, orth_open):
            return False
        if not self._fs_try_sentinel(ct, E, p):
            return False
        self.v527_tfirst = False
        self.v527_tfirst_n = getattr(self, "v527_tfirst_n", 0) + 1
        self._v527_log("TFIRST", rnd, "orth", orth_open, "ti", ti,
                       "n", self.v527_tfirst_n)
        return True

    # --- M3: the seal-path order -------------------------------------------

    def _v527_apron(self, ct, E):
        """The bounded flood domain: tiles within FS_V527_PATH_DSQ of the enemy
        core footprint that a builder of ours could stand on.  Cached per round
        per body -- the census already walks the ring, and this walks one apron
        outside it, so it must not be recomputed per candidate seat."""
        try:
            rnd = ct.get_current_round()
        except Exception:
            return frozenset()
        if getattr(self, "v527_apron_rnd", None) == rnd:
            return self.v527_apron_set
        ok = set()
        r = int(FS_V527_PATH_DSQ ** 0.5) + 2
        for dx in range(-r, r + 1):
            for dy in range(-r, r + 1):
                ...
                if not (0 <= x < self.mw and 0 <= y < self.mh):
                    continue
                t = Position(x, y)
                if dsq_core(t, E) > FS_V527_PATH_DSQ:
                    continue
                try:
                    ...
                    continue
        self.v527_apron_rnd = rnd
        self.v527_apron_set = frozenset(ok)
        return self.v527_apron_set

    def _v527_path_cuts(self, ct, E, p, t, needed):
        """⭐ M3 -- WOULD A BARRIER ON `t` CUT US OFF FROM THE SEATS WE STILL
        OWE?  Magnus, marker 14: "builder blocked its way to the next barrier
        with this barrier".

        A bounded flood from `p` over the apron with `t` removed.  A seat is
        "reachable" when the flood touches a tile orthogonally adjacent to it
        -- which is the arrival-by-adjacency convention every build in this bot
        uses (`_bfs_direction` returns CENTRE on arrival at an adjacent tile).

        ⛔ IT ANSWERS ONLY THE QUESTION IT CAN ANSWER.  Seats already
        unreachable BEFORE the barrier are excluded, so this reports the
        barrier's OWN contribution and not the map's.  Returns False whenever
        the domain is degenerate -- the fail-open direction, because the
        parent's behaviour is the fallback and it is not a defect, only a
        slower one.
        """
        if not (self._v527_on() and FS_V527_SEALPATH):
            return False
        apron = self._v527_apron(ct, E)
        if not apron:
            return False
        rest = [s for s in needed
                if (s.x, s.y) != (t.x, t.y)
                and (s.x, s.y) not in self.fs_blocked_now]
        if not rest:
            return False
        ...
        def _reach(block):
            ...
            while q:
                ...
                for dx, dy in CARD_DELTAS:
                    nx, ny = cx + dx, cy + dy
                    if (nx, ny) in seen or (nx, ny) == block:
                        continue
                    if (nx, ny) not in apron:
                        continue
                    seen.add((nx, ny))
                    q.append((nx, ny))
            return seen

        after = _reach((t.x, t.y))

        def _touch(seen, s):
            for dx, dy in CARD_DELTAS:
                if (s.x + dx, s.y + dy) in seen:
                    return True
            return False

        lost = [s for s in rest if not _touch(after, s)]
        if not lost:
            return False
        # ⛔ THE CONTROL FLOOD.  Without it a seat that was ALREADY unreachable
        # (a walled pocket, an enemy building ring) would be charged to our
        # barrier, and the clause would veto rung 1 on maps where it changes
        # nothing.  Only run when the first flood found a loss, so the common
        # case pays for one flood, not two.
        before = _reach(None)
        for s in lost:
            if _touch(before, s):
                return True
    ...
    def _v527_seal_pick(self, ct, E, p, needed, ti):
        """(tile_or_None, all_cut).  M3's answer for rung 1.

        Walks the census order and returns the FIRST orthogonally-adjacent seat
        that does not cut the route to the rest.  `all_cut` is True when every
        adjacent candidate cuts -- the state where marker 14's opportunity-cost
        switch applies.
        """
        cand = [t for t in needed
                if abs(t.x - p.x) + abs(t.y - p.y) == 1]
        if not cand:
            return None, False
        cut = 0
        for t in cand:
            if self._v527_path_cuts(ct, E, p, t, needed):
                cut += 1
                continue
            return t, False
        return None, cut == len(cand) and cut > 0
        ...
        if ct.get_action_cooldown() == 0:
            ...
            # still on the table (FS_V519_GF_TI_FLOOR).  It is BELOW the
                ...
                if self._v519_gunfirst(ct, E, p, rnd):
                    return
            # ⭐ v527 M2, CLAUSES (a)-ORDERING AND (b): BOTH SHIP **OFF**.
            # `_v518_early_sentinel` above already ranks the turret over rung 1
            # whenever no forward sentinel is alive and the gate is open, so
            # both of these are strict subsets of a clause that has already had
            # its round.  Kept, flagged off, and named here so the ladder reads
            # honestly (doctrine.py, the INCUMBENT GREP block).
            if self._v527_turret_first(ct, E, p, rnd, needed, ti, orth_open):
                return
            if self._v527_lastseat_buy(ct, E, p, rnd, needed, ti, orth_open):
                return
            # rung 1 -- BARRIER.
            if FS_SEAL_ON and self._fs_try_seal(ct, E, p, needed, ti,
                                                orth_open):
                ...
                return
            # ⭐⭐ v527 M1 -- RUNG 1''' , THE BUNKER SWAP.  BELOW rung 1 and
            # ABOVE the WAIT, and both halves of that placement are deliberate:
            #   * BELOW rung 1, because a seat we can still barrier is cheaper
            #     denial than a seat we convert -- Magnus's priority ruling 1
            #     is not amended by this build;
            #   * ABOVE THE WAIT, because the WAIT holds the bank for a barrier
            #     and a TRAPPED body with a closed collar has no barrier to
            #     hold it for.  The swap's own gate already reserves the whole
            #     remaining collar (`_fs_sentinel_ok`), so the WAIT would be
            #     redundant here and, for a trapped body, absorbing.
            if self._v527_bunker_swap(ct, E, p, rnd, needed, orth_open, ti):
                self._fs_rung(ct, rnd, 4, E, p, needed, ti, orth_open)
            ...
            if _v521_near or not self._fs_seal_pending(ct, needed, ti):
                ...
                if not _v521_near \
                    ...
                    self._v521_rung(ct, rnd, 4, _v521_st)
                    return
            # ⭐⭐ v541 -- RUNG 5, THE ARRIVED RAIDER'S DAMAGE VERB.
            # BELOW EVERY EXISTING RUNG AND BELOW THE WAIT, deliberately: a
            # barrier, an evictor, a cleared seat and a sentinel all outrank
            # 2 damage, and Magnus's priority ruling 1 is not amended by this
            # build.  Note it is OUTSIDE the `_v521_near or not seal_pending`
            # block on purpose -- the WAIT holds the bank for a BARRIER, and
            # `_v541_funded` already reserves that barrier and the sentinel
            # above it, so holding a bank this verb cannot touch would only
            # mean "never".
            # ⛔ AND IT IS GATED ON THE MOVE, NOT ON ITS PLACE IN THIS LADDER.
            # Being last only proves no better ACTION existed; `_v541_idle_ok`
            # reproduces `_fs_walk`'s own decision and permits the peck only
            # where that decision was "stand still".  That is the clause
            # `_v178salt` did not have, and its absence cost that arm the leg
            # (kill round r179 vs pooled r129, p=0.008).
            if FS_V541_COREPECK and self._v541_idle_ok(ct, E, p, needed) \
                    and self._v541_core_attack(ct, E, p, needed):
                self._fs_rung(ct, rnd, 5, E, p, needed, ti, orth_open)
                return
    ...
    def _fs_supp_turn(self, ct, E, p, rnd, needed, orth_open):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if self._fs_sentinel_ok(ct, ti, needed, orth_open) \
                ...
                self._fs_rung(ct, rnd, 4, E, p, needed, ti, orth_open)
                return
            # ⭐⭐ v541 -- the support body gets the same last verb, under its
            # OWN walker's idle test.  The support is the body most often
            # parked: `_fs_supp_walk`'s whole job is to STAND on a seat and
            # deny it, so once it has arrived at that seat every subsequent
            # round is a round it spends doing nothing.  That is precisely the
            # population the field tape found at d^2 <= 2 of the enemy core in
            # 25 of 25 games, dealing 0 HP.
            # ⛔ It still never places a barrier and never clears a seat --
            # those stay the sealer's, and splitting the verb sets is what
            # keeps the two-body ladder from competing for the same round.
            if FS_V541_COREPECK and self._v541_supp_idle_ok(ct, E, p, needed) \
                    and self._v541_core_attack(ct, E, p, needed):
                self._fs_rung(ct, rnd, 5, E, p, needed, ti, orth_open)
                return
    ...
    def _fs_try_seal(self, ct, E, p, needed, ti, orth_open=0):
        if not needed:
            return False
        ...
        try:
            ...
            bar = 3
        # ⭐⭐ v527 M3 -- THE SEAL-PATH ORDER.  Magnus, marker 14: "(10,19) r21
        # builder blocked its way to the next barrier with this barrier."  The
        # census orders on arc / NW-bias / wear / distance and has NO term for
        # whether the tile we are about to fill is the tile we must walk
        # THROUGH to reach the seats we still owe.  `_v527_seal_pick` re-walks
        # the SAME census order and returns the first adjacent seat that does
        # not cut; `order` below is that seat hoisted to the front, so when
        # nothing cuts this loop is the parent's, tile for tile.
        order = needed
        _cut_all = False
        if self._v527_on() and FS_V527_SEALPATH:
            _pick, _cut_all = self._v527_seal_pick(ct, E, p, needed, ti)
            if _pick is not None:
                order = [_pick] + [t for t in needed
                                   if (t.x, t.y) != (_pick.x, _pick.y)]
                # ⛔⛔ THE DOSE COUNTER, AND ITS BASELINE IS THE ONE THING
                # EASY TO GET WRONG -- THIS BUILD GOT IT WRONG FIRST.  The
                # parent does NOT build on `needed[0]`: it builds on the first
                # seat in census order that is ORTHOGONALLY ADJACENT to the
                # body, because the loop below skips the rest.  Comparing
                # against `needed[0]` counts every round whose census head is
                # merely out of reach.
                # MEASURED ON THE SAME 30 DETERMINISTIC CELLS, both counters,
                # same games: `needed[0]` baseline **624**, first-adjacent
                # baseline **6** -- a 104x over-report.  Control: SELFCUT,
                # which the fix does not touch, reads 108 in both.
                # The baseline is therefore the first ADJACENT seat.
                _first_adj = None
                for _t in needed:
                    if abs(_t.x - p.x) + abs(_t.y - p.y) == 1:
                        _first_adj = _t
                        break
                if _first_adj is not None \
                        and (_pick.x, _pick.y) != (_first_adj.x, _first_adj.y):
                    self.v527_selfcut_avoid = \
                        getattr(self, "v527_selfcut_avoid", 0) + 1
            elif _cut_all:
                # ⭐ MARKER 14's OPPORTUNITY-COST SWITCH.  Every adjacent seat
                # cuts our own route to the rest of the collar.  The parent
                # seals anyway and pays for a lap of the ring; here, IF THE
                # SENTINEL GATE IS OPEN, the turret takes the round instead --
                # "when the next seat requires a detour AND the gate is open,
                # the turret outranks the walk", his words.
                # ⛔ THE GATE IS THE PARENT'S, UNRELAXED.  With it shut this
                # falls through to the parent's behaviour exactly: seal, and
                # pay for the lap.  A detour is not a licence to spend.
                self.v527_selfcut_n = getattr(self, "v527_selfcut_n", 0) + 1
                if FS_V527_PATH_SWITCH \
                        and self._fs_sentinel_ok(ct, ti, needed, orth_open) \
                        and self._fs_try_sentinel(ct, E, p):
                    self.v527_switch_n = getattr(self, "v527_switch_n", 0) + 1
                    self._v527_log("SWITCH", rnow, "adj", "allcut",
                                   "n", self.v527_switch_n)
                    return True
                self._v527_log("SELFCUT", rnow, "n", self.v527_selfcut_n)
        for t in order:
            if abs(t.x - p.x) + abs(t.y - p.y) != 1:
                continue
            if not gate:
    ...
    # --- v541: THE ARRIVED RAIDER'S DAMAGE VERB -----------------------------
    # Doctrine, the field measurement and the LOKI-SALT precedent that prices
    # the risk: doctrine.py, the `FS_V541_COREPECK` block at the end of the file.

    def _v541_station(self, ct, E, p, needed):
        """`_fs_stand_target` memoised for this body, this round.

        The idle probe and the walker that follows it ask the same question,
        and asking twice is a CPU cost for nothing (10 ms per unit per turn).
        Cached on the round number, so the two calls cannot disagree either --
        an idle test that read a different station from the walk it is
        predicting would not be a prediction.
        """
        try:
            rnd = ct.get_current_round()
        except Exception:
            return self._fs_stand_target(ct, E, p, needed)
        if self.v541_st_rnd == rnd:
            return self.v541_st
        st = self._fs_stand_target(ct, E, p, needed)
        self.v541_st_rnd = rnd
        self.v541_st = st
        return st

    def _v541_idle_ok(self, ct, E, p, needed):
        """Would the SEALER have moved this round?  If yes, refuse the attack.

        ⭐ THIS IS THE WHOLE SAFETY ARGUMENT OF THE PLANK, and it is LOKI-48's
        (`_salt_idle_ok`, raid.py) with the siege walker mirrored instead of
        the raid walker.  Being last in the rung ladder only proves no better
        ACTION existed; the parent's next move after a declined action is to
        WALK, and an action cancels that walk (acting and moving are mutually
        exclusive for a builder).  `_v178salt` died on exactly that
        unmeasured step -- kill round r179 against a pooled r129, p=0.008.

        So this reproduces `_fs_walk`'s decision, clause for clause, and
        permits the attack only in the three states where that decision moves
        nothing:
          * move cooldown non-zero -- `_fs_walk` returns before it picks a
            station, so the round is free by construction;
          * no station at all (`st is None`) -- `_fs_walk`'s own early return;
          * already standing on the station it picked -- `_fs_walk`'s
            `st == p` branch.
        No side effect: `_fs_walk` writes `self.tgt` only on the branch that
        actually navigates, which is the one branch this returns False for.
        """
        try:
            if ct.get_move_cooldown() != 0:
                return True
            ...
            return False
        try:
            st = self._v541_station(ct, E, p, needed)
        except Exception:
            return False
        if st is None:
            return True
        return st.x == p.x and st.y == p.y

    def _v541_supp_idle_ok(self, ct, E, p, needed):
        """The same question for the SUPPORT body, whose walker is different.

        `_fs_supp_walk` is body-denial, not a sweep: it walks to the far end of
        the collar and STANDS on a seat.  Mirroring it needs its side effect
        too (`fs_supp_seat`), because on the idle branches this predicate
        replaces the walker rather than preceding it -- so the seat bookkeeping
        is performed here exactly as the branch we are standing in would have
        performed it, or the next round's denial read would be stale.
        """
        try:
            if ct.get_move_cooldown() != 0:
                return True
        ...
        if order and len(needed) >= 2 and FS_CREW_DENY_SEAT:
            ...
            for t in order:
                ...
                if t.x == p.x and t.y == p.y:
                    self.fs_supp_seat = (t.x, t.y)
                    return True          # parent: "already denying it"
                try:
                    if not ct.is_tile_passable(t):
                        continue
                except Exception:
                    continue
                return False             # parent navigates to t
        # parent falls through to the sealer's walker over the reversed list
        self.fs_supp_seat = None
        return self._v541_idle_ok(ct, E, p, order)

    def _v541_funded(self, ct, E, needed):
        """The FULL reserve: the build claims, plus the AMMUNITION clause.

        ⭐ TWO FORMS, AND WHICH ONE APPLIES IS A PROPERTY OF THE CLAUSE, NOT A
        PREFERENCE.  `_v541_funded_build` (below) is the BUILD reserve only;
        this function adds the AMMUNITION clause on top.  The REDIRECT clause
        (`_v541_corefirst`) takes the build form, because in that state the
        titanium is being spent on a conveyor either way; the ADDITIVE clause
        (`_v541_core_attack`) takes this full form, because there the
        alternative really is leaving the titanium in the bank for the
        magazine.  Applying the ammunition clause to the redirect would refuse
        the core and hand the same 2 Ti to the belt.
        """
        try:
            live = self._fs_live_sentinels(ct, E)
            # ⛔⛔ THE AMMUNITION CLAUSE, AND IT IS THE ONE THIS BUILD LEARNED
            # FROM ITS OWN BATTERY RATHER THAN FROM A DOCTRINE.  See the
            # `FS_V541_AMMO_AWARE` block in doctrine.py: a peck is 2 Ti for 2
            # damage (1.00 HP/Ti); the SAME 2 Ti as sentinel ammunition is
            # 2/10 of an 18-damage shot (1.80 HP/Ti).  While a sentinel of ours
            # is alive and its magazine is not already full, EVERY ADDITIVE
            # PECK IS A STRICTLY LOSING CONVERSION of the one bank both draw
            # on -- and `main.py`'s KILL-phase magazine drains that bank to the
            # collar floor, so the titanium it takes is titanium that WOULD
            # have become ammunition.  Measured cost of not having this clause:
            # timely-kill 49.4% vs the parent's 58.3% on the beltbreak2 cell.
            if FS_V541_AMMO_AWARE and live > 0 \
                    and ct.get_global_ammo() < FS_V541_AMMO_MIN:
                return False
        except Exception:
            return False
        return self._v541_funded_build(ct, E, needed)

    def _v541_funded_build(self, ct, E, needed):
        """The BUILD half of the reserve: the collar, and the first sentinel.

        ⛔ THE s50 BELT-LASTLINK AUTOPSY IS THE STANDING WARNING: two reserves
        that were never checked against each other left the bank permanently
        insolvent (2,653 of 2,809 eco denials had a raw bank that covered the
        cost).  A 2 Ti peck is small enough to look free and small enough to
        drain a bank one titanium at a time across fifty rounds -- `_fs_try_
        clear`'s own comment records it doing exactly that, 40 -> 34 across the
        rounds the seal gate wanted 42.

        So the peck is junior to BOTH senior claims on the bank:
          * the whole remaining collar (barriers still owed + FS_SEAL_MARGIN),
            the same expression `_fs_try_clear` uses; and
          * while no forward sentinel is alive, a whole sentinel.  100% of our
            measured damage into enemy cores is sentinel damage; a peck may
            never be the reason the first one is unaffordable.
        """
        try:
            ti = ct.get_global_resources()
            reserve = len(needed) * ct.get_barrier_cost() + FS_SEAL_MARGIN \
                + FS_V541_TI_FLOOR
            if FS_V541_KEEP_SENT and self._fs_live_sentinels(ct, E) == 0:
                reserve += ct.get_sentinel_cost()
            return ti >= reserve
        except Exception:
            return False

    def _v541_core_target(self, ct, E, p):
        """The enemy-core TILE this body may legally attack, or None.

        Orthogonal adjacency to a footprint tile -- the engine's own
        requirement for the builder attack verb, and NOT implied by the field
        report's `arr2` (d^2 <= 2, which includes the diagonals)."""
        if E is None:
            return None
        for c in core_tiles(E):
            if abs(p.x - c.x) + abs(p.y - c.y) == 1:
    ...
    def _v541_fire(self, ct, tgt, tag):
        if self.v541_pecks >= FS_V541_MAX_PECKS:
            return False
        try:
            if ct.get_action_cooldown() != 0:
                return False
        ...
        if FS_V541_LOG:
            try:
                print(tag, ct.get_id(), ct.get_current_round(),
                      "n", self.v541_pecks, "tgt", tgt.x, tgt.y,
                      "ti", ct.get_global_resources(), file=sys.stderr)
            except Exception:
                pass
        return True

    def _v541_corefirst(self, ct, E, p):
        """⭐⭐ ON-SEAT TARGET PRIORITY -- THE CORE BEFORE THE CONVEYOR.

        ⛔ THIS IS THE PLANK'S PRIMARY CLAUSE AND IT IS A *REDIRECT*, NOT AN
        ADDITION.  Premise: `docs/research/AUTOPSY-v174-losses-2026-08-21.md`
        §5.2, which corrects the field debut's framing --

          **MEASURED: 556 builder attacks across ten match games.  517 into
          enemy CONVEYORS, 12 into gunners, 27 onto tiles whose target was
          already dead, and ZERO into an enemy CORE.  419 of those attacks were
          made while standing next to an enemy core; 407 of the 419 went into a
          20 HP conveyor.**

        ⇒ **THE VERB IS NOT SILENT.  IT IS MISDIRECTED.**  `LOKI_QUIET_ON`
        forbids the core (raid.py clause 1) while the LOKI-SALT carve-out
        (doctrine.py:1744) permits the belt, so a raider established on their
        socket spends its one action per turn on the cheapest object in reach
        while the win condition sits one tile away.  kladde g1: bot #3 parked on
        their socket for 64 consecutive rounds and spent 51 attacks on the
        conveyor at (25,27) and 0 on the core.  kladde g3: 229 attacks, 226 into
        conveyors, 0 into the core.

        ⭐⭐ AND THIS IS WHY THE CLAUSE IS CHEAP IN THE ONE CURRENCY LOKI-QUIET
        IS ABOUT.  It does not take a round from MOVEMENT -- it takes a round
        the parent was ALREADY spending on an action, and swaps the target.
        Same 2 Ti, same one action, same body, same tile.  `_v178salt` regressed
        kill round because it BOUGHT actions with steps; this buys nothing with
        steps at all.  **That is a strictly stronger safety argument than the
        idle gate, and it is why this clause sits ABOVE the salt verb while
        `_v541_idlepeck` sits below it.**

        FUNDING, and the asymmetry with `_v541_idlepeck` is deliberate: this
        clause does NOT apply the ammunition clause.  That clause prices "peck
        versus ammunition" and its premise is that the titanium would otherwise
        have become a sentinel shot -- FALSE here, where the live alternative is
        a conveyor peck at the identical price.  Refusing the core in this state
        does not save the 2 Ti; it spends it on the worse target.  The collar
        and first-sentinel reserves DO still bind (`_v541_funded_build`).
        """
        if not (FS_V541_COREPECK and FS_V541_COREFIRST):
            return False
        tgt = self._v541_core_target(ct, E, p)
        if tgt is None:
            return False
        if not self._v541_finishable(ct, tgt):
            return False
        if not self._v541_funded_build(ct, E, []):
            return False
        return self._v541_fire(ct, tgt, "V541CF")

    def _v541_finishable(self, ct, tgt):
        """⭐⭐⭐ IS THE CORE ACTUALLY FINISHABLE?  THE CLAUSE THE ws1 BATTERY
        FORCED, AND THE ONE THAT RECONCILES THE AUTOPSY WITH THE MEASUREMENT.

        THE MEASUREMENT.  The unconditional redirect -- core over conveyor at
        core adjacency, exactly as the autopsy specified -- was run on the
        DETERMINISTIC (NOISE_OFF) paired fixture, 180 matched cells per
        opponent:

          vs _v488beltbreak2  18 of 180 cells changed.  **ALL 18 the same way:
                              the parent took a timely kill and we did not.**
                              timely-kill 108/180 vs 126/180, McNemar
                              A-only=0 B-only=18, p=0.0000.
          vs _x3r0v173mjolnir 24 of 180 changed.  Timely-kill EXACTLY level
                              (64/64) -- but over the 115 cells where both
                              arms killed, **we were slower in 18 and faster
                              in 0**, sign test p=0.0000.

        **TWO CELLS, ZERO ROWS OF COUNTER-EVIDENCE, IN A FIXTURE WHOSE OWN
        NOISE CONTROL IS 0.**  The redirect is a real negative.

        THE MECHANISM, AND IT IS LOKI-QUIET'S ORIGINAL ARGUMENT ARRIVING FROM
        THE OTHER SIDE.  The autopsy is right that 517-to-0 is absurd; what it
        prices as waste is the LOKI-SALT carve-out doing its job.  A conveyor
        is 20 HP -- TEN pecks -- and the tenth SEVERS A DELIVERY CHAIN: 2
        damage a round FINISHES it.  A 500 HP core that one enemy builder heals
        at +4 does not finish, at 250 pecks and 500 Ti.  **The redirect trades a
        target where the arithmetic inverts for the one target where it never
        has.**  doctrine.py:1744 says this in advance and the battery measured
        the price of ignoring it.

        ⇒ THE SYNTHESIS.  Both are right, about different core HP.  Pecking a
        FULL core instead of cutting a belt is the mistake the battery found;
        pecking a core that is 30 HP from dead while sawing a belt is the
        mistake the autopsy found.  So the redirect is conditioned on the only
        thing that separates them: **IS THIS CORE INSIDE FINISHING RANGE OF THE
        PECK BUDGET THIS BODY STILL HOLDS?**  That makes the clause a FINISHER,
        which is what the brief asked for and what the price of the verb
        supports -- never a solo kill plan.

        ⚠ THE HONEST STATUS OF THIS CLAUSE: the unconditional form is MEASURED
        and refuted; this conditioned form is measured in the build report on
        the same fixture, and the threshold is REASONED (the body's own
        remaining budget), not swept.  A sweep of FS_V541_FINISH_HP is named as
        deferred work, not claimed as done.
        """
        if not FS_V541_FINISH_ON:
            return True
        budget = 2 * max(0, FS_V541_MAX_PECKS - self.v541_pecks)
        cap = min(FS_V541_FINISH_HP, budget)
        if cap <= 0:
            return False
        try:
            ...
            if bid is None:
                return False
            return ct.get_hp(bid) <= cap
        except Exception:
            # ⛔ AN UNREADABLE CORE IS NOT A FINISHABLE ONE.  Failing OPEN here
            # would restore the unconditional redirect on exactly the boards
            # where the read is flaky, which is the form the battery refuted.
            return False

    def _v541_core_attack(self, ct, E, p, needed):
        """THE ADDITIVE CLAUSE: attack the enemy core on a round the parent
        spent doing NOTHING AT ALL.

        ⭐ THE DISTINCTION FROM `_v541_corefirst` IS THE WHOLE DESIGN AND IT IS
        A DIFFERENCE IN WHAT THE ROUND WAS ALREADY BUYING:
          * the REDIRECT swaps the target of an action the parent was already
            taking -- free in movement, free in titanium;
          * THIS clause adds an action the parent did not take at all.  It is
            therefore the one that has to argue about both currencies, and it
            carries the idle gate (its caller) and the ammunition clause.
        Its intended population is the arr2 -> first-sentinel gap: a body
        established at the ring with no belt in reach to redirect from.

        ⛔ THE TARGET IS THE ENEMY CORE AND NOTHING ELSE.  Destroying an enemy
        BUILDING removes its contribution to their cost scale and therefore
        makes everything they build cheaper -- demolition is a gift (CLAUDE.md,
        guard-matrix sweep).  The core is the one target where the damage is
        the win condition itself, which is why this carve-out exists and why it
        cannot be widened without a different argument.

        MEASURED ABSENCE THIS FILLS: builder-attack damage into an enemy core
        is 0 HP in 25 of 25 rated games (FIELD-DEBUT-v174-2026-08-21 §5.3),
        while an own builder stands at d^2 <= 2 of that core in 25 of 25.
        """
        if not (FS_V541_COREPECK and FS_V541_IDLEPECK):
            return False
        tgt = self._v541_core_target(ct, E, p)
        if tgt is None:
            return False
        if FS_V541_NEED_SENTINEL:
            try:
                if self._fs_live_sentinels(ct, E) == 0:
                    return False
            except Exception:
                return False
        if not self._v541_funded(ct, E, needed):
            return False
        return self._v541_fire(ct, tgt, "V541IP")
    ...
    def _fs_try_sentinel(self, ct, E, p, probe=False):
        ...
        for dx, dy in CARD_DELTAS:
            ...
            for target in tiles:
                ...
                # ⭐ v527 M1b -- THE DEFENDED-TILE PREFERENCE.  Magnus, marker
                # 5: "perfect spot for a Sentinel, defended behind the Launcher
                # already there".  ORDERING TERM: +20 sits below the gun-axis
                # penalty (64) so it can never buy a tile back onto a visible
                # enemy gunner's ray, and below the side penalty (24) so it
                # cannot collapse the twin onto one side of the core.
                if self._v527_on() and FS_V527_DEFENDED \
                        and self._v527_defended(ct, bx, by):
                    score += FS_V527_DEFENDED_BONUS
                if best_k is None or score > best_k:
                    best, best_k, best_ray = (bp, facing), score, target
        if best is None:
