...
# ===========================================================================
# 1.9  v613 -- THE ANTI-APRON AXIS.  Five planks, ranked cheapest first, every
...
SK_TUBE_SEAT_FIELDS = (0, 22)  # slot 7 b0-9 = seat 0, b22-31 = seat 1.  These
                          # are the bits v603's site/dx/dy diagnostics used.
SK_TUBE_PHASES = 3        # slot-7 write phases: seat 0 -> 0, seat 1 -> 1,
                          # SIEGE ENGINEER -> 2.  ⛔ THIS IS WHAT KEEPS ONE
                          # WRITER PER ROUND once the tubes write for
                          # themselves; see the SK_TEAM_TUBES note for the
                          # measured failure it replaces.
SK_TUBE_STALE = 3         # rounds.  ⛔ IT IS DERIVED, NOT TUNED.  A seat writes
                          # on every 3rd round and the store is buffered one
                          # round, so a live tube's beat is age 1 on the round
                          # after its write and age 3 the round before its next
                          # one -- max 3, exactly.  Lower reads a LIVE tube as
                          # dead; higher only delays noticing a dead one.
                          # Cost as set: a dead tube reads alive for <= 5 rounds.
