...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- LOKI-SENTSHELL (#73) shell obligation, per raider.  Per-unit and
        # NOT a store slot: all 16 slots are already allocated (doctrine.py
        # SLOT_* block), and every other raider re-derives the same obligation
        # from vision in _shell_sentinels, so nothing needs communicating. ---
        self.shell_sent = None
        self.shell_rnd = -10 ** 9
        self.shell_stall = 0
        self.shell_stall_rnd = -1
        ...
        # --- LOKI-BODYBLOCK (v262) terminal-fallback state, PER UNIT.
        # The comms store is 16 GLOBAL slots shared by every unit on the team,
        # so it physically cannot hold a per-bot trail; and this tree already
        # keeps every per-bot movement memory as an instance attribute
        # (`pave_prev`, `raid_prev`, `last`, `raid_ban`, `salt_marks`), because
        # a Player instance is constructed per unit and lives as long as the
        # unit does.  bb_trail is the anti-oscillation memory and is a bounded
        # deque, so it cannot grow with the match; bb_seen and bb_ban are both
        # pruned where they are written. ---
        self.bb_idle = 0        # consecutive idle-AND-free rounds
        self.bb_tgt = None      # the committed blocking tile, or None
        self.bb_rescan = 0      # round at which that tile is re-validated
        self.bb_stall = 0       # consecutive blocked steps toward it
        self.bb_rnd = -1        # round `_bb_drive` last ran (idempotence)
        self.bb_trail = deque(maxlen=LOKI_BB_TRAIL)
        self.bb_ban = {}        # abandoned destinations -> expiry round
        self.bb_seen = {}       # enemy builder id -> (round, x, y)
        self.bb_pecks = 0
        self.bb_icepts = 0      # lifetime budget, LOKI_BB_ICEPT_MAX
    ...
    def _builder(self, ct):
        ...
        # LOKI-BODYBLOCK (v262).  Read the cooldowns BEFORE the ladder so the
        # terminal can tell "emitted nothing" from "was not free to".  Both
        # reads are inside the flag: with the flag off this file executes the
        # same statements in the same order as _v223sealrepair.
        bb_ac0 = bb_mc0 = None
        if LOKI_BODYBLOCK_ON:
            try:
                bb_ac0 = ct.get_action_cooldown()
                bb_mc0 = ct.get_move_cooldown()
            except Exception:
                bb_ac0 = bb_mc0 = None
        ...
        # THE TERMINAL.  Everything above is the ladder; this runs only on the
        # rounds the ladder declined every verb it has.  See doctrine.py.
        if LOKI_BODYBLOCK_ON and bb_ac0 is not None:
            self._bb_terminal(ct, bb_ac0, bb_mc0)
