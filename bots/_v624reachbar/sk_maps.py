...
# --- COPY 9, the cage ------------------------------------------------------
SK_CAGE_REACH_BAR = True     # ⭐ v624 L1 -- the REACHABILITY form of "the bar
                             # is met": when the ring-wide census holds NO empty
                             # seal seat (walls/off-map count sealed, their belt
                             # seats count occupied) and this body stands on a
                             # seal tile with seal-behind/seal-adjacent/eviction
                             # all declined THIS round, seal work does not exist
                             # anywhere this round and the core peck is admitted.
                             # Distinct from the refuted SK_CAGE_CEIL (which
                             # lowered `accept` and thereby ENDED seal work):
                             # `accept` is untouched, every seal/evict action
                             # keeps priority above this branch, and a knocked-
                             # out seat reopens `empty_seals` next round, which
                             # closes the branch until re-sealed.  Dose basis:
                             # docs/research/DIAG-cagegate-decomposition
                             # -2026-08-22.md (sealed>=7 concordance 65/65;
                             # 1,960 non-firing core-adjacent walker rounds in
                             # 54/65 live first-contact games; fixture
                             # precondition present F1 3/30 games / F2 14/30).
