...
SK_CAGE_CYCLE_TABOO = True   # ⭐ v625 L2 -- the shuttle-rate fix FIX 3 is not:
                             # the A-B-A-B detector needs two full periods to arm
                             # and the hold resets it, so the walker re-enters the
                             # same 2-cycle immediately (measured: 53.5% of live
                             # ring moves are period-2 returns WITH FIX 3 live;
                             # fixture 53.4%/31.6% -- DIAG-cagegate-decomposition
                             # -2026-08-22.md §2.5 + tail).  After a FIX-3 hold,
                             # keep the struck back-tile taboo for
                             # SK_CYCLE_TABOO_ROUNDS so the cycle cannot restart;
                             # a held walker falls through to the peck ladder.
                             # SCOPE: CAGE WALKER only, never a hurt body
                             # (SK_DISENGAGE keeps priority), taboo cleared by
                             # the displacement guard (build rule 5).
SK_CYCLE_TABOO_ROUNDS = 4    # taboo lifetime after each hold, in rounds
