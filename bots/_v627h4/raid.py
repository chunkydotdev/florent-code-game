...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        # 0. LOKI-COLLARSEAL: BUILD BEFORE YOU ATTACK.  A delivered body
        # converts to a core attack 1.55% of the time and lives five rounds
        # (157,816 INSERT throws) -- the peck below is not what an arrived
        # body is for.  Same routine as step 2, lifted above the peck and
        # widened from the 8 heal seats to all 12 SPAWN tiles.
        if LOKI_COLLARSEAL_ON and self._collar_seal(ct, E, ti, seatkeys):
            return True
        ...
        # 1b. LOKI-SPAWNLOCK: the launcher goes in BEFORE the seal.  Fix (b):
        # step 2 below returns as soon as it lays one barrier, so a launcher
        # branch placed after it is unreachable on every round a seat is free,
        # and the seal additionally deletes the tiles a spawn-lock launcher
        # would want to sit on.  Ordered here, the launcher takes the tile it
        # needs once and the seal takes every remaining seat forever after.
        if self._try_spawnlock_launcher(ct, E):
            return True
    ...
    def _collar_seal(self, ct, E, ti, seatkeys):
        """Barrier one of the enemy core's TWELVE spawn tiles.

        The parent seals the 8 heal seats only, and does it below the peck.
        The 4 diagonal CORNERS are spawn tiles too (CORE_SPAWNING_RADIUS_SQ=2)
        but they are also our own raiders' standing stations -- a corner is
        where a body stands to reach the two seats flanking it.  So a corner is
        converted to a barrier only once BOTH of its flanking seats are already
        sealed, at which point the station has no job left and the corner is
        pure spawn denial.  _open_seats_by is the parent's own predicate for
        that and counts an unreadable seat as OPEN, so the conversion is
        refused rather than guessed at.
        """
        if not LOKI_BARRIER_SEAL_ON:
            return False
        try:
            if ti < ct.get_barrier_cost() + LOKI_SEAL_TI_FLOOR:
                return False
        except Exception:
            return False
        p = ct.get_position()
        corners = set()
        try:
            corners = {(c.x, c.y) for c in self.raid_corners}
        except Exception:
            corners = set()
        for d in CARDINALS:
            t = p.add(d)
