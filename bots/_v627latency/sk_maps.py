...
SK_TUBE_LATENCY_SOLO = False # ⭐ v627 (QUEUE #124) -- UN-WELD THE LATENCY HALF
                             # FROM THE CLOSED FLOOR ROAD.  PREPREP and STAGE
                             # were deliberately split from SK_TUBE_FLOOR2 "so
                             # the spending half can be priced" -- and then both
                             # call sites conjoined them back onto SK_TUBE_FLOOR2
                             # = False (sk_roles :5441-:5442 and :6273): the
                             # fifth weld-class instance, found at the #124
                             # admission.  This master admits the latency half
                             # ALONE: in the live>=want hold state the engineer
                             # stages at (STAGE) or pre-preps (PREPREP) the next
                             # band site instead of idling beside the newest
                             # tube.  NO floor semantics: `want` is untouched and
                             # no purchase is ever refused by this flag.  The
                             # dose evidence: the engineer is SITE-limited (648
                             # NOSITE / 150 NOFUND per 67 purchases, 30 F1
                             # games) and replacement latency is the unclosed
                             # half of the v614/v620 closures.
