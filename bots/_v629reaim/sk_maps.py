...
# --- v629 -- THE HOME ANSWER (FOCUS=defence, Magnus 2026-08-22): three planks,
# all ZERO-purchase / ~zero-ammo -- the only class the line's seven measured
# purchase-plank negatives leave open (DESIGN-v629-homeanswer §1.3/§4).
SK_REAIM_CAP_LIVE = True     # ⛔ un-weld the SIXTH weld instance: the rotation
                             # cap was gated on two False masters (sk_roles
                             # :7181), so the empty-ray rotation loop ships
                             # UNCAPPED -- measured 30 rotations = 300 Ti in one
                             # fixture game (s56 rotations instrument).
SK_DOOR_REAIM = True         # ⭐ PLANK A: a gunner may rotate to bring a MARKED
                             # tile (slot-15 corefire shooter / slot-2 home
                             # threat / inferred belt killer) into its ray,
                             # PRE-EMPTING a current target of strictly lower
                             # priority.  Today rotation happens ONLY on an
                             # empty ray -- churn without aim.  OFF = exact
                             # identity ("rotate only when the ray is empty").
SK_DOOR_REAIM_PRI = 5        # SK_PRI_MARKED: only a marked tile buys a re-aim
SK_DOOR_REAIM_CAP = 2        # PER TURRET PER GAME, on top of the (now live)
                             # global rotation cap
SK_DOOR_REAIM_FLOOR = 40     # bank left standing after the 10 Ti
SK_ARMED_SWEEP = True        # ⭐ PLANK B (#121): drop an armed_memo/armed_facing
                             # tile that sits INSIDE this body's vision and was
                             # NOT re-sensed this round -- the liveness form,
                             # not a TTL (SK_COUNTER_LIVE_TGT's naive
                             # invalidation measured worse alone).  Free.
SK_ARMED_SWEEP_MARGIN = 2    # d² shaved off vision; boundary-flap guard
