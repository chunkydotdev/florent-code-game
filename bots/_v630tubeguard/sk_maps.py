...
# --- v630 -- SK_TUBE_GUARD: screen the tube toward the enemy + babysit it ---
SK_TUBE_GUARD = False        # ⭐⭐ v630 THE TUBE GUARD.  Evidence chain, all
                             # banked before this flag existed: the drip study
                             # (STUDY-drip-collapse-decomposition-2026-08-22)
                             # measured 59.4% of our contact turrets dying at
                             # life 23r vs 130r fixture, 79% killed by enemy
                             # GUNNERS at median d²=4 for 7 dmg/round, with one
                             # of our builders adjacent in 9.2% of deaths; the
                             # engine probe (scratchpad/s56_probe_barrier)
                             # showed a barrier on the intervening tile flips
                             # the gunner's target to the barrier (occlusion
                             # holds until its 30 HP falls); sentinel shots
                             # ignore obstacles, so a front screen never blocks
                             # our own tube.  THE DESIGN FACT: builders build
                             # only on cardinal-adjacent tiles, so orienting
                             # the screen toward the enemy IS standing on the
                             # enemy side of the site — and that tile IS the
                             # babysit seat.  ON, three changes, all in
                             # _siege_engineer: (1) walks to prep/hold target
                             # the site's cardinal neighbour toward the enemy
                             # core (fallback: the old target if that seat is
                             # unreachable — a bias, never a refusal state);
                             # (2) a heal rung (_heal_action, 1 Ti -> +4 HP,
                             # most-damaged-first — under occlusion that is
                             # naturally the screen barrier) runs in the hold
                             # branch AHEAD of the _preprep/STAGE gate, so
                             # STAGE becomes the fallback when nothing adjacent
                             # is damaged.  No new state (mechanism diagnostics
                             # read from replays: HP-regen events = heals).
                             # ⛔ NOT conjoined with SK_TUBE_FLOOR2 or
                             # SK_TUBE_RELIGHT (both permanently False — the
                             # weld class, three shipped instances).  OFF = the
                             # guard rung and the seat bias vanish at their
                             # call sites; control flow is v628's line for line.
                             # ⚠ Registered confound: ON keeps the engineer in
                             # vision of the tube, which silences the phantom-
                             # death booking (_nest_watch books an out-of-vision
                             # get_hp raise as a death) — part of any measured
                             # effect may be ledger accuracy, so the powered
