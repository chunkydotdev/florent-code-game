...
LAUNCHER_RESERVE = 80
# Outside this radius-squared from the Core, an outward-walking expander faces
# each new trail conveyor back at the tile it came from (exact trail link);
# inside it, the original dominant-axis toward-Core rule is kept as the global
# anchor. Value measured on our own line (bots/ladder1), not re-tuned here.
            ...
            if ct.read_store(SLOT_HARVESTERS) >= 1 and ct.get_global_resources() >= ct.get_conveyor_cost():
                if dist_core(nxt, self.core) > 0:
                    ...
                    if abs(nxt.x - self.core.x) + abs(nxt.y - self.core.y) < abs(here.x - self.core.x) + abs(here.y - self.core.y):
                        # Inward step: keep the original dominant-axis facing.
                        ...
                        # Outward step (ported from ladder1, accepted at 58.5%
                        # on our line): face the new trail conveyor back at the
                        # tile we are stepping off -- an exact link that
                        # survives bends and detours. Only safe anchored to a
                        # global rule: inside NEAR_CORE_FACING_DIST_SQ the
                        # converging dominant-axis field is kept, so trails
                        # terminate on the Core instead of on each other
                        # (a per-builder local rule with no anchor measured
                        # 43.6% of conveyors in closed cycles).
                        if nxt.distance_squared(self.core) > NEAR_CORE_FACING_DIST_SQ:
                            card = d.opposite()
                        else:
                            card = nearest_cardinal(nxt.direction_to(nearest_core_tile(nxt, self.core)))
                        if ct.can_build_conveyor(nxt, card):
