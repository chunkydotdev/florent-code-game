    ...
    def _core(self, ct):
        ...
        if self.n < spawn_cap and can_spend_spawn and ti >= ct.get_builder_bot_cost():
            ...
            if self.n == 0 and ec is not None:
                ...
                # but the old primary key was a raw-coordinate hash
                # ((x*17 + y*31 + n*13) % 97) -- maximally non-equivariant.
                # Mirrored ring tiles land on unrelated hash ranks, so the two
                # seats of a symmetric map opened with systematically different
                # builder placements.  Order the ring only on quantities that
                # survive the rotation: distance from the map centre (integer,
                # scaled by 4 to stay off floats) and distance to our own Core.
                # Raw coordinates stay last purely to guarantee a total order.
                # Dispersion then comes from rotating that fixed order by the
                # builder index instead of from the hash: 5 shares no factor
                # with the ring sizes we see (24, 16, 12), so successive
                # builders still stride well around the ring -- and because the
                # list being rotated is itself equivariant, mirrored seats
                # rotate through mirrored tiles in lockstep.
                mw1 = w - 1
                mh1 = h - 1
                cands.sort(key=lambda sp: (
                    (2 * sp.x - mw1) ** 2 + (2 * sp.y - mh1) ** 2,
                    (sp.x - p.x) ** 2 + (sp.y - p.y) ** 2,
                    sp.y, sp.x,
                ))
                off = (self.n * 5) % len(cands)
