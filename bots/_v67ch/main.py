...
class Player:
    ...
    def _builder(self, ct):
        ...
        # teams, heals delivered to our own Core: loss 0, loss 0, win 82
        # (+328 HP, the siege was survived).  The only difference was whether
        # the single role_n == 4 defender happened to be free that round --
        # healing was a role, not a reflex.  It is proximity work: any builder
        # standing beside a Core that is actually losing HP repairs it, before
        # any melee/sabotage short-circuit below can claim the action.
        # SLOT_UNDER == 2 (not 1) is the gate -- see _core: 2 means the Core
        # measurably lost HP within 20 rounds, so this cannot fire on the
        # spawn-tile proximity noise that level 1 also reports.  _heal_core
        # walks the 2x2 footprint and lets can_heal() enforce orthogonal
        # adjacency, so a builder that is merely near the Core is unaffected.
        if ct.get_action_cooldown() == 0 and ct.read_store(SLOT_UNDER) == 2:
        ...
        if hive_bunker and ct.get_action_cooldown() == 0:
            ...
            if abs(p.x - bp.x) + abs(p.y - bp.y) == 1:
                ...
                if (
                    ...
                    ct.build_barrier(bp)
                    return
        under_lvl = ct.read_store(SLOT_UNDER)
        under = under_lvl != 0
        # Level 2 = the Core has measurably lost HP; level 1 may be proximity
        # noise.  Only the proven case reorders the action priority below.
        shelled = under_lvl == 2
        chase_battery = (
            self.mw == 20 and self.mh == 26
