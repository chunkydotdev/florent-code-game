...
# Minimum one-round Core HP loss that counts as turret-class damage and so
# escalates SLOT_UNDER to 2 (which unlocks the free pre-economy home battery
# in _try_counterbattery).  Damage table: builder melee 2/hit, gunner 7,
# sentinel 18.  Measured need: _v67hg2 escalated on ANY HP drop, so a single
# enemy builder chipping the Core for 2/round bought a fixed-facing Sentinel
# before the first harvester existed -- on fjordgate (10x10) that halved the
# sweep against opp_v50, 32 -> 16.  5 is the only break that ignores one or
# two simultaneous peckers (2 or 4) while still catching every gun.
#
# Accepted edge: the Core acts first each round, so this delta aggregates a
# whole round of enemy actions, and the 2x2 footprint has 8 orthogonally
# adjacent tiles -- 3+ enemy builders pecking at once sum to >= 6 and do
# escalate.  That is fine: three adjacent attackers on the Core is a real
# assault, not chip noise, and is arguably worth a battery on its own.
CORE_HIT_MIN_DMG = 5
...
SLOT_LAUNCH_RND = 11
SLOT_LAUNCHED_ID = 12
# Slot 13 was SLOT_HOME_SENT, declared but never read or written -- reclaimed.
# SLOT_HOME_GUN now counts only home-defence emergency batteries; forward
# siege turrets get their own counter so an opening siege Sentinel no longer
# spends the home battery's free-first allowance (see _try_counterbattery).
# Both are monotonic build counters, like SLOT_HARVESTERS: the store defaults
# to 0 = "none built" and is only ever written >= 1 after a successful build,
# so no +1 offset encoding is needed (unlike pack_pos, which reserves 0).
SLOT_FORWARD_GUN = 13
SLOT_THREAT = 14
SLOT_SIEGE = 15
...
class Player:
    def __init__(self):
        ...
        self.last_hp = None
        # Round of the last confirmed core HP drop.  Proximity to an enemy
        # unit is noise; a shrinking core is proof, and only proof unlocks
        # the free pre-economy home battery.
        self.core_hit_rnd = None
    ...
    def _core(self, ct):
        ...
        if self.last_hp is not None and hp < self.last_hp:
            under = True
            # Any drop still raises the proximity-level response (SLOT_UNDER 1).
            # Only turret-class damage escalates to 2 -- see CORE_HIT_MIN_DMG.
            if self.last_hp - hp >= CORE_HIT_MIN_DMG:
                self.core_hit_rnd = rnd
        self.last_hp = hp
        # SLOT_UNDER is 0/1/2: 1 = threat seen (may be spawn-tile noise),
        # 2 = the core itself has actually lost HP in the last 20 rounds.
        # Every reader treats it as a truthy flag; only _try_counterbattery
        # distinguishes 2, to unlock its free pre-economy battery.
        hit = self.core_hit_rnd is not None and rnd - self.core_hit_rnd <= 20
        if under:
            ct.write_store(SLOT_UNDER, 2 if hit else 1)
            ct.write_store(SLOT_ATK_RND, rnd)
        else:
            last = ct.read_store(SLOT_ATK_RND)
            under = bool(last and rnd - last < 35)
            ct.write_store(SLOT_UNDER, (2 if hit else 1) if under else 0)
        ...
        # "Do we own any turret at all" -- home batteries plus forward siege
        # guns.  Same total the single counter used to hold before the split.
        weapons = ct.read_store(SLOT_HOME_GUN) + ct.read_store(SLOT_FORWARD_GUN)
    ...
    def _builder(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            if (et in (EntityType.GUNNER, EntityType.SENTINEL) and d <= 64) or (
                ...
                # Spawn order runs the core first, builders after, and the last
                # write of a round wins -- so an unguarded 1 here overwrites the
                # core's SLOT_UNDER == 2 escalation every single round and the
                # damage-gated battery gate can never be read.  Reads see last
                # round's committed value, so guarding on 0 lets the builder
                # raise the alarm once and then yield the slot to the core.
    ...
    def _try_siege_build(self, ct):
        ...
        if built:
            self.forward_guns += 1
            ct.write_store(SLOT_FORWARD_GUN, ct.read_store(SLOT_FORWARD_GUN) + 1)
    ...
    def _try_counterbattery(self, ct):
        ...
        if threat is None:
            return False
        # Pre-economy gate balancing two measured failure modes.
        #   fjordgate: a free first battery granted on mere proximity noise
        #   buys fixed-facing Sentinels aimed at transient spawn tiles before
        #   the first harvester exists -- opening bankruptcy, 45.6% vs opp_v50.
        #   meander: a shelled core that could never afford a battery, because
        #   an r2 forward siege Sentinel had eaten the free-first allowance and
        #   harassment kept harvesters below ECO_NEED -- lost to a 1745 team.
        # So before ECO_NEED harvesters a home battery needs SLOT_UNDER == 2
        # (the core is actually losing HP, not merely being loitered near),
        # and even then only the first one is free; SLOT_HOME_GUN only, since
        # a forward siege turret is offence and must not consume the home
        # battery's allowance.  After ECO_NEED the gate passes as before.
        if ct.read_store(SLOT_HARVESTERS) < ECO_NEED:
            if ct.read_store(SLOT_HOME_GUN) >= 1 or ct.read_store(SLOT_UNDER) != 2:
                return False
        p = ct.get_position()
        choices = (
            (
    ...
    def _expand(self, ct):
        ...
        hive_freeze = (
            ...
            and ct.read_store(SLOT_HOME_GUN) + ct.read_store(SLOT_FORWARD_GUN) >= 1
