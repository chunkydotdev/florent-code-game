...
ECO_CAP = 18
ECO_NEED = 3
# ECONOMY-SCALED BUILDER CAP.  Measured across three replays vs 1650-1776
# teams: five builders is enough hands to open with and never enough to
# recover.  In the eider loss the single defend unit died at round 36 and no
# home turret was built for the remaining 252 rounds while 1124 titanium sat
# banked; in the meander loss five units could not run the economy, hunt a
# saboteur and rebuild at the same time; even the win vs a 1745 team ran on
# five thin hands.  The opening stays at EARLY_BUILDERS -- it is not the
# opening that is short-handed -- but once the economy is genuinely paying
# (ECO_NEED harvesters and up) each further pair of live harvesters funds one
# more builder, to ECO_BUILDERS_MAX.  Builder bots are the joint most
# scale-expensive entity (+20% each, docs/strategy-notes.md), so the ceiling
# is deliberately low: 8 builders is +60% scale over the old cap, paid only by
# a team that already has six harvesters running.
ECO_BUILDERS_MAX = 8
# Builders spawned as REPLACEMENTS for dead units, over the whole match, above
# whatever the economy-scaled cap allows.  See the live-builder accounting in
# _core: the Core cannot count live builders exactly, so this is the bound on
# how wrong that estimate is allowed to be.
BUILDER_REPLACEMENTS = 3
# A replacement is surplus titanium turned into labour, never a purchase that
# starves the economy: only spend down to this floor for one.
REPLACE_TI_FLOOR = 120
...
class Player:
    ...
    def _core(self, ct):
        ...
        # Economy-scaled cap (see ECO_BUILDERS_MAX).  The map-specific numbers
        # keep their original meaning: nordkap's 4 is a CEILING -- that layout
        # cannot feed a fifth builder -- so it is a min(); snowflake's 6 is a
        # FLOOR that the old flat cap could not express, so it is a max().
        harv_extra = min(ECO_BUILDERS_MAX - MAX_BUILDERS, harv // 2) if harv >= ECO_NEED else 0
        scaled_cap = MAX_BUILDERS + harv_extra
        mature_cap = (
            min(4, scaled_cap) if nordkap_home_a
            else (max(6, scaled_cap) if snowflake_home_b else scaled_cap)
        )
        spawn_cap = mature_cap if harv >= 1 else min(EARLY_BUILDERS, mature_cap)
        can_spend_spawn = ti >= ct.get_builder_bot_cost()

        # LIVE-BUILDER ACCOUNTING.  self.n counts builders SPAWNED and is never
        # decremented, so on its own it reads every death as a permanent loss
        # of one slot -- exactly the eider failure.  There is no getter for
        # "live builder bots": the Core's vision is r^2=36 and builders work
        # far outside it, and the cost scale is one team-wide number
        # (docs/game-model.md), so get_builder_bot_cost() cannot be inverted
        # into a count either.  get_unit_count() is the only global signal and
        # it lumps Core, builders and every turret together -- so we use its
        # DROPS rather than its value: a decrease between two Core turns is a
        # unit that died, and each one buys back one spawn, BUILDER_REPLACEMENTS
        # for the whole match.  A turret loss therefore also buys a builder,
        # and a builder death hidden by a turret finished the same round buys
        # nothing; both errors are bounded by that constant.
        ...
        replace = min(BUILDER_REPLACEMENTS, self.lost_units) if ti >= REPLACE_TI_FLOOR else 0

        if self.n < spawn_cap + replace and can_spend_spawn and ti >= ct.get_builder_bot_cost():
            cands = ring(p, 2)
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            if n == 0:
                ...
                # Sixth and later builder -- either economy-scaled surplus or a
                # replacement for a dead unit.  Generic expander, deliberately:
                # there is exactly one defend seat (role_n == 4, with the
                # role_n == 2 succession behind it) and one interceptor seat
                # (role_n == 1), and both are single-occupancy by design -- a
                # second defender would double the counterbattery scan and a
                # second interceptor would abandon the economy in pairs.  The
                # generic path is also where the measured shortfall was: not
                # enough hands laying harvesters and conveyors.  This
                # generalises the snowflake role_n == 5 special case below,
                # which is now redundant and left in place only as a no-op.
                self.role = "expand"
