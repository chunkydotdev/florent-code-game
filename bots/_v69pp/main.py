...
# PERIMETER PATROL.  _find_intruder only sees enemy builders inside the
# interceptor's own vision (dist^2 <= 20, about 4.5 tiles), so interception is
# worth exactly as much as the interceptor's standing position.  In the meander
# loss it had none: with no intruder tracked the unit fell through to ordinary
# ore wandering and spent rounds 12-111 on the far side of the map while a
# saboteur killed both harvesters, never once inside sensing range.  Harvesters
# only ever exist on ore tiles, so the ore field in our own half IS the thing
# being guarded, and self.map_ores already holds it exactly on every pool map.
# Rounds the patrol pushes toward one waypoint before writing it off and taking
# the next -- bounds the cost of a waypoint terrain or a building made
# unreachable.
PATROL_STUCK_RNDS = 10
# The patrol stays out of the opening build order: until the first harvester
# exists (or this round, whichever comes first) role_n == 1 expands as before.
PATROL_MIN_RND = 8
...
class Player:
    def __init__(self):
        ...
        # Perimeter patrol state, likewise per unit instance and likewise read
        # and written by role_n == 1 only, so no store slot is spent on it.
    ...
    def _intercept(self, ct):
        ...
        if eid is not None:
            ...
            # Nothing in sight and no chase running: this is the interceptor's
            # idle time, and the diagnosed failure is precisely what it used to
            # do with it -- fall through to ore wandering that walked it off our
            # half for a hundred rounds while a saboteur ate the economy out of
            # sensing range.  Aim it around the home ore field instead, so the
            # per-round scan above has something to find.  _patrol replaces only
            # the movement TARGET and still returns control here: the caller
            # runs its full expand turn (link chain, harvester build, adjacent
            # ore override), so the patrol claims idle travel and nothing else.
            self._patrol(ct)
            return False
        elif (
    ...
    def _patrol(self, ct):
        """Point the idle interceptor at the next home-ore waypoint.

        On an unknown map map_grid is None, map_ores is therefore empty and
        this is a no-op -- the caller's pre-existing behaviour (fall through to
        _expand) is kept exactly.  A committed conveyor chain also outranks the
        patrol: link_queue is a multi-round positional job and leaving one half
        built costs more than a few rounds of standing in the wrong place.
        """
        if not self.map_ores or self.link_queue:
            return
        if ct.read_store(SLOT_HARVESTERS) < 1 and ct.get_current_round() < PATROL_MIN_RND:
            return
        if self.patrol_route is None:
            # Our half is the same plain bisector test _find_intruder uses, so
            # the loop covers exactly the ground the scan is responsible for.
            # Distance to Core first makes the loop start on the tiles a raider
            # reaches first and spiral outward; (y, x) only breaks ties, which
            # keeps the order deterministic without reordering by distance.
            self.patrol_route = sorted(
                (t for t in self.map_ores
                 if t.distance_squared(self.core) < t.distance_squared(self.enemy)),
                key=lambda t: (t.distance_squared(self.core), t.y, t.x),
            )
        if not self.patrol_route:
            return
        p = ct.get_position()
        wp = self.patrol_route[self.patrol_i]
        if (
            abs(p.x - wp.x) + abs(p.y - wp.y) <= 1
            or self.patrol_wait >= PATROL_STUCK_RNDS
        ):
            # Reached (standing on it, or beside it -- a harvester of ours may
            # already own the tile), or unreachable for long enough.  Next one.
            self.patrol_i = (self.patrol_i + 1) % len(self.patrol_route)
            self.patrol_wait = 0
            wp = self.patrol_route[self.patrol_i]
        else:
            self.patrol_wait += 1
