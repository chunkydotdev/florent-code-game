...
_v70ec variant: CONVEYOR-CHURN DAMPING + LABOR RESERVE.  On the eider tiebreak
losses the binding constraint is money-then-labour, not combat: we re-lay the
same handful of chokepoint conveyor tiles every time the enemy clears them, pay
+1% permanent team-wide cost scale for each re-lay, and thereby keep the bank
under the titanium floor the builder respawn gate needs -- so we play half the
match on three hands.  This branch adds a per-unit per-tile rebuild ledger that
refuses to re-lay a tile the enemy keeps taking, and a reserve that stops a
builder starting NEW conveyor spending while the bank is below the respawn
budget.  Rounds before REPLACE_MIN_RND and every first build of a tile are
untouched, so the opening is bit-for-bit _v70th.
"""
import math
...
# CONVEYOR-CHURN DAMPING AND LABOUR RESERVE.  Measured in the two eider
# 1000-round tiebreak losses to kladde_probe (28x20, both decided on titanium
# delivered): we built 146 conveyor tiles to serve 13 harvesters -- 11.2 tiles
# per harvester -- while the winner built 65 for 16 (4.1 per harvester).  Of
# our 146, 40 were dead at the final bell (28% churn) against the winner's 1
# (1.5%), and the deaths concentrate on a couple of tiles: (10,10)/(11,10)/
# (12,10) alone were rebuilt and lost SIX times between round 429 and round
# 907, under an enemy Sentinel (#415, built r212 at (15,10)) that camped that
# spot and fired on it 12 times between r458 and r673.
#
# Nothing in this file calls ct.destroy(), so the churn is not us tearing our
# own network down -- it is stateless RE-BUILDING.  The opportunistic pave in
# _move lays a conveyor on ANY empty tile one step closer to the Core, every
# time any builder walks it, and _link_path grows one deterministic reverse
# tree from the Core inputs, so every harvester chain merges onto the same few
# tiles beside the Core.  A camping Sentinel needs two shots to clear one of
# them (18 damage against a conveyor's 20 HP, about 4 rounds) and the next
# builder through the funnel re-lays it, forever.  Enemy builder melee does
# the same job more cheaply: 376 melee hits for 752 damage on our economy
# buildings in s1, which at 20 HP a conveyor is 37 conveyors -- almost exactly
# the 40 that churned.
#
# The bill is not the 3 Ti.  Cost scale is one team-wide additive number
# (docs/game-model.md) and every conveyor adds +1% of base cost to everything
# built afterwards, forever.  146 conveyors is +146% on its own, which is what
# turns a 30 Ti builder bot into a ~74 Ti one; our bank sat under 250 Ti
# almost continuously from round 0 to round ~600, i.e. under REPLACE_TI_FLOOR,
# so the respawn gate above essentially never fired.  We played ~500 rounds on
# 3 live builders against their 8-14, finished with 9 harvesters to 12, and
# then sat on 5,076 unspent titanium at round 999 -- by the time the money
# arrived there were no hands to spend it.
#
# Two coupled brakes, both per-unit instance state (all 16 store slots are
# spoken for, and a per-tile ledger is local knowledge anyway):
#
#   * CHURN_TILE_BUILDS / CHURN_REBUILD_WAIT -- a per-tile ledger.  A unit may
#     lay a conveyor on a given tile at most CHURN_TILE_BUILDS times for the
#     whole match, and never twice inside CHURN_REBUILD_WAIT rounds.  The wait
#     alone breaks the ~4-round clear-and-relay loop under a camping turret;
#     the cap makes a tile the enemy has taken from us three times permanently
#     theirs, which is the honest reading of a chokepoint we lost six times.
#     25 rounds is set from that same camp: it is six clear-and-relay cycles,
#     long enough that a tile only comes back when the pressure on it has
#     actually moved on.  A tile the ledger refuses inside _build_next_link
#     abandons the whole remaining chain rather than parking the builder
#     beside it -- three chains through one tile is the enemy telling us where
#     the front is, and a builder waiting there is a builder not building.
#
#   * The reserve -- after REPLACE_MIN_RND a builder does not START new
#     conveyor spending (a fresh link plan, or a pave tile) while the bank is
#     below REPLACE_TI_FLOOR plus the conveyor's own scaled cost, i.e. while
#     that spending would eat the respawn budget the Core is waiting for.  A
#     link already in progress finishes untouched: an unfinished chain is pure
#     cost, 20 Ti and +5% scale for a harvester that delivers nothing.  A
#     deferred link is remembered rather than discarded and is planned the
#     round the bank recovers.  Builders cannot see the live builder count --
#     that is exactly why the respawn gate lives on the Core, keyed on
#     get_unit_count() drops -- so the reserve is unconditional rather than
#     conditioned on being short of hands.  Holding ~250 Ti back while already
#     at full labour costs a few delayed conveyor tiles; not holding it back
#     cost 500 rounds at 3 builders out of 5.
CHURN_TILE_BUILDS = 3
CHURN_REBUILD_WAIT = 25
...
class Player:
    def __init__(self):
        ...
        # CONVEYOR-CHURN LEDGER (see the block above).  (x, y) -> [times this
        # unit has laid a conveyor there, round of the last one].  Per unit
        # instance, like self.hunting and the interception state: it records
        # what THIS unit did, which is the only thing it can know without
        # spending a store slot, and the pave/link paths that cause the churn
        # are per-unit paths anyway.  Bounded by the number of distinct tiles
        # one builder ever paves, so at worst the map area.
        self.tile_builds = {}
        # A link plan the labour reserve deferred: the harvester position it
        # would have been grown from, replayed by _resume_link once the bank
        # clears the reserve.  None whenever there is nothing owed.
        self.link_deferred = None
        self.dropped = False
        self.map_grid = None
        self.map_walls = set()
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if not defended:
                if harv < 1 and ti >= ct.get_harvester_cost() and self._try_harvester(ct, harv):
                    return
                # Pick up a link the labour reserve deferred, so a recovered
                # bank starts paying for the chain the round it recovers.
                self._resume_link(ct)
    ...
    def _expand(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            # Pick up a link the labour reserve deferred before anything else
            # touches link_queue this turn, so a recovered bank starts paying
            # for the chain on the same round it recovers.
            self._resume_link(ct)
    ...
    def _reserve_holds(self, ct):
        """True while NEW conveyor spending would eat the respawn budget.

        See the CONVEYOR-CHURN DAMPING AND LABOUR RESERVE block at the top of
        the file.  Deliberately the same two numbers the Core's replacement
        gate uses -- REPLACE_MIN_RND and REPLACE_TI_FLOOR -- because this is
        the other half of that gate: the Core will not buy a replacement
        builder below the floor, so a builder that spends the bank down to it
        is the reason the gate never fires.  Adding the conveyor's own scaled
        cost is what makes it a floor rather than a boundary the next tile
        crosses immediately.

        Round-gated so the opening is untouched: the first harvester and its
        link are laid before REPLACE_MIN_RND, at which point there is no
        respawn budget to protect (no builder has died and the Core is still
        spawning its opening five against the base cap).
        """
        if ct.get_current_round() < REPLACE_MIN_RND:
            return False
        return ct.get_global_resources() < REPLACE_TI_FLOOR + ct.get_conveyor_cost()

    def _churn_allows(self, ct, tile):
        """True if this unit may lay a conveyor on tile now.  See the ledger.

        A tile this unit has never built on always passes, so no opening build
        and no first pass over any tile changes behaviour.  Only re-laying is
        rationed: not inside CHURN_REBUILD_WAIT rounds of our own last one,
        and never more than CHURN_TILE_BUILDS times in the match.
        """
        row = self.tile_builds.get((tile.x, tile.y))
        if row is None:
            return True
        if row[0] >= CHURN_TILE_BUILDS:
            return False
        return ct.get_current_round() - row[1] >= CHURN_REBUILD_WAIT

    def _churn_note(self, ct, tile):
        """Record a conveyor this unit just laid.  Call only after a build."""
        key = (tile.x, tile.y)
        row = self.tile_builds.get(key)
        if row is None:
            self.tile_builds[key] = [1, ct.get_current_round()]
        else:
            row[0] += 1
            row[1] = ct.get_current_round()

    def _start_link(self, ct, hpos):
        """Plan a fresh chain from hpos, unless the labour reserve says wait.

        The single place a new link is created, so the reserve has exactly one
        seam to sit on and a link that is already in progress can never be
        interrupted by it.  A deferred plan is remembered rather than dropped:
        the harvester is built either way, and a harvester with no chain is
        the pure-cost case the reserve exists to avoid financing twice.
        """
        if self._reserve_holds(ct):
            self.link_deferred = hpos
            return
        self.link_deferred = None
        self.link_source = hpos
        self.link_queue = self._link_path(ct, hpos)

    def _resume_link(self, ct):
        """Plan a link the reserve deferred, once the bank allows it.

        _start_link tests the reserve before running any search, so this is
        three getters on the rounds it cannot act and one BFS on the round it
        can.  Guarded on an empty queue so it can never displace live work.
        """
        if self.link_deferred is not None and not self.link_queue:
            self._start_link(ct, self.link_deferred)
    ...
    def _build_next_link(self, ct):
        ...
        if not self.link_queue:
            ...
            return False
        tile = self.link_queue[0]
        # CHURN DAMPING.  This head tile is one we have already laid a
        # conveyor on, too recently or too often -- the enemy has been
        # clearing it (a camping Sentinel takes two shots, about four rounds)
        # and we have been paying +1% permanent cost scale per re-lay to feed
        # it.  Abandon the remaining chain instead of waiting beside the tile:
        # standing on a contested funnel cell is the failure mode this file
        # already avoids at line "Already in build range", and the tiles laid
        # so far keep whatever they are already carrying.
        if not self._churn_allows(ct, tile):
            self.link_queue = []
            return False
    ...
    def _move(self, ct, d, pave=True):
        ...
        #
        # This is the single largest conveyor spender in the file and the one
        # the eider census indicts: it is stateless, so a funnel tile the
        # enemy clears is re-laid by whichever builder next walks it, and it
        # produced 11.2 tiles per harvester against the winner's 4.1.  Both
        # brakes attach here -- the reserve so a starved bank is never paved
        # away below the respawn floor, the ledger so no single tile is fed
        # more than CHURN_TILE_BUILDS times.  A refusal only skips the build;
        # the move below is attempted exactly as before.
        if pave and self.core and ct.is_tile_empty(nxt) and ct.get_action_cooldown() == 0:
            if (
                ct.read_store(SLOT_HARVESTERS) >= 1
                and ct.get_global_resources() >= ct.get_conveyor_cost()
                and not self._reserve_holds(ct)
            ):
                if dist_core(nxt, self.core) > 0:
                    ...
                    if abs(nxt.x - self.core.x) + abs(nxt.y - self.core.y) < abs(here.x - self.core.x) + abs(here.y - self.core.y):
                        ...
                        if self._churn_allows(ct, nxt) and ct.can_build_conveyor(nxt, card):
                            ct.build_conveyor(nxt, card)
                            self._churn_note(ct, nxt)
        if ct.can_move(d):
            ct.move(d)
            return True
        return False
