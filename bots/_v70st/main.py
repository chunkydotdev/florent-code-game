...
_v70sb variant: ORE-BLOCKING BARRIERS.  Strictly _v70th plus one mechanism, and
only inside the saboteur role: a saboteur standing beside an empty ore tile on
the ENEMY side of the map spends the action planting a 3 Ti Barrier on it
instead of pecking a building for 2 damage.  A Barrier on ore is a permanent
Harvester veto -- the tile is theirs to clear at 15 builder-rounds and 30 Ti of
melee before they may spend another 20 Ti on the Harvester they wanted -- and
the same rule re-denies any tile whose Harvester we or our turrets have already
killed, which is where the measured opponent is slowest to react.  The
mechanism is bounded (6 barriers per unit, disabled while the bank is under
100 Ti), refuses to run at all where the Core anchor pair is not in the table,
and touches nothing outside _saboteur -- every other role, and the whole
opening, is bit-for-bit _v70th.
"""
import math
...
# ORE DENIAL BY BARRIER (this variant's only new mechanism; saboteur-only).
#
# Measured against kladde_probe over kladde_eider_s1/s2 (28x20) and
# kladde_hive_s1 (25x25), plus a census of all 15 pool maps
# (SCRATCH/starvation_premortem.md):
#
#   * Cost asymmetry, 10x in our favour.  A Barrier is 3 Ti and 30 HP.  Builder
#     melee is 2 damage for 2 Ti, so clearing one costs THEM 15 builder-rounds
#     and 30 Ti -- and only then may they spend the 20 Ti Harvester they wanted.
#     Denial value while it stands, at the observed (not nominal) throughput of
#     ~1.5 Ti/round per mature Harvester, is ~22 Ti of forgone income per 15
#     rounds blocked, for 3 Ti spent.
#   * The window is real and stays open all match.  13/16 of the opponent's
#     eider Harvester builds and 3/6 of its hive builds land AFTER the earliest
#     round any scout of ours could have reached that tile; its LAST Harvester
#     goes down at r845 (eider) / r748 (hive).  This is not an opening-round
#     sprint -- a pre-placed Barrier only has to occupy the tile before they
#     ever get there.
#   * They do not react.  Observed rebuild latency after real building deaths
#     in those replays is 41-400 rounds (median ~150-290, one 10-round
#     outlier), and 2 of 14 deaths were never rebuilt at all.  So a blocked
#     tile plausibly stays blocked for a large fraction of the match, which is
#     also why denying REBUILDS is worth as much as denying first builds: a
#     dead Harvester frees its ore tile, the "tile is empty" test below picks
#     that up automatically, and they are measurably slow to come back for it.
#   * Ore-poor maps are where it bites hardest.  hive has 12 ore tiles in
#     total, 4 of them clearly on their side; the opponent still only reached
#     5 Harvesters after 786 uncontested rounds there.  Two or three barriers
#     remove a large fraction of its entire ore access, not a marginal slice.
#   * Both tiebreakers are attacked at once.  After round 1000 the order is
#     titanium delivered to Core, then Harvesters alive -- denied ore lowers
#     the first and caps the second.
#
# Why barriers rather than the obvious alternatives: killing a delivery
# conveyor costs us 20 Ti for their ~3-6 Ti rebuild (asymmetry reversed), and
# killing a finished Harvester is 30 Ti of melee to undo a 20 Ti investment
# they can simply re-make.  Blocking the tile BEFORE they invest is the only
# one of the three where the arithmetic runs our way.
#
# ORE_BLOCK_MAX is per unit, not per team: each barrier adds +1% to the
# conveyor/splitter/barrier cost scale, which is the same pool our economy
# links spend from, so the creep is capped at +6% per saboteur (and at most 18
# Ti of spend) rather than left open-ended.
# ORE_BLOCK_TI_FLOOR keeps the mechanism off entirely while the bank is thin:
# home defense, replacement builders (REPLACE_TI_FLOOR = 250) and emergency
# counterbattery all outrank a speculative 3 Ti denial, and a saboteur that
# cannot see home must not be the reason a respawn is unaffordable.
ORE_BLOCK_MAX = 6
ORE_BLOCK_TI_FLOOR = 100
# SELF-SHUTOFF.  Barriers are documented as buildable on anything that is not a
# WALL tile (docs/game-model.md: "Cannot be placed on a wall tile"; ore is
# "traversable, Harvester-buildable" and is_tile_empty means "no building and
# not a wall"), but the engine is a compiled binary and this was NOT verified
# by running it.  If can_build_barrier turns out to refuse ore tiles, this
# counter notices after a handful of otherwise-eligible refusals and switches
# the whole mechanism off for that unit, so the failure mode is "reverts to
# _v70th behaviour" rather than "saboteur loiters beside ore forever".
# Tiles occupied by a builder bot are skipped rather than counted, so ordinary
# traffic cannot trip it.
ORE_BLOCK_FAIL_MAX = 8
# STEAL BEFORE BLOCK.  A Harvester on an enemy-side ore tile is a strict
# upgrade on a Barrier when the bank affords it: same denial (they clear 30 HP
# at 2 dmg/2 Ti a peck), plus it is impassable ("the winner walls the loser
# out" is a measured dynamic -- game-model.md), plus it counts for tiebreak #2
# (most harvesters alive), which is what decides the starving games this
# mechanism exists for.  Unconnected it idles harmlessly ("doesn't waste
# titanium, just won't produce" -- game-model.md), so no chain home is needed.
# Three traps, each guarded at the build site: (1) DONATION -- a harvester
# outputs to ANY adjacent accepting building round-robin, and stacks can be
# pushed onto the enemy network, so a steal cardinal-adjacent to an enemy
# conveyor/splitter/core would gift them 10 Ti per 4 rounds; (2) COUNTER
# INFLATION -- steals deliberately never write SLOT_HARVESTERS/SLOT_ECO_READY,
# or home expanders would read the economy as finished; (3) SCALE CREEP --
# +5% per harvester applies to our HOME rebuilds too, so steals are capped
# hard and require a fat margin over the harvester cost so they never outbid
# a home farm.  Barrier remains the fallback when the bank is thin.
ORE_STEAL_MAX = 3
ORE_STEAL_TI_MARGIN = 150
...
def known_enemy_core_for(w, h, own):
    """The enemy Core anchor, but ONLY when the map is in the CORE_PAIRS table.

    enemy_core_for above always answers, falling back to the 180-degree
    reflection ``(w-2-x, h-2-y)`` on an unknown map.  Six of the fifteen pool
    maps are mirror-symmetric rather than rotationally symmetric
    (docs/game-model.md), so that fallback can name a tile that is not the
    enemy Core at all.  Ore denial decides "their side" by comparing distances
    to the two anchors, and a wrong anchor would flip the test and make us
    barrier OUR OWN ore, so this refuses to guess and returns None instead --
    every caller then skips the mechanism outright.
    """
    for mw, mh, ax, ay, bx, by in CORE_PAIRS:
        if w != mw or h != mh:
            continue
...
class Player:
    def __init__(self):
        ...
        # ORE DENIAL state, per unit instance -- no store slot is spent on it
        # (all 16 are already allocated) and none is needed: every field is
        # read and written only by its own unit, and two saboteurs
        # independently blocking the same tile is a no-op, since the second
        # one's "tile is empty" test fails against the first one's barrier.
        # deny_ores is the cached enemy-side ore list (None = not computed yet,
        # empty tuple = mechanism disabled for this unit), deny_set its (x, y)
        # index for the adjacency test, ore_blocks the per-unit build counter
        # bounded by ORE_BLOCK_MAX, deny_fail the ORE_BLOCK_FAIL_MAX shutoff.
        self.deny_ores = None
        self.deny_set = frozenset()
        self.ore_blocks = 0
        self.ore_steals = 0
        self.deny_fail = 0
    ...
    def _core(self, ct):
        ...
        if (
            self.n < spawn_budget
            and (self.n < spawn_cap or (ti >= REPLACE_TI_FLOOR and rnd >= REPLACE_MIN_RND))
    ...
    def _saboteur(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if self._try_siege_build(ct):
                return
            # ORE DENIAL, ABOVE GENERIC MELEE PECKING (see the ORE DENIAL BY
            # BARRIER block at the top of the file for the measured numbers).
            #
            # Placement, from the top of this action phase downward:
            #   1. the melee_first peck stays first.  It only exists on
            #      near-wall-free maps (MELEE_FIRST_MAX_WALL_FRAC, one map in
            #      the pool) and _sabotage_prio ranks enemy Gunners/Sentinels
            #      above everything else, so on those maps it is the
            #      self-preservation branch -- shooting first in an open gun
            #      duel -- and it keeps its measured priority.
            #   2. the primary-battery repair escort stays second.  Our forward
            #      Sentinel is 18 damage a shot and 30 Ti to replace; +4 HP
            #      keeping it alive dominates 3 Ti of denial.
            #   3. _try_siege_build stays third.  It is the whole offensive
            #      plan of the role and it fires at most a handful of times per
            #      match, whereas the tile beside us is still there next round
            #      -- this branch holds position on it (see _deny_ore_target).
            #   4. THIS.  3 Ti to veto an ore tile permanently outranks 2 Ti
            #      for 2 damage against a 20-30 HP building, which is what the
            #      generic peck below buys.
            # It is deliberately NOT above the peck for the whole file: only
            # _saboteur is touched, so _home_defend and _launchwait -- the
            # emergency home-defense paths -- keep _sabotage_prio first.
            if self._deny_ore_build(ct):
                return
        ...
        if self.siege_approach is not None:
            ...
            # ORE DENIAL, MOVEMENT.  Slotted into the LAST branch only, so the
            # siege approach (above) and the economy hold (above) are both
            # untouched: this replaces nothing but the blind "walk at the enemy
            # Core" default, and only while a visible enemy-side ore tile is
            # actually on offer.  _deny_ore_target returns None whenever the
            # mechanism is off, out of budget, capped, or has nothing in sight,
            # and then this is bit-for-bit the _v70th line.
            deny = self._deny_ore_target(ct)
            self.tgt = ec if deny is None else deny
        self._nav(ct, pave=False)
        self._nav(ct, pave=False)

    def _deny_ore_tiles(self):
        """Enemy-side ore tiles from the packed map.  Computed once per unit.

        "Enemy side" is strictly nearer their Core anchor than ours, by
        Manhattan distance to the nearest tile of each 2x2 footprint -- the
        same cardinal-step metric the pre-mortem used to count blockable tiles,
        and the metric a builder actually walks.  Ties (equidistant tiles) are
        excluded: they are the most contested ground on the map and we would
        rather harvest them ourselves than wall them off.

        Returns an empty tuple -- permanently, for this unit -- whenever the
        map is not one we hold terrain for, or the Core anchor pair is not in
        CORE_PAIRS (see known_enemy_core_for: guessing the anchor on a
        mirror-symmetric map would invert the test and barrier our own ore).
        On those maps the whole mechanism is dead and behaviour is _v70th's.
        """
        if self.deny_ores is not None:
            return self.deny_ores
        if self.map_grid is None or self.core is None:
            # Not decided yet -- do NOT cache: the grid is decoded on the first
            # turn this unit sees its own Core, which may be after this call.
            return ()
        anchor = known_enemy_core_for(self.mw, self.mh, self.core)
        if anchor is None:
            self.deny_ores = ()
            return self.deny_ores
        ours = core_tiles(self.core)
        theirs = core_tiles(anchor)
        picked = []
        for t in self.map_ores:
            d_us = min(abs(t.x - c.x) + abs(t.y - c.y) for c in ours)
            d_them = min(abs(t.x - c.x) + abs(t.y - c.y) for c in theirs)
            if d_them < d_us:
                picked.append(t)
        self.deny_ores = tuple(picked)
        self.deny_set = frozenset((t.x, t.y) for t in self.deny_ores)
        return self.deny_ores

    def _deny_ore_ready(self, ct):
        """Shared gate for both halves of the mechanism."""
        if self.ore_blocks >= ORE_BLOCK_MAX:
            return False
        if ct.get_global_resources() < ORE_BLOCK_TI_FLOOR:
            return False
        return bool(self._deny_ore_tiles())

    def _deny_ore_build(self, ct):
        """Barrier an adjacent, empty, enemy-side ore tile.  True == turn spent.

        Only called from _saboteur's action phase, so the action cooldown is
        already known to be zero.  Every mutating call is gated by its own
        can_*() check and every tile query is on an orthogonally adjacent tile,
        which is always inside a builder's r^2 = 20 vision, so none of them can
        raise for being out of vision.
        """
        if not self._deny_ore_ready(ct):
            return False
        p = ct.get_position()
        for d in CARDINALS:
            t = p.add(d)
            if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                continue
            if (t.x, t.y) not in self.deny_set:
                continue
            try:
                # The packed map says this is ore; confirm against the live
                # tile so a mis-decoded grid can never spend titanium on the
                # wrong tile.  A tile that already holds a building is exactly
                # the case we are waiting out -- and a Harvester killed later
                # frees it again, which is half the value of the mechanism.
                if ct.get_tile_env(t) != Environment.ORE_TITANIUM:
                    continue
                if ct.get_tile_building_id(t) is not None:
                    continue
                # A bot standing there blocks the build for reasons that have
                # nothing to do with legality; skip without counting a failure.
                if ct.get_tile_builder_bot_id(t) is not None:
                    continue
                # SELF-TRAP GUARD.  A Barrier blocks movement for both teams
                # and _bfs_direction treats one as blocked terrain, so a bot
                # that walls its own last exit is stranded for the match.
                # Require at least one other passable cardinal neighbour.
                escapes = 0
                for e in CARDINALS:
                    n = p.add(e)
                ...
                if escapes == 0:
                    continue
                # STEAL BEFORE BLOCK (see the constants block): a Harvester on
                # the tile when the bank is fat, a Barrier when it is thin.
                # The escapes guard above covers both -- a Harvester is just as
                # impassable as a Barrier.  Trap guards in order: cap, margin,
                # donation adjacency, then the engine's own legality check.
                if (
                    self.ore_steals < ORE_STEAL_MAX
                    and ct.get_global_resources()
                        >= ct.get_harvester_cost() + ORE_STEAL_TI_MARGIN
                    and not self._enemy_acceptor_adjacent(ct, t)
                    and ct.can_build_harvester(t)
                ):
                    # Deliberately NO SLOT_HARVESTERS / SLOT_ECO_READY write:
                    # a steal is territory, not economy, and home expanders
                    # must keep counting real farms only.
                    ct.build_harvester(t)
                    self.ore_steals += 1
                    return True
                buildable = ct.can_build_barrier(t)
            except Exception:
                continue
            if buildable:
                ct.build_barrier(t)
                self.ore_blocks += 1
                return True
            # Eligible by every rule we know and still refused -- see the
            # SELF-SHUTOFF note on ORE_BLOCK_FAIL_MAX.
            self.deny_fail += 1
            if self.deny_fail >= ORE_BLOCK_FAIL_MAX:
                self.deny_ores = ()
                self.deny_set = frozenset()
                return False
        return False

    def _enemy_acceptor_adjacent(self, ct, t):
        """True if a cardinal neighbour of t could accept our stacks -- the
        donation trap: an enemy conveyor/splitter/core beside a stolen
        Harvester turns it into a 10 Ti / 4 rounds gift to their tiebreak.
        Neighbours of an adjacent tile are at most two steps away, well inside
        r^2 = 20 vision; the try/except still catches any edge surprise and
        answers True, which fails safe (we barrier instead of steal)."""
        for e in CARDINALS:
            n = t.add(e)
            if not (0 <= n.x < self.mw and 0 <= n.y < self.mh):
                continue
    ...
    def _deny_ore_target(self, ct):
        """Nearest visible, empty, enemy-side ore tile -- or None.

        Returns this unit's own position when the nearest such tile is already
        orthogonally adjacent: the build above owns that round, and walking on
        would either step ONTO the tile (where a Barrier can never be built --
        builds are adjacent-only, never one's own tile) or straight past it.
        _nav treats tgt == own position as "nothing to do", so this is a hold,
        and it is bounded by ORE_BLOCK_FAIL_MAX in the pathological case where
        the build is refused every round.
        """
        if not self._deny_ore_ready(ct):
            return None
        p = ct.get_position()
        best, best_d = None, None
        for t in self.deny_ores:
        ...
        if best is None:
            return None
        if abs(p.x - best.x) + abs(p.y - best.y) == 1:
            return p
        return best
    ...
    def _hunt_turret(self, ct):
        ...
        if self.role not in ("defend", "expand") or self.role_n == 1:
            return False
        # See ROUND FLOOR above: below this the rush machinery owns the turn.
        ...
        for bid in ct.get_nearby_buildings():
            ...
            if self.core.distance_squared(bp) > INTRUDER_CORE_DSQ:
                continue
