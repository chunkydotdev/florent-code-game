...
# Rich-bank floor for the counterbattery eco-gate waiver (see
# _try_counterbattery): a turret bought while poor is bought with heal and
# respawn money.  150 covers a sentinel plus change on an opening-scale bank.
COUNTERBATTERY_RICH_TI = 150
...
class Player:
    ...
    def _try_counterbattery(self, ct):
        ...
        # (A seat-B "second gun on threat evidence" variant was tried here
        # and refuted by ablation: flotte guard 96.7 -> 90.0 with it, exactly
        # restored without it.  The seat-B counter is the frontier deferral
        # in _pick alone.)
    ...
    def _pick(self, ct):
        if self.map_ores and self.role == "expand":
            ...
            # SEAT-B FRONTIER DEFERRAL.  Measured across 20 production games
            # (seatB_diagnosis, 2026-08-07): the engine resolves all of Team
            # A's actions before any of Team B's, so in a lethal exchange the
            # seat-A builder acts before dying and the seat-B builder dies
            # before acting -- 19 vs 38 builder deaths by r80 corpus-wide,
            # ONE mutual trade in 20 games.  The attrition starves seat B's
            # harvester expansion and the collected gap never washes out:
            # 9/9 of the corpus's round-1000 tiebreaks went to whichever bot
            # held seat A, ours or theirs.  Contested-ore avoidance was
            # tested and REJECTED (3-3, no seat bias) -- the tax is exposure,
            # not tile races.  So as Team B, frontier ore (nearer their Core
            # than ours) is deferred to the back of each worker's sweep: the
            # safe half is claimed with the same tempo, and the frontier is
            # entered later, with more of the map built out behind us.  Team
            # A's ordering is bit-for-bit unchanged.
            # THREAT-CONDITIONAL: a blanket seat-B deferral was refuted by
            # ablation (kladde eider 8/16 -> 0/16 -- those eight wins were
            # all seat-B economy races where the frontier was safe and
            # necessary).  The exposure tax only exists while someone is
            # actually in our half shooting; SLOT_UNDER is the cheapest
            # evidence of that, and vs passive-until-late grinders it stays
            # 0 through the whole claim window, leaving tempo untouched.
            seat_b = (
                self.team == Team.B
            ...
            ordered = sorted(
                ...
                key=lambda t: (
                    (
                        1 if seat_b and (
                            abs(t.x - self.enemy.x) + abs(t.y - self.enemy.y)
