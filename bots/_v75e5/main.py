...
class Player:
    ...
    def _core(self, ct):
        ...
        if ENDGAME_SWITCH_ON and rnd >= ENDGAME_RND and self._core_sees_turret(ct):
            amt = ti - 2 * ct.get_harvester_cost()
    ...
    def _core_sees_turret(self, ct):
        """PIECE H -- is there a friendly Gunner/Sentinel to drink the ammo?
        ...
        siege gun out of Core sight reads False -- conservative in the right
        ...
        for bid in ct.get_nearby_buildings():
            try:
                if (
                    ct.get_team(bid) == mine
                    and ct.get_entity_type(bid) in (EntityType.GUNNER, EntityType.SENTINEL)
                ):
                    return True
            except Exception:
                continue
    ...
    def _idle_rotate(self, ct, p, turret_type):
        ...
        ROTATE_DISCIPLINE_ON for the measurement and the three rules."""
        if turret_type != EntityType.GUNNER:
            return
