...
# PIECE K -- STANDING CORE+TRUNK HEAL BUDGET.  Piece J's heal-dispatch reorder
# is the right mechanism at the wrong scope, and BOTH halves of that are
# measured.
#
...
# own early window at 40).  So K's heal runs from round 0 and covers the trunk
# -- conveyors, splitters and harvesters standing beside the healer -- not just
# the Core, and it runs for every role, not only for expanders.
#
...
class Player:
    ...
    def _builder(self, ct):
        ...
        #
        # PIECE K widens this from "the Core, while SLOT_UNDER is latched" to a
        # STANDING core+trunk repair line with a spend cap (see
        # K_HEAL_BUDGET_ON).  Three differences, each with its own reason:
        #  1. NO SLOT_UNDER GATE ON THE CORE HEAL.  The latch is not a safety
        #     property -- can_heal() already refuses a full-HP Core, so the
        #     only rounds it actually suppresses are ones where the Core is
        #     damaged and the attacker left more than 50 rounds ago.  Repairing
        #     then is what puts us back at 500 HP before the NEXT wave, which
        #     is sporks' measured profile exactly (500 -> 369 at r150 -> 500 at
        #     r200 -> 500 for the remaining 328 rounds).  Cost is four
        #     can_heal() calls per builder on quiet rounds.
        #  2. THE TRUNK JOINS THE PRIORITY.  Today conveyor/splitter/harvester
        #     repair lives only at the BOTTOM of _expand's action phase, behind
        #     a round gate (40 early / 150 late) and available to expanders
        #     only.  The family that kills us in r64-119 is inside that gate,
        #     and sporks' 1,496 HP of conveyor repair is half its survival
        #     story.  _heal_trunk keeps the measured depth discriminator that
        #     made the early window safe.
        #  3. THE BUDGET.  Over budget this block claims NOTHING and dispatch
        #     proceeds -- the direct repair for v65's 972-heal antler game,
        #     where this very return statement starved piece H's endgame
        #     harvester arm for three straight r1000 games.
        ...
        if ct.get_action_cooldown() == 0:
            if K_HEAL_BUDGET_ON:
                ...
                # live-gun scan off the quiet rounds this block newly visits.
                if self._heal_budget_left(ct) > 0 and not (
                    ct.read_store(SLOT_UNDER) != 0 and self._cb_over_heal(ct)
                ):
    ...
    def _heal_trunk(self, ct):
        ...
        bottom of one role's action phase into the standing priority line and
        opened to round 0.  Everything the ablations measured about that medic
        is carried over intact:
         ...
         - MEDIC_TI_FLOOR.  Below it every titanium belongs to the first
           harvesters and links.
         - THE DEPTH DISCRIMINATOR, which is the whole guard against
           re-creating the measured opening flips (fjordgate A-core-kill@297 ->
           B-core-kill@138, lighthouse A-tiebreak@1000 -> B-core-kill@110, both
           restored exactly by the no-medic ablation).  A builder peck is 2
           damage, so MEDIC_EARLY_MIN_DMG down is four accumulated pecks --
           sustained attention on that tile, i.e. a raid clearing the farm,
           not a passer-by.  Cosmetic 1-peck patches, which is what cost the
           opening tempo, stay excluded until MEDIC_MIN_RND.
        ...
        if ct.get_global_resources() < MEDIC_TI_FLOOR:
            return False
        depth = 1 if rnd >= MEDIC_MIN_RND else MEDIC_EARLY_MIN_DMG
        p = ct.get_position()
        for d in CARDINALS:
            bp = p.add(d)
            ...
            try:
                ...
                if (
                    ...
                    and ct.get_hp(bid) <= ct.get_max_hp(bid) - depth
                    and ct.can_heal(bp)
                ):
                    ct.heal(bp)
                    return True
