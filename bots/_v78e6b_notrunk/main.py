        ...
        if ct.get_action_cooldown() == 0:
            if K_HEAL_BUDGET_ON:
                ...
                if self._heal_budget_left(ct) > 0 and not (
                    under and self._cb_over_heal(ct)
                ):
                    if under and self._heal_core(ct):
                        self.heal_spent += 1
                        return
                    # ABLATION notrunk: trunk arm disabled; capped Core arm
                    # as-built is the only K' spender in this cell.
                    if False and self._heal_trunk(ct):
                        self.heal_spent += 1
                        return
