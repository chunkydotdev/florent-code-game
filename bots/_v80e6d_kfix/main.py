...
# PIECE KF (Eir 6d, branch A) -- THE TRUNK ARM STANDS OFF A LIVE GUN'S RAY.
# K' above is loose by design ("repair ANY damaged trunk building we are
# standing beside") and the budget is its only bound.  Measured cost of that
# looseness on the one battery built to punish it: vs kladde_probe -- a 2-3
# staggered-Sentinel grind -- 6c scores 67.5/120 against a 74.2/120 baseline,
# a soft -7, and the losses cluster on eider and hive, i.e. exactly the
# sentinel-strike maps.
#
# The arithmetic says why.  A heal is +4 HP for 1 Ti and one builder ACTION
# (and therefore one builder MOVE not taken); a Sentinel answers with 18 at
# reload 2, a Gunner with 7 at reload 1.  Repairing a 20-HP conveyor that a
# live gun is presently covering is a losing race in HP, in titanium and in
# tempo simultaneously -- the tile is not damaged, it is CONDEMNED, and the
# turn is better spent anywhere else.  This is the same fact piece D reads
# from the attacker's side ("never melee one alone into a live gun whose ray
# covers me"), applied to the repairman: a tile under a gun's ray is not worth
# standing on, whichever verb we came to perform there.
#
# Scope is deliberately the TRUNK ARM ONLY.  The Core heal is untouched:
# healing a besieged Core into fire is lineage behaviour that the Eir 5.1
# ablation grid measured as load-bearing (exempt core arm 95.0 band vs 56.7
# for the capped variant), the Core is a 500-HP 2x2 that actually survives the
# race, and losing it ends the game -- so "condemned" never applies there.
#
# The skip reuses _ray_covers, the walk lifted out of piece D's _duel_safe
# clause (c), so both callers ask the identical question and can never drift.
# Enemy ammunition is unobservable, so any VISIBLE enemy Gunner/Sentinel whose
# ray covers the tile counts as live -- the same assumption piece D ships
# with.  Turrets come from get_nearby_buildings (vision-bounded by
# construction, the _live_home_gun idiom) and the list is built lazily, only
# once an actual heal candidate exists, so the quiet rounds -- the large
# majority -- pay nothing at all for this gate.
K_FIX_LIVEGUN_ON = True
...
class Player:
    ...
    def _duel_safe(self, ct, tpos, tid):
        ...
        # is the unsafe answer: we cannot prove the gun is pointed away.  The
        # walk itself lives in _ray_covers, lifted out unchanged so that piece
        # KF's trunk stand-off asks this exact question and the two can never
        # drift apart; "covers my tile" is precisely "not safe".
        return not self._ray_covers(ct, tid, et, tpos, ct.get_position())

    def _ray_covers(self, ct, tid, et, tpos, target):
        """Does turret tid (type et, standing at tpos) cover `target` right now?

        Piece D's clause (c) walk, shared with piece KF's trunk-heal stand-off
        (see K_FIX_LIVEGUN_ON).  Both callers want the same conservative
        reading, so the failure modes are shared and stated once here:
         - Unreadable facing (no direction, or the turret has walked out of
           vision) reads as COVERED.  We cannot prove the gun points away.
         - A CENTRE facing has no ray at all and covers nothing.
         - An unreadable tile between the gun and the target reads as BLOCKED,
           i.e. something standing there eats the shot, so the Gunner ray
           stops short.  The Sentinel line ignores obstacles by rule and is
           walked straight through to the target tile.
        Ammunition is not consulted: enemy ammo is unobservable, so a visible
        gun is assumed loaded.
        """
        try:
        ...
        while True:
            ...
            if et == EntityType.SENTINEL:
                # The Sentinel line ignores obstacles, so nothing in between
                # can shield the target -- keep walking to its tile.
                continue
            n = Position(x, y)
            try:
                blocked = (
                ...
                # shot.  The Gunner's ray stops before reaching the target.
                blocked = True
            if blocked:
                return False
    ...
    def _live_gun_covers(self, ct, tile):
        """PIECE KF -- is `tile` inside a visible enemy turret's firing ray?

        The stand-off test for K's trunk arm (see K_FIX_LIVEGUN_ON).  The scan
        shape is _live_home_gun's, mirrored to the enemy team:
        get_nearby_buildings is vision-bounded by construction, so this only
        ever sees guns the healer itself can see -- and a gun we cannot see is
        one we cannot stand off from, which is the honest answer and the same
        blind spot piece D already lives with.

        Ordered cheapest-first, the _cb_over_heal discipline: team and type
        filters, then a range^2 compare that rejects every gun which could not
        reach this tile from where it stands, and only then the <= 5-tile ray
        walk.  Launchers are excluded on type -- they throw builder bots and
        have no ray, so there is nothing to stand off from.
        """
        for bid in ct.get_nearby_buildings():
            try:
                if ct.get_team(bid) == self.team:
                ...
                if bp.distance_squared(tile) > (32 if et == EntityType.SENTINEL else 13):
                    continue
                if self._ray_covers(ct, bid, et, bp, tile):
                    return True
            except Exception:
                continue
        return False
    ...
    def _heal_trunk(self, ct):
        ...
        four gates are carried over; the fourth is the one K v1 died of.
         ...
         - PIECE KF (Eir 6d) adds the one gate v1 did NOT have and should:
           a candidate a live enemy gun is presently covering is skipped, not
           healed.  See K_FIX_LIVEGUN_ON for the arithmetic.  This is a target
           FILTER, not a stand-down -- an uncovered candidate on the same turn
           is still repaired, and a turn where every candidate is covered falls
           through to normal role dispatch with the budget unspent.
        """
        rnd = ct.get_current_round()
        if ENDGAME_SWITCH_ON and rnd >= ENDGAME_RND:
            return False
        ...
        for d in CARDINALS:
            ...
            try:
                ...
                if (
                    ...
                    # PIECE KF: a tile a live gun covers is condemned, not
                    # damaged -- +4 HP against 18 a shot loses that race -- so
                    # leave it and keep the turn.  Reached only once a real
                    # heal candidate exists, which is what keeps the scan off
                    # the quiet rounds; next cardinal is still considered.
                    if K_FIX_LIVEGUN_ON and self._live_gun_covers(ct, bp):
                        continue
                    ct.heal(bp)
                    return True
            except Exception:
