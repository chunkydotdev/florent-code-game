...
Eir 6d branch B (_v80e6d_tb): PIECE T -- DELIVERY CONTINUITY.  Everything
outside the three T blocks is bit-for-bit _v79e6c.  See T_DELIVERY_ON.
"""
import math
import random
...
# PIECE T (Eir 6d branch B) -- DELIVERY CONTINUITY.  One toggle, three blocks,
# all of them about the same scoreboard line: EVERY r1000 game involving the
# current slot holder resolved at tiebreak step 1, titanium DELIVERED (11 of 11
# measured; steps 2 and 3 were never reached -- v68-chokewall-first-read).  v68
# delivers ~250 Ti/100 rounds while healthy and ZERO in ~45% of its long games:
# in 5 of 11 grinds its delivered total froze permanently at r59-350 (649-940
# dead rounds) with 95 conveyors alive at r999 and exactly 1 of them wired to
# its Core.  It lost 9 of those 11 grinds on delivered titanium, several by
# 5-15x.  So the lever is not "collect more" and not "spend the bank" -- it is
# KEEP A WIRED CHAIN ALIVE TO ROUND 1000, and v68 has no behaviour change of
# any kind after r300 to contest it with.
#
# Our own line has the same disease from the other end.  _build_next_link
# treats an OCCUPIED tile as a CORRECT link ("occupied implies correct", the F
# diagnosis) -- so one misfaced head, laid by a pave or by a chain that merged
# in from another harvester, poisons a trunk for the rest of the match and the
# linker walks straight past it forever.  Measured price: directed-connected
# harvesters atoll 2, heart 2, jackpot 2, meander 1 against 5/5/3/7 ALIVE, with
# delivery rates matching the DIRECTED count to the decimal (v79-analysis.md:
# 178-179).  The hive gate in _move names the fix explicitly as the follow-on:
# "the linker verifies facing and destroy()+rebuilds wrong heads -- destroy is
# measured free".  T.1 is that follow-on.
#
#  T.1 CHAIN-CONNECTIVITY REPAIR.  A builder standing beside one of our own
#      conveyors whose output tile can never take a stack destroys it (free, no
#      cooldown, no scale cost -- the destroy refunds the +1% the rebuild
#      re-adds) and rebuilds it aimed at something that can.  Local to the four
#      orthogonal neighbours of the acting builder: no map sweep, no rescan.
#  T.2 LATE-GAME HARVESTER SUSTAIN.  The harvester ceiling is compared against
#      SLOT_HARVESTERS, which is a MONOTONE HIGH-WATER MARK of harvesters ever
#      BUILT (_sync_harvesters only ever raises it).  So a farm that is raided
#      to zero at r400 reads as a farm at the cap and is never rebuilt -- the
#      exact freeze shape we are trying to exploit in v68, live in our own
#      file.  Past T_LATE_HARV_MIN_RND a builder that can SEE fewer than
#      T_LATE_HARV_FLOOR live harvesters may rebuild one, out of surplus only.
#  T.3 TRUNK-REPAIR PRIORITY.  When several damaged chain pieces are adjacent
#      at once, K's budgeted trunk arm spends its one heal on the piece whose
#      loss costs the most delivery, instead of on whichever cardinal happens
#      to come first in iteration order.
T_DELIVERY_ON = True
# T.1 -- how often one builder may pay for a re-facing, and how often it is
# even allowed to look.  The scan is ~20 engine calls (four neighbours, a
# handful of getters each) against a 10 ms budget, so the stride is thrift
# rather than necessity; it is staggered by self.idx so five builders standing
# in a line do not all scan on the same round.
T_REPAIR_SCAN_EVERY = 2
T_REPAIR_COOLDOWN_RNDS = 6
# A repaired head is aimed at something live BY CONSTRUCTION, so it cannot
# re-qualify the next time it is looked at -- unless the world changed under
# it.  This latch bounds what happens when it does: the same tile is not
# re-faced twice inside the window, whatever the verdict says.  Same shape and
# same reasoning as ROTATE_COOLDOWN_RNDS, which exists because a per-tile
# liveness test turned out to be exactly the wrong thing to latch on.
T_REPAIR_SAME_TILE_RNDS = 40
# T.2 -- both floors are deliberately conservative.  r300 is where the surge
# already decides a game is an economy race (SURGE_MIN_RND), and 2 visible
# harvesters is a FLOOR, not a target: this rebuilds a dead farm, it does not
# expand a live one (that is what _eco_cap is for).  The bank floor reuses
# SIEGE_HEAL_RESERVE_TI so a rebuild can never eat the heal line's till.
T_LATE_HARV_MIN_RND = 300
T_LATE_HARV_FLOOR = 2
# T.3 -- lower rank wins the heal.  A harvester is the SOURCE (30 HP, and the
# only piece that is also tiebreak 2); a splitter is a junction whose loss
# severs every branch behind it; a conveyor is one replaceable link at 3 Ti.
# Anything not in MEDIC_TYPES -- barriers, turrets, the Core -- is not in this
# arm at all and stays out (barriers are combat capital with their own logic,
# the Core has its own uncapped heal above this one).
T_CHAIN_RANK = {
...
class Player:
    def __init__(self):
        ...
        # PIECE T.1 repair memory (see T_DELIVERY_ON).  t_fix_rnd is the round
        # this builder last paid for a re-facing and t_fix_tile the tile it
        # paid it on.  Per unit instance, no store slot, same argument as the
        # pave trail memory above: only this unit's own scan writes it and only
        # its own scan reads it, and a stale value can only ever make this unit
        # skip a repair, never take a wrong one.
        self.t_fix_rnd = -10 ** 9
        self.t_fix_tile = None
        ...
        for d in CARDINALS:
            ...
            try:
                ...
                if (
                    ...
                    if not T_DELIVERY_ON:
                        ct.heal(bp)
                        return True
                    # PIECE T.3 -- TRUNK-REPAIR PRIORITY (see T_DELIVERY_ON).
                    # The gates, the budget and the target set are untouched;
                    # only the CHOICE among qualifying neighbours changes.
                    # Today that choice is iteration order, i.e. north beats a
                    # harvester one HP from rubble standing to the west.  The
                    # key ranks delivery first (T_CHAIN_RANK: source, then
                    # junction, then link) and breaks ties on lowest HP, i.e.
                    # the piece closest to leaving the chain.  Barriers and
                    # turrets are not in MEDIC_TYPES and so are already ranked
                    # below every chain piece by exclusion; nothing here adds
                    # them back.
                    key = (T_CHAIN_RANK.get(ct.get_entity_type(bid), 3), ct.get_hp(bid))
    ...
    def _expand(self, ct):
        ...
        # PIECE T.2 -- LATE-GAME HARVESTER SUSTAIN (see T_DELIVERY_ON).  The
        # ceiling one line below is compared against SLOT_HARVESTERS, a
        # monotone high-water mark of harvesters ever BUILT, so a farm raided
        # to zero at r400 still reads as a farm at the cap and is never rebuilt
        # -- the same permanent delivery freeze we are exploiting in v68, live
        # in our own file.  This clause is the only way past that gate, and it
        # is deliberately narrow:
        #  - A FLOOR, NOT A TARGET.  Below two VISIBLE live harvesters only.
        #    Growing a working farm stays _eco_cap's job and its +5%/build
        #    scale curve is untouched in every game that keeps two alive.
        #  - NEVER UNDER SIEGE.  SLOT_UNDER == 0, the same signal the rest of
        #    the file reads.  A 20 Ti harvester built into an active raid is a
        #    donation, and the reserve it would spend belongs to the heal line.
        #  - SURPLUS ONLY.  Cost plus SIEGE_HEAL_RESERVE_TI, the same reserve
        #    _eco_spendable holds back, applied here unconditionally rather
        #    than only under siege.
        #  - LATE ONLY.  r300 is where the surge already decides a game is an
        #    economy race (SURGE_MIN_RND); before it the opening bootstrap and
        #    the cap own every harvester decision, bit-for-bit as today.
        late_sustain = (
            T_DELIVERY_ON
            and not endgame
            and ct.get_current_round() >= T_LATE_HARV_MIN_RND
            and ct.get_action_cooldown() == 0
            and ct.read_store(SLOT_UNDER) == 0
            and ct.get_global_resources() >= ct.get_harvester_cost() + SIEGE_HEAL_RESERVE_TI
            and self._t_visible_harvesters(ct) < T_LATE_HARV_FLOOR
        )

        if ct.get_action_cooldown() == 0:
            # PIECE T.1 -- CHAIN-CONNECTIVITY REPAIR (see T_DELIVERY_ON), ahead
            # of the link build on purpose: extending a trunk whose head cannot
            # pass a stack delivers exactly as much as not extending it, and
            # _build_next_link is the code that cannot see the difference
            # (occupied implies correct).  Off past ENDGAME_RND with the rest
            # of the link machinery -- a re-faced conveyor forty rounds from
            # the bell delivers at most one stack, a harvester scores tiebreak
            # 2 outright.
            if not endgame and self._t_repair_chain(ct):
                return
    ...
    def _t_core_md(self, pos):
        """Manhattan distance from pos to the nearest Core footprint tile.

        Deliberately NOT dist_core, which is Chebyshev: a conveyor's output
        step is cardinal, and under Chebyshev a cardinal step along the
        diagonal approach the trail walks most often scores as no progress at
        all.  Manhattan is the metric the pave's own coreward test in _move
        already uses, so T.1 and the pave agree on which way is downhill.
        """
        c = nearest_core_tile(pos, self.core)
        return abs(pos.x - c.x) + abs(pos.y - c.y)

    def _t_facing_verdict(self, ct, tile, facing):
        """PIECE T.1 -- can a stack leave a conveyor at `tile` facing `facing`?

        Answers with the ONE-STEP downstream test, never a walk of the chain:
        the tile a conveyor outputs onto either takes a stack or it does not,
        and that is a constant number of getters.  A broken link deeper down is
        somebody else's adjacent neighbour and gets the same test when a
        builder stands beside it, so the repair propagates along the trunk over
        rounds without anybody ever pathing it.

        Three verdicts:
          "live"   -- leave it alone.  Our own Core footprint (the delivery
                      point) or a friendly conveyor/splitter that is not aimed
                      straight back at us.
          "orphan" -- the output can never take a stack, whatever we build
                      later: the map edge, a wall, an enemy building (their
                      network and their Core included -- pushing there is legal
                      and scores us nothing), a friendly building that never
                      accepts one (harvester, barrier, turret), or a conveyor
                      facing straight back at us, which is a two-cycle that
                      delivers nothing in either direction.
          "stray"  -- the output is an EMPTY tile.  Deliberately NOT split on
                      whether that tile is coreward: a head aimed at open
                      ground is either a chain still being laid or the stranded
                      zig-zag head the pave used to leave on ~50% of trail
                      tiles (see PAVE_TRAIL_ON), and NOTHING LOCAL TELLS THE
                      TWO APART.  So the verdict does not try.  It is the
                      CHOOSER that separates them: a stray is only ever
                      re-aimed onto something that is already standing (the
                      Core, or a live neighbouring chain piece), and when no
                      such thing is adjacent the head is left exactly where it
                      is.  A chain under construction has no such neighbour by
                      definition -- that is what "under construction" means --
                      so it is never touched, while a head stranded one tile
                      off a live trunk is re-attached to it.

        Every getter that can raise answers "live", i.e. an unreadable tile is
        never acted on.  The whole point of the destroy+rebuild is that it is
        free when it is right; it must also be impossible when we are guessing.
        """
        o = tile.add(facing)
        if not (0 <= o.x < self.mw and 0 <= o.y < self.mh):
            return "orphan"
        if dist_core(o, self.core) == 0:
            return "live"
        try:
            if ct.get_tile_env(o) == Environment.WALL:
                return "orphan"
        ...
        try:
            ...
            if et not in (EntityType.CONVEYOR, EntityType.SPLITTER):
                return "orphan"
            # Splitters accept only from the back, so a side feed is dead too --
            # but this file never builds one, so the only splitters on the map
            # are the enemy's and those are already excluded by the team test
            # above.  Left as "live" rather than modelled on an untested rule.
            if et == EntityType.CONVEYOR and ct.get_direction(bid) == facing.opposite():
                return "orphan"
        except Exception:
            return "live"
        return "live"

    def _t_best_facing(self, ct, tile, allow_open):
        """PIECE T.1 -- the facing to rebuild a dead head with, or None.

        Ranked by how much of the answer is already standing on the map:
          0. our own Core footprint -- this tile IS a delivery point;
          1. a friendly conveyor/splitter no farther from the Core that is not
             aimed back at us AND whose OWN one-step verdict is live -- hand the
             stack to a chain that exists and that goes somewhere.  This is the
             only place T.1 looks two steps instead of one, and it is what makes
             re-aiming a stray safe: the target is not a guess, it is a link
             already carrying stacks;
          2. open ground strictly closer to the Core -- a chain that does not
             exist yet, offered ONLY to a hard orphan (allow_open), never to a
             stray.  A stray is aimed at open ground already; re-aiming it at
             different open ground trades one guess for another, and a guess is
             exactly what invented a route into a wall pocket the last time
             this file guessed (the hive dead end in _move).

        Rank 0/1 accept an equal Core distance, rank 2 demands a strictly
        smaller one.  A real link that merely runs level for a tile is how
        every chain rounds an obstacle, and the pairwise loop it could
        otherwise create is excluded outright by the "not aimed back at us"
        test on both sides; open ground has no such evidence behind it, so it
        only ever earns a facing by moving the stack downhill.

        Returning None is a normal outcome and means "leave it standing": the
        caller never destroys anything it cannot immediately improve on.
        """
        here = self._t_core_md(tile)
        best = None
        best_rank = 9
        best_md = 10 ** 9
        for d in CARDINALS:
                ...
                if bid is None:
                    ...
                    try:
                        ...
                        if et == EntityType.CONVEYOR:
                            nf = ct.get_direction(bid)
                            if nf == d.opposite():
                                continue
                            if self._t_facing_verdict(ct, o, nf) != "live":
                                continue
                    except Exception:
                        continue
                    rank = 1
            if rank < best_rank or (rank == best_rank and md < best_md):
                best, best_rank, best_md = d, rank, md
        return best

    def _t_repair_chain(self, ct):
        """PIECE T.1 -- re-face one dead conveyor head we are standing beside.

        Returns True only when the rebuild actually consumed this turn's
        action.  Destroy is free and uncapped by contract, so the ONLY spend
        here is the rebuild, and it is routed through _eco_spendable like every
        other economy purchase -- under siege the reserve holds and the heal
        line keeps its till.

        Cost-scale neutral by construction: destroying an entity removes its
        contribution to the category scale and the rebuild puts the same one
        back, so unlike the stateless relay of a raided conveyor (the +1%-per-
        relay churn the chain medic was written against) this leaves the curve
        exactly where it found it and pays only the 3-odd titanium.

        SAFETY, in the order it matters:
         1. Only a conveyor of OURS, only orthogonally adjacent, only when the
            verdict says the head delivers nothing today.
         2. Only when a strictly better facing already exists (_t_best_facing);
            no better aim, no destroy.
         3. So the tile we destroy is one that is provably delivering nothing.
            If the rebuild then fails for any reason we have lost no delivery
            at all -- the same zero it was already returning -- and the tile is
            left open for the pave, the linker or the next pass to take.
        """
        if not T_DELIVERY_ON or self.core is None:
            return False
        rnd = ct.get_current_round()
        if (rnd + self.idx) % T_REPAIR_SCAN_EVERY:
            return False
        if rnd - self.t_fix_rnd < T_REPAIR_COOLDOWN_RNDS:
            return False
        ...
        for d in CARDINALS:
            tile = p.add(d)
            if not (0 <= tile.x < self.mw and 0 <= tile.y < self.mh):
                continue
            try:
                ...
                if ct.get_entity_type(bid) != EntityType.CONVEYOR:
                    continue
                cur = ct.get_direction(bid)
            except Exception:
                continue
            verdict = self._t_facing_verdict(ct, tile, cur)
            if verdict == "live":
                continue
            if self.t_fix_tile == (tile.x, tile.y) and rnd - self.t_fix_rnd < T_REPAIR_SAME_TILE_RNDS:
                continue
            want = self._t_best_facing(ct, tile, allow_open=(verdict == "orphan"))
            ...
            ct.destroy(tile)
            self.t_fix_rnd = rnd
            self.t_fix_tile = (tile.x, tile.y)
            if ct.can_build_conveyor(tile, want):
                ct.build_conveyor(tile, want)
                return True
            return False
        return False

    def _t_visible_harvesters(self, ct):
        """PIECE T.2 -- live friendly harvesters inside THIS unit's vision.

        A local count on purpose: SLOT_HARVESTERS cannot answer the question
        (it is a monotone high-water mark and never falls when a farm dies),
        and there is no global getter for it.  The count is therefore a LOWER
        bound, and it errs toward rebuilding -- which is the safe direction for
        a floor whose whole job is to notice that the farm is gone.
        """
        live = 0
        for eid in ct.get_nearby_buildings():
            try:
    ...
    def _build_next_link(self, ct):
        ...
        while self.link_queue:
            ...
            if ct.get_tile_building_id(tile) is not None:
                # This is the "occupied implies correct" skip itself (the F
                # diagnosis).  It is left exactly as it was on purpose: the
                # queue head is orthogonally adjacent whenever this line runs
                # -- the manhattan test one line above guarantees it -- so it
                # is already one of the four tiles PIECE T.1 verifies, and T.1
                # runs AHEAD of this call in _expand's action phase.  Fixing
                # the head is therefore T.1's job on the rounds its stride and
                # cooldown allow, and popping a planned tile that someone else
                # already built stays this loop's job on every round.
