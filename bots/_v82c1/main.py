...
class Player:
    ...
    def _homering_plan_valid(self, ct):
        ...
        if self.homering_spot is None:
            return True
        t = unpack_pos(ct.read_store(SLOT_THREAT))
    ...
    def _plan_homering(self, ct):
        ...
        """
        if not C1_HOME_RING_ON or self.map_grid is None or self.core is None:
            return False
        threat = unpack_pos(ct.read_store(SLOT_THREAT))
        if threat is None:
            return False
    ...
    def _defend(self, ct):
        ...
        if (
            C1_HOME_RING_ON and self.role_n == 4 and under
