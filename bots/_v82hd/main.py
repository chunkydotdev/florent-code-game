...
# HEIMDALL RING -- DEFENDER-SIDE LAUNCHER DISPOSAL OF INSERTED RAIDERS.
#
# Two independent teams evolved the same defensive use of a Launcher, and the
# insertion class it answers is a measured soft spot of both of OUR lineages:
#  - CAD-family defenders recycle the attacker's inserted raiders at a fixed
#    edge tile, 17-28 repeat throws a game (ferry re-attribution, 2026-08-07:
#    every long-game throw loop in a7aa49ec/b10cce55/cdbd5b52 belongs to the
#    DEFENDER, not to CAD -- CAD's own launcher is r1-build/r6-self-destroy in
#    13 of 15 games).
#  - Clankers run a 266-throw all-outward ejection perimeter (5 games).
#  - Against that, OUR whole corpus shows 3 throws in 5 games.
# The class being answered: CAD/KCM throw 2-3 raiders to ~3-5 tiles from our
# Core on rounds 2-4; those raiders then plant turrets inside core-dsq 10-41
# (median first turret r11, first fire r12).  A Launcher is 20 Ti, no ammo,
# facing-independent, and picks up an adjacent Builder Bot from EITHER team,
# so one ejection costs the raider its walk back (~5+ rounds) for one action.
#
# MEASURED CORRECTION to the premise this branch was specified under (parent
# _v81e6e vs cad_probe, seed 1, all 15 pool maps): our incumbent "insertion"
# Launcher is NOT forward.  It is built by the role_n == 4 defender, which
# holds inside dsq 8 of the Core, so it lands at footprint-dsq 1-5 -- inside
# the home band -- in 4 of 4 games where it is built at all, and it is built
# in only 4 of 15 games, at rounds 9-21, i.e. never before the r2-4 insertion
# and usually not at all.  Position alone therefore cannot separate the two
# roles, and a purely positional home test would silently disable the
# insertion ferry on every map.  So SLOT_LAUNCHER is widened from a 0/1 flag
# into a REGISTRY: it holds pack_pos() of the tile of the one Launcher that
# owns the insertion role (still nonzero, and every existing reader of the
# slot tests truthiness only -- lines documented at each site).  A Launcher is
# in HOME MODE iff it stands in the home band AND the registry does not name
# its own tile; a home-mode Launcher runs the ejection reflex only, never
# claims the registry, and never picks up a friendly builder.
#
# What is actually new, then, is the REACTIVE BUILD: a defender plants the
# disposal Launcher the round raiders reach the home band, out of the plain
# bank (emergency semantics, not _eco_spendable), instead of waiting for
# harv >= ECO_NEED.  The insertion Launcher keeps its own late, economy-gated
# build; the two are separated by the registry, not by geometry.
HEIMDALL_ON = True
# Home band: footprint-measured dsq, i.e. distance to the NEAREST of the four
# Core tiles.  8 is "two tiles off the footprint diagonal" -- the raider has to
# be adjacent to the Launcher for a pickup, so the ring has to sit on the
# Core's own doorstep to cover the tiles a turret can be planted from.
HEIMDALL_HOME_DSQ = 8
# Raider band that triggers the build.  Same constant the melee emergency in
# _builder already senses on (dsq 20 of the Core), so the reactive build fires
# on exactly the population that already recalls hands to home defense.
HEIMDALL_BAND_DSQ = 20
...
SLOT_HARVESTERS = 4
SLOT_ECO_READY = 5
# INSERTION-LAUNCHER REGISTRY (widened by the Heimdall ring, see HEIMDALL_ON).
# 0 = no insertion Launcher; otherwise pack_pos() of its tile, which is always
# nonzero (pack_pos adds 1 to both coordinates before packing).  Every reader
# outside _launcher tests truthiness only -- _live_builders, the two
# launchwait clauses in _builder, _expand's has_launch and both guards in
# _try_build_launcher -- so widening the payload cannot change their answers.
SLOT_LAUNCHER = 6
SLOT_HOME_GUN = 7
SLOT_DROPPED = 8
...
def core_dsq(pos, o):
    """Squared distance to the NEAREST Core footprint tile.

    The measure every band constant in this file is stated in (HUNT_BAND_DSQ,
    the counterbattery reach test, the Heimdall bands); named here because the
    Heimdall ring needs it at five call sites, three of them per-turn.
    """
    return min(pos.distance_squared(c) for c in core_tiles(o))
...
class Player:
    ...
    def _core(self, ct):
        ...
        pop_refill = POP_FLOOR_ON and self._live_builders(
            ct, units, weapons, self._home_launchers(ct, p)
        ) < pop_floor
        pop_expand = (
            POP_FLOOR_ON
    ...
    def _home_launchers(self, ct, anchor):
        """HEIMDALL -- live friendly Launchers in the home band, MINUS the one
        the registry already accounts for (see HEIMDALL_ON, SLOT_LAUNCHER).

        Core-side census term.  A Launcher is a living unit that
        get_unit_count() counts and _live_builders subtracts nothing for
        unless SLOT_LAUNCHER names it, so a standing Heimdall Launcher would
        inflate by_census by one and the Core would answer a real population
        crash with one fewer body -- the measured loss class RIDE-ALONG 2
        exists to close.

        `anchor` is the Core's own position (the NW footprint tile); this runs
        Core-side, where self.core is never set.  get_nearby_units is used
        rather than get_nearby_buildings because it returns only Core,
        builders and turrets -- a handful near home -- instead of every
        conveyor in a 113-tile vision disc.  Core vision is r^2 = 36 and a
        tile at footprint-dsq 8 is at anchor-dsq <= 18, so the band is fully
        covered and the dsq argument is the cheap bound, not a new constant.

        Over-counting Launchers here is the SAFE direction (it lowers
        by_census, i.e. spawns more); under-counting is the hazard.  So a
        registry that names a DEAD Launcher deliberately still costs its 1.
        """
        if not HEIMDALL_ON:
            return 0
        reg = unpack_pos(ct.read_store(SLOT_LAUNCHER))
        n = 0
        for uid in ct.get_nearby_units(18):
            try:
                if ct.get_entity_type(uid) != EntityType.LAUNCHER:
                    continue
                if ct.get_team(uid) != ct.get_team():
                    continue
                up = ct.get_position(uid)
                if core_dsq(up, anchor) <= HEIMDALL_HOME_DSQ and up != reg:
                    n += 1
            except Exception:
                continue
        return n

    def _live_builders(self, ct, units, weapons, extra_launchers=0):
        ...
        `extra_launchers` closes the one hole the Heimdall ring opens in (b):
        the registry accounts for a single insertion Launcher, so every OTHER
        live Launcher (a home disposal ring) has to be subtracted by the
        caller's own live count -- see _home_launchers.
        ...
        by_census = (
            units - 1 - weapons
            - (1 if ct.read_store(SLOT_LAUNCHER) else 0)
            - extra_launchers
        )
    ...
    def _note_friendly_launcher(self, ct):
        ...
        for eid in ct.get_nearby_buildings():
            if ct.get_team(eid) == self.team and ct.get_entity_type(eid) == EntityType.LAUNCHER:
                # HEIMDALL -- the registry means "the INSERTION Launcher
                # exists", and it drives the launchwait role flow.  A home
                # disposal Launcher must not fill it: doing so would leave the
                # waiting raiders staging beside a Launcher that never picks a
                # friendly up, and would suppress the real insertion build.
                # Registered by tile so _launcher can recognise itself.
                lp = ct.get_position(eid)
                if (
                    HEIMDALL_ON and self.core is not None
                    and core_dsq(lp, self.core) <= HEIMDALL_HOME_DSQ
                ):
                    continue
                ct.write_store(SLOT_LAUNCHER, pack_pos(lp))
                return
    ...
    def _try_build_launcher(self, ct):
        ...
        for eid in ct.get_nearby_buildings():
            if ct.get_team(eid) == self.team and ct.get_entity_type(eid) == EntityType.LAUNCHER:
                # HEIMDALL -- same hygiene as _note_friendly_launcher: a home
                # disposal Launcher is not the insertion Launcher, so it must
                # neither fill the registry nor suppress this build.
                lp = ct.get_position(eid)
                if (
                    HEIMDALL_ON and self.core is not None
                    and core_dsq(lp, self.core) <= HEIMDALL_HOME_DSQ
        ...
        for d in DIRECTIONS:
            ...
            if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh and ct.can_build_launcher(bp):
                ...
                # Registry, not a flag: the tile is what lets the Launcher
                # built here recognise itself as the insertion one on its
                # first turn, even standing inside the home band (which,
                # measured, is where this defender always builds it).  Same
                # round as the claim above, so the buffered store carries only
                # this second write into the next round.
                ct.write_store(SLOT_LAUNCHER, pack_pos(bp))
                return True
        # Build failed — release claim so we retry next turn
        ct.write_store(SLOT_LAUNCHER, 0)
        return False

    def _try_heimdall(self, ct):
        """HEIMDALL RING -- plant the home disposal Launcher (see HEIMDALL_ON).

        Fires on the state the insertion class creates and nothing else: an
        enemy BUILDER BOT already inside the home band, no live Launcher on
        the Core's doorstep, and a bank that covers one.

        The bank test is a plain read, deliberately NOT _eco_spendable: this
        is defense, and defense in this file spends to the last titanium
        because SIEGE_HEAL_RESERVE_TI exists for it.  The Launcher is still
        the cheapest turret-class answer on the board and the only one that
        costs no ammunition afterwards -- but the get_launcher_cost() read is
        load-bearing, because the cost scale in this engine is ONE team-wide
        number: measured across 13 reactive builds, the 20 Ti base was already
        scaled to 30-62 Ti (median 48) by rounds 1-54.

        Ordered cheapest-first: cooldown and the two cost calls, then the
        raider scan, and only on a confirmed raider the local Launcher scan --
        so a quiet defender turn pays three engine calls for this.
        """
        if not HEIMDALL_ON or self.core is None:
            return False
        if ct.get_action_cooldown() != 0:
            return False
        if ct.get_global_resources() < ct.get_launcher_cost():
            return False
        raider, best = None, 10 ** 9
        for eid in ct.get_nearby_entities():
            if ct.get_entity_type(eid) != EntityType.BUILDER_BOT:
                continue
            if ct.get_team(eid) == self.team:
                continue
            rp = ct.get_position(eid)
            d = core_dsq(rp, self.core)
            if d <= HEIMDALL_BAND_DSQ and d < best:
                best, raider = d, rp
        if raider is None:
            return False
        # The live scan below is VISION-bounded, and the home band is wider
        # than one builder's vision (r^2 = 20): measured, the same defender
        # planted a second ring Launcher 17 rounds after the first from a tile
        # that could not see it (drumlin seed 4, two standing all game -- 50 Ti
        # and +10% on the team-wide cost scale for a duplicate).  So a builder
        # that has already planted one remembers the tile and re-reads it; an
        # out-of-vision tile raises, and the conservative answer to "is my ring
        # still standing" is yes.
        mine = getattr(self, "hd_tile", None)
        if mine is not None:
            try:
                bid = ct.get_tile_building_id(mine)
                still = (
        ...
        for bid in ct.get_nearby_buildings():
            if (
                ...
                and ct.get_entity_type(bid) == EntityType.LAUNCHER
                and core_dsq(ct.get_position(bid), self.core) <= HEIMDALL_HOME_DSQ
            ):
                return False
        p = ct.get_position()
        cands = []
        for d in DIRECTIONS:
            ...
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            if core_dsq(bp, self.core) > HEIMDALL_HOME_DSQ:
                continue
            cands.append(bp)
        # Nearest to the raider first -- a pickup needs adjacency, so range to
        # the body being disposed of is the whole point.  Two tiebreaks behind
        # it: prefer a tile that is NOT touching the footprint (every such tile
        # is also a Core spawn seat, and the Core spawns at most one builder a
        # turn onto exactly those), then a fixed order so paired runs stay
        # reproducible.
        cands.sort(key=lambda t: (
            t.distance_squared(raider), dist_core(t, self.core) == 1, t.y, t.x
        ))
        for bp in cands:
    ...
    def _home_defend(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            if self._sabotage_prio(ct):
                ...
                pass
            # HEIMDALL RING.  Behind the two damage answers -- a peck or an
            # aligned turret removes the raider, an ejection only rewinds its
            # walk -- and ahead of the heal, because +4 HP a round does not
            # answer a raider that is planting turrets.
            elif self._try_heimdall(ct):
                pass
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if under:
                ...
                if shelled and not self._cb_over_heal(ct) and self._heal_core(ct):
                    ...
                    # HEIMDALL RING sits last in this chain for the same
                    # reason it does in _home_defend: it rewinds a raider's
                    # walk, it does not kill it, so both damage answers get
                    # the action first.  The heal fallback below still catches
                    # the rounds all three fail.
                    defended = (
                        self._sabotage_prio(ct)
                        or self._try_counterbattery(ct)
                        or self._try_heimdall(ct)
                    )
    ...
    def _launcher(self, ct):
        ...
        if self.core is None:
            for eid in ct.get_nearby_buildings():
                if ct.get_entity_type(eid) == EntityType.CORE and ct.get_team(eid) == self.team:
                    self.core = ct.get_position(eid)
                    break
        # HEIMDALL RING -- ROLE, decided from the registry, not from geometry
        # alone (see HEIMDALL_ON: measured, the insertion Launcher stands in
        # the home band too, at footprint-dsq 1-5).  HOME MODE is "in the band
        # and not the registered insertion Launcher": ejection reflex only, no
        # registry claim, no friendly pickup, no touching of the insertion
        # bookkeeping slots.  Everyone else -- the registered Launcher and any
        # Launcher outside the band -- keeps the shipped behaviour exactly,
        # including re-registering its own tile every turn.
        home = (
            HEIMDALL_ON and self.core is not None
            and core_dsq(ct.get_position(), self.core) <= HEIMDALL_HOME_DSQ
            and unpack_pos(ct.read_store(SLOT_LAUNCHER)) != ct.get_position()
        )
        if not home:
            ct.write_store(SLOT_LAUNCHER, pack_pos(ct.get_position()))
        if self.core is None:
            return
        w, h = ct.get_map_width(), ct.get_map_height()
        ...
        # HOME MODE skips the whole insertion half -- targeting, the claim
        # heartbeat and the pickup scan alike.  Skipped rather than filtered:
        # a disposal Launcher must not pay ~100 bounds checks and a sort for
        # drop sites it will never throw to, and must not clear SLOT_LAUNCH_ID
        # (that claim belongs to the registered insertion Launcher; expiring
        # it from here would cancel a raider that is already staging).  With
        # the list left empty the throw loop at the bottom is a no-op, so the
        # non-home path below is byte-for-byte the shipped one.
        drop_sites = []
        chosen = 0
        if not home:
