...
# PIECE U -- HARVESTER-FAMINE RE-TRIGGER.  The largest measured our-side leak
# of the night.  Across 120 games the tiebreak decode found nine harvester
# famines (live harvesters below 2 after r300, having peaked at 2+) and in ALL
# NINE the economy never rebuilt: zero harvester rebuilds, delivery pinned at
# zero for 276-699 rounds, paving continuing as if the farm still existed
# (docs/research/tiebreak-split-decode-2026-08-07.md, finding (d)).  The
# mechanism is ancestral and shared with v69: SLOT_HARVESTERS is a deliberate
# monotone HIGH-WATER mark (_sync_harvesters writes only when live > stored)
# and every economy gate in the file reads it, so after a wipe the whole file
# still believes the farm exists and nothing routes a builder back to ore.
#
# DETECTION IS THE HARD HALF.  Three candidates, two refuted by measurement:
#  (a) re-encode SLOT_HARVESTERS as high_water*64 + live_count.  DEAD ON
#      ARRIVAL -- nothing in this game can produce the live count.  The Core's
#      vision is r^2=36 and harvesters sit at ore far outside it; a builder
#      sees only its own r^2=20 disk, which is exactly WHY _sync_harvesters is
#      a high-water mark; and the cost-scale inversion that would have given an
#      exact global count (harvester cost == base + live harvesters) is refuted
#      by probe: the scale is ONE team-wide number, not per category.  Measured
#      2026-08-07 with a conveyor-only builder -- get_harvester_cost() climbed
#      20 -> 27 while zero harvesters existed, tracking the conveyor builds
#      one-for-one at +1%.  docs/game-model.md:365 is right; the per-category
#      reading of the official cost table is wrong.
#  (b) the bank-delta signature (income == passive schedule for N rounds).
#      Refuted by this file's own measured note -- see the INCOME METER in
#      _core: deliveries and spends land between two Core turns, so a bank
#      delta nets them against each other and reads 0 on a round that earned 30
#      and spent 30.  Every round our builders spend what the farm earns would
#      read as famine, and every famine round with a live pave would not.
#  (c) SHIPPED -- meter the delivery itself, at the door.  Titanium enters the
#      Core only from an orthogonally adjacent relay whose output tile is a
#      footprint tile, and that relay HOLDS the stack for the round before
#      end-of-round distribution moves it in.  So the Core alone, with no store
#      slot, no second writer and no builder cooperation, can count deliveries
#      exactly.  Validated against the replay's own distributeResources
#      records: 491 metered vs 491 recorded and per-round agreement on 1000 of
#      1000 rounds (lighthouse s1 vs opp_v63, 2026-08-07).
#
# THE THRESHOLD IS MEASURED, not guessed.  Over 60 games vs opp_v69 our
# inter-delivery gaps are n=15,333: median 1, p99 4, p99.9 6, and the largest
# gap in any game whose economy was alive is 10.  Exactly ONE gap in the whole
# corpus exceeded 30 -- 367 rounds, a genuine wipe.  24 rounds therefore sits
# 4x above the healthy p99.9 and an order of magnitude below every observed
# famine, i.e. the two distributions do not touch.
PIECE_U_ON = True
U_DRY_RNDS = 24

# SIGNALLING, and why it needs no slot: SLOT_ECO_READY is WRITE-ONLY in this
# file.  Four sites write 1 (_core, _sync_harvesters, _try_harvester, _expand)
# and NOTHING has ever read it, in this file or any ancestor -- so giving it a
# reader costs no behaviour.  The famine flag is the CONTRADICTION between the
# two economy slots: SLOT_HARVESTERS >= ECO_NEED (the farm was built) while
# SLOT_ECO_READY == 0 (the Core says nothing is arriving).
#
# BOOTSTRAP-UNREACHABLE BY CONSTRUCTION, which is the hard requirement here
# (the _v70ec lesson: never touch the opening economy gates).  Every writer of
# SLOT_HARVESTERS raises SLOT_ECO_READY to 1 in the SAME round it crosses
# ECO_NEED, and store writes are buffered, so the pair (harv >= ECO_NEED,
# ready == 0) cannot be observed in ordinary play at all.  The only way to
# produce it is the Core's famine write below, which additionally requires that
# a delivery has already been seen at least once.  Before the first harvester
# there is no famine state to reach.
#
# COLLISION ANALYSIS (writes are buffered and last-write-wins).  The Core is
# the only writer that ever writes 0, and it suppresses its own 1 in the same
# branch, so it never races itself.  Two builder sites can write 1 during a
# famine: _sync_harvesters when it SEES ECO_NEED live harvesters near home (the
# famine is over -- correct), and a successful harvester build (the famine is
# being repaired -- correct).  Either way the flag clears one round early at
# worst, and the Core re-asserts it the next round if delivery is still dry.
# Builders hold no famine latch, so a one-round flicker costs one round of the
# walk and nothing else.
#
# Manhattan cap on the famine walk.  An expander that answers a famine by
# marching across the map into enemy ground trades a dead economy for a dead
# builder; 12 is about the radius our own half is worth on the pool's larger
# maps, and every famine episode in the decode had unmined ore far closer than
# that (the ore was covered by our own paving or simply never revisited).
U_ORE_MAX_WALK = 12
# Rounds a builder that has just planted a famine harvester is left alone to
# wire it.  A harvester delivers its first stack on build and then one every
# four rounds, so the flag clears as soon as the link reaches home; the window
# only has to cover laying that link, and it is what stops the walk from
# abandoning the unfinished chain it just started.
U_REBUILD_GRACE = 60
# What a famine builder may destroy off an ore tile to get at it: our own
# relays and barriers only -- never a turret (that is defense) and never a
# harvester (nothing to fix if one already stands there).
U_CLEARABLE = (EntityType.CONVEYOR, EntityType.SPLITTER, EntityType.BARRIER)
# Rounds a famine walker keeps the ore tile it committed to before re-picking.
# Measured need: the first cut re-picked the NEAREST free ore every round, and
# on meander a builder oscillated between two tiles for 250 rounds because the
# nearest ore flips as soon as it takes a step -- one tile away from the ore it
# was standing beside, with the bank three titanium short of the harvester.
# Commitment is the whole fix; the timeout only covers ore that turns out to be
# unreachable.
U_TGT_RNDS = 40
...
class Player:
    def __init__(self):
        ...
        # PIECE U famine meter, Core-only (see PIECE_U_ON and _u_famine).
        # u_ring is the Core's own delivery doorstep -- the in-bounds tiles
        # orthogonally adjacent to the 2x2 footprint -- cached on first use
        # because the Core never moves.  u_dry counts consecutive rounds with
        # no delivery observed, u_fed records that delivery was ever seen at
        # all (a famine cannot precede the economy it starves).
        self.u_ring = None
        self.u_core = None
        self.u_dry = 0
        self.u_fed = False
        # PIECE U, builder side: the round this unit last planted a famine
        # harvester (see U_REBUILD_GRACE) and the ore tile its famine walk is
        # committed to (see U_TGT_RNDS).  Per unit instance, no store slot --
        # only this unit's own build writes them and only its own walk reads
        # them, exactly like self.converging and self.hunting above.
        self.u_built_rnd = -10 ** 9
        self.u_tgt = None
        self.u_tgt_rnd = -10 ** 9
        self.u_tgt_gap = 10 ** 9
    ...
    def _core(self, ct):
        ...
        # PIECE U -- the Core owns the famine flag (see PIECE_U_ON).  It is the
        # only unit that can meter delivery and the only writer that ever puts
        # a 0 in this slot, so the single-writer discipline the store demands is
        # kept without a protocol.  With the toggle off the write below is the
        # shipped one-liner, unchanged.
        u_famine = self._u_famine(ct, p, w, h, harv) if PIECE_U_ON else False
        if harv >= ECO_NEED:
            ct.write_store(SLOT_ECO_READY, 0 if u_famine else 1)
        ...
        if sporks_ammo:
            ...
            # PIECE U -- the same starvation arithmetic as the builder half.
            # With a turret standing this drip spends the bank down to 12 every
            # turn it can, which is below the scaled harvester price, so a
            # famine bank never reaches the one purchase that restarts income.
            # Ammunition bought out of a dead economy is a magazine with no
            # match after it; the floor is lifted only while the famine holds
            # and drops back the round delivery resumes.
            if u_famine:
                ti_floor = max(ti_floor, ct.get_harvester_cost() + 12)
    ...
    def _u_inbound(self, ct):
        """True if titanium is landing in the Core this round (PIECE_U_ON).

        Delivery has exactly one shape: a friendly relay orthogonally adjacent
        to the footprint, holding a stack, whose output tile is a footprint
        tile -- end-of-round distribution then moves that stack in.  Reading it
        one round early is the point: the meter sees every delivery exactly
        once and never double-counts a stack that is merely passing by.
        Measured exact against the replay's distributeResources records (491 of
        491 stacks, 1000 of 1000 rounds agreeing).

        A harvester adjacent to the footprint delivers straight into the Core
        on its own 4-round cadence and never parks its stack on a tile we can
        read, so its mere existence counts as fed -- it is also, trivially, a
        live harvester, which is the one thing a famine cannot coexist with.

        Every lookup fails safe to "not inbound": get_stored_resource_id raises
        on entity kinds that cannot hold a resource (measured on a Launcher),
        and a Core that mistakenly reads dry only delays the flag by a round.
        """
        team = ct.get_team()
        for t in self.u_ring:
            ...
            if kind not in (EntityType.CONVEYOR, EntityType.SPLITTER):
                continue
            try:
                if ct.get_stored_resource_id(bid) is None:
                    continue
                # A splitter rotates its output among three directions, so a
                # loaded one on the doorstep reaches the Core on its own; a
                # conveyor has to be aimed at it.
                if kind == EntityType.SPLITTER:
                    return True
                if dist_core(t.add(ct.get_direction(bid)), self.u_core) == 0:
                    return True
            except Exception:
                continue
        return False

    def _u_famine(self, ct, p, w, h, harv):
        """Core-side famine detector (see the PIECE U block).

        Famine is "the farm was built and nothing has arrived for U_DRY_RNDS
        rounds".  Gated on the high-water mark reaching ECO_NEED so the whole
        path is unreachable before the economy exists, and on having seen at
        least one delivery so a farm that was never wired cannot be read as one
        that died.  The meter costs eight get_tile_building_id calls on a bare
        doorstep and only runs once the economy is up.
        """
        if harv < ECO_NEED:
            return False
        if self.u_ring is None:
            self.u_core = p
            fp = {(c.x, c.y) for c in core_tiles(p)}
            self.u_ring = [
                t for c in core_tiles(p) for d in CARDINALS
                for t in (c.add(d),)
                if (t.x, t.y) not in fp and 0 <= t.x < w and 0 <= t.y < h
            ]
        if self._u_inbound(ct):
            self.u_fed = True
            self.u_dry = 0
            return False
        if not self.u_fed:
    ...
    def _expand(self, ct):
        ...
        # PIECE U -- famine is the CONTRADICTION between the two economy slots:
        # the high-water mark says the farm was built, the Core says nothing is
        # arriving.  See the PIECE U block for why that pair cannot occur before
        # the economy latches, and for the collision analysis on slot 5.
        famine = (
            PIECE_U_ON and harv >= ECO_NEED
            and ct.read_store(SLOT_ECO_READY) == 0
        )
        ...
        # PIECE U -- THE STARVATION HALF, and the measured reason the naive
        # re-trigger would have been another dead instrument.  At every famine
        # detection in the first smoke leg the bank read 5-10 Ti against a
        # harvester at 25-27: passive income is 10 Ti per 4 rounds and the
        # economy paths nibble it away faster than it arrives -- a pave or a
        # link tile is 3-5 Ti, the chain medic is 1 Ti a tick above a 20 Ti
        # floor (i.e. it caps the bank BELOW the harvester price), and none of
        # them can deliver anything while the farm is dead.  So a famine
        # expander stops paving, stops extending links it is not standing at
        # the head of, and stops patching relays, and the trickle accumulates
        # to one harvester in about six rounds instead of never.
        #
        # The grace window is the exception in both directions: a builder that
        # just planted a famine harvester DOES get to spend on the link that
        # wires it, because that link is the delivery.  It is a PROGRESS
        # watchdog, not a fixed window -- every link tile actually laid re-arms
        # it -- because the first cut abandoned half-built chains on a timer and
        # measured the result: saga seed 1 rebuilt six harvesters in 300 rounds
        # and delivered nothing from any of them, each one dropped 60 rounds
        # later for the next.  A chain that is progressing is never abandoned;
        # one that has not moved in U_REBUILD_GRACE rounds is, and its stale
        # queue goes with it so the move phase below cannot be recaptured by it.
        u_wiring = (
            famine and self.link_queue
            and ct.get_current_round() - self.u_built_rnd < U_REBUILD_GRACE
        )
        u_saving = famine and not u_wiring
        if u_saving and self.link_queue:
            self.link_queue = []
        if ct.get_action_cooldown() == 0:
            if not endgame and not u_saving and self.link_queue:
                if self._build_next_link(ct):
                    if famine:
                        self.u_built_rnd = ct.get_current_round()
                    return
            # PIECE U spends on the same terms the endgame switch already
            # spends on -- bank covers the scaled cost, no ceiling, no reserve.
            # The ceiling is a high-water read and therefore exactly the broken
            # gate; the siege reserve exists to keep 16 Ti for the heal line,
            # and in a famine the harvester IS what refills that till.
            if (
                ct.get_global_resources() >= ct.get_harvester_cost()
                if (endgame or famine)
                else (
                ...
                for d in DIRECTIONS:
                    ...
                    if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh and ct.can_build_harvester(bp):
                        ct.build_harvester(bp)
                        built = True
                        # ANCESTRAL OFF-BY-ONE, fixed here because PIECE U is
                        # the first reader slot 5 has ever had.  Store writes
                        # are buffered -- invisible even to the writer, probed
                        # directly 2026-08-07 -- so the shipped
                        # `read_store(...) >= ECO_NEED` after the increment
                        # tested the value from BEFORE the build and latched
                        # ECO_READY one harvester late, leaving exactly one
                        # round in which the high-water mark says ECO_NEED and
                        # the ready flag still says 0.  That pair IS the famine
                        # signature, so the latch is computed the way
                        # _try_harvester already computes it: from the value
                        # being written.  (The same buffering makes
                        # `read_store(SLOT_HARVESTERS) + 1` and `harv + 1`
                        # the same number, so the first line is a rename.)
                        # Toggled, not unconditional: the shipped expression is
                        # kept when PIECE U is off so the ablation leg is
                        # byte-identical, store snapshot included -- the replay
                        # records the team store, so even a slot no code reads
                        # shows up in a paired-replay diff.
                        ct.write_store(SLOT_HARVESTERS, harv + 1)
                        u_new_h = (
                            harv + 1 if PIECE_U_ON
                            else ct.read_store(SLOT_HARVESTERS)
                        )
                        if u_new_h >= ECO_NEED:
                            ct.write_store(SLOT_ECO_READY, 1)
                        if not self.link_queue:
                        ...
                        if famine:
                            self.u_built_rnd = ct.get_current_round()
                        break
                # PIECE U -- OUR OWN PAVING COVERS THE ORE.  The trail pave lays
                # a conveyor on any empty tile it walks off, ore included, and
                # can_build_harvester then refuses that tile for the rest of the
                # match while _pick skips it as "occupied".  destroy() is free,
                # costs no action cooldown and is unlimited per turn, so the
                # clear and the harvester both fit in this same turn.  Famine
                # only: outside one, that conveyor is carrying titanium.
                if famine and not built:
                    self._u_clear_ore(ct, p)
            ...
            # PIECE U saves through the medic too -- see the starvation block:
            # MEDIC_TI_FLOOR = 20 sits BELOW the scaled harvester price, so a
            # famine bank that the medic is allowed to trim can never reach the
            # one purchase that ends the famine.
            if not endgame and not u_saving and ct.get_global_resources() >= MEDIC_TI_FLOOR and (
                medic_late
        ...
        if (
            ...
            for d in CARDINALS:
                ...
                if (
                    ...
                    self.stuck = 0
                    return
        # PIECE U -- THE FAMINE WALK.  Ranked ABOVE the link queue, and that is
        # the whole point of the piece: after a wipe the queue is a road to a
        # head that no longer exists, and the measured signature of all nine
        # decoded episodes is exactly that -- relays kept going up (+1, +4 in
        # two of them) while zero harvesters were rebuilt.  Paving is suppressed
        # on this walk (pave=False): the trail pave is what covers ore in the
        # first place, and a famine is not the moment to lay more road.
        #
        # The grace window is what stops the obvious oscillation: a builder that
        # has just planted a famine harvester holds a fresh link_queue, and the
        # famine flag cannot clear until that link actually delivers, so without
        # it the same builder would abandon its own new chain and walk to the
        # next ore tile forever.  Per unit instance, no store slot -- only this
        # unit's own build sets it and only its own walk reads it.
        if u_saving:
            rnd_u = ct.get_current_round()
            gap = (
                abs(p.x - self.u_tgt.x) + abs(p.y - self.u_tgt.y)
                if self.u_tgt is not None else 10 ** 9
            )
            # U_TGT_RNDS is a NO-PROGRESS watchdog, not a lease.  Measured on
            # jackpot: a plain lease expired while the builder was already
            # standing beside free ore waiting for the bank, and the re-pick
            # (ranked by core distance) then walked it away from the tile it
            # was one titanium short of building on.  Closing the gap -- or
            # holding in build range, which IS the work while the bank fills --
            # re-arms the clock.
            if gap <= 1 or gap < self.u_tgt_gap:
                self.u_tgt_rnd = rnd_u
                self.u_tgt_gap = gap
            if not (
                self.u_tgt is not None
                and rnd_u - self.u_tgt_rnd <= U_TGT_RNDS
                and self._u_ore_free(ct, self.u_tgt)
            ):
                self.u_tgt = self._u_ore_target(ct, p)
                self.u_tgt_rnd = rnd_u
                self.u_tgt_gap = 10 ** 9
                self.stuck = 0
                self.wall = None
            ...
            if self.u_tgt is not None:
                if gap == 1:
                    # In build range.  HOLD -- the action phase owns this tile
                    # and the bank is still filling; stepping away and back is
                    # exactly the oscillation U_TGT_RNDS was written against.
                    return
                if gap == 0:
                    # A builder can never build on its own tile, so one step
                    # off any ore is the whole move (the wall-density-gated
                    # step-off above does not fire on open maps).
                    for d in CARDINALS:
                        n = p.add(d)
                        if (
                            0 <= n.x < self.mw and 0 <= n.y < self.mh
        ...
        for d in DIRECTIONS:
            ...
            if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh:
                if ct.get_tile_env(bp) == Environment.ORE_TITANIUM and ct.get_tile_building_id(bp) is None:
                    self.tgt = bp
                    break
        # PIECE U: no paving while saving (see the starvation block) -- the
        # trail pave is both the drain on the bank and the thing that covers
        # ore tiles in the first place.
        self._nav(ct, pave=allow_pave and not u_saving)

    def _u_ore_target(self, ct, p):
        """Nearest known unbuilt ore tile within U_ORE_MAX_WALK (PIECE U).

        Deliberately NOT _pick: _pick partitions the map's ore between four
        static worker slots and walks a cursor through its own slice, which is
        the right rule for parallel expansion and the wrong one for an
        emergency -- the ore that is still free after a wipe is whatever the
        farm used to sit on, not whatever this worker's slice points at.
        Vision-gated occupancy only: a tile we cannot see is worth walking to
        and re-judging on arrival.
        """
        best = None
        best_k = None
        for t in self.map_ores:
            d = abs(t.x - p.x) + abs(t.y - p.y)
            if d > U_ORE_MAX_WALK:
                continue
            # Ranked by distance to the CORE first, walk second.  What ends a
            # famine is delivery, not a harvester: the chain back is paid for
            # out of the same starved bank at 3-7 Ti a tile, so the ore three
            # tiles from home beats the ore one step from this builder.
            k = (dist_core(t, self.core), d)
            if best_k is not None and k >= best_k:
                continue
            if not self._u_ore_free(ct, t):
                continue
            best, best_k = t, k
        return best

    def _u_ore_free(self, ct, t):
        """True if a famine harvester could end up on ore tile t.

        Empty, or out of vision (worth walking to and re-judging on arrival),
        or holding one of OUR OWN clearable relays -- destroy() is free, so a
        conveyor our own paving left there is not an obstacle (see
        _u_clear_ore).  Anything else -- a harvester already there, a turret,
        an enemy building -- disqualifies the tile.
        """
        try:
            ...
            return (
                ...
                and ct.get_entity_type(bid) in U_CLEARABLE
            )
        except Exception:
            return False

    def _u_clear_ore(self, ct, p):
        """Famine only: destroy our own relay squatting on adjacent ore and put
        the harvester there in the same turn (see the call site in _expand)."""
        for d in CARDINALS:
            bp = p.add(d)
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            try:
                ...
                bid = ct.get_tile_building_id(bp)
                if (
                    bid is None or ct.get_team(bid) != self.team
                    or ct.get_entity_type(bid) not in U_CLEARABLE
                    or not ct.can_destroy(bp)
                ):
                    continue
                ct.destroy(bp)
                if not ct.can_build_harvester(bp):
                    return False
                ct.build_harvester(bp)
            except Exception:
                return False
            new_h = ct.read_store(SLOT_HARVESTERS) + 1
            ct.write_store(SLOT_HARVESTERS, new_h)
            if new_h >= ECO_NEED:
                ct.write_store(SLOT_ECO_READY, 1)
            self.u_built_rnd = ct.get_current_round()
            if not self.link_queue:
