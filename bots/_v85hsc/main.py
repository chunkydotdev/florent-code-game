...
class Player:
    ...
    def _seat_seek_target(self, ct):
        ...
        if seekers >= len(free):
            return None
        # c-rev tie-break fix (decode-named defect, jackpot_1_a -25 heals):
        # free[] re-sorts by this unit's own moving position every round, so a
        # walker shuffles between equidistant seats and the abandoned one never
        # backfills.  Two-halves fix: a unit already standing on a seat stops
        # seeking (pre-plank heal-in-place takes over), and a walker holds its
        # chosen seat while that seat stays free.
        p = ct.get_position()
        if (p.x, p.y) in seats:
            return None
    ...
    def _try_counterbattery(self, ct):
        ...
        if ct.read_store(SLOT_HARVESTERS) < ECO_NEED and (
            ...
            if not self._core_shelled(ct):
                return False
        p = ct.get_position()
        # PLANK HS (c-rev): the seat ban is DISARMED at this site only.  The
        # mechanism decode named the b-rev's blocking here as a systematic
        # regression (archipelago-b 8/8): an own turret ON a heal seat is the
        # ring's GARRISON -- banning it hands the contested seat to the enemy
        # (their gunner planted on it r175), while §10 established that seat
        # availability never binds the heal ratio (bodies do).  Harvester,
        # barrier and launcher sites keep their bans: those are measured or
        # rationale-sound; this one was measured harmful.
        ban = None
        choices = (
            (
