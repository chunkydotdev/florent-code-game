...
class Player:
    ...
    def _seat_seek_target(self, ct):
        ...
        if not HS_HEAL_DETAIL_ON or self.core is None:
            return None
        # e-rev: CONSCRIPTION EXEMPTION -- the primary expansion builder is never
        # drafted onto the heal ring.  role_n == 2 is this file's ONE pure
        # expander (the defend-succession block above states it: 1 is the
        # interceptor, 3 turns saboteur by design, so 2 is the only permanent
        # economy hand), and on archipelago seat B it is exactly the unit this
        # plank captured -- builder #8, parked on a seat from r27 at 96.7%
        # residency, killed on seat (20,18) at r177, after which delivery
        # flatlined at 570, no replacement spawns were affordable and the ring
        # emptied to 0.00 bodies for the last 120 rounds (channel ii,
        # docs/research/v85hs-mechanism-read-2026-08-08.md A.1).  The expansion
        # builder funds the endgame economy, and the endgame economy is what pays
        # for the bodies that staff the ring: healing staff must come from
        # surplus hands (role_n >= 5) and the defender, never from the economy's
        # engine.  Returned here, above the cap and above the sticky tie-break,
        # so an exempt unit never stores hs_seek_seat state.
        if self.role_n == 2 and self.role == "expand":
            return None
