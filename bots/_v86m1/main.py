...
# M1 DON'T-FEED-REBUILDS (v86m1).  Measured in the v74 mineguard delta read
# (docs/research/v74-mineguard-delta-read-2026-08-08.md, "Channel B"): an enemy
# mineguard Sentinel has a FIXED facing -- only Gunners can rotate() -- and its
# single-tile-wide ray (r^2 = 32, ignores obstacles) chews through whatever
# unattended economy of ours happens to sit on that line.  26/26 M1-signature
# sentinel hits on buildings landed on OUR harvesters and conveyors, several as
# repeat kills on the SAME tile: one sentinel took our harvester at (12,10),
# then (13,10), then a REBUILT conveyor at (11,10) in one game.  Our line is
# stateless about where it died, so it relays into the beam and pays the 20 Ti
# (and the +5% harvester / +1% conveyor scale) again every few rounds.
#
# The counter is not to fight the turret, it is to stop feeding it: remember
# where our economy buildings died, remember which tiles a visible enemy
# Sentinel aims at, and refuse to rebuild on the intersection.  Deliberately
# conservative -- a death must actually have been recorded on the tile, ray
# coverage alone never deflects anything -- because the ray set is only as good
# as this unit's own vision and a false positive costs real economy.
M1_ON = True
# Rounds a recorded death keeps deterring a harvester rebuild on its own, i.e.
# with no live sentinel ray visible to confirm the killer is still there.  A
# sentinel that is out of our vision is the common case at forward ore, so the
# ledger has to be able to speak without one; 60 rounds is ~15 harvester output
# cycles, long enough that a genuinely cleared site is retried cheaply.
M1_DEATH_COOLDOWN = 60
# Harvester deaths on a tile before deferral arms.  One is enough: harvesters
# are the single most scale-expensive economy building (+5% team-wide each) and
# a beam that killed one kills the replacement on the same schedule.
M1_HARV_DEATHS_MIN = 1
# Sentinel attack radius squared (GameConstants mirrors this).  Used only by the
# fallback ray walk, when the engine's hypothetical-turret getter is unavailable.
M1_SENTINEL_RANGE_SQ = 32
# Ledger caps, per builder instance.  Both dicts are keyed by tile and swept
# once per turn, so they are the only unbounded thing this piece could grow;
# eviction is oldest-insertion-first, which is also least-recently-relevant.
M1_SITE_MAX = 40
M1_DEATH_MAX = 32
...
class Player:
    def __init__(self):
        ...
        # M1 don't-feed-rebuilds ledgers (see M1_ON).  Per builder instance --
        # NOT team-wide: the engine creates one Player per UNIT (docs/game-model
        # .md:27), so "on self" is this builder's own memory, and all sixteen
        # store slots are already spoken for.  That is the right locality
        # anyway: the builder that laid a site is the one working that corner of
        # the map, so it is the one that sees the site die and the one that
        # would relay it.  m1_sites maps a tile we built economy on to
        # (building id, is_harvester); m1_deaths maps a tile to
        # [deaths, last_death_round, harvester_deaths]; m1_rays caches, per
        # enemy Sentinel id, (position key, facing, ray tile keys) -- a Sentinel
        # neither moves nor rotates, so that is computed once per sighting.
        self.m1_sites = {}
        self.m1_deaths = {}
        self.m1_rays = {}
        self.m1_ray_keys = set()
        self.m1_vision_sq = 0
        # Tiles this unit has already re-planned its link chain around, so one
        # dangerous tile can cost at most one extra BFS for the whole match.
        self.m1_replanned = set()
    ...
    def _m1_note_site(self, pos, bid, harvester):
        """Remember an economy building we just placed (see M1_ON).

        Insertion-ordered and capped: the oldest site is dropped first, which is
        also the one furthest behind this builder's working front and therefore
        the one it is least likely to ever see die."""
        if not M1_ON:
            return
        self.m1_sites[(pos.x, pos.y)] = (bid, harvester)
        while len(self.m1_sites) > M1_SITE_MAX:
            self.m1_sites.pop(next(iter(self.m1_sites)))

    def _m1_ray_tiles(self, ct, pos, facing):
        """Tile keys an enemy Sentinel at pos facing `facing` can hit.

        get_attackable_tiles_from is the engine's own hypothetical-turret
        pattern, so it is asked first and is authoritative about the ray's exact
        geometry.  The fallback walks the facing line out to M1_SENTINEL_RANGE_SQ
        by hand -- a Sentinel shot is a single-tile-wide line that ignores
        obstacles, so no occupancy test belongs here either way."""
        try:
            return {(t.x, t.y) for t in ct.get_attackable_tiles_from(pos, facing, EntityType.SENTINEL)}
        except Exception:
            pass
        keys = set()
        try:
        ...
        k = 1
        while True:
            x, y = pos.x + dx * k, pos.y + dy * k
            if (dx * dx + dy * dy) * k * k > M1_SENTINEL_RANGE_SQ:
                break
    ...
    def _m1_note_sentinel(self, ct, sid, ep):
        """Cache the firing ray of a visible enemy Sentinel (see M1_ON).

        Sentinels neither move nor rotate -- rotate() is Gunner-only -- so this
        recomputes only on the first sighting, and the position/facing pair is
        stored purely so a stale cache from a launcher-shuffled world self-heals.
        Gunners are deliberately excluded: they CAN rotate, so their current
        facing is not a stable enough signal to refuse economy over."""
        old = self.m1_rays.get(sid)
        key = (ep.x, ep.y)
        try:
            facing = ct.get_direction(sid)
        except Exception:
            return
        if old is not None and old[0] == key and old[1] == facing:
            return
        self.m1_rays[sid] = (key, facing, self._m1_ray_tiles(ct, ep, facing))
        self._m1_rebuild_rays()

    def _m1_rebuild_rays(self):
        keys = set()
        for rec in self.m1_rays.values():
            keys |= rec[2]
        self.m1_ray_keys = keys

    def _m1_observe(self, ct):
        """Once-per-turn sweep of both ledgers (see M1_ON).

        O(recorded sites + cached sentinels), both hard-capped, and the engine
        is only asked about entries whose tile is inside this unit's own vision
        -- that test is plain arithmetic against the cached vision radius, so
        the common case costs no engine call at all."""
        if not M1_ON:
            return
        p = ct.get_position()
        if self.m1_vision_sq <= 0:
            try:
                self.m1_vision_sq = ct.get_vision_radius_sq()
            except Exception:
                self.m1_vision_sq = 20
        vs = self.m1_vision_sq
        rnd = ct.get_current_round()
        dead = None
        for key, rec in self.m1_sites.items():
            dx, dy = key[0] - p.x, key[1] - p.y
            if dx * dx + dy * dy > vs:
                continue
            try:
                ...
                continue
            # Building ids are unique for the match, so "the tile no longer
            # holds the id we put there" is exactly one thing: ours died.
            if bid == rec[0]:
                continue
            if dead is None:
                dead = []
        ...
        if dead:
            for key in dead:
                rec = self.m1_sites.pop(key)
                d = self.m1_deaths.get(key)
                if d is None:
                    d = [0, 0, 0]
                    self.m1_deaths[key] = d
                d[0] += 1
                d[1] = rnd
                if rec[1]:
                    d[2] += 1
            while len(self.m1_deaths) > M1_DEATH_MAX:
                self.m1_deaths.pop(next(iter(self.m1_deaths)))
        gone = None
        for sid, rec in self.m1_rays.items():
            dx, dy = rec[0][0] - p.x, rec[0][1] - p.y
            if dx * dx + dy * dy > vs:
                continue
            try:
                bid = ct.get_tile_building_id(Position(rec[0][0], rec[0][1]))
            except Exception:
                continue
            if bid == sid:
                continue
        ...
        if gone:
            for sid in gone:
                self.m1_rays.pop(sid, None)
            self._m1_rebuild_rays()

    def _m1_harv_deterred(self, ct, pos):
        """Defer a harvester here?  A harvester can only stand on ore, so there
        is no site-shift available and deferral IS the counter -- it lapses on
        its own once the beam owner dies or the cooldown runs out."""
        if not M1_ON:
            return False
        key = (pos.x, pos.y)
        d = self.m1_deaths.get(key)
        if d is None or d[2] < M1_HARV_DEATHS_MIN:
            return False
        return key in self.m1_ray_keys or ct.get_current_round() - d[1] < M1_DEATH_COOLDOWN

    def _m1_link_avoid(self):
        """Tiles the link planner should route around: a live Sentinel ray AND a
        death actually recorded there.  Ray coverage alone never deflects a
        chain -- connectivity beats avoidance, and the ray set is only as good
        as one builder's vision."""
        if not M1_ON or not self.m1_ray_keys or not self.m1_deaths:
            return None
        keys = {k for k in self.m1_deaths if k in self.m1_ray_keys}
        return keys or None
    ...
    def _builder(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            # M1 (see M1_ON): piggyback the hostile-ray census on the scan that
            # is already running, so it costs one type comparison per sighting.
            if M1_ON and et == EntityType.SENTINEL:
                self._m1_note_sentinel(ct, eid, ep)
    ...
    def _try_harvester(self, ct, harv):
        ...
        for d in DIRECTIONS:
            ...
            if ban is not None and (bp.x, bp.y) in ban:
                continue
            # M1 don't-feed-rebuilds (see M1_ON).
            if self._m1_harv_deterred(ct, bp):
                continue
            if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh and ct.can_build_harvester(bp):
                hid = ct.build_harvester(bp)
                self._m1_note_site(bp, hid, True)
            ...
            if (
                ...
                for d in DIRECTIONS:
                    ...
                    if seat_ban is not None and (bp.x, bp.y) in seat_ban:
                        continue
                    # M1 don't-feed-rebuilds (see M1_ON), same rule as
                    # _try_harvester's -- the two harvester sites stay in step.
                    if self._m1_harv_deterred(ct, bp):
                        continue
                    if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh and ct.can_build_harvester(bp):
                        hid = ct.build_harvester(bp)
    ...
    def _link_path(self, ct, hpos, avoid=None):
        # `avoid` is M1's don't-feed-rebuilds set (see M1_ON): extra blocked
        # ground for this one search only.  Routing it through the existing
        # planner rather than nudging a tile by hand is what keeps the detour's
        # connectivity and conveyor facings correct by construction; if the
        # avoidance disconnects the harvester the search simply returns [] and
        # the caller keeps its original chain.
        raw_goals = set()
        for c in core_tiles(self.core):
            for d in CARDINALS:
                t = c.add(d)
        ...
        # pave gate.
        goals = raw_goals - avoid if avoid else raw_goals
        prev = {start: None}
        q = deque([start])
        found = None
    ...
    def _build_next_link(self, ct):
        ...
        if ban is not None and (tile.x, tile.y) in ban:
            self.link_queue = []
            return False
        # M1 CONVEYOR SITE-SHIFT (see M1_ON).  This is the rebuild loop the
        # measured exemplar feeds: the beam clears a link tile, the queue still
        # holds it, and we relay onto the same square for another 3 Ti and
        # another +1% of team-wide scale.  Only a tile that has ACTUALLY buried
        # a building of ours AND sits on a live Sentinel ray is re-planned
        # around, at most once per tile per unit -- and if the planner cannot
        # find another way home the original chain stands, because a severed
        # chain delivers nothing at all while a shot-at one still delivers
        # between kills.
        m1_key = (tile.x, tile.y)
        if (
            M1_ON and m1_key not in self.m1_replanned
            and m1_key in self.m1_ray_keys and m1_key in self.m1_deaths
            and self.link_source is not None
        ):
            self.m1_replanned.add(m1_key)
            alt = self._link_path(ct, self.link_source, avoid=self._m1_link_avoid())
            if alt:
                self.link_queue = alt
                return False
        ...
        if ct.can_build_conveyor(tile, f):
            cid = ct.build_conveyor(tile, f)
            self._m1_note_site(tile, cid, False)
            self.link_queue.pop(0)
            return True
        return False
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 and self._eco_spendable(ct, ct.get_conveyor_cost()):
                    if dist_core(pp, self.core) > 0:
                        ...
                        if coreward_ok and facing is not None and ct.can_build_conveyor(pp, facing):
                            # M1: trail conveyors go in the death ledger too --
                            # the placement itself is untouched (an opportunistic
                            # pave has no alternative site to shift to), but a
                            # tile that eats one is a tile the LINKER must learn
                            # to route around.
                            self._m1_note_site(pp, ct.build_conveyor(pp, facing), False)
                ...
                if dist_core(nxt, self.core) > 0:
                    ...
                    if abs(nxt.x - self.core.x) + abs(nxt.y - self.core.y) < abs(here.x - self.core.x) + abs(here.y - self.core.y):
                        ...
                        if ct.can_build_conveyor(nxt, card):
                            # M1: same ledger entry as the trail site above.
                            self._m1_note_site(nxt, ct.build_conveyor(nxt, card), False)
        if ct.can_move(d):
            ct.move(d)
            if PAVE_TRAIL_ON:
                self.pave_prev = p0
