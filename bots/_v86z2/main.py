...
M1 variant: MINE-GUARD SENTINELS.  kladde forensics (2026-08-08): kladde
loses 0-1 harvesters per game to our ~10 (6,000+ Ti per game in rebuilds and
lost income) by standing sentinels OVER their own mines -- 34-48% of income
at t200 on 13-16 sentinels plus ammo -- while our turret construction is
exclusively threat-reactive (counterbattery) or forward (siege).  The
role_n == 4 defender gains a quiet-map branch: walk to the nearest visible
friendly harvester with no friendly sentinel within dsq 32 and stand a
sentinel beside it, capped at MINEGUARD_CAP.  Everything else is bit-for-bit
Y1_chainwatch.

M2 variant: SLINGSHOT NESTS ON THE M1 CHASSIS.  L3_slingshot's OFFENSE --
the raider/ferry/nest machine (launcher catapult line, forward gunner nests
rebuilt persistently at dsq 4-13 of the enemy Core, mine plugging, heal-war
retarget, dry-fire ammo floor, adjacent-raider nest healing) -- grafted in
place of the v74 saboteur/launchwait offense.  Everything defensive and
economic stays v74: economy, mine-guard sentinels, interceptor, choke walls,
counterbattery, home defense, chain watchdog, endgame switch.  Seat map:
role_n 0 = raider #1 (owns launcher L1 + the primary nest), role_n 3 =
raider #2 (owns L2, promoted on the same eco gate v74 promoted its second
saboteur on), role_n >= 5 replacements alternate raider/expander so the
ferry keeps running without gutting the siege heal line.  Store slot 5
(ECO_READY, write-only in every ancestor) is reclaimed as SLOT_ENEMY_HP;
slot 13 stays DEFEND_BEAT and slot 6 becomes the L3 launcher COUNT (0/1/2).
"""
import math
import random
...
# S1 SPEED.  Every helper on the engine's Direction enum builds a fresh
# 9-entry dict literal on EVERY call (fcode/_types.py:133-145), and
# Position.add() calls delta() -- so one `.add()` measures 1.675 us on this
# machine.  A full-map BFS makes ~2,400 of them (600 cells x 4 dirs) = 4.0 ms,
# and a turn that runs two BFS (a raider replanning its siege, then
# navigating) spends ~8 ms of its 10 ms budget rebuilding the same dict 4,800
# times.  That is the whole story of our measured 9,312 us p99 against a
# 10,000 us TLE while every top-4 team runs 800-2,000 us.
#
# Snapshot the tables once here: `_DELTA[d]` measures 0.104 us, 12x faster,
# and the tuple form below is 7.4x faster than `.add()` end to end.
# Position is a NamedTuple, so `(x + dx, y + dy)` compares equal to and hashes
# identically to `Position(x + dx, y + dy)`; every BFS below keys sets and
# dicts on exactly the same values it did before.  This is a pure speed
# change -- no decision this bot makes can differ.  (Locally that is directly
# verifiable: get_cpu_time_elapsed() reads 0 under `fcode run`, so the CPU
# guard never trips and replays must be identical.  On ladder hardware the
# guard trips LESS often, which only ever replaces a degraded fallback step
# with the real pathfinding result.)
_DELTA = {d: d.delta() for d in Direction}
_CARD_DELTA = [_DELTA[d] for d in CARDINALS]
...
REPLACE_MIN_RND = 60
# === SLINGSHOT DOCTRINE CONSTANTS (M2 graft, from L3_slingshot) ===
# Launcher ferry line: ideal distances (in Chebyshev steps along the
# core-to-core axis) of launcher L1 and L2 from OUR core anchor.  L1 sits
# within reach of the spawn yard (built by r8-14), L2 ~5 tiles further
# (built by r20-30).  Sites snap to the nearest legal tile at plan time.
LAUNCHER_L1_STEPS = 4
LAUNCHER_L2_STEPS = 9
# Give-up round for launcher duty: past this a raider whose site never
# became buildable stops trying and joins the siege on foot.
LAUNCHER_GIVEUP_RND = 60
# A raider farther than this (dsq to the nearest enemy-Core tile) is IN
# TRANSIT and rides the ferry; at or below it, it is in the nest zone and
# plans/heals/rebuilds gunner nests instead.  36 ~= 6 tiles, just past the
# gunner siege ring.
NEST_ZONE_DSQ = 36
# Live forward guns (visible friendly GUNNERs within NEST_GUN_DSQ of the
# enemy Core) a single raider will sustain.  Counting LIVE guns rather than
# builds-ever is the rebuild mechanism: a dead nest re-arms the planner.
NEST_CAP = 3
NEST_GUN_DSQ = 20
# Third-and-later nests wait for a working economy (SLOT_HARVESTERS).
NEST_ECO_FREE = 2
# Mine plugging: a 3 Ti barrier on an empty ore tile in the ENEMY half
# deletes an income stream.  Round floor keeps the opening bank for the
# eco/launcher bootstrap; the Ti floor keeps the medic's heal money.
PLUG_MIN_RND = 25
PLUG_TI_RESERVE = 12
# Ferry hold stall: a raider standing in the pickup ring holding the claim
# waits at most this many rounds for its throw.  A launcher that CAN throw
# does so within a round or two (cooldown 1); a longer hold means every
# legal advance site is walled/occupied, and waiting further is a deadlock
# -- measured on saga (28.5% walls): one raider held an unthrowable claim
# for ~200 rounds while the nest clock ran.  After the ban the raider
# simply walks; the ferry is an accelerant, never a dependency.
FERRY_HOLD_MAX = 6
FERRY_BAN_RNDS = 30
# Heal-war rule: enemy Core HP unchanged-or-rising for this many rounds
# while our turrets are on it flips their priority to the adjacent enemy
# builders (the medics), then back once HP drops again.
HEALWAR_STALL_RNDS = 15
# Ammo engine additions (merged with the v74 magazine below, max of the two
# demands): prime at r0; with guns standing never let the pool dry-fire.
AMMO_PRIME = 5
AMMO_BANK = 46
AMMO_STEP = 8
...
# M2 STORE SLOT MAP (16 slots, writes land NEXT round):
#  0-4  unchanged from v74 (ROLE_N, UNDER, ATK_RND, ENEMY_CORE, HARVESTERS).
#  5    ENEMY_HP -- RECLAIMED from ECO_READY, which was written in four
#       places and read in ZERO across every ancestor of both forks
#       (verified by grep before the graft).  Packs
#       ((last_drop_rnd + 1) << 9) | enemy Core HP (HP <= 500 < 512).
#       Written by any unit that SEES the enemy Core; drives the heal-war
#       flip and the ammo engine's race logic.  Slot 13 could NOT be used:
#       v74's defend succession heartbeat lives there and is kept.
#  6    LAUNCHER -- REPURPOSED per L3: COUNT of ferry launchers BUILT
#       (0/1/2), written by the raider that builds each; a live launcher
#       unit backfills a 1 if it runs with the slot still 0.  v74's home
#       launcher claim protocol (write-1-then-maybe-0) is deleted with the
#       subsystem that owned it, so no writer ever decrements the count.
#  7-12 unchanged (HOME_GUN, DROPPED, LINKS_DONE, LAUNCH_ID, LAUNCH_RND,
#       LAUNCHED_ID -- the ferry claim/ack protocol is identical in both
#       forks; M2 additionally consumes the ack like L3 so repeat throws
#       each reset walking state exactly once).
# 13    DEFEND_BEAT unchanged (v74 defend succession -- kept).
# 14-15 unchanged (THREAT; SIEGE = primary nest tile, owner is the
#       role_n == 0 raider, same seat number v74's siege engineer used).
SLOT_ROLE_N = 0
SLOT_UNDER = 1
SLOT_ATK_RND = 2
SLOT_ENEMY_CORE = 3
...
# DOCTRINE M1 -- MINE-GUARD SENTINELS.  kladde forensics (2026-08-08): kladde
# loses 0-1 harvesters per game where we lose ~10 -- worth 6,000+ Ti per game
# in rebuild price and lost income -- and the mechanism is visible in the
# replays: sentinels positioned OVER their own mines (34-48% of income at t200
# sunk into 13-16 sentinels plus ammo).  Every turret this file builds today
# is either threat-reactive (counterbattery) or forward (siege); nothing
# stands guard over the economy before the raider arrives.  This branch gives
# the role_n == 4 defender a proactive third mode: on a quiet map, once the
# four-harvester shell exists, walk to the nearest visible friendly harvester
# with no friendly sentinel within dsq 32 (a sentinel's own reach -- inside it
# the mine is already covered) and stand a sentinel beside it.  Capped low:
# two guards is insurance priced at ~2 sentinels + ammo headroom, thirteen is
# kladde's whole doctrine and unmeasured on our economy curve.
MINEGUARD_CAP = 2
MINEGUARD_MIN_HARV = 4
MINEGUARD_MIN_RND = 80
...
def nearest_cardinal(d):
    # Same table as before, built once at import instead of per call.
    return _NEAREST_CARDINAL[d]
...
def dist_core(pos, o):
    # Chebyshev distance to the nearest of the Core's 2x2 tiles.  Was a
    # generator over core_tiles(o), which allocated a 4-element list of
    # Position objects on every call; this is the same four maxima computed
    # inline.  dist_core is called from inside BFS goal construction, so it
    # runs hundreds of times per turn.
    ex = pos.x - o.x
    ey = pos.y - o.y
    ax, ay = abs(ex), abs(ey)
    bx, by = abs(ex - 1), abs(ey - 1)
    return min(max(ax, ay), max(bx, ay), max(ax, by), max(bx, by))
...
class Player:
    def __init__(self):
        ...
        # SLINGSHOT ferry/launcher state (M2 graft), per unit instance (all
        # 16 store slots are allocated; only this unit reads or writes
        # these): launcher_no is which ferry launcher this raider owns (1,
        # 2, or 0 for none), launcher_done latches when it is built / given
        # up, launcher_site caches the snapped axis site, ferrying marks a
        # raider currently riding/claiming the ferry line.
        self.launcher_no = 0
        self.launcher_done = False
        self.launcher_site = None
        self.ferrying = False
        self.ferry_hold = 0
        self.ferry_ban_until = 0
        # DOCTRINE M1 mine-guard count (see MINEGUARD_CAP).  Per unit
        # instance, same idiom as forward_guns: all 16 store slots are taken,
        # and only the single role_n == 4 defender (plus its role_n == 2
        # successor, which starts a fresh interpreter and thus a fresh count)
        # ever builds these, so a per-unit counter is the true global count in
        # any game where the defender survives.
        self.mine_guards = 0
        ...
        # V1 FACINGFIX: set of (x, y) tiles this builder has already
        # destroyed and rebuilt with the correct facing.  Bounds worst-case
        # to at most one destroy per tile even if the facing oracle is wrong
        # somewhere (oscillation safety).
        self.repaired_tiles = set()
    ...
    def _core(self, ct):
        ...
        # SLINGSHOT AMMO MERGE (M2).  Two additions from L3's engine, on top
        # of -- never instead of -- the v74 magazine below (the graft spec:
        # take the max of the two demands).
        #   PRIME: convert AMMO_PRIME at r0 so the first nest never waits.
        #   DRY-FIRE PREVENTION: a gunner with 0 ammo deals zero damage, and
        #   the measured L3 failure (saga vs sporks3_g3) was the floors --
        #   eco churn pinned the bank AT the under-siege floor, `ti > floor`
        #   never opened, and two standing nests spent 500 rounds at 0-8
        #   ammo shelling nothing.  With guns standing, ammo below the
        #   warfighting minimum (4 per gun-shot; 16 covers two nests' next
        #   volleys) converts down to a 12 Ti heal reserve, every round,
        #   ahead of every v74 floor.
        # RACE LOGIC: SLOT_ENEMY_HP is written by any unit that sees the
        # enemy Core.  While their Core is LOWER than ours we are winning
        # the race, so the magazine keeps pouring at the under-siege floor
        # even while our own Core takes fire -- never recall, never blink.
        weapons = ct.read_store(SLOT_HOME_GUN)
        ev = ct.read_store(SLOT_ENEMY_HP)
        race_lead = bool(ev) and (ev & 511) < hp
        if rnd == 0:
            if ti > AMMO_PRIME and ct.can_convert_ammo(AMMO_PRIME):
                ct.convert_ammo(AMMO_PRIME)
                ti = ct.get_global_resources()
                ammo = ct.get_global_ammo()
        elif not endgame_dumped and weapons and ammo < 16:
            amt = min(AMMO_STEP, 16 - ammo, ti - 12)
        ...
        if weapons:
            ...
            # L3 bank demand, merged as the max of the two (M2): with any
            # gun standing the nests drink ~4-6/turn, so the target never
            # sits below AMMO_BANK.  The peacetime ti_floor of 46 below is
            # untouched, so this only ever spends bank above one scaled
            # harvester rebuild -- the eco keeps its buffer.
            ammo_target = max(ammo_target, min(72, max(AMMO_BANK, 6 * weapons)))
        ...
        #
        # W4: THE 46 HAS SILENTLY DECAYED.  The comment above says it means
        # "one scaled harvester + a couple of conveyor links" and prices a
        # harvester at ~23 Ti.  That was true at the cost scale of the games it
        # was tuned on.  Reconstructed from living entities across 20 real
        # games vs the top four, our scale is 241% at t25, 256% at t100 and
        # 261% at t150 -- a harvester costs ~51 Ti, so 46 is now 90% of ONE
        # harvester with nothing left for the links.  Worse, AMMO_BANK is also
        # 46, so line 1105 holds ammo_target >= 46 whenever a gun stands: the
        # Core converts until the bank sits AT 46 while the harvester it is
        # reserving for costs 51.  The bot converges to a bank permanently a
        # few Ti SHORT of the only thing that grows it.  Measured: median bank
        # 19 Ti, harvester affordable on 15% of turns; the top four run 74 Ti
        # and 50%.  The intent was always right -- it was denominated in the
        # wrong unit.  Ask the engine for the price instead of remembering it.
        # `under` is untouched (12), so a rush still gets its full magazine;
        # this only stops PEACETIME milking, which is where the 522-1,384 Ti
        # of wasted conversion in the forensics above was actually burned.
        eco_step = ct.get_harvester_cost() + 4 * ct.get_conveyor_cost()
        ti_floor = 12 if (under or race_lead) else (eco_step if weapons else 52)
    ...
    def _track_enemy_core(self, ct):
        """HEAL-WAR / RACE bookkeeping: publish enemy Core HP to SLOT_ENEMY_HP.

        Called by raiders and by turrets -- anything that can actually SEE
        the enemy Core (get_nearby_buildings only returns visible entities,
        so the scan never raises; the HP read is guarded anyway).  The slot
        packs ((last_drop_round + 1) << 9) | hp: the round half only
        advances when HP is observed to DROP, so `now - last_drop` measures
        how long the enemy's medics have been out-healing our fire.
        """
        ...
        for bid in ct.get_nearby_buildings():
            try:
                ...
                continue
            ehp = max(0, min(511, ehp))
            prev = ct.read_store(SLOT_ENEMY_HP)
            rnd = ct.get_current_round()
            if prev == 0 or ehp < (prev & 511):
                ct.write_store(SLOT_ENEMY_HP, ((rnd + 1) << 9) | ehp)
            elif ehp != (prev & 511):
                # Healed back up: record the new HP but keep the old
                # last-drop round, so the stall clock keeps counting.
                ct.write_store(SLOT_ENEMY_HP, (prev & ~511) | ehp)
            return
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            if n == 0:
                # M2: the v74 siege engineer seat becomes SLINGSHOT raider #1
                # -- owns ferry launcher L1 and the primary nest (SLOT_SIEGE,
                # same seat-0 ownership v74's engineer had).
                self.role = "raider"
                self.launcher_no = 1
            elif n <= 3:
                self.role = "expand"
                ...
                # v74 made every one a generic expander; L3 made every one a
                # ferry raider.  M2 alternates: odd seats reinforce the nests
                # (the persistence mechanism needs bodies arriving all game),
                # even seats stay home so the siege-respawn heal line and the
                # measured harvester shortfall both keep their hands.  The
                # defend seat (role_n == 4, with role_n == 2 succession) and
                # the interceptor (role_n == 1) stay single-occupancy.
                if n % 2 == 1:
                    self.role = "raider"
                    # A reinforcement raider rebuilds a ferry launcher that
                    # was never built (both original owners dead before their
                    # site went down).  SLOT_LAUNCHER counts builds-ever, so
                    # a DESTROYED launcher is not rebuilt -- accepted: the
                    # throw line is an accelerant, not a dependency.
                    built = ct.read_store(SLOT_LAUNCHER)
                    if built < 2:
                        self.launcher_no = built + 1
                else:
                    self.role = "expand"
                ...
                if len(self.map_walls) >= ORE_STEPOFF_MIN_WALLS \
                    ...
                    while ck_q:
                        ...
                        if cur in ck_goals:
                            ck_hit = cur
                            break
                        for dx, dy in _CARD_DELTA:
                            nx, ny = cur[0] + dx, cur[1] + dy
                            key = (nx, ny)
                            if (
                                key in ck_parent or key in ck_blocked
                                or not (0 <= nx < self.mw and 0 <= ny < self.mh)
                            ):
                                continue
                            ck_parent[key] = cur
                            ck_q.append(key)
        ...
        # FERRY ACK (L3 semantics).  The Launcher acknowledges the exact bot
        # it threw.  A thrown raider stays a raider -- it simply finds itself
        # 5-6 tiles further down the line (possibly inside the next
        # launcher's pickup ring, where it claims again).  Position jumped,
        # so walking state and the pave-trail memory are both stale and
        # cleared.
        if ct.read_store(SLOT_LAUNCHED_ID) == ct.get_id() + 1:
            ...
            if PAVE_TRAIL_ON:
                ...
                self.pave_dir = None
                self.pave_rnd = -2
            # Consume the ack (buffered; lands next round) so a single throw
            # does not keep resetting this unit's walking state forever --
            # under the ferry a raider is thrown MANY times per game and
            # each ack must be processed exactly once.
            ct.write_store(SLOT_LAUNCHED_ID, 0)
        ...
        if self.role == "expand" and snowflake_attack_now:
            # M2: the early snowflake attacker is a raider now, same seat.
            self.role = "raider"
            self.launcher_no = 2
        # The fourth macro engineer becomes raider #2 once the initial
        # four-harvester shell exists (v74's own promotion gate, kept).  It
        # owns ferry launcher L2 -- the L3 seat-3 duty -- and walks/ferries
        # forward for the rest of the match.  Two others continue economy.
        if (
        ...
        # M2: the v74 saboteur melee recall is deleted with the subsystem it
        # belonged to.  Raiders NEVER recall (L3 spec 8: the nest persistence
        # IS the mechanism; the ferry line only runs forward).  Home defense
        # against melee visitors keeps every v74 hand it already had: the
        # universal heal above, the role_n == 4 defender's threat chase, the
        # role_n == 1 interceptor, and the counterbattery machinery.
    ...
    def _sabotage_prio(self, ct):
        ...
        for d in CARDINALS:
            ...
            if self.melee_futile.get(bid, (0, None, 0))[2] > rnd:
                continue
            et = ct.get_entity_type(bid)
            # CORE-GRIND GATE (M2, from L3).  A 2 dmg / 2 Ti peck into a
            # medic-defended Core is a losing exchange on its own (their
            # heal is +4 HP / 1 Ti): measured on saga, two raiders sank
            # ~800 Ti of pecks r125-325 into a Core their medics held at
            # ~400 HP, and that was precisely the money the first nests
            # never got.  Pecks on the Core are allowed only as a
            # SUPPLEMENT (two or more guns already standing -- the grind
            # then out-paces the heal) or as a FINISHER (Core <= 150 HP).
            if et == EntityType.CORE:
                _ev = ct.read_store(SLOT_ENEMY_HP)
                if ct.read_store(SLOT_HOME_GUN) < 2 and not (
                    _ev and (_ev & 511) <= 150
                ):
                    continue
            pr = {
    ...
    def _live_nests(self, ct):
        """Visible friendly GUNNERs within NEST_GUN_DSQ of the enemy Core.

        Counting LIVE guns instead of builds-ever IS the rebuild mechanism
        (L3 spec 3): a nest the defenders clear drops out of this count the
        moment its tile is rubble, so the planner re-arms and the raider
        stands the next gun on the same or an adjacent tile -- Landers
        rebuilt one tile six times, and the persistence is what ends a 500
        HP Core.  Vision-limited by construction (get_nearby_buildings), so
        a raider far from the nest zone undercounts; the planner is only
        consulted inside the zone, where the count is honest.
        """
        if self.enemy is None:
            return 0
        n = 0
        for bid in ct.get_nearby_buildings():
            try:
                ...
                continue
            if min(t.distance_squared(bp) for t in core_tiles(self.enemy)) <= NEST_GUN_DSQ:
                n += 1
        return n
    ...
    def _plan_siege(self, ct):
        ...
        if self.map_grid is None or self.enemy is None:
            return False
        live = self._live_nests(ct)
        if live >= NEST_CAP:
            return False
        # Third-and-later nests wait for a working economy; the first two go
        # down on the opening bank (Flotte's whole doctrine fit in 2 nests).
        if live >= NEST_ECO_FREE and ct.read_store(SLOT_HARVESTERS) < ECO_NEED:
            return False
        ...
        # GUNNER NESTS ONLY (L3 spec 3): 20 Ti, 7 dmg on cooldown 1 against
        # the sentinel's 30 Ti / 18 dmg on cooldown 2 -- similar sustained
        # DPS, but the gunner is a third cheaper to rebuild, and rebuilds
        # are the doctrine.  Ray lengths 3 cardinal / 2 diagonal put the
        # nest at dsq 4-9 from the Core tile it shells.
    ...
    def _launcher_axis_site(self, ct):
        """Snapped build site for this raider's ferry launcher (L1 or L2).

        The ideal point sits LAUNCHER_L{1,2}_STEPS Chebyshev steps down the
        core-to-core axis from OUR anchor.  On decoded maps it snaps to the
        nearest legal tile (empty terrain, not ore, not a Core yard seat)
        within a radius-2 box; on unknown maps the ideal point is returned
        as-is and can_build_launcher arbitrates at build time.  Returns None
        when no legal tile exists -- the duty then gives up cleanly.
        """
        if self.launcher_site is not None:
            return self.launcher_site
        if self.core is None:
            return None
        e = self.enemy or enemy_core_for(self.mw, self.mh, self.core)
        steps = LAUNCHER_L1_STEPS if self.launcher_no == 1 else LAUNCHER_L2_STEPS
        dx, dy = e.x - self.core.x, e.y - self.core.y
        span = max(abs(dx), abs(dy), 1)
        ideal = Position(
            max(0, min(self.core.x + int(round(dx * steps / span)), self.mw - 1)),
            max(0, min(self.core.y + int(round(dy * steps / span)), self.mh - 1)),
        )
        if self.map_grid is None:
            self.launcher_site = ideal
            return ideal
        # Our Core's yard seats (footprint + cardinal neighbors) are conveyor
        # delivery / heal seats -- never stand the catapult on one.
        no = set()
        for c in core_tiles(self.core):
            no.add((c.x, c.y))
            for d in CARDINALS:
                ...
                no.add((nn.x, nn.y))
        offsets = sorted(
            ((ox, oy) for ox in range(-2, 3) for oy in range(-2, 3)),
            key=lambda o: (abs(o[0]) + abs(o[1]), o),
        )
        for ox, oy in offsets:
            t = Position(ideal.x + ox, ideal.y + oy)
            if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
                continue
            if self.map_grid[t.y][t.x] != ".":
                continue
            if (t.x, t.y) in no:
                continue
            self.launcher_site = t
            return t
        return None

    def _try_build_ferry_launcher(self, ct):
        """Action half of launcher duty.  True == the action was spent.

        Builds on any tile within Chebyshev 1 of the snapped site (exact
        tile preferred by the walk, but a neighbor is just as good a
        catapult).  Latches launcher_done on success, on seeing a friendly
        launcher already standing there, on the give-up round, and when the
        store says both launchers exist.
        """
        if not self.launcher_no or self.launcher_done:
            return False
        rnd = ct.get_current_round()
        if rnd > LAUNCHER_GIVEUP_RND or ct.read_store(SLOT_LAUNCHER) >= 2:
            self.launcher_done = True
            return False
        site = self._launcher_axis_site(ct)
        if site is None:
            self.launcher_done = True
            return False
        p = ct.get_position()
        if max(abs(p.x - site.x), abs(p.y - site.y)) > 2:
            return False
        if ct.is_in_vision(site):
            bid = None
            try:
            ...
            if bid is not None:
                try:
                    if (
                        ...
                        self.launcher_done = True
                        ct.write_store(SLOT_LAUNCHER, max(
                            ct.read_store(SLOT_LAUNCHER), self.launcher_no,
                        ))
                        return False
                except Exception:
                    pass
        ...
        for d in CARDINALS:
            ...
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            if max(abs(bp.x - site.x), abs(bp.y - site.y)) > 1:
                continue
    ...
    def _ferry_launcher(self, ct, ec):
        """Nearest visible friendly Launcher that is not BEHIND us.

        A throw lands ~5 tiles past the launcher toward the enemy Core, so
        riding pays whenever the launcher is at most slightly behind our own
        Manhattan progress line.  Builder vision is r^2=20, so anything this
        returns is already at or near the sqrt(26) pickup ring.
        """
        p = ct.get_position()
        my_d = abs(p.x - ec.x) + abs(p.y - ec.y)
        best = None
        best_d = None
        for bid in ct.get_nearby_buildings():
            try:
                ...
                continue
            if abs(lp.x - ec.x) + abs(lp.y - ec.y) > my_d + 2:
                continue
            d = p.distance_squared(lp)
            if best_d is None or d < best_d:
                best, best_d = lp, d
        return best

    def _heal_battle_adjacent(self, ct):
        """Repair an adjacent friendly COMBAT structure (nest gunner, ferry
        launcher, plug barrier).  1 Ti for +4 HP out-paces the 2 dmg builder
        pecks that clear most nests -- adjacent-raider repair is half of the
        nest persistence mechanism (the rebuild count is the other half).
        Economy buildings are deliberately excluded: patching conveyors is
        the eco hands' chain-medic job, not a raider's.
        """
        p = ct.get_position()
        for d in CARDINALS:
            bp = p.add(d)
            ...
            try:
                ...
                if (
                    ...
                    and ct.get_entity_type(bid) in (
                        EntityType.GUNNER, EntityType.LAUNCHER, EntityType.BARRIER,
                    )
                    and ct.can_heal(bp)
                ):
                    ct.heal(bp)
    ...
    def _try_mine_plug(self, ct):
        """MINE PLUGGING (L3 spec 4): 3 Ti barrier on an empty ore tile in
        the ENEMY half.  The usual sequence is melee kills their harvester
        (the _sabotage_prio priority already ranks harvesters right after
        guns and Core), the tile is bare ore next round, and this plugs it
        before they rebuild -- 3 Ti deletes a 2.5 Ti/round income stream.
        The half test keeps our own mines safe; the reserve keeps heal/peck
        money.
        """
        if self.enemy is None or self.core is None:
            return False
        if ct.get_current_round() < PLUG_MIN_RND:
            return False
        if ct.get_global_resources() < ct.get_barrier_cost() + PLUG_TI_RESERVE:
            return False
        p = ct.get_position()
        for d in CARDINALS:
            bp = p.add(d)
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            if bp.distance_squared(self.enemy) >= bp.distance_squared(self.core):
                continue
            try:
    ...
    def _raider(self, ct):
        """SLINGSHOT raider: ferry forward, stand the catapults, nest at the
        enemy Core, plug their mines, and never come home (L3 spec 2/3/4/8).

        Life cycle: launcher duty (role_n 0/3 stand L1/L2 on the axis on the
        way out) -> transit (march at the enemy Core; any friendly launcher
        ahead throws us 5-6 tiles per hop via the ferry claim) -> nest zone
        (plan/build/rebuild gunners whose ray covers the Core, heal them,
        melee-grind, plug mines).  Reinforcement raiders repeat it all game.
        """
        p = ct.get_position()
        rnd = ct.get_current_round()
        ec = self.enemy or Position(self.mw // 2, self.mh // 2)
        self._track_enemy_core(ct)
        if self.enemy is not None:
            near_d = min(p.distance_squared(t) for t in core_tiles(self.enemy))
        else:
            near_d = p.distance_squared(ec)
        in_nest_zone = self.enemy is not None and near_d <= NEST_ZONE_DSQ

        if ct.get_action_cooldown() == 0:
            if self._try_build_ferry_launcher(ct):
                return
            # NEST PERSISTENCE: the primary nest first, then any adjacent
            # combat structure.  can_heal enforces adjacency and real damage.
            ...
            if can_repair:
                ct.heal(primary)
                return
            if self._heal_battle_adjacent(ct):
                return
            if in_nest_zone and self._try_siege_build(ct):
                return
            if self._sabotage_prio(ct):
                return
            if self._try_mine_plug(ct):
                return

        # Action phase left nothing half-set (every branch above returns
        # right after its engine action).  Check before the BFS-heavy
        # planning and nav below.
        if self._cpu_exhausted(ct):
            return

        if ct.get_move_cooldown() != 0:
            # Keep the ferry claim heartbeat alive on no-move rounds, or the
            # 4-round expiry hands the slot away mid-pickup.
            if self.ferrying:
                self._offer_launch(ct)
            return

        # LAUNCHER DUTY WALK.  Same gates as the action half, checked live
        # so a duty that became moot mid-walk releases the unit.
        if (
            self.launcher_no and not self.launcher_done
            and rnd <= LAUNCHER_GIVEUP_RND and ct.read_store(SLOT_LAUNCHER) < 2
        ):
            site = self._launcher_axis_site(ct)
            if site is not None:
                if p.x == site.x and p.y == site.y:
                    # Builds are adjacent-only, never own-tile: step off.
                    for d in CARDINALS:
                        n = p.add(d)
                        if (
                            0 <= n.x < self.mw and 0 <= n.y < self.mh
                            ...
                            return
                    return
                if max(abs(p.x - site.x), abs(p.y - site.y)) <= 1:
                    return  # in build range; hold for the action phase
                self.tgt = site
                self._nav(ct, pave=False)
                return
        ...
        if in_nest_zone:
            ...
            if self.siege_approach is not None:
                if self.stuck >= 3:
                    self.siege_spot = self.siege_approach = None
                    self.siege_direction = self.siege_type = None
                    self._plan_siege(ct)
                self.tgt = self.siege_approach or ec
            else:
                # No plannable nest right now (cap reached or no candidate):
                # orbit the Core for melee/plug/heal work.  _bfs_direction
                # treats the enemy footprint as blocked and aims at its
                # neighbors, which is exactly the melee seat we want.
                self.tgt = ec
            self._nav(ct, pave=False)
            return

        # TRANSIT: ride the ferry line when a launcher ahead is in reach,
        # march otherwise.
        launcher = None
        if rnd >= self.ferry_ban_until:
            launcher = self._ferry_launcher(ct, ec)
        if launcher is not None:
            self.ferrying = True
            chosen = self._offer_launch(ct)
            if p.distance_squared(launcher) > 18:
                self.ferry_hold = 0
                self.tgt = launcher
                self._nav(ct, pave=False)
                return
            if chosen == ct.get_id() + 1:
                # Inside the pickup ring holding the claim: STAND STILL and
                # let the launcher's own turn throw us -- but only for
                # FERRY_HOLD_MAX rounds.  A launcher with any legal site
                # throws within ~2; a longer hold means the local terrain
                # has no legal advance site and waiting is a deadlock, so
                # ban the ferry briefly and march (see FERRY_HOLD_MAX).
                self.ferry_hold += 1
                if self.ferry_hold <= FERRY_HOLD_MAX:
                    return
                self.ferry_ban_until = rnd + FERRY_BAN_RNDS
                self.ferry_hold = 0
                self.ferrying = False
            # Another raider holds the slot; keep marching -- the ring is
            # sqrt(26) wide, so walking forward rarely leaves it before the
            # claim frees up.
        else:
            self.ferrying = False
            self.ferry_hold = 0
    ...
    def _mine_guard_harvester(self, ct):
        """DOCTRINE M1 target pick: nearest visible friendly HARVESTER with no
        friendly SENTINEL within dsq <= 32 of it (a sentinel's own reach --
        inside it the mine already has a guard).  Vision-limited getters raise
        at the edge (see _hunt_turret's note), so every lookup is guarded; an
        unreadable sighting is skipped, never trusted.  Both lists come from
        one get_nearby_buildings pass, so a guard sentinel just outside this
        builder's own r^2=20 disk can be missed -- MINEGUARD_CAP bounds the
        cost of that misread at one redundant turret."""
        harvs = []
        sentinels = []
        for bid in ct.get_nearby_buildings():
            try:
                if ct.get_team(bid) != self.team:
                    continue
                bet = ct.get_entity_type(bid)
                if bet == EntityType.HARVESTER:
                    harvs.append(ct.get_position(bid))
                elif bet == EntityType.SENTINEL:
                    sentinels.append(ct.get_position(bid))
            except Exception:
                continue
        p = ct.get_position()
        best = None
        best_d = 0
        for hp_ in harvs:
            if any(hp_.distance_squared(s) <= 32 for s in sentinels):
                continue
            d = p.distance_squared(hp_)
            if best is None or d < best_d:
                best, best_d = hp_, d
        return best

    def _mine_guard_site(self, ct, hpos):
        """First legal mine-guard sentinel tile: orthogonally adjacent to this
        builder AND within dsq <= 8 of hpos (the same "besieging, not passing
        by" band _turret_on_harvester measured), with its facing.  None if no
        such tile.  Shared by the build half (action phase) and the hold test
        (move phase) so the two cannot disagree about what "in build range"
        means.  Facing is toward the fight: nearest_cardinal of the bearing to
        the enemy Core anchor, the same collapse the pave trail and siege
        planner already trust."""
        p = ct.get_position()
        homes = core_tiles(self.core)
        ec = self.enemy or Position(self.mw // 2, self.mh // 2)
        for d in CARDINALS:
            ...
            if not (0 <= bp.x < self.mw and 0 <= bp.y < self.mh):
                continue
            if bp.distance_squared(hpos) > 8:
                continue
            # NEVER build ON an ore tile: same lesson _try_screen's wall ban
            # encodes -- an ore tile is a harvester site (the fjordgate ghost
            # froze 3 of 4 sites under self-laid conveyors).
            if self.map_grid is not None and self.map_grid[bp.y][bp.x] == "o":
                continue
            # Core-adjacent cells are conveyor delivery / heal / battery
            # seats (same exclusion as _try_screen).
            if any(abs(bp.x - h.x) + abs(bp.y - h.y) <= 1 for h in homes):
                continue
            facing = nearest_cardinal(bp.direction_to(ec))
            if ct.can_build_sentinel(bp, facing):
                return bp, facing
        return None

    def _try_mine_guard(self, ct):
        """DOCTRINE M1 build half (see the MINEGUARD_* constants for the
        kladde forensics: 0-1 harvester deaths per game against our ~10, on
        34-48% of income standing sentinels over their own mines).  Build
        only; the walk that gets the defender in range lives in _defend's
        move phase.  True iff a sentinel went down this call."""
        if self.mine_guards >= MINEGUARD_CAP:
            return False
        if ct.get_current_round() < MINEGUARD_MIN_RND:
            return False
        if ct.read_store(SLOT_HARVESTERS) < MINEGUARD_MIN_HARV:
            return False
        # +10 headroom over the sentinel price: a dry sentinel guards nothing,
        # and 10 Ti is exactly one sentinel shot's worth of ammo conversion.
        if not self._eco_spendable(ct, ct.get_sentinel_cost() + 10):
            return False
        hpos = self._mine_guard_harvester(ct)
        if hpos is None:
            return False
        site = self._mine_guard_site(ct, hpos)
        if site is None:
            return False
        bp, facing = site
        ct.build_sentinel(bp, facing)
        self.mine_guards += 1
        # Count it in the magazine, like every other build site in this file
        # (_try_siege_build, _try_counterbattery): SLOT_HOME_GUN scales the
        # Core's ammo target per gun, and the measured failure mode is dry
        # turrets on a full bank (atoll: 2,782 banked, 13 shots all match).
        # The counterbattery economy gate this slot also feeds cannot bite:
        # it triggers on SLOT_HARVESTERS < ECO_NEED, and MINEGUARD_MIN_HARV
        # is above ECO_NEED on a slot that is a monotone lower bound.
        ct.write_store(SLOT_HOME_GUN, ct.read_store(SLOT_HOME_GUN) + 1)
        return True
    ...
    def _defend(self, ct):
            ...
            if not defended:
                ...
                if not endgame and self.link_queue and ti >= ct.get_conveyor_cost():
                    if self._build_next_link(ct):
                        return
                # M2: the v74 home-launcher build is deleted with its
                # subsystem -- ferry launchers are stood on the core-to-core
                # axis by the raiders (see _try_build_ferry_launcher), and
                # SLOT_LAUNCHER is their 0/1/2 count now.
                ...
                if (endgame or harv < self._eco_cap(ct)) and ti >= ct.get_harvester_cost() and self._try_harvester(ct, harv):
                    return
                # DOCTRINE M1 -- MINE-GUARD SENTINELS, build half (see the
                # MINEGUARD_* constants).  Strictly the quiet-map path: under
                # threat the action belongs to the defense branches above, and
                # past ENDGAME_RND a turret delivers nothing any tiebreak
                # reads (PIECE H), so both gates sit on the call.
                if not under and not endgame and self._try_mine_guard(ct):
                    return
                if not under:
                    self._heal_core(ct)
        ...
        # DOCTRINE M1 -- MINE-GUARD SENTINELS, walk half.  Close on the
        # unguarded mine so the build half up in the action phase finds
        # itself adjacent to a legal tile.  Ordering is deliberate: the
        # under-threat chase, the shelled come-home and the link queue have
        # all returned above, so this can never preempt defense or the
        # conveyor work, and the whole branch is additionally gated on
        # `not under` so a threat with no position report cannot be walked
        # away from either.  The gates mirror the build half exactly --
        # walking without the funds or past the cap would stand the defender
        # over a mine it will never guard.  Fallback is deliberate inaction:
        # no visible unguarded harvester means no target, and control falls
        # through to the normal home orbit -- no wandering.
        if (
            not under and not endgame
            and self.mine_guards < MINEGUARD_CAP
            and ct.get_current_round() >= MINEGUARD_MIN_RND
            and harv >= MINEGUARD_MIN_HARV
            and self._eco_spendable(ct, ct.get_sentinel_cost() + 10)
        ):
            mg = self._mine_guard_harvester(ct)
            if mg is not None:
                if self._mine_guard_site(ct, mg) is not None:
                    # In build range of a legal tile.  Hold position for the
                    # action phase instead of letting the orbit below drag us
                    # home -- the same wait-in-range idiom as the link-queue
                    # branch above.
                    return
                self.tgt = mg
                self._nav(ct, pave=False)
    ...
    def _link_path(self, ct, hpos):
        ...
        while q:
            ...
            for dx, dy in _CARD_DELTA:
                nx, ny = x + dx, y + dy
                key = (nx, ny)
                if key in prev or not (0 <= nx < self.mw and 0 <= ny < self.mh):
                    continue
                # This branch hands `n` to the engine, which wants a real
                # Position -- so build one, but only after the two cheap
                # rejections above have already discarded most neighbours.
                n = Position(nx, ny)
    ...
    def _link_facing(self, tile):
        """Return the intended conveyor facing for *tile* (self.link_queue[0]).

        Both the build branch and the facing-repair branch in _build_next_link
        derive their direction from this single expression so that they can
        never disagree -- which would cause oscillation between destroy and
        rebuild on the same tile.  Callers must ensure tile == self.link_queue[0]
        before calling.
        """
    ...
    def _build_next_link(self, ct):
        ...
        while self.link_queue:
            ...
            if bid is not None:
                # V1 FACINGFIX: a PAVE_TRAIL conveyor may sit here facing the
                # wrong direction.  If so, and if we have not already repaired
                # this tile, destroy it (cooldown-free) and fall through to
                # build the correctly-faced replacement this same turn.
                # Fail closed: any engine error in the checks aborts to the
                # original "occupied implies correct" skip.
                repaired = False
                tile_key = (tile.x, tile.y)
                if tile_key not in self.repaired_tiles:
                    try:
                        is_friendly_conveyor = (
                            ct.get_team(bid) == self.team
                            and ct.get_entity_type(bid) == EntityType.CONVEYOR
                        )
                        if is_friendly_conveyor:
                            intended_f = self._link_facing(tile)
                            actual_f = ct.get_direction(bid)
                            if actual_f != intended_f and ct.can_destroy(tile):
                                ct.destroy(tile)
                                self.repaired_tiles.add(tile_key)
                                repaired = True
                    except Exception:
                        pass
                if not repaired:
                    ...
                    continue
                # Fall through: tile is now empty; build the correct conveyor
                # below (same turn, since destroy has no action cooldown).
        ...
        try:
            for eid in ct.get_nearby_entities():
                ...
                if et == EntityType.CORE:
                    ...
                    # SLINGSHOT jam fix (M2, from L3).  M1 left bots
                    # walkable-through in the plan ("they move"), and its
                    # roles spread out enough for that to hold.  SLINGSHOT
                    # masses raiders down ONE axis, and the measured result
                    # on saga was a four-bot gridlock in a one-exit ore
                    # pocket: every unit planned THROUGH a teammate,
                    # can_move refused, and the pile stood for 250 rounds.
                    # Planning around visible bots costs nothing (the BFS is
                    # per-round, so a moved bot unblocks next round) and
                    # dissolves the pile from the outside in.
                    blocked.add((ep.x, ep.y))
        except Exception:
            pass
        start = (p.x, p.y)
    ...
    def _nav(self, ct, pave=True):
        ...
        for d in (CARDINALS[(idx + 1) % 4], CARDINALS[(idx + 3) % 4], desired.opposite()):
            if self._move(ct, d, pave):
                return
        # JAM-BREAKER (M2, from L3).  The ordered fallbacks above never move
        # BACKWARD off the desired axis unless the opposite tile is the one
        # free cell -- which in a mutual pile-up it never is for the inner
        # bots.  Past a real stall, step to ANY free cardinal (id-rotated so
        # two jammed bots do not mirror each other forever): one bot backing
        # out is what turns a deadlock back into a queue.
        if self.stuck >= 8:
            for k in range(4):
                d = CARDINALS[(self.idx + k) % 4]
                if ct.can_move(d):
                    ct.move(d)
                    return
        self.stuck += 1
    ...
    def _move(self, ct, d, pave=True):
        ...
        if PAVE_TRAIL_ON and not hive_map:
            ...
            if pp is not None and self.pave_rnd != ct.get_current_round() - 1:
                pp = None
            # TRACEBACK FIX (M2).  A bot displaced by a LAUNCHER between its
            # own turns still holds last round's pave_prev many tiles behind
            # it: an ENEMY launcher's exile-throw sends no ack at all, and
            # our own ferry ack is store-buffered one round, so the
            # pave_rnd == round - 1 test alone cannot catch a same-round
            # displacement.  is_tile_empty on that far tile raises GameError
            # ("Position out of vision range") -- the exact frame behind
            # every traceback in the L3 h2h battery (6/204) and in the M1
            # mirror sweep (fjordgate/antler/atoll/moonrise).  The vacated
            # tile is one cardinal step away by construction, so a
            # non-adjacent pp is proof of displacement: drop it.
            if pp is not None and abs(p0.x - pp.x) + abs(p0.y - pp.y) != 1:
                pp = None
    ...
    def _turret(self, ct):
        ...
        # HEAL-WAR bookkeeping + rule (M2, from L3 spec 7).  A nest turret
        # that can see the enemy Core publishes its HP; if the published HP
        # has not DROPPED for HEALWAR_STALL_RNDS while we sit in siege
        # range, their medics are out-healing our fire, so the target
        # priority flips to the adjacent enemy BUILDERS first (kill the
        # medics), then resumes on the Core once its HP moves again.  The
        # v74 snowflake healer_focus special keeps its own trigger and
        # shares the same priority table.
        self.enemy = enemy_anchor
        self._track_enemy_core(ct)
        healer_focus = (
        ...
        if not healer_focus and enemy_anchor is not None and min(
            t.distance_squared(p) for t in core_tiles(enemy_anchor)
        ) <= 32:
            ev = ct.read_store(SLOT_ENEMY_HP)
            if ev:
                last_drop = (ev >> 9) - 1
                healer_focus = (
                    ct.get_current_round() - last_drop > HEALWAR_STALL_RNDS
                )
    ...
    def _launcher(self, ct):
        if self.team is None:
            self.team = ct.get_team()
        # SLOT_LAUNCHER is a COUNT under the M2 graft (0/1/2, written by the
        # raiders that build).  A live launcher only backfills the floor: if
        # the slot still reads 0 (builder died the round it built), publish
        # at least 1 so ferry recruits know the line exists.
        if ct.read_store(SLOT_LAUNCHER) == 0:
        ...
        if self.core is None:
            # MIDFIELD LAUNCHER (the SLINGSHOT case): our own Core is outside
            # this launcher's r^2=26 vision, so the visible-scan above finds
            # nothing -- the M1 ancestor returned here and a ferry launcher
            # never threw a single bot.  The anchor table is symmetric, so
            # the enemy anchor (written to the store by our Core at r0) maps
            # straight back to ours.
            if dest is not None:
                self.core = enemy_core_for(w, h, dest)
            else:
                return
        if dest is None:
