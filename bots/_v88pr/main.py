...
#
# KNOWN BIAS, carried deliberately.  The baseline is frozen at the rising edge,
# where the window still straddles the pre-bleed rounds (deficit 0) and the
# ramp, so it under-states the steady-state level and the test is therefore
# BIASED TOWARD STANDING DOWN.  HS3_PROD_GRACE is set above the recall travel
# time so the comparison is at least equilibrium-against-onset, and a
...
HS3_PROD_WINDOW = 24
# Rounds after the flag is raised before the first productivity check, sized
# above the recall walk so the reserve has actually arrived and acted.
HS3_PROD_GRACE = 30
...
class Player:
    def __init__(self):
        ...
        # window frozen at the last rising edge; hs3_check_rnd is the round of
        ...
        # every read of them is gated on hs3_flag, which only a rising edge --
        # which sets them -- can turn on.
            ...
            if hp >= mx:
                ...
                if (
                    ...
                    if not self.hs3_flag:
                        # Rising edge: freeze the window the conscription will
                        # be judged against, and book the first check.
                        self.hs3_base_sum = self.hs3_sum
                        self.hs3_base_n = self.hs3_n
                        self.hs3_check_rnd = rnd + HS3_PROD_GRACE
                    self.hs3_flag = True
                if self.hs3_flag and rnd >= self.hs3_check_rnd:
                    # Mean-against-mean, cross-multiplied so it stays integer
                    # (the two windows can differ in length only in the opening
                    # rounds, before the ring has filled).  Bound: deficit <=
                    # max_hp = 500, so the products stay under 3e7.
                    if (
                        self.hs3_sum * self.hs3_base_n * 100
                        <= self.hs3_base_sum * self.hs3_n
                        * (100 - HS3_PROD_MIN_DROP)
                    ):
                        # Incoming is falling: the recalled bodies are removing
                        # the shooter.  Keep going, re-check next window.
                        self.hs3_check_rnd = rnd + HS3_PROD_WINDOW
                    else:
                        # Incoming is flat or worse: the reserve is PAYING the
                        # damage, not removing it, and every further round of it
                        # is pure opportunity cost.  Hand the seats back and bar
                        # re-arming for HS3_STANDDOWN rounds.
                        self.hs3_flag = False
