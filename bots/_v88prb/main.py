...
# PLANK HS, MECHANISM 4 -- THIRD-HEALER CONSCRIPTION (FIX A of the archb
# residual-owner decode, docs/research/archb-residual-owner-2026-08-08.md).
#
# THE MEASURED HOLE.  On archipelago seed 1 seat B the Core sits in a 480-round
# limit cycle taking exactly 9 HP/round and healing exactly 8 (two permanently
# seated builders), bleeding -1/round from r252 to death at r732 while THREE of
# our five builders do nothing at all and FIVE of the eight heal seats stand
# free every round.  The reason is not the seat chooser -- it is who is a
# CANDIDATE.  Every recall path in this file is gated on the Core being inside
# the walker's own vision (r^2 = 20, `_core_shelled`) and on holding a specific
# role: `_defend`'s come-home is `shelled and role_n == 4`, `_expand`'s
# convergence is `role_n in (2, >=5) and SLOT_UNDER and shelled`.  The idle
# three were at d^2 = 104, 181 and on the wrong roles -- structurally invisible
# to the heal machinery for 600 rounds.  One more body on a seat is +4 HP/round,
# which turns -1 into +3.
#
# WHAT THIS ADDS, AND WHAT IT REUSES.  Only the REACH is new.  The Core (the one
# unit that can always see its own HP bar) meters a sustained bleed and raises a
# broadcast flag; a conscript walks home on that flag alone, with no vision or
# role test, and the moment it is standing on a seat the EXISTING universal
# adjacent heal in _builder claims its action exactly as it does for today's two
# healers.  No second heal system, no new heal call, no new seat chooser --
# `_seat_seek_target` is asked for the target the instant the seats come into
# vision, and before that the walk aims at the Core itself, which
# `_bfs_direction` already expands into "any unblocked seat".
#
# THE BROADCAST CHANNEL -- BIT 20 OF SLOT_HEAL_BUDGET.  All 16 store slots are
# assigned, so no slot is taken; the flag rides in the high bits of slot 9.  The
# collision-safety argument, in full:
#
#  1. SINGLE WRITER.  SLOT_HEAL_BUDGET has exactly one writer in the file -- the
#     Core, in the income-meter block of _core -- and exactly one reader,
#     `_heal_budget_left`.  (The slot's own comment states this invariant; it is
#     what made slot 9 safe for the budget in the first place.)  So the
#     last-write-wins hazard that rules out multi-writer packing does not arise:
#     there is no second writer to lose a race with.  The Core also writes the
#     slot exactly ONCE per round, and the flag is OR-ed into that same single
#     write, so the two payloads cannot clobber each other even within the
#     Core's own turn.
#  2. DISJOINT BIT RANGES, WITH A PROVEN BOUND.  The budget payload is
#     `(income_q // 4) * K_HEAL_RATE_PCT // 100` with
#     `income_q = sum over rounds of (10 + K_HEAL_HARV_Q * harv)`.  Rounds are
#     capped at MAX_TURNS+1 = 1001 and `harv` (SLOT_HARVESTERS) is a count of
#     harvesters, which cannot exceed the tile count of the largest legal map,
#     30 x 30 = 900.  Worst case: 1001 * (10 + 5*900) / 4 * 5 / 100 = 56,431 --
#     three orders of magnitude below 2^20 = 1,048,576, and ~1,500 at realistic
#     harvester counts.  Bit 20 is therefore unreachable by the budget on any
#     legal map.  The reader masks it off regardless (below), so even a bound
#     violation would degrade to a slightly low heal allowance, never to a
#     corrupt one.
#  3. BUFFERED-WRITE SEMANTICS.  Store writes are visible from the NEXT round,
#     so a conscript acts on a one-round-old flag.  That is immaterial by
#     construction: the flag arms only after HS3_BLEED_ROUNDS consecutive
#     bleeding Core turns and clears only at full HP, so it is a slow, sticky
#     signal, not an edge.
#  4. TOGGLE-OFF IDENTITY.  With HS3_ON False the Core takes the original write
#     statement verbatim and the reader skips the mask, so slot 9 carries the
#     same bits it carries in _v85hsd.
#
# QUIET GAMES SEE NOTHING.  The trigger is a SUSTAINED bleed -- below max HP for
# HS3_BLEED_ROUNDS consecutive Core turns AND having actually lost HP inside
# that same window -- so a single hit, or old unrepaired scratch damage long
# after the attacker left, never arms it.  Release is hysteretic: the flag drops
# only when the Core is back at FULL HP, and each conscript then hands a clean
# nav state back to its role machine exactly as _expand's `converging` falling
# edge does.
HS3_ON = True
# Consecutive Core turns below max HP before the flag arms, and (as the second
# half of the same test) the staleness window on the last observed HP drop.
HS3_BLEED_ROUNDS = 12
# How many builders may be conscripted.  The cap is STRUCTURAL, not a counter:
# role_n is unique per builder (handed out from SLOT_ROLE_N), so whitelisting
# the first HS3_MAX_CONSCRIPTS entries of the order below caps the conscription
# exactly, with no cross-unit coordination -- which a buffered, last-write-wins
# store slot could not have provided anyway.
#
# The order is deliberate.  role_n == 2 is the pure expander the defend-role
# succession already treats as the spare, and it is the one role that ALREADY
# has a convergence path -- conscription just removes its vision gate.
# role_n == 3 is the second attacker, whose sabotage is worth less than the Core
# is.  role_n == 0 (the siege engineer holding the forward gun) is last and out
# of scope at the default cap.  role_n == 1 (the single interceptor) and
# role_n == 4 (the defender, which already comes home) are never conscripted.
HS3_MAX_CONSCRIPTS = 2
HS3_ROLE_ORDER = (2, 3, 0)
HS3_ROLES = frozenset(HS3_ROLE_ORDER[:HS3_MAX_CONSCRIPTS])
# Bit position of the bleed flag inside SLOT_HEAL_BUDGET, and the mask that
# strips it back off for the budget reader.  See point 2 above for the headroom
# proof.
HS3_BLEED_BIT = 1 << 20
HS3_BUDGET_MASK = HS3_BLEED_BIT - 1

# PRODUCTIVITY-CONDITIONED RELEASE (the ad-flips decode, 2026-08-08, section 9).
#
# THE MEASURED HOLE.  The trigger above is fine; the RELEASE is wrong.  "Full HP
# is the ONLY release" is unreachable inside the equilibrium the conscription
# itself creates: a heal-matched siege pins the Core one volley below max on
# every Core turn forever, so the flag never falls.  Measured dwell: the rest of
# the match in 3 of 5 decoded games; on lighthouse one conscript held a seat for
# ~900 rounds and moved three times in 870, while the two builders that in the
# parent performed the counterbattery were recalled at r100 and dead by r149.
#
# THE DISCRIMINATOR IS THE LEVEL, NOT THE RATIO.  `heal / incoming` is ~1.00 in
# every decoded leg, winning and losing alike -- the homeostat always closes, so
# heal throughput carries no information.  What separates the one leg the
# conscription wins from the four it loses is whether incoming FALLS (8.53 ->
# 3.45 HP/round on archipelago, where the recalled bodies could reach the gun)
# or RISES/stays flat (lighthouse 2.42 -> 14.00, where they could not).  So the
# release test is: did recalling the reserve actually REMOVE incoming?
#
# WHAT THE CORE CAN ACTUALLY OBSERVE.  The Core sees only its own HP trajectory
# (`get_hp` / `get_max_hp`); there is no getter for damage taken, and heals
# received are indistinguishable from any other HP gain.  That rules out the
# obvious estimator: `max(0, -(hp_t - hp_t-1))` is `max(0, damage - heal)`, i.e.
# NET bleed, and in a homeostat with heal / incoming ~ 1.00 it reads ~0 whether
# incoming is 3 HP/round or 16.  It measures exactly the quantity the decode
# proved uninformative, and same-round heals mask it completely.
#
# WHAT IS USED INSTEAD: the rolling mean HP DEFICIT, `max_hp - hp`, over
# HS3_PROD_WINDOW Core turns.  It is not a difference of two HP readings, so
# same-round healing cannot cancel it.  Its equilibrium value is the damage that
# lands between the last heal and the Core's next read -- which scales with the
# LEVEL at which the homeostat closes, which is the discriminator the decode
# names.  A siege that is being paid for holds a large steady deficit; a siege
# whose guns are being removed decays toward 0, and 0 IS the existing full-HP
# success release, so the two exits agree at the limit.  Cost: one subtract, one
# store and one modulo per Core turn, over a fixed-length ring.
#
# KNOWN BIAS, carried deliberately.  The baseline is frozen at the ARRIVAL edge
# (see below), where the window still straddles the pre-arrival rounds and the
# ramp, so it under-states the steady-state level and the test is therefore
# BIASED TOWARD STANDING DOWN.  HS3_PROD_GRACE then runs from arrival, and a
# conscription whose gun-removal lands later than the first check gets a second
# chance: the stand-down expires, the flag re-arms against a FRESH baseline (the
# then-current siege level), and the same test runs again.  The net effect on an
# unproductive lock is a bounded duty cycle -- HS3_PROD_GRACE-ish rounds of
# conscription per HS3_STANDDOWN-ish rounds of reserve output -- rather than the
# measured near-absorbing state.
HS3_PROD_WINDOW = 24
# Rounds after the conscripts ARRIVE before the first productivity check (see
# the CLOCK RESTART block below -- this used to run from flag-raise, which is
# what made the check fire before the mechanism it guards could pay).
HS3_PROD_GRACE = 30
# Relative fall in the windowed deficit, in percent, that counts as "the
# conscription is removing incoming rather than paying it".
HS3_PROD_MIN_DROP = 20
# Rounds during which an unproductive conscription may not re-arm.
HS3_STANDDOWN = 120

# CLOCK RESTART AT SEATS-FILLED, AND THE CONTACT OVERRIDE (archipelago det
# signature, 2026-08-08).
#
# THE MEASURED HOLE.  The release above stood the PRODUCTIVE conscription down.
# On archipelago the full-conscription line wins r1000 by counterbattery: the
# recalled bodies kill the enemy gun parked on our own seat ring at ~r308.  With
# the release wired to flag-raise, the check fired ~54 rounds after the flag went
# up -- i.e. while the conscripts were still WALKING -- read a deficit that had
# not moved yet, and handed the seats back; the Core then died at r243, 65 rounds
# before the counterbattery would have landed.  The test was right; its clock was
# started from the wrong event.
#
# FIX 1 -- THE CLOCK STARTS AT ARRIVAL, NOT AT FLAG-RAISE.  The recall walk takes
# tens of rounds, and nothing about the deficit can change until a body is
# actually standing on a seat.  So both the grace timer AND the baseline snapshot
# now start on the ARRIVAL EDGE: the first Core turn on which the number of
# friendly builder bots standing on the heal-seat ring RISES above its low-water
# mark since flag-raise.  The Core cannot see distant builders, but the seat ring
# is the eight orthogonal neighbours of its own 2x2 footprint -- dsq <= 2, deep
# inside the Core's own vision (r^2 = 36) -- so this edge is directly observable
# with no channel and no broadcast.  Comparing against the low-water mark rather
# than the flag-raise reading means a seat vacated and then re-taken still counts
# as an arrival, which is the conservative direction: it starts the clock, and
# the clock only ever leads to a stand-down.
#
# If no arrival is ever observed the conscription is genuinely unproductive --
# the bodies died on the way, or cannot path home -- and HS3_ARRIVE_MAX stands it
# down without waiting for a deficit test that would have nothing to measure.
#
# FIX 2 -- CONTACT OVERRIDES THE STAND-DOWN.  Counterbattery in progress is
# productivity that the deficit meter cannot see yet: the gun is still firing
# right up until it dies, so the windowed deficit stays flat through the whole
# attack and the meter votes to stand down on the round before the payoff.  The
# direct observable is CONTACT -- a friendly builder bot standing orthogonally
# adjacent to a live enemy Gunner or Sentinel, i.e. in position to attack it.
# While that holds, the stand-down is suppressed.
#
# LIMITATION, ACCEPTED AND NOT WORKED AROUND.  Both the seat count and the
# contact test read `get_nearby_units()` from the CORE, so both are blind outside
# the Core's own vision.  Counterbattery that happens far from home is invisible
# and the override simply never fires there -- the deficit test alone decides, as
# before.  That is the correct scope: this is the archipelago/archb case, where
# the enemy gun sits ON our seat ring, and it is deliberately NOT broadcast
# around via a store slot (the conscripts have no free slot, and a builder-side
# writer would race the Core's own single write to SLOT_HEAL_BUDGET).
#
# BOUNDED.  A futile grind -- a builder chipping at a turret it can never finish,
# or one that is itself being healed faster than it damages -- must not hold the
# lock forever, so the override expires after HS3_CONTACT_MAX rounds of
# CONTINUOUS contact.  Any round without contact resets the run.
#
# Cost: one `get_nearby_units()` pass per Core turn WHILE FLAGGED ONLY (units,
# not entities -- the conveyor field is not enumerated), plus O(8) integer tile
# comparisons.  Zero cost on a quiet Core, and zero cost with HS3_ON off.
#
# Rounds after flag-raise within which the conscripts must be seen to arrive.
# Above the observed recall walk with margin; past it, stand down and re-arm on
# the normal HS3_STANDDOWN cycle.
HS3_ARRIVE_MAX = 60
# Maximum rounds of CONTINUOUS builder-adjacent-to-enemy-turret contact that may
# suppress the stand-down.  Sized above the archipelago counterbattery's measured
# lead time over the first check, below a whole-match lock.
HS3_CONTACT_MAX = 90

# DELIVERY-SPUR REPAIR (FIX D of the same decode).
#
# THE MEASURED HOLE.  In the archb game the enemy's thrown saboteur cut five
# conveyors of one spur between r101 and r153.  Nothing in this file ever
# rebuilt them: `link_queue` plans a route once, `_build_next_link` pops each
# tile as it is laid, and no caller ever re-derives the plan from "this
# harvester has stopped shipping".  Delivery through that spur read ZERO stacks
# for the last 532 rounds while the builder that laid it stood ten tiles away
# doing nothing and a conveyor cost 3 Ti against a bank that touched 20 every
# fourth round.
#
# THE MECHANISM.  Each builder remembers, in plain instance state, the tiles it
# has personally laid a conveyor on.  When it is idle of links and one of those
# tiles is inside its own vision and carries no building, the tile is re-queued
# into `link_queue` -- i.e. through the EXISTING linker path, which already
# walks to the tile, funds it via `_eco_spendable` and orients it.  Gaps are
# re-queued in original build order, so `_build_next_link`'s
# `link_queue[1]` facing rule reproduces the original orientation for every
# contiguous tile but the tail, which falls back to the same
# direction-to-the-Core rule the planner used for it.
#
# STATE LIFETIME, ACCEPTED.  The memory is per-unit and dies with the builder:
# a link whose owner is killed is never repaired by anyone.  That is deliberate
# -- the alternative is a team-wide tile ledger, and no store slot can hold one
# (16 integers, buffered a round, last write wins).  The archb case is exactly
# the one this shape covers: the owner is alive and idle for 578 rounds.
#
# TREADMILL GUARD.  The saboteur that cut the spur is still sitting on it with
# ~25 free action-rounds per 100, so an unbounded repair loop would be a
# titanium pump for the attacker.  Each tile may be re-queued at most
# SPUR_MAX_REBUILDS times over the match; after that the gap is left alone.  A
# bounded treadmill is a trade we are happy to make (3 Ti a conveyor against a
# harvester's 10 Ti per 4 rounds), an unbounded one is not.
SPUR_ON = True
SPUR_MAX_REBUILDS = 3
# Most gap tiles re-queued in one go.  A repair is meant to be a short detour,
# not a new construction project; anything longer is a re-plan, which is a
# different piece.
SPUR_MAX_QUEUE = 4
# Rounds a repair queue may live before it is abandoned.  This is a LIVELOCK
# guard, not a tuning knob.  The linker's move phase deliberately idles a
# builder that is standing beside its next planned tile but cannot fund it
# ("wait for action/resources instead of occupying the future conveyor cell"),
# and under siege `_eco_spendable` holds back SIEGE_HEAL_RESERVE_TI on top of
# the price -- so a repair aimed at a tile we cannot currently afford would park
# its owner beside that tile for the rest of the match, and a permanently
# non-empty link_queue would also freeze the role_n == 3 saboteur conversion
# that reads `not self.link_queue`.  Abandoning the queue returns the unit to
# its role machine; the tile is still remembered and still under the rebuild
# cap, so a later, richer round can try again.
SPUR_GIVEUP_RNDS = 12
# Cap on remembered tiles per builder, oldest dropped first.  Bounds both the
# scan cost and the dict; a single builder rarely lays this many conveyors.
SPUR_MEMORY = 48
...
class Player:
    def __init__(self):
        ...
        # PLANK HS MECHANISM 4 (see HS3_ON).  Core side: hs3_bleed counts
        # consecutive Core turns spent below max HP, hs3_drop_rnd is the last
        # round the Core actually LOST HP, and hs3_flag is the published bleed
        # bit.  Builder side: hs3_on is True while this unit is conscripted, and
        # exists only so the falling edge can hand a clean nav state back to the
        # role machine, exactly as `converging` above does.
        self.hs3_bleed = 0
        self.hs3_drop_rnd = -10 ** 9
        self.hs3_flag = False
        self.hs3_on = False

        # PRODUCTIVITY-CONDITIONED RELEASE (see HS3_PROD_WINDOW).  Core side
        # only, per unit instance like the meter above: hs3_ring is the
        # fixed-length HP-deficit window with hs3_ring_i its write cursor,
        # hs3_sum its running sum and hs3_n its fill count (< window only for
        # the first few rounds of the match); hs3_base_sum / hs3_base_n are the
        # window frozen at the last ARRIVAL edge; hs3_check_rnd is the round of
        # the next productivity check and hs3_standdown the round before which
        # the flag may not re-arm.  Both round counters are safe at 0 because
        # every read of them is gated on hs3_flag (and hs3_armed), which only a
        # rising / arrival edge -- which sets them -- can turn on.
        self.hs3_ring = [0] * HS3_PROD_WINDOW
        self.hs3_ring_i = 0
        self.hs3_sum = 0
        self.hs3_n = 0
        self.hs3_base_sum = 0
        self.hs3_base_n = 0
        self.hs3_check_rnd = 0
        self.hs3_standdown = 0

        # CLOCK RESTART AT SEATS-FILLED / CONTACT OVERRIDE (see HS3_ARRIVE_MAX).
        # Core side only, same per-unit-instance argument as everything above:
        # hs3_flag_rnd is the round the bleed flag last rose (the arrival
        # deadline runs from it), hs3_seats is the LOW-WATER mark of friendly
        # builders on the heal-seat ring since that raise, hs3_armed is True
        # once the arrival edge has fired and the productivity clock is running,
        # and hs3_contact_rnd is the round the current CONTINUOUS run of
        # builder-adjacent-to-enemy-turret contact began (0 == no contact).
        # Zero is safe as "no contact" because the flag cannot be up before
        # round HS3_BLEED_ROUNDS, so a real contact round is never 0.
        self.hs3_flag_rnd = 0
        self.hs3_seats = 0
        self.hs3_armed = False
        self.hs3_contact_rnd = 0

        # DELIVERY-SPUR REPAIR (see SPUR_ON).  spur_built is this unit's own
        # laid-conveyor tiles in build order; spur_rebuilds counts how many
        # times each has been re-queued, for the treadmill guard.  Per unit
        # instance and no store slot -- see the SPUR_ON block for why a
        # team-wide ledger is not available.  vis_sq caches this unit's vision
        # radius so the gap scan filters by arithmetic before it spends an
        # engine call.
        self.spur_built = []
        self.spur_rebuilds = {}
        self.spur_until = None
        self.vis_sq = None
    ...
    def _core(self, ct):
        ...
        if self.last_hp is not None and hp < self.last_hp:
            under = True
        # PLANK HS MECHANISM 4 (see HS3_ON) -- sustained-bleed meter.  Placed
        # here, before self.last_hp is overwritten, because the drop test needs
        # the previous turn's reading.  Costs one get_max_hp() per Core turn.
        if HS3_ON:
            mx = ct.get_max_hp()
            # PRODUCTIVITY METER (see HS3_PROD_WINDOW).  Ring-buffered rolling
            # sum of the HP DEFICIT -- deliberately not a delta of two HP
            # readings, because same-round heals cancel those and the decode
            # showed the net bleed is ~0 at every incoming level.  Runs on every
            # Core turn, flagged or not, so the rising edge has a window to
            # freeze.  O(1) integer arithmetic, no extra engine call.
            deficit = mx - hp
            self.hs3_sum += deficit - self.hs3_ring[self.hs3_ring_i]
            self.hs3_ring[self.hs3_ring_i] = deficit
            self.hs3_ring_i = (self.hs3_ring_i + 1) % HS3_PROD_WINDOW
            if self.hs3_n < HS3_PROD_WINDOW:
                self.hs3_n += 1
            if hp >= mx:
                # Hysteresis: full HP is the ONLY *success* release, so a Core
                # that is still one peck down keeps its conscripts.  (The
                # unproductive-siege release is the productivity check below,
                # which exists because this one is unreachable inside a
                # heal-matched siege -- see HS3_PROD_WINDOW.)
                self.hs3_bleed = 0
                self.hs3_flag = False
            else:
                self.hs3_bleed += 1
                if self.last_hp is not None and hp < self.last_hp:
                    self.hs3_drop_rnd = rnd
                # Both halves are required, which is what makes this a
                # SUSTAINED bleed rather than a hit: long enough below max, and
                # still actually losing HP inside that same window.  Old,
                # unrepaired scratch damage on a quiet map fails the second.
                # The third clause is the stand-down cooldown, and is the only
                # change to the TRIGGER: the test itself is untouched.
                was_flag = self.hs3_flag
                if (
                    self.hs3_bleed >= HS3_BLEED_ROUNDS
                    and rnd - self.hs3_drop_rnd <= HS3_BLEED_ROUNDS
                    and rnd >= self.hs3_standdown
                ):
                    if not self.hs3_flag:
                        # Rising edge: the productivity clock does NOT start
                        # here (see HS3_ARRIVE_MAX) -- the conscripts are still
                        # walking.  All this books is the arrival deadline.
                        self.hs3_flag_rnd = rnd
                        self.hs3_armed = False
                        self.hs3_contact_rnd = 0
                    self.hs3_flag = True
                if self.hs3_flag:
                    # One units pass per flagged Core turn; nothing here runs on
                    # a quiet Core.  See HS3_ARRIVE_MAX for the vision scope.
                    seats, contact = self._hs3_watch(ct, p, w, h)
                    if contact:
                        if not self.hs3_contact_rnd:
                            self.hs3_contact_rnd = rnd
                    else:
                        self.hs3_contact_rnd = 0
                    if not was_flag:
                        # Seat occupancy at flag-raise -- the arrival edge is a
                        # rise above this, so bodies already home do not count.
                        self.hs3_seats = seats
                    elif not self.hs3_armed:
                        if seats > self.hs3_seats:
                            # ARRIVAL EDGE.  Only now is there anything for the
                            # deficit meter to measure: freeze the window the
                            # conscription will be judged against, and book the
                            # first check a grace period out from HERE.
                            self.hs3_armed = True
                            self.hs3_base_sum = self.hs3_sum
                            self.hs3_base_n = self.hs3_n
                            self.hs3_check_rnd = rnd + HS3_PROD_GRACE
                        else:
                            if seats < self.hs3_seats:
                                self.hs3_seats = seats
                            if rnd - self.hs3_flag_rnd >= HS3_ARRIVE_MAX:
                                # Nobody ever arrived: dead or unable to path.
                                # Genuinely unproductive -- same exit as a
                                # failed deficit test.
                                self.hs3_flag = False
                                self.hs3_standdown = rnd + HS3_STANDDOWN
                    elif rnd >= self.hs3_check_rnd:
                        # Mean-against-mean, cross-multiplied so it stays
                        # integer (the two windows can differ in length only in
                        # the opening rounds, before the ring has filled).
                        # Bound: deficit <= max_hp = 500, so the products stay
                        # under 3e7.
                        if (
                            self.hs3_sum * self.hs3_base_n * 100
                            <= self.hs3_base_sum * self.hs3_n
                            * (100 - HS3_PROD_MIN_DROP)
                        ):
                            # Incoming is falling: the recalled bodies are
                            # removing the shooter.  Keep going, re-check next
                            # window.
                            self.hs3_check_rnd = rnd + HS3_PROD_WINDOW
                        elif (
                            self.hs3_contact_rnd
                            and rnd - self.hs3_contact_rnd < HS3_CONTACT_MAX
                        ):
                            # CONTACT OVERRIDE.  A body is standing next to the
                            # gun right now; the gun fires until it dies, so the
                            # deficit cannot have moved yet.  This IS the
                            # productivity, in progress -- re-check next window
                            # instead of standing down.  Bounded by
                            # HS3_CONTACT_MAX rounds of continuous contact.
                            self.hs3_check_rnd = rnd + HS3_PROD_WINDOW
                        else:
                            # Incoming is flat or worse: the reserve is PAYING
                            # the damage, not removing it, and every further
                            # round of it is pure opportunity cost.  Hand the
                            # seats back and bar re-arming for HS3_STANDDOWN
                            # rounds.
                            self.hs3_flag = False
                            self.hs3_standdown = rnd + HS3_STANDDOWN
        self.last_hp = hp
        ...
        # PLANK HS MECHANISM 4 (see HS3_ON) -- the bleed flag rides bit 20 of
        # this slot and is OR-ed into the SAME single write, so the Core touches
        # slot 9 exactly once a round and the two payloads cannot clobber each
        # other.  With HS3_ON off the original statement runs verbatim.
        if HS3_ON:
            word = income_ti * K_HEAL_RATE_PCT // 100 if K_HEAL_BUDGET_ON else 0
            if self.hs3_flag:
                word |= HS3_BLEED_BIT
            ct.write_store(SLOT_HEAL_BUDGET, word)
    ...
    def _hs3_watch(self, ct, o, mw, mh):
        """CLOCK RESTART / CONTACT OVERRIDE (see HS3_ARRIVE_MAX).  Core side.

        Returns `(seats, contact)` for the current Core turn:

         - `seats`: how many FRIENDLY builder bots are standing on the heal-seat
           ring, i.e. on one of the eight orthogonal neighbours of the Core's own
           2x2 footprint (`heal_seats`, pure arithmetic).  Those tiles are at
           dsq <= 2 from the footprint, so they are always inside the Core's
           vision (r^2 = 36) and the count is exact, with no channel involved.
         - `contact`: whether any friendly builder bot is orthogonally adjacent
           to a VISIBLE enemy Gunner or Sentinel -- i.e. is standing where it can
           attack it.  Launchers are excluded, matching the threat scan above:
           they deal no damage, so there is no counterbattery to be in progress.

        One `get_nearby_units()` call, not `get_nearby_entities()`: turrets and
        builder bots are both units, so the conveyor/harvester field is never
        enumerated.  The rest is integer tile comparison against a set -- O(8)
        for the ring, O(4) per visible enemy turret, of which a siege parks a
        handful.  Called only while the bleed flag is up.

        SCOPE, deliberate: both readings are bounded by the CORE's vision, so a
        counterbattery fought away from home is invisible here and `contact` is
        simply False.  That is the documented limitation -- see HS3_ARRIVE_MAX
        for why it is not broadcast around.
        """
        me = ct.get_team()
        bots = set()
        turrets = []
        for uid in ct.get_nearby_units():
            ...
            if et == EntityType.BUILDER_BOT:
                ...
                if ct.get_team(uid) != me:
                    turrets.append(ct.get_position(uid))
        if not bots:
            return 0, False
        seats = 0
        for s in heal_seats(o, mw, mh):
            if (s.x, s.y) in bots:
                seats += 1
        contact = False
        for t in turrets:
            if ((t.x, t.y - 1) in bots or (t.x, t.y + 1) in bots
                    or (t.x - 1, t.y) in bots or (t.x + 1, t.y) in bots):
                contact = True
                break
        return seats, contact
    ...
    def _builder(self, ct):
        ...
        # PLANK HS MECHANISM 4 (see HS3_ON).  Sits directly above the role
        # dispatch because that is exactly what it overrides: a conscript's job
        # for the duration is to be on a heal seat, and every role machine below
        # would walk it somewhere else.  It claims the MOVE only -- the action
        # was already offered to the universal adjacent heal further up, which
        # is what does the actual healing once the walk arrives.
        if HS3_ON and self._hs3_recall(ct):
            return

        # DELIVERY-SPUR REPAIR (see SPUR_ON).  Reached only when this unit is
        # not conscripted; it seeds link_queue and then falls through, so the
        # role machine below does the walking and the building through the
        # linker path it already owns.
        if SPUR_ON:
            self._spur_check(ct)
    ...
    def _hs3_busy_building(self, ct):
        """True if this unit is one action away from laying its next link tile.

        The "not mid-build" half of the conscription gate (see HS3_ON), and
        deliberately the narrow reading of it: a builder standing beside its
        planned conveyor with the bank to pay for it finishes that tile first
        and is conscripted next turn instead.  A link_queue that is merely
        non-empty does NOT defer -- that is the state the archb idlers sat in
        for 578 rounds, and a Core bleeding out outranks a conveyor.  Pure
        arithmetic plus the two cheap getters the linker would ask anyway.
        """
        ...
        t = self.link_queue[0]
        return (
            abs(p.x - t.x) + abs(p.y - t.y) == 1
            and ct.get_action_cooldown() == 0
            and ct.get_global_resources() >= ct.get_conveyor_cost()
        )

    def _hs3_recall(self, ct):
        """PLANK HS MECHANISM 4: heal-seat conscription.  True == turn claimed.

        Reads the Core's bleed broadcast out of SLOT_HEAL_BUDGET's flag bit (see
        HS3_ON for the channel's collision-safety argument) and, for a
        whitelisted role, walks this unit home with NO vision test -- which is
        the single gate the decode names as the owner of the archb loss.

        Everything after arrival is existing machinery: `_seat_seek_target`
        picks the seat as soon as the ring is in vision, self.core stands in for
        it before that (`_bfs_direction` expands a Core target into "any
        unblocked seat"), and the healing itself is the universal adjacent heal
        in _builder, which already fired above this call.  Once adjacent this
        holds position rather than walking on, exactly as _expand's convergence
        does and for the same reason.
        """
        if self.core is None:
            return False
        # Falling edge -- the Core is whole again, or this unit was promoted out
        # of the conscription whitelist (role_n == 2 can become the defend-role
        # successor mid-match).  Hand a CLEAN state back to the role machine,
        # exactly as _expand's `converging` reset does: self.tgt still holds a
        # seat or the Core itself, neither of which the role machines below
        # would ever have chosen.
        if not (ct.read_store(SLOT_HEAL_BUDGET) & HS3_BLEED_BIT) \
                or self.role_n not in HS3_ROLES:
            if self.hs3_on:
                self.hs3_on = False
                self.tgt = None
                self.stuck = 0
        ...
        if not self.hs3_on:
            ...
            self.wall = None
        p = ct.get_position()
        if any(abs(p.x - c.x) + abs(p.y - c.y) == 1 for c in core_tiles(self.core)):
            return True
        if ct.get_move_cooldown() == 0:
    ...
    def _spur_remember(self, tile):
        """Record a conveyor this unit just laid, for the repair scan (SPUR_ON).

        Build order is preserved, which is what lets a contiguous run of gaps be
        re-queued with its original orientations.  A tile already known keeps
        its rebuild count -- a repaired tile must not launder itself past the
        treadmill guard.
        """
        key = (tile.x, tile.y)
        if key in self.spur_rebuilds:
            return
        self.spur_rebuilds[key] = 0
        self.spur_built.append(tile)
        if len(self.spur_built) > SPUR_MEMORY:
            old = self.spur_built.pop(0)
            self.spur_rebuilds.pop((old.x, old.y), None)

    def _spur_check(self, ct):
        """DELIVERY-SPUR REPAIR (see SPUR_ON): re-queue destroyed own segments.

        Only runs while this unit is idle of links, so it can never overwrite a
        plan in progress, and only inspects remembered tiles that are inside
        this unit's own vision -- filtered by arithmetic first, so the engine
        call is spent only on tiles that could answer.  Sets link_queue and
        returns; the walk, the funding and the build are the linker path's, not
        this method's.
        """
        rnd = ct.get_current_round()
        if self.link_queue:
            # Only ever true for a queue THIS method planted: the real linker
            # sets link_queue inside the role dispatch below, i.e. on a turn
            # this method already saw the queue empty and disarmed the timer.
            if self.spur_until is not None and rnd >= self.spur_until:
                self.link_queue = []
                self.spur_until = None
            return
        self.spur_until = None
        if not self.spur_built:
            return
        if self.vis_sq is None:
            self.vis_sq = ct.get_vision_radius_sq()
        p = ct.get_position()
        gaps = []
        for t in self.spur_built:
            dx, dy = p.x - t.x, p.y - t.y
            if dx * dx + dy * dy > self.vis_sq:
                continue
            if self.spur_rebuilds.get((t.x, t.y), 0) >= SPUR_MAX_REBUILDS:
                continue
            try:
                ...
                continue
            gaps.append(t)
            if len(gaps) >= SPUR_MAX_QUEUE:
                break
        if not gaps:
            return
        for t in gaps:
            self.spur_rebuilds[(t.x, t.y)] = self.spur_rebuilds.get((t.x, t.y), 0) + 1
        self.link_queue = gaps
        self.spur_until = rnd + SPUR_GIVEUP_RNDS
    ...
    def _heal_budget_left(self, ct):
        ...
        """
        raw = ct.read_store(SLOT_HEAL_BUDGET)
        # PLANK HS MECHANISM 4 (see HS3_ON): strip the bleed flag out of the
        # high bits before the budget arithmetic.  Masked only under the toggle,
        # so with HS3_ON off this reads the slot exactly as _v85hsd does.
        if HS3_ON:
            raw &= HS3_BUDGET_MASK
        allowance = (K_HEAL_BASE_GRANT + raw) // K_HEAL_SHARES
    ...
    def _build_next_link(self, ct):
        ...
        if ct.can_build_conveyor(tile, f):
            ...
            self.link_queue.pop(0)
            # DELIVERY-SPUR REPAIR (see SPUR_ON): the only place a segment
            # enters this unit's repair memory is the moment it personally
            # lays it, so "the link's owner" is exact by construction.
            if SPUR_ON:
                self._spur_remember(tile)
            return True
        return False
