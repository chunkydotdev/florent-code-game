...
# RESERVE AGREEMENT (the v70cg-era converter/spawner agreement, never merged).
# Measured wild in the archb decode as a LIQUIDITY TRAP: a besieged Core
# converted 2,339 Ti into ammunition nothing could fire while its bank sat
# pinned 2 Ti BELOW the scaled harvester price for 500+ rounds, so neither the
# POP_FLOOR refill (bank-gated at get_builder_bot_cost()) nor a harvester
# rebuild could ever clear its price.  Piece E1 already reserves for the
# harvester, but only on the peacetime side of the siege latch (`not under`),
# and no reserve at all has ever existed for the refill -- exactly the two
# spends a siege needs most.  The agreement: while either spend is actually
# wanted, the conversion floor sits ABOVE its price instead of at 12, so the
# drip can only ever spend genuinely surplus titanium.  RA_MARGIN keeps the
# floor strictly above the price so the >-comparison in the drip still passes
# the bank through intact.
RA_ON = True
RA_MARGIN = 4
...
class Player:
    ...
    def _core(self, ct):
        ...
        # RESERVE AGREEMENT (see RA_ON) -- pure hoist, no logic changed.  The
        # spawn-cap lines, the replacement accounting and pop_refill below are
        # the ORIGINAL statements from the spawn block, moved above the ammo
        # branch verbatim so the conversion floor can read the very predicate
        # the refill spawns on rather than an approximation of it.  Every input
        # (w/h/p/harv/units/weapons) is already final at this point and none of
        # them is touched by conversion, the accounting still runs exactly once
        # per Core turn and before _live_builders as it always did, and the
        # explanatory comments stay with their consumers below.
        snowflake_home_b = (
        ...
        if sporks_ammo:
            ...
            # RESERVE AGREEMENT (see RA_ON).  Two raises, both floor arithmetic
            # only: pop_refill is the spawn block's OWN predicate (hoisted, not
            # restated), and the second clause is E1's own reserve with `not`
            # dropped, i.e. the same capped harvester price carried across the
            # siege latch on the small RA_MARGIN instead of E1's peacetime 23.
            if RA_ON:
                if pop_refill:
                    ti_floor = max(ti_floor, ct.get_builder_bot_cost() + RA_MARGIN)
                if E1_AMMO_FLOOR_ON and under:
                    ti_floor = max(ti_floor, min(ct.get_harvester_cost(), E1_RESERVE_CAP) + RA_MARGIN)
        ...
        # spawn_cap is computed with the ammo block above (RESERVE AGREEMENT).
        ...
        # units and the accounting itself run with the ammo block above.
        ...
        # pop_floor/pop_refill are computed with the ammo block above.
        pop_expand = (
            POP_FLOOR_ON
