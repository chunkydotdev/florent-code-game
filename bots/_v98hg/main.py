...
# PIECE FB -- the fjordgate bootstrap (fjordgate-collapse-owner read,
# 2026-08-08).  The det discriminator showed the under-siege latch running a
# ZERO-HARVESTER economy for an entire game on fjordgate seat B: the r1
# builder-radius latch put every role on the siege posture, and the one eco
# seat (role 3) was money-gated by the siege reserve (bank 24-25 < 20+16=36
# for 98 straight rounds in the wb arm, expander standing next to ore the
# whole time).  Wild CAD builds its fjordgate harvester at r4 -- no wild
# zero-harvester game exists -- so the collapse is strictly worse than what
# the latch is defending against.  Principle, applied at exactly two call
# sites and nowhere else: THE TEAM'S FIRST HARVESTER OUTRANKS THE SIEGE
# POSTURE.  (B) harv == 0 exempts the siege reserve at _expand's money gate;
# (C) a shelled defender standing next to buildable ore with harv == 0
# builds it before spending the round on defence.  Deliberately NOT touched
# (same read, menu items A/D/E): the d^2<=16 builder trigger radius --
# anchor-measured, and seat A's healthy opening depends on the 18>16
# accident, footprint-"fixing" it would hand seat A the same latch -- the
# sighting write, and the CB bleeding waiver.
FB_ON = True
...
# PIECE HG -- PUBLISH THE HOME-GUN FACT INSTEAD OF EACH UNIT GUESSING IT.
#
# `_live_home_gun` asks "does a friendly turret stand within HUNT_BAND_DSQ=41 of
# the Core" and answers it from the CALLER's vision, which is r^2=20 for a
# builder.  Band radius 6.40 tiles; builder sight radius 4.47.  A unit standing
# exactly ON the Core still cannot see d^2 in (20, 41] -- 21 of the band's 41
# units of range, 51% of it.  So the predicate is not merely position-dependent,
# it is structurally incapable of answering its own question from ANY standing
# point.  And since it returns True only on a turret it can actually see, every
# error it makes is a FALSE NEGATIVE.
#
# WHERE THAT COSTS US.  At the two counterbattery sites a false negative reads
# as "no home defence exists": `_cb_over_heal` returns `not <this>`, so the
# defender skips its heal and buys another Sentinel beside the ones already
# standing.  MEASURED FIELD-WIDE over 1,170 archived games, our v64-v81, all
# opponents: **2.76 in-band turret builds per game, 1.67 of them REDUNDANT --
# 60.5% of every home-band turret we build goes up while another of ours is
# already standing in band.**  Stable across recent versions (v75 2.09, v77
# 2.87, v79 1.66, v80 1.96 redundant/game), so this is not a regression anyone
# introduced and is unlikely to self-correct.  The rate tracks opponents who
# pressure our home band -- CtrlAltDefeat 72.5%, Lunds 70.2%, Kings College and
# OopsGotYourElo ~70%, down to Team 48 at 0.66/game -- which is expected, and
# means SOME of it is legitimate cover of distinct lanes.
#
# SO THE TARGET IS NOT ZERO.  `_cb_over_heal`'s intent explicitly includes a
# second turret covering a second lane, and the metric cannot separate "covers a
# new lane" from "duplicates cover" -- it counts any concurrent in-band turret.
# The fix must move redundant builds materially below 1.67 WITHOUT the in-band
# total collapsing from 2.76; a collapse would mean a turret-suppressor rather
# than a redundancy fix, and the defender still has to answer real threats.
#
# THE FIX.  The Core publishes the bit once per round for the whole team.  It is
# the right publisher three ways: vision r^2=36 covers 36/41 = 88% of the band
# against a builder's 49%; it sits at the band's centre by construction rather
# than wherever a builder wandered; and it is a SINGLE writer, so the value is
# one coherent team fact per round instead of a different answer per unit.  No
# new slot (see SLOT_HG_LIVE) and no extra engine call in the hot path -- the
# scan reuses the `_ents` list the Core already captured, and the Core has its
# own 10ms budget, so it is free from every builder's point of view.
#
# NOT REWIRED, DELIBERATELY: hive_freeze.  Its false negatives are BENIGN -- the
# freeze arms only when the predicate is True, so a miss means the economy keeps
# running, and the freeze is the harmful behaviour.  Feeding it an accurate
# signal would make it arm more reliably and the harm go UP.  An accurate signal
# into a bad gate is worse than a broken one; that gate's behaviour has to be
# settled on its own terms.
#
# NOT MONOTONE, deliberately, and this is the one that looks wrong at a glance:
# `_sync_harvesters` protects a partial view by never erasing a higher reading,
# but that is wrong here.  A turret inside the Core's r^2=36 that stops being
# visible has been DESTROYED -- turrets are immovable -- so preserving a stale
# True is precisely the SLOT_HOME_GUN rubble bug this piece exists to end.  And
...
# PLANK FT -- THE FERRY TEST.  An enemy builder can only enter play on the ring
# immediately surrounding its own Core's 2x2 footprint (engine: spawn_builder
# takes "a tile immediately surrounding the core's footprint", so manhattan <= 2
# from the nearest footprint tile, corners included), and thereafter it moves at
# most one CARDINAL tile per round.  So at round r no honestly-walked enemy
# builder can be farther than r + 2 manhattan from that footprint, and
#
#     manhattan(enemy builder, their Core footprint) > r + 2  =>  it was THROWN.
#
# This depends on no opponent constant at all -- only on the move rules and the
# spawn ring -- so unlike every behavioural tell it cannot decay when an
# opponent ships a new version.  Measured (recognition study, 880 games):
# precision 1.000 with ZERO false alarms, recall 0.53, fires median r5, which is
# a median 5 rounds EARLIER than the shipped SLOT_UNDER latch in the 68% of
# games where both fire, plus 27 games where the shipped latch never fires.
#
# TRIGGER ONLY.  It writes the existing SLOT_UNDER (widened 0/1 -> 0/1/2, see
# below) and the existing SLOT_ATK_RND, so the entire payoff is that the
# insertion responses this file already carries latch earlier.  No new response
# code, no new slot, no new scan: the test rides inside the two enemy scans that
# already run (Core _core, builder _builder).
FERRY_ON = True
# Extra manhattan slack on top of the +2 spawn ring.  0 is provably sound with
# the anchor convention used here: SLOT_ENEMY_CORE / enemy_core_for hold the NW
# tile of the enemy 2x2 footprint (same convention as ct.get_position on a Core,
# and the same convention the study validated), and ferried() measures to the
# NEAREST footprint tile, not to the anchor.  Kept as a named knob because a
# false alarm is worse than a late fire; note the study measured recall
...
SLOT_ENEMY_CORE = 3
SLOT_HARVESTERS = 4
SLOT_HG_LIVE = 5   # PIECE HG -- repurposed dead slot (4 writes, ZERO reads on
                   # the shipped v83 bytes; verified by both arms, no numeric
                   # read_store(5) and no alias).  Its four writes were each the
                   # sole body of an `if`, so those ifs went too.  Encoding:
                   # 0 = never published (fall back to the local scan),
                   # 1 = no home gun, 2 = one stands.  0 deliberately does NOT
                   # mean "no gun" -- that is the store default and would hand
                   # every unit a confident false before the Core has spoken.
SLOT_LAUNCHER = 6
SLOT_HOME_GUN = 7
SLOT_DROPPED = 8
...
# ============================================================================
# PIECE E1 (RING) -- HOME COUNTER-BATTERY RING.
# docs/e1-incoming-side-design-2026-08-08.md, sections 2 / 5 / 6.
#
# NAME COLLISION, deliberate and load-bearing to read: the E1_AMMO_FLOOR_ON
# block directly above is an unrelated shipped plank that also carries the E1
# tag (the peacetime ammo floor).  Everything in THIS block and every comment
# tagged "PIECE E1 (RING)" belongs to the counter-battery ring; the two share
# nothing but the letter.
#
# THE MEASURED PROBLEM (design doc section 1, Eir 8 production read 2.4):
# against the CAD family our staffing is fine -- 3.32 bodies per damage-round,
# heal/incoming 0.82-0.97 -- and cores die anyway, because incoming is 18.06
# HP/r against a heal line that delivers 11.55.  A 6.5 HP/r structural deficit
# is not closable by more heal (eight staffed seats is 32 HP/r and we seat
# 3.3); it is a DAMAGE-SUPPRESSION problem wearing a heal-line costume.  The
# lever is killing the source turrets.
#
# THE DECODED ASYMMETRY THAT MAKES SUPPRESSION CHEAP (refreeze spec 4.2, design
# doc section 2): CAD answers turrets planted within d^2 <= 36 of THEIR core --
# counter-gunner at d^2 <= 13, 100% coverage, 76% of our sentinels killed,
# median life 27 rounds.  Beyond d^2 ~ 64 of their core it does essentially
# nothing: our turrets survive 237-377 rounds, 11-19% killed.  Their siege
# turrets shelling OUR core are by construction far from THEIR core, i.e. the
# home ring plants inside the zone where the decoded mechanism structurally
# cannot answer it.  95.6% of their counter-damage is gunner counter-plants and
# those only trigger near their own core.
#
# The three mechanisms below are PORTED from the parked _v83c1b dev branch
# (verdict KEEP-dev, 85% at wild-median load), re-cut for that envelope.  v80
# and its descendants have none of them: _try_counterbattery builds only from
# wherever a builder ALREADY stands with a legal ray, nothing walks a builder to
# a firing position, nothing caps or dedups home coverage, nothing answers a
# second simultaneous threat.
E1_RING_ON = True
# FJORDGATE GUARD, clock half (design doc 6.5).  No ring build -- and no ring
# WALK, which is the same thing one round later -- before this round on any
# path.  The measured precedent is in _hunt_turret's own comment block: ambient
# early aggression cost eider 8/16 -> 0/16 AND produced a fjordgate rush
# regression, and the fjordgate-collapse read is the reason this whole plank was
# sequenced behind it.  Arming itself is unaffected: it spends nothing, and the
# insertion signature legitimately fires at r3-r8.
E1_RING_MIN_RND = 4

# RING CAP 3 (C1b line 649).  Their established forward battery tops out at 3 in
# the corpus, and cost scale is ONE team-wide multiplier (+20% per sentinel), so
# an uncapped ring taxes the economy that wins these games.  Paired with
# COVERAGE DEDUP: if a live home turret's CURRENT facing already covers the
# threat tile, a second one buys no extra shots at it.
HOME_RING_CAP = 3
# Return-fire penalty in BFS steps (C1b line 657) on a firing position inside a
# live GUNNER threat's own range (GUNNER_RANGE_DSQ = 13).  A SENTINEL threat
# outranges everything we can build (r^2 = 32 both ways) so no penalty can
# separate spots against one and none is applied.  8 is deliberately below the
# 20 the in-own-ray penalty carries: taking return fire is a cost, standing in
# our own line is a broken build.
HOME_RING_RETFIRE_PEN = 8
# APPROACH LANE, footprint-dsq (C1b line 674).  The ray-coverage decode
# (docs/research/kcm-win-c1-validation-2026-08-07.md): of 23 enemy turrets that
# reached our home band, the 8 that lay on a reachable friendly firing ray died
# 8/8 and the 15 that did not took ZERO turret shots -- perfect separation,
# where radius-and-count went 3/5.  9 is that decode's own number.  It is ranked
# ABOVE walking distance in the planner because A SENTINEL CANNOT BE RE-AIMED:
# rotate() is gunner-only and the decode found 0 direction changes across 277
# turret re-emissions, so a facing chosen for today's threat tile alone is dead
# weight the round that threat dies.
HOME_RING_LANE_DSQ = 9
# RE-CUT vs C1b (design doc 6.1).  C1b took the first AFFORDABLE type in
# PRIMARY_SENTINEL order.  Against the CAD envelope the sentinel is the correct
# home gun on arithmetic AND on the asymmetry: 18 dmg on reload 2 TWO-SHOTS the
# 25 HP gunner class that is their shelling offence, and their counter-plant
# only fires near their own core, so a home sentinel goes unanswered where a
# forward one is killed 76% of the time.  Applied only when the threat tile
# actually holds a turret -- against a raider bot the extra 10 Ti and the +20%
# scale buy nothing the gunner does not.
E1_RING_SENTINEL_FIRST = True

# PIECE E1 (RING), MECHANISM A -- INSERTION-CLASS ARMING GATE (C1b line 736).
# The ring taxes the games it was never built for: the _v82c1 gate row measured
# the mechanism 8/8 lethal where it fired while the leg against the
# non-insertion slot holder came in at 41.7 against the parent lineage's 52.5.
# The fix is not a weaker ring, it is a ring that does not exist until a
# signature has been SEEN.  Arming spends no titanium and builds nothing.
#
# SIGNATURE 1 -- EARLY RAIDER (ported unchanged).  An enemy BUILDER_BOT at
# footprint-dsq <= 64 of our Core before round 30 THAT CANNOT HAVE WALKED THERE.
# Radius-and-clock alone was measured refuted: opp_v63, a non-insertion
# opponent, tripped it in 12 of 12 games with its own early saboteur.  Distance
# and time do not separate the classes; the THROW does, and the throw IS the
# class.  The test is a travel budget -- a builder bot moves one CARDINAL step
# per round at best and the earliest one can exist is spawned adjacent to its
# own Core footprint on round 0, so at round R nothing that walked is more than
# R + 1 Manhattan steps from the enemy footprint.  Deliberately the MOST
# PERMISSIVE walk model, so every assumption tilts toward calling an arrival a
# walk and the gate errs toward the parent lineage.  Smoke-measured 2026-08-07:
# 12/12 armed vs cad_probe (rounds 3-8), 0/12 vs opp_v63 -- perfect separation.
#
# SIGNATURE 2 -- turret arrival at any round.  Kept DISABLED exactly as C1b
# parked it (-1): it armed vs opp_v63 in 14/16 smoke games at median r29, i.e.
# it re-imported the tax the gate exists to remove, and the UNARMED path already
# answers a parked forward snipe with the parent's single-gun counterbattery.
#
# SIGNATURE 3 -- SHELLING SHAPE (NEW, design doc 6.2).  Signature 1 keys on the
# THROW, and the CAD-family shape this plank targets is standoff shelling, not
# raiding: against pure shelling signature 1 may never arm at all and the ring
# would never exist in the games it was designed for.  Two or more DISTINCT
# enemy GUNNER/SENTINEL entities inside footprint-dsq 41 of our Core, any round,
# is the s14 predictor's shape stated on the band constant this file already
# shares (HUNT_BAND_DSQ, "past sentinel range, footprint-measured").  Two, not
# one, is the whole separation: one turret at our door is a forward snipe, which
# the disabled signature 2 armed on and was refuted for; two is a battery.
# Turret-signature re-tax risk is priced by the ablation grid, not assumed away.
#
# DIVERGENCE, accepted and carried over from C1b: arming is PER UNIT INSTANCE,
# derived from that unit's own observation.  There is no shared channel -- all
# 16 store slots are assigned, and the two that fire on attacks cannot carry it
# (SLOT_UNDER is written 0 by the Core when its 50-round latch decays;
# SLOT_ATK_RND is consumed as an arithmetic round number by that same decay;
# SLOT_HOME_GUN has a live reader in _core).  So different units arm on
# different rounds and a defender whose own vision never contained the raider
# stays unarmed while a builder that saw it does not.  It errs toward the parent
# lineage, which is the safe side of this trade.
C1B_ARMING_GATE_ON = True
C1B_ARM_RAIDER_DSQ = 64
C1B_ARM_RAIDER_RND = 30
# Signature 2's radius; -1 = ABLATED, which is the shipped C1b verdict.
C1B_ARM_TURRET_DSQ = -1
# Signature 3 (new).  Band = HUNT_BAND_DSQ's number, stated as its own constant
# so the ablation grid can move it without touching the hunt band.
E1_ARM_SHELL_DSQ = 41
E1_ARM_SHELL_N = 2

# PIECE E1 (RING), MECHANISM B -- COVERAGE SUPPLY / SECOND RESPONDER (C1b line
# 787, ported as-is).  The other half of the _v82c1 gate row: coverage was
# SUPPLY-BOUND.  18 of 60 games were ZERO-covered against 6.9 establishments per
# game, because the answering side is ONE role-4 defender holding ONE plan
# against ONE SLOT_THREAT tile -- and that defender is also the unit the heal
# line, the link queue and _sabotage_prio compete for.  The wild simultaneity
# budget says the common case needs two hands, not seven (max simultaneous alive
# at fp-dsq <= 36 is median 2), so this adds exactly ONE more responder, never a
# squad.
#
...
# SLOT_THREAT is by construction one the defender is not planning against, since
# the defender's plan is justified by SLOT_THREAT and dies with it.  The
# equal-tile case is admitted only when SLOT_DEFEND_BEAT says no defender has
# beaten for C1B_SUPPLY_BEAT_STALE rounds (or never at all, which is the opening
# before the fifth builder exists -- exactly the window an r2-r5 insertion lands
# in).
#
...
# any competitor standing closer to SLOT_THREAT than to this threat -- that hand
# is the defender's threat's business.  Vision is per-unit, so two helpers on
# opposite sides of a wall can both elect.  That race is BOUNDED, not prevented:
# _home_ring_check runs immediately before the build on the losing unit's next
# action and aborts on the turret that now covers, and HOME_RING_CAP still caps
...
# PIECE HF -- the hive economy self-freeze (see _expand). Kept as a toggle so
# the removal is ablatable; measured strictly positive at ablation.
...
# PIECE E1 (RING), SITING BAN 2 -- THE DELIVERY CHAIN.  Decoded meander game:
# our own counterbattery sentinel, built at r5 under the core-bleeding waiver,
# landed directly on our planned link path.  The belt was then built on BOTH
# SIDES of it, stacks froze at the break and the delivery pipeline never
# recovered for the rest of the match.  A turret is impassable AND permanent --
# the same property PLANK HS's _seat_ban already protects the heal seats
# against -- so the ring has to refuse chain tiles for exactly the same reason
# it refuses seats, and the ring is strictly more dangerous here than the base
# counterbattery because it WALKS to a chosen tile instead of taking whatever
# happens to be adjacent.
#
# Three tile classes, all BUILD-only: walking through a belt is free and
# conveyors are bot-passable, so nothing here touches routing or the terrain
# flood.
#  1. self.link_queue -- the planned path.  This is the measured case: the tile
#     is EMPTY when the turret is sited, so neither can_build_* nor the
#     planner's occupancy test can see the collision coming.
#  2. Standing own conveyors/splitters.  Belt and braces: an occupied tile
#     already fails the occupancy test and can_build_*, so this arm is an
#     invariant rather than a behaviour change.
#  3. GAP TILES -- an empty tile with our own belt on BOTH sides along one axis.
#     This is the severing tile in a chain that is already laid, which neither
#     (1) nor (2) catches: not queued by this unit, not itself a conveyor.  It
#     is the same failure the decode describes, reached from the other
#     direction, and it is pure-Python set arithmetic over the scan (2) already
#     pays for.  Beyond the literal spec, hence its own constant.
E1_RING_LINK_BAN_ON = True
E1_RING_LINK_BAN_GAP = True

# PIECE E1 (RING), COMPONENT 4 -- CONVERT-CAP LIQUIDITY (design doc 6.4).  The
# working-magazine drip in _core converts at most 16 Ti per turn, which is four
# gunner shots but ONE sentinel shot plus change.  The wb evidence (_v93wb
# archb: an r732 loss becoming an r1000 WIN, with the RA-off control
# byte-reproducing the loss) shows ammo LIQUIDITY gates the home response
# class-wide -- whatever the ring plants must not be dry at the moment of
# contact.  24 funds a sentinel duel same-turn.  Deliberately NOT the wb
# reserve-agreement branch, which failed its re-gate 0/24 vs cad_probe: just the
# cap, and only while the ring is armed AND a turret is actually standing to
# drink it.  ammo_target and ti_floor are untouched, so this raises the RATE the
# magazine fills at, never the amount held or the floor it stops at.
E1_CONVERT_CAP = 24
# ============================================================================
...
# PLANK OS -- OPENING STANDOFF SENTINEL (timing, siting, funding of a
# behaviour this file already has).  Sources: the ouroboros re-freeze spec
# (docs/research/ouro-probe-refreeze-spec-2026-08-08.md) SS4 D-CRITICAL and
# SS5.1, plus docs/research/opponent-recognition-feasibility-2026-08-08.md
# SS3.3 (this lever's commitment window closes before recognition can ever
# name the class -- so it is gated on MAP GEOMETRY or shipped unconditionally,
# never on a behavioural read).
#
# THE MEASURED CASE.  Against wild Ouroboros v8 (40 games, we won 7) every one
# of our 3 core-kill wins ran through an early standoff Sentinel that strangled
# their economy; win-median first standoff-Sentinel round r5, loss-median r15.
# Their entire kit answers a turret outside Gunner range (r^2=13) only by
# walking a builder out to plant a Gunner within 13 OF THE TURRET, and they
# hate that excursion: median 1 builder lost per game against our 12.  The
# honest bound from the same section: we already plant a standoff Sentinel in
# 39 of 40 games and still lose 33 -- 63 of 78 die, median lifespan 25 rounds.
# So the lever is TIMING (r5 vs r15) and keeping the tile out of their
# Gunner-plantable set, NOT the plant itself.  Nothing here adds a mechanism.
#
# WHAT WAS BINDING (measured, meander vs ouroboros_probe, instrumented run).
# _plan_siege already picks a Sentinel tile whose ray hits the enemy Core, and
# on a small-D map that tile is 2-3 steps from our own door: the unhijacked
# plant lands r2.  What delays it is the SABOTEUR RECALL -- the
# `role in ("saboteur", "launchwait") ... distance_squared(self.core) <= 25`
# block in _builder hands the turn to _home_defend for every round an enemy
# builder loiters inside d^2 <= 20 of our Core, and on a small-D map the whole
# standoff route lies inside that 25-radius, so the recall can hold the plant
# for a dozen rounds (measured r4-r14 in one game).  The melee-first branch at
# the top of _saboteur is the same shape one level down.  OS therefore does not
# add a builder or a build path; it exempts the ONE designated early builder
# (role_n == 0, pre-plant, inside the opening window) from both.
OS_ON = True
# "geometry" = fire only where the core gap is inside OS_D_SQ_MAX, computed at
# r0 from the two Core anchors (SS5.1: D <= 12 is the r2-r10 opening-gunner
# regime, where the timing lever is sharpest).  "always" = unconditional.
# Geometry mode stays silent on an unrecognised map, where enemy_core_for
# falls back to a reflection and D would be a guess.
OS_GATE_MODE = "geometry"
OS_D_SQ_MAX = 49
# Opening window.  Past it the saboteur is an ordinary siege engineer again,
# recall and all -- this buys the FIRST standoff turret, nothing later.
OS_EARLY_RND = 40
OS_HOMEDEF_EXEMPT = True
# SITING (SS4's honest bound).  Their only answer is a Gunner planted within
# r^2 = 13 of our Sentinel, so among the tiles _plan_siege already validated
# prefer the one with the fewest tiles inside that disk they could plant on.
# Walls, ore and Core footprints shrink the set.  Re-ranked over the top-K
# rows of the existing sort ONLY, so a quieter tile is never bought with route
# distance -- the timing lever dominates, and every candidate already carries
# the firing line onto the enemy Core approach that the turret exists for.
OS_SITE_TOPK = 6
OS_SITE_DSQ = 13
# FUNDING.  A Sentinel is 10 ammunition a shot and the peacetime magazine
# target is AMMO_FLOOR = 16 -- one shot.  While the gate is armed, a turret
# stands and the game is still young, hold 24: two banked shots by the time
# their first builder or Gunner walks into the lane.  The 16 Ti/turn drip and
# the E1 peacetime harvester reserve (ti_floor) are untouched, so this costs
# at most 8 Ti more than today and cannot dip below the harvester floor.
OS_AMMO_TARGET = 24
OS_AMMO_RND = 120

# PLANK SP -- SITE PREDICATE (plant-time facing veto).  Siting only; no new
# mechanism, no new scan, no titanium.  Source:
# docs/research/ptp-feasibility-2026-08-08.md SS1.5 (85 CAD-family games, 280
# siege turrets, 0 unattributed damage), promoted in SS1.6.
#
# THE MEASURED CASE.  Median latency from our siege plant to the plant of the
# turret that kills it is ZERO: 53% of the killers were ALREADY ON THE BOARD
# when we planted.  The predicate the same corpus fits is
#
#   AIMED-EXPOSED(tile) := some live enemy Gunner/Sentinel is 8-way aligned
#   with `tile`, inside its attack range, AND IS ALREADY FACING THAT WAY.
#
# It flags 25% of our near-core plants; those die 87% of the time with a median
# life of 5 rounds and 3 shots, against 23 rounds and 13 shots for unflagged
# plants (v117-only: 92% / 4.0; vs OpenSverige: 96% / 4.0).  It survives
# stratification on live enemy turret count, plant distance and plant round,
# holds in 14/15 matches and 4/5 opponents -- a generic geometry law, so it
# needs no opponent identification and inherits the recognition refutation.
#
# WHAT CARRIES IT -- THE FACING TEST, NOT THE ALIGNMENT.  "Ray-exposed but NOT
# yet aimed" is statistically indistinguishable from clean (25.0 rounds vs
# 23.0) and would flag 55% of plants for nothing; alignment + CURRENT facing
# flags 25% and buys the whole separation.  Their rotate() costs 10 Ti and a
# cooldown, which is presumably why an un-aimed ray is not a real threat.  So
# this file tests coverage by the facing the turret HAS, never by alignment.
#
# FEASIBILITY.  68 of the 69 flagged plants had a non-exposed alternative tile
# in the same band (median 12 free of 26), so the answer is one tile over, not
# a strategy change -- hence a veto that re-sites and NEVER refuses to plant.
SP_ON = True
...
def core_anchor_exact(w, h, own):
    """True when enemy_core_for's answer came from the CORE_PAIRS table.

    PLANK FT's guard.  enemy_core_for falls back to a 180-degree reflection,
    and several ladder maps are mirror-symmetric instead (see CORE_PAIRS), so
    on an unrecognised map the fallback anchor can be most of a board away from
    the real enemy Core.  The ferry test measures distance FROM that anchor, and
    an anchor that is too far away MANUFACTURES false positives -- which this
    test may not have: zero false alarms is its whole identity.  So it runs only
    where the pairing is exact, and stays silent (degrading to today's
    behaviour) on an unknown map.
    """
    for mw, mh, ax, ay, bx, by in CORE_PAIRS:
        if w == mw and h == mh and (
            (own.x == ax and own.y == ay) or (own.x == bx and own.y == by)
        ):
            return True
    return False


def os_gate_open(w, h, own):
    """PLANK OS's gate: may this game run the opening standoff Sentinel?

    OS_GATE_MODE "always" ships it unconditionally.  "geometry" fires only
    inside OS_D_SQ_MAX, the small/mid core gap where the r2-r10 opening regime
    lives -- and only where the Core pairing is EXACT, because enemy_core_for
    otherwise falls back to a 180-degree reflection and several ladder maps are
    mirror-symmetric, which would make D a guess (same guard as PLANK FT's).
    Both anchors are the NW tile of a 2x2 footprint, so anchor-to-anchor is
    centre-to-centre distance -- the D of the spec's SS5.1 table.
    """
    if not OS_ON:
        return False
    if OS_GATE_MODE != "geometry":
        return True
    if not core_anchor_exact(w, h, own):
        return False
    return own.distance_squared(enemy_core_for(w, h, own)) <= OS_D_SQ_MAX


def ferried(ep, anchor, rnd):
    """PLANK FT: could a builder standing at ``ep`` on round ``rnd`` have walked
    there from the Core whose footprint anchor is ``anchor``?  If not, it was
    thrown by a Launcher.  ``anchor`` is the NW tile of the 2x2 footprint, so
    the footprint spans [x, x+1] x [y, y+1] and the distance is taken to the
    nearest footprint tile.  ``anchor`` None (unknown map, or the store write
    has not landed yet) means "do not know" -- never "ferry".
    """
    if anchor is None:
        return False
    dx = 0 if anchor.x <= ep.x <= anchor.x + 1 else min(
        abs(ep.x - anchor.x), abs(ep.x - anchor.x - 1))
    dy = 0 if anchor.y <= ep.y <= anchor.y + 1 else min(
        abs(ep.y - anchor.y), abs(ep.y - anchor.y - 1))
    return dx + dy > rnd + 2 + FERRY_SLACK
...
def fp_dsq(pos, o):
    """PIECE E1 (RING).  Footprint-dsq: min squared distance from pos to the
    2x2 Core at o.
    ...
    axis -- but without four Position constructions and four method calls.  The
    arming gate calls it once per visible ENEMY entity inside the sensing loop
    that already runs every round, so this difference IS the whole cost of the
    gate's unarmed state.
    """
    dx = pos.x - o.x
    dy = pos.y - o.y
    if dx > 1:
...
def fp_man(pos, o):
    """PIECE E1 (RING).  Footprint MANHATTAN distance -- the walk-feasibility
    metric behind arming signature 1.

    Builder bots move one CARDINAL step at a time, so Manhattan (not Chebyshev,
    not euclidean) is the fewest rounds a walk from the 2x2 Core at o to pos can
    possibly take.  Same product-set separation as fp_dsq.
    """
    dx = pos.x - o.x
    dy = pos.y - o.y
    if dx > 1:
...
class Player:
    def __init__(self):
        ...
        self.map_grid = None
        self.map_walls = set()
        # PLANK FT: enemy-Core footprint anchor for the ferry test, resolved
        # once per unit (None = unknown map, test disabled for this unit).
        self.ferry_anchor = None
        self.ferry_anchor_done = False
        # PLANK OS gate, resolved once per unit (None = not asked yet).  Pure
        # function of map dimensions and the Core anchor, so every unit that
        # asks gets the same answer and no store slot is needed -- which
        # matters, all 16 are already spoken for.
        self.os_gate = None
        ...
        # PIECE E1 (RING) home-ring plan.  Per unit instance, on the same
        # argument as the siege plan beside it: only the unit that made the plan
        # ever reads it, so no store slot is spent -- which matters, all 16 are
        # already assigned.  homering_threat is the threat tile the plan was
        # made against: a plan may not outlive the threat that justified it.
        ...
        # PIECE E1 (RING) arming state and the helper's claim (see
        # C1B_ARMING_GATE_ON / C1B_SUPPLY_ON).  Both per unit instance and
        # neither shareable -- see the divergence note in the arming block.
        # c1b_armed is one-way: nothing ever clears it, because a game that has
        # shown an insertion or a battery does not stop being that game.
        # c1b_threat is the tile a supply helper claimed; while it is set it
        # OVERRIDES SLOT_THREAT for the ring planner (see _homering_target), so
        # the defender and the helper can hold plans against two threats at
        # once.
    ...
    def _ferry_anchor(self, w, h, own):
        """PLANK FT: the enemy Core's footprint anchor, or None where the test
        must stay silent.  Resolved once per unit -- CORE_PAIRS is walked twice
        on that one call and never again, and no engine call is involved."""
        if not self.ferry_anchor_done:
            self.ferry_anchor_done = True
            if FERRY_ON and core_anchor_exact(w, h, own):
                self.ferry_anchor = enemy_core_for(w, h, own)
        return self.ferry_anchor

    def _os_armed(self, w, h, own):
        """PLANK OS: cached os_gate_open for this unit."""
        if self.os_gate is None:
            self.os_gate = os_gate_open(w, h, own)
        return self.os_gate

    def _os_early(self, ct):
        """PLANK OS: is this unit the designated early builder, still owing the
        team its first standoff Sentinel, inside the opening window?

        role_n == 0 is already the saboteur that _plan_siege hands the primary
        SENTINEL to (PRIMARY_SENTINEL), so this designates nobody new.  A
        launcher-dropped raider is excluded: it did not walk here and has no
        opening to protect.  False the moment the plant lands (forward_guns) or
        the window closes, after which every caller behaves exactly as today.
        """
        return (
            OS_ON
            and self.role_n == 0
            and self.role == "saboteur"
            and not self.dropped
            and self.forward_guns == 0
            and self.core is not None
            and ct.get_current_round() <= OS_EARLY_RND
            and self._os_armed(self.mw, self.mh, self.core)
        )

    def _os_plantable(self, spot, blocked):
        """PLANK OS siting: how many tiles inside a Gunner's r^2 = 13 disk of
        ``spot`` could they actually plant that answering Gunner on?

        Terrain proxy, deliberately: walls and ore are never Gunner seats and
        the two Core footprints are permanently occupied, so those are the
        shrinkers that exist at plant time.  Their own conveyors shrink it
        further later, which this cannot see and does not claim to.  Called at
        most OS_SITE_TOPK times, once, at the moment a site is chosen -- never
        per round; 45 tiles per call over an already-decoded grid.
        """
        n = 0
        for dy in range(-3, 4):
            y = spot.y + dy
    ...
    def _sp_veto_tiles(self, ct):
        """PLANK SP: tiles already covered by a live, visible enemy turret's
        CURRENT facing -- the AIMED-EXPOSED set of the SP constants block.

        Geometry is the engine's own: get_attackable_tiles_from(position,
        facing, type) is the raw attack pattern of a turret standing exactly
        where that one stands, pointing exactly where it points, so a Gunner
        that is aligned with a tile but facing elsewhere contributes nothing --
        which is the whole finding.  The facing is re-read by id THIS turn, so
        a rotation since the sighting is picked up, and a turret that has died
        or left vision raises and is dropped rather than vetoing from memory.
        can_fire_from cannot answer this question: for Gunners it requires the
        TARGET TILE TO BE OCCUPIED, and a candidate site is by construction
        empty.

        The one correction on top of the engine call: a raw Gunner pattern
        includes tiles behind walls by contract, and a wall makes that shot
        inert, so decoded terrain (already in memory, no call) cuts those back
        out.  Sentinels ignore obstacles and are taken whole.  Called once, at
        the moment a site is chosen -- never per round.
        """
        tiles = set()
        for eid, ep, et in self.sp_turrets:
            try:
                facing = ct.get_direction(eid)
                pattern = ct.get_attackable_tiles_from(ep, facing, et)
            except Exception:
                continue
            if et == EntityType.GUNNER and self.map_walls:
                dx, dy = facing.delta()
                for t in pattern:
                    steps = max(abs(t.x - ep.x), abs(t.y - ep.y))
                    if any(
                        (ep.x + dx * s, ep.y + dy * s) in self.map_walls
                        for s in range(1, steps)
                    ):
                        continue
                    tiles.add((t.x, t.y))
            else:
                tiles.update((t.x, t.y) for t in pattern)
        return tiles
    ...
    def _core(self, ct):
        ...
        # PLANK FT rides this scan: same iteration, same getters, no break of
        # its own, so the existing latch/THREAT branches below are reached in
        # exactly the order and with exactly the results they are today.
        anchor = self._ferry_anchor(w, h, p)
        # PIECE E1 (RING).  The Core needs its own armed bit for the convert-cap
        # component below -- arming is per unit instance and no store slot can
        # carry it (see the arming block).  The entity list is captured once and
        # shared, so this adds no get_nearby_entities call; the scan itself runs
        # only while the ring is on AND this instance is still unarmed, and it
        # cannot be folded into the loop below because that loop BREAKS on the
        # first qualifying sighting and signature 3 has to count two.
        _ents = ct.get_nearby_entities()
        # PIECE HG -- publish the home-gun bit for the team (see HG_PUBLISH_ON).
        # Reuses the _ents capture above: no extra get_nearby_entities call.
        if HG_PUBLISH_ON:
            _hg = HG_NONE
            _ctiles = core_tiles(p)
            _myteam = ct.get_team()
            for _eid in _ents:
                try:
                    if ct.get_team(_eid) != _myteam:
                        continue
                    if ct.get_entity_type(_eid) not in (EntityType.GUNNER, EntityType.SENTINEL):
                        continue
                    if min(t.distance_squared(ct.get_position(_eid)) for t in _ctiles) <= HUNT_BAND_DSQ:
                        _hg = HG_GUN
                        break
                except Exception:
                    continue
            ct.write_store(SLOT_HG_LIVE, _hg)
        if E1_RING_ON and C1B_ARMING_GATE_ON and not self.c1b_armed:
            _arm_ecore = unpack_pos(ct.read_store(SLOT_ENEMY_CORE))
            if _arm_ecore is None:
                _arm_ecore = enemy_core_for(w, h, p)
            self._e1_arm_scan(ct, _ents, p, _arm_ecore, rnd, ct.get_team())
        for eid in _ents:
            ...
            if et == EntityType.BUILDER_BOT and ferried(ep, anchor, rnd):
                under = True
                ferry = True
        ...
        if sporks_ammo:
            ...
            # PLANK OS, funding arm (see the OS constants block).  The opening
            # standoff Sentinel burns 10 ammunition a shot against a peacetime
            # target of 16, i.e. it banks ONE shot; hold two while the gate is
            # armed, a turret stands and the game is young.  Raises the target
            # only -- the drip rate, the E1 harvester reserve below and the
            # under-siege and endgame paths are all untouched.
            if (
                OS_ON and weapons and not under
                and rnd <= OS_AMMO_RND
                and self._os_armed(w, h, p)
            ):
                ammo_target = max(ammo_target, OS_AMMO_TARGET)
            ...
            # PIECE FB (B, the floor half): with ZERO harvesters the ammo
            # trickle may not spend the bank below harvester cost -- the
            # under-siege 12-floor is exactly what pinned the fjordgate-B
            # bank at hand-to-mouth forever (det instrument r10-392).  harv
            # here is last round's SLOT_HARVESTERS, same staleness as every
            # other reader; one round of lag is 2.5 Ti of error.
            if FB_ON and harv < 1:
                ti_floor = max(ti_floor, ct.get_harvester_cost())
            if not endgame_dumped and (under or weapons or harv >= 2) and ammo < ammo_target and ti > ti_floor:
                amt = min(self._e1_convert_cap(ct), ammo_target - ammo, ti - ti_floor)
    ...
    def _builder(self, ct):
        ...
        _threat_best = None
        _threat_best_d = 0
        # PLANK FT.  The builders are spread across the board, so this scan sees
        # a thrown raider several rounds before it walks into the Core's own
        # vision -- which is where the median 5 rounds of extra warning come
        # from.  Trigger only: SLOT_UNDER/SLOT_ATK_RND, the two the proximity
        # branch below already writes, and nothing else.  SLOT_THREAT is
        # deliberately NOT written here -- a sighting outside b_sense would
        # otherwise redirect the defenders, which is response behaviour, and
        # "latched with no fresh THREAT" is a state this file already reaches
        # (the Core's HP-drop latch).
        _ferry = False
        _anchor = self._ferry_anchor(self.mw, self.mh, self.core)
        _rnd = ct.get_current_round()
        # PLANK SP: the veto rides on this scan rather than adding one of its
        # own -- get_nearby_entities, get_team, get_entity_type and
        # get_position are all already called here, for every hostile, every
        # turn.  Cleared first: what the unit saw last turn is not evidence
        # about this one.
        if SP_ON:
            self.sp_turrets = []
        # PIECE E1 (RING), MECHANISM A -- the arming scan, piggybacked on the
        # loop below rather than added beside it: no new engine call and no
        # second iteration over nearby entities, which is the "unarmed costs
        # nothing" requirement met from both sides.  Once armed the whole thing
        # collapses to one False test per visible enemy; unarmed it is one
        # fp_dsq (six integer ops, no engine call) per visible enemy.  The
        # round is read once here -- `rnd` proper is not bound until after this
        # loop.  (The Core runs the same signatures through _e1_arm_scan; it
        # cannot ride its own scan because that one breaks early.)
        _arm_scan = E1_RING_ON and C1B_ARMING_GATE_ON and not self.c1b_armed
        _arm_rnd = ct.get_current_round() if _arm_scan else 0
        # Signature 3's tally: DISTINCT enemy turrets in band, this turn.
        # get_nearby_entities returns each entity once, so counting sightings
        # counts entities.
        _arm_shellers = 0
        # Signature 1's walk-feasibility anchor (see C1B_ARM_RAIDER_RND) -- last
        # round's store value, or the symmetry guess the Core itself publishes
        # from.  Both are available before any enemy Core has been seen and
        # neither costs an engine call.  A wrong anchor moves the travel budget,
        # not the shape of the test.
        _arm_ecore = None
        if _arm_scan:
        ...
        for eid in ct.get_nearby_entities():
            ...
            et = ct.get_entity_type(eid)
            ep = ct.get_position(eid)
            if SP_ON and et in (EntityType.GUNNER, EntityType.SENTINEL):
                self.sp_turrets.append((eid, ep, et))
            ...
            if _arm_scan:
                if et in (EntityType.GUNNER, EntityType.SENTINEL):
                    _fp = fp_dsq(ep, self.core)
                    # SIGNATURE 2 -- turret arrival, any round.  ABLATED (-1).
                    if _fp <= C1B_ARM_TURRET_DSQ:
                        self.c1b_armed = True
                        _arm_scan = False
                    # SIGNATURE 3 -- the shelling shape: a BATTERY, not a single
                    # forward snipe (see E1_ARM_SHELL_N).
                    elif _fp <= E1_ARM_SHELL_DSQ:
                        _arm_shellers += 1
                        if _arm_shellers >= E1_ARM_SHELL_N:
                            self.c1b_armed = True
                            _arm_scan = False
                elif (
        ...
        # PIECE E1 (RING).  A claim belongs to an ELIGIBLE helper only, and this
        # is the first line past every role override above, so the release
        # happens on the same turn eligibility is lost.  It cannot be left to
        ...
        if E1_RING_ON and C1B_SUPPLY_ON and self.c1b_threat is not None and (
        ...
        # PLANK OS exempts the designated early builder from this recall until
        # its standoff Sentinel is planted (see the OS constants block): this
        # is the MEASURED binding constraint on the r15-class first plant --
        # on a small-D map the whole standoff route lies inside this 25-radius,
        # so one loitering enemy builder holds the plant for a dozen rounds.
        # Exactly one unit, only pre-plant, only inside the opening window;
        # every other saboteur and every later round is unchanged.
        os_hold = OS_HOMEDEF_EXEMPT and self._os_early(ct)
        if (
            self.role in ("saboteur", "launchwait") and self.core
            and not keep_artillery_forward and not os_hold
        ...
        # PIECE E1 (RING), MECHANISM B -- the second responder, taken BELOW the
        # CPU guard above (it plans a BFS, unlike the melee emergency) and ABOVE
        # dispatch, so a recruited expander answers instead of laying its next
        # conveyor and is handed straight back the round it stops answering.  No
        # role is written, so there is no lock to release: _c1b_supply returning
        # False falls through to exactly the dispatch below.
        if E1_RING_ON and self._c1b_supply(ct):
            return
    ...
    def _plan_siege(self, ct):
        ...
        # PLANK SP -- the plant-time facing veto (see the SP constants block).
        # Placed HERE, between the sort and every consumer of it, so the veto
        # runs FIRST and the pick below and the OS re-rank further down both
        # range over survivors only.  That order is the measured one: exposure
        # is a survival test (87% dead, median life 5 rounds) while the OS disk
        # count is a preference AMONG survivors, and no amount of quiet disk
        # pays for a tile that already sits in a live Gunner's line.  Dropping
        # rows rather than penalising them also leaves the role_n spacing rule
        # untouched -- the non-primary engineer still takes the third-best
        # tile, just the third-best CLEAN one.  If every candidate is covered,
        # the unfiltered list stands and the plant happens anyway: this is a
        # preference between tiles, never a refusal to build.  One-time cost at
        # the plant decision (a set lookup per row), nothing per round.
        if SP_ON and self.sp_turrets:
            veto = self._sp_veto_tiles(ct)
            if veto:
                clean = [c for c in candidates if (c[2], c[3]) not in veto]
                if clean:
                    candidates = clean
        pick = 0 if self.role_n == 0 else min(2, len(candidates) - 1)
        row = candidates[pick]
        # PLANK OS siting (see the OS constants block).  Every row here already
        # carries a clear ray onto the enemy Core -- the firing line the turret
        # exists for -- and is sorted by route distance, so re-ranking the top
        # OS_SITE_TOPK rows by "how many tiles could they plant their answering
        # Gunner on" trades at most a few steps of walk for a quieter disk, and
        # never the plant round or the lane.  Strict `<` keeps the closest tile
        # on a tie.  One-time cost at the plant decision, not per round.
        if turret_type == EntityType.SENTINEL and self._os_early(ct):
            best = None
            counted = {}
            for cand in candidates[:OS_SITE_TOPK]:
                skey = (cand[2], cand[3])
                got = counted.get(skey)
                if got is None:
                    got = self._os_plantable(cand[6], blocked)
                    counted[skey] = got
    ...
    def _saboteur(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            # PLANK OS: the same recall shape one level down -- a 2-damage peck
            # is not worth a round of the opening standoff plant, so the
            # designated early builder skips it until its Sentinel stands.
            if self.melee_first and not self._os_early(ct) and self._sabotage_prio(ct):
                return
    ...
    def _fb_floor_ok(self, ct, spend):
        """PIECE FB (B, the floor half).  The det instrument showed WHY the
        two gate exemptions alone changed nothing: on fjordgate seat B the
        bank cycles 0 -> 10 -> 6 -> 2 -> 0 forever -- the heal line (~4 Ti/r)
        plus the ammo trickle outspend the 2.5 Ti/r passive income, so no
        money gate ever sees harvester cost again after ~r10.  An eco floor
        that "survives the latch" therefore has to be an ACCUMULATION floor:
        while the team has ZERO harvesters, the siege spenders may only spend
        bank ABOVE harvester cost.  ~8 rounds of income rebuilds 0 -> 20,
        the bootstrap gates (already exempted) fire, harv >= 1 disarms this
        immediately.  Bounded exposure by construction: it can never bind in
        a game where a single harvester exists, i.e. every normal opening
        (harvester at r5) leaves this dormant."""
        if not FB_ON:
            return True
        if ct.read_store(SLOT_HARVESTERS) >= 1:
            return True
        return ct.get_global_resources() - spend >= ct.get_harvester_cost()

    def _heal_core(self, ct):
        if not self._fb_floor_ok(ct, 1):
            return False
    ...
    def _home_gun(self, ct):
        """PIECE HG -- the team's published home-gun fact; local scan as fallback.

        Replaces `_live_home_gun` at the two COUNTERBATTERY sites ONLY, which is
        where a false negative costs titanium and a defender's action.  Falls
        back to the caller's own scan while the value is HG_UNSET -- round 0, or
        a game where the Core died before publishing.  That fallback is the OLD
        behaviour exactly, so the piece can only match it or improve on it.
        """
        if not HG_PUBLISH_ON:
            return self._live_home_gun(ct)
        v = ct.read_store(SLOT_HG_LIVE)
        if v == HG_UNSET:
            return self._live_home_gun(ct)
        return v == HG_GUN
        ...
        for hp, _d, bid, bp in cands:
            ...
            if abs(p.x - bp.x) + abs(p.y - bp.y) == 1:
                ...
                if (
                    ...
                    and self._fb_floor_ok(ct, HUNT_FIRE_TI)
                    and ct.can_fire(bp)
                ):
                    ct.fire(bp)
                return True
    ...
    def _try_counterbattery(self, ct):
        ...
        if B8_ON and min(
            ...
            return False
        # PIECE E1 (RING) -- ring semantics, asked HERE and not after the
        # bootstrap gate: a threat that is already dead, or already inside a
        # standing turret's ray, wants no second turret at any harvester count
        # or bank.  This is also the gate the walk-to-position planner asks, so
        # the two can never disagree about whether the ring is full.  home_gun
        # below is this scan's own count, which makes the bootstrap gate's
        # question ("a home gun already stands") free rather than a second
        # dozen-call scan.  Unarmed (or ring off) the whole block is skipped and
        # home_gun stays None, which routes the bootstrap gate into the parent
        # lineage's own expression byte-for-byte.
        home_gun = None
        if self._ring_live(ct):
            ring_blocked, ring_n = self._home_ring_check(ct, threat)
            if ring_blocked:
                return False
            home_gun = ring_n >= 1
        ...
        if ct.read_store(SLOT_HARVESTERS) < ECO_NEED and (
            ...
            else self._home_gun(ct) if CB_OVER_HEAL_ON
            ...
            if not self._core_shelled(ct):
                return False
        # PIECE E1 (RING), ray-coverage amendment: a held ring plan carries the
        # lane-scored (spot, facing, type).  The neighbour scan below builds the
        # FIRST enum-order facing that covers the threat, which is how every
        # smoke-run ring turret got placed while the planner's facing never
        # reached the board.  Spend the plan first whenever this builder stands
        # beside its spot; the scan remains the fallback for planless builders
        # (melee emergencies via _home_defend), and the held-plan gate keeps
        # them off the planner's BFS.
        if (
            self._ring_live(ct)
            ...
            for d in scan_dirs:
                ...
                for facing in DIRECTIONS:
                    ...
                    if turret_type == EntityType.SENTINEL and ct.can_build_sentinel(bp, facing):
                        ...
                        ct.write_store(SLOT_HOME_GUN, ct.read_store(SLOT_HOME_GUN) + 1)
                        # PIECE E1 (RING): this turret answers the same threat
                        # any held plan was made against, so the plan is spent.
    ...
    # ------------------------------------------------------------------
    # PIECE E1 (RING) -- the home counter-battery ring.  See the PIECE E1
    # (RING) constants block and docs/e1-incoming-side-design-2026-08-08.md.
    # ------------------------------------------------------------------

    def _e1_convert_cap(self, ct):
        """PIECE E1 (RING), COMPONENT 4 -- the Core's per-turn convert cap.

        16 is the shipped drip rate and stays the answer on every path but one:
        a sentinel duel is 10 ammunition a shot, so a magazine filling at 16 a
        turn funds one shot and change while the ring turret it was built for is
        being answered.  Ordered so that a game where the ring never arms pays
        exactly one module-constant test and one attribute read -- the live
        turret scan is only reached in the games this component exists for.
        """
        if not (E1_RING_ON and self._ring_armed()):
            return 16
        guns, sents = self._core_turret_mix(ct)
        return E1_CONVERT_CAP if (guns or sents) else 16

    def _e1_arm_scan(self, ct, ents, core, ecore, rnd, mine):
        """PIECE E1 (RING), MECHANISM A -- the arming signatures, Core side.

        The builder path rides its own sensing loop for free (see _builder); the
        Core cannot, because its scan BREAKS on the first qualifying sighting
        and signature 3 has to count two.  So this walks the entity list the
        Core already fetched.  It runs only while this instance is unarmed and
        stops forever the moment it arms, so the cost is bounded by the rounds
        before the first signature -- and is zero in every game that has none.
        """
        shellers = 0
        for eid in ents:
            try:
            ...
            if et in (EntityType.GUNNER, EntityType.SENTINEL):
                fp = fp_dsq(ep, core)
                # SIGNATURE 2 -- ablated (C1B_ARM_TURRET_DSQ = -1).
                if fp <= C1B_ARM_TURRET_DSQ:
                    self.c1b_armed = True
                    return
                # SIGNATURE 3 -- a battery, not a single forward snipe.
                if fp <= E1_ARM_SHELL_DSQ:
                    shellers += 1
                    if shellers >= E1_ARM_SHELL_N:
                        self.c1b_armed = True
                        return
            elif (
                et == EntityType.BUILDER_BOT
                and rnd < C1B_ARM_RAIDER_RND
                and fp_dsq(ep, core) <= C1B_ARM_RAIDER_DSQ
                and ecore is not None
                and fp_man(ep, ecore) > rnd + 1
            ):
                # SIGNATURE 1 -- early raider that cannot have walked here.
                self.c1b_armed = True
                return

    def _ring_armed(self):
        """PIECE E1 (RING), MECHANISM A -- has a signature been seen?

        Two attribute reads and no engine call, which is the point: with the
        gate on and nothing seen, every ring gate in this file answers here and
        the planner, the walk and the ring scans are never reached, so an
        opponent of neither target class plays the parent lineage exactly.  With
        E1_RING_ON off this is a constant False and the file is _v94fb.
        """
        return E1_RING_ON and (self.c1b_armed or not C1B_ARMING_GATE_ON)

    def _ring_live(self, ct):
        """PIECE E1 (RING) -- armed AND past the opening clock (FJORDGATE GUARD).

        Every ring gate asks THIS, not _ring_armed, so the round floor covers
        the walk and the plan as well as the build: a defender that walks out to
        a firing seat at r2 has already spent the opening rounds the fjordgate
        rush regression was measured on, whether or not it ever builds.
        """
        return self._ring_armed() and ct.get_current_round() >= E1_RING_MIN_RND

    def _ring_cost(self, ct):
        """PIECE E1 (RING) -- scaled price of the currently planned turret."""
        return (
    ...
    def _ring_affordable(self, ct, cost):
        """PIECE E1 (RING) -- THE FJORDGATE GUARD, spend half (design doc 6.5).

        Every titanium the ring spends is a turret build (the walk is free), and
        all of them come through here.  BOTH guards, because they answer
        different failures and neither implies the other:
         - _eco_spendable keeps the ring out of SIEGE_HEAL_RESERVE_TI, so a
           turret can never be bought with the money the heal line and the hunt
           pecks are holding for the same siege.
         - _fb_floor_ok is the ACCUMULATION floor: with ZERO team harvesters the
           bank may not be spent below harvester cost.  The det instrument on
           fjordgate seat B showed the bank cycling 0 -> 10 -> 6 -> 2 -> 0 for
           392 rounds with no harvester ever affordable again; a 20-30 Ti ring
           turret bought out of that bank is exactly the spend that keeps it
           there.  harv >= 1 disarms it immediately, so this is dormant in every
           normal opening.
        """
        return self._eco_spendable(ct, cost) and self._fb_floor_ok(ct, cost)

    def _ring_link_ban(self, ct):
        """PIECE E1 (RING), SITING BAN 2 -- tiles the ring may not BUILD on
        because our own delivery chain runs, or is planned to run, through them.

        Returns a key set or None, matching the `lban` idiom in
        _try_build_launcher: a locally computed forbidden-key set that candidate
        sites are tested against, with None as the "no ban" answer every call
        site falls through on.  See the SITING BAN 2 constants block for the
        decoded meander severing this exists to prevent.

        BUILD-only by construction -- the caller never feeds this to the terrain
        flood.  Conveyors are bot-passable and walking a belt costs nothing; it
        is the impassable, permanent turret ON the belt that freezes the stacks.

        One get_nearby_buildings pass, paid once per PLAN (not per candidate)
        and once at the moment of build, both behind the planner's CPU guard.
        Vision-bounded like every other scan in this file, which is the
        conservative direction: a chain tile we cannot see is one we are not
        standing next to and therefore cannot build on this turn anyway.
        """
        if not E1_RING_LINK_BAN_ON:
            return None
        ban = {(t.x, t.y) for t in self.link_queue}
        belt = set()
        for bid in ct.get_nearby_buildings():
            try:
                if ct.get_team(bid) != self.team:
                ...
                if et == EntityType.HARVESTER:
                    # Every orthogonal neighbour of an own harvester: its
                    # output must exit through one of them, and at plan time
                    # the belt may not exist yet (the meander sentinel went
                    # down at r5, BEFORE the belt grew around it).
                    hp_ = ct.get_position(bid)
                    for hd in CARDINALS:
                        n = hp_.add(hd)
                        if 0 <= n.x < self.mw and 0 <= n.y < self.mh:
                            ban.add((n.x, n.y))
                    continue
                if et not in (EntityType.CONVEYOR, EntityType.SPLITTER):
                    continue
                bp = ct.get_position(bid)
                # The tile this belt piece FACES, while still empty, is where
                # the chain grows next -- the predictive half of the ban.
                fn = bp.add(ct.get_direction(bid))
                if (
                    0 <= fn.x < self.mw and 0 <= fn.y < self.mh
                    and ct.is_tile_empty(fn)
                ):
                    ban.add((fn.x, fn.y))
            except Exception:
                continue
            belt.add((bp.x, bp.y))
        ban |= belt
        if E1_RING_LINK_BAN_GAP and belt:
            # A hole with belt on both sides along one axis IS the severing
            # tile: build there and the stacks freeze at the break exactly as
            # the meander decode describes.  Pure set arithmetic over the scan
            # above -- four probes per belt tile, no engine call.
            for bx, by in belt:
                for dx, dy in ((1, 0), (0, 1)):
                    gap = (bx + dx, by + dy)
                    if gap not in belt and (bx + 2 * dx, by + 2 * dy) in belt:
                        ban.add(gap)
        return ban or None

    def _ring_threat_kind(self, ct, threat):
        """PIECE E1 (RING) -- what stands on the threat tile, if we can see it.

        One probe, two consumers: the sentinel-first re-cut below and the
        return-fire penalty in the planner (which is a GUNNER problem only).
        Out of vision is None -- "unknown", which both consumers read as the
        conservative answer rather than guessing a class.
        """
        try:
            if not ct.is_in_vision(threat):
                return None
            tid = ct.get_tile_building_id(threat)
            if tid is None or ct.get_team(tid) == self.team:
                return None
            return ct.get_entity_type(tid)
        except Exception:
            return None

    def _ring_turret_type(self, ct, kind):
        """PIECE E1 (RING) -- which turret to plant, RE-CUT from C1b.

        C1b took the first affordable type in PRIMARY_SENTINEL order.  Against
        the CAD envelope the sentinel is the right home gun twice over: 18 dmg
        on reload 2 TWO-SHOTS the 25 HP gunner class that does the shelling, and
        their counter-plant only triggers within d^2 ~ 36-64 of THEIR core, so a
        turret standing at ours is structurally unanswered (design doc 2) --
        the opposite of a forward sentinel, which their counter-gunner kills 76%
        of the time.  Applied only when the threat tile actually holds a turret:
        against a raider bot the extra titanium and the +20% team-wide scale buy
        nothing a gunner does not.

        The first AFFORDABLE type wins, and affordable means the fjordgate
        guard's definition of it.  A bank that affords neither plans nothing,
        rather than walking a defender away from the heal line for a turret it
        cannot buy.
        """
        sentinel_first = PRIMARY_SENTINEL or (
            E1_RING_SENTINEL_FIRST
            and kind in (EntityType.GUNNER, EntityType.SENTINEL)
        )
        order = (
            (
                (EntityType.SENTINEL, ct.get_sentinel_cost()),
            ...
            if sentinel_first else
            (
                (EntityType.GUNNER, ct.get_gunner_cost()),
                (EntityType.SENTINEL, ct.get_sentinel_cost()),
            )
        )
        for et, cost in order:
            if self._ring_affordable(ct, cost):
                return et
        return None

    def _homering_target(self, ct):
        """PIECE E1 (RING) -- the tile the ring planner is currently aimed at.

        SLOT_THREAT for the role-4 defender, EXCEPT while a supply helper holds
        a claim of its own (see C1B_SUPPLY_ON).  The override is per unit
        instance, so the two never see each other's target and the defender's
        plan is untouched by the helper's existence.
        """
    ...
    def _c1b_drop(self):
        """PIECE E1 (RING) -- release the helper's claim, hand back clean state.

        The claim and the plan it justified die together, and self.tgt/self.stuck
        are reset for the same reason the defend-role succession resets them: the
        expand machine is about to be handed this unit back and a stale walk
        target is how a released helper oscillates.  No role was ever changed, so
    ...
    def _c1b_find(self, ct):
        """PIECE E1 (RING) -- nearest UNCOVERED, ANSWERABLE established turret.
        ...
        C1B_SUPPLY_BAND_DSQ -- and coverage is decided from that list with one
        can_fire_from per (ours, theirs) pair, stopping at the first candidate
        that qualifies.
        ...
        ANSWERABLE, folded into the same loop: the nearest uncovered turret may
        be the published SLOT_THREAT with a live defender already planning
        against it, and rejecting the whole round on that would hide the second
        threat this mechanism exists for.  Two admitting cases, both off existing
        signals: the tile is not SLOT_THREAT, so no defender plan can be aimed at
        it; or no defender has beaten for C1B_SUPPLY_BEAT_STALE rounds.
        beat == 0 is the opening -- the fifth builder does not exist yet -- and
        counts as silence, which is exactly the r2-r5 window an insertion lands
        in.  The stored value is round + 1, hence beat - 1.
    ...
    def _c1b_elected(self, ct, threat):
        """PIECE E1 (RING) -- am I the one hand this threat gets?

        Nearest-wins with a lower-id tiebreak, over the friendly builders this
        unit can SEE, skipping any competitor that stands closer to the published
        SLOT_THREAT than to this threat -- that one is the defender's threat's
        business and must not veto this one.  Role is not observable across
        units, so a saboteur or the defender itself can win the election and
        answer nothing; the election re-runs every round as they move, and the
        failure mode is a missed round, not a wasted build.
        """
        me = ct.get_id()
        p = ct.get_position()
    ...
    def _c1b_supply(self, ct):
        """PIECE E1 (RING), MECHANISM B -- the second responder's whole turn.
        ...
        cheapest-first throughout: three attribute tests, then a store read, then
        the band arithmetic, and only then the two scans and the planner.

        SLOT_UNDER is the outer cheap gate.  An established turret at fp-dsq <=
        36 is at Core-anchor dsq <= 62 by construction, i.e. inside the gun_sense
        64 that the same sensing loop publishes UNDER from, so the gate costs at
        most the one round the write buffer delays -- and it keeps both scans off
        every quiet round of every quiet game.
        """
        if not (C1B_SUPPLY_ON and self._ring_live(ct)):
            return False
        if self.role != "expand" or self.role_n in (0, 4) or self.core is None:
            ...
            # Dropping here is load-bearing, not tidiness: a retained claim keeps
            # overriding _homering_target for the rest of the game, so a promoted
            # defender would plan its ring against a tile it inherited instead of
            # against SLOT_THREAT.
            self._c1b_drop()
            return False
        ...
        if threat is None:
            ...
            if not self._c1b_elected(ct, threat):
                return False
        # THE RACE ABORT, and it is the existing coverage dedup rather than a new
        # one: _home_ring_check answers "threat dead, already covered by a live
        # home turret, or ring at HOME_RING_CAP" off a live scan, and it runs on
        # THIS turn immediately before the build below.  Two helpers that both
        # elected therefore cost one wasted walk, never a second turret -- the
        # loser sees the winner's turret covering and drops.
    ...
    def _threat_live(self, ct, threat):
        """PIECE E1 (RING) -- False only when the threat tile is VISIBLE and empty.

        SLOT_THREAT is a tile, not an entity handle, and no writer clears it when
        its occupant dies; the UNDER latch decays on 50 rounds, so a defender can
        otherwise plan, walk and build against a turret that was killed thirty
        rounds ago.  Out of vision returns True: "I cannot see it" is not
        evidence of death, and _hostile_at's except arm reads an unseeable tile
        as not-hostile, which alone would cancel every plan the moment the
        defender steps out of sight of the threat.  Read-only -- no new writer on
        SLOT_THREAT.
        """
        try:
            if not ct.is_in_vision(threat):
    ...
    def _home_ring_state(self, ct, threat):
        """PIECE E1 (RING) -- (live home turrets, threat already in one of their
        rays).

        LIVE local scan, never SLOT_HOME_GUN -- that counter is incremented by
        the saboteur's FORWARD gun at the ENEMY core and counts rubble forever,
        so it cannot answer either question (see CB_OVER_HEAL_ON).  The band is
        HUNT_BAND_DSQ (41, footprint-measured), the constant every other home
        gate in this file already shares.  Vision-bounded by construction, so a
        defender out in the field undercounts -- which errs toward one turret too
        many, not toward a ring that never answers.
        """
        tiles = core_tiles(self.core)
        live = 0
        covered = False
    ...
    def _home_ring_check(self, ct, threat):
        """PIECE E1 (RING) -- (no new home turret wanted, live home turret count).
        ...
        """
        if self.core is None or threat is None:
            return True, 0
    ...
    def _clear_homering(self):
        """PIECE E1 (RING) -- drop the held home-ring plan."""
        self.homering_spot = None
        self.homering_approach = None
        self.homering_direction = None
        self.homering_type = None
    ...
    def _homering_plan_valid(self, ct):
        """PIECE E1 (RING) -- may the held plan still be walked to?
        ...
        if self.homering_spot is None:
            return True
        # The helper's claim stands in for SLOT_THREAT while it holds one, so its
        # plan is validated against the threat it was actually made against.
    ...
    def _plan_homering(self, ct):
        """PIECE E1 (RING) -- choose a reachable tile whose ray covers the THREAT.

        The home-side sibling of _plan_siege: the same machinery (ray arithmetic
        backwards off the target, ONE terrain flood so the chosen ray is a real
        route and not a geometrically close tile behind a wall, cardinal approach
        tiles because construction is cardinal-adjacent only, and the same two
        CPU guards), aimed at the current threat tile instead of at the enemy
        Core.  Deliberately a sibling and not a refactor: the forward snipe is a
        separately gated plank and ablation purity beats DRY here.

        The differences from the siege planner, each with its reason:
         - can_fire_from is the qualifier instead of a hand-rolled wall test.  A
           Sentinel's line IGNORES obstacles, so the siege planner's "a wall
           anywhere before the target" rejection would throw away exactly the
           spots this plank exists to find.  It is the same predicate
         ...
         - a small pull toward our own Core, so a tie between two firing
           positions breaks toward the one inside the ring, where the heal line
           and the Core's own vision already are.
         - PLANK HS: heal seats are excluded as SPOTS.  A turret is impassable,
           so one planted on a seat costs that seat's +4 HP/round for the rest of
           the match -- the worst trade on the board when the reason we are
           building it is that the Core is bleeding.  Approach tiles are NOT
           banned: standing on a seat is what seats are for.

        Nothing is written to self.homering_* until a candidate is chosen at the
        very end, so every bail below is a clean no-op.
        """
        if (
            not self._ring_live(ct)
        ...
        # this band no turret built against our own footprint reaches the threat,
        # so every can_fire_from below would return False.
        ...
        if (
            ring_n >= 1 and ct.read_store(SLOT_HARVESTERS) < ECO_NEED
            and not self._core_shelled(ct)
        ):
            return False
        # One probe of the threat tile, two consumers: the type re-cut and the
        # return-fire penalty below.  Type is decided at PLAN time because it
        # sets the ray lengths.
        kind = self._ring_threat_kind(ct, threat)
        turret_type = self._ring_turret_type(ct, kind)
        ...
        blocked.discard((p.x, p.y))
        # The two BUILD-site bans.  Deliberately kept OUT of `blocked`, which is
        # the terrain flood's input: both ban classes are things a builder may
        # freely walk over (a heal seat is a standing spot and a conveyor is
        # bot-passable), and folding them into the route would refuse firing
        # positions that are perfectly reachable.
        ban = self._seat_ban()
        lban = self._ring_link_ban(ct)
        ...
        retfire = kind == EntityType.GUNNER

        # THE APPROACH LANE (see HOME_RING_LANE_DSQ).  Every in-bounds, non-wall
        # tile off the Core footprint out to footprint-dsq HOME_RING_LANE_DSQ --
        # the tiles an insertion actually plants on.  Built once per plan off a
        # bounding box rather than a map sweep: at dsq 9 the box is 8x8 around a
        # 2x2 footprint, so this is ~64 pure-Python tests and no engine calls.
        lane = set()
        ...
        for facing in DIRECTIONS:
            ...
            for ray_len in range(max_range, 0, -1):
                ...
                if (
                    ...
                    or self.map_grid[spot.y][spot.x] != "."
                    or skey in blocked
                    or (ban is not None and skey in ban)
                    or (lban is not None and skey in lban)
                ):
                    continue
                if ct.is_in_vision(spot) and ct.get_tile_building_id(spot) is not None:
                    continue
                ...
                for ad in CARDINALS:
                    ...
                    if (
                        ...
                        continue
                    seen.add(key)
                    # HOME GROUND IS NOT EMPTY GROUND, and this is the one place
                    # the siege sibling's assumptions do not carry over.  `dist`
                    # is a TERRAIN flood: it knows walls and the two Core
                    # footprints and nothing else, which is fine out in the field
                    # the saboteur walks but wrong here, where our own conveyor
                    # trunk and harvesters carpet the approach tiles.  Measured
                    # on lighthouse: an occupied approach put the defender in a
                    # two-tile A-B-A oscillation for the rest of the siege,
                    # replanning the identical unreachable spot every round.
                    # is_tile_passable answers for buildings AND for a bot
                    # already standing there, and out-of-vision is left to the
                    # terrain flood rather than guessed at.
        ...
        if not candidates:
            return False
        # RANKING (the ray-coverage decode): covering the CURRENT threat is
        # mandatory -- can_fire_from above is a filter, not a term -- and lane
        # coverage outranks walking distance, because A SENTINEL CANNOT BE
        # RE-AIMED (rotate() is gunner-only; 0 direction changes on any sentinel
        # across 277 re-emissions).  A facing chosen for today's threat tile
        # alone is dead weight the round that threat dies, while a facing that
        # also crosses the approach lane keeps paying for the rest of the game.
        # The measured predicate it proxies: turrets on a reachable friendly ray
        # died 8/8, uncovered ones took zero turret shots 15/15.
    ...
    def _try_homering_build(self, ct):
        """PIECE E1 (RING) -- build the planned ring turret once we stand beside it.
        ...
        The spend gate is _ring_affordable, not a bare bank read: this is the one
        line where the ring actually costs titanium, so it is where the fjordgate
        guard has to hold (design doc 6.5).
        """
        if not self._ring_live(ct):
            return False
        if not self._homering_plan_valid(ct):
            self._clear_homering()
        ...
        if max(abs(p.x - spot.x), abs(p.y - spot.y)) > 1 or p == spot:
            return False
        # Belt and braces behind the planner's own exclusions, and NOT
        # redundant: a plan is held across rounds while the defender walks to
        # it, and both ban sets move under it -- the link queue is re-planned by
        # _link_path / _wire_tick and the belt itself grows a tile at a time.  A
        # spot that was clean when it was chosen can be the severing tile by the
        # time we arrive, and this is the last line before the build that can
        # still refuse it.  Dropping the whole plan (rather than building
        # anyway) is the self-healing answer -- the next round re-plans against
        # the current chain, exactly as _build_next_link drops a queue that has
        # outlived its ban.
        ban = self._seat_ban()
        if ban is not None and (spot.x, spot.y) in ban:
            self._clear_homering()
            return False
        lban = self._ring_link_ban(ct)
        if lban is not None and (spot.x, spot.y) in lban:
            self._clear_homering()
            return False
        built = False
        if (
            self.homering_type == EntityType.SENTINEL
            and self._ring_affordable(ct, ct.get_sentinel_cost())
            ...
            and self._ring_affordable(ct, ct.get_gunner_cost())
        ...
        if built:
            ...
            # nothing in this plank gates on it.
            ct.write_store(SLOT_HOME_GUN, ct.read_store(SLOT_HOME_GUN) + 1)
            self._clear_homering()
            return True
        return False
    ...
    def _defend(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            defended = False
            # PIECE FB (C): with ZERO team harvesters, a defender standing
            # next to buildable ore spends this round on the bootstrap
            # harvester BEFORE the defence chain -- under permanent shelling
            # `defended` comes out True every round, so the harv < 1 fallback
            # at the bottom of this block is provably unreachable exactly when
            # it matters (the fjordgate det set: 392 rounds, 0 harvesters,
            # heal/CB winning the action every turn).  One action, once per
            # game-phase by construction: harv >= 1 disables it again.
            if (
                FB_ON and harv < 1
                and ti >= ct.get_harvester_cost()
            ...
            if under_x:
                ...
                if shelled and not self._cb_over_heal(ct) and self._heal_core(ct):
                    ...
                    defended = (
                        ...
                        # PIECE E1 (RING): the immediate build above only fires
                        # when this builder ALREADY stands beside a tile whose
                        # ray holds the threat.  This is the walk case -- it
                        # builds if we have arrived, and otherwise makes the
                        # plan the move phase below walks to.  Defender seat
                        # only: the all-hands melee path (_home_defend) keeps
                        # the immediate build alone, since a raider already in
                        # the footprint is not a problem a five-round walk
                        # solves.
                        or (
                            E1_RING_ON and self.role_n == 4
                            and self._try_homering_build(ct)
                        )
                    )
        ...
        # PIECE E1 (RING) -- WALK TO THE FIRING POSITION.  Ordered ABOVE
        # "defender comes home" deliberately, and the ordering is the whole
        # mechanism: that rule sends a shelled defender back to the Core to
        # heal, and +4 HP a round against an 18-25 dmg battery is the losing
        # side of the arithmetic piece J already measured.  Placing this below
        # it would also OSCILLATE -- step out to the firing seat, be dragged
        # home next round for not touching the footprint, forever.
        #
        ...
        # the ring is under cap and the bank affords the turret under BOTH
        # spend guards (see _plan_homering's gate), so the heal rule takes the
        # defender straight back the moment the ring answers; and the action
        # phase above still heals on every round the walk cannot build.
        # Standing ON the approach holds the seat -- the same "already in build
        # range, wait for the action instead of vacating" idiom the link queue
        # below uses -- but only while we can still pay for the turret; going
        # broke drops the plan and hands the round back to the heal line.  Same
        # stuck-replan discipline the saboteur applies to the siege plan.
        if (
            self._ring_live(ct)
            ...
            if ap is not None:
                if p.x == ap.x and p.y == ap.y:
                    if self._ring_affordable(ct, self._ring_cost(ct)):
                        return
                    self._clear_homering()
                else:
                    self.tgt = ap
    ...
    def _expand(self, ct):
        ...
        # past round 42.
        # PIECE HF (s18): the hive economy self-freeze is REMOVED. Measured on
        # the shipped v80 bytes, hive seat A, 6/6 paired seeds: delivered
        # titanium 5,260 -> 11,030 and standing buildings 28 -> 155 with this
        # gate ablated, at ZERO outcome flips -- i.e. it was invisible to
        # flip-accounting and cost half an economy on a live pool map. The
        # long-game census makes that decisive: delivered titanium is the SOLE
        # tiebreak decider in 219/219 full-length games (26.2% of all games,
        # 36.7% under the v80 line). The freeze's original rationale is not
        # recorded anywhere in this file beyond "confirmed economy self-freeze",
        # and piece J already narrowed its arming without removing it.
        hive_freeze = (
            HIVE_FREEZE_ON
            ...
            if (
                ...
                if endgame
                    ...
                    (
                        ...
                        # PIECE FB (B): bootstrapping from ZERO harvesters is
                        # exempt from the siege reserve -- the reserve exists
                        # to fund heals through a siege, but with no economy
                        # there is nothing to fund them FROM next interval.
                        or (
                            FB_ON and harv < 1
