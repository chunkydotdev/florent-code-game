"""v120h_gun68_fwdcap2 OFFLINE — overnight promote (+ live CPU wrapper).

Verified NEW 75 / OLD 78 / 9436 20 / LR 19.
Lever: gun68 + FWD_GUN_CAP2 (Loki-H2H recovery)
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            try:
                ...
                continue
            if (et in (EntityType.GUNNER, EntityType.SENTINEL) and d <= (68)) or (
                et == EntityType.BUILDER_BOT and d <= (20 if ((self.mw, self.mh) == (10, 10) and self.core is not None and (self.core.x, self.core.y) in ((1, 1), (7, 7))) else 16)
            ):
                under = True
        ...
        if not endgame_dumped:
            _ammo_floor = 12 if (self.mw, self.mh) == (24, 24) else AMMO_FLOOR
            ammo_target = 29 if under else _ammo_floor  # v117d saga
            if weapons:
        ...
        _hive = ((self.mw, self.mh) == (25, 25) and self.core is not None and (self.core.x, self.core.y) in ((2, 20), (21, 3)))
        if harv >= 1 and ti >= LOKI_SURPLUS_TI and not (_hive and rnd < 140):
    ...
    def _nearest_home_intruder(self, ct):
        ...
        for eid in ct.get_nearby_units():
            try:
                ...
                continue
            if self.core.distance_squared(ep) > (49 if ((self.mh or 0) > (self.mw or 0) and (self.mw or 0) * (self.mh or 0) <= 560) else 36):
                continue
    ...
    def _try_build_launcher(self, ct):
        ...
        area = (self.mw or 0) * (self.mh or 0)
        if area and area <= 196 and ct.get_current_round() < 200:
            return False
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
