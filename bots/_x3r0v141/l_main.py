...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        self.idx = 0
        self.role = "expand"
        # UNDERECO (s36): round the current siege latch FIRST engaged, or None.
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            if under and self.under_since is None:
                self.under_since = rnd
            elif not under:
                self.under_since = None
            # UNDERECO: chronic siege (>100r latched) restores the harvester
            # reserve â€” the acute-phase skip must not disable eco recovery
            # forever under a permanent ring camp (Leviathan, 18f30710).
            chronic = under and self.under_since is not None and (rnd - self.under_since) > 100
            ti_floor = 12 if (under or weapons) else 52
            if E1_AMMO_FLOOR_ON and (not under or chronic):
                ti_floor = max(
                    ti_floor,
        ...
        if not hasattr(self, "spawn_salt"):
            # Terrain-only best-response table fitted on two independent,
            # colour-swapped sweeps against multisalt_v2.  Exact terrain
            # fingerprints avoid ambiguous dimension/anchor map keys.
            sig = 2166136261
    ...
    def _builder(self, ct):
        ...
        # TWORAID (s36): seat 4 converts under the SAME conditions (volume
        # vs core-tanks â€” the one two-simultaneous game is the one Leviathan win).
        _raid_seat = (self.role_n == LOKI_LATE_RAID_SEAT or (
            LOKI_TWORAID_ON and self.role_n == LOKI_LATE_RAID_SEAT + 1))
        if (
            self.role == "expand" and _raid_seat
    ...
    def _try_build_launcher(self, ct):
        ...
        # â‡’ If LAUNCH0's gain is the PREMIUM, deferral captures most of it while
        ...
        # âš  NOT a free lunch: deferring also delays the ferry, and early exile
