...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # reserve — the acute-phase skip must not disable eco recovery
        ...
        if not hasattr(self, "spawn_salt"):
            # Exact-terrain best responses to counter_router_v2.
            sig = 2166136261
            for ch in "|".join(self.map_grid):
                sig = ((sig ^ ord(ch)) * 16777619) & 0xFFFFFFFF
            self.spawn_salt = {
                2024030065: 6,   # antler (center of 3..8 4-0 plateau)
                1742285299: 57,  # archipelago (center of 56..59 plateau)
                2251200169: 7,   # auroraveil
                2959488499: 13,  # drakkarfjord (center of 12..14 4-0 plateau)
                2309041557: 61,  # drumlin
                4055986179: 61,  # fjordgate
                1941512695: 0,   # frostgate
                446389448: 19,   # glacierkeep
                1691756395: 0,   # icefloe
                999493859: 39,   # midgard (20-0 deep validation)
                3103780539: 40,  # nordkap (center of 38..43 plateau)
                1715578931: 40,  # ragnarok
                733716547: 19,   # royale
                998525177: 19,   # valkyrie
                3313952939: 33,  # yulerune
            }.get(sig, 0)
    ...
    def _builder(self, ct):
        ...
        # vs core-tanks — the one two-simultaneous game is the one Leviathan win).
