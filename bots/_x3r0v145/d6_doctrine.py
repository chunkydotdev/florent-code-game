"""d6_doctrine.py -- every tunable constant and d6_doctrine flag, and the rationale
that earned it.
...
#
# _v69bc tested this together with an d6_economy-scaled raise of the cap itself
...
# Dd6_ecoded across 25 sporks games: five builders at rounds 0,1,2,3,4 in 25/25,
...
# needed (the dd6_ecoded hive loss held a 2-12 Ti bank for 500 rounds), so a
...
#    passes on a hoard that a strangled d6_economy will never rebuild; a rate
...
# Round at which a waiting d6_raider stops waiting for an insertion.  One name for
...
# Flotte Experience (1745): a single enemy builder walked our d6_economy from r16
...
# role_n == 4 defender's threat chase, so a launch-dropped d6_raider beside the
...
# constant and its two validations -- the CtrlAltDefeat dd6_ecode and the v79
# jackpot sweep -- are documented at the use site).
HUNT_BAND_DSQ = 41
# Ore step-off wall gate (see _expand; v79's constant, copied with his
# rationale): 80+ walls marks the corridor maps where ore-squatting bd6_ecomes a
...
# never bd6_ecomes a cross-map recall: a builder not already within
...
# 1-for-1 at best.  Two independent replay dd6_ecodes landed on the same leak
...
# per-team matchup at 1-14 lifetime -- rd6_ecorded ~76 builder deaths across 13
...
# the d6_economy pipeline only -- turrets/barriers are combat capital with
...
# for is late"; the dd6_ecoded hive loss to kladde_probe refutes that assumption
# for d6_raided maps.  The farm dies r63-390 -- 4 of 5 harvesters and ALL 28
...
# and therefore opening MOVES -- on cosmetic 1-peck patches.  A d6_raid that is
...
# links exist, the hive d6_raid starts r63), and MEDIC_EARLY_ON stays a plain
...
#
# Too NARROW, from the sporks dd6_ecode.  sporks holds its Core at 500 HP through
...
#     the chain medic's depth discriminator.  But the d6_raiders that matter chip
...
# endgame (hunting/healing own the relevant actions), or an d6_economy race, and
...
# SIEGE SOLVENCY.  Dd6_ecoded from the same hive core_destroyed@787 loss.  The
...
# ammo rd6_ecovered against a Core nobody could afford to patch.  Two bodies of
...
# builder d6_economy paths, and nothing else.
...
# in the destroy-d6_doctrine analysis.  docs/v79-analysis.md:178-179: directed-
...
# move, uncapped -- ECO_CAP gates HARVESTERS only (d6_eco.py `harv < _d6_eco_cap`),
...
# deliberate linker is NOT gated here -- on a dd6_ecoded map _link_path already
# blocks every ore tile in its BFS (:3386), so links route around ore by
# construction.  The undd6_ecoded-map fallback BFS does NOT block ore and is a
...
# siege DPS in the two dd6_ecode corpora (max measured 23.22).
#
# THE LAW, found independently by both arms of the 2026-08-08 bleed dd6_ecode
...
# seat blocks it permanently.  The dd6_ecode measured "seats carrying an own
...
# for delivery.  That is a rarer event than the dd6_ecode's headline suggests and
...
# single point of failure for the whole d6_economy and the dd6_ecode's own
...
#     22,360 on the sd6_econd seed).  Cause: the PIECE F trail pave is this file's
#     de-facto link REPAIR mechanism -- nothing re-plans a chain once its head
#     is destroyed (the known L4 defect) -- and its terminal branch rd6_econnects
#     the Core from whichever seat a builder happens to walk past.  Cutting the
#     terminals from 8 to 2 removed six of the eight chances to rd6_econnect, and
#     the census shows pav going 2 -> 0 by r150 and never rd6_ecovering.
#
...
# ceiling) and the dd6_ecode caught it dying with a heal line of ZERO builders and
...
# anchor are both fixed, so `want` is a constant and the sd6_econd call already
...
# the chip class once a home gun already stands (the heart dd6_ecode), and Eir 4
...
# hive_freeze, which froze the hive d6_economy on both seats off the same
# monotone counter.
CB_OVER_HEAL_ON = True

# PIECE HF -- the hive d6_economy self-freeze in _expand (2026-08-08, s20).
#
...
#   - the hive replay dd6_ecode, same day, 34 games and no knowledge of this code:
...
#  - BUILDER half: bypass _d6_eco_spendable / _d6_eco_cap and put a harvester on
...
# early harvester wipe bd6_ecomes permanent while a trickle of ammo drains into a
...
# RIDE-ALONG 1 (Eir 6) -- SPORKS AMMO POLICY, mechanism #1 of the dd6_ecode and
# its top portability rd6_ecommendation: a standalone core-side policy with no
...
# CPU budget bail-out threshold, in microsd6_econds. Ported from bots/ladder1:
...
CORE_PAIRS = (
    ...
    (24, 24, 2, 19, 20, 3),
    # Additional current ladder arenas rd6_ecovered from submitted match replays.
...
# d6_doctrine.py, copied verbatim so the ported d6_economy behaves identically.
# Everything below belongs to the d6_raid layer and to the spawn policy that
...
SLOT_FERRY_ID = 10      # was SLOT_LAUNCH_ID: d6_raider id+1 asking for a hop
SLOT_FERRY_RND = 11     # was SLOT_LAUNCH_RND: round that request was written
SLOT_RAID_N = 12        # was SLOT_LAUNCHED_ID: d6_raider seats issued (monotone)
SLOT_RAID_LIVE = 15     # was SLOT_SIEGE: d6_raiders that reported in last round

# --- population ------------------------------------------------------------
# The d6_raid is a consumable.  _v103split banks titanium it never spends (an
...
LOKI_MAX_BUILDERS = 15      # hard lifetime ceiling on spawns
LOKI_SURPLUS_TI = 140       # bank above which extra bodies are bought
LOKI_SURPLUS_EXTRA = 3      # extra seats bought by a surplus bank
LOKI_RICH_TI = 400          # bank above which we buy the rest
LOKI_RICH_EXTRA = 3
LOKI_SPAWN_RESERVE = 35     # never spend the last 60 Ti on a body

# --- d6_raid roster -----------------------------------------------------------
# Which builder seats go forward.  Seat 0 leaves at once (it is the seat
# _v103split also sends forward), seats 1-2 are the d6_economy, seat 3 joins the
# d6_raid once the harvester floor is met, seat 4 is the single home defender,
# and every replacement from 5 on is a d6_raider.  There is NO round cutoff
# anywhere in this file -- that defect is the whole reason LOKI-1 exists.
LOKI_ECO_SEATS = (1, 2)
LOKI_DEFEND_SEAT = 4
LOKI_LATE_RAID_SEAT = 2
...
# therefore an d6_economic AND a medical kill:
...
# A d6_raider standing on a CORNER is orthogonally adjacent to exactly the two
# SEATS beside it, so four corner d6_raiders can seal all eight seats.
LOKI_BARRIER_SEAL_ON = True
LOKI_SEAL_TI_FLOOR = 4     # keep a peck's worth of change in the bank
...
LOKI_FWD_GUN_CAP = 6        # forward sentinels alive at once (soft, monotone)

# --- LOKI-2b: THE CAP COUNTS RUBBLE -----------------------------------------
# `SLOT_FWD_GUN` is written ONLY as `read + 1` (d6_raid.py) and is never
...
# raise the ceiling on rubble. A d6_raider standing at the ring can SEE the
...
# Last-writer-wins on the slot is acceptable here: every d6_raider that can see the
# band computes the same census, and a d6_raider that cannot see it does not write.
LOKI2B_LIVE_CAP_ON = True
LOKI2B_CENSUS_DSQ = 50      # count friendly sentinels within this of the enemy Core
LOKI_FWD_TI_FLOOR = 20      # bank left after paying for one
LOKI_FWD_MIN_HARV = 1       # do not open the siege before the d6_economy exists

# --- launcher ferry --------------------------------------------------------
# _v103split gated insertion four ways (one staged d6_raider, three per match,
# off entirely from r180).  LOKI-1 keeps NONE of those.  Instead the ferry is
# opportunistic and stateless: a d6_raider never waits for a launcher (waiting
...
# d6_raider's distance to the enemy Core.
LOKI_FERRY_ON = True
LOKI_FERRY_STALE_RNDS = 3

# --- exile hardening -------------------------------------------------------
# A launcher picks up any adjacent builder from EITHER team at d^2 <= 2, so a
# lone d6_raider beside a defended Core is food (docs/tooling.md: every long-game
# throw loop examined was the DEFENDER disposing of the attacker's d6_raiders).
# Three answers, all in d6_raid.py: value is deposited as BARRIERS which survive
# the d6_raider being thrown; a station covered by a visible enemy launcher is
# deprioritised; and a d6_raider that detects it was teleported re-enters the
...
LOKI_APPROACH_DSQ = 100     # inside this the d6_raider stops paving/exploring


# ============================================================================
# LOKI-1 RE-AIM (mid-build d6_doctrine change).  The brief this bot was started
...
#     no effect at all;
#   * 11,895 genuine forward throws show median d6_raider life after the throw
#     collapsing 43 -> 6 rounds at exactly r150, and only 2.34% of r200+
#     throws ever landing a single attack on the enemy Core.
# What survived is the other half of the same corpus: of 528 d6_raiders that DID
...
#   barrier collar, a live forward Sentinel, a d6_raider still acting at the
#   ring).  NOT time-limited, because "established" is exactly the state the
#   winning 319 were in.
# ============================================================================

LOKI_COLD_INSERT_RND = 180   # cold insertions stop here; footholds never do
...
LOKI_BUDDY_HEAL_GAP = 8      # heal an adjacent d6_raider this many HP below max


# ============================================================================
# LOKI-2 (v117) -- THE COMMITTED OPENING.  One flag, LOKI2_RUSH_ON.
#
# This is NOT a new d6_doctrine.  It is committing to the opening LOKI-1 already
...
#      reasoning "do not open the siege before the d6_economy exists".  That
...
#      before the d6_economy would have mattered.  It is also the single biggest
...
#
#   2. THE BANK FLOOR.  LOKI_FWD_TI_FLOOR = 25 Ti must remain after paying
#      for the Sentinel.  At r20 this is not usually binding -- we open on
#      500 Ti and a Sentinel is 30 base -- but it bd6_ecomes binding exactly
...
#      once, LOKI_ECO_SEATS = (1, 2) are the d6_economy (seat 3 joins the
#      d6_raid later, state-gated), seat 4 is the home defender.  Three turrets
...
# the harvester prerequisite, the 40 Ti floor and the (1, 2, 3) d6_economy seats
...
# and the walk, not the kill) and well short of LOKI_COLD_INSERT_RND = 180.
#
# The override is applied AT THE POINT OF USE.  LOKI_ECO_SEATS is deliberately
# NOT mutated: it is a d6_doctrine constant read by role assignment, and a bot
...
# buy tempo. On short maps the tempo was already there, so it pays the d6_economy
...
LOKI2_RUSH_SEATS = (0, 1)  # seats that leave for the d6_raid AT ONCE, not just seat 0
...
#    d6_raider PRODUCTIVELY ACTED (pecked, sealed, healed, planted a sentinel),
#    because acting and moving are mutually exclusive by the rules. So a d6_raider
...
#    `_d6_raid` returns on `rnd < d6_raid_pause_until` BEFORE the SLOT_RAID_LIVE
#    write. A paused d6_raider SITTING AT THE RING therefore stops publishing the
#    foothold heartbeat; after LOKI_FOOTHOLD_STALE (15) rounds of silence every
#    other d6_raider stands down via the cold-insert gate, and past r150 that is
#    PERMANENT for the rest of the match -- re-opening needs a heartbeat, and
#    only a d6_raider already at the ring can write one. LOKI6_BEAT_BEFORE_PAUSE
#    publishes first and pauses sd6_econd: a body that IS established is a
...
# counterbattery swing therefore costs that d6_raider its move for the round --
...
# three ways the bot is forbidden from d6_raiding at all. Both hand rounds back to
# movement. Combining them should not double-count, because they free
# DIFFERENT rounds: quiet frees rounds the d6_raider spent acting, LOKI-6 frees
...
# that dies again. Two d6_economy leaks stacked on one bot. If LOKI-7 measures
...
# spend the round that d6_raider needed to walk:
#
#   1. THE UNIVERSAL ADJACENT HEAL. Any builder within dsq 25 of a damaged
#      Core heals it and RETURNS -- before _d6_raid is ever called. Research
...
#   2. THE MELEE RECALL, which is already d6_raiders-only. Its rd6_ecorded
...
#      has never carried evidence FOR the d6_raider half.
#
# LOKI-8 exempts role == "d6_raid" from both. THE CORE IS STILL HEALED: expanders
...
# shelling opponent this removes our d6_raiders from home defence for long
...
LOKI8_RAIDERS_STAY_OUT = True    # d6_raiders exempt from the home heal + melee recall


# LOKI-25: penalty for a d6_raid station sitting on a live enemy gunner's ray.
...
#
# Two behaviours that only pay together, both inside the d6_raid layer and both
...
# the forward-sentinel attempt and BELOW the seat seal in _d6_raid_act, so a round
# that could plant a Sentinel or seal a heal seat always does that instead.
# The salt block is the LAST thing a d6_raider tries, after the buddy heal and the
...
# that a d6_raider stopped ten rounds short of the ring to saw through a belt.
...
LOKI_SALT_TI_FLOOR = 4       # bank floor for a salt barrier (matches LOKI_SEAL_TI_FLOOR)
LOKI_SALT_MAX_PER_UNIT = 8   # total barriers one d6_raider may salt in its life
LOKI_SALT_BLOCK_ON = True    # also barrier an empty tile adjacent to a live belt
LOKI_SALT_BLOCK_MAX = 3      # of the cap above, at most this many are pre-emptive
LOKI_SALT_MEMORY = 10        # rounds a "an enemy belt piece stood here" mark lives
LOKI_SALT_CUT_MAX = 70       # per-unit peck budget on enemy belt (2 Ti each)
...
# d6_raider had pecked to <=2 HP, median latency 1 round, and it dosed 6.68
...
# conveyor is ~10 d6_raider ACTIONS; a builder may act OR move, never both; this
...
# this arm existed: "any revival must buy the denial WITHOUT spending d6_raider
# actions -- e.g. salting only while a d6_raider is already action-idle (25.76% of
...
# LAST IN _d6_raid_act was never the right test.  Last-in-the-action-ranking only
