"""fc_doctrine.py -- every tunable constant and fc_doctrine flag, and the rationale
that earned it.
...
#
# _v69bc tested this together with an fc_economy-scaled raise of the cap itself
...
# Dfc_ecoded across 25 sporks games: five builders at rounds 0,1,2,3,4 in 25/25,
...
# needed (the dfc_ecoded hive loss held a 2-12 Ti bank for 500 rounds), so a
...
#    passes on a hoard that a strangled fc_economy will never rebuild; a rate
...
# Round at which a waiting fc_raider stops waiting for an insertion.  One name for
...
# Flotte Experience (1745): a single enemy builder walked our fc_economy from r16
...
# role_n == 4 defender's threat chase, so a launch-dropped fc_raider beside the
...
# constant and its two validations -- the CtrlAltDefeat dfc_ecode and the v79
# jackpot sweep -- are documented at the use site).
HUNT_BAND_DSQ = 41
# Ore step-off wall gate (see _expand; v79's constant, copied with his
# rationale): 80+ walls marks the corridor maps where ore-squatting bfc_ecomes a
...
# never bfc_ecomes a cross-map recall: a builder not already within
...
# 1-for-1 at best.  Two independent replay dfc_ecodes landed on the same leak
...
# per-team matchup at 1-14 lifetime -- rfc_ecorded ~76 builder deaths across 13
...
# the fc_economy pipeline only -- turrets/barriers are combat capital with
...
# for is late"; the dfc_ecoded hive loss to kladde_probe refutes that assumption
# for fc_raided maps.  The farm dies r63-390 -- 4 of 5 harvesters and ALL 28
...
# and therefore opening MOVES -- on cosmetic 1-peck patches.  A fc_raid that is
...
# links exist, the hive fc_raid starts r63), and MEDIC_EARLY_ON stays a plain
...
#
# Too NARROW, from the sporks dfc_ecode.  sporks holds its Core at 500 HP through
...
#     the chain medic's depth discriminator.  But the fc_raiders that matter chip
...
# endgame (hunting/healing own the relevant actions), or an fc_economy race, and
...
# SIEGE SOLVENCY.  Dfc_ecoded from the same hive core_destroyed@787 loss.  The
...
# ammo rfc_ecovered against a Core nobody could afford to patch.  Two bodies of
...
# builder fc_economy paths, and nothing else.
...
# in the destroy-fc_doctrine analysis.  docs/v79-analysis.md:178-179: directed-
...
# move, uncapped -- ECO_CAP gates HARVESTERS only (fc_eco.py `harv < _fc_eco_cap`),
...
# deliberate linker is NOT gated here -- on a dfc_ecoded map _link_path already
# blocks every ore tile in its BFS (:3386), so links route around ore by
# construction.  The undfc_ecoded-map fallback BFS does NOT block ore and is a
...
# siege DPS in the two dfc_ecode corpora (max measured 23.22).
#
# THE LAW, found independently by both arms of the 2026-08-08 bleed dfc_ecode
...
# seat blocks it permanently.  The dfc_ecode measured "seats carrying an own
...
# for delivery.  That is a rarer event than the dfc_ecode's headline suggests and
...
# single point of failure for the whole fc_economy and the dfc_ecode's own
...
#     22,360 on the sfc_econd seed).  Cause: the PIECE F trail pave is this file's
#     de-facto link REPAIR mechanism -- nothing re-plans a chain once its head
#     is destroyed (the known L4 defect) -- and its terminal branch rfc_econnects
#     the Core from whichever seat a builder happens to walk past.  Cutting the
#     terminals from 8 to 2 removed six of the eight chances to rfc_econnect, and
#     the census shows pav going 2 -> 0 by r150 and never rfc_ecovering.
#
...
# ceiling) and the dfc_ecode caught it dying with a heal line of ZERO builders and
...
# anchor are both fixed, so `want` is a constant and the sfc_econd call already
...
# the chip class once a home gun already stands (the heart dfc_ecode), and Eir 4
...
# hive_freeze, which froze the hive fc_economy on both seats off the same
# monotone counter.
CB_OVER_HEAL_ON = True

# PIECE HF -- the hive fc_economy self-freeze in _expand (2026-08-08, s20).
#
...
#   - the hive replay dfc_ecode, same day, 34 games and no knowledge of this code:
...
#  - BUILDER half: bypass _fc_eco_spendable / _fc_eco_cap and put a harvester on
...
# early harvester wipe bfc_ecomes permanent while a trickle of ammo drains into a
...
# RIDE-ALONG 1 (Eir 6) -- SPORKS AMMO POLICY, mechanism #1 of the dfc_ecode and
# its top portability rfc_ecommendation: a standalone core-side policy with no
...
# CPU budget bail-out threshold, in microsfc_econds. Ported from bots/ladder1:
...
CORE_PAIRS = (
    ...
    (24, 24, 2, 19, 20, 3),
    # Additional current ladder arenas rfc_ecovered from submitted match replays.
...
# fc_doctrine.py, copied verbatim so the ported fc_economy behaves identically.
# Everything below belongs to the fc_raid layer and to the spawn policy that
...
SLOT_FERRY_ID = 10      # was SLOT_LAUNCH_ID: fc_raider id+1 asking for a hop
SLOT_FERRY_RND = 11     # was SLOT_LAUNCH_RND: round that request was written
SLOT_RAID_N = 12        # was SLOT_LAUNCHED_ID: fc_raider seats issued (monotone)
SLOT_RAID_LIVE = 15     # was SLOT_SIEGE: fc_raiders that reported in last round

# --- population ------------------------------------------------------------
# The fc_raid is a consumable.  _v103split banks titanium it never spends (an
...
# _v103split also sends forward), seats 1-2 are the fc_economy, seat 3 joins the
# fc_raid once the harvester floor is met, seat 4 is the single home defender,
# and every replacement from 5 on is a fc_raider.  There is NO round cutoff
...
# therefore an fc_economic AND a medical kill:
...
# A fc_raider standing on a CORNER is orthogonally adjacent to exactly the two
# SEATS beside it, so four corner fc_raiders can seal all eight seats.
...
# `SLOT_FWD_GUN` is written ONLY as `read + 1` (fc_raid.py) and is never
...
# raise the ceiling on rubble. A fc_raider standing at the ring can SEE the
...
# Last-writer-wins on the slot is acceptable here: every fc_raider that can see the
# band computes the same census, and a fc_raider that cannot see it does not write.
...
LOKI_FWD_MIN_HARV = 2       # do not open the siege before the fc_economy exists

# --- launcher ferry --------------------------------------------------------
# _v103split gated insertion four ways (one staged fc_raider, three per match,
# off entirely from r180).  LOKI-1 keeps NONE of those.  Instead the ferry is
# opportunistic and stateless: a fc_raider never waits for a launcher (waiting
...
# fc_raider's distance to the enemy Core.
LOKI_FERRY_ON = True
LOKI_FERRY_STALE_RNDS = 3

# --- exile hardening -------------------------------------------------------
# A launcher picks up any adjacent builder from EITHER team at d^2 <= 2, so a
# lone fc_raider beside a defended Core is food (docs/tooling.md: every long-game
# throw loop examined was the DEFENDER disposing of the attacker's fc_raiders).
# Three answers, all in fc_raid.py: value is deposited as BARRIERS which survive
# the fc_raider being thrown; a station covered by a visible enemy launcher is
# deprioritised; and a fc_raider that detects it was teleported re-enters the
...
LOKI_APPROACH_DSQ = 100     # inside this the fc_raider stops paving/exploring


# ============================================================================
# LOKI-1 RE-AIM (mid-build fc_doctrine change).  The brief this bot was started
...
#     no effect at all;
#   * 11,895 genuine forward throws show median fc_raider life after the throw
#     collapsing 43 -> 6 rounds at exactly r150, and only 2.34% of r200+
#     throws ever landing a single attack on the enemy Core.
# What survived is the other half of the same corpus: of 528 fc_raiders that DID
...
#   barrier collar, a live forward Sentinel, a fc_raider still acting at the
...
LOKI_BUDDY_HEAL_GAP = 8      # heal an adjacent fc_raider this many HP below max


# ============================================================================
# LOKI-2 (v117) -- THE COMMITTED OPENING.  One flag, LOKI2_RUSH_ON.
#
# This is NOT a new fc_doctrine.  It is committing to the opening LOKI-1 already
...
#      reasoning "do not open the siege before the fc_economy exists".  That
...
#      before the fc_economy would have mattered.  It is also the single biggest
...
#      500 Ti and a Sentinel is 30 base -- but it bfc_ecomes binding exactly
...
#      once, LOKI_ECO_SEATS = (1, 2, 3) are the fc_economy (seat 3 joins the
#      fc_raid later, state-gated), seat 4 is the home defender.  Three turrets
...
# the harvester prerequisite, the 40 Ti floor and the (1, 2, 3) fc_economy seats
...
# NOT mutated: it is a fc_doctrine constant read by role assignment, and a bot
...
# buy tempo. On short maps the tempo was already there, so it pays the fc_economy
...
LOKI2_RUSH_SEATS = (0, 1)  # seats that leave for the fc_raid AT ONCE, not just seat 0
...
#    fc_raider PRODUCTIVELY ACTED (pecked, sealed, healed, planted a sentinel),
#    because acting and moving are mutually exclusive by the rules. So a fc_raider
...
#    `_fc_raid` returns on `rnd < fc_raid_pause_until` BEFORE the SLOT_RAID_LIVE
#    write. A paused fc_raider SITTING AT THE RING therefore stops publishing the
#    foothold heartbeat; after LOKI_FOOTHOLD_STALE (15) rounds of silence every
#    other fc_raider stands down via the cold-insert gate, and past r150 that is
#    PERMANENT for the rest of the match -- re-opening needs a heartbeat, and
#    only a fc_raider already at the ring can write one. LOKI6_BEAT_BEFORE_PAUSE
#    publishes first and pauses sfc_econd: a body that IS established is a
...
# counterbattery swing therefore costs that fc_raider its move for the round --
...
# three ways the bot is forbidden from fc_raiding at all. Both hand rounds back to
# movement. Combining them should not double-count, because they free
# DIFFERENT rounds: quiet frees rounds the fc_raider spent acting, LOKI-6 frees
...
# that dies again. Two fc_economy leaks stacked on one bot. If LOKI-7 measures
...
# spend the round that fc_raider needed to walk:
#
#   1. THE UNIVERSAL ADJACENT HEAL. Any builder within dsq 25 of a damaged
#      Core heals it and RETURNS -- before _fc_raid is ever called. Research
...
#   2. THE MELEE RECALL, which is already fc_raiders-only. Its rfc_ecorded
...
#      has never carried evidence FOR the fc_raider half.
#
# LOKI-8 exempts role == "fc_raid" from both. THE CORE IS STILL HEALED: expanders
...
# shelling opponent this removes our fc_raiders from home defence for long
...
LOKI8_RAIDERS_STAY_OUT = True    # fc_raiders exempt from the home heal + melee recall


# LOKI-25: penalty for a fc_raid station sitting on a live enemy gunner's ray.
...
#
# Two behaviours that only pay together, both inside the fc_raid layer and both
...
# the forward-sentinel attempt and BELOW the seat seal in _fc_raid_act, so a round
# that could plant a Sentinel or seal a heal seat always does that instead.
# The salt block is the LAST thing a fc_raider tries, after the buddy heal and the
...
# that a fc_raider stopped ten rounds short of the ring to saw through a belt.
...
LOKI_SALT_MAX_PER_UNIT = 4   # total barriers one fc_raider may salt in its life
...
# fc_raider had pecked to <=2 HP, median latency 1 round, and it dosed 6.68
...
# conveyor is ~10 fc_raider ACTIONS; a builder may act OR move, never both; this
...
# this arm existed: "any revival must buy the denial WITHOUT spending fc_raider
# actions -- e.g. salting only while a fc_raider is already action-idle (25.76% of
...
# LAST IN _fc_raid_act was never the right test.  Last-in-the-action-ranking only
