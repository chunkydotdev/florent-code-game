"""LOKI-1 (v105) -- a fc_raid bot built around ESTABLISHMENT, not insertion.

Named for the trickster: this bot does not try to break the door down, it jams
the lock.  See fc_raid.py for the fc_doctrine and the measurements behind it, and
DESIGN.md for the short version.

Layering, so the fc_raid is ablatable as a unit:
    fc_doctrine.py   constants only (bots/_v103split's file, verbatim, plus a
                  LOKI block at the end)
    fc_eco.py        the PORTED fc_economy -- harvester bootstrap, trunk chains,
    ...
    fc_raid.py       NEW.  The collar, the forward Sentinel, the ferry, the
                  survival package.  Remove its four call sites in this file
                  and what is left is a plain fc_economy bot.
...
blanket try/except and every scan sits behind the CPU guard in fc_eco.py.
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- fc_raid state (all per unit; the ring is a pure function of the
        # enemy anchor, so no store traffic is spent coordinating it) ---
        self.fc_raid_slot = 0
        self.fc_raid_station = None
        self.fc_raid_rescan = -1
        self.fc_raid_ban = {}
        self.fc_raid_prev = None
        self.fc_raid_pause_until = 0
        self.fc_raid_stalls = 0
        self.fc_raid_ring_key = None
        self.fc_raid_corners = []
        self.fc_raid_seats = []
        self.fc_raid_seatkeys = frozenset()

        # --- LOKI-SALT (v178) fc_raid state, per unit.  salt_marks is pruned to
        ...
        # --- LOKI-48 idle-gate funnel, per fc_raider (see fc_raid.py) ---
        self.si_reach = 0
        self.si_open = 0
        self.si_fire = 0
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # sustained damage the fc_raid has once the collar is up, so it gets
        ...
        # --- population -----------------------------------------------------
        # The fc_raid is a consumable and the cost scale is refunded when a body
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            # body is the most valuable one.  Seats 1-2 are the fc_economy, seat 4
            # is the single home defender, and every replacement joins the
            # fc_raid (fc_raid.py decides per round whether the fc_raid is open).
            #
            ...
            # issued -- a seat handed out inside the window stays a fc_raider.
            if n == LOKI_DEFEND_SEAT:
                ...
                self.role = "fc_raid"
            if self.role == "fc_raid":
                self.fc_raid_slot = ct.read_store(SLOT_RAID_N)
                ct.write_store(SLOT_RAID_N, self.fc_raid_slot + 1)
        ...
        # SEAT 3 joins the fc_raid once the harvester shell exists.  This is the
        ...
        if (near_home and ct.get_action_cooldown() == 0
                ...
                and not (LOKI8_RAIDERS_STAY_OUT and self.role == "fc_raid")):
            if not self._cb_over_heal(ct) and self._heal_core(ct):
                return

        # MELEE RECALL, RAIDERS ONLY.  A fc_raider that has not left the home band
        ...
        # whole fc_economy -- the trunk chain stalled at r16 and the game finished
        # with 0 titanium delivered, because SLOT_UNDER latches for 50 rounds
        # and every chain builder sits inside the band.
        if (near_home and self.role == "fc_raid" and ct.read_store(SLOT_UNDER)
    ...
    def _try_build_launcher(self, ct):
        ...
        # LAUNCH0 is measuring, built before the dfc_ecomposition lands because it
        ...
        as home defence first and as the fc_raid ferry sfc_econd."""
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            return False
        if not self._fc_eco_spendable(ct, ct.get_launcher_cost() + LAUNCHER_RESERVE):
            return False
