"""LOKI-1 fc_raid layer -- THE COLLAR.  Ablatable as a unit: delete this module's
call sites in main.py and what remains is a plain fc_economy bot.
...
dfc_ecoded ladder games the NET HP to kill a Core is a stable 500-512 while the
...
A fc_raider standing on one of the four DIAGONAL ring tiles is orthogonally
adjacent to exactly the two seats flanking it and to no Core tile, so four
corner fc_raiders can seal all eight seats.  Sealed, the defender's heal rate is
...
The corpus that refuted it (11,895 forward throws) shows median fc_raider life
...
corpus is why this module still exists: of the 528 fc_raiders that did land
...
So the two are separated.  COLD INSERTION -- a fc_raider that holds nothing
walking at an undamaged ring -- is open only until LOKI_COLD_INSERT_RND.
FOOTHOLD REINFORCEMENT -- any round in which some fc_raider is still acting at the
ring, published as a heartbeat in SLOT_RAID_LIVE -- has no cutoff at all,
because that is precisely the state the winning fc_raiders were in.  Everything
...
fc_raiders, and a forward Sentinel -- a 40 HP BUILDING that keeps firing after
every body in the fc_raid is dead.
"""
from fcode import Direction, EntityType, Position

from fc_doctrine import *  # noqa: F401,F403
from fc_eco import core_corners, core_tiles, enemy_core_for, heal_seats, unpack_pos
...
class RaidMixin:
    ...
    def _ring(self, E):
        ...
        fc_raider derives the same twelve tiles with no store traffic at all.
        """
        key = (E.x, E.y)
        if self.fc_raid_ring_key != key:
            self.fc_raid_ring_key = key
            self.fc_raid_corners = core_corners(E, self.mw, self.mh)
            self.fc_raid_seats = heal_seats(E, self.mw, self.mh)
            self.fc_raid_seatkeys = frozenset((s.x, s.y) for s in self.fc_raid_seats)
        return self.fc_raid_corners, self.fc_raid_seats

    # --- the turn ----------------------------------------------------------

    def _foothold_live(self, ct, rnd):
        """Is some fc_raider still ACTING at the enemy ring right now?

        The heartbeat is written below by any fc_raider inside LOKI_ESTABLISH_DSQ
        of an enemy Core tile.  It is the one signal that separates the state
        the winning 319 fc_raiders were in ("established") from the state the
    ...
    def _fc_raid_open(self, ct, rnd, established):
        """May THIS fc_raider be on the fc_raid this round?

        Three ways in, and only the first is time-limited:
          * COLD INSERTION, open until LOKI_COLD_INSERT_RND;
          * this fc_raider is itself established at the ring -- never withdrawn,
            because a fc_raider that is still acting there is the asset;
    ...
    def _fc_raid(self, ct):
        E = self._enemy_anchor(ct)
        rnd = ct.get_current_round()
        if LOKI6_BEAT_BEFORE_PAUSE and E is not None:
            # FIX 2. Publish BEFORE any early return. A fc_raider standing at the
            ...
            try:
                ...
                pass
        if E is None or rnd < self.fc_raid_pause_until:
            # STATE-BASED stand-down: with no anchor there is nothing to fc_raid,
            # and a fc_raider that cannot reach the ring is worth more on ore than
        ...
        if established:
            ct.write_store(SLOT_RAID_LIVE, rnd + 1)
        elif not self._fc_raid_open(ct, rnd, established):
            # Cold-insertion window closed and no live foothold to reinforce.
            # The body goes back to the fc_economy; it is NOT retired, so the
        ...
        if self.fc_raid_prev is not None and p.distance_squared(self.fc_raid_prev) > LOKI_TELEPORT_DSQ:
            if (
                self.fc_raid_station is not None
                and p.distance_squared(E) > self.fc_raid_prev.distance_squared(E)
            ):
                self.fc_raid_ban[(self.fc_raid_station.x, self.fc_raid_station.y)] = rnd + 80
            self.fc_raid_station = None
            self.fc_raid_rescan = rnd
            self.tgt = None
            self.stuck = 0
            self.pave_prev = None
            self.pave_dir = None
        ...
        if ct.get_action_cooldown() == 0 and self._fc_raid_act(ct, E, near):
            return
        ...
        st = self._fc_raid_station(ct, E, near)
        ...
        if stalled:
            ...
            # the fc_economy for a bounded spell instead of grinding a wall.
            if self.fc_raid_station is not None:
                self.fc_raid_ban[(self.fc_raid_station.x, self.fc_raid_station.y)] = rnd + 120
            self.fc_raid_station = None
            self.fc_raid_rescan = rnd
            self.stuck = 0
            ...
            self.nav_fail = 0
            self.fc_raid_slot += 1
            self.fc_raid_stalls += 1
            if self.fc_raid_stalls >= 3:
                self.fc_raid_stalls = 0
                self.fc_raid_pause_until = rnd + 60

    # --- the productive action ---------------------------------------------

    def _fc_raid_act(self, ct, E, near):
        ...
        attacker's fc_raiders, so a lone fc_raider beside a defended Core is food.
        ...
        self._ring(E)
        seatkeys = self.fc_raid_seatkeys
        ...
        # and occupancy, so a seat one of our own fc_raiders is standing on is
        ...
        # fc_raider more than cancels it, and the measured failure mode is exactly
        # this: median fc_raider life after a throw is six rounds.  Gated on a
        ...
        # fc_raider parked beside its own barriers out-repairs a pecker two to one
        # on HP and eight to one on titanium, so the seal does not decay -- and
        # the same barriers are the fc_raider's own LOS cover against Gunner rays.
        if near:
            for d in CARDINALS:
                t = p.add(d)
                if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
        ...
        if (ti >= LOKI_PECK_TI_FLOOR and not LOKI_QUIET_ON
                and self._fc_raid_peck(ct, seatkeys)):
            return True
        ...
        # sfc_econd gate is the whole arm; see _salt_idle_ok.
        if LOKI_SALTIDLE_ON:
            self.si_reach += 1
            if self._salt_idle_ok(ct, E, p, near):
                ...
                if self._salt_turn(ct, E, p, ti):
                    ...
                    if LOKI_SALTIDLE_LOG:
                        # Funnel, cumulative per fc_raider: reached step 7 /
    ...
    def _salt_idle_ok(self, ct, E, p, near):
        """Would this fc_raider have MOVED this round?  If yes, salt is refused.
        ...
        20 HP conveyor costs ~10 fc_raider ACTIONS, acting and moving are mutually
        ...
        _fc_raid_act only proves no better ACTION existed; the parent's next move
        ...
        `_fc_raid_station` is safe to call here: it caches on `fc_raid_rescan`, which
        ...
        and the expensive scan still happens at most once per fc_raider per round.
    ...
    def _salt_forward(self, t, E):
        ...
        A fc_raider that has been thrown home, or one working a belt that runs
        past the midline, must never barrier our own ground or saw at a
        conveyor on our side of the map -- both of those spend the fc_raider on
        something the fc_economy layer owns.
        """
    ...
    def _salt_turn(self, ct, E, p, ti):
        ...
        Ordered: salt the corpse first (that is the round the cut bfc_ecomes
        ...
        # (0) MARK.  Rfc_ecord every adjacent enemy belt piece BEFORE anything is
        ...
        # tile gone, and it is the cheapest thing a fc_raider can do with a round
    ...
    def _fc_raid_peck(self, ct, seatkeys):
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        and the bank floor are relaxed (fc_doctrine.py, LOKI-2 block).  99.3% of
        1,269 early Core kills are turret fire and the sub-r80 recipe is three
        turrets planted by r22 -- a rush is over before the fc_economy the LOKI-1
        ...
        for d in CARDINALS:
            ...
            for target in tiles:
                ...
                if LOKI2B_LIVE_CAP_ON:
                    ...
                    # sfc_econd fc_raider in the same round does not double-spend the
    ...
    def _fc_raid_station(self, ct, E, near):
        """Which of the twelve ring tiles this fc_raider is walking to.

        FAR: a deterministic seat derived from this unit's fc_raid slot, so the
        fc_raid spreads across the ring on the way in without a single store write
        ...
        if not stations:
            return None
        if not near:
            return stations[self.fc_raid_slot % len(stations)]
        if self.fc_raid_station is not None and rnd < self.fc_raid_rescan:
            return self.fc_raid_station
        self.fc_raid_rescan = rnd + LOKI_RAID_RESCAN
        ...
        try:
            for bid in ct.get_nearby_buildings():
                ...
                # where our fc_raiders stand, not of the game.  A gunner's shot is a
        ...
        for i, s in enumerate(stations):
            key = (s.x, s.y)
            if self.fc_raid_ban.get(key, 0) > rnd:
                continue
            ...
            k = (score, (s.x * 17 + s.y * 31 + self.fc_raid_slot * 7) % 97, s.y, s.x)
            if best_k is None or k < best_k:
                best, best_k = s, k
        self.fc_raid_station = best
        return best
    ...
    def _open_seats_by(self, ct, corner):
        ...
        for d in CARDINALS:
            t = corner.add(d)
            if (t.x, t.y) not in self.fc_raid_seatkeys:
                continue
            try:
    ...
    def _fc_raid_ferry_ping(self, ct):
        ...
        table says belongs to the opponent.  Here the fc_raider simply walks, and
    ...
    def _launcher_turn(self, ct):
        """Launcher: exile intruders first, then ferry fc_raiders forward.
        ...
        # against our fc_raiders, and it is the cheapest home defence we own.
        friendly_bots = []
        ...
        # 2. FERRY.  Only the fc_raider that pinged, only while the ping is fresh,
