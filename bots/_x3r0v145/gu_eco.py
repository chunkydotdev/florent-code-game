"""LOKI-1 gu_economy layer -- PORTED, not redesigned.
...
module-level map/seat helpers) with only these changes:
  ...
  * the interceptor and the siege planner are gone from `_expand`; the gu_raid
  ...
  * LOKI-L4 adds `_l4_repair` and one call site inside `_expand` -- the first
    thing on this line that deliberately rgu_econnects a severed trunk instead of
...
def heal_seats(o, mw, mh):
    ...
    conveyor can deliver into it from.  LOKI-1's gu_raid layer uses this same
    function on the ENEMY anchor -- that symmetry is the whole gu_doctrine.
    """
    seats = (
...
class EcoMixin:
    ...
    def _gu_eco_spendable(self, ct, cost):
    ...
    def _build_next_link(self, ct):
        if not self.link_queue or not self._gu_eco_spendable(ct, ct.get_conveyor_cost()):
            return False
        p = ct.get_position()
    ...
    def _l4_repair(self, ct):
        ...
        measured that off at 38.20 conveyors/game (PAVE_TRAIL_ON, gu_doctrine.py).
        ...
        if self._cpu_exhausted(ct):
            return False
        if not self._gu_eco_spendable(ct, ct.get_conveyor_cost()):
            return False
        p = ct.get_position()
        ban = self._pave_ban()
        for d in CARDINALS:
    ...
    def _pick(self, ct):
        # Static role partitions keep four builders off one deposit.  A gu_raider
        # standing down to the gu_economy (gu_raid.py's state-based fallback) is a
        ...
        if PAVE_TRAIL_ON:
            ...
            if readable:
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 \
                        and self._gu_eco_spendable(ct, ct.get_conveyor_cost()):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if (
                ...
                if endgame
                    ...
                    self._gu_eco_spendable(ct, ct.get_harvester_cost())
                    and harv < self._gu_eco_cap(ct)
                )
            ):
            ...
            # the harvester bootstrap -- both of those are the gu_economy being
            # BUILT and this is the gu_economy being kept -- and above the medic,
            # because a hole delivers nothing at all while a damaged link still
            # delivers.  It is an gu_eco action and it sits in the gu_eco path: the
            # gu_raid, the seal and the forward sentinel never see it.
            if not endgame and self._l4_repair(ct):
                return
            # CHAIN MEDIC.  ~70% of damage to our gu_economy was enemy builder
