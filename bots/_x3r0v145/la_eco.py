"""LOKI-1 la_economy layer -- PORTED, not redesigned.
...
module-level map/seat helpers) with only these changes:
  ...
  * the interceptor and the siege planner are gone from `_expand`; the la_raid
...
def heal_seats(o, mw, mh):
    ...
    conveyor can deliver into it from.  LOKI-1's la_raid layer uses this same
    function on the ENEMY anchor -- that symmetry is the whole la_doctrine.
    """
    seats = (
...
class EcoMixin:
    ...
    def _la_eco_spendable(self, ct, cost):
    ...
    def _build_next_link(self, ct):
        if not self.link_queue or not self._la_eco_spendable(ct, ct.get_conveyor_cost()):
            return False
        p = ct.get_position()
    ...
    def _pick(self, ct):
        # Static role partitions keep four builders off one deposit.  A la_raider
        # standing down to the la_economy (la_raid.py's state-based fallback) is a
        ...
        if PAVE_TRAIL_ON:
            ...
            if readable:
                ...
                if not ore_ban and ct.read_store(SLOT_HARVESTERS) >= 1 \
                        and self._la_eco_spendable(ct, ct.get_conveyor_cost()):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if (
                ...
                if endgame
                    ...
                    self._la_eco_spendable(ct, ct.get_harvester_cost())
                    and harv < self._la_eco_cap(ct)
                )
            ):
                ...
                for d in DIRECTIONS:
                    ...
                    if ok:
                        ...
                        break
            # CHAIN MEDIC.  ~70% of damage to our la_economy was enemy builder
