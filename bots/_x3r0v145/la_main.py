"""LOKI-1 (v105) -- a la_raid bot built around ESTABLISHMENT, not insertion.

Named for the trickster: this bot does not try to break the door down, it jams
the lock.  See la_raid.py for the la_doctrine and the measurements behind it, and
DESIGN.md for the short version.

Layering, so the la_raid is ablatable as a unit:
    la_doctrine.py   constants only (bots/_v103split's file, verbatim, plus a
                  LOKI block at the end)
    la_eco.py        the PORTED la_economy -- harvester bootstrap, trunk chains,
    ...
    la_raid.py       NEW.  The collar, the forward Sentinel, the ferry, the
                  survival package.  Remove its four call sites in this file
                  and what is left is a plain la_economy bot.
...
blanket try/except and every scan sits behind the CPU guard in la_eco.py.
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- la_raid state (all per unit; the ring is a pure function of the
        # enemy anchor, so no store traffic is spent coordinating it) ---
        self.la_raid_slot = 0
        self.la_raid_station = None
        self.la_raid_rescan = -1
        self.la_raid_ban = {}
        self.la_raid_prev = None
        self.la_raid_pause_until = 0
        self.la_raid_stalls = 0
        self.la_raid_ring_key = None
        self.la_raid_corners = []
        self.la_raid_seats = []
        self.la_raid_seatkeys = frozenset()

        # --- LOKI-SALT (v178) la_raid state, per unit.  salt_marks is pruned to
        ...
        # --- LOKI-48 idle-gate funnel, per la_raider (see la_raid.py) ---
        self.si_reach = 0
        self.si_open = 0
        self.si_fire = 0
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # sustained damage the la_raid has once the collar is up, so it gets
        ...
        # --- population -----------------------------------------------------
        # The la_raid is a consumable and the cost scale is refunded when a body
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            # body is the most valuable one.  Seats 1-2 are the la_economy, seat 4
            # is the single home defender, and every replacement joins the
            # la_raid (la_raid.py decides per round whether the la_raid is open).
            #
            ...
            # issued -- a seat handed out inside the window stays a la_raider.
            if n == LOKI_DEFEND_SEAT:
                ...
                self.role = "la_raid"
            if self.role == "la_raid":
                self.la_raid_slot = ct.read_store(SLOT_RAID_N)
                ct.write_store(SLOT_RAID_N, self.la_raid_slot + 1)
        ...
        # SEAT 3 joins the la_raid once the harvester shell exists.  This is the
        ...
        if (near_home and ct.get_action_cooldown() == 0
                ...
                and not (LOKI8_RAIDERS_STAY_OUT and self.role == "la_raid")):
            if not self._cb_over_heal(ct) and self._heal_core(ct):
                return

        # MELEE RECALL, RAIDERS ONLY.  A la_raider that has not left the home band
        ...
        # whole la_economy -- the trunk chain stalled at r16 and the game finished
        # with 0 titanium delivered, because SLOT_UNDER latches for 50 rounds
        # and every chain builder sits inside the band.
        if (near_home and self.role == "la_raid" and ct.read_store(SLOT_UNDER)
    ...
    def _try_build_launcher(self, ct):
        ...
        # LAUNCH0 is measuring, built before the dla_ecomposition lands because it
        ...
        as home defence first and as the la_raid ferry sla_econd."""
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            return False
        if not self._la_eco_spendable(ct, ct.get_launcher_cost() + LAUNCHER_RESERVE):
            return False
