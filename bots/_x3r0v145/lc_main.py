"""LOKI-CHAMP -- a lc_raid bot built around ESTABLISHMENT, not insertion.

Named for the trickster: this bot does not try to break the door down, it jams
the lock.  See lc_raid.py for the lc_doctrine and the measurements behind it, and
DESIGN.md for the short version.

Layering, so the lc_raid is ablatable as a unit:
    lc_doctrine.py   constants only (bots/_v103split's file, verbatim, plus a
                  LOKI block at the end)
    lc_eco.py        the PORTED lc_economy -- harvester bootstrap, trunk chains,
    ...
    lc_raid.py       NEW.  The collar, the forward Sentinel, the ferry, the
                  survival package.  Remove its four call sites in this file
                  and what is left is a plain lc_economy bot.
...
blanket try/except and every scan sits behind the CPU guard in lc_eco.py.
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- lc_raid state (all per unit; the ring is a pure function of the
        # enemy anchor, so no store traffic is spent coordinating it) ---
        self.lc_raid_slot = 0
        self.lc_raid_station = None
        self.lc_raid_rescan = -1
        self.lc_raid_ban = {}
        self.lc_raid_prev = None
        self.lc_raid_pause_until = 0
        self.lc_raid_stalls = 0
        self.lc_raid_ring_key = None
        self.lc_raid_corners = []
        self.lc_raid_seats = []
        self.lc_raid_seatkeys = frozenset()

        # --- LOKI-SALT (v178) lc_raid state, per unit.  salt_marks is pruned to
        ...
        # --- LOKI-48 idle-gate funnel, per lc_raider (see lc_raid.py) ---
        self.si_reach = 0
        self.si_open = 0
        self.si_fire = 0
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # sustained damage the lc_raid has once the collar is up, so it gets
        ...
        # --- population -----------------------------------------------------
        # The lc_raid is a consumable and the cost scale is refunded when a body
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            # body is the most valuable one.  Seats 1-2 are the lc_economy, seat 4
            # is the single home defender, and every replacement joins the
            # lc_raid (lc_raid.py decides per round whether the lc_raid is open).
            #
            ...
            # issued -- a seat handed out inside the window stays a lc_raider.
            if n == LOKI_DEFEND_SEAT:
                ...
                self.role = "lc_raid"
            if self.role == "lc_raid":
                self.lc_raid_slot = ct.read_store(SLOT_RAID_N)
                ct.write_store(SLOT_RAID_N, self.lc_raid_slot + 1)
        ...
        # SEAT 3 joins the lc_raid once the harvester shell exists.  This is the
        ...
        if (near_home and ct.get_action_cooldown() == 0
                ...
                and not (LOKI8_RAIDERS_STAY_OUT and self.role == "lc_raid")):
            if not self._cb_over_heal(ct) and self._heal_core(ct):
                return

        # MELEE RECALL, RAIDERS ONLY.  A lc_raider that has not left the home band
        ...
        # whole lc_economy -- the trunk chain stalled at r16 and the game finished
        # with 0 titanium delivered, because SLOT_UNDER latches for 50 rounds
        # and every chain builder sits inside the band.
        if (near_home and self.role == "lc_raid" and ct.read_store(SLOT_UNDER)
    ...
    def _try_build_launcher(self, ct):
        ...
        # LAUNCH0 is measuring, built before the dlc_ecomposition lands because it
        ...
        as home defence first and as the lc_raid ferry slc_econd."""
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            return False
        if not self._lc_eco_spendable(ct, ct.get_launcher_cost() + LAUNCHER_RESERVE):
            return False
