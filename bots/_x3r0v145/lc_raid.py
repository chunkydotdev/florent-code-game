"""LOKI-1 lc_raid layer -- THE COLLAR.  Ablatable as a unit: delete this module's
call sites in main.py and what remains is a plain lc_economy bot.
...
dlc_ecoded ladder games the NET HP to kill a Core is a stable 500-512 while the
...
A lc_raider standing on one of the four DIAGONAL ring tiles is orthogonally
adjacent to exactly the two seats flanking it and to no Core tile, so four
corner lc_raiders can seal all eight seats.  Sealed, the defender's heal rate is
...
The corpus that refuted it (11,895 forward throws) shows median lc_raider life
...
corpus is why this module still exists: of the 528 lc_raiders that did land
...
So the two are separated.  COLD INSERTION -- a lc_raider that holds nothing
walking at an undamaged ring -- is open only until LOKI_COLD_INSERT_RND.
FOOTHOLD REINFORCEMENT -- any round in which some lc_raider is still acting at the
ring, published as a heartbeat in SLOT_RAID_LIVE -- has no cutoff at all,
because that is precisely the state the winning lc_raiders were in.  Everything
...
lc_raiders, and a forward Sentinel -- a 40 HP BUILDING that keeps firing after
every body in the lc_raid is dead.
"""
from fcode import Direction, EntityType, Position

from lc_doctrine import *  # noqa: F401,F403
from lc_eco import core_corners, core_tiles, enemy_core_for, heal_seats, unpack_pos
...
class RaidMixin:
    ...
    def _ring(self, E):
        ...
        lc_raider derives the same twelve tiles with no store traffic at all.
        """
        key = (E.x, E.y)
        if self.lc_raid_ring_key != key:
            self.lc_raid_ring_key = key
            self.lc_raid_corners = core_corners(E, self.mw, self.mh)
            self.lc_raid_seats = heal_seats(E, self.mw, self.mh)
            self.lc_raid_seatkeys = frozenset((s.x, s.y) for s in self.lc_raid_seats)
        return self.lc_raid_corners, self.lc_raid_seats

    # --- the turn ----------------------------------------------------------

    def _foothold_live(self, ct, rnd):
        """Is some lc_raider still ACTING at the enemy ring right now?

        The heartbeat is written below by any lc_raider inside LOKI_ESTABLISH_DSQ
        of an enemy Core tile.  It is the one signal that separates the state
        the winning 319 lc_raiders were in ("established") from the state the
    ...
    def _lc_raid_open(self, ct, rnd, established):
        """May THIS lc_raider be on the lc_raid this round?

        Three ways in, and only the first is time-limited:
          * COLD INSERTION, open until LOKI_COLD_INSERT_RND;
          * this lc_raider is itself established at the ring -- never withdrawn,
            because a lc_raider that is still acting there is the asset;
    ...
    def _lc_raid(self, ct):
        E = self._enemy_anchor(ct)
        rnd = ct.get_current_round()
        if LOKI6_BEAT_BEFORE_PAUSE and E is not None:
            # FIX 2. Publish BEFORE any early return. A lc_raider standing at the
            ...
            try:
                ...
                pass
        if E is None or rnd < self.lc_raid_pause_until:
            # STATE-BASED stand-down: with no anchor there is nothing to lc_raid,
            # and a lc_raider that cannot reach the ring is worth more on ore than
        ...
        if established:
            ct.write_store(SLOT_RAID_LIVE, rnd + 1)
        elif not self._lc_raid_open(ct, rnd, established):
            # Cold-insertion window closed and no live foothold to reinforce.
            # The body goes back to the lc_economy; it is NOT retired, so the
        ...
        if self.lc_raid_prev is not None and p.distance_squared(self.lc_raid_prev) > LOKI_TELEPORT_DSQ:
            if (
                self.lc_raid_station is not None
                and p.distance_squared(E) > self.lc_raid_prev.distance_squared(E)
            ):
                self.lc_raid_ban[(self.lc_raid_station.x, self.lc_raid_station.y)] = rnd + 80
            self.lc_raid_station = None
            self.lc_raid_rescan = rnd
            self.tgt = None
            self.stuck = 0
            self.pave_prev = None
            self.pave_dir = None
        ...
        if ct.get_action_cooldown() == 0 and self._lc_raid_act(ct, E, near):
            return
        ...
        st = self._lc_raid_station(ct, E, near)
        ...
        if stalled:
            ...
            # the lc_economy for a bounded spell instead of grinding a wall.
            if self.lc_raid_station is not None:
                self.lc_raid_ban[(self.lc_raid_station.x, self.lc_raid_station.y)] = rnd + 120
            self.lc_raid_station = None
            self.lc_raid_rescan = rnd
            self.stuck = 0
            ...
            self.nav_fail = 0
            self.lc_raid_slot += 1
            self.lc_raid_stalls += 1
            if self.lc_raid_stalls >= 3:
                self.lc_raid_stalls = 0
                self.lc_raid_pause_until = rnd + 60

    # --- the productive action ---------------------------------------------

    def _lc_raid_act(self, ct, E, near):
        ...
        attacker's lc_raiders, so a lone lc_raider beside a defended Core is food.
        ...
        self._ring(E)
        seatkeys = self.lc_raid_seatkeys
        ...
        # and occupancy, so a seat one of our own lc_raiders is standing on is
        ...
        # lc_raider more than cancels it, and the measured failure mode is exactly
        # this: median lc_raider life after a throw is six rounds.  Gated on a
        ...
        # lc_raider parked beside its own barriers out-repairs a pecker two to one
        # on HP and eight to one on titanium, so the seal does not decay -- and
        # the same barriers are the lc_raider's own LOS cover against Gunner rays.
        if near:
            for d in CARDINALS:
                t = p.add(d)
                if not (0 <= t.x < self.mw and 0 <= t.y < self.mh):
        ...
        if (ti >= LOKI_PECK_TI_FLOOR and not LOKI_QUIET_ON
                and self._lc_raid_peck(ct, seatkeys)):
            return True
        ...
        # slc_econd gate is the whole arm; see _salt_idle_ok.
        if LOKI_SALTIDLE_ON:
            self.si_reach += 1
            if self._salt_idle_ok(ct, E, p, near):
                ...
                if self._salt_turn(ct, E, p, ti):
                    ...
                    if LOKI_SALTIDLE_LOG:
                        # Funnel, cumulative per lc_raider: reached step 7 /
    ...
    def _salt_idle_ok(self, ct, E, p, near):
        """Would this lc_raider have MOVED this round?  If yes, salt is refused.
        ...
        20 HP conveyor costs ~10 lc_raider ACTIONS, acting and moving are mutually
        ...
        _lc_raid_act only proves no better ACTION existed; the parent's next move
        ...
        `_lc_raid_station` is safe to call here: it caches on `lc_raid_rescan`, which
        ...
        and the expensive scan still happens at most once per lc_raider per round.
    ...
    def _salt_forward(self, t, E):
        ...
        A lc_raider that has been thrown home, or one working a belt that runs
        past the midline, must never barrier our own ground or saw at a
        conveyor on our side of the map -- both of those spend the lc_raider on
        something the lc_economy layer owns.
        """
    ...
    def _salt_turn(self, ct, E, p, ti):
        ...
        Ordered: salt the corpse first (that is the round the cut blc_ecomes
        ...
        # (0) MARK.  Rlc_ecord every adjacent enemy belt piece BEFORE anything is
        ...
        # tile gone, and it is the cheapest thing a lc_raider can do with a round
    ...
    def _lc_raid_peck(self, ct, seatkeys):
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        and the bank floor are relaxed (lc_doctrine.py, LOKI-2 block).  99.3% of
        1,269 early Core kills are turret fire and the sub-r80 recipe is three
        turrets planted by r22 -- a rush is over before the lc_economy the LOKI-1
        ...
        for d in CARDINALS:
            ...
            for target in tiles:
                ...
                if LOKI2B_LIVE_CAP_ON:
                    ...
                    # slc_econd lc_raider in the same round does not double-spend the
    ...
    def _lc_raid_station(self, ct, E, near):
        """Which of the twelve ring tiles this lc_raider is walking to.

        FAR: a deterministic seat derived from this unit's lc_raid slot, so the
        lc_raid spreads across the ring on the way in without a single store write
        ...
        if not stations:
            return None
        if not near:
            return stations[self.lc_raid_slot % len(stations)]
        if self.lc_raid_station is not None and rnd < self.lc_raid_rescan:
            return self.lc_raid_station
        self.lc_raid_rescan = rnd + LOKI_RAID_RESCAN
        ...
        try:
            for bid in ct.get_nearby_buildings():
                ...
                # where our lc_raiders stand, not of the game.  A gunner's shot is a
        ...
        for i, s in enumerate(stations):
            key = (s.x, s.y)
            if self.lc_raid_ban.get(key, 0) > rnd:
                continue
            ...
            k = (score, (s.x * 17 + s.y * 31 + self.lc_raid_slot * 7) % 97, s.y, s.x)
            if best_k is None or k < best_k:
                best, best_k = s, k
        self.lc_raid_station = best
        return best
    ...
    def _open_seats_by(self, ct, corner):
        ...
        for d in CARDINALS:
            t = corner.add(d)
            if (t.x, t.y) not in self.lc_raid_seatkeys:
                continue
            try:
    ...
    def _lc_raid_ferry_ping(self, ct):
        ...
        table says belongs to the opponent.  Here the lc_raider simply walks, and
    ...
    def _launcher_turn(self, ct):
        """Launcher: exile intruders first, then ferry lc_raiders forward.
        ...
        # against our lc_raiders, and it is the cheapest home defence we own.
        friendly_bots = []
        ...
        # 2. FERRY.  Only the lc_raider that pinged, only while the ping is fresh,
