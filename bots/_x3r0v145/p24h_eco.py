    ...
    def _nav(self, ct, pave=True):
        ...
        for d in (CARDINALS[(idx + 1) % 4], CARDINALS[(idx + 3) % 4], desired.opposite()):
            if self._move(ct, d, pave):
                return
        self.stuck += 1
        # DIGOUT (s36): every cardinal is blocked. If any blocker is OUR OWN
        # building, destroy is FREE (no cooldown, unlimited, ally-only) and we
        # never called it anywhere in this tree — a seat-sealer that walls its
        # own exit paces forever otherwise (observed live by Magnus, and the
        # engine hands us the tool). Prefer the desired direction, then the
        # cheapest blocker (barrier before conveyor before anything else).
        if LOKI_DIGOUT_ON and self.stuck >= 2 and ct.get_action_cooldown() == 0:
            best, best_rank = None, 99
            rank = {EntityType.BARRIER: 0, EntityType.CONVEYOR: 1, EntityType.SPLITTER: 2}
            for d in (desired,) + tuple(c for c in CARDINALS if c != desired):
                ...
                try:
                    ...
                    continue
                r = rank.get(et, 99)
                if r < best_rank and ct.can_destroy(t):
                    best, best_rank = t, r
                    if d == desired and r == 0:
                        break
            if best is not None:
                ct.destroy(best)
                self.stuck = 0
