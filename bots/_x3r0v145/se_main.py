"""LOKI-SEAL2 -- seal/salt floor 4 + fwd_ti_floor 10; RUSH off.

Named for the trickster: this bot does not try to break the door down, it jams
the lock.  See se_raid.py for the se_doctrine and the measurements behind it, and
DESIGN.md for the short version.

Layering, so the se_raid is ablatable as a unit:
    se_doctrine.py   constants only (bots/_v103split's file, verbatim, plus a
                  LOKI block at the end)
    se_eco.py        the PORTED se_economy -- harvester bootstrap, trunk chains,
    ...
    se_raid.py       NEW.  The collar, the forward Sentinel, the ferry, the
                  survival package.  Remove its four call sites in this file
                  and what is left is a plain se_economy bot.
...
blanket try/except and every scan sits behind the CPU guard in se_eco.py.
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- se_raid state (all per unit; the ring is a pure function of the
        # enemy anchor, so no store traffic is spent coordinating it) ---
        self.se_raid_slot = 0
        self.se_raid_station = None
        self.se_raid_rescan = -1
        self.se_raid_ban = {}
        self.se_raid_prev = None
        self.se_raid_pause_until = 0
        self.se_raid_stalls = 0
        self.se_raid_ring_key = None
        self.se_raid_corners = []
        self.se_raid_seats = []
        self.se_raid_seatkeys = frozenset()

        # --- LOKI-SALT (v178) se_raid state, per unit.  salt_marks is pruned to
        ...
        # --- LOKI-48 idle-gate funnel, per se_raider (see se_raid.py) ---
        self.si_reach = 0
        self.si_open = 0
        self.si_fire = 0
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # sustained damage the se_raid has once the collar is up, so it gets
        ...
        # --- population -----------------------------------------------------
        # The se_raid is a consumable and the cost scale is refunded when a body
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            # body is the most valuable one.  Seats 1-2 are the se_economy, seat 4
            # is the single home defender, and every replacement joins the
            # se_raid (se_raid.py decides per round whether the se_raid is open).
            #
            ...
            # issued -- a seat handed out inside the window stays a se_raider.
            if n == LOKI_DEFEND_SEAT:
                ...
                self.role = "se_raid"
            if self.role == "se_raid":
                self.se_raid_slot = ct.read_store(SLOT_RAID_N)
                ct.write_store(SLOT_RAID_N, self.se_raid_slot + 1)
        ...
        # SEAT 3 joins the se_raid once the harvester shell exists.  This is the
        ...
        if (near_home and ct.get_action_cooldown() == 0
                ...
                and not (LOKI8_RAIDERS_STAY_OUT and self.role == "se_raid")):
            if not self._cb_over_heal(ct) and self._heal_core(ct):
                return

        # MELEE RECALL, RAIDERS ONLY.  A se_raider that has not left the home band
        ...
        # whole se_economy -- the trunk chain stalled at r16 and the game finished
        # with 0 titanium delivered, because SLOT_UNDER latches for 50 rounds
        # and every chain builder sits inside the band.
        if (near_home and self.role == "se_raid" and ct.read_store(SLOT_UNDER)
    ...
    def _try_build_launcher(self, ct):
        ...
        # LAUNCH0 is measuring, built before the dse_ecomposition lands because it
        ...
        as home defence first and as the se_raid ferry sse_econd."""
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            return False
        if not self._se_eco_spendable(ct, ct.get_launcher_cost() + LAUNCHER_RESERVE):
            return False
