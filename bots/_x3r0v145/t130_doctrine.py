"""t130_doctrine.py -- every tunable constant and t130_doctrine flag, and the rationale
that earned it.
...
#
# _v69bc tested this together with an t130_economy-scaled raise of the cap itself
...
# Dt130_ecoded across 25 sporks games: five builders at rounds 0,1,2,3,4 in 25/25,
...
# needed (the dt130_ecoded hive loss held a 2-12 Ti bank for 500 rounds), so a
...
#    passes on a hoard that a strangled t130_economy will never rebuild; a rate
...
# Round at which a waiting t130_raider stops waiting for an insertion.  One name for
...
# Flotte Experience (1745): a single enemy builder walked our t130_economy from r16
...
# role_n == 4 defender's threat chase, so a launch-dropped t130_raider beside the
...
# constant and its two validations -- the CtrlAltDefeat dt130_ecode and the v79
# jackpot sweep -- are documented at the use site).
HUNT_BAND_DSQ = 41
# Ore step-off wall gate (see _expand; v79's constant, copied with his
# rationale): 80+ walls marks the corridor maps where ore-squatting bt130_ecomes a
...
# never bt130_ecomes a cross-map recall: a builder not already within
...
# 1-for-1 at best.  Two independent replay dt130_ecodes landed on the same leak
...
# per-team matchup at 1-14 lifetime -- rt130_ecorded ~76 builder deaths across 13
...
# the t130_economy pipeline only -- turrets/barriers are combat capital with
...
# for is late"; the dt130_ecoded hive loss to kladde_probe refutes that assumption
# for t130_raided maps.  The farm dies r63-390 -- 4 of 5 harvesters and ALL 28
...
# and therefore opening MOVES -- on cosmetic 1-peck patches.  A t130_raid that is
...
# links exist, the hive t130_raid starts r63), and MEDIC_EARLY_ON stays a plain
...
#
# Too NARROW, from the sporks dt130_ecode.  sporks holds its Core at 500 HP through
...
#     the chain medic's depth discriminator.  But the t130_raiders that matter chip
...
# endgame (hunting/healing own the relevant actions), or an t130_economy race, and
...
# SIEGE SOLVENCY.  Dt130_ecoded from the same hive core_destroyed@787 loss.  The
...
# ammo rt130_ecovered against a Core nobody could afford to patch.  Two bodies of
...
# builder t130_economy paths, and nothing else.
...
# in the destroy-t130_doctrine analysis.  docs/v79-analysis.md:178-179: directed-
...
# move, uncapped -- ECO_CAP gates HARVESTERS only (t130_eco.py `harv < _t130_eco_cap`),
...
# deliberate linker is NOT gated here -- on a dt130_ecoded map _link_path already
# blocks every ore tile in its BFS (:3386), so links route around ore by
# construction.  The undt130_ecoded-map fallback BFS does NOT block ore and is a
...
# siege DPS in the two dt130_ecode corpora (max measured 23.22).
#
# THE LAW, found independently by both arms of the 2026-08-08 bleed dt130_ecode
...
# seat blocks it permanently.  The dt130_ecode measured "seats carrying an own
...
# for delivery.  That is a rarer event than the dt130_ecode's headline suggests and
...
# single point of failure for the whole t130_economy and the dt130_ecode's own
...
#     22,360 on the st130_econd seed).  Cause: the PIECE F trail pave is this file's
#     de-facto link REPAIR mechanism -- nothing re-plans a chain once its head
#     is destroyed (the known L4 defect) -- and its terminal branch rt130_econnects
#     the Core from whichever seat a builder happens to walk past.  Cutting the
#     terminals from 8 to 2 removed six of the eight chances to rt130_econnect, and
#     the census shows pav going 2 -> 0 by r150 and never rt130_ecovering.
#
...
# ceiling) and the dt130_ecode caught it dying with a heal line of ZERO builders and
...
# anchor are both fixed, so `want` is a constant and the st130_econd call already
...
# the chip class once a home gun already stands (the heart dt130_ecode), and Eir 4
...
# hive_freeze, which froze the hive t130_economy on both seats off the same
# monotone counter.
CB_OVER_HEAL_ON = True

# PIECE HF -- the hive t130_economy self-freeze in _expand (2026-08-08, s20).
#
...
#   - the hive replay dt130_ecode, same day, 34 games and no knowledge of this code:
...
#  - BUILDER half: bypass _t130_eco_spendable / _t130_eco_cap and put a harvester on
...
# early harvester wipe bt130_ecomes permanent while a trickle of ammo drains into a
...
# RIDE-ALONG 1 (Eir 6) -- SPORKS AMMO POLICY, mechanism #1 of the dt130_ecode and
# its top portability rt130_ecommendation: a standalone core-side policy with no
...
# CPU budget bail-out threshold, in microst130_econds. Ported from bots/ladder1:
...
CORE_PAIRS = (
    ...
    (24, 24, 2, 19, 20, 3),
    # Additional current ladder arenas rt130_ecovered from submitted match replays.
...
EXTRA_MAP_CODES = (
    ...
    ((30, 30, 14, 2, 14, 26), ""),  # glacierkeep
    ...
    ((30, 30, 2, 14, 26, 14), ""),  # valkyrie
...
# t130_doctrine.py, copied verbatim so the ported t130_economy behaves identically.
# Everything below belongs to the t130_raid layer and to the spawn policy that
...
SLOT_FERRY_ID = 10      # was SLOT_LAUNCH_ID: t130_raider id+1 asking for a hop
SLOT_FERRY_RND = 11     # was SLOT_LAUNCH_RND: round that request was written
SLOT_RAID_N = 12        # was SLOT_LAUNCHED_ID: t130_raider seats issued (monotone)
SLOT_RAID_LIVE = 15     # was SLOT_SIEGE: t130_raiders that reported in last round

# --- population ------------------------------------------------------------
# The t130_raid is a consumable.  _v103split banks titanium it never spends (an
...
# _v103split also sends forward), seats 1-2 are the t130_economy, seat 3 joins the
# t130_raid once the harvester floor is met, seat 4 is the single home defender,
# and every replacement from 5 on is a t130_raider.  There is NO round cutoff
...
# therefore an t130_economic AND a medical kill:
...
# A t130_raider standing on a CORNER is orthogonally adjacent to exactly the two
# SEATS beside it, so four corner t130_raiders can seal all eight seats.
LOKI_BARRIER_SEAL_ON = True
LOKI_SEAL_TI_FLOOR = 12     # keep a peck's worth of change in the bank
...
# `SLOT_FWD_GUN` is written ONLY as `read + 1` (t130_raid.py) and is never
...
# raise the ceiling on rubble. A t130_raider standing at the ring can SEE the
...
# Last-writer-wins on the slot is acceptable here: every t130_raider that can see the
# band computes the same census, and a t130_raider that cannot see it does not write.
...
LOKI_FWD_MIN_HARV = 2       # do not open the siege before the t130_economy exists

# --- launcher ferry --------------------------------------------------------
# _v103split gated insertion four ways (one staged t130_raider, three per match,
# off entirely from r180).  LOKI-1 keeps NONE of those.  Instead the ferry is
# opportunistic and stateless: a t130_raider never waits for a launcher (waiting
...
# t130_raider's distance to the enemy Core.
LOKI_FERRY_ON = True
LOKI_FERRY_STALE_RNDS = 3

# --- exile hardening -------------------------------------------------------
# A launcher picks up any adjacent builder from EITHER team at d^2 <= 2, so a
# lone t130_raider beside a defended Core is food (docs/tooling.md: every long-game
# throw loop examined was the DEFENDER disposing of the attacker's t130_raiders).
# Three answers, all in t130_raid.py: value is deposited as BARRIERS which survive
# the t130_raider being thrown; a station covered by a visible enemy launcher is
# deprioritised; and a t130_raider that detects it was teleported re-enters the
...
# --- t130_raid navigation -------------------------------------------------------
LOKI_RAID_RESCAN = 6        # rounds between full station rescans
LOKI_APPROACH_DSQ = 100     # inside this the t130_raider stops paving/exploring


# ============================================================================
# LOKI-1 RE-AIM (mid-build t130_doctrine change).  The brief this bot was started
...
#     no effect at all;
#   * 11,895 genuine forward throws show median t130_raider life after the throw
#     collapsing 43 -> 6 rounds at exactly r150, and only 2.34% of r200+
#     throws ever landing a single attack on the enemy Core.
# What survived is the other half of the same corpus: of 528 t130_raiders that DID
...
#   barrier collar, a live forward Sentinel, a t130_raider still acting at the
...
LOKI_BUDDY_HEAL_GAP = 8      # heal an adjacent t130_raider this many HP below max


# ============================================================================
# LOKI-2 (v117) -- THE COMMITTED OPENING.  One flag, LOKI2_RUSH_ON.
#
# This is NOT a new t130_doctrine.  It is committing to the opening LOKI-1 already
...
#      reasoning "do not open the siege before the t130_economy exists".  That
...
#      before the t130_economy would have mattered.  It is also the single biggest
...
#      500 Ti and a Sentinel is 30 base -- but it bt130_ecomes binding exactly
...
#      once, LOKI_ECO_SEATS = (1, 2, 3) are the t130_economy (seat 3 joins the
#      t130_raid later, state-gated), seat 4 is the home defender.  Three turrets
...
# the harvester prerequisite, the 40 Ti floor and the (1, 2, 3) t130_economy seats
...
# NOT mutated: it is a t130_doctrine constant read by role assignment, and a bot
...
# buy tempo. On short maps the tempo was already there, so it pays the t130_economy
...
LOKI2_RUSH_SEATS = (0, 1)  # seats that leave for the t130_raid AT ONCE, not just seat 0
...
#    t130_raider PRODUCTIVELY ACTED (pecked, sealed, healed, planted a sentinel),
#    because acting and moving are mutually exclusive by the rules. So a t130_raider
...
#    `_t130_raid` returns on `rnd < t130_raid_pause_until` BEFORE the SLOT_RAID_LIVE
#    write. A paused t130_raider SITTING AT THE RING therefore stops publishing the
#    foothold heartbeat; after LOKI_FOOTHOLD_STALE (15) rounds of silence every
#    other t130_raider stands down via the cold-insert gate, and past r150 that is
#    PERMANENT for the rest of the match -- re-opening needs a heartbeat, and
#    only a t130_raider already at the ring can write one. LOKI6_BEAT_BEFORE_PAUSE
#    publishes first and pauses st130_econd: a body that IS established is a
...
# counterbattery swing therefore costs that t130_raider its move for the round --
...
# three ways the bot is forbidden from t130_raiding at all. Both hand rounds back to
# movement. Combining them should not double-count, because they free
# DIFFERENT rounds: quiet frees rounds the t130_raider spent acting, LOKI-6 frees
...
# that dies again. Two t130_economy leaks stacked on one bot. If LOKI-7 measures
...
# spend the round that t130_raider needed to walk:
#
#   1. THE UNIVERSAL ADJACENT HEAL. Any builder within dsq 25 of a damaged
#      Core heals it and RETURNS -- before _t130_raid is ever called. Research
...
#   2. THE MELEE RECALL, which is already t130_raiders-only. Its rt130_ecorded
...
#      has never carried evidence FOR the t130_raider half.
#
# LOKI-8 exempts role == "t130_raid" from both. THE CORE IS STILL HEALED: expanders
...
# shelling opponent this removes our t130_raiders from home defence for long
...
LOKI8_RAIDERS_STAY_OUT = True    # t130_raiders exempt from the home heal + melee recall


# LOKI-25: penalty for a t130_raid station sitting on a live enemy gunner's ray.
...
# LOKI-APPR (QUEUE #47 = #28 x #45-iter3): build ONE launcher on APPROACH
# detection — not standing (screen-opposed: launcher premium −6.34pp,
# LAUNCH0 vs BOTH0) and not at siege time (t130_eco-shut: iter-2 falsifier,
# bank 1-44 vs cost 52-61 once the ladder is up). Approach = an enemy
# builder bot within LOKI_APPR_DSQ of our core, seen by the unit deciding
# the launcher build. Creep precedes plants, plants precede the bank drain
# (LingLing40's ladder starts r89) — the trigger window is the only point
# where the launcher is both WANTED and AFFORDABLE. Inside the triggered
# state both gates are waived (MIN_RND, RESERVE→0); outside it the tree is
# byte-identical in behaviour to the incumbent.
LOKI_APPR_LAUNCHER = True
LOKI_APPR_DSQ = 50
LOKI_APPR_LOG = True       # dose tags APPR45/GATE45/EVICT45; platform strips
                           # stdout, so local-only by nature


# ============================================================================
# LOKI-SALT (v178) -- CUT THEIR BELT, THEN SALT THE TILE.
#
# Two behaviours that only pay together, both inside the t130_raid layer and both
...
# the forward-sentinel attempt and BELOW the seat seal in _t130_raid_act, so a round
# that could plant a Sentinel or seal a heal seat always does that instead.
# The salt block is the LAST thing a t130_raider tries, after the buddy heal and the
...
# that a t130_raider stopped ten rounds short of the ring to saw through a belt.
...
LOKI_SALT_MAX_PER_UNIT = 4   # total barriers one t130_raider may salt in its life
...
# t130_raider had pecked to <=2 HP, median latency 1 round, and it dosed 6.68
...
# conveyor is ~10 t130_raider ACTIONS; a builder may act OR move, never both; this
...
# this arm existed: "any revival must buy the denial WITHOUT spending t130_raider
# actions -- e.g. salting only while a t130_raider is already action-idle (25.76% of
...
# LAST IN _t130_raid_act was never the right test.  Last-in-the-action-ranking only
