"""LOKI-1 (v105) -- a t130_raid bot built around ESTABLISHMENT, not insertion.

Named for the trickster: this bot does not try to break the door down, it jams
the lock.  See t130_raid.py for the t130_doctrine and the measurements behind it, and
DESIGN.md for the short version.

Layering, so the t130_raid is ablatable as a unit:
    t130_doctrine.py   constants only (bots/_v103split's file, verbatim, plus a
                  LOKI block at the end)
    t130_eco.py        the PORTED t130_economy -- harvester bootstrap, trunk chains,
    ...
    t130_raid.py       NEW.  The collar, the forward Sentinel, the ferry, the
                  survival package.  Remove its four call sites in this file
                  and what is left is a plain t130_economy bot.
...
blanket try/except and every scan sits behind the CPU guard in t130_eco.py.
"""
import math
...
class Player(EcoMixin, RaidMixin):
    ...
    def __init__(self):
        ...
        # --- t130_raid state (all per unit; the ring is a pure function of the
        # enemy anchor, so no store traffic is spent coordinating it) ---
        self.t130_raid_slot = 0
        self.t130_raid_station = None
        self.t130_raid_rescan = -1
        self.t130_raid_ban = {}
        self.t130_raid_prev = None
        self.t130_raid_pause_until = 0
        self.t130_raid_stalls = 0
        self.t130_raid_ring_key = None
        self.t130_raid_corners = []
        self.t130_raid_seats = []
        self.t130_raid_seatkeys = frozenset()

        # --- LOKI-SALT (v178) t130_raid state, per unit.  salt_marks is pruned to
        ...
        # --- LOKI-48 idle-gate funnel, per t130_raider (see t130_raid.py) ---
        self.si_reach = 0
        self.si_open = 0
        self.si_fire = 0
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            # sustained damage the t130_raid has once the collar is up, so it gets
        ...
        # --- population -----------------------------------------------------
        # The t130_raid is a consumable and the cost scale is refunded when a body
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            # body is the most valuable one.  Seats 1-2 are the t130_economy, seat 4
            # is the single home defender, and every replacement joins the
            # t130_raid (t130_raid.py decides per round whether the t130_raid is open).
            #
            ...
            # issued -- a seat handed out inside the window stays a t130_raider.
            if n == LOKI_DEFEND_SEAT:
                ...
                self.role = "t130_raid"
            if self.role == "t130_raid":
                self.t130_raid_slot = ct.read_store(SLOT_RAID_N)
                ct.write_store(SLOT_RAID_N, self.t130_raid_slot + 1)
        ...
        # SEAT 3 joins the t130_raid once the harvester shell exists.  This is the
        ...
        if (near_home and ct.get_action_cooldown() == 0
                ...
                and not (LOKI8_RAIDERS_STAY_OUT and self.role == "t130_raid")):
            if not self._cb_over_heal(ct) and self._heal_core(ct):
                return

        # MELEE RECALL, RAIDERS ONLY.  A t130_raider that has not left the home band
        ...
        # whole t130_economy -- the trunk chain stalled at r16 and the game finished
        # with 0 titanium delivered, because SLOT_UNDER latches for 50 rounds
        # and every chain builder sits inside the band.
        if (near_home and self.role == "t130_raid" and ct.read_store(SLOT_UNDER)
    ...
    def _try_build_launcher(self, ct):
        ...
        # LAUNCH0 is measuring, built before the dt130_ecomposition lands because it
        ...
        # LOKI-APPR (#47): approach detection by the deciding unit's own
        # vision — an enemy builder bot already inside LOKI_APPR_DSQ of our
        # core. Triggered state waives the round gate and the reserve; the
        # per-round GATE45 log keeps the no-fire attributable (research's
        # iter-2 requirement, carried forward on request).
        approached = False
        if LOKI_APPR_LAUNCHER and self.core is not None:
            for eid in ct.get_nearby_entities():
                try:
                    if (ct.get_team(eid) != self.team
                            and ct.get_entity_type(eid) == EntityType.BUILDER_BOT
                            and ct.get_position(eid).distance_squared(
                                self.core) <= LOKI_APPR_DSQ):
                        approached = True
                        break
                except Exception:
                    continue
        if ct.get_current_round() < LAUNCHER_MIN_RND:
            if not approached:
                return False
            if LOKI_APPR_LOG:
                print(f"APPR45 r={ct.get_current_round()} "
                      f"bank={ct.get_global_resources()}")
        ...
        as home defence first and as the t130_raid ferry st130_econd."""
        seen_launcher = False
        for eid in ct.get_nearby_buildings():
            try:
        ...
        if ct.read_store(SLOT_HARVESTERS) < 1:
            if approached and LOKI_APPR_LOG:
                print(f"GATE45 r={ct.get_current_round()} gate=harvester "
                      f"harv={ct.read_store(SLOT_HARVESTERS)}")
            return False
        reserve = 0 if approached else LAUNCHER_RESERVE
        if not self._t130_eco_spendable(ct, ct.get_launcher_cost() + reserve):
            if approached and LOKI_APPR_LOG:
                print(f"GATE45 r={ct.get_current_round()} gate=t130_eco "
                      f"bank={ct.get_global_resources()} "
                      f"cost={ct.get_launcher_cost()} reserve={reserve}")
            return False
