...
from dirty_doctrine import *  # noqa: F401,F403
from dirty_eco import core_corners, core_tiles, enemy_core_for, heal_seats, unpack_pos
...
class RaidMixin:
    ...
    def _raid_act(self, ct, E, near):
        ...
        """
        # LABOR_DENY carve-out: kill adjacent enemy builders for tempo.
        if self._labor_deny(ct):
            return True
    ...
    def _labor_deny(self, ct):
        """Peck an orthogonally adjacent enemy builder (dirty tempo steal)."""
        try:
            ti = ct.get_global_resources()
        except Exception:
            return False
        if ti < LOKI_PECK_TI_FLOOR or ct.get_action_cooldown() != 0:
            return False
        p = ct.get_position()
        for d in CARDINALS:
            tpos = p.add(d)
            try:
                uid = ct.get_tile_builder_bot_id(tpos)
