...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        # The new top-team replays do not grow from bank thresholds alone:
        # each working harvester funds roughly two additional builders.  Feed
        # live production back into the population cap so a healthy economy
        # compounds, while the five-builder opening still protects the bank.
        if harv >= 2:
            budget = max(budget, LOKI_BASE_BUILDERS + 2 * harv)
    ...
    def _builder(self, ct):
        ...
        # Jython's winning line opens a Launcher on turn 2, before the first
        # harvester.  Let the initial raider establish that ferry immediately.
        if (self.role_n == 0 and rnd <= 5 and ct.get_action_cooldown() == 0
                and self._try_build_launcher(ct)):
            return
