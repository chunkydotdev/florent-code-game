...
class EcoMixin:
    ...
    def _bfs_direction(self, ct, target):
        ...
        LOKI-TURBO.  Same blocked set, same three goal cases, same neighbour
        ordering (target-biased, mirrored on `self.idx & 1`), same first-step
        tie-break, same three fallbacks to `cardinal_direction_to` -- run over
        the padded flat grid described above instead of over tuple sets.  The
        state byte is 0 = free, 1 = blocked or already seen, 2 = goal, which
        collapses LOKI's `key in seen or key in blocked` plus `key in goals`
        into one subscript.  Goals are never blocked by construction (all three
        branches filter on the blocked state), and a goal can never be reached
        twice because the first neighbour that lands on one returns, so the
        three states cannot collide.

        Verified identical to LOKI's version by tools/turbo_identity.py.
        """
        p = ct.get_position()
        ...
        if not (0 <= tx < mw and 0 <= ty < mh):
            ...
            # greedy step.  Same answer, without the flood.
            return p.cardinal_direction_to(target)
        ...
        st[start] = 0                       # LOKI: blocked.discard(start)
        ...
        # greedy step when it was already over budget.  Time is frozen inside
        # the sandbox (engine_mechanics.md M) so that clock is the ONLY one,
        # and the whole flood now costs less than the budget slice the probe
        # was protecting -- so it is asked once, up front, which preserves the
        # degradation for a unit that arrives here already spent.
        if self._cpu_exhausted(ct):
            return p.cardinal_direction_to(target)

        # A frontier list per BFS level, in discovery order, is the same order
        # a single FIFO queue produces -- and `cf` carries LOKI's `first`, the
        # step this branch of the flood started with.
        cur = []
        cf = []
        for j in (0, 1, 2, 3):
            n = start + flat[oi[j]]
