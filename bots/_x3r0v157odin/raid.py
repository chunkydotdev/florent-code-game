...
class RaidMixin:
    ...
    def _tw_try_launcher(self, ct, E, p, ti, rnd):
        ...
        if n is None or n >= TW_LAUNCH_CAP:
        ...
        if n >= 1 and rnd - self.tw_launch_since < TW_LAUNCH_AGE:
        ...
        for dx, dy in CARD_DELTAS:
            ...
            try:
                ...
                continue
            if TW_LOG_ON:
                print("TW launch (%d,%d) n=%d" % (tx, ty, n))
            return True
    ...
    def _tw_launch_walk(self, ct, E):
        ...
        # census, so the walk for launcher #1 is untouched.
        if (self._tw_reserve_gated(n)
                and not self._tw_reserve(ct, E, rnd, ti, cost)):
            return None
        ...
        for c in corners:
            ...
            for dx, dy in CARD_DELTAS:
                ...
                if tx == p.x and ty == p.y:
                    return p        # already in place: hold, and build below
                key = (abs(tx - p.x) + abs(ty - p.y), tx, ty)
                if best_key is None or key < best_key:
                    best, best_key = Position(tx, ty), key
        return best
    ...
    def _tw_launcher(self, ct, lp, E):
        ...
        for eid in ents:
            try:
                ...
                continue
            key = (0 if (bp.x, bp.y) in seatkeys else 1, d, bp.x, bp.y)
