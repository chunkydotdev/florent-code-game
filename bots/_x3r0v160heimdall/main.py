...
class Player(EcoMixin, RaidMixin):
    ...
    def _arch_note(self, ct, pressure=False, intruder=False,
                   turret_anywhere=False, near_count=0):
    ...
    def _rg_gun(self, ct, rnd):
        ...
        if not RG_ON or self.rg_done or self.core is None:
            return False
        if rnd < RG_MIN_RND or rnd >= RG_UNTIL:
            return False
        ...
        if not self._eco_spendable(ct, cost + RG_TI_FLOOR):
            return False
