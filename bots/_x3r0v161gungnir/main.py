...
class Player(EcoMixin, RaidMixin):
    ...
    def _sh_cap(self, ct):
        ...
        if EB_ON and EB_SH_ON and self._eb_latched(ct):
            return EB_SH_BODIES
    ...
    def _rg_gun(self, ct, rnd):
        ...
        if rnd < (EB_RG_MIN_RND if eb else RG_MIN_RND) or rnd >= RG_UNTIL:
            return False
        if (EB_ON and EB_RG_HOLD_ON and not eb_latch and rnd < EB_HOLD_UNTIL):
            if EB_LOG and not self.eb_held and self._rg_trigger(ct, rnd):
                self.eb_held = True
                print("EB hold r=%d" % rnd)
            return False
    ...
    def _turret(self, ct):
        ...
        if (RG_ON and RG_CHASE_ON and turret_type == EntityType.GUNNER
                and self.rg_rot_ti + RG_ROT_COST <= RG_ROT_BUDGET
                and self._rg_is_mine(ct, p)):
            self._rg_chase(ct, p, turret_type)
            return
