...
from eco import (
    SG_SOCKET_FACE, dsq_core, nearest_cardinal, nearest_core_tile, sg_socket,
)
        ...
        try:
            for uid in ct.get_nearby_units():
                ...
                if abs(up.x - t.x) + abs(up.y - t.y) == 1:
                    lower += 1
                    if lower >= RING_EVICT_BODIES:
                        return False
        except Exception:
            return False
        return lower < RING_EVICT_BODIES
