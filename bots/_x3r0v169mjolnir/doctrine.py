...
# ===========================================================================
# WAVE 37 -- PLANK RANK.  THE COUNTER-BATTERY IS *REACHED*, NOT RE-PRICED.
# ONE ARM, ONE CUT, ON THE LIVE BOT (v168 = bots/leap35_fix1only).
#
# THE DEFECT, AND IT HAS NOW BEEN NAMED TWICE.  `results/wave35/SCREEN_P1B.md`
# finding 1, replicated as the recommendation of `results/wave36/SCREEN_YARD.md`
# section 4: over 925 instrumented P1-state rounds (a live besieger inside
# HUNT_BAND_DSQ of our Core with no home turret answering it), the CB GATE was
# opened on 63 of them -- 3.3x the parent's 19 -- and `_try_counterbattery` was
# nevertheless REACHED on ~3 %.  `_defend` was entered on 33 of the 925, the
# buy loop on 22, and two gunners were bought in thirty games.  The price half
# of the family works and it is worthless, because the routine it funds sits at
# the bottom of `_builder`'s job ladder -- under SAP, SEATHOLD, REPAIR, SIPHON
# and the whole role split -- and BEHIND a `role == "defend"` test that exactly
# ONE body in the match ever satisfies (LOKI_DEFEND_SEAT = 4; a dead defender
# is never replaced, every replacement joins the raid).  `TEST_V167.md` defect
# #2 is what it costs: `their_ge2` (they hold >= 2 forward tubes) separates
# 16 % wins from 63 %, and it is Pantheon's 20/20 rule.
#
# THE CUT, AND ITS WHOLE CONTENT: **rank, not price.**  In the P1 state the
# nearest eligible HOME body runs `_try_counterbattery` -- the routine already
# in the tree, unmodified, at its own prices -- from a rank ABOVE the station
# and errand work that currently eats the turn.  No new build path, no new
# turret type, no discount, no reserve change, no gate change.  With RANK_ON
# False the tree is `bots/leap35_fix1only` statement for statement.
#
# WHAT IT IS RANKED BELOW, and each of these is a pre-registered protection:
#   * the UNIVERSAL ADJACENT CORE HEAL.  `bots/leap35_p1b` was KILLED for
#     exactly one mechanism -- it skipped the heal in this state, bought
#     nothing with the freed action, and our Core died .289 vs .156 (p = .017)
#     on `mimic_pivot2`.  THIS ARM NEVER SKIPS A HEAL.  It is reached only by a
#     body that has already declined to heal, so the convicted mechanism cannot
#     re-appear through it.
#   * OPEN prefill / trunk -- the opening.  Both are self-latching
#     (`op_prefill_done` / `op_trunk_done`) and finished well inside r12, so
#     leaving them above costs the arm nothing after the opening and protects
#     the registered "= 2 sockets @r10" bar inside it.
#   * RING refill and RING evict -- our own delivery sockets, the five-round
#     window and the plug rule.  BELT evict is NOT protected: an enemy building
#     on a belt tile is income, a turret in the home band is the Core.
# and what it is ranked ABOVE: BELT evict, LPECK, RG, SAP, SEATHOLD, REPAIR,
# SIPHON and the entire role split (`_raid` / `_defend` / `_expand`).
#
# IT SPENDS THE ACTION AND NEVER THE TURN.  Same construction as LPECK and RG,
# which sit ten lines below it and were shipped on this argument: the body
# still takes its ordinary move, so an expander, a trunk carrier or a raider
# that happens to be the nearest hand loses no ground -- it loses one action it
# was about to spend on a 2-damage peck or a 3 Ti conveyor.  Everything ranked
# under it re-tests `get_action_cooldown()` itself, so the elevation IS the
# ranking; no `return` is needed and none is taken.
#
# THE STATE IS `_cb_over_heal`'s, MINUS THE ROLE TEST AND MINUS THE BANK TEST.
# A live `_cb_target`, no home turret bearing (`_home_gun_bears`, which is
# `_live_home_gun` while CB_BEARING_GATE_ON is False) -- and then ONE test the
# defender's gate does not make: `_enemy_type_at` must read a GUNNER or a
# SENTINEL standing on the threat tile THIS round.  That is the brief's "enemy
# forward turret", and it is also the ghost filter: SLOT_THREAT is at best one
# round old, is never cleared when its subject dies, and named a tile holding
# no enemy at all on 893 of the ladder-loss rounds (`_cb_target`'s docstring).
# Wave 34 classified 2 502 such targets: GUNNER on 516, BUILDER_BOT on 772,
# None on 247.  A ghost and a wandering builder both buy nothing here -- which
# is the entire reason the arm cannot inflate the global +10 %/build cost scale
# on transients, the harm CB_TARGET_BUILDERS_ON was turned off for.
#
# RANK_BODIES = 1, AND HOW IT IS ENFORCED WITHOUT A STORE SLOT.  Comm writes
# are at best one round old, so a claim slot cannot cap a within-round race.
# The cap is an ELECTION instead: a body defers to any friendly builder it can
# see that is also inside the home band and is strictly closer to the threat
# (ties by entity id, so the order is total and every body computes the same
# winner).  A builder's vision is r^2 = 20 and the band is r^2 = 41, so two
# bodies at opposite edges cannot see each other; that residue is closed one
# layer down, because the first body's turret is a BUILDING standing beside it
# and the second body's `_live_home_gun` then answers True and shuts the state.
#
# NEVER A BODY ON THEIR RING, and THE PLUG RULE is the one implementation of
# that: `_f0_plug`, verbatim, the wave-27 rule ring.py already calls at its own
# walk site.  The arm is home defence and a forward body dragged into a home
# build is the LOKI-QUIET defect.  In practice the home-band test already
# excludes it on every map in the pool; the plug test is the guard for a map
# where the two bands could touch, and it fails CLOSED.
#
# THE CAP HOLDS ONLY WHILE THE STATE PERSISTS.  There is no latch and no
# memory: every term is re-derived from this round's sighting, so the round the
# besieger dies or a home gun starts bearing, the elevation stops.
#
# INERTNESS.  Both call sites are `if RANK_ON and ...` guards and `_rank_cb`
# re-tests RANK_ON at its head, so with the flag off no helper body runs, no
# store is read and no API call is issued.  `bots/leap37_rank_off` is that
# twin; the two trees differ in this one line only.
RANK_ON = True               # master.  False == bots/leap35_fix1only exactly.
RANK_BODIES = 1              # bodies elevated per round.  Enforced by the
                             # nearest-visible election, not by a comm claim.
RANK_HOME_DSQ = HUNT_BAND_DSQ   # 41.  "a HOME body" is one inside the same
                             # band the besieger has to be in -- the arm never
                             # elevates a body that could not site a gun on it.
RANK_TURRET_ONLY_ON = True   # require `_enemy_type_at` to read a live GUNNER
                             # or SENTINEL on the threat tile this round.
RANK_LOG = False             # `RANK` markers, off in competition; the _dbg
                             # twin turns them on.
