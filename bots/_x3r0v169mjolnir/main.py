...
class Player(EcoMixin, RaidMixin, OpenMixin, RingMixin, SipMixin):
    ...
    def __init__(self):
        ...
        # WAVE 28, F6.  Per-unit, one assignment at spawn.
        self.f6_refuse = 0              # F6: illegal targets refused
        ...
        # --- WAVE 31, PLANK B RESTORE_HOLE (doctrine.py "WAVE 31").  Per unit,
        # all of it: the facing each of our conveyors had while it was ALIVE
        # (capped at RESTORE_FACE_MAX in `_rep_watch`), this round's candidate
        # holes and the round they were computed, the hole this body is walking
        # to and when it committed, its relay ceiling, and the memo for the
        # severance predicate.  None of it is published and none of it grows
        # with the match. ---
        self.rest_face = {}             # tile -> Direction our belt pointed
        self.rest_holes = ()            # candidate holes, nearest first
        self.rest_rnd = -1              # round `rest_holes` was computed
        self.rest_tgt = None            # (x, y) of the hole being walked to
        self.rest_since = 0             # round that commitment was made
        self.rest_n = 0                 # per-body ceiling on hole relays
        self.rest_sev = False           # memo: is the trunk severed?
        self.rest_sev_rnd = -1          # ...and the round it was read
        ...
        self.col_log = {}
        # WAVE 32, FIX 3 (STICK).  Budget-waived relays THIS BODY has laid on a
        # socket it owned and lost.  One integer; it cannot grow with the match
        # and it is capped by STICK_MAX_RELAYS.  Per-body because `col_bricks`
        # is per-body -- a team-wide cap would need a comm slot, which FIX 3
        ...
        # WAVE 30, ARM 4 BELT EVICT.  Per unit and bounded: one memo
        # pair for the belt census, one for the target list, a tile
        # clock, a lifetime counter and the turret-side Core latch.
        # No shared state and no comm slot, exactly as ARMS 1-3.
        self.belt_tiles_rnd = -1        # round of the belt census...
        self.belt_tiles = ()            # ...and its result
        self.belt_ev_rnd = -1           # round of the target scan...
        self.belt_ev_tgts = ()          # ...and its result, best first
        self.belt_ev_key = None         # (x, y) being pecked
        self.belt_ev_left = 0           # pecks left on that tile
        self.belt_ev_total = 0          # lifetime pecks, this arm
        self.belt_core = None           # turret-side OUR-Core anchor
        self.belt_core_done = False     # ...looked up once per life
    ...
    def _builder(self, ct):
        ...
        if RING_ON and ct.get_action_cooldown() == 0:
            ...
            if self._ring_evict(ct, rnd):
                return
        # WAVE 37, PLANK RANK -- THE COUNTER-BATTERY REACHABILITY CUT, AND ITS
        # RANK IS THE WHOLE ARM (doctrine.py "WAVE 37 -- PLANK RANK").
        # BELOW the universal adjacent Core heal (that is `leap35_p1b`'s
        # convicted mechanism and this arm may never re-create it), BELOW the
        # OPENING and BELOW the two RING actions that answer for our own
        # delivery sockets; ABOVE the belt evict, LPECK, RG, SAP, SEATHOLD,
        # REPAIR, SIPHON and the entire role split, which between them ate 892
        # of 925 measured P1-state rounds.  It spends the ACTION and never the
        # turn -- the LPECK/RG construction ten lines below -- so the body
        # still makes its ordinary move, and everything ranked under it
        # re-tests `get_action_cooldown()` itself.
        #
        # The BELT EVICT below is split out of the RING block above ONLY so
        # that this hook can sit between the two; with RANK_ON False the three
        # calls run in the parent's order under the parent's guard and the one
        # extra `get_action_cooldown()` read is pure.
        if RANK_ON and ct.get_action_cooldown() == 0:
            self._rank_cb(ct, rnd)
        if RING_ON and BELT_EVICT_ON and ct.get_action_cooldown() == 0:
            # WAVE 30, ARM 4 BELT EVICT -- ranked directly BELOW the
            # socket evict and above everything else, on RING's own
            # argument one band further out: a socket under one of
            # their buildings is income we have lost, a BELT TILE
            # under one of their buildings is income we have lost
            # too (kladde v126: 0 Ti in 269 of 530 rounds), and the
            # socket is the one with a five-round refill window, so
            # it goes first.  Refuses itself in two integer tests on
            # every body that is not standing at home.
            if self._belt_evict(ct, rnd):
                return
        ...
        # WAVE 31 PLANK B: the same death-watch feeds the hole class, so the
        # call site keeps the incumbent condition as its FIRST disjunct (both
        # flags are True in v164, so the OFF twin's condition is unchanged) and
        # adds `RESTORE_HOLE_ON` as the second.
        ...
        if abs(p.x - tgt.x) + abs(p.y - tgt.y) == 1:
            if ct.get_action_cooldown() == 0:
                if ct.get_global_resources() >= 2 + SAP_TI_FLOOR:
                    try:
                        if ct.can_fire(tgt) and self._f6_ok(ct, tgt):
                            # WAVE 28, F6.
    ...
    def _gd_reaim(self, ct, p, turret_type, rnd):
        ...
        try:
            ...
            return False
        # WAVE 30, ARM 4, THE GUNNER HALF (re-aim).  The incumbent gate
        # is `the lane is plugged by one of OURS`; a barrel pointed at
        # empty ground while a barrier of theirs stands on our belt two
        # tiles away is the same waste for a different reason, so a
        # target this arm can name is a second, equally narrow reason
        # to pay the 10 Ti.  Every budget above still applies
        # (GD_REAIM_MAX, GD_REAIM_MIN_GAP, GD_REAIM_TI_FLOOR) and the
        # rotation is still refused unless a facing is strictly better.
        bkeys = (self._belt_evict_keys(ct, rnd)
                 if BELT_EVICT_ON and BELT_EVICT_GUN_ON else frozenset())
        cur_belt = bool(bkeys) and any(
            (t.x, t.y) in bkeys for t in cur_tiles)
        lane_own = self._gd_ray_owner(ct, p, cur_tiles) == self.GD_OWN
        # BELT MODE is entered ONLY when there is a target of theirs on
        # our belt that this barrel is not already on, and it accepts
        # ONLY a facing that covers one -- so the arm can never pay a
        # rotate to point somewhere else that is also wrong, which is
        # the churn GD_REAIM_MAX exists to bound.
        belt_mode = bool(bkeys) and not cur_belt and not lane_own
        if not lane_own and not belt_mode:
        ...
        for di, d in enumerate(DIRECTIONS):
            ...
            # WAVE 30: a facing that covers a building of theirs on our
            # belt outranks both -- it is the one target we know will
            # still be there next round.
            belt = bool(bkeys) and any(
                (t.x, t.y) in bkeys for t in tiles)
            if cur_belt and not belt:
                continue                 # already on the one that matters
            if belt_mode and not belt:
                continue                 # belt mode buys belt cover only
            key = (0 if belt else 1,
                   0 if who == self.GD_FOE else 1,
    ...
    def _rg_gun(self, ct, rnd):
        ...
        # WAVE 30, ARM 4, THE GUNNER HALF.  One frozenset, memoised
        # per round, empty when the arm is off -- the tiles a barrier
        # of theirs is standing on beside our belt.  A gunner ray
        # clears a 30 HP barrier in 5 shots against a builder's 15
        # pecks and keeps doing it to the re-lay, so a post that
        # covers one is worth more than a post that covers ordinary
        # work tiles.  It re-ranks; it never relaxes a gate.
        bkeys = (self._belt_evict_keys(ct, rnd)
                 if BELT_EVICT_ON and BELT_EVICT_GUN_ON else frozenset())
        ...
        for dx, dy in CARD_DELTAS:
            ...
            for di, d in enumerate(DIRECTIONS):
                ...
                score = 0
                for t in tiles:
                    if bkeys and (t.x, t.y) in bkeys:
                        score += BELT_EVICT_GUN_BONUS   # WAVE 30
    ...
    # ------------------------------------------------------------------
    # WAVE 37 PLANK RANK -- reaching the counter-battery
    # ------------------------------------------------------------------

    def _rank_elect(self, ct, target):
        """RANK_BODIES = 1, without a comm slot.  True = this body is the one.

        A comm claim cannot cap a WITHIN-ROUND race -- store writes are at best
        one round old -- so the cap is an election every body can compute for
        itself: defer to any friendly builder this body can SEE that is also
        inside the home band and is strictly closer to the threat, ties broken
        on entity id so the order is total and every body picks the same
        winner.  A builder's vision is r^2 = 20 and the band is r^2 = 41, so
        two bodies at opposite edges of the band cannot see each other; that
        residue is closed one layer down by `_live_home_gun`, which sees the
        turret the first body has just built standing beside it.
        """
        p = ct.get_position()
        mine = (p.x - target.x) ** 2 + (p.y - target.y) ** 2
        try:
            me = ct.get_id()
        except Exception:
            return True
        for uid in ct.get_nearby_units():
            try:
                ...
                continue
            if dsq_core(up, self.core) > RANK_HOME_DSQ:
                continue
            d = (up.x - target.x) ** 2 + (up.y - target.y) ** 2
            if d < mine or (d == mine and uid < me):
                return False
        return True

    def _rank_cb(self, ct, rnd):
        """THE P1 STATE, AND THE ONE THING THIS ARM DOES ABOUT IT.

        `_cb_over_heal`'s state minus the `role == "defend"` test (one body a
        match satisfies it, and a dead defender is never replaced) and minus
        the bank test (wave 35 convicted the price half; nothing here is
        re-priced) -- plus ONE test the defender's gate does not make: a live
        GUNNER or SENTINEL must be READ standing on the threat tile this round.
        That is the brief's "enemy forward turret" and it is also the ghost
        filter SLOT_THREAT needs.

        Spends the ACTION only.  The caller does not return on True.
        """
        if not RANK_ON or self.core is None:
            return False
        p = ct.get_position()
        # Cheapest test first: most bodies most rounds are not at home at all.
        if dsq_core(p, self.core) > RANK_HOME_DSQ:
            return False
        target, _ = self._cb_target(ct)         # memoised on the round
        if target is None:
            return False
        if RANK_TURRET_ONLY_ON:
            kind = self._enemy_type_at(ct, target)
            if kind is None or kind not in CORE_THREAT_TYPES:
                return False
        # NEVER a body on THEIR ring, and THE PLUG RULE is the one
        # implementation of that (`_f0_plug`, verbatim, wave 27 / wave 28).
        # The home-band test above already excludes it on every map in the
        # pool; this is the guard for a map where the two bands could touch.
        if self._f0_plug(ct, p):
            return False
        # "no home turret answering" -- `_cb_over_heal`'s own predicate.
        if self._home_gun_bears(ct, target):
            return False
        if not self._rank_elect(ct, target):
            return False
        if not self._try_counterbattery(ct):
            return False
        if RANK_LOG:
            print("RANK cb (%d,%d) r=%d seat=%d role=%s"
                  % (target.x, target.y, rnd, self.role_n, self.role))
        return True
    ...
    def _turret(self, ct):
            ...
            try:
                ...
                if tgt is not None and ct.can_fire(tgt):
                    ...
                    if et is not None and self._t5_barrier_ok(ct, tgt, et):
                        ...
                        # WAVE 28, F6.
                        if not self._f6_ok(ct, tgt, body_ok=True):
                            return
                        ct.fire(tgt)
                        self._gd_note_shot()
        ...
        try:
            ...
            if best is not None:
                ...
                # WAVE 28, F6.
                if not self._f6_ok(ct, best, body_ok=True):
                    return
                ct.fire(best)
                if turret_type == EntityType.GUNNER:
                    self._gd_note_shot()
        ...
        try:
            for eid in ct.get_nearby_entities():
                if ct.get_team(eid) != self.team:
                    ...
                    if ct.can_fire(ep):
                        # WAVE 28, F6.
                        if not self._f6_ok(ct, ep, body_ok=True):
                            continue
                        ct.fire(ep)
                        if turret_type == EntityType.GUNNER:
                            self._gd_note_shot()
