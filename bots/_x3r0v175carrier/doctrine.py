...
# ======================================================================
# WAVE 65 -- PLACEMENT.  LAY WHERE THE BELT ACTUALLY ROUTES.
# ======================================================================
# THE RULE (results/wave65/ROUTING_RULE.md 1, 783 socket lays at r >= 40 over
# both w62 legs / 240 replays).  The engine has NO pathfinder.  Every feeder
# carries a `direction` fixed at BUILD time, so the belt is a STATIC DIRECTED
# GRAPH and feeding is graph reachability, not geometry:
#
#     conveyor   accepts from its 3 non-output sides, OUTPUTS to tile+facing
#     splitter   accepts from its BACK only, OUTPUTS to facing + both perps
#     harvester  never receives, OUTPUTS to any of 4 cardinals holding a
#                building, LRU, every 4 rounds
#     core       accepts, never outputs
#
#   P := at the LAY round, a directed edge-path runs from one of OUR live
#        harvesters to the laid tile.
#
#     P true    170 lays, 170 fed (100.0%), first-feed latency min/median/max
#               = 1 / 1 / 1 -- the signature of a mechanism, not a correlation
#     P false   613 lays,  12 fed within 5r (2.0%), 96 ever (15.7%)
#     confusion TP 170 / FP 0 / FN 12 / TN 601; precision 100.0%
#
# THE DEFECT (ROUTING_RULE.md 2).  `ring.py _ring_lay` puts a conveyor on a
# socket with NO routing predicate at all.  `_ring_targets` computes the test
# -- `live = 0 if (i in aimed or i in hfed) else 1` -- and then uses it as a
# SORT KEY, returning every free socket anyway; the one routed socket is
# normally the trunk terminus and already occupied, so the arm falls straight
# through to tier 2, "the sockets nearest THEIR core", which has no routing
# content whatever.  The only source test in the arm is `_ring_eco_ready`,
# i.e. "two harvesters exist SOMEWHERE", which is true equally for all eight
# sockets.  Tagged, not inferred (`bots/leap65_ringlog`): 13 of 13 `RING claim`
# lays were dead at the lay; every source-connected lay at r >= 40 came from
# the trunk planner or from `_l4_repair`, the two sites that DO carry a feeder
# test.  Price: 613/783 dead lays (78.3%), 3.05 dead conveyors and ~+3%
# PERMANENT team cost scale per game, 9.1 Ti/game.
#
# THE FIX SHIPPED HERE IS FIX 1 AND ONLY FIX 1: turn the sort key into a GATE.
# Fix 2 (spend the refused action on the routed frontier) and Fix 3 (re-aim a
# sourced neighbour instead of re-laying) are NOT in this build -- a refused
# claim simply falls through to the parent's ladder, which is a REFUSAL, never
# a new action.  `ct.build_conveyor(` sites in ring.py: 1 -> 1.
#
# WHY `P` AND NOT `aimed`.  `_sg_aimed_at` asks only "is one of our conveyors
# pointing into this tile", an in-edge with no source walk.  Measured at the
# lay: in-edge AND harvester-sourced -> 100.0% fed (n=170); in-edge with a
# SOURCELESS chain -> 37.0% (n=54); no in-edge -> 13.6% (n=559).  The source
# walk is the load-bearing half, so this gate walks it.
W65_PLACE_ON = True          # MASTER.  False == bots/leap35_fix1only (v168)
                             # exactly: `_w65_lay_ok` returns True in its first
                             # test, `_ring_targets` scores every socket 0 on
                             # the new key (so the sort is v168's sort, term
                             # for term), and the three per-unit fields are
                             # written once and never read.
W65_MIN_RND = 40             # THE ANCHOR.  The corpus that derived `P` is
                             # r >= 40 (both w62 legs), and this arm's r6-r20
                             # `RING_FLOOR_OWN` plugs are a DIFFERENT
                             # population that no measurement here covers --
                             # the opening is also where every measured
                             # disaster of this plank happened
                             # (`titanium_collected@r30` of 0).  So pre-r40 is
                             # v168 byte for byte, and the guard "nothing
                             # before r40" is checkable off the markers.
W65_GATE_TARGETS = True      # PART A -- the sort key becomes a gate in
                             # `_ring_targets`: a free socket that fails `P`
                             # is not a target at all, so neither the claim
                             # nor the CLAIM WALK spends a round on it.  ONE
                             # inserted `continue`; with this False the loop
                             # is v168's term for term and PART B still gates
                             # the lay -- that is the ablation.
W65_GATE_LAY = True          # PART B -- belt and braces at the lay itself
                             # (`_ring_claim`'s loop), so the gate still binds
                             # if PART A is ablated off.
W65_GATE_RELAY = False       # PART C -- the RE-LAY half, i.e. `_ring_refill`'s
                             # retake after an evict.  Shipped True because the
                             # registration asks for both fresh lays and
                             # re-lays; SEPARATELY ablatable because it is the
                             # one half with a known counter-argument -- the
                             # peck that opened the socket is already spent, so
                             # a refused retake leaves the socket open, and the
                             # measured window is five rounds to a re-brick.
                             # `_ring_evict` is deliberately NOT gated: this
                             # build changes no firing decision.
W65_SRC_BUDGET = 48          # belt-graph tiles this body will expand per ROUND
                             # across ALL `P` walks (they share one memo and
                             # one budget).  The corpus chain is a few tiles;
                             # 48 is far past the longest observed walk and
                             # still bounds the API traffic at ~4 reads a tile.
                             # Exhausted -> the walk fails CLOSED.
W65_VISION_STRICT = True     # a tile this body cannot SEE is not evidence of a
                             # source, so the walk stops there and fails CLOSED
                             # (`is_in_vision` first, the base's own P0-B order
                             # for every tile predicate).  This can only make
                             # the gate REFUSE more, never lay more, and the
                             # cost of over-refusal is bounded by the same
                             # attribution above: this arm produced 0 of the
                             # 170 sourced lays, so there is no measured lay it
                             # can take away.
W65_LOG = False              # `W65 lay/refuse/gateoff` markers.  Measurement
                             # scaffolding; False in any verdict cell.

# ---------------------------------------------------------------------------
# WAVE 66 -- THE FRESH/RECOVERY SPLIT (`results/wave66/SMOKE_FRESHGATE.md`)
#
# `W65_GATE_RELAY` is flipped to False here and NOWHERE ELSE in this build:
# `bots/leap66_freshgate` is `bots/leap65_placement` with this one name
# rebound.  The wave-65 judge's named fix, verbatim: `P` is a PLACEMENT
# predicate and a perfect one (262/262 fed, fed-share 80.2% vs 34.1%,
# delivered Ti +493 p=.017 on kladde), but it is the WRONG RECOVERY rule --
# a chop makes `P` false, so the gate refused to re-lay exactly when
# re-laying restores the feed (`mimic_jython2`: 0.53 vs 2.27 socket lays/g,
# wired_zero +10.9 pp p=.0015, uptime 39.7% vs 50.6%).  With PART C off the
# retake in `_ring_refill` is v168's, ungated; PARTS A and B still gate
# every FRESH lay.
# ---------------------------------------------------------------------------


# ============================================================================
# WAVE 69, ARM 2 BUILD A -- MVETO_ROY: THE ROYALE OPENING VETO.
# Ticket: results/wave68/DESIGN_MAPARMS.md section 3 ("ARM 2 -- MVETO_ROY_ON").
#
# THESIS.  royale regressed 0.48 -> 0.20 at v162 (p=9.5e-4, same-opponent
# controlled) when the OPEN arm's `need_eff` banding took it (band B, need 3,
# path 17).  The mechanism of the loss is NOT isolated, so this arm invents
# nothing: on royale identity ONLY the opening stands down and every hook falls
# through to the parent (v161) book, exactly as `OPEN_ON = False` does.  On the
# other 14 maps OPEN keeps firing where it gained.  Restore target = royale's
# own measured pre-v162 rate, .48.
#
# THE KEYING RULE (mandatory).  The identity test compares the FULL RESOLVED
# GRID, never the (w, h, anchor) key.  `MAP_CODES` + `EXTRA_MAP_CODES` hold
# colliding keys -- midgard/ragnarok, frostgate/yulerune, eider/heart, the two
# 26x26s -- and a key-level test would veto a map this arm was never priced on.
# royale's key (20, 20, 9, 16, 9, 2) happens to have no collision partner in
# today's frozen pool; the full-grid rule is applied anyway, because the RULE,
# not today's pool, is what keeps the arm honest.
#
# WITH THE FLAG FALSE this file's parent is reproduced byte-for-byte: the
# `op_map_veto` latch can then never be set, so all four guarded sites
# (opening `_op_geom` x2, eco `_seat_ban`, raid `_cg_ferry`) reduce to the
# parent expression.  Proved by ast in the T0 static, not by assertion.
MVETO_ROY_ON = True          # MASTER.  False == bots/leap35_fix1only exactly.

# royale, packed base-3 three cells per character, byte-copied from this
# file's own EXTRA_MAP_CODES entry `((20, 20, 9, 16, 9, 2), ...)` -- the line
# commented `# royale`.  Duplicated as a NAMED constant rather than looked up
# at runtime, because the only available lookup is BY KEY and the key is
# exactly the thing that collides.  `tools/analysis_scratch/w69_mvroy_edit1.py`
# wrote this line by extracting it from that entry, so the two cannot drift.
MVETO_ROY_CODE = "AAAAAAAAAAAAAAIAAASCSCAAAYAAAAAAAAAAAAAAAAAAAAAAAAAAAMEMNNJNBBBADDMJJAABBNEMNNJNAAAAAAAAAAAAAAAAAAAAAAAAAAAYAAAAIAIAAASCAAAAAAAAAAAAAA"
MVETO_ROY_W = 20             # royale's dimensions, for the module-level
MVETO_ROY_H = 20             # `_decode_grid` in opening.py.
