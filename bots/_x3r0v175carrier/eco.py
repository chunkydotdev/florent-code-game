...
class EcoMixin:
    ...
    # --- WAVE 32 FIX 2: the exit guard (ported from bots/leap27_exitguard) --

    def _exit_free(self, ct, p):
        """The body's free orthogonal exits at `p`, as a set of (x, y) keys.

        Computed at most ONCE per (round, tile): several guarded call sites
        rank many candidate tiles inside one loop, and the answer depends only
        on where the BODY stands, never on which candidate is being weighed.
        The cache key carries the position, so a body that moves and then
        builds in the same round recomputes.

        Passability is engine_mechanics G, read exactly as `_cg_outer_station`
        (`raid.py`) reads it: conveyors and splitters of EITHER team are
        PASSABLE and therefore still exits; walls, barriers, harvesters,
        turrets, core footprints and any tile holding a builder body are not.
        An unreadable neighbour counts as FREE -- see the fail-open note in
        doctrine.py "WAVE 32 -- FIXIT", FIX 2.
        """
        try:
        ...
        for dx, dy in CARD_DELTAS:
            ...
            try:
                ...
                pass
            free.add((qx, qy))
        self.eg_cache = (key, free)
        return free

    def _exit_ok(self, ct, p, t):
        """WAVE 32 FIX 2 EXIT GUARD.  May a body at `p` make `t` impassable?

        True iff, with `t` spent, the body at `p` still has at least
        EXIT_GUARD_MIN free orthogonal exits.  See doctrine.py "WAVE 32 --
        FIXIT" FIX 2 for why, and for why this is a REFUSAL and not a new
        behaviour.  `t` is a Position; `p` is the builder's OWN position.
        """
        if not EXIT_GUARD_ON:
            return True
        free = self._exit_free(ct, p)
        n = len(free) - (1 if (t.x, t.y) in free else 0)
        return n >= EXIT_GUARD_MIN

    def _exit_refuse(self, ct, p, t, site):
        """The one line every guarded call site runs.  True == refuse the build.

        Exists so the guard reads identically at all sites and so the log
        marker cannot drift between them.
        """
        if not EXIT_GUARD_ON:
            return False
        # WAVE 27 FIX 1, carried over.  The home / economy / opening sites
        # stand down: their refusals have no alternative tile and cost titanium
        # conversion, and the idle population is the raider at the ENEMY Core.
        # On this carrier those sites are not instrumented at all, so this
        # clause is belt and braces rather than the live narrowing it was.
        if EXIT_GUARD_RAID_ONLY and site in EXIT_GUARD_HOME_SITES:
            return False
        if self._exit_ok(ct, p, t):
            return False
        self.exit_guard_n = getattr(self, "exit_guard_n", 0) + 1
        if EXIT_GUARD_LOG:
            try:
                print("EXITGUARD refuse r=%d site=%s p=%d,%d t=%d,%d n=%d"
                      % (ct.get_current_round(), site, p.x, p.y, t.x, t.y,
                         self.exit_guard_n))
            except Exception:
                pass
        return True

    # --- WAVE 28 CLEAN: shared predicates ----------------------------------
    # See doctrine.py "WAVE 28 -- CLEAN".  Two helpers used by more than one
    # fix, kept here (rather than beside their callers) so THE PLUG RULE has
    # exactly one implementation and the friendly-fire refusal reads
    # identically at all fourteen `ct.fire` sites.

    def _f0_plug(self, ct, p=None):
        """THE PLUG RULE.  True == this body is parked on THEIR ring8/ring12.

        `dsq_core(p, self.enemy) <= 2` is exactly their twelve collar tiles:
        the eight heal seats are at squared distance 1 from the 2x2 footprint
        and the four corners at 2 (`heal_seats` / `core_corners`, eco.py).  A
        body standing on one of those tiles is counted HELD by `_cg_seal`, so
        it is denying a heal seat and a spawn tile just by being there --
        wave 27 measured moving it at -6 wins / +93 kill rounds / +23 pp of
        our own core dying (`results/wave27/VERDICT_27.md` section 2).

        No wave-28 fix may give such a body a job, a target or a move.  Fails
        CLOSED (True = "treat it as a plug, leave it alone") on anything
        unreadable and whenever their Core anchor is not known yet, because
        "we do not know where their core is" is not a licence to retask.
        """
        E = self.enemy
        if E is None:
            return False              # nothing known: not at their core
        try:
            if p is None:
                p = ct.get_position()
            return dsq_core(p, E) <= 2
        except Exception:
            return True

    def _f6_ok(self, ct, t, body_ok=False):
        """F6.  May we fire at `t`?  False == it is OUR OWN building.

        REFUSES ONLY ON POSITIVE EVIDENCE (doctrine.py F6): an unreadable
        tile and a tile with no building id at all (their Core footprint reads
        this way from some tiles, and CAGE/FIN must keep the core peck) both
        pass.  Only "positively our building" is refused.

        `body_ok` -- WAVE 28 FIX A, and it is a TURRET-ONLY carve-out.  A
        turret shooting an enemy BUILDER that happens to be standing on one of
        our own conveyors is taking a legitimate shot, so the three `_turret`
        fire sites pass `body_ok=True`.  A BUILDER peck may not: a builder
        attack targets BUILDINGS ONLY (engine_mechanics, and the instrument
        `w27_misbehave` scores it that way), so the damage from a builder
        firing at "the enemy body on our conveyor" lands on OUR CONVEYOR.  The
        first wave-28 smoke measured exactly that -- 8 friendly pecks in 30
        games with the carve-out unconditional, two of them two bodies firing
        at the SAME tile of ours one round apart (`nordkap_seed1101_AB` r43 and
        r46 at (12,7)).  Default False: the carve-out is opt-in, per site.
        """
        if not F6_TEAM_TEST_ON:
            return True
        try:
            ...
            return True               # fail OPEN, like every guard here
        self.f6_refuse = getattr(self, "f6_refuse", 0) + 1
        if F6_LOG:
            try:
                print("F6 refuse r=%d t=%d,%d n=%d"
                      % (ct.get_current_round(), t.x, t.y, self.f6_refuse))
            except Exception:
                pass
        return False
    ...
    def _seat_ban(self):
        ...
        # WAVE 69, MVETO_ROY (edit 4 of the ticket).  Under a vetoed geometry
        # `self.op_geom` is permanently None, so without this term the block
        # below takes its "geometry unresolved" TRANSIENT path -- recomputing
        # `delivery_seats` on every single call, and returning a seat ban the
        # parent never returns.  The parent's own `HS_SEAT_PROTECT_ON` block is
        # what a stood-down opening is supposed to fall through to.
        if (OPEN_ON and not self.op_map_veto
                and self.core is not None and self.mw and self.mh):
        ...
        try:
            for bid in ct.get_nearby_buildings():
                if ct.get_team(bid) != self.team:
                    continue
                et = ct.get_entity_type(bid)
                if et not in BELT_TYPES:
                    continue
                bp = ct.get_position(bid)
                cur.add((bp.x, bp.y))
                # WAVE 31 PLANK B.  Remember the FACING while the belt is
                # ALIVE: a relay laid into a remembered hole has to point back
                # DOWN the chain, and once the tile is empty the direction is
                # unrecoverable from the engine.  CONVEYOR only -- a splitter's
                # output rotates among three directions and is not a facing
                # this plank could restore, so a splitter tile never becomes a
                # candidate (`_restore_hole_ok` requires a remembered facing).
                if RESTORE_HOLE_ON and et == EntityType.CONVEYOR:
                    k = (bp.x, bp.y)
                    if k in self.rest_face or len(self.rest_face) < RESTORE_FACE_MAX:
                        try:
                            self.rest_face[k] = ct.get_direction(bid)
                        except Exception:
                            pass
        except Exception:
            return
    ...
    def _rep_tick(self, ct):
        ...
        # inside `_expand`.
        # WAVE 31 PLANK B -- THE SEVERED-TRUNK OVERRIDE.  Computed ONCE per
        # turn and passed down: `_restore_override` returns False on an empty
        # `self.rep_lost` before reading anything, so on every round where this
        # body has watched nothing die it is one dict truth test.
        ovr = self._restore_override(ct, rnd)
        may_walk = True
        if REPAIR_CHAIN_GUARD and self.link_queue and not ovr:
            may_walk = False
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if (REPAIR_REBUILD_ON and REPAIR_DEFENDER_REBUILD
                ...
                return True
            # WAVE 31 PLANK B -- the RELAY, ranked BELOW both incumbent rules.
            # A remembered hole this body is already standing beside is filled
            # here; the walk below is only for one it has to reach.  Below the
            # adjacent heal because a heal is 1 Ti and keeps a building that
            # still stands, and below `_l4_repair` because the inherited
            # feeder/acceptor rule is the proven one and must never be
            # pre-empted by the wider class.
            if RESTORE_HOLE_ON and self._restore_lay(ct, rnd):
                return True
        if not may_walk or REPAIR_DETOUR <= 0:
            return False
        # The role gate.  `role != "expand"` stands unless the trunk is SEVERED
        # -- with no socket of ours feeding the Core the defender and a
        # stood-down home-band raider are the only bodies standing where the
        # hole is (Pivot g4: five builders alive, `ownsock` 0 from r60 to r184).
        if self.role != "expand" and not ovr:
            return False
        if ct.get_move_cooldown() != 0:
            return False
        # WHAT THE OVERRIDE MAY AND MAY NOT WIDEN.  A body reaching this line
        # ONLY because the trunk is severed -- a non-expander, or an expander
        # still carrying a chain -- gets PLANK B's class and NOTHING ELSE.  The
        # incumbent DAMAGED-building detour keeps exactly the population it had
        # in v164: expanders, chain-free.  Without this split the override would
        # quietly hand `_rep_detour_target`'s old class two new caller roles,
        # which is not the plank that was registered.  `ovr_used` is False on
        # every path v164 could reach, so the OFF twin always takes the
        # incumbent branch.
        ovr_used = ovr and (self.role != "expand"
                            or (REPAIR_CHAIN_GUARD and bool(self.link_queue)))
        if ovr_used:
            tgt = self._restore_detour(ct, rnd)
        else:
    ...
    def _rep_detour_target(self, ct, rnd):
        ...
        WAVE 31 PLANK B adds a SECOND CANDIDATE CLASS, ranked FIRST: a tile this
        body watched one of our own belts DIE on.  A destroyed conveyor is not a
        damaged building -- it is an empty tile with no id -- so the incumbent
        class below cannot see it at all, and `w31_hole_probe` measures the cost
        at a body within 4 steps of the one-wide hole on 74 of 76 unwired rounds
        (Pivot g5).  It is ranked above the damaged class because a hole
        delivers ZERO through the whole line (the engine zeroes a line on one
        broken tile) while a damaged conveyor still delivers.
        """
        if RESTORE_HOLE_ON:
            seat = self._restore_detour(ct, rnd)
            if seat is not None:
                return seat
        p = ct.get_position()
        cur = self.rep_tgt
    ...
    # ------------------------------------------------------------------
    # WAVE 31, PLANK B -- RESTORE_HOLE.  See doctrine.py "WAVE 31 -- PLANK
    # RESTORE".  Every method here returns its typed empty value on the head
    # guard when `RESTORE_HOLE_ON` is False, so the OFF twin executes none of
    # it and the `_rep_detour_target` / `_rep_tick` call sites are each behind
    # an `if RESTORE_HOLE_ON` clause as well.
    # ------------------------------------------------------------------

    def _restore_body_ok(self, ct, p):
        """May THIS body spend a round on a remembered hole?

        THREE STANDING BOUNDS, and they are `_belt_home_ok`'s, deliberately:
        never a raider by role, NEVER a body on THEIR ring (`_f0_plug`
        verbatim -- wave 27 measured moving one at -6 wins), and a body already
        inside our own home band.  A body outside the band is walking, not
        repairing, and `RESTORE_HOLE_STEPS` alone would not stop it starting
        the walk from the far side of the map.
        """
        if self.core is None:
            return False
        if self.role == "raid":
        ...
        if self._f0_plug(ct, p):
            return False
        return dsq_core(p, self.core) <= RESTORE_HOLE_HOME_DSQ

    def _restore_severed(self, ct):
        """Is our delivery trunk SEVERED?  Memoised per unit per round.

        The predicate `w31_chain_probe` scores off a replay -- "is any harvester
        wired to the core footprint?" -- read at the only place a body can see
        it cheaply and without a graph walk: OUR EIGHT SOCKETS.  Delivery enters
        the Core through a socket carrying OUR conveyor whose output tile is a
        Core tile (`_sg_socket_scan`'s `feed`).  No such socket => no harvester
        anywhere can be wired, whatever the rest of the graph looks like.  It is
        therefore SOUND in the direction that matters and conservative in the
        other: a trunk cut half way out still reads "not severed" here, and the
        override stays off.

        AND IT IS EVIDENCE, NOT AN ABSENCE OF ONE.  Every socket tile must be in
        THIS body's vision (builder vision is r^2 = 20, so a body at d 8 sees
        some of the ring and not the rest).  One socket out of vision => False.
        That is the valkyrie FIX-3 rule applied to a predicate instead of a
        tile, and without it a body walking home would read an unseen ring as an
        unfed one and license the override on nothing.
        """
        if not (REPAIR_ON and RESTORE_HOLE_ON) or self.core is None:
            return False
        rnd = ct.get_current_round()
        if self.rest_sev_rnd == rnd:
            return self.rest_sev
        self.rest_sev_rnd = rnd
        self.rest_sev = False
        core = self.core
        for i in range(8):
            dx, dy = SG_SOCKET_DELTAS[i]
            tx, ty = core.x + dx, core.y + dy
            if not (0 <= tx < self.mw and 0 <= ty < self.mh):
                continue                      # off the map: not a door at all
            try:
                if not ct.is_in_vision(Position(tx, ty)):
                    return False              # unknown ring: no licence
            except Exception:
                return False
        try:
            feed = self._sg_socket_scan(ct)[0]
        except Exception:
            return False
        self.rest_sev = not feed
        return self.rest_sev

    def _restore_hole_ok(self, ct, key, rnd):
        """Is tile `key` a candidate of PLANK B's class RIGHT NOW?

        The full definition, in the order that costs least: watched to die by
        THIS body, a facing remembered from when our conveyor stood there, not
        under the detour's ban, in vision, empty, pave-legal, our own half.
        """
        if key not in self.rep_lost:
            return False
        if self.rest_face.get(key) is None:
            return False
        until = self.rep_ban.get(key)
        if until is not None and rnd < until:
            return False
        t = Position(key[0], key[1])
        if (
            LOKI_L4_OWN_HALF_ONLY and self.enemy is not None and self.core is not None
            and t.distance_squared(self.core) > t.distance_squared(self.enemy)
        ):
            return False
        try:
            # The valkyrie FIX-3 lesson, and it is FIRST: never queue a tile you
            # cannot see.  `is_tile_empty` is "no building and not a wall",
            # which is the hole condition; a body standing there is caught by
            # `can_build_conveyor` at the lay.
            if not ct.is_in_vision(t):
                return False
            ...
            return False
        return not pave_blocked(ct, t, self._pave_ban())

    def _restore_holes(self, ct, rnd):
        """This body's candidate holes, NEAREST FIRST.  Memoised per round.

        Bounded by construction: `self.rep_lost` is capped at
        `REPAIR_LOST_MAX` (32) and the Manhattan filter runs BEFORE any engine
        call, so the vision/emptiness reads only ever happen for tiles already
        inside `RESTORE_HOLE_STEPS`.
        """
        if self.rest_rnd == rnd:
            return self.rest_holes
        self.rest_rnd = rnd
        self.rest_holes = ()
        if not (REPAIR_ON and RESTORE_HOLE_ON):
            return ()
        if not self.rep_lost:
            return ()
        if rnd < REPAIR_MIN_RND:
            return ()
        if self.rest_n >= RESTORE_HOLE_MAX:
            return ()
        try:
            p = ct.get_position()
        except Exception:
            return ()
        if not self._restore_body_ok(ct, p):
            return ()
        out = []
        for key in self.rep_lost:
            d = abs(key[0] - p.x) + abs(key[1] - p.y)
            if d > RESTORE_HOLE_STEPS:
                continue
            if not self._restore_hole_ok(ct, key, rnd):
                continue
            out.append((d, key[1], key[0]))
        out.sort()
        self.rest_holes = tuple((x, y) for _d, y, x in out)
        return self.rest_holes

    def _restore_override(self, ct, rnd):
        """May the SEVERED-TRUNK override lift `REPAIR_CHAIN_GUARD` and the
        `role != "expand"` gate for this body, this round?

        Ordered so the common case is one dict truth test: no watched death, no
        override, nothing read.  Only then the candidate scan, and only then the
        severance predicate -- which is the expensive one (eight vision reads
        plus the socket census) and is therefore last.
        """
        if not (REPAIR_ON and RESTORE_HOLE_ON):
            return False
        if not self.rep_lost:
            return False
        if not self._restore_holes(ct, rnd):
            return False
        return self._restore_severed(ct)

    def _restore_lay(self, ct, rnd):
        """Relay OUR conveyor into an ADJACENT remembered hole.  True = action.

        The facing is the REMEMBERED one -- what our own conveyor pointed at
        while it stood there -- so the relay rejoins the chain in the direction
        the chain already ran.  `can_build_conveyor` is the engine's own last
        word on legality (a body on the tile, a building laid this round by a
        teammate, terrain), and a refusal simply moves on to the next candidate.
        """
        if not (REPAIR_ON and RESTORE_HOLE_ON):
            return False
        if self.rest_n >= RESTORE_HOLE_MAX:
            return False
        holes = self._restore_holes(ct, rnd)
        if not holes:
            return False
        try:
            p = ct.get_position()
        ...
        if not self._eco_spendable(ct, cost):
            return False
        for (hx, hy) in holes:
            if abs(hx - p.x) + abs(hy - p.y) != 1:
                continue
            f = self.rest_face.get((hx, hy))
            if f is None:
                continue
            t = Position(hx, hy)
            try:
                ...
                continue
            self.rest_n += 1
            # The tile is no longer a hole, so it leaves the death-watch memory
            # exactly as `_rep_gap2` drops the tiles it relays -- otherwise a
            # conveyor shot out twice would still be counted once.
            self.rep_lost.pop((hx, hy), None)
            self.rest_holes = ()
            if self.rest_tgt == (hx, hy):
                self.rest_tgt = None
            if RESTORE_LOG:
                print("REP relay (%d,%d) f=%s r=%d n=%d"
                      % (hx, hy, getattr(f, "name", f), rnd, self.rest_n))
            self._rep_mark(ct, "relay", t, dedupe=False)
            return True
        return False

    def _restore_detour(self, ct, rnd):
        """A standable tile beside the nearest remembered hole, or None.

        `_rep_detour_target`'s shape, its time-box and ITS ban dictionary -- one
        `self.rep_ban`, so a body that writes a tile off writes it off for both
        classes and the two can never trade a body back and forth.
        """
        if not (REPAIR_ON and RESTORE_HOLE_ON):
            return None
        cur = self.rest_tgt
        if cur is not None:
            # THE WAVE 31 SMOKE'S OWN CORRECTION.  The first build reused
            # `REPAIR_WALK_RNDS` (6), which was sized to `REPAIR_DETOUR` (4).
            # This class reaches to `RESTORE_HOLE_STEPS` (6) and `_rep_tick` is
            # not reached on every round of a body's life, so a hole at the edge
            # of the reach could not be arrived at inside the window -- the
            # commitment expired and BANNED the hole for `REPAIR_WALK_BAN`
            # rounds, i.e. the arm was writing off the very tiles it exists to
            # fill.  Measured in the 6-game _dbg run: 30 commitments, 4 relays.
            # `RESTORE_HOLE_WALK_RNDS` is the same time-box, sized to this
            # class's own reach; the ban itself (loki_cage's guard) is kept.
            if rnd - self.rest_since > RESTORE_HOLE_WALK_RNDS:
                if len(self.rep_ban) > 16:
                    self.rep_ban = {k: v for k, v in self.rep_ban.items() if v > rnd}
                self.rep_ban[cur] = rnd + REPAIR_WALK_BAN
                self.rest_tgt = None
                cur = None
            elif not self._restore_hole_ok(ct, cur, rnd):
                # Refilled, walled, out of vision or banned.  All of them mean
                # "stop", and none of them is worth remembering.
                self.rest_tgt = None
                cur = None
        try:
        ...
        if cur is None:
            ...
            if not holes:
                return None
            cur = holes[0]
            if abs(cur[0] - p.x) + abs(cur[1] - p.y) <= 1:
                return None               # adjacent already: lay, do not walk
            self.rest_tgt = cur
            self.rest_since = rnd
            self._rep_mark(ct, "hole", Position(cur[0], cur[1]))
        seat, seat_key = None, None
        grid = self.map_grid
        for dx, dy in CARD_DELTAS:
            qx, qy = cur[0] + dx, cur[1] + dy
            if not (0 <= qx < self.mw and 0 <= qy < self.mh):
                continue
            if grid is not None and grid[qy][qx] == "#":
                continue
    ...
    def _expand(self, ct):
        ...
        if RING_ON:
            ...
            if self._ring_evict_walk(ct, ring_rnd):
                return
            # WAVE 30, ARM 4 BELT EVICT, the move half.  Between the
            # two incumbent walks and on the SAME lifetime budget
            # (`ring_walk_total` / `RING_WALK_CAP`), never a second
            # one: the socket brick outranks it and the free socket
            # does not, because a tile of theirs on our belt is income
            # already lost and a free socket is only income at risk.
            if BELT_EVICT_ON and self._belt_evict_walk(ct, ring_rnd):
                return
