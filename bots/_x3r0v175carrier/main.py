...
class Player(EcoMixin, RaidMixin, OpenMixin, RingMixin, SipMixin):
    ...
    def __init__(self):
        ...
        # WAVE 65, PLACEMENT (ring.py `_w65_sourced`, doctrine's W65 block).
        # Per unit, per round, bounded: ONE memo of the routing predicate
        # keyed on tile and ONE expansion budget shared by every walk in the
        # round.  No shared state, no comm slot, no new class of API call.
        # With `W65_PLACE_ON` False these three are written once and read
        # never, which is the inertness proof for the OFF twin.
        self.w65_src_rnd = -1           # round the memo below belongs to...
        self.w65_src = {}               # ...(x, y) -> P, that round only
        self.w65_budget = 0             # belt-graph tiles left to expand
        ...
        # WAVE 69, ARM 2 build A -- MVETO_ROY.  Per-unit latch, set exactly
        # once by `opening._op_geom` when the FULL resolved grid is royale's,
        # and read by the three sites that would otherwise be handed the
        # opening's artifacts on a map the opening is standing down on.  With
        # `MVETO_ROY_ON` False it can never leave this value.
        self.op_map_veto = False        # royale identity latched (MVETO_ROY)
    ...
    def _builder(self, ct):
        ...
        if RING_ON and ct.get_action_cooldown() == 0:
            ...
            if BELT_EVICT_ON and self._belt_evict(ct, rnd):
                return
