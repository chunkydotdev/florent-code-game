...
class RaidMixin:
            ...
            for dx, dy in CARD_DELTAS:
                ...
                try:
                    ...
                    if ct.can_build_barrier(t):
                        # WAVE 32 FIX 2 EXIT GUARD, site 4.  Alternate author
                        # of the same tile as site 1.
                        if self._exit_refuse(ct, p, t, "seat_seal"):
                            continue
                        ct.build_barrier(t)
                        return True
                except Exception:
            ...
            for dx, dy in CARD_DELTAS:
                ...
                try:
                    if not ct.can_build_barrier(t):
                        continue
                    # WAVE 32 FIX 2 EXIT GUARD, site 1.  THE FATAL BUILD of
                    # match 69827b4b game 2 (r42, (26,17), the tile the raider
                    # had walked in FROM) and of Pivot g4 auroraveil (r173,
                    # (12,2), unit 21, 248 rounds of dead idle after it).
                    # Every candidate in this arm is a CARD_DELTAS offset of
                    # the body's own tile, so every one of them is by
                    # construction one of its four exits.
                    if self._exit_refuse(ct, p, t, "salt_corpse"):
                        continue
                    ct.build_barrier(t)
                except Exception:
                    continue
        ...
        if (LOKI_SALT_BLOCK_ON and can_pay_barrier
            ...
            for dx, dy in CARD_DELTAS:
                ...
                if not self._salt_beside_belt(ct, t):
                    continue
                # WAVE 32 FIX 2 EXIT GUARD, site 2.  Same cardinal scan off the
                # body's own tile as arm (1), same exposure.
                if self._exit_refuse(ct, p, t, "salt_deny"):
                    continue
                try:
                    ct.build_barrier(t)
                except Exception:
    ...
    def _t5_try_fwd_gunner(self, ct, E):
        ...
        for dx, dy in CARD_DELTAS:
            ...
            try:
                ...
                continue
            # WAVE 32 FIX 2 EXIT GUARD, site 5.  A gunner is impassable.
            if self._exit_refuse(ct, p, bp, "fwd_gunner"):
                continue
    ...
    def _try_forward_sentinel(self, ct, E):
        ...
        # WAVE 32, FIX 1 -- TUBE_REPLACE.  Resolved ONCE, here, because both
        # gates it relaxes (the harvester floor immediately below and the bank
        # floor further down) are asked of the same round and the same body.
        # `repl` is True only when the LIVE census could see and counted zero
        # AND the monotone store says a tube was once built -- i.e. only for the
        # replacement of a dead LAST tube.  See doctrine.py "WAVE 32 -- FIXIT",
        # FIX 1.  With TUBE_REPLACE_ON False this is a constant False and every
        # line below is the parent's.
        repl = self._tube_replace_ok(ct, E, live)
        if repl and TUBE_REPLACE_HARV_ON and TUBE_REPLACE_MIN_HARV < min_harv:
            # A REPLACEMENT IS NOT AN OPENING.  LOKI_FWD_MIN_HARV exists to stop
            # the siege being opened before the economy exists; a tube already
            # stood here, so that economy already existed and this floor is
            # being re-asked about a question it has answered.  Only ever
            # LOWERED, and only in the replacement state.
            min_harv = TUBE_REPLACE_MIN_HARV
        ...
        if T5_BATTERY_GATE_ON:
            ...
            # WAVE 32, FIX 1.  THE MISSING ARM OF THE SAME DISCOUNT.  The clause
            # above is `n >= 1`, so it cheapens tube 2 and tube 3 and can never
            # cheapen the REPLACEMENT of a dead last tube -- which is the state
            # of all 3 247 zero-tube rounds D3 measured, and the state in which
            # the bank test alone refused 45% of them.  Same floor, same
            # one-directional discount, same "never a veto": `ti_floor` only
            # ever moves DOWN, so this arm cannot refuse a tube the parent
            # bought.  `elif`, because `repl` requires the live census to read
            # zero and the branch above requires n >= 1 -- they are disjoint by
            # construction and the elif says so.
            elif repl and TUBE_REPLACE_TI_FLOOR < ti_floor:
                # 2 == "the replacement arm was ARMED for this tube"
                # 3 == "...and it was BINDING: the parent's floor refused it".
                # Both are logged; only 3 is a tube the parent would not have
                # bought, which is the number that prices the arm.
                disc = 3 if ti < cost + ti_floor else 2
                ti_floor = TUBE_REPLACE_TI_FLOOR
            if ti < cost + ti_floor:
                return False
        p = ct.get_position()
            ...
            if pick is not None:
                ...
                if abs(s.x - p.x) + abs(s.y - p.y) == 1:
                    try:
                        ...
                        ok = False
                    # WAVE 32 FIX 2 EXIT GUARD, site 6.  A sentinel is
                    # impassable.
                    if ok and self._exit_refuse(ct, p, s, "fwd_nest"):
                        ok = False
                    if ok:
        ...
        for dx, dy in CARD_DELTAS:
            ...
            if not (0 <= bx < self.mw and 0 <= by < self.mh):
                continue
            bp = Position(bx, by)
            # WAVE 32 FIX 2 EXIT GUARD, site 7.  Tested once per POST, which
            # covers all three sentinel builds fed by this loop (the
            # SIEGE_SITE_ON False first-come build, the ranked `best`, and the
            # `par` parent fallback): a refused post is never ranked and never
            # becomes `par`, so no build line below can reach it.
            if self._exit_refuse(ct, p, bp, "fwd_post"):
                continue
    ...
    def _sge_note_build(self, ct, bp, E, n, disc, fallback=False):
        ...
        """
        # WAVE 32 FIX 1's own event, ABOVE the SIEGE_LOG guard so the two
        # instruments stay independent flags.  `n <= 0` and `disc == 2` is
        # exactly "the live census read zero, a tube had existed, and the
        # replacement floor is what bought this one" -- the marker the smoke
        # test counts as "a dead tube was replaced".
        if TUBE_REPLACE_LOG and n <= 0 and disc in (2, 3):
            try:
                print("TUBEREP tube (%d,%d) r=%d bind=%d"
                      % (bp.x, bp.y, ct.get_current_round(), 1 if disc == 3 else 0))
            except Exception:
                pass
        ...
        for dx, dy in CARD_DELTAS:
            ...
            for d in dirs:
                ...
                try:
                    ...
                    continue
                # WAVE 32 FIX 2 EXIT GUARD, site 8.
                if self._exit_refuse(ct, p, bp, "sge_screen"):
                    continue
    ...
    def _tube_replace_ok(self, ct, E, live):
        """WAVE 32 FIX 1.  Is this the REPLACEMENT of a dead LAST forward tube?

        TWO SOURCES, BOTH REQUIRED, because neither can answer the question
        alone:

          * `live` -- the raw return of `_live_fwd_guns`, the only thing in the
            tree that can say NO TUBE STANDS.  It must be a literal integer
            zero.  The census returns None rather than zero when the body
            cannot see the siege band (`dsq_core > LOKI2B_CENSUS_DSQ * 2`, or
            any exception), and None is refused here -- so a raider across the
            map can never read its own blindness as "the tube is dead, buy
            another".  THIS IS THE FAIL-CLOSED DIRECTION and it is the whole
            reason the argument is the raw census and not the caller's `n`,
            which has already substituted the monotone store for a blind read.
          * `SLOT_FWD_GUN` -- monotone, incremented once per forward sentinel
            ever built and never decremented (doctrine.py; main.py "both gun
            slots are monotone").  Being monotone is exactly what makes it the
            right EVER-EXISTED test and the wrong ALIVE-NOW test, and it is
            used here only for the former.

        The other two conditions of the replacement state are already
        established by the caller and are not re-tested: THEIR CORE IS STANDING
        (a destroyed Core ends the game, so every round this runs in is one
        where it stands) and OUR RAIDER IS IN THE BAND (`_try_forward_sentinel`
        returns on `dsq_core(p, E) > 50` and the census itself refuses past
        100).

        Returns False, never raises: a store that cannot be read is not a
        licence to spend.
        """
        if not TUBE_REPLACE_ON:
            return False
        if not LOKI2B_LIVE_CAP_ON:
            # Without the live census there is no "no tube stands" reading to
            # be had, and the monotone store on its own would call every round
            # after the first tube a replacement.  Stand down.
            return False
        if live != 0:
            # None (blind) or >= 1 (one is standing).  Neither is a replacement.
            return False
        try:
            ever = ct.read_store(SLOT_FWD_GUN)
        except Exception:
            return False
        return ever >= 1
    ...
    def _t5_nest_walk_target(self, ct, E):
        ...
        if T5_BATTERY_GATE_ON:
            ...
            # WAVE 32, FIX 1.  THE WALK CARRIES THE SAME DISCOUNT AS THE BUILD.
            # This gate exists so a raider does not leave the collar for a nest
            # the build gate will refuse -- so if the build gate is repaired and
            # this one is not, the raider is released to build a replacement and
            # then refused the walk that puts it beside the nest, and the repair
            # buys nothing.  Same flag, same floor, same one-directional move.
            if TUBE_REPLACE_ON and TUBE_REPLACE_TI_FLOOR < ti_floor:
                if self._tube_replace_ok(ct, E, live):
                    ti_floor = TUBE_REPLACE_TI_FLOOR
            if ti < cost + ti_floor:
                return None
        pick = self._t5_nest_pick(ct, E, n)
    ...
    def _stick_ok(self, ct, E):
        """WAVE 32 FIX 3.  May the collar prefer a LOST socket this round?

        The one window STICK stands down in is the terminal one: below
        STICK_MIN_CORE_HP their Core is about to fall and 3-5 titanium belongs
        in damage, not in a seat whose denial has no time left to pay for
        itself.

        `_tw_core_hp` returns None when no footprint tile is in vision, and
        UNKNOWN DOES NOT REFUSE -- a collar body is orthogonally adjacent to
        their ring and sees a footprint tile essentially always, so the None
        branch is the far-away caller, for whom the parent's behaviour is the
        right one and the fix is simply not engaged.
        """
        if not STICK_ON:
            return False
        try:
            ...
            hp = None
        if hp is not None and hp < STICK_MIN_CORE_HP:
            return False
        return True
    ...
    def _collar_act(self, ct, E, p, ti, rnd):
        ...
        if rat and free and self.rat_watch:
            ...
            if not ratfree:
                ratkeys = None
        # WAVE 32, FIX 3 -- STICK.  A SOCKET WE OWNED AND LOST.
        #
        # `col_bricks` is the tree's existing memory of "this seat carried one
        # of ours": written on our own build below, and written above on SIGHT
        # of our barrier on an adjacent seat.  A key that is in `col_bricks`
        # and ALSO in `free` (no building, no body -- `free` is built from
        # `get_tile_building_id is None` and `get_tile_builder_bot_id is None`)
        # is therefore a socket we held and they took off us.  D2 measured 95
        # of those a corpus, 42 of them never re-laid, because `order = free`
        # promotes only RATCHET tiles and `reseal` is computed after the build
        # for a log print.  This ranks them, and -- up to STICK_MAX_RELAYS per
        # body -- lets them waive the collar's budget exactly as
        # RAT_BRICK_WAIVE lets a ratchet tile waive it.
        #
        # NO MOTION, SO NO PLUG-RULE EXPOSURE: every tile in `free` is already
        # a CARD_DELTAS neighbour of this body's own position.
        #
        # A ratchet tile keeps precedence and is never counted twice: the
        # ratchet is an eviction that has just landed and is the more perishable
        # claim of the two.
        stickfree, stickkeys, stickwaive = (), None, False
        if STICK_ON and free and self.col_bricks and self._stick_ok(ct, E):
            picked = [t for t in free if (t.x, t.y) in self.col_bricks]
            if ratkeys is not None:
                picked = [t for t in picked if (t.x, t.y) not in ratkeys]
            if picked:
                stickfree = picked
                stickkeys = set((t.x, t.y) for t in picked)
                # PAST THE CAP THE ORDERING SURVIVES AND ONLY THE WAIVER STOPS.
                # Ranking costs nothing; waiving the budget costs titanium.
                stickwaive = self.stick_n < STICK_MAX_RELAYS
        if (free and ti >= cost0 + LOKI_SEAL_TI_FLOOR
                and (self._collar_afford(ct, E, cost0)
                     or (RAT_BRICK_WAIVE and ratfree)
                     or (stickfree and stickwaive))):
            cost = cost0
            order = free
            if ratfree or stickfree:
                skip = set()
                if ratkeys:
                    skip |= ratkeys
                if stickkeys:
                    skip |= stickkeys
                rest = [t for t in free if (t.x, t.y) not in skip]
                order = list(ratfree) + list(stickfree) + rest
            for t in order:
                try:
                    if not ct.can_build_barrier(t):
                        continue
                    # WAVE 32 FIX 2 EXIT GUARD, site 3.  The ring12 seal.  A
                    # refused seat falls through to the NEXT candidate, so the
                    # collar keeps bricking -- and so FIX 3's ordering is
                    # preserved -- it just does not brick the last door.
                    if self._exit_refuse(ct, p, t, "collar_brick"):
                        continue
                    ct.build_barrier(t)
                except Exception:
                    continue
                ...
                if stickkeys is not None and key in stickkeys:
                    self.stick_n += 1
                    if STICK_LOG:
                        print("STICK relay (%d,%d) n=%d"
                              % (t.x, t.y, self.stick_n))
        ...
        for t in cands:
            try:
                if not ct.can_build_launcher(t):
                    continue
                # WAVE 32 FIX 2 EXIT GUARD, site 9.  A launcher is impassable.
                if self._exit_refuse(ct, p, t, "tw_launcher"):
                    continue
                ct.build_launcher(t)
            except Exception:
                continue
    ...
    def _tw_try_gunner(self, ct, E, p, ti, rnd):
        ...
        if best is None:
            return False
        bp, d, seats = best
        # WAVE 32 FIX 2 EXIT GUARD, site 10.
        if self._exit_refuse(ct, p, bp, "tw_gunner"):
            return False
        try:
            ct.build_gunner(bp, d)
        except Exception:
    ...
    def _cg_ferry_try(self, ct, E, p, ti, rnd):
        ...
        # WAVE 69, MVETO_ROY (edit 5 of the ticket -- THE LOAD-BEARING ONE).
        # Under a vetoed geometry `_op_rungs` returns 0, NOT None (opening.py:
        # `return 0 if g is None else g["rungs"]`), and 0 is not None, so the
        # sprint test forty lines down reads `self.spr_n >= 0` -- always true.
        # Every sprint would return "opcap" and THE FERRY WOULD BE PERMANENTLY
        # CAPPED: the veto would not be a restore, it would be a new bot.
        # Found by reading the parent, not by a crash; asserted in the T0
        # static proof.
        #
        # Written as a nullify AFTER the call, never as `if OPEN_ON and not
        # self.op_map_veto` before it: the latch is set by `_op_geom`, and for
        # a raider whose first `_op_geom` call of the match IS this line the
        # pre-test would still be False and would still hand back 0.  Calling
        # first and nullifying second is correct on the first evaluation and
        # on every later one.
        if self.op_map_veto:
            op_rungs = None
        ...
        # WAVE 32 FIX 2 EXIT GUARD, site 11.
        if self._exit_refuse(ct, p, best, "cg_ferry"):
            return self._cg_why(ct, rnd, "site", ti)
        try:
    ...
    def _cg_est_post(self, ct, E, p, ti, rnd):
        ...
        # WAVE 32 FIX 2 EXIT GUARD, site 12.
        if self._exit_refuse(ct, p, best, "cg_post"):
            return self._cg_why(ct, rnd, "postsite", ti)
        try:
            ct.build_launcher(best)
        except Exception:
