...
class RingMixin:
    ...
    # ---- WAVE 65, PLACEMENT: THE ROUTING PREDICATE `P` -------------------
    # (see the WAVE 65 block at the end of doctrine.py for the corpus)

    def _w65_sourced(self, ct, t, rnd):
        """`P` -- does a directed edge-path run from one of OUR harvesters to
        tile `t`, RIGHT NOW, over the static belt graph?

        The engine has no pathfinder: a feeder's out-edges are fixed at build
        time by its `direction`, so this is a bounded walk UPSTREAM over
        in-edges, and it is the whole of the fix.  100.0% of the 170 corpus
        lays that satisfied it fed on the very next round, with zero false
        positives in 783 lays.

        MEMOISED PER UNIT PER ROUND on the query tile, and every walk in a
        round shares ONE expansion budget (`W65_SRC_BUDGET`), so eight socket
        candidates cost one bounded census between them rather than eight.
        """
        if self.w65_src_rnd != rnd:
            self.w65_src_rnd = rnd
            self.w65_src = {}
            self.w65_budget = W65_SRC_BUDGET
        k = (t.x, t.y)
        v = self.w65_src.get(k)
        if v is None:
            v = self._w65_walk(ct, t)
            self.w65_src[k] = v
        return v

    def _w65_walk(self, ct, t):
        """The bounded upstream walk itself.  True = a harvester reaches `t`.

        FAILS CLOSED on every uncertainty -- budget out, CPU out, tile not in
        vision, an unreadable building -- because this gate may only ever pass
        on POSITIVE evidence, exactly as `_ring_door_shut` may only ever arm
        on it.

        Each frontier entry carries the ONE tile it accepts from, or None for
        "any side":

          * a CONVEYOR is pushed onto the frontier only because its single
            out-edge lands on the tile we came from, and that tile is already
            in `seen` -- so "any side" is exact for it (it accepts from its
            other three);
          * a SPLITTER accepts from its BACK ONLY, so it carries that tile and
            a conveyor pointing into its flank is NOT a source.  Getting this
            wrong is the one way a walk of this shape invents a path the
            engine does not have.
        """
        if self.core is None or not (self.mw and self.mh):
            return False
        frontier = [(t.x, t.y, None)]
        seen = set(((t.x, t.y),))
        while frontier:
            if self.w65_budget <= 0 or self._cpu_exhausted(ct):
                return False
            self.w65_budget -= 1
            qx, qy, only = frontier.pop()
            for dx, dy in CARD_DELTAS:
                nx, ny = qx + dx, qy + dy
                if only is not None and (nx, ny) != only:
                    continue              # a splitter's non-back side
                if (nx, ny) in seen:
                    continue
                ...
                n = Position(nx, ny)
                try:
                    if W65_VISION_STRICT and not ct.is_in_vision(n):
                        continue          # unseen is not evidence
                ...
                if et == EntityType.HARVESTER:
                    return True           # pushes into ANY adjacent building
                if et not in BELT_TYPES:
                    continue              # a Core accepts and never outputs
                try:
                    odx, ody = DELTA[ct.get_direction(bid)]
                except Exception:
                    continue
                if et == EntityType.CONVEYOR:
                    if nx + odx != qx or ny + ody != qy:
                        continue          # its one out-edge lands elsewhere
                    nxt = None
                else:
                    if nx - odx == qx and ny - ody == qy:
                        continue          # that side is the splitter's INPUT
                    nxt = (nx - odx, ny - ody)
                seen.add((nx, ny))
                frontier.append((nx, ny, nxt))
        return False

    def _w65_lay_ok(self, ct, t, rnd, relay):
        """THE GATE.  May this plank put a conveyor on tile `t`?

        `W65_PLACE_ON` False, or `rnd < W65_MIN_RND`, and this is v168 line
        for line -- the only refusal in the build lives here and in
        `_ring_targets`, and both are REFUSALS: nothing new is ever built,
        fired, or walked to.
        """
        if not (W65_PLACE_ON and rnd >= W65_MIN_RND):
            return True
        if relay and not W65_GATE_RELAY:
            return True
        if not relay and not W65_GATE_LAY:
            return True
        ok = self._w65_sourced(ct, t, rnd)
        if W65_LOG:
            print("W65 %s (%d,%d) P=%d relay=%d r=%d"
                  % ("lay" if ok else "refuse", t.x, t.y, 1 if ok else 0,
                     1 if relay else 0, rnd))
        return ok
    ...
    def _ring_targets(self, ct, rnd):
        ...
        if own_n >= RING_MAX_OWN:
            return ()
        E = self.enemy
        # WAVE 65, PART A -- THE SORT KEY BECOMES A GATE.  `live` below is the
        # routing test v168 already computes and then throws away as a
        # tie-break; `P` is that same test with the SOURCE WALK it is missing
        # (an in-edge alone feeds 84.8% of the time, an in-edge that is
        # harvester-sourced feeds 100.0%).  A socket that fails it is not a
        # target AT ALL, so neither a claim nor a CLAIM WALK is spent on a tile
        # the belt can never reach.  One inserted refusal: with the flag off
        # the loop below is v168's, term for term.
        w65 = bool(W65_PLACE_ON and W65_GATE_TARGETS and rnd >= W65_MIN_RND)
        out = []
        for i in free:
            if w65 and not self._w65_sourced(ct, sg_socket(self.core, i), rnd):
                if W65_LOG:
                    print("W65 skip s=%d P=0 r=%d" % (i, rnd))
                continue
    ...
    def _ring_claim(self, ct, rnd):
        ...
        for i in self._ring_targets(ct, rnd):
            ...
            if abs(t.x - p.x) + abs(t.y - p.y) != 1:
                continue
            # WAVE 65, PART B.  The gate at the lay itself, so it still binds
            # if PART A is ablated off.  A refusal falls through to the next
            # target and then out of the arm entirely -- the parent's ladder
            # owns the action from there, and this build adds no use for it.
            if not self._w65_lay_ok(ct, t, rnd, False):
                continue
    ...
    def _ring_refill(self, ct, rnd):
        ...
        # WAVE 65, PART C -- the RE-LAY half of the same gate.  The intent is
        # NOT cleared: `RING_REFILL_RNDS` already expires it, and clearing it
        # here would be a state change the registration does not ask for.
        if not self._w65_lay_ok(ct, t, rnd, True):
            return False
        if not self._ring_lay(ct, t):
            return False
        self.ring_refill = None
