...
# hypothesis h1 (see docs/strategy-notes.md "Harvesters have absurd ROI --
# delivery is the real constraint" + the measured fact that crediting is
# delivery-only): after building a harvester, the SAME builder deliberately
# walks the route back toward the core, laying a correctly-facing conveyor
# chain one tile at a time, instead of relying on incidental trail-laying
# (_try_move below) to eventually and accidentally close the path. Capped in
# rounds so an unreachable core (walled off, or just far on a 30x30 map)
# can't strand a builder forever -- ~2 rounds/tile (walk + build) covers a
# generous Manhattan distance with room for one-off detours.
CHAIN_MAX_ROUNDS = 200
...
class Player:
    def __init__(self):
        ...
        # Deliberate delivery-chain state (h1): once a harvester is built,
        # this builder switches into "lay conveyors back to the core" mode
        # until the chain closes or gives up. See _run_chain.
        self.chain_active = False
        self.chain_pos: Position | None = None   # anchor: last confirmed link
        self.chain_dir: Direction | None = None  # that link's committed output direction
        self.chain_rounds = 0                    # safety counter, see CHAIN_MAX_ROUNDS
    ...
    def _run_builder(self, ct: Controller) -> None:
        ...
        # If we're mid-delivery-chain, that owns the whole round: no other
        # phase runs until the chain closes or gives up (_end_chain resets
        # self.target too, so normal duty picks a fresh target afterward).
    ...
    def _try_build_harvester(self, ct: Controller) -> bool:
        ...
        for d in dirs:
            ...
            if ct.can_build_harvester(build_pos):
                ...
                # Hand this builder to the delivery-chain state machine: it
                # will walk itself back toward the core laying a correctly-
                # facing conveyor chain (h1 -- see CHAIN_MAX_ROUNDS comment).
                # This also clears self.target, so we don't need to here.
                self._start_chain(ct, build_pos)
                return True
        return False

    # ------------------------------------------------------------------
    # Builder bot -- deliberate delivery chain (h1)
    # ------------------------------------------------------------------
    #
    # Mechanism: crediting is delivery-only (game-model.md, measured) -- a
    # harvester with no closed path to the core earns zero balance and zero
    # tiebreak-#1 credit no matter how long it sits there. _try_move below
    # only lays conveyors *incidentally*, as a side effect of ordinary
    # wandering, with no guarantee the trail ever closes a path. This state
    # machine instead walks the specific route from one harvester back to
    # the core and lays a *connected* chain, tile by tile.
    #
    # Chain invariant: self.chain_pos is a tile whose output already points
    # (or, for the harvester itself, is about to be made to point) in
    # self.chain_dir -- i.e. self.chain_pos.add(self.chain_dir) is exactly
    # the tile the next link must occupy to receive from it. Each new link's
    # own facing is computed fresh, from *its own* position, not inherited
    # from the direction used to reach it -- the two are only the same when
    # the route runs straight, and get_...direction is a pure function of
    # position, so recomputing it at build time is always consistent with
    # recomputing it again next round from the same (now-fixed) position.
    #
    # We keep it deliberately simple: one shot at each obstacle, no path
    # replanning/detouring. Blocked or out of round budget -> give up and
    # let the builder resume ordinary duty; a stub chain still leaves the
    # harvester at the incumbent's baseline (zero credited, but no worse).

    def _start_chain(self, ct: Controller, harvester_pos: Position) -> None:
        """Begin (or skip) a deliberate delivery chain from a just-built harvester."""
        self.target = None
        if self.core_pos is None:
            return
        first_dir = self._chain_step_direction(harvester_pos)
        if first_dir is None:
            return
        self.chain_active = True
        self.chain_pos = harvester_pos
        self.chain_dir = first_dir
        self.chain_rounds = 0

    def _end_chain(self) -> None:
        """Stop chain-building (success, give-up, or stuck) and resume normal duty."""
        self.chain_active = False
        self.chain_pos = None
        self.chain_dir = None
        self.target = None

    def _chain_step_direction(self, anchor: Position) -> Direction | None:
        """Pick the cardinal direction that most reduces distance from anchor to the core.

        Pure function of (anchor, self.core_pos): the same anchor always
        yields the same direction (mod the random tie-break when both axes
        are equally off), which is what keeps the chain's facings
        self-consistent -- see the chain-invariant note above. Prefers
        whichever axis has more distance left to cover, so the route is a
        short "staircase" rather than resolving x then y unconditionally
        (which would be exactly the absolute-direction bias this codebase
        has already been bitten by once).
        """
        if self.core_pos is None:
            return None
        dx = self.core_pos.x - anchor.x
        dy = self.core_pos.y - anchor.y
        options = []
        if dx > 0:
    ...
    def _run_chain(self, ct: Controller) -> None:
        """Advance the delivery chain by (at most) one link this round."""
        if self._cpu_exhausted(ct):
            return
        if self.core_pos is None or self.chain_pos is None or self.chain_dir is None:
            self._end_chain()
            return

        self.chain_rounds += 1
        if self.chain_rounds > CHAIN_MAX_ROUNDS:
            self._end_chain()
            return

        next_tile = self.chain_pos.add(self.chain_dir)
        if not in_bounds(ct, next_tile):
            self._end_chain()
            return

        pos = ct.get_position()
        if pos == next_tile:
            # We're standing exactly on the tile the chain needs next (can
            # happen on the very first link, if we built the harvester from
            # the core-facing side) -- step off so we can build there
            # instead of on our own tile.
            self._chain_step_aside(ct)
            return

        if pos.distance_squared(next_tile) != 1:
            # Not adjacent yet -- walk toward it. A single productive
            # cardinal step always reduces Manhattan distance by exactly 1,
            # so from >1 away this can never overshoot onto next_tile.
            self._chain_move(ct, next_tile)
            return

        existing = ct.get_tile_building_id(next_tile)
        if existing is not None:
            # Either we've reached the core / an existing friendly conveyor
            # (success -- the previous link's facing already points here),
            # or something else occupies the tile. Either way we can't build
            # our own conveyor on top of it, so this is where the chain ends.
            self._end_chain()
            return

        if ct.get_action_cooldown() != 0:
            return  # waiting on cooldown; retry next round

        if ct.get_global_resources() < ct.get_conveyor_cost():
            return  # waiting on funds; retry next round

        new_dir = self._chain_step_direction(next_tile)
        if new_dir is None:
            self._end_chain()
            return

        if ct.can_build_conveyor(next_tile, new_dir):
            ct.build_conveyor(next_tile, new_dir)
            self.chain_pos = next_tile
            self.chain_dir = new_dir
        else:
            # Blocked (wall, off-map facing, occupied tile we didn't catch
            # above, etc.) -- give up rather than replanning around it, per
            # the "one shot per obstacle" design above.
            self._end_chain()

    def _chain_step_aside(self, ct: Controller) -> None:
        """Step off the chain's target tile so it becomes buildable next round."""
    ...
    def _chain_move(self, ct: Controller, target: Position) -> None:
        """Walk one cardinal step toward target, same anti-bias shape as _move_toward_target."""
        if ct.get_move_cooldown() != 0:
            return
        pos = ct.get_position()
        dx = target.x - pos.x
        ...
        for d in [desired, *perpendicular, desired.opposite()]:
            next_pos = pos.add(d)
            if not in_bounds(ct, next_pos):
                continue
            if ct.can_move(d):
