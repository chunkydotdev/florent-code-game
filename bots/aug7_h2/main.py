...
class Player:
    ...
    def _try_build_sentinel(self, ct: Controller) -> bool:
        """Build a sentinel turret on the best-scoring adjacent (position, facing)
        pair we can find, instead of the first legal one facing "away from core".
        ...
        A sentinel is 30 Ti plus a 20% cost-scale tax -- the joint most
        expensive irreversible decision this bot makes (strategy-notes.md,
        "Underused API surface") -- and its value is almost entirely a
        function of what its line actually crosses. The incumbent picked
        facing from wherever the builder happened to be standing; this
        version scores candidates with get_attackable_tiles_from() before
        spending, see _best_sentinel_placement().
        ...
        choice = self._best_sentinel_placement(ct, pos)
        if choice is None:
            return False
        build_pos, facing = choice
        if ct.can_build_sentinel(build_pos, facing):
            ct.build_sentinel(build_pos, facing)
            return True
        return False

    def _estimate_enemy_core(self, ct: Controller) -> Position | None:
        """Guess the enemy Core's location from map symmetry.

        Maps are symmetric by reflection or rotation (CLAUDE.md). Point
        reflection through the map centre is exact for 180-degree rotational
        symmetry and a reasonable direction-only approximation for mirror
        symmetry too, since bases sit at opposite corners/edges either way.
        We only need a rough *direction* out of this -- the corridor the
        enemy must approach through -- not an exact tile.
        """
        if self.core_pos is None:
            return None
        w, h = ct.get_map_width(), ct.get_map_height()
        # Core is a 2x2 footprint anchored at its NW corner (get_position());
        # mirror the footprint's centre, not just the anchor tile.
        mx = (w - 1) - (self.core_pos.x + 1)
        my = (h - 1) - (self.core_pos.y + 1)
        mx = max(0, min(w - 1, mx))
        my = max(0, min(h - 1, my))
        return Position(mx, my)

    def _friendly_sentinel_coverage(self, ct: Controller) -> set[int]:
        """Packed tiles already covered by nearby friendly sentinels' arcs.

        Used so a new sentinel isn't placed to re-cover ground an existing
        one already watches -- a duplicated arc is wasted titanium at 30 Ti
        + 20% scale tax. Bounded to buildings within this builder's own
        vision, so it stays cheap.
        """
        covered: set[int] = set()
        my_team = ct.get_team()
        for bid in ct.get_nearby_buildings():
            try:
                ...
                if ct.get_team(bid) != my_team:
                    continue
                spos = ct.get_position(bid)
                sdir = ct.get_direction(bid)
                for t in ct.get_attackable_tiles_from(spos, sdir, EntityType.SENTINEL):
                    covered.add(pack_pos(t))
            except Exception:
                continue
        return covered

    def _best_sentinel_placement(
        self, ct: Controller, pos: Position
    ) -> tuple[Position, Direction] | None:
        """Score candidate (position, facing) pairs and return the best one.

        Candidate positions are the (up to 4) orthogonally adjacent tiles --
        building requires orthogonal adjacency, not diagonal. Candidate
        facings are all 8 compass directions (turret facing is unrestricted,
        unlike builder movement). That's at most 4 x 8 = 32 hypothetical
        lines, each capped at ~5-6 tiles by the sentinel's r^2=32 attack
        radius, so this stays cheap even called every round.

        Scoring ingredients per attackable tile:
          - proximity to our Core (1/(1+dist_sq)): the immediate perimeter
            around the Core is where every undefended approach ends up.
          - corridor alignment: projected distance along, and perpendicular
            deviation from, the line toward the estimated enemy Core --
            rewards lines that point down the approach corridor instead of
            out to open map or back over our own base.
          - a discount (not exclusion -- some overlap is fine for burst) on
            tiles an existing friendly sentinel's arc already covers.
        """
        if self.core_pos is None:
            return None
        enemy_est = self._estimate_enemy_core(ct)
        ex = ey = 0
        if enemy_est is not None:
            corridor_dir = self.core_pos.direction_to(enemy_est)
            if corridor_dir != Direction.CENTRE:
                ex, ey = corridor_dir.delta()

        covered = self._friendly_sentinel_coverage(ct)

        candidate_dirs = list(CARDINALS)
        random.shuffle(candidate_dirs)
        facings = list(DIRECTIONS)
        random.shuffle(facings)
        ...
        for d in candidate_dirs:
            ...
            for facing in facings:
                if not ct.can_build_sentinel(build_pos, facing):
                    continue
                tiles = ct.get_attackable_tiles_from(build_pos, facing, EntityType.SENTINEL)
                if not tiles:
                    continue
                score = 0.0
                for t in tiles:
                    rel_x = t.x - self.core_pos.x
                    rel_y = t.y - self.core_pos.y
                    core_d = rel_x * rel_x + rel_y * rel_y
                    proximity = 1.0 / (1.0 + core_d)
                    corridor = 0.0
                    if ex or ey:
                        proj = rel_x * ex + rel_y * ey
                        if proj > 0:
                            cross = rel_x * ey - rel_y * ex
                            corridor = proj / (1.0 + abs(cross))
                    weight = 0.4 if pack_pos(t) in covered else 1.0
                    score += (proximity + corridor) * weight
                if score > best_score:
                    best_score = score
                    best = (build_pos, facing)
        return best
