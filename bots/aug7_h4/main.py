...
class Player:
    def __init__(self):
        ...
        # --- STAGE 1 INSTRUMENTATION (h4 probe) ---
        # Pending harvesters this builder built, awaiting a verdict on whether
        # they pick up an orthogonally-adjacent friendly conveyor. Each entry:
        # {"pos": Position, "build_round": int}. Resolved entries are dropped.
        self.h4_pending: list[dict] = []
    ...
    def _run_builder(self, ct: Controller) -> None:
        ...
        # --- STAGE 1 INSTRUMENTATION (h4 probe) ---
        # Cheap: h4_pending is small (this builder's own recent harvesters).
        self._h4_check_pending(ct)
    ...
    def _try_build_harvester(self, ct: Controller) -> bool:
        ...
        for d in dirs:
            ...
            if ct.can_build_harvester(build_pos):
                ...
                # --- STAGE 1 INSTRUMENTATION (h4 probe) ---
                import sys
                print(f"H4_BUILT pos=({build_pos.x},{build_pos.y}) round={ct.get_current_round()}", file=sys.stderr)
                self.h4_pending.append({"pos": build_pos, "build_round": ct.get_current_round()})
    ...
    # ------------------------------------------------------------------
    # STAGE 1 INSTRUMENTATION (h4 probe) — measurement only, no strategy
    # change. Answers: when a harvester is built, does it end up with an
    # orthogonally-adjacent friendly conveyor, and how many rounds does
    # that take? Verdicts print to stderr as H4_RESULT lines; aggregate
    # with grep/sort/uniq, never read into context directly.
    # ------------------------------------------------------------------

    H4_MAX_AGE = 10  # rounds to wait before giving up on a pending harvester

    def _h4_check_pending(self, ct: Controller) -> None:
        if not self.h4_pending:
            return
        import sys
        my_team = ct.get_team()
        round_now = ct.get_current_round()
        still_pending = []
        for entry in self.h4_pending:
            hpos = entry["pos"]
            build_round = entry["build_round"]
            age = round_now - build_round

            # Harvester destroyed before we got a verdict?
            if ct.is_in_vision(hpos):
                hid = ct.get_tile_building_id(hpos)
                if hid is None or ct.get_entity_type(hid) != EntityType.HARVESTER or ct.get_team(hid) != my_team:
                    print(f"H4_RESULT pos=({hpos.x},{hpos.y}) build_round={build_round} outcome=LOST age={age}", file=sys.stderr)
                    continue

                if self._h4_has_adjacent_friendly_conveyor(ct, hpos, my_team):
                    print(f"H4_RESULT pos=({hpos.x},{hpos.y}) build_round={build_round} outcome=YES delay={age}", file=sys.stderr)
                    continue

                if age >= self.H4_MAX_AGE:
                    print(f"H4_RESULT pos=({hpos.x},{hpos.y}) build_round={build_round} outcome=NO age={age}", file=sys.stderr)
                    continue
            else:
                if age >= self.H4_MAX_AGE:
                    print(f"H4_RESULT pos=({hpos.x},{hpos.y}) build_round={build_round} outcome=UNRESOLVED age={age} reason=out_of_vision", file=sys.stderr)
                    continue

            still_pending.append(entry)
        self.h4_pending = still_pending

    def _h4_has_adjacent_friendly_conveyor(self, ct: Controller, hpos: Position, my_team) -> bool:
        for d in CARDINALS:
            npos = hpos.add(d)
            if not in_bounds(ct, npos):
                continue
            if not ct.is_in_vision(npos):
                continue
            nid = ct.get_tile_building_id(npos)
            if nid is None:
                continue
            if ct.get_entity_type(nid) == EntityType.CONVEYOR and ct.get_team(nid) == my_team:
                return True
        return False
