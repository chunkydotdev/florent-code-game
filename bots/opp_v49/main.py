"""v62 OFFLINE — Beat Coreflood/Lorem replay rushes (+ live CPU wrapper).

v61 baseline plus current-pool map packs, barrier-priority melee, Nordkap-A
role-3 defend disabled, and rush-tile barrier denial on Nordkap-B / Jackpot-A /
Moonrise-A. Includes the ladder-required CPU-budget bail-out and run()
exception wrapper so units are not permanently deleted on live hardware.
"""
import math
...
class Player:
    ...
    def _rush_deny_tiles(self):
        if self.mw == 20 and self.mh == 26 and (self.core.x, self.core.y) == (9, 18):
            return (Position(9, 15), Position(10, 15), Position(9, 16)), Position(9, 16)
        if self.mw == 21 and self.mh == 8 and (self.core.x, self.core.y) == (5, 3):
            return (Position(9, 4), Position(9, 2), Position(10, 3), Position(8, 3), Position(10, 2)), Position(9, 4)
        if self.mw == 16 and self.mh == 16 and (self.core.x, self.core.y) == (0, 0):
    ...
    def _try_rush_deny(self, ct):
        ...
        if not tiles:
            return False
        # Nordkap/Moonrise denial stays on the dedicated defend bunker.
        if (self.mw == 20 and self.mh == 26) or (self.mw == 21 and self.mh == 8):
            return False
    ...
    def _defend(self, ct):
        ...
        moonrise_bunker = (
            self.mw == 21 and self.mh == 8
            and (self.core.x, self.core.y) == (5, 3)
            and ct.read_store(SLOT_UNDER)
        )
        jackpot_bunker = (
            self.mw == 16 and self.mh == 16
            and (self.core.x, self.core.y) == (0, 0)
            and ct.read_store(SLOT_UNDER)
        )
        bunker_tiles = (
            (Position(9, 15), Position(10, 15), Position(9, 16)) if nordkap_bunker else
            (Position(9, 4), Position(9, 2), Position(10, 3), Position(8, 3), Position(10, 2)) if moonrise_bunker else
            (Position(3, 3), Position(4, 1), Position(2, 2), Position(3, 2)) if jackpot_bunker else
            ()
        )
        bunker_station = (
            Position(9, 16) if nordkap_bunker else
            Position(9, 4) if moonrise_bunker else
            Position(2, 2) if jackpot_bunker else
            None
        )
