"""v89_open_ti_floor8 OFFLINE — overnight promote (+ live CPU wrapper).

Verified NEW 80 / OLD 80 / 9436 20 / LR 20 / broad 336/336.
Lever: v89_open_ti_floor8
"""
import math
...
CORE_PAIRS = (
    ...
    (24, 24, 4, 4, 18, 18), (25, 25, 5, 5, 18, 18),
...
class Player:
    ...
    def _core(self, ct):
        ...
        ammo_target = (
            ...
            else (32 if atoll_burst_magazine else (32 if under else AMMO_FLOOR))
        )
        if weapons:
        ...
        ti_floor = (8 if (_open_lsq_far and under) else 12) if (under or weapons) else 52
        # Tiny arenas: keep TI for the opening harvester under early UNDER.
        if harv < 1 and w * h <= 120:
            ti_floor = max(ti_floor, ct.get_harvester_cost() + 4)
        ...
        _early = (
            4 if (_midsq_near or (_lsq650_far and _arch_dense))
            else (6 if _lsq650_far else EARLY_BUILDERS)
        )
        spawn_cap = mature_cap if harv >= 1 else min(_early, mature_cap)
        ...
        if self.n < spawn_cap and can_spend_spawn and ti >= ct.get_builder_bot_cost():
            ...
            if self.n == 0 and ec is not None:
                ...
                cands.sort(key=lambda sp: (dist_core(sp, ec), sp.y, sp.x))
            else:
    ...
    def _builder(self, ct):
        ...
        for eid in ct.get_nearby_entities():
            ...
            _midsq_far = (
                240 <= self.mw * self.mh <= 280 and self.mw == self.mh
                and self.core.x + self.core.y >= (self.mw + self.mh) // 2
            )
            _tiny_near = (
                self.mw * self.mh <= 120
                and self.core.x + self.core.y < (self.mw + self.mh) // 2
            ...
            _boost = _midsq_far or _tiny_near or _nonsq_near
            gun_sense = 100 if (self.mw * self.mh >= 650 and self.mw == self.mh) else (81 if _boost else 64)
            b_sense = 36 if (self.mw * self.mh >= 650 and self.mw == self.mh) else (25 if _boost else 16)
            if (et in (EntityType.GUNNER, EntityType.SENTINEL) and d <= gun_sense) or (
                et == EntityType.BUILDER_BOT and d <= b_sense
            ):
                ct.write_store(SLOT_UNDER, 1)
        ...
        rnd = ct.get_current_round()
        # Disabled: forcing role 3 onto defend on Nordkap-A cedes the mid-map
        # gun race to Coreflood/Lorem-style rushes (local coreflood_proxy).
        if False and (
        ...
        if (
            ...
            self.role = "saboteur"
        # Fjordgate-class arenas only: second attacker after one harvester.
        if (
            self.role == "expand" and self.role_n == 2 and not self.link_queue
            and self.mw * self.mh <= 120
            and ct.read_store(SLOT_HARVESTERS) >= 1 and rnd >= 6
        ):
            self.role = "saboteur"
        # Square compact (lighthouse/jackpot 16x16): second attacker.
        if (
            self.role == "expand" and self.role_n == 2 and not self.link_queue
            and 240 <= self.mw * self.mh <= 280 and self.mw == self.mh
            and ct.read_store(SLOT_HARVESTERS) >= 2 and rnd >= 8
        ):
            self.role = "saboteur"
        if (
            self.role == "expand" and self.role_n == 2 and not self.link_queue
            and 500 <= self.mw * self.mh <= 560 and self.mw < self.mh
            and self.core is not None
            and self.core.x + self.core.y >= (self.mw + self.mh) // 2
            and rnd >= 4
        ):
            self.role = "defend"
        # Non-square compact: no early sab on near-half (antler A).
        if (
            self.role == "expand" and self.role_n == 2 and not self.link_queue
            and 240 <= self.mw * self.mh <= 280 and self.mw != self.mh
            and self.core is not None
        ...
        if (
            self.role == "expand" and self.role_n == 3 and not self.link_queue
            and self.mw == 20 and self.mh == 26
            and (self.core.x, self.core.y) == (9, 18)
            and ct.read_store(SLOT_HARVESTERS) >= 2 and rnd >= 10
        ):
            self.role = "saboteur"
        # Eider-B (open 28x20): lazy/sporks plant mid guns by t8–15. Flip the
        # spare expander into artillery once one harvester exists. Heart shares
        # the same Core anchors but is wall-dense — leave expanders mining.
        if (
            self.role == "expand" and self.role_n == 3 and not self.link_queue
            and self.mw == 28 and self.mh == 20
            and (self.core.x, self.core.y) == (19, 9)
            and self.map_grid is not None
            and sum(row.count("#") for row in self.map_grid) < 80
            and ct.read_store(SLOT_HARVESTERS) >= 1 and rnd >= 8
        ):
            self.role = "saboteur"
        ...
        tiny_arena = self.mw * self.mh <= 120
        square_band = 240 <= self.mw * self.mh <= 280 and self.mw == self.mh
        large_square = self.mw * self.mh >= 600 and self.mw == self.mh
        ...
        if (
            keep_artillery_forward
            and 240 <= self.mw * self.mh <= 280 and self.mw != self.mh
            and ct.read_store(SLOT_UNDER)
            and ct.read_store(SLOT_HOME_GUN) < 1
        ):
            keep_artillery_forward = False
    ...
    def _home_defend(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            if self._sabotage_prio(ct):
                pass
            elif self._deny_threat_tile(ct):
                pass
    ...
    def _sabotage_prio(self, ct):
        ...
        for d in CARDINALS:
            ...
            pr = {
                ...
                EntityType.BARRIER: 1, EntityType.CORE: 2,
                EntityType.HARVESTER: 3, EntityType.LAUNCHER: 4,
                EntityType.CONVEYOR: 5, EntityType.SPLITTER: 5,
            }.get(et, 6)
    ...
    def _plan_siege(self, ct):
        ...
        if self.map_grid is None or self.enemy is None:
            return False
        _lsq650_far = (
            self.mw * self.mh >= 650 and self.mw == self.mh
            and self.core is not None
            and self.core.x + self.core.y >= (self.mw + self.mh) // 2
        )
        cap = (2 if _lsq650_far else 3) if self.role_n == 0 else (1 if _lsq650_far else 2)
        if self.forward_guns >= cap:
            return False
        _wide = 500 <= self.mw * self.mh <= 560 and self.mw > self.mh
        _lsq = 600 <= self.mw * self.mh < 650 and self.mw == self.mh
        eco_need = 1 if (self.mw * self.mh <= 120 or ((_wide or _lsq) and ct.read_store(SLOT_UNDER))) else ECO_NEED
        ...
        turret_type = (
            EntityType.SENTINEL
            if PRIMARY_SENTINEL and self.role_n == 0 and self.forward_guns == 0 and not tiny_arena
    ...
    def _try_siege_build(self, ct):
        ...
        if max(abs(p.x - spot.x), abs(p.y - spot.y)) > 1 or p == spot:
            return False
        if (
            600 <= self.mw * self.mh < 650 and self.mw == self.mh and self.core is not None and abs(self.core.x - self.core.y) >= 15 and self.core.x >= self.mw // 2 and self.forward_guns == 0 and not ct.read_store(SLOT_UNDER)
            and ct.get_current_round() < 26
        ):
            return False
        built = False
        if (
    ...
    def _saboteur(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            # Melee a mid-map gun before spending the turn on siege repair.
            if self._sabotage_prio(ct):
                return
        ...
        # South-side Nordkap only: keep_artillery_forward blocks home recall, so
        # walk onto the mid-map gun line and melee it. North-side keeps pure siege.
        if (
            self.mw == 20 and self.mh == 26
            and self.core.y == 18
            and ct.read_store(SLOT_UNDER)
            ...
            if threat is not None and 11 <= threat.y <= 16:
                self.tgt = threat
                self._nav(ct, pave=False)
                return
        ...
        elif self.forward_guns >= 1 and ct.read_store(SLOT_HARVESTERS) < (
            1 if (self.mw * self.mh <= 120 or (500 <= self.mw * self.mh <= 560 and self.mw < self.mh and ct.read_store(SLOT_UNDER))) else ECO_NEED
        ):
            self.tgt = p
        else:
            self.tgt = ec
    ...
    def _deny_threat_tile(self, ct):
        """Melee a hostile building on the reported threat cell (any map)."""
        if not ct.read_store(SLOT_UNDER) or ct.get_action_cooldown() != 0:
            return False
        threat = unpack_pos(ct.read_store(SLOT_THREAT))
        if threat is None:
            return False
        p = ct.get_position()
        if abs(p.x - threat.x) + abs(p.y - threat.y) != 1:
            return False
        bid = ct.get_tile_building_id(threat)
        if bid is not None and ct.get_team(bid) != self.team and ct.can_fire(threat):
            ct.fire(threat)
            return True
        return False
    ...
    def _rush_deny_tiles(self):
        c = (self.core.x, self.core.y)
        if self.mw == 20 and self.mh == 26 and c == (9, 18):
            return (Position(9, 15), Position(10, 15), Position(9, 16)), Position(9, 16)
        if self.mw == 21 and self.mh == 8 and c == (5, 3):
            return (Position(9, 4), Position(9, 2), Position(10, 3), Position(8, 3), Position(10, 2)), Position(9, 4)
        if self.mw == 21 and self.mh == 8 and c == (14, 3):
            return (
                Position(11, 3), Position(12, 2), Position(11, 4), Position(12, 3),
                Position(12, 4), Position(13, 3),
            ), Position(12, 3)
        if self.mw == 16 and self.mh == 16 and c == (0, 0):
            return (Position(3, 3), Position(4, 1), Position(2, 2), Position(3, 2)), Position(2, 2)
        if self.mw == 16 and self.mh == 16 and c == (14, 14):
            return (
                Position(12, 12), Position(15, 11), Position(15, 12), Position(13, 12),
                Position(12, 14),
            ), Position(13, 13)
        if self.mw == 25 and self.mh == 15 and c == (11, 3):
            return (
                Position(12, 5), Position(11, 5), Position(12, 6), Position(13, 6),
                Position(11, 6), Position(12, 7),
            ), Position(11, 5)
        if self.mw == 28 and self.mh == 20 and c == (19, 9):
            return (
                Position(16, 9), Position(17, 7), Position(18, 9), Position(17, 8),
                Position(16, 10), Position(18, 8),
            ), Position(17, 9)
        return (), None

    def _try_rush_deny(self, ct):
        """Melee or barrier-deny known rush gun tiles. Any builder may do this."""
        if not ct.read_store(SLOT_UNDER) or ct.get_action_cooldown() != 0:
            return False
        tiles, station = self._rush_deny_tiles()
        if not tiles:
            return False
        # Nordkap/Moonrise/Meander/Eider-Heart denial stays on defend bunkers.
        if (self.mw == 20 and self.mh == 26) or (self.mw == 21 and self.mh == 8) or (
            self.mw == 25 and self.mh == 15
        ) or (self.mw == 28 and self.mh == 20):
            return False
        if self.role == "expand" and ct.read_store(SLOT_HARVESTERS) < 2:
            return False
        p = ct.get_position()
        for bp in tiles:
            ...
            if bid is not None and ct.get_team(bid) != self.team and ct.can_fire(bp):
                ct.fire(bp)
                return True
            if (
                bid is None
    ...
    def _defend(self, ct):
        ...
        tiles, bunker_station = self._rush_deny_tiles()
        # Meander-A needs the defend engineer on the corridor before Pantheon's
        # t3 mid gun. Moonrise-B waits for UNDER so eco can open.
        always_bunker = (
            (self.mw == 25 and self.mh == 15 and (self.core.x, self.core.y) == (11, 3))
            or (self.mw == 28 and self.mh == 20 and (self.core.x, self.core.y) == (19, 9))
            or (
                500 <= self.mw * self.mh <= 560 and self.mw < self.mh
                and self.core is not None
                and self.core.x + self.core.y >= (self.mw + self.mh) // 2
            )
        )
        use_bunker = bool(tiles) and (always_bunker or ct.read_store(SLOT_UNDER))
        bunker_tiles = tiles if use_bunker else ()
        if not use_bunker:
            bunker_station = None
        if hive_bunker and ct.get_action_cooldown() == 0:
            # Melee/deny adjacent enemy guns before the peacetime heal loop.
            for d in DIRECTIONS:
                tile = p.add(d)
                ...
                if (
                    ...
                    and ct.get_entity_type(eid) in (EntityType.GUNNER, EntityType.SENTINEL)
                    and ct.can_fire(tile)
                ):
                    ct.fire(tile)
                    return
            bp = Position(20, 4)
            ...
            if abs(p.x - bp.x) + abs(p.y - bp.y) == 1:
                if (
                    ...
                    and ct.get_entity_type(bid) == EntityType.BARRIER
                    and ct.can_heal(bp)
                    and not ct.read_store(SLOT_UNDER)
                ):
                    ct.heal(bp)
                    return
                if (
                    ...
                    ct.build_barrier(bp)
                    return
        # Deny the rush gun tiles observed in Coreflood/Lorem replays.
        if bunker_tiles and ct.get_action_cooldown() == 0:
            for bp in bunker_tiles:
                if abs(p.x - bp.x) + abs(p.y - bp.y) != 1:
                    continue
                bid = ct.get_tile_building_id(bp)
                if (
        ...
        if bunker_station is not None:
            if ct.get_move_cooldown() == 0:
                self.tgt = bunker_station
                self._nav(ct, pave=False)
            return
    ...
    def _expand(self, ct):
        ...
        sq25_freeze = (
            self.mw == 25 and self.mh == 25
            and ct.read_store(SLOT_HOME_GUN) >= 2
            and ct.get_current_round() >= 42
        )
        if hive_freeze or sq25_freeze:
            return
        ...
        if ct.get_action_cooldown() == 0:
            # Heal under fire near home, but not on tiny arenas where every
            # action must stay on the gun race (fjordgate-class ≤120 cells).
            # Area>600: smash adjacent rush guns before heal/eco.
            if (
                ct.read_store(SLOT_UNDER)
                and self.core is not None
                and (
                    ...
                    or (
                        self.mw * self.mh <= 120
                        and self.core.x + self.core.y < (self.mw + self.mh) // 2
                        and ct.get_position().distance_squared(self.core) <= 25
                    )
                    or (
                        self.mw * self.mh <= 120
                        and self.core.x + self.core.y >= (self.mw + self.mh) // 2
                        and ct.get_position().distance_squared(self.core) <= 16
                    )
                )
                and ct.get_position().distance_squared(self.core) <= 36
            ):
                for d in DIRECTIONS:
                    tile = p.add(d)
                    if not (0 <= tile.x < self.mw and 0 <= tile.y < self.mh):
            ...
            if (
                ...
                and self.core is not None
                and ct.get_position().distance_squared(self.core) <= 16
                and self._heal_core(ct)
            ):
                return
            if self._try_rush_deny(ct):
                return
        ...
        # Maze maps (Heart): parking on an ore cell never builds a Harvester.
        # Only step off when the known pack is wall-dense so open maps keep
        # the proven approach-onto-ore behaviour.
        if (
            ...
            for d in CARDINALS:
                ...
                if ct.get_tile_env(bp) == Environment.ORE_TITANIUM:
                    continue
                if (bp.x, bp.y) in self.map_walls:
                    continue
                if ct.can_move(d):
                    ct.move(d)
                    return
            ...
            for t in ct.get_attackable_tiles():
                ...
                if healer_focus:
                    prio = {
                        ...
                        EntityType.BARRIER: 4, EntityType.LAUNCHER: 5,
                        EntityType.HARVESTER: 6,
                        EntityType.CONVEYOR: 7, EntityType.SPLITTER: 7,
                    }.get(et, 8)
                else:
                    prio = {
