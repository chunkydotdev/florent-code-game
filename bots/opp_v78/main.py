"""v61/S5 OFFLINE — replay-routed macro with verified choke spending.
...
S5 keeps S4's known-map choke idea but verifies the actual end-to-end path
delay before spending.  The old local-width test selected barriers that added
zero path length on most wall-heavy pool maps.  A choke now has to preserve a
finite route and add CHOKE_MIN_PATH_DELAY steps; otherwise it is dead capital
and stays unbuilt.  Only the interceptor plans it, removing the same one-time
BFS from every other builder.
...
CARDINAL_DELTAS = {d: d.delta() for d in CARDINALS}
...
# SLOT_LAUNCHER is a heartbeat, not a permanent boolean.  A destroyed
# Launcher used to remain "alive" forever in shared state, preventing rebuilds
# and repeatedly recruiting builders into an impossible launch queue.
LAUNCHER_STALE_RNDS = 5
...
CHOKE_WIDTH_MAX = 2
CHOKE_MAX_TILES = 3
# The local-width heuristic is only a candidate generator.  Require the final
# wall set to add a meaningful, finite detour on the full decoded terrain.
# A disconnected route is rejected too: it blocks our own ground saboteurs.
CHOKE_MIN_PATH_DELAY = 8
...
SLOT_ENEMY_CORE = 3
SLOT_HARVESTERS = 4
# Slot 5 is intentionally free.  SLOT_ECO_READY was write-only.
SLOT_LAUNCHER = 6
SLOT_HOME_GUN = 7
SLOT_DROPPED = 8
# Slot 9 is intentionally free.  SLOT_LINKS_DONE was write-only.
SLOT_LAUNCH_ID = 10
SLOT_LAUNCH_RND = 11
SLOT_LAUNCHED_ID = 12
...
class Player:
    def __init__(self):
        ...
        self.map_ores = []
        self.ore_cursor = 0
        self.ore_plan_key = None
        self.ore_plan = []
        self.forward_guns = 0
        # Forward-gun capacity tracks live, locally verifiable batteries.
        # The old monotonic counter permanently disabled rebuilds after loss.
        self.forward_gun_sites = []
        ...
        # Launcher geometry is invariant for the lifetime of the building.
        # Cache it instead of rebuilding/sorting the same target lists every
        # round in the launcher's hot path.
        self.launch_cache_key = None
        self.launch_drop_sites = []
        self.launch_exile_sites = []
        self.launch_advance_sites = []
    ...
    def _launcher_alive(self, ct):
        """Whether SLOT_LAUNCHER contains a fresh launcher heartbeat."""
        beat = ct.read_store(SLOT_LAUNCHER)
        return bool(
            beat
            and ct.get_current_round() - (beat - 1) <= LAUNCHER_STALE_RNDS
        )

    def _note_friendly_launcher(self, ct):
        if self._launcher_alive(ct):
            return
        for eid in ct.get_nearby_buildings():
            if ct.get_team(eid) == self.team and ct.get_entity_type(eid) == EntityType.LAUNCHER:
                ct.write_store(SLOT_LAUNCHER, ct.get_current_round() + 1)
                return
    ...
    def _builder(self, ct):
        ...
        if self.map_grid is None:
            ...
            if self.map_grid is not None:
                ...
                # CHOKEPOINT WALLS (S5), planned once by the interceptor here
                ...
                if self.role_n == 1 \
                        and len(self.map_walls) >= ORE_STEPOFF_MIN_WALLS \
                    ...
                    if ck_hit is not None:
                        ...
                        for i in range(ck_third, len(ck_path) - ck_third):
                            ...
                            if (tx, ty) in ck_ore or (tx, ty) in ck_no:
                                continue
                            # Candidate wall set: the choke tile plus its
                            # passable off-path
                            ...
                            for d in CARDINALS:
                                ...
                                if (
                                    ...
                                    ck_walls.append(n)
                            # The old code accepted the first local pinch.
                            # That is not enough: alternate routes elsewhere
                            # on the map made most shipped candidates add zero
                            # steps.  Measure the full route with the candidate
                            # blocked and retain only the best finite detour.
                            ck_after = self._terrain_path_length(
                                ct, ck_enemy, self.core, ck_walls
                            )
                            if ck_after is None:
                                # Either fully disconnected (bad for our own
                                # ground attackers) or the CPU guard tripped.
                                continue
                            ck_delay = ck_after - ck_base_len
                            if ck_delay > ck_best_delay:
                                ck_best_delay = ck_delay
                                ck_best_walls = ck_walls
                        if ck_best_delay >= CHOKE_MIN_PATH_DELAY:
                            self.choke_tiles = ck_best_walls
        ...
        if self.role == "launchwait":
            if self.dropped:
                self.role = "saboteur"
            elif rnd >= 70 and not self._launcher_alive(ct) and self.role_n != 5:
                self.role = "saboteur"
        ...
        if (
            ...
            and rnd < LAUNCH_GIVEUP_RND and self._launcher_alive(ct)
    ...
    def _sync_forward_guns(self, ct):
        """Drop forward batteries that are visibly gone from the local cap."""
        if not self.forward_gun_sites:
            self.forward_guns = 0
            return
        kept = []
        for spot in self.forward_gun_sites:
            ...
            try:
                ...
                # Vision-edge failures must not create phantom capacity.
                kept.append(spot)
        self.forward_gun_sites = kept
        self.forward_guns = len(kept)
    ...
    def _try_siege_build(self, ct):
        ...
        if (
            ...
            built = True
        if built:
            self.forward_gun_sites.append(spot)
            self.forward_guns = len(self.forward_gun_sites)
            ...
            if not defended:
                ...
                if self.link_queue and ti >= ct.get_conveyor_cost():
                    if self._build_next_link(ct):
                        return
                # Wake the Launcher subsystem. _try_build_launcher() claims
                # SLOT_LAUNCHER before building; the heartbeat lets this path
                # recover only after a real Launcher disappears.
    ...
    def _expand(self, ct):
        ...
        has_launch = self._launcher_alive(ct)
        harv = ct.read_store(SLOT_HARVESTERS)
        ...
        if ct.get_action_cooldown() == 0:
            if self.link_queue and self._build_next_link(ct):
                return
            if self._eco_spendable(ct, ct.get_harvester_cost()) and harv < self._eco_cap(ct):
                for d in DIRECTIONS:
                    bp = p.add(d)
                    if 0 <= bp.x < self.mw and 0 <= bp.y < self.mh and ct.can_build_harvester(bp):
                        if not self._harvester_site_safe(ct, bp):
            ...
            if ct.get_global_resources() >= MEDIC_TI_FLOOR and (
                medic_late
    ...
    def _terrain_path_length(self, ct, start_anchor, goal_anchor, extra_blocked=()):
        """Static ground distance between two Core footprints.

        Used only to validate a proposed choke.  Returning None means either
        no finite route remains or the per-unit CPU guard asked us to stop;
        both outcomes correctly reject spending on the candidate.
        """
        blocked = set(self.map_walls)
        blocked.update((c.x, c.y) for c in core_tiles(start_anchor))
        blocked.update((c.x, c.y) for c in core_tiles(goal_anchor))
        blocked.update((p.x, p.y) for p in extra_blocked)

        starts = set()
        for c in core_tiles(start_anchor):
            for d in CARDINALS:
                dx, dy = CARDINAL_DELTAS[d]
                key = (c.x + dx, c.y + dy)
                if (
                    0 <= key[0] < self.mw and 0 <= key[1] < self.mh
                    and key not in blocked
                ):
                    starts.add(key)
        goals = set()
        for c in core_tiles(goal_anchor):
            for d in CARDINALS:
                dx, dy = CARDINAL_DELTAS[d]
                key = (c.x + dx, c.y + dy)
                if (
        ...
        seen = set(starts)
        q = deque((x, y, 0) for x, y in starts)
        steps = 0
        while q:
            x, y, dist = q.popleft()
            if (x, y) in goals:
            ...
            for d in CARDINALS:
                ...
                if (
                    ...
                    or not (0 <= key[0] < self.mw and 0 <= key[1] < self.mh)
                ):
                    continue
                seen.add(key)
                q.append((key[0], key[1], dist + 1))
        return None
    ...
    def _pick(self, ct):
        if self.map_ores and self.role == "expand":
            ...
            plan_key = (workers, worker)
            if self.ore_plan_key != plan_key:
                ordered = sorted(
                    self.map_ores,
                    key=lambda t: (
                ...
                self.ore_plan = ordered[worker::workers] or ordered
                self.ore_plan_key = plan_key
            assigned = self.ore_plan
        ...
        while q:
            ...
            for d in order:
                ...
                if key in goals:
                    return first_step
                seen.add(key)
                q.append((key[0], key[1], first_step))
        return p.cardinal_direction_to(target)
    ...
    def _turret(self, ct):
        ...
        try:
            ...
            pass
        nearby = tuple(ct.get_nearby_entities())
        for eid in nearby:
    ...
    def _launcher(self, ct):
        ...
        lp = ct.get_position()
        cache_key = (
            w, h, self.core.x, self.core.y,
            dest.x, dest.y, lp.x, lp.y,
        )
        if self.launch_cache_key != cache_key:
            drop_sites = []
            ...
            for dx in range(-5, 6):
                for dy in range(-5, 6):
                    ...
                    if 0 <= site.x < w and 0 <= site.y < h:
                        throw_sites.append(site)
            self.launch_drop_sites = uniq
            self.launch_exile_sites = sorted(
                throw_sites,
                key=lambda t: t.distance_squared(self.core),
                reverse=True,
            )
            self.launch_advance_sites = sorted(
                throw_sites,
                key=lambda t: t.distance_squared(dest),
            )
            self.launch_cache_key = cache_key

        drop_sites = self.launch_drop_sites
        cands = []
        chosen = ct.read_store(SLOT_LAUNCH_ID)
        chosen_rnd = ct.read_store(SLOT_LAUNCH_RND)
        ...
        for bp in enemy_bots:
            for site in self.launch_exile_sites:
        ...
        for _, bp in cands:
            ...
            for site in self.launch_advance_sites:
