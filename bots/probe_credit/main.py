"""probe_credit — measure WHEN produced titanium is credited. EXPERIMENT ONLY.

Builds exactly ONE harvester and ONE dead-end conveyor next to it (facing
away from the core, output onto empty ground), then idles forever while the
core logs the team balance every round to stderr. Discriminates the three
crediting hypotheses in docs/open-questions.md: credited at harvester output,
credited when a stack exits the network, or only on core delivery. The
`probe_credit_nc` variant builds no conveyor at all (harvester should idle).

Balance slope pre-harvester is passive income only (+10/4 rounds); what the
slope does after setup, and what `*_titanium_collected` reports in the final
JSON, answer the question. Road-laying in _try_move is stripped so the network
contains exactly one conveyor.
...
class Player:
    def __init__(self):
        ...
        # Probe state: one harvester, one dead-end conveyor, then idle.
        self.harvester_pos: Position | None = None
        self.conv_pos: Position | None = None
        self.conveyor_done = False
    ...
    def _run_core(self, ct: Controller) -> None:
        ...
        # Probe: per-round balance tape on stderr; the deltas reveal exactly
        # when produced titanium is credited.
        import sys
        print(f"CREDIT r={ct.get_current_round()} ti={ct.get_global_resources()}", file=sys.stderr)
    ...
    def _run_builder(self, ct: Controller) -> None:
        """Probe state machine: build ONE harvester, then ONE dead-end
        conveyor next to it, then stop forever. See module docstring.
        """
        pos = ct.get_position()
        ...
        if self.conveyor_done:
            return

        # Stuck detection (feeds _move_toward_target's target refresh)
        ...
        if ct.get_action_cooldown() == 0:
            ...
            elif self.conv_pos is not None and pos.distance_squared(self.conv_pos) == 1:
                self._build_dead_end_conveyor(ct)

        if self.conveyor_done:
            return

        # Once the harvester exists, pick the conveyor tile and walk to it
        if self.harvester_pos is not None and self.conv_pos is None:
            self._pick_conveyor_spot(ct)
        if self.conv_pos is not None:
            # Walk to a STAGING tile (an orthogonal neighbour of the conveyor
            # spot), not at the spot itself — greedy pathing aimed at the spot
            # oscillates forever when the direct approach tiles are blocked.
            staging = self._staging_tile(ct)
            if staging is None:
                self.conveyor_done = True
                return
            self.target = staging
            # The forced target must survive _move_toward_target's stuck-based
            # re-pick, or the builder wanders off to ore mid-approach; the
            # four-direction fallback in _try_move still routes around blocks.
            self.stuck = 0

        if self.conv_pos is not None and ct.get_current_round() % 25 == 0:
            import sys
            print(
                f"CREDIT walk r={ct.get_current_round()} pos=({pos.x},{pos.y}) "
                f"conv=({self.conv_pos.x},{self.conv_pos.y}) "
                f"d2={pos.distance_squared(self.conv_pos)} acd={ct.get_action_cooldown()}",
                file=sys.stderr,
            )

        self._move_toward_target(ct)

    def _staging_tile(self, ct: Controller) -> Position | None:
        """Rotate through the conveyor spot's orthogonal neighbours (skipping
        the harvester) so a blocked approach eventually tries another side."""
        options = []
        for d in CARDINALS:
            s = self.conv_pos.add(d)
            if in_bounds(ct, s) and s != self.harvester_pos:
                options.append(s)
        if not options:
            return None
        self.staging_wait += 1
        if self.staging_wait > 40:
            self.staging_wait = 0
            self.staging_idx += 1
        return options[self.staging_idx % len(options)]

    def _pick_conveyor_spot(self, ct: Controller) -> None:
        """Choose the harvester's empty cardinal neighbour FARTHEST from the
        core. A conveyor there, facing away from the core, cannot point back
        at the harvester and outputs onto empty ground — a true dead end."""
        import sys
        best_c = None
        best_d = -1
        for d in CARDINALS:
            c = self.harvester_pos.add(d)
            if not in_bounds(ct, c) or not ct.is_tile_empty(c):
                continue
            dist = c.distance_squared(self.core_pos) if self.core_pos else 0
            if dist > best_d:
                best_d = dist
                best_c = c
        if best_c is None:
            print("CREDIT setup=NO_CONV_SPOT", file=sys.stderr)
            self.conveyor_done = True
        else:
            self.conv_pos = best_c

    def _build_dead_end_conveyor(self, ct: Controller) -> None:
        import sys
        if self.core_pos is not None:
            facing = nearest_cardinal(self.core_pos.direction_to(self.conv_pos))
        else:
            facing = Direction.NORTH
        if ct.can_build_conveyor(self.conv_pos, facing):
            ct.build_conveyor(self.conv_pos, facing)
            print(
                f"CREDIT setup=CONV_OK r={ct.get_current_round()} "
                f"conv=({self.conv_pos.x},{self.conv_pos.y}) facing={facing.name}",
                file=sys.stderr,
            )
            self.conveyor_done = True
        else:
            self.conv_attempts += 1
            print(f"CREDIT convfail n={self.conv_attempts} facing={facing.name}", file=sys.stderr)
            if self.conv_attempts >= 20:
                print(f"CREDIT setup=CONV_GAVE_UP at ({self.conv_pos.x},{self.conv_pos.y})", file=sys.stderr)
                self.conveyor_done = True
    ...
    def _try_build_harvester(self, ct: Controller) -> bool:
        ...
        for d in Direction:
            ...
            if ct.can_build_harvester(build_pos):
                ...
                self.harvester_pos = build_pos
                import sys
                print(
                    f"CREDIT setup=HARV_OK r={ct.get_current_round()} "
                    f"harv=({build_pos.x},{build_pos.y})",
                    file=sys.stderr,
                )
                self.target = None
                return True
    ...
    def _try_move(self, ct: Controller, d: Direction) -> bool:
        ...
        # Road-laying stripped for the probe: the network must contain EXACTLY
        # the one dead-end conveyor, or the measurement is contaminated.
        if ct.can_move(d):
            ct.move(d)
            return True
        return False
    ...
    def _pick_target(self, ct: Controller) -> Position:
        ...
        # 4. No known ore — head for the map centre first (deterministic and
        #    ore-rich on symmetric maps), then random-walk once we're there.
        #    With a single builder and no road-laying, pure random exploration
        #    sometimes never finds ore in 1000 rounds.
        w, h = ct.get_map_width(), ct.get_map_height()
        centre = Position(w // 2, h // 2)
        if pos.distance_squared(centre) > 8:
            return centre
