...
class Player:
    ...
    def _run_builder(self, ct: Controller) -> None:
        ...
        if ct.get_action_cooldown() == 0:
            if self.harvester_pos is None:
                ...
                if self.harvester_pos is not None:
                    # NC variant: no conveyor at all — the harvester has no
                    # adjacent building to output to and should idle.
        ...
        if self.conv_pos is not None:
            if pos == self.conv_pos and self.core_pos is not None:
                # Standing on the build tile — step off so we can build it
                self.target = self.core_pos
            else:
                self.target = self.conv_pos
