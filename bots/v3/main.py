...
class Player:
    ...
    def _run_core(self, ct: Controller) -> None:
        ...
        # Spawn on a random legal tile from the WHOLE 12-tile ring around the
        # 2x2 footprint. The old pos.add(d) scan reached only the N/W sides of
        # the ring, because get_position() is the footprint's NW corner — an
        # absolutely oriented candidate set that handed the whole game to one
        # seat on some maps (measured: mid20 went 0/32 for seat A, and became
        # fair the moment this was fixed). Random choice keeps it seat-neutral;
        # smarter placement is a later, real-map-pool decision.
