...
### v1.21 — s51, 2026-08-18→20. Instance: `docs/retro-research-s51-2026-08-20.md`. **FIRINGS: FIVE.**
**Q9's second-ever firing is the bump's incident: a correction derived from the same under-powered surface as the claim it corrected (gsxWins "field improvement", a one-day two-version cut) was consumed by the builder and stood ~2 days until this lane's own commissioned study refuted it.** Durable form: **when correcting your own inference, state both branches with n unless the correcting cut is BETTER-powered than the original error — a correction inherits the authority of care, which is exactly why an under-powered one is worse than the marked inference it replaces.**
**Q4 mechanical-rule lesson:** a thrice-recurring mechanical error (untracked watcher chaining) survived a rule that named the FAILURE; rewritten to name the only ALLOWED FORM (the watcher is always the sole content of its own background call). Prefer allowed-form rules over named-failure rules for mechanical habits.
**Q3b′ ran serially clean (every anchor opened, zero unverified relays)** — its first full-session validation since adoption. Consumption 100% with receipts; retractions reaching a lane 3→2; four declines upheld including two charter-boundary declines (bots/ writes, arena) that the builder discharged in minutes — the boundary cost nothing and kept the audit surface clean.

### v1.22 — s52, 2026-08-20→21. Instance: `docs/retro-research-s52-2026-08-21.md`. **FIRINGS: FIVE.**
**The bump's incidents, both mechanical-habit class:** (1) the v1.21 allowed-form rule was VIOLATED at first temptation (commit chained into the watcher's background call — succeeded, verified, logged; the named failure recurring is the rule's own proof); (2) **NEW RULE from a peer-caught incident: on a SHARED working tree, `git commit <paths>` always** — a bare commit takes the whole index and swept 19 of another lane's staged files. Both are allowed-form statements about shared surfaces — the v1.21 lesson generalising.
**The session's shape:** ten agent deliverables, zero unread, consumption measured in same-day BUILDS (one study became the day's ship); Q3c's refutable-premise form let agents kill five wrong inputs; the relay-compression failure moved into my own summaries ("silenced" for "misdirected") and was caught by my own commissioned autopsy — the correction-of-a-correction can be under-SPECIFIED, not only under-powered (Q9's family, new face). Bank-early briefing adopted as standard after machine-sleep kills.

### v1.23 — s53, 2026-08-21. Instance: `docs/retro-research-s53-2026-08-21.md`. **FIRINGS: FIVE.**
**The bump's incident, occurring TWICE in one day: a COMMISSIONING lane's framing entered my agent briefs and was wrong both times** — "v68 is a line under test" (it was the incumbent, 29-0, verified digit-for-digit before relay) and "O(1) is a fast riser" (a fallen 2152-peak bot recovering). Both were caught by verification, one BEFORE the brief shipped, one by the study itself. **NEW SUB-QUESTION Q3d — *did I verify the COMMISSIONING framing, not only my own premises?* Mechanical form: a relayed premise in a brief carries the same open-it-or-label-it duty as my own** — the s40 fresh-drafter rule protects against MY anchoring; nothing protected against the commissioner's until today.
**Second durable yield — the PRE-SPAWN PREMISE FIX:** when an upstream fact moves after a commission file is written but before its agent spawns, fixing the file costs a minute and beats every downstream correction; the running-consumer rule applies to briefs. (Worked instance: the KLADDEDOSE v2 amendment reached the DOORWAVE commission before its drafter read it.)
**Q3c's new face:** both brief errors were premises I generated from MY OWN session memory of my own reads — the compression risk includes self-memory as a source, not only agent output and relayed claims.
**Counts:** consumption 12/12, 0 unread; retractions reaching a lane 2 (flat vs s52), both corrected in-hour; self-vs-peer catches ~5:5 with the delegation mechanism responsible for the two that mattered most. Sunset clause NOT armed.
