...
| 6 | ⭐ **TURN ORDER IS ENTITY-ID ASCENDING, AND IDS COME FROM ONE GLOBAL CREATION COUNTER** (26,078 consecutive pairs, 0 inversions; 8,802 cross-team pairs, 0 inversions). With `launcher_id < victim_id`, **P(victim still on the landing tile ≥1 round) = 99.64%** (N=6,177) against **1.83%** otherwise. **48.79% of the enemy bots we currently throw are on the losing side of the comparison.** | `post-throw-tile-dwell-2026-08-09.md:404,437,454,496` |
...
**THE UNEXPLOITED HALF, and it is one line of code:** fact 6 — filtering to `victim_id < ct.get_id()` takes P(dwell>=1) from 1.83% to **99.64%**, and **48.79% of the bots we throw today are on the wrong side of it.** Ids are a global creation counter, so **a late-built launcher is favourably ordered by construction.** Not implemented anywhere in the tree.
...
### 5.5 ⭐ The immunity theorem — the hard ceiling on every mass-body assault
Builders may stand on their **own** core (`is_tile_passable` admits conveyor / splitter / allied core). Two healers on a 1x1 repair 8 HP/round for 2 Ti against two attackers' 4 dmg for 4 Ti; **net HP never falls, and since bodies cannot attack bodies the healers cannot be removed.** On the 2x2 core, **four healers = +16 HP/round against the theoretical maximum 16 dmg/round from all eight ring tiles = exact standoff; a fifth makes the core mathematically unkillable by builders.** **Only turrets and launchers break it.**
⇒ **This is the single most important number in the atlas for planning: any body-mass plan against a core has a proof of futility attached, and the exits are turret concentration (§4.4's 3+ inversion) and launcher eviction (§2.4).**
...
| **2** | **ID-ORDER THROW FILTER** (9.3) | 99.64% vs 1.83% dwell, N=6,177; ids are one global counter, 0 inversions in 26,078 pairs | every throw plank incl. shipped EXILE and the deferred AIMTHROW | **one integer comparison** | **PROBE only** — decodable off `throws.tsv` with no games at all |
