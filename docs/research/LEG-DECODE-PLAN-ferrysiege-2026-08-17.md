...
## D3 — SEAL GEOMETRY
Tile order actually executed (NEAR-first like Juusto, or spec order); time-to-4/6/8; 8/8 achieved? (local 0/13, best 7/8 — the leg's first binary question); far-pair latency (Juusto's stall point); diagonal timing vs the kill-window rule; corner-birth escape rate when orthogonals close (Juusto's defenders escaped via 41.4% corner births).
