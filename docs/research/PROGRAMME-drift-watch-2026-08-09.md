...
| D17 | **a genuinely NEW exploit CLASS built or fired without a Magnus→organisers norms question** — OR, inversely, per-instance exploit work STALLED waiting for approval it does not need. Magnus 2026-08-10 (CLAUDE.md point 0, `46e407a`): the exploit hunt is the standing brief; build and fire instances of an APPROVED class WITHOUT asking; ask the organisers only for a new CLASS, because a league can rule a class out of bounds. **Crash-induction: asked, APPROVED.** The watch flags both failure directions — an unasked new class (over-reach) and instance-work frozen on unneeded approval (under-reach). | Magnus point-0 directive; the class-vs-instance boundary that replaced the crash-induction norms hold |
