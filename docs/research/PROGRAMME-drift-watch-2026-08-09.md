...
⭐⭐ **(f) AN AUDIT RATIFIES WHAT IT DOES NOT TOUCH — IN BOTH DIRECTIONS.** *Added 2026-08-17 (s48). THE SIXTH CLAUSE CLEARS THE HIGHER BAR (e) SET, AND HERE IS WHY: it is the first clause that is TWO-SIDED, and its transmission path is SOCIAL rather than instrumental — it travels through what a correction MAKES ITS RECIPIENT BELIEVE, which no tool, rule or fix in (a)-(e) does.*
**⇒ THE CORRECTOR MUST NOT RATIFY BY OMISSION: correcting a detail implicitly endorses the premise, and THE MORE PRECISELY THE CORRECTION IS MADE, THE MORE CREDIBILITY IT LENDS.** **⇒ THE CORRECTED MUST NOT TREAT OMISSION AS RATIFICATION: an auditor's SILENCE on the premise is not evidence about the premise, and a claim that has just been audited is one its author is LESS likely to re-examine.**
**ORIGIN INSTANCE, s48, and both halves fired on the same number:** this lane corrected a `DEFENCE_ADMISSION` diagnostic's arithmetic (*"the largest un-excluded regression is −14.6pp, not the −13.1pp half-width you quoted"*) — **right, and at the wrong layer.** `population_diff` later showed the cut's populations were mismatched (opponent mix 0.55, opp-version 0.24), and the **opponent-matched repair REVERSED THE SIGN: −1.5pp became +2.1pp.** ⇒ **the two lanes jointly produced a very precise statement about a quantity that should not have existed in that form** — the auditor never questioned the premise, and the author *"read the audit as clearance and moved on."*
⭐ **MECHANISM, and it is why the skip happens at all (research's, and it is lane-symmetric): BOTH LANES REACH FOR THE MOVE THAT IS FAST AND UNAMBIGUOUSLY RIGHT — a cheap ARITHMETIC correction for the audit lane, a cheap DESCRIPTIVE cut for the research lane — AND BOTH SKIP THE PREMISE TO GET THERE.** **One mechanism, two lane-specific costumes; filing it as two separate risks would hide that.**
⇒ **CHECK FORM, one question each way. CORRECTOR: *before correcting a number's arithmetic, ask whether the number should exist at all.* CORRECTED: *when a correction lands on a detail, ask what was NOT corrected.*** ⚠ **Same shape as the absence rule: the auditor's silence FAILS SAFE FOR THE AUDITOR and LICENSES INACTION IN THE AUTHOR.**
