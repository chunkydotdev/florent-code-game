...
| **CPU denial** | **REOPEN on evidence** — the only statement of the refutation in the repo is one clause in a wrap, with no number, denominator, n, or script output; the 201,469 rows sit in an untracked scratch dir. **Separately, CPU-timeout *induction* is HELD ON NORMS, not evidence** — Magnus owes the organisers one question first. Do not merge the two. | ⭐⭐ **THE EVIDENCE THIS ROW ASKED FOR LANDED 2026-08-14 ~14:0xZ (research s40), off the REBUILT `econ.tsv` v2** (40,367 files, 0 decode errors; the old table was silently corrupt on recent decodes, which is why nobody could produce this cut before). **Per-team CPU headroom, band-cell maxima, joined via `meta_join`:** `Kvarnholmen` **p50 11,037 µs — over budget in the MEDIAN cell — 3,473 cells >10 ms, 2,816,302 TLE'd turns = 23.4% of all their unit-turns** · `Tyvrets` p95 11,063, 4.12% · `Ouroboros` p95 10,611, 2.08% · **`Erebus` p95 11,048, 1,052 cells >10 ms, 59,110 TLE'd turns = 0.77% — and Erebus is IN OUR ADMISSIBLE BAND at 1824** · `0033` p99 11,261, 0.0078%. **Control: ours is p95 8,847 / p99 9,411, 107 TLE'd turns of 23.3M (0.0005%) — the instrument plainly separates teams rather than returning a constant column.** ⇒ **The road is no longer 'one clause in a wrap with no number'. What the numbers support, and it is deliberately narrow: (i) the DEFENSIVE half is live and urgent — our own p99 sits at 94% of budget and `QUEUE #44` flags 87.6% ceiling usage on 30×30; (ii) several teams we actually play are ALREADY discarding turns without any help from us, which prices the INCIDENTAL half (entity-dense long games) as real rather than theoretical. ⛔ (iii) IT DOES NOT TOUCH THE NORMS QUESTION. Deliberate timeout-INDUCTION stays HELD pending Magnus's question to the organisers, exactly as this row says. Measuring headroom is not inducing exhaustion, and merging them is the error this row already warns about.**
