...
**TWO MORE TRAPS, FOUND IN SWEEP 24 (s32), both producing a false "not found" on a quote
you can SEE on the screen:**

1. **ZERO-WIDTH SPACES (U+200B).** Google-Docs-exported PDFs carry them around bullet
   glyphs. Just Woke Up's BC2025 unit list reads `Defense Tower ○​ Shoots enemies to earn
   money` with a ZWSP after the `○`. **Nothing in a terminal shows it and no amount of
   whitespace flattening removes it** — `tr -s ' \n\t\f\r'` does not touch U+200B.
   **Fix: quote a substring that avoids the glyph**, or add `| tr -d '\u200b'` to the
   flatten step.
2. **PAGE FURNITURE INJECTED MID-SENTENCE — the `\f` fix does not cover it.** Page
   numbers and figure captions land INSIDE sentences: *"having our starting 14 tower have
   a chance of not dying"*, *"if there were still 8 Figure 10: Tower marks enemy units
   around"*. **Fix: quote the two halves as separate spans and say in the file that you
   did. Never silently splice them** — a spliced quote is a fabricated one.
...
| 4 | CPU/time-limit exploitation — inducing opponent timeouts | **SWEPT.** Effect is real & tournament-deciding (StarCraft natural experiment). ⛔ **THIS ROW ASSERTED "BANNED BY NAME in BASIL and SC2 AI Arena" UNTIL 2026-08-10 (s28). THAT IS FALSE and its own files had retracted it TWICE.** Sweep 11 read the rule text: BASIL/SC2 AI Arena/AIIDE protect the **engine and the ladder**, not the opponent's bot. Sweep 21 then found the one league that *does* ban it: **Battlesnake, by name, on an INTENT test.** Net field position: **most comparable leagues do not ban it; exactly one does.** CPU-timeout induction remains **HELD ON NORMS** pending a Magnus→organisers ask — **note that hold is now the ONLY thing held: crash-induction was asked and is APPROVED** (`CLAUDE.md` D17), so do not read this row as covering it | 2026-08-09, corrected 2026-08-10 | [cpu-timeout-induction](cpu-timeout-induction.md), [no-league-bans-inducing-an-opponent-timeout](no-league-bans-inducing-an-opponent-timeout.md) |
...
| **22** | **THE OPPONENT IS NON-STATIONARY, AND EVERY EXPERIMENT WE RUN ASSUMES IT IS NOT.** Generated by a measured surprise, not by the wheel: one opponent had **13 versions** in our archive against one of ours, shipped **4 in the 4.5 h before our leg window**, and ran **67→57→67 in 66 minutes**; a cell-selection decision was computed on a statistic pooled across all thirteen, 60% of it from a version they no longer run. *(A) how did competitors handle benchmark opponents changing under them (B) paired/blocked/interleaved designs against live opponents (C) the POOLING TRAP — anyone who drew a wrong conclusion by pooling across opponent versions (D) did anyone EXPLOIT non-stationarity — timing challenges to a rival's deploy cadence, targeting a fresh version, reading a public version timeline as intelligence.* | **SWEPT** (s30). **20 files + summary. 185 strings self-reported (177 first-pass verbatim, 2 near-misses, 1 whole claim CUT); 40 re-grepped independently by the research arm → 38 verbatim, 2 curly-apostrophe corrections, 0 cut. 7 negative controls all 0, two of them adversarial near-miss strings.** **(D) IS REFUTED — the fresh version is STRONGER, not weaker.** **Produced a new library method trap** (two-column PDFs) and an in-repo measurement **verified by re-derivation, magnitude NOT confirmed** (see below). | 2026-08-11 | [sweep 22](2026-08-11-sweep-22.md), [block-on-opponent-version](block-on-opponent-version-not-opponent-id.md), [the-fresh-version-is-not-the-weak-version](the-fresh-version-is-not-the-weak-version.md), [the-pooled-opponent-statistic-pointed-at-the-wrong-opening](the-pooled-opponent-statistic-pointed-at-the-wrong-opening.md), [not-enough-recent-data-is-a-reason-to-spend-nothing](not-enough-recent-data-is-a-reason-to-spend-nothing.md), [our-shipped-bot-is-a-published-pure-policy](our-shipped-bot-is-a-published-pure-policy.md), [resubmit-as-reroll-is-closed-on-our-ladder](resubmit-as-reroll-is-closed-on-our-ladder.md), [the-rollback-signature](the-rollback-signature-marks-a-rivals-prototype-window.md), [the-ciao-matrix-instead-of-the-column-mean](the-ciao-matrix-instead-of-the-column-mean.md), [anchor-on-opponents-who-did-not-change](anchor-on-opponents-who-did-not-change.md) |
| **23** | **FORWARD DWELL AND THE WITHDRAWAL TRIGGER** — generated by a MEASUREMENT, not the wheel: our raiders spend **2.28× the rounds forward per structure** the top tier does and lose **4.57× the builders per forward build** (`QUEUE-forward-efficiency-2026-08-11.md`). *(A) what signal did competitors branch on to pull a forward unit out (B) timer vs event (C) what happened to an IDLE forward unit (D) where LONGER dwell was correct.* | **SWEPT** (s31). **13 files. 79 quoted spans, 79 verbatim (72 exact, 7 quote-char normalised), 0 unverified; 6 of the agent's OWN PARAPHRASES were found sitting inside quotation marks by its end-to-end audit and de-quoted; 1 claim CUT, 2 referent errors corrected. 4 negative controls all 0, two adversarial near-misses; positive control `enemy` 22/23.** **⭐ (A) IS A NEGATIVE THAT DECIDES THE PLANK: every shipped dwell limiter in the field is a RESUPPLY trigger, and our builder carries no consumable — global titanium, no ammo, healed by an adjacent builder. There is no rule-level reason for our raider to go home, ever. DO NOT PORT A WITHDRAWAL THRESHOLD.** (D) came back loaded — three teams weakened or deleted their withdrawal and won. **The field reframes our own metric as an IDLE-TIME/TARGETING defect, not a dwell defect.** ⚠ One repo-internal claim re-verified by the research arm before relay: the HP-delta arming primitive at `bots/_v135loki18/main.py:176-178` exists and IS core-only (`# --- Core-only accounting ---`, `:95`), so it is unavailable to the raider as the sweep states. | 2026-08-11 | [resupply-trigger](every-withdrawal-trigger-in-the-field-is-a-resupply-trigger.md), [withdraw-when-relieved](withdraw-when-relieved-or-when-the-job-became-impossible.md), [hp-delta-arming](arm-the-flee-on-an-hp-delta-and-disarm-it-on-absence.md), [excluded-from-go-home](the-forward-unit-is-excluded-from-the-go-home-branch.md), [three-teams-deleted-it](three-teams-deleted-the-withdrawal-and-it-won.md), [finish-the-task-first](finish-the-task-before-you-withdraw.md), [destination-clause](the-withdrawal-predicate-needs-a-destination-clause.md), [the-only-clock](the-only-clock-the-field-runs-forward-is-an-idleness-timer.md), [destination-not-recall](the-idle-forward-unit-gets-a-destination-not-a-recall.md), [dwell-is-the-payload](dwell-is-the-payload-when-the-forward-work-is-denial.md), [withdrawal-carries-the-report](the-withdrawal-carries-the-report.md), [convert-instead-of-withdrawing](convert-the-forward-unit-instead-of-withdrawing-it.md), [commit-but-work-en-route](commit-to-the-withdrawal-but-keep-working-en-route.md) |
| **24** | **DEFENCE THAT DOES NOT SLOW THE KILL** — a NEW row generated by a DOCTRINE CHANGE the same day: `PLAY_DEFENCE: never` became **`PLAY_DEFENCE: not_at_the_kill_s_expense`** + **`DEFENCE_ADMISSION_BAR: kill_round_non_regression`**. *(A) what a defensive purchase bought through a BOUNDED window and what it cost in tempo (B) did anyone MEASURE that a defensive addition did not slow their own attack (C) where the purchase cost the kill, and the TELL (D) investments that serve both roles.* | **SWEPT** (s32). **10 files + summary. 98 quoted spans, 96 machine-verified verbatim by literal `grep -F`, 2 by targeted inspection, 1 claim CUT; 9 near-misses corrected IN THIS SWEEP'S OWN DRAFT — 1 wrong-file attribution, 4 paraphrases inside quotation marks, 3 quote-char, 1 zero-width space. 8 negative controls all 0 (4 adversarial near-misses of real quotes); positive control `enemy` 644 hits / 35 of 41 files.** **⭐ (B) IS A HARD MEASURED NEGATIVE: A/B testing is ROUTINE in this field and EVERY reported outcome is a WIN RATE. 27 duration / time-to-win phrasings return ONE hit in 166,155 words — and that hit is about COWS.** Nobody has ever run our bar; there are no constants to port. **And the BC2025 CHAMPION's defensive A/B said NO (*"even in wins, or even slightly lost"*) against their own past bots, they shipped it on a POPULATION argument (*"our bot wasn't super aggressive"*), and it won them the tournament — so a kill-round bar read on `bots/*_probe` inherits exactly that defect.** (A) every window purchase in the field terminates on an EVENT, never a round number. (C) the tell was always an OPPONENT farming the reflex, never a statistic. (D) dual-purpose is solved in TIME, not placement. **Most buildable item is NOT new — it is [`the-rush-cost-budget-gate`](the-rush-cost-budget-gate.md), newly load-bearing: the only mechanism in the corpus that turns the admission bar from a POST-HOC BAR into a RUNTIME GATE.** ⚠ **FOUND** a misquote in [`2026-08-09-sweep-1.md`](2026-08-09-sweep-1.md) §3 (*"defensive units **to be** overproduced"* → source reads *"would be"*) and adds two method traps: **zero-width spaces** and **page furniture injected mid-sentence**. **⛔ THIS ROW SAID "ALSO CORRECTS" UNTIL 2026-08-11T19:3xZ AND THAT WAS FALSE — the file was never touched (unmodified since 08-09 07:34) and still carried the misquote for ten hours under a certification that it had been fixed. Caught by an ADVERSARIAL AUDIT of this lane, not by the sweep that claimed the fix, and not by this index. Actually corrected now. A believed-fixed defect is worse than a known one: nobody re-checks a row that says "corrected".** | 2026-08-11 | [sweep 24](2026-08-11-sweep-24.md), [nobody-measured-the-kill-round](nobody-in-the-field-has-ever-measured-the-kill-round.md), [name-what-the-safety-bought](static-defence-must-name-what-the-safety-bought.md), [event-not-a-round-number](the-window-purchase-terminates-on-an-event-not-a-round-number.md), [reprioritise-the-queue](reprioritise-the-existing-queue-instead-of-buying-a-defender.md), [reflex-outlived-its-window](the-defensive-reflex-that-outlived-its-window.md), [recall-deleted-for-costing-time](the-recall-to-defend-was-deleted-for-costing-time.md), [time-multiplexed-dual-role](the-dual-role-structure-is-time-multiplexed-and-it-names-its-cost.md), [consumable-vs-permanent](the-consumable-blocker-versus-the-permanent-one.md), [income-paying-defender](the-defender-that-pays-income-is-an-affordance-we-do-not-have.md), [rebuild-loop-substitutes](a-cheap-rebuild-loop-substitutes-for-defence.md), [the-mirror-argument](the-mirror-argument-and-the-one-thing-it-cannot-mirror.md) |
| **25** | **THE THIRTEEN-ROUND RACE** — a NEW row generated by a MEASUREMENT, not the wheel: our **median kill round is 174, our median death round is 187**, and our core dies in **46.3% of all games**. We are losing a photo finish, roughly half the time, to opponents doing the same thing to us at the same moment. *(A) who diagnosed a close race and what did they CHANGE in code (B) ⭐ TEMPO vs ROBUSTNESS at the margin — arrive sooner, or survive the interception (C) the VARIANCE argument: did anyone reason about the DISTRIBUTION of their kill time rather than its mean (D) interception — what did one failed push cost, and what made a push RESUMABLE.* | **SWEPT** (s32). **11 files + summary. 51 quoted spans, 51 machine-verified verbatim by literal `grep -F` on the first pass, 0 unverified, 0 cut. 10 negative controls all 0, NINE of them adversarial near-misses of this sweep's own real quotes; positive control `enemy` 911 hits / 49 of 65 files, 222,488 words.** **1 near-miss corrected and it was MINE, in a new place: I drafted the Jay Scott turtle post's URL as `archives/117-static-defense-is-bad.html` FROM MEMORY — no such page; the real permalink is `archives/353-turtle-strategies.html`, titled *"turtle strategies"*. The quote was verbatim and the CITATION was invented — the index's second rule arriving in the `source:` field.** **⭐ (B) IS A HARD NEGATIVE PLUS A DISSOLUTION.** Negative: 29 of 32 duration/tempo-trade phrasings return **0** in 222,488 words (`kill round` 0 · `time to win` 0 · `tempo cost` 0 · `delaying the attack` 0); the lone `game duration` hit is a **simulation horizon** inside a CodinGame search, reported in-file so it cannot be reconstructed as a counterexample. Dissolution: only **4 spans in the whole corpus** treat delay as a quantity to protect (`critical path` 1, `no delay` 1, `without delaying` 2) — **all four are Jay Scott, one three-post series, one build order** — and his answer is NEITHER faster NOR harder-to-stop: **fund the robustness out of INCOME, never off the CRITICAL PATH**, which is `DEFENCE_ADMISSION_BAR` as a **construction rule** instead of a post-hoc bar. **And the BC2025 CHAMPION shipped a tempo change and a robustness change TWO SENTENCES APART under one pooled win rate, owning an automated A/B harness they used elsewhere.** (A) smite BC2019 shipped a **team-wide one-turn spending freeze** so the shared bank could afford the racing unit, and reported the result **as an arrival round** (*"turn 4-6"*) — we have six titanium FLOORS and no HALT. (C) exactly **one** span reasons about the spread of one's own arrival time and it is an **explicitly unbuilt proposal** resting on cross-game persistence our sandbox forbids; the one team that shipped a mean-for-variance trade (**Oak BC2019**) **LOST and diagnosed why themselves** — the population moved, which is sweep 22's non-stationarity with a tournament exit attached. (D) **no figure for "one failed push = N rounds" exists anywhere in the field.** ⚠ **REAL HOLE: Liquipedia `Timing Attack`/`All-in`/`Tempo` were NOT read** — Cloudflare rate-limit pages instructing agents to halt; the agent complied. That is exactly where a counterexample to (B)'s negative would live. **Most buildable item: [`rebuild-on-zero-needs-a-counter-that-can-reach-zero`](rebuild-on-zero-needs-a-counter-that-can-reach-zero.md) — om nom's liveness invariant, and OUR TREE HAS THE BUG IN ITS THIRD INSTANCE: `SLOT_FWD_GUN` fixed by LOKI-2b, `SLOT_LAUNCHER` fixed by LOKI-6, and `SLOT_HARVESTERS` still a monotone high-water mark (`doctrine.py:381`) that can never reach zero while `raid.py:409` gates the whole siege on it. `eco.py:381` already computes the live value and discards it. NO NEW STORE SLOT NEEDED.** **⛔ SIZED AND PARTLY RETRACTED BY THE RESEARCH ARM THE SAME EVENING, BEFORE IT REACHED THE QUEUE. The DEFECT is real and line-exact. The IMPACT is not what this row implies, and the FIX does not work:** (1) **exposure is an upper bound of 5.24%** — the costly branch (`harv < _eco_cap` blocks rebuilds and the mark cannot fall) needs `ECO_CAP = 18`, and over **our own 5,398 side-games** (`meta_join.us_side` × `build_agg`, **us-only, NOT field-pooled**) the median is **5** harvesters built, p90 **13**, and only **283 side-games (5.24%)** ever reach 18; it further requires that they then died and we wanted to rebuild. (2) **"NO NEW STORE SLOT NEEDED" is true but misleading, because the proposed substitute is differently scoped:** `live` at `eco.py:375` comes from `get_nearby_buildings()` with **no `dist_sq`, i.e. THIS UNIT'S VISION (builder r²=20)**, while the store mark is **GLOBAL** — so a builder that has walked away from the harvester field reads `live = 0` and fires a **spurious rebuild**. Swapping a vision-scoped count for a global one is not a fix. **NOT QUEUED at the size this row implies; ranked below #16 and #8.** | 2026-08-11 | [sweep 25](2026-08-11-sweep-25.md), [buy-the-escort-out-of-income](buy-the-escort-out-of-income-not-off-the-critical-path.md), [nobody-separated-tempo-from-robustness](nobody-separated-tempo-from-robustness.md), [halt-the-other-spenders](halt-the-other-spenders-so-the-racing-unit-lands-early.md), [rebuild-on-zero](rebuild-on-zero-needs-a-counter-that-can-reach-zero.md), [champions-margin-patch](the-champions-margin-patch-bought-both-and-measured-neither.md), [failed-push-leaves-a-crater](the-failed-push-still-leaves-a-crater.md), [time-the-defensive-purchase](time-the-defensive-purchase-to-their-measured-attack.md), [converting-a-won-race-into-safety](converting-a-won-race-into-safety-threw-the-game.md), [losses-land-against-opponents-you-beat](your-losses-land-against-opponents-you-usually-beat.md), [the-consistency-choice-that-lost](the-consistency-choice-that-lost-the-tournament.md), [mean-versus-variance-never-built](the-only-mean-versus-variance-proposal-was-never-built.md) |

### ⚠ SWEEP 22's IN-REPO MEASUREMENT — VERIFIED IN SIGN, **NOT IN MAGNITUDE** (research arm, same session)

**The sweep was briefed as an external-literature sweep and it also computed statistics
on our own corpus.** That is scope the brief did not authorise, and it means the headline
number arrived with no second pair of eyes. **Re-derived independently, without reading
`tools/corpus/version_drift.py`:**

| | agent | independent re-derivation |
|---|---:|---:|
| headline (opponent's later versions vs earlier, our bot frozen) | **−8.00pp**, t = −15.81, 1,970 blocks | **−6.55pp**, t = −15.63, **4,157 blocks** |
| coinflip split (must be null) | null, 3 seeds | null, 3 seeds (t = −0.03 / +1.62 / −0.64) |
| **discriminating control** (opponent version CONSTANT, split by time) | −0.0023, **t = −0.61** | −0.0049, **t = −1.51** |

**Four defensible estimators of the same quantity, on the same blocks:**
`first-vs-last −8.75` · `median split −6.55` · `size-weighted −8.43` · `rank slope −4.83`
— **all t < −13, all negative, spanning 1.8x.**

**⇒ THE DIRECTION AND THE SIGNIFICANCE REPLICATE AND ARE SAFE TO BUILD ON. THE MAGNITUDE
IS ESTIMATOR-DEPENDENT AND MUST NOT BE QUOTED TO TWO DECIMALS** — and the derived
*"2.6 rating points per match"* inherits the whole spread (~1.6 to ~2.9). **The
discriminating control is also less clean in my hands (t = −1.51 vs −0.61); still not
significant, but it is not the "dead confound" the file implies.** This is the repo's own
standing lesson arriving in a sweep: *a bar names its estimator, or it can be met and
missed by choosing one afterwards.*

**What survives unqualified, and it is the part that matters:** pooling a per-opponent
statistic across an opponent's versions **OVERSTATES** our expected game share against the
version we will actually face. Every per-opponent gate, panel cell and target-band
estimate in this repo currently reads **high**. That was found independently the same
morning on one cell (`GATE-5abis-loki19-arrival-2026-08-11.md` §4); sweep 22 generalises
it league-wide over 4,157 blocks.
