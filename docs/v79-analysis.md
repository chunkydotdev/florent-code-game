...
| **B0b** | **Do not medic/escort a building under turret fire.** `MEDIC_*` was sized against 2 dmg/rd melee; a gunner is ~~~3.5/rd~~ **7/rd** and a sentinel ~9/rd. **[CORRECTED 2026-08-09, research arm, s24: the gunner figure was 2× low. `docs/reference/official-docs.md:257` states it outright — *"18 every 2 rounds against 7 every round"* — so `reload N` means "fires once every N rounds", making a gunner 7/rd (not 3.5) and a sentinel 9/rd (which was right). CHECKED IN THE BOT BEFORE FLAGGING: this stale figure sized NOTHING shipped. `MEDIC_TI_FLOOR=20`, `MEDIC_MIN_RND=150`, `MEDIC_EARLY_MIN_DMG=8` and `MEDIC_TYPES` carry no damage-rate at all, and the heal-seat arithmetic at `_v100hf/main.py:550-560` uses a *measured* siege DPS from the decode corpora (max 23.22), not this number. **The error is latent, not live — it would have become live the moment anyone built B0b from this line.** It doubles the healer count B0b implies: one healer at 4 HP/rd does NOT out-heal a gunner, two do (8 HP/rd vs 7).]** Add "no enemy GUNNER/SENTINEL in vision with the target in its attackable tiles" to the medic and escort predicates, and prefer `destroy()` + relocate the link. | meander (9,11): 905 Ti, 453 rounds, never delivered | ~20 | **Medium.** `destroy()` is free and cooldown-free, but relocating a link means recomputing `_link_path`, which is the one place chains break (C2). Consider shipping the "don't heal it" half alone first. |
