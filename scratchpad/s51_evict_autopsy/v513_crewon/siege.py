...
class SiegeMixin:
    ...
    def _fs_turn(self, ct):
        ...
        if at_ring:
            ...
            # s51 EVICT AUTOPSY -- PRINT ONLY, no behaviour.  Emitted AFTER the
            # action so a CPU timeout inside the instrument cannot cost the
            # body its turn (a TLE interrupts the turn; it does not kill).
            self._fs_dlog(ct, E, p, rnd, needed, orth_open)
        else:
            self._fs_ferry_turn(ct, E, p, rnd)

    def _fs_dlog(self, ct, E, p, rnd, needed, orth_open):
        """s51 autopsy instrument.  STDERR only, never mutates.

        One line per at-ring body per round:
          DL <rnd> id <id> role <seal|supp> orth <open orth seats>
             need <len(needed)> ebody <enemy bodies on OPEN orth seats>
             lau <our live ring launchers> ti <bank> lcost <launcher cost>
             bar <barrier cost> obs <fs_healer_obs> hist <distinct healer tiles>
             pend <1 if _fs_seal_pending else 0>
        """
        if not FS_LOG:
            return
        try:
            ...
            ebody = 0
            for s in seats:
                if self._fs_denied(ct, s):
                    continue
                try:
                    oid = ct.get_tile_builder_bot_id(s)
                except Exception:
                    continue
                if oid is not None and ct.get_team(oid) != self.team:
                    ebody += 1
            ti = ct.get_global_resources()
            lau = self._fs_live_evictors(ct, E)
            try:
                pend = 1 if self._fs_seal_pending(ct, needed, ti) else 0
            except Exception:
                pend = -1
            self._fs_log("DL", rnd, "id", ct.get_id(),
                         "role", self.fs_role,
                         "orth", orth_open,
                         "need", len(needed or ()),
                         "ebody", ebody,
                         ...
                         "obs", self.fs_healer_obs,
                         "hist", len(self.fs_healer_hist),
                         "pend", pend)
        except Exception:
            return
