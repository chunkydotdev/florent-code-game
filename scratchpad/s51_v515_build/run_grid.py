#!/usr/bin/env python3
"""s51 v515 grid runner (from the s51 v514 build, NOREPLAY option added).  Paired maps x reps vs a fixed opponent, local --tle 10.
...
MAPS = (os.environ.get("MAPS") or
        "glacierkeep,nordkap,atoll,midgard,drakkarfjord").split(",")
...
def one(job):
    ...
    rp = (os.path.join(repdir, tag + ".replay26")
          if (repdir and not os.environ.get("NOREPLAY")) else None)
