...
class Player(EcoMixin, RaidMixin, SiegeMixin):
    ...
    def _core(self, ct):
        ...
        # === PROBE: does get_hp(id) work beyond the caller's vision? ========
        # Reads the ferry-siege raider id off SLOT_FS (published by the raider
        # itself), which sits at the ENEMY ring -- far outside the Core's
        # r^2=36 vision.  Prints the raw answer and the distance so the two
        # cases (in / out of vision) are separable, plus an explicit DEAD probe
        # on an id that has been destroyed.
        try:
            import sys as _sys
            _v = ct.read_store(SLOT_FS)
            _rid = (_v >> 14) - 1
            if _rid >= 0:
                _dsq = -1
                _inv = -1
                ...
                _exc = ""
                try:
                    _pp = ct.get_position(_rid)
                    _dsq = _pp.distance_squared(p)
                    _inv = 1 if ct.is_in_vision(_pp) else 0
                except Exception as _e:
                    _exc = "POS:" + type(_e).__name__
                try:
                    _hp = ct.get_hp(_rid)
                except Exception as _e:
                    _exc += "|HP:" + type(_e).__name__
                if not hasattr(self, "_probe_seen"):
                    self._probe_seen = {}
                self._probe_seen[_rid] = rnd
                print("PROBEHP", rnd, "rid", _rid, "dsq", _dsq,
                      "invision", _inv, "hp", _hp, "exc", _exc,
                      file=_sys.stderr)
            # DEAD control: every id we have ever probed that is no longer the
            # published one gets re-probed, so a destroyed entity's answer is
            # on the same tape as a live one's.
            for _old in list(getattr(self, "_probe_seen", {}).keys()):
                if _old == _rid:
                    continue
                _hp2 = None
                _exc2 = ""
                try:
                    ...
                    _exc2 = type(_e).__name__
                print("PROBEDEAD", rnd, "rid", _old, "hp", _hp2,
                      "exc", _exc2, file=_sys.stderr)
        except Exception:
            pass
        # === /PROBE ========================================================
        ...
        if not endgame_dumped:
            ...
            if fs_live:
                ...
                try:
                    ...
                    bar, sen = 3, 30
                if fs_ph == FS_PH_KILL or (fs_ph == FS_PH_KILL_OPEN
                                           and LOKI_FS_V514
                                           and FS_V514_MAGGATE):
