...
body saw, not the arm's.  The required count for the v517 flags is ZERO.
...
GUARD_POS = "LOKI_FS_V517 = True\nX = LOKI_FS_V517 and 3\n"
GUARD_NEG = "LOKI_FS_V517 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V517 = True\nif LOKI_FS_V517:\n    X = 3\n"


def main():
    path = sys.argv[1] if len(sys.argv) > 1 else \
        "bots/_v517twin/doctrine.py"
    ok = True
    if not scan(GUARD_POS, V517):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V517):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V517):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V517)), bool(scan(GUARD_NEG, V517)),
           bool(scan(GUARD_IF, V517))))
    src = open(path).read()
    h517 = scan(src, V517)
    hcrew = scan(src, CREW)
    print("v517 derived defaults: %d %s" % (len(h517), h517))
