...
class SiegeMixin:
    ...
    def _v518_early_sentinel(self, ct, E, p, rnd, needed, ti, orth_open):
        ...
        and the clause is shut for the rest of the match.  Its total cost to
        the collar is AT MOST ONE BARRIER-ROUND per game, against the ~81
        rounds it is trying to buy back.  v510's failure was five verbs
        competing every round for ever; this is one verb, once.
    ...
    def _fs_ladder_turn(self, ct, E, p, rnd, needed, orth_open):
        ...
        if ct.get_action_cooldown() == 0:
            # ⭐⭐ v518 CHANGE 2(a) -- RUNG 1', THE FIRST FORWARD SENTINEL JUMPS
            # THE COLLAR.  See `_v518_early_sentinel` for the argument and for
            # the flagged collision with Magnus's priority ruling 1.
            if self._v518_early_sentinel(ct, E, p, rnd, needed, ti, orth_open):
                return
            # rung 1 -- BARRIER.
            if FS_SEAL_ON and self._fs_try_seal(ct, E, p, needed, ti):
                return
