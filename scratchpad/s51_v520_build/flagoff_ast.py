...
GUARD_POS = "LOKI_FS_V520 = True\nX = LOKI_FS_V520 and 3\n"
GUARD_NEG = "LOKI_FS_V520 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V520 = True\nif LOKI_FS_V520:\n    X = 3\n"


def main():
    path = sys.argv[1] if len(sys.argv) > 1 else \
        "bots/_v520pincer/doctrine.py"
    ok = True
    if not scan(GUARD_POS, V520):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V520):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V520):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V520)), bool(scan(GUARD_NEG, V520)),
           bool(scan(GUARD_IF, V520))))
    src = open(path).read()
    h520 = scan(src, V520)
    h519 = scan(src, V519)
    h518 = scan(src, V518)
    hcrew = scan(src, CREW)
    print("v520 derived defaults: %d %s" % (len(h520), h520))
