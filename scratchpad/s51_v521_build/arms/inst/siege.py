...
class SiegeMixin:
    ...
    def _fs_ladder_turn(self, ct, E, p, rnd, needed, orth_open):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            #
            # ⭐⭐ v521 1a, SECOND HALF -- UNDER *NEAR*, RUNGS 2 AND 4 STAND
            # DOWN AND RUNG 3 DOES NOT.  Rung 1 did not fire, which means the
            # last seats are not placeable THIS round (unaffordable, blocked,
            # or out of reach).  The parent would now drop to rungs 2-4 and
            # spend the round somewhere else; under NEAR the body takes only
            # the one lower rung that is ITSELF a seat-closing act, then falls
            # through to the walk, which `_fs_stand_target` already aims at the
            # nearest remaining seat.
            #
            # ⛔⛔ RUNG 3 IS KEPT BECAUSE THE FIRST BUILD OF THIS CLAUSE COULD
            # LATCH FOREVER, AND THE SMOKE TAPE IS WHAT SHOWED IT.  Suppressing
            # every lower rung means a seat held by an ENEMY BUILDING is never
            # pecked off, `orth_open` never leaves 1, and NEAR holds for the
            # rest of the match with the body doing nothing but walking -- the
            # nordkap smoke read 170 NEAR rounds against 35 rung-1 fires before
            # this correction.  Rung 3's own comment already says what it is:
            # "Rung 1 barriers it the same or the next round".  It is the
            # collar's work, at 2 Ti a peck, so it is admitted under NEAR even
            # while the seal WAIT is holding the bank -- the WAIT exists to
            # stop a BUILD spending the collar's money, and a peck is not one.
            if _v521_near:
                if self._fs_try_clear(ct, E, p, needed):
                    self._fs_rung(ct, rnd, 3, E, p, needed, ti, orth_open)
                    self._v521_rung(ct, rnd, 3, _v521_st)
                    return
            elif not self._fs_seal_pending(ct, needed, ti):
                ...
                # ⭐ v521 1b KEEPS THIS RUNG UNDER *HOLD*, DELIBERATELY.  A
                # SECOND aligned sentinel raises the damage rate INSIDE the
                # overlap window, which is the quantity this build is buying,
                # and v517's twin plank already prices it against the collar's
                # own repair allowance.  Suppressing it would be optimising the
                # window's LENGTH against its VALUE.
