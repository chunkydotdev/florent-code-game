#!/bin/bash
# v522 HEADLINE: interleaved CONCURRENT blocks, THREE arms on the SAME seeds.
...
#   v522     -- `bots/_v522floor` as fired.
#   flagoff  -- `LOKI_FS_V522 = False`, the KNOWN-ZERO control, proved
...
for i in $(seq 1 "$BLOCKS"); do
  ...
  P1=$!
  .venv/bin/python "$B/run_grid.py" bots/_v522floor \
      "$B/grid/b$i/v522.tsv" "$B/grid/b$i/repv522" "$B/grid/b$i/logv522" \
      > "$B/grid/b$i/v522.log" 2>&1 &
  P2=$!
