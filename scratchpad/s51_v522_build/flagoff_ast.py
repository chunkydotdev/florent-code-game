...
GUARD_POS = "LOKI_FS_V522 = True\nX = LOKI_FS_V522 and 3\n"
GUARD_NEG = "LOKI_FS_V522 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V522 = True\nif LOKI_FS_V522:\n    X = 3\n"


def main():
    path = sys.argv[1] if len(sys.argv) > 1 else \
        "bots/_v522floor/doctrine.py"
    ok = True
    if not scan(GUARD_POS, V522):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V522):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V522):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V522)), bool(scan(GUARD_NEG, V522)),
           bool(scan(GUARD_IF, V522))))
    src = open(path).read()
    h522 = scan(src, V522)
    h521 = scan(src, V521)
    h520 = scan(src, V520)
    ...
    h518 = scan(src, V518)
    hcrew = scan(src, CREW)
    print("v522 derived defaults: %d %s" % (len(h522), h522))
    ...
    print("REAL-CASE CONTROL (FS_CREW_ON readers, the known v515 hazard): "
          "%d %s" % (len(hcrew), hcrew))
    if h522 or h521 or h520 or h519 or h518:
        ok = False
    if not hcrew:
