...
class SiegeMixin:
    ...
    def _v520_arc_at(self, ct, slot):
        ...
        try:
            ...
            return FS_V520_ARC_NONE
        arc = (v >> FS_V520_ARC_SHIFT) & FS_V520_ARC_MASK
        # ⛔⛔ v523 -- THE DECONFLICTION READ MUST STILL GET AN IDENTITY.
        # This function's ONE caller class is `_v520_claim_arc`, which uses the
        # peer's arc to detect a collision and to yield.  v523 overloads the
        # same two bits with a CLOSED code, so without this line a peer that
        # closed its arc would read as `3` -- matching neither FRONT nor BACK
        # -- and the collision detector, and the DUP alarm that must be zero,
        # would both go silent for exactly as long as the peer was doing well.
        # An alarm that stops firing when the plank starts working is worse
        # than no alarm.
        if arc == FS_V523_ARC_CLOSED and LOKI_FS_V523 and FS_V523_SALTEYES \
                and FS_V523_ARC_UNION:
            return self._v523_peer_arc(arc)
        return arc
    ...
    def _fs_publish(self, ct, rnd, phase, rid):
        ...
        if LOKI_FS_V520 and FS_V520_PINCER and FS_V520_ARC_PUBLISH:
            ...
            _arc = getattr(self, "v520_arc", FS_V520_ARC_NONE)
            # ⭐⭐ v523 CHANGE 1 -- THE ARC-CLOSED CODE RIDES THE ARC FIELD.
            # No new slot, no new writer, no new bit: FS_V523_ARC_CLOSED = 3 is
            # the only unused encoding left anywhere in the word (beat 0-10,
            # phase 11-13 with all eight codes taken, rid 14-29, arc 30-31).
            # It is published ONLY by a body that has already claimed an arc,
            # which is what makes it resolvable by a peer -- see
            # `_v523_peer_arc`.
            if (LOKI_FS_V523 and FS_V523_SALTEYES and FS_V523_ARC_UNION
                    and not FS_V523_PROBE_NOARC
                    and getattr(self, "v523_arc_closed", False)
                    and _arc != FS_V520_ARC_NONE):
                _arc = FS_V523_ARC_CLOSED
                self.v523_arc_pub += 1
            word |= ((_arc & FS_V520_ARC_MASK) << FS_V520_ARC_SHIFT)
        try:
    ...
    def _fs_map_gated(self, mw, mh, ours, E, ct=None):
        ...
        if ok and LOKI_FS_V519 and FS_V519_MODESWITCH:
            ...
            if sig519 in FS_V519_CRIPPLE_MAPS:
                ok = False
    ...
    def _fs_census(self, ct, E):
        ...
        for t in ring:
            ...
            if is_seat and not covered:
                orth_open += 1
                # ⭐ v523 -- THE SEATS, NOT ONLY THEIR COUNT.  `orth_open` is a
                # scalar and the arc merge has to know WHICH seats are still
                # open before it can ask whether they are all on the peer's
                # half.  Recorded as an attribute rather than added to the
                # return tuple: `_fs_census`'s signature is read at six sites
                # and none of the others wants this.
                # ⛔ UNCONDITIONAL, i.e. NOT gated on LOKI_FS_V523.  It is a
                # pure assignment to an attribute nothing else reads, it costs
                # one list append per open seat, and gating it would make the
                # flag-off tree take a different code path through the hot
                # loop for no behavioural reason.  The BYTE-IDENTITY test is
                # what proves that claim rather than this comment.
                _v523_open.append((t.x, t.y))
        ...
        self.v523_open_seats = tuple(_v523_open)
    ...
    def _fs_turn(self, ct):
        ...
        if at_ring:
            ...
            # ⭐⭐⭐ v523 CHANGE 1, THE PUBLISH SIDE.  Both of these are settled
            # BEFORE `_fs_publish` runs at the bottom of this function, because
            # the arc-closed bit and the merged phase are what that call
            # stamps.  `v523_merged` is cached for the round so the SAME
            # verdict reaches the phase channel and `_fs_salt_ok` -- two reads
            # of one predicate in one round is how the parent's salt latch grew
            # its "which rung happened to ask it" defect (v513 change A).
            self.v523_arc_closed = self._v523_my_arc_closed(ct, E, orth_open)
            self.v523_merged_rnd = rnd
            self.v523_merged = (
                self._v523_ring_sealed(ct, rnd, E, orth_open)
                if self._v523_on() else (orth_open == 0))
            ...
            if orth_open == 0 or (self._v523_on() and FS_V523_PHASE_SEALED
                                  and self.v523_merged):
                ...
                # ⭐ v523: ...and "reads closed" is now the CREW's reading, not
                # this body's.  The latch is the parent's; only the predicate
            ...
            if self._fs_live_sentinels(ct, E):
                ...
                phase = FS_PH_SEALED
            elif (self._v523_on() and FS_V523_PHASE_SEALED
                    and self.v523_merged):
                # ⭐⭐⭐ v523 CHANGE 1, AND THIS IS THE LINE THE SEALED-RATE
                # MOVES ON.  My own eyes see an open seat; the crew's merged
                # census does not.  Publishing the merged verdict is what
                # carries the fix to every consumer that reads the CHANNEL
                # rather than calling the predicate -- the Core's magazine, the
                # spawn reserve, the budget door -- and those are most of them.
                # ⛔ NOT A NEW CODE.  It publishes FS_PH_SEALED, which already
                # means exactly this ("every orthogonal heal seat is denied"),
                # and the only thing that changed is WHOSE EYES ESTABLISHED IT.
                phase = FS_PH_SEALED
            else:
                phase = FS_PH_RING
        else:
        ...
        if FS_V517_TWIN_LOG:
            ...
            try:
                ...
                pass
        # ⭐⭐ v523 -- THE SEALED-RATE INSTRUMENT, AND IT IS GATED ON ITSELF
        # RATHER THAN ON THE MASTER (v521 surprise 7).  With LOKI_FS_V523 False
        # this line still emits one PH523 record per at-ring round, so the
        # master-off arm's ZERO in the `merged`/`arcpub` columns has a VISIBLE
        # DENOMINATOR and means something.  A zero with no denominator
        # validates anything.
        if FS_V523_SALT_LOG and at_ring:
            try:
                print("PH523", rnd, "id", ct.get_id(),
                      "body", getattr(self, "fs_body", 1),
                      "arc", getattr(self, "v520_arc", 0),
                      "orth", orth_open,
                      "own", 1 if orth_open == 0 else 0,
                      "merged", 1 if getattr(self, "v523_merged", False) else 0,
                      "arcclosed", 1 if getattr(self, "v523_arc_closed",
                                                False) else 0,
                      "ph", phase, file=sys.stderr)
            except Exception:
                pass
    ...
    # ==================================================================
    # v523 -- THE BODY-2 BLINDNESS TRIO
    # ==================================================================

    def _v523_on(self):
        """The master read, at RUN time.  One expression, one place."""
        return LOKI_FS_V523 and FS_V523_SALTEYES

    def _v523_inert(self):
        """⛔ THE VERIFICATION SHORT-CIRCUIT, AND IT IS DELIBERATELY SEPARATE
        FROM THE MASTER.  Under `FS_V523_STORE_ONLY` every v523 word is still
        PUBLISHED and no consumer acts on it, which is the same-store-write
        baseline the v522 instrument finding makes mandatory: `.replay26`
        serialises the private comms store, so a dose measured against a plain
        baseline scores a full dose for a clause that plays an identical game.
        """
        return FS_V523_STORE_ONLY

    # --- change 1: SALTEYES ------------------------------------------------

    def _v523_arc_raw(self, ct, slot):
        """The raw 2-bit arc field on `slot`, ARC_CLOSED included.

        ⛔ NOT `_v520_arc_at`, and the difference is the whole point of the
        channel: `_v520_arc_at` is the DECONFLICTION read and must keep
        returning an arc IDENTITY, so it maps the closed code back onto an
        identity (below).  This one returns what is actually in the bits.
        """
        if not (LOKI_FS_V520 and FS_V520_PINCER and FS_V520_ARC_PUBLISH):
            return FS_V520_ARC_NONE
        try:
    ...
    def _v523_peer_arc(self, code):
        """Resolve a peer's published arc code to an arc IDENTITY, from mine.

        ⛔ THE ONE INFERENCE IN THIS BUILD, AND ITS PRECONDITION IS STATED
        RATHER THAN ASSUMED.  A body only ever publishes ARC_CLOSED after it
        has CLAIMED an arc; the claim is sticky for the body's life
        (`_v520_claim_arc` was made sticky in v520 precisely because a
        re-derived claim thrashed); and the two bodies are deconflicted onto
        DIFFERENT arcs.  So a reader that holds an arc knows a peer publishing
        ARC_CLOSED holds the other one.

        ⛔ A READER THAT HOLDS NO ARC CANNOT RESOLVE IT AND RETURNS NONE.  That
        is a real blind spot -- the Core is exactly such a reader -- and it is
        why change 2 merges PHASES (which need no such inference) while change
        1 merges ARCS on the raider, where the arc is known.
        """
        if code != FS_V523_ARC_CLOSED:
            return code
        mine = getattr(self, "v520_arc", FS_V520_ARC_NONE)
        if mine == FS_V520_ARC_FRONT:
            return FS_V520_ARC_BACK
        if mine == FS_V520_ARC_BACK:
            return FS_V520_ARC_FRONT
        return FS_V520_ARC_NONE

    def _v523_my_arc_closed(self, ct, E, orth_open):
        """Is every OPEN seat on somebody ELSE's arc?  (i.e. is MY arc done?)

        Returns False whenever the answer is not well defined -- the ring is
        already closed (nothing to say), I hold no arc, the plank is off, or
        the census recorded nothing.  ⛔ NEVER True when `orth_open == 0`: the
        phase machine publishes FS_PH_SEALED in that round and a full-ring
        SEALED is strictly more informative than an arc-closed code.
        """
        if not (self._v523_on() and FS_V523_ARC_UNION):
            return False
        if not orth_open:
            return False
        mine = getattr(self, "v520_arc", FS_V520_ARC_NONE)
        if mine == FS_V520_ARC_NONE:
            return False
        seats = getattr(self, "v523_open_seats", ())
        if not seats:
            return False
        for sx, sy in seats:
            try:
                if self._v520_arc_of(Position(sx, sy), E) == mine:
                    return False
            except Exception:
                return False              # unresolvable: claim nothing
        return True

    def _v523_arc_attest(self, ct, rnd):
        """Which arcs a FRESH peer word attests as closed, as a set.

        Three ways a slot can attest, and all three are read off channels that
        already existed:
          * phase >= SEALED (its own full-ring census read zero)  -> BOTH arcs;
          * arc code == ARC_CLOSED                                -> that body's
            arc, resolved against mine;
          * neither                                               -> nothing.

        ⛔ EVERY SLOT IS FRESHNESS-TESTED (FS_V523_ARC_STALE).  The `SLOT_FS`
        read this build replaces has no such test, which is how a dead body's
        last word could pin a gate for the rest of a match.
        """
        out = set()
        if not (self._v523_on() and FS_V523_ARC_UNION):
            return out
        ...
        for s in slots:
            if s == me:
                continue                 # my own word is my own census
            try:
                beat, ph, _r = self._fs_state_at(ct, s)
            except Exception:
                continue
            if not beat or rnd - (beat - 1) > FS_V523_ARC_STALE:
                continue
            if ph == FS_PH_SEALED or ph == FS_PH_KILL:
                out.add(FS_V520_ARC_FRONT)
                out.add(FS_V520_ARC_BACK)
                continue
            code = self._v523_arc_raw(ct, s)
            arc = self._v523_peer_arc(code)
            if code == FS_V523_ARC_CLOSED and arc != FS_V520_ARC_NONE:
                out.add(arc)
        return out

    def _v523_ring_sealed(self, ct, rnd, E, orth_open):
        """⭐⭐ THE MERGE.  Is the orthogonal-8 closed, ACROSS THE CREW?

        Order of answers, cheapest first, and each is strictly weaker than the
        one above it:
          1. my own eyes say so                     (the parent's answer);
          2. a fresh crew slot publishes SEALED/KILL (SALT_UNION -- the literal
             fix for v522 doctrine collision 2, which read `SLOT_FS` alone);
          3. MY arc is closed and a fresh peer attests the OTHER arc
             (ARC_UNION -- the pincer's merge).

        ⛔ (3) REQUIRES BOTH ARCS.  A single arc-closed word can never seal the
        ring: the whole failure this build exists for is a gate answered off
        half the evidence, and answering it off the other half would be the
        same defect wearing the opposite sign.
        """
        if orth_open == 0:
            return True
        if not self._v523_on() or self._v523_inert():
            return False
        if FS_V523_SALT_UNION:
            try:
                slots = self._fs_crew_slots()
            except Exception:
                slots = ()
            for s in slots:
                ...
                if FS_PH_SEALED <= ph <= FS_PH_KILL:
                    return True
        if FS_V523_ARC_UNION and self._v523_my_arc_closed(ct, E, orth_open):
            mine = getattr(self, "v520_arc", FS_V520_ARC_NONE)
            other = (FS_V520_ARC_BACK if mine == FS_V520_ARC_FRONT
                     else FS_V520_ARC_FRONT)
            if other in self._v523_arc_attest(ct, rnd):
                self.v523_arc_credit += 1
                if FS_V523_ARC_LOG:
                    try:
                        print("ARC523 CREDIT", rnd, "id", ct.get_id(),
                              "mine", mine, "other", other,
                              "orth", orth_open,
                              "n", self.v523_arc_credit, file=sys.stderr)
                    except Exception:
                        pass
                return True
        return False
    ...
    def _v523_crew_phase(self, ct, rnd, site):
        """(beat, phase) -- the BEST FRESH phase across every crew slot.

        `_fs_state` reads `SLOT_FS`, which is body 1's word.  Body 2 publishes
        into `FS_SUPP_SLOT` (v514 change D, one writer per slot) and no Core
        consumer has ever read it -- measured at 60 of 69 nordkap publishes and
        68 of 269 glacierkeep publishes lost to that gap, BEFORE any v522
        headline game ran, and then fixed for exactly one term.

        ⛔ "BEST" IS A RANK, NOT A MAXIMUM.  `FS_PH_DEGRADE = 7` is the highest
        integer and the LOWEST state; ranking by `>` would let one body's
        terminal refusal outrank a live body's KILL.  `FS_V523_PH_RANK` is the
        order, written out.

        ⛔ AND THE BEAT RETURNED IS THE WINNING SLOT'S OWN BEAT, not SLOT_FS's:
        every consumer that tests staleness must test the freshness of the word
        it is about to believe.  Returning body 1's beat beside body 2's phase
        would be a new instance of the very defect being fixed.
        """
        try:
            base = self._fs_state_at(ct, SLOT_FS)
        except Exception:
            base = (0, FS_PH_NONE, 0)
        if not (LOKI_FS_V523 and FS_V523_CREWREAD) or self._v523_inert():
            return base[0], base[1]
        if not site:
            return base[0], base[1]
        self.v523_ph_reads += 1
        best_b, best_ph = base[0], base[1]
        if best_b and rnd - (best_b - 1) > FS_V523_ARC_STALE:
            pass                          # stale: still the fallback, not a win
        try:
            slots = self._fs_crew_slots()
        except Exception:
            slots = ()
        ...
        for s in slots:
            ...
            if not beat or rnd - (beat - 1) > FS_V523_ARC_STALE:
                continue
            if FS_V523_PH_RANK.get(ph, 0) > FS_V523_PH_RANK.get(best_ph, 0):
                best_b, best_ph, won = beat, ph, s
        if won != SLOT_FS:
            self.v523_ph_merge += 1
        if FS_V523_CREW_LOG:
            try:
                print("CREW523", rnd, "site", site, "slotfs", base[1],
                      "merged", best_ph, "won", 1 if won != SLOT_FS else 0,
                      "n", self.v523_ph_merge, "of", self.v523_ph_reads,
                      file=sys.stderr)
            except Exception:
                pass
        return best_b, best_ph

    # --- change 3: COREFUND ------------------------------------------------

    def _v523_funded(self, ct, shots, ti_floor=0):
        """⭐⭐ "FUNDED" MEANS FUNDED, NOT "THE MAGAZINE HAPPENS TO HOLD A SHOT".

        v522's pre-headline census: the Core's own funding re-check killed
        **100 of 100** glacierkeep NEAR rounds, every one on `ammo` at 8 or 0,
        while the raider's check had passed at >= 10 one round earlier.  A
        sentinel shot costs 10 on a 2-round reload, so the magazine cycles
        10 -> 0 EVERY RELOAD and an instantaneous `ammo >= 10` is False for
        half of every cycle BY CONSTRUCTION.  A guard written to prevent v521's
        failure mode reproduced it.

        The sustained form asks whether the magazine PLUS THE CONVERTIBLE BANK
        covers `shots` shots.  Titanium converts 1:1, at most once per team per
        turn, usable the SAME turn and without the core's action cooldown -- so
        bank above the reserve is ammunition one round out, and counting it is
        arithmetic, not optimism.

        Returns (sustained, spare_shot) so every caller can log the DISAGREEMENT
        rather than only the verdict.  A configuration where the two never
        disagree has an inert change 3, and that is a measurement, not a
        ...
        try:
            ...
            ammo = 0
        spare = ammo >= FS_V523_SHOT_AMMO
        if not (LOKI_FS_V523 and FS_V523_COREFUND) or self._v523_inert():
            return spare, spare
        bank = 0
        if FS_V523_FUND_BANK:
            try:
                bank = max(0, ct.get_global_resources() - ti_floor)
            except Exception:
                bank = 0
        need = FS_V523_FUND_SHOTS * FS_V523_SHOT_AMMO
        sustained = (ammo + bank) >= need
        self.v523_fund_q += 1
        if sustained != spare:
            self.v523_fund_flip += 1
        if FS_V523_FUND_LOG:
            try:
                print("FUND523", ct.get_current_round(), "ammo", ammo,
                      "bank", bank, "floor", ti_floor, "need", need,
                      "spare", 1 if spare else 0,
                      "sust", 1 if sustained else 0,
                      "flip", self.v523_fund_flip, "of", self.v523_fund_q,
                      file=sys.stderr)
            except Exception:
                pass
        return sustained, spare
    ...
    def _fs_salt_ok(self, ct, rnd, orth_open):
        ...
        if orth_open == 0:
            self.fs_sealed_rnd = rnd
            self.v523_salt_own += 1
            return True
        # ⭐⭐⭐ v523 CHANGE 1 -- THE PARAGRAPH BELOW DESCRIBED A MERGE THE CODE
        # NEVER PERFORMED, AND THAT IS THIS BUILD'S FIRST SITE.  It says the
        # published phase is "the crew's shared answer"; `_fs_state` reads
        # `SLOT_FS`, which is BODY 1's word.  Body 2 has published into
        # `FS_SUPP_SLOT` since v514 change D and nothing here has ever looked at
        # it -- so on a board where the support seals the far arc, the gate that
        # decides whether a SENTINEL MAY BE BOUGHT AT ALL is answered off half
        # the evidence.  v522 measured the same asymmetry on the Core's
        # magazine at 60 of 69 nordkap publishes, fixed it for that one term,
        # and filed this site as doctrine collision 2: "it is a bigger lever
        # than anything v522 does".
        # ⛔ MONOTONE, AND THAT IS THE SAFETY ARGUMENT.  `_v523_ring_sealed`
        # unions over a SUPERSET of what the parent read, so it can only ever
        # return True where the parent returned False.  The gate opens earlier
        # or identically -- never later.
        if self._v523_on():
            # ⛔ ONE VERDICT PER ROUND.  The phase machine already answered this
            # a few lines earlier in the same turn and cached it; re-deriving it
            # here would (a) pay for the store reads twice on a 10 ms budget and
            # (b) let the channel and the gate disagree inside one round, which
            # is the class of defect this whole build is about.
            if (getattr(self, "v523_merged_rnd", None) == rnd
                    and getattr(self, "v523_merged", False)) \
                    or (getattr(self, "v523_merged_rnd", None) != rnd
                        and self._v523_ring_sealed(
                            ct, rnd, self._enemy_anchor(ct), orth_open)):
                self.fs_sealed_rnd = rnd
                self.v523_salt_union += 1
                if FS_V523_SALT_LOG:
                    try:
                        print("SALT523", rnd, "id", ct.get_id(),
                              "orth", orth_open, "own", 0, "union", 1,
                              "n", self.v523_salt_union,
                              "ownN", self.v523_salt_own, file=sys.stderr)
                    except Exception:
                        pass
                return True
