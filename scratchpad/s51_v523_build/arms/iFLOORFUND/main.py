...
class Player(EcoMixin, RaidMixin, SiegeMixin):
    ...
    def __init__(self):
        ...
        # --- v523 state.  Counters, so every mechanism reads zero-vs-nonzero
        # against a VISIBLE denominator rather than off an outcome column.
        self.v523_open_seats = ()       # the OPEN orthogonal seats this body's
                                        # last census saw, as (x, y) pairs.
                                        # Written unconditionally by
                                        # `_fs_census`; read only under
                                        # FS_V523_ARC_UNION.
        self.v523_merged = False        # the merged closure verdict, cached
        self.v523_merged_rnd = None     # ...for THIS round only.  One verdict
                                        # per round: the channel and the gate
                                        # must not disagree inside one turn.
        self.v523_arc_closed = False    # ⭐ MY ARC IS DONE AND THE RING IS NOT.
                                        # The one bit change 1 publishes.
        self.v523_salt_own = 0          # closure questions my OWN eyes answered
        self.v523_salt_union = 0        # ...that only the UNION answered.  The
                                        # numerator; the sum is the denominator.
        self.v523_arc_credit = 0        # rounds an arc-closed PEER word was
                                        # credited toward ring closure.
        self.v523_arc_pub = 0           # rounds THIS body published the code.
        self.v523_ph_merge = 0          # Core reads where the merged phase beat
                                        # the SLOT_FS phase (change 2's dose).
        self.v523_ph_reads = 0          # ...out of this many.  The denominator.
        self.v523_fund_q = 0            # funding questions asked (change 3)
        self.v523_fund_flip = 0         # ...where sustained and spare-shot
                                        # DISAGREED.  A zero here means change 3
                                        # is inert in this configuration, and it
    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            if LOKI_FERRY_SIEGE_ON:
                # ⭐⭐⭐ v523 CHANGE 2, SITE 1 OF 3 -- AND THIS IS THE SITE THE
                # 60-OF-69 MEASUREMENT WAS TAKEN ON.  `_fs_state` reads
                # `SLOT_FS`, body 1's word.  v522's pre-headline census found
                # the Core blind to body 2's channel in 60 of 69 nordkap
                # publishes and 68 of 269 glacierkeep publishes, fixed it for
                # its own NEAR term alone, and left the ARMING term -- the one
                # that decides whether the magazine converts at all -- on the
                # old channel.
                _fb, fs_ph = self._v523_crew_phase(
                    ct, rnd, "mag" if (LOKI_FS_V523 and FS_V523_CREWREAD
                                       and FS_V523_CREW_MAG) else None)
                ...
                if _v516_kill or fs_ph == FS_PH_KILL \
                    ...
                    if LOKI_FS_CREW and FS_SALT_GATE:
                        ...
                        if (LOKI_FS_V522 and FS_V522_FLOOR) or FS_V522_MAG_LOG:
                            ...
                            # ⭐⭐⭐ v523 CHANGE 3 -- THE PREDICATE, CORRECTED
                            # RATHER THAN DELETED.  v522's answer to the census
                            # (100 of 100 glacierkeep NEAR rounds killed by
                            # this line) was to switch the term OFF
                            # (`FS_V522_CORE_FUND = False`), which restores the
                            # reachability and gives up the guard.  v523 fixes
                            # the SEMANTICS: "funded" is the magazine PLUS the
                            # convertible bank covering FS_V523_FUND_SHOTS
                            # shots, which is what "the conversion is keeping
                            # pace with the reload cycle" means in arithmetic.
                            # ⛔ THE GUARD COMES BACK ON WITH IT.  Turning a
                            # safety term off because it was mis-specified
                            # leaves the design unprotected in exactly the
                            # state it was written for.
                            _v523_sust, _v522_fund_raw = self._v523_funded(
                                ct, FS_V523_FUND_SHOTS, ti_floor)
                            if LOKI_FS_V523 and FS_V523_COREFUND:
                                _v522_fund = _v523_sust
                            else:
        ...
        if LOKI_FERRY_SIEGE_ON and self._fs_gate(ct):
            # ⭐ v523 CHANGE 2, SITE 2 OF 3 -- THE BUDGET / REPLACEMENT DOOR.
            # `ph` here decides whether the plank is DEGRADED (stop paying for
            # replacement bodies) and whether the opening is still capped.  A
            # dead body 1 publishing DEGRADE while body 2 is alive at the ring
            # should not shut that door -- which is why the merge RANKS phases
            # (FS_V523_PH_RANK) instead of taking a maximum: DEGRADE = 7 is the
            # highest integer and the lowest state.
            beat, ph = self._v523_crew_phase(
                ct, rnd, "budget" if (LOKI_FS_V523 and FS_V523_CREWREAD
                                      and FS_V523_CREW_BUDGET) else None)
            ...
            if ph != FS_PH_DEGRADE:
                ...
                if (stale and rnd >= FS_BEAT_STALE and self.n >= FS_OPEN_BUILDERS
                        and self.fs_replaced < FS_MAX_REPLACE
        ...
        if LOKI_FERRY_SIEGE_ON and FS_SPAWN_RESERVE_ON:
            # ⭐ v523 CHANGE 2, SITE 3 OF 3 -- THE SPAWN COLLAR RESERVE.  This
            # holds a whole collar's price back from the biggest spender on the
            # board while a raider is at the ring.  Read off body 1's word
            # alone, it releases the moment body 1 dies -- with body 2 still
            # standing on the collar and still needing barriers.
            # ⛔ AND THE BEAT RETURNED IS THE WINNING SLOT'S OWN, so the
            # staleness test below tests the freshness of the word it believes.
            _fb, _fp = self._v523_crew_phase(
                ct, rnd, "spawn" if (LOKI_FS_V523 and FS_V523_CREWREAD
                                     and FS_V523_CREW_SPAWN) else None)
        ...
        if LOKI_FS_V516 and FS_V516_GLOBALSENT \
            ...
            try:
                ...
                if _E is not None and _ours is not None \
                        and self._fs_map_gated(self.mw, self.mh, _ours, _E) \
