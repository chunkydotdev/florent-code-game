...
FS_V522_FLOOR = False       # ⛔⛔ v523 FIRED-CONFIG CORRECTION (i), AT THE
                            # DEFINITION SITE.  The s51 builder verdict on v522:
                            # "FS_V522_FLOOR: NULL, exemplary controls (every
                            # column inside the known-zero arm's own movement;
                            # binding raised 13%->96% by the pre-headline census
                            # and STILL null -- the floor was not the
                            # constraint).  Ships OFF/indifferent."  The code
                            # stays; the flag is the decision.  ⭐ AND IT IS NOT
                            # DEAD FOR EVER: v523's SALTEYES makes SEALED
                            # reachable in pincer games for the first time, so
                            # the floor is re-measured in the `iFLOORFUND`
                            # isolation arm ON TOP of the corrected funding
                            # predicate rather than being carried in the fired
                            # config as a confound.
...
# =============================================================================
# v523 -- THE BODY-2 BLINDNESS TRIO.  Make the team's gates SEE the second
# raider.
# =============================================================================
#
# ⭐⭐⭐ THE DIAGNOSIS, AND IT IS THE PARENT'S OWN.  v522's build report banked
# two measurements and one unfixed doctrine collision, all of the same shape:
#
#   (a) "THE CORE HAS NEVER READ BODY 2's PUBLISH CHANNEL, AND ON NORDKAP THAT
#       IS 60 OF 69 PUBLISHES."  `FS_SUPP_SLOT` has existed since v514 and
#       EVERY Core-side consumer of the phase reads `SLOT_FS` -- body 1's word
#       -- only.  v522 fixed it for its OWN term (`_v522_crew_near`) and left
#       every other consumer on the old channel.
#   (b) DOCTRINE COLLISION 2, UNRESOLVED: "`_fs_salt_ok` CALLS THE `SLOT_FS`
#       PHASE 'THE CREW'S SHARED ANSWER' WHILE READING ONE BODY'S WORD ... The
#       salt gate -- which decides whether a sentinel may be bought at all --
#       has the same asymmetry and is untouched here.  IT IS A BIGGER LEVER
#       THAN ANYTHING v522 DOES."
#   (c) "THE CORE'S OWN FUNDING RE-CHECK -- THE MOST OBVIOUS SAFETY GUARD IN
#       THE DESIGN -- KILLED 100 OF 100 GLACIERKEEP NEAR ROUNDS", because the
#       magazine cycles 10 -> 0 on every sentinel reload and `ammo >= 10` read
#       one round after the raider's own check measures A SPARE SHOT, not
#       FUNDING.
#
# ⛔ WHY THIS IS ONE BUILD AND NOT THREE.  All three are the same defect --
# a team-wide question answered off ONE body's word or ONE round's snapshot --
# and they sit on the SAME path: the salt gate decides whether a sentinel is
# bought, the Core's phase read decides whether the magazine that feeds it is
# armed, and the funding predicate decides whether either is believed.  Fixing
# one and leaving the others is what v522 did, and its own report named the
# result.  ⭐ Each is nonetheless its OWN FLAG with its OWN mechanism arm, so
# the composite can be decomposed if it surprises.
#
# ⛔⛔ AND THE ROOT CAUSE OF (b) IS NOT A TYPO -- IT IS A MISSING MERGE.
# `_fs_census` counts `orth_open` over the WHOLE 12-ring "from live vision",
# and `_fs_denied` documents its own failure direction: "Unreadable (out of
# vision) counts as NOT denied, which is the conservative direction."  Under
# the v520 PINCER the two bodies stand on OPPOSITE arcs by construction
# (`_v520_claim_arc`), each sealing its own arc first (`FS_V520_ARC_SEAL`).
# So the question "is the orthogonal-8 closed?" is answered by ONE body, from
# ONE position, about a curve the OTHER body is half-way round -- and the
# answer that reaches every consumer is body 1's, because `_fs_state` reads
# `SLOT_FS`.  ⇒ THE PINCER'S SECOND ARC CAN SEAL PERFECTLY AND NEVER COUNT.

LOKI_FS_V523 = True         # master.  False == `bots/_v522floor` AS CONFIGURED
                            # by the v523 fired-config correction above
                            # (FS_V522_FLOOR False, FS_V521_SYNC False,
                            # FS_V521_PHASE_HONEST / FS_V521_GATEFIX True) --
                            # byte-proved below, and the concurrent baseline of
                            # this build's headline.
                            # ⛔ EVERY v523 BRANCH READS THIS AT RUN TIME.  No
                            # module-level derived default may read it (the v515
                            # `FERRY_HOME_ON` hazard); `flagoff_ast.py` proves
                            # that, with a positive control.

# --- CHANGE 1: FS_V523_SALTEYES -- the closure census sees both raiders ------
FS_V523_SALTEYES = True     # the mechanism.
FS_V523_SALT_UNION = True   # ⭐ THE LITERAL FIX FOR v522 DOCTRINE COLLISION 2.
                            # `_fs_salt_ok` reads the phase off EVERY slot a
                            # ferry-siege body publishes into (`_fs_crew_slots`)
                            # instead of `SLOT_FS` alone, with the FRESHNESS
                            # test the SLOT_FS path does not have.  ⛔ MONOTONE:
                            # a union over a superset of slots can only ever ADD
                            # a "sealed" verdict, never remove one, so the gate
                            # can open earlier and never later.  With the crew
                            # off `_fs_crew_slots()` is `(SLOT_FS,)` and this is
                            # the parent read, character for character.
FS_V523_ARC_UNION = True    # ⭐⭐ THE MERGE THE PINCER NEEDS, AND IT COSTS NO
                            # NEW SLOT AND NO NEW WRITER.  A body whose own
                            # ARC's seats are all denied, while the ring as a
                            # whole is not, publishes the arc code
                            # FS_V523_ARC_CLOSED in the two bits `_fs_publish`
                            # already stamps.  A reader then declares the ring
                            # closed when BOTH arcs are attested by FRESH words:
                            # its own census, a peer's full-ring SEALED, or a
                            # peer's arc-closed code.  ⛔ BOTH, never one -- a
                            # single arc-closed word can NEVER seal the ring, or
                            # the pincer would gate the sentinel on half a
                            # collar.
FS_V523_ARC_CLOSED = 3      # ⭐ THE ONE FREE CODE IN THE WHOLE WORD, AND THAT
                            # IS WHY THE CHANNEL IS FREE.  The publish word is
                            # beat 0-10, phase 11-13 (all 8 codes taken: 0-6
                            # plus DEGRADE=7), rid 14-29, arc 30-31 -- 32 bits,
                            # fully packed, and a store slot is an unsigned
                            # 32-bit integer (s50 probe).  The ARC field's
                            # value 3 is the only unused encoding left:
                            # NONE=0, FRONT=1, BACK=2.
                            # ⛔ AND THE ARC IDENTITY IS NOT LOST BY USING IT.
                            # A body may only publish 3 after it has CLAIMED an
                            # arc, the claim is STICKY for the body's life
                            # (`_v520_claim_arc`), and the two bodies are
                            # DECONFLICTED onto different arcs.  So a reader
                            # holding arc X that sees a peer publishing 3 knows
                            # the peer's arc is the complement of X.  A reader
                            # that has claimed NOTHING cannot resolve it and
                            # therefore does not credit it -- stated here
                            # because that is a real, bounded blind spot and
                            # not a rounding error.
FS_V523_ARC_STALE = 6       # rounds of silence before an arc attestation is
                            # dropped.  FS_CREW_STALE's value, reused rather
                            # than re-derived: a dead body's last word must not
                            # gate a sentinel purchase for the rest of the
                            # match, which is the hazard `_v522_crew_near` was
                            # given a freshness test for and the SLOT_FS path
                            # still has not got.
FS_V523_PHASE_SEALED = True  # ⭐ the phase machine PUBLISHES the merged verdict
                            # (FS_PH_SEALED) when the union says the ring is
                            # closed and this body's own eyes do not.  Without
                            # this the merge is invisible to every consumer that
                            # reads the CHANNEL rather than calling the
                            # predicate -- which is most of them -- and the
                            # SEALED-RATE, the headline instrument this build is
                            # judged on, could not move.

# --- CHANGE 2: FS_V523_CREWREAD -- the Core's consumers read both slots ------
FS_V523_CREWREAD = True     # the mechanism.  `_v523_crew_phase` returns the
                            # BEST fresh phase across `_fs_crew_slots()`, and
                            # the Core's three consumers use it instead of
                            # `_fs_state(ct)`:
                            #   * the MAGAZINE arming term (`main.py`, the
                            #     `armed` ladder) -- the 60/69 nordkap
                            #     measurement's own site;
                            #   * the BUDGET / replacement door;
                            #   * the SPAWN COLLAR RESERVE.
                            # ⛔ "BEST" IS DEFINED, NOT ASSUMED: the phase codes
                            # are not a total order (DEGRADE = 7 is the LOWEST
                            # state, not the highest), so the merge ranks them
                            # explicitly through FS_V523_PH_RANK and never by
                            # integer comparison.
FS_V523_CREW_MAG = True     # sub-site 1: the magazine arming term.
FS_V523_CREW_BUDGET = True  # sub-site 2: the budget / replacement door.
FS_V523_CREW_SPAWN = True   # sub-site 3: the spawn collar reserve.
#: ⛔ THE RANK, WRITTEN OUT.  Higher = more advanced.  DEGRADE is ranked BELOW
#: NONE deliberately: it is a terminal refusal, and a merge that let it win
#: would let one dead body veto a live one's report -- the exact asymmetry this
#: change exists to remove, with the sign reversed.
FS_V523_PH_RANK = {0: 1, 1: 2, 2: 3, 3: 4, 4: 7, 5: 5, 6: 6, 7: 0}

# --- CHANGE 3: FS_V523_COREFUND -- "funded" means funded, not "a spare shot" -
FS_V523_COREFUND = True     # the mechanism.  `_v523_funded(ct, shots)` asks
                            # whether the magazine PLUS the convertible bank
                            # covers `shots` sentinel shots, instead of asking
                            # whether the magazine happens to hold one right
                            # now.  ⛔ THE ARITHMETIC IS THE WHOLE POINT: a
                            # sentinel costs 10 ammunition on a 2-round reload,
                            # so the magazine cycles 10 -> 0 every reload and an
                            # instantaneous `ammo >= 10` is FALSE for half of
                            # every cycle by construction.  Titanium converts to
                            # ammunition 1:1, at most once per team per turn,
                            # usable the same turn and WITHOUT the core's action
                            # cooldown -- so bank above the reserve IS
                            # ammunition, one round out.
FS_V523_FUND_SHOTS = 2      # shots' worth the sustained test asks for.  TWO,
                            # not one: one shot is exactly the quantity the
                            # broken predicate already measured, and the reload
                            # is 2 rounds, so two shots is the smallest number
                            # that spans a full cycle.  UNSWEPT.
FS_V523_SHOT_AMMO = 10      # a sentinel shot's ammunition cost (GameConstants
                            # has no name for it; the gunner's 4 is not what
                            # this plank fires).
FS_V523_FUND_BANK = True    # count convertible bank toward funding.  False
                            # reduces the predicate to `ammo >= shots * 10`,
                            # which is the mechanism arm that separates "two
                            # shots instead of one" from "bank counts too".

# --- verification-only flags.  NEVER SHIPPED TRUE. --------------------------
FS_V523_STORE_ONLY = False  # ⛔⛔ THE SAME-STORE-WRITE DOSE BASELINE, AND IT IS
                            # A MANDATE ITEM RATHER THAN A NICETY.  v522 proved
                            # that `.replay26` SERIALISES THE PRIVATE COMMS
                            # STORE: a clause that only writes a different
                            # number into a store slot scores a FULL DOSE on a
                            # replay-byte diff -- 167 differing bytes across 5
                            # games, every one a varint differing by exactly
                            # `1 << FS_PHASE_SHIFT`, files identical in length,
                            # not one game event changed.  With this True the
                            # tree publishes every v523 word it would publish
                            # (the merged SEALED phase, the arc-closed code) and
                            # NO consumer acts on it, so a dose measured against
                            # THIS arm is the behaviour and nothing else.
FS_V523_PROBE_NOARC = False  # publishes the merged phase but never the arc
                            # code, to separate the two channels' byte
                            # contributions if the dose control disagrees.

# --- instruments.  Gated on THEMSELVES, not on the master, so a master-off ---
# --- arm still emits the DENOMINATOR and its zero is readable (v521 s7). -----
FS_V523_SALT_LOG = False    # SALT523: one line per closure question, with the
                            # own-eyes verdict, the union verdict and which slot
                            # carried it.
FS_V523_CREW_LOG = False    # CREW523: one line per Core-side phase read, with
                            # the SLOT_FS phase, the merged phase and the slot.
FS_V523_FUND_LOG = False    # FUND523: one line per funding question, with ammo,
                            # bank, the spare-shot verdict and the sustained
                            # verdict.
FS_V523_ARC_LOG = False     # ARC523: one line per arc-closed publish/credit.
...
# --- ARM OVERRIDE: mUNION (src bots/_v523eyes) ---
FS_V523_SALT_LOG = True
FS_V523_CREW_LOG = True
FS_V523_FUND_LOG = True
FS_V523_ARC_LOG = True
FS_V522_MAG_LOG = True
FS_V523_SALT_UNION = False
