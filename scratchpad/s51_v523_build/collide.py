...
def main():
    ...
    fired = REPO / "bots/_v523eyes"
    cases.append(("v523 FIRED (crew ON)", fired, 0))

    off = tmp / "off"
    shutil.copytree(fired, off)
    subprocess.run(["chmod", "-R", "u+w", str(off)])
    with open(off / "doctrine.py", "a") as fh:
        fh.write("\nLOKI_FS_V523 = False\n")
    cases.append(("v523 master OFF == the v522-verdict parent (crew still ON)", off, 0))

    # ⛔ AND THE CREW-OFF BRANCH IS STILL EXERCISED, because turning v521 off
    # does NOT turn the crew off (v521's flag-off state is v520-PINCER-ONLY,
    # which is a crew-ON configuration).  Without this row the "crew OFF ->
    # home ferry ON" half of the predicate would go untested in this build.
    off20 = tmp / "off20"
    shutil.copytree(fired, off20)
    subprocess.run(["chmod", "-R", "u+w", str(off20)])
    with open(off20 / "doctrine.py", "a") as fh:
        fh.write("\nLOKI_FS_V520 = False\n")
    cases.append(("v520 master OFF (crew OFF, home ferry ON)", off20, 0))

    bad = tmp / "bad"
    shutil.copytree(fired, bad)
    subprocess.run(["chmod", "-R", "u+w", str(bad)])
