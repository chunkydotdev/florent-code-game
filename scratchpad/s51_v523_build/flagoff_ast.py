...
V523 = {"LOKI_FS_V523", "FS_V523_SALTEYES", "FS_V523_SALT_UNION",
        "FS_V523_ARC_UNION", "FS_V523_ARC_CLOSED", "FS_V523_ARC_STALE",
        "FS_V523_PHASE_SEALED", "FS_V523_CREWREAD", "FS_V523_CREW_MAG",
        "FS_V523_CREW_BUDGET", "FS_V523_CREW_SPAWN", "FS_V523_PH_RANK",
        "FS_V523_COREFUND", "FS_V523_FUND_SHOTS", "FS_V523_SHOT_AMMO",
        "FS_V523_FUND_BANK", "FS_V523_STORE_ONLY", "FS_V523_PROBE_NOARC",
        "FS_V523_SALT_LOG", "FS_V523_CREW_LOG", "FS_V523_FUND_LOG",
        "FS_V523_ARC_LOG"}
...
GUARD_POS = "LOKI_FS_V523 = True\nX = LOKI_FS_V523 and 3\n"
GUARD_NEG = "LOKI_FS_V523 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V523 = True\nif LOKI_FS_V523:\n    X = 3\n"


def main():
    path = sys.argv[1] if len(sys.argv) > 1 else \
        "bots/_v523eyes/doctrine.py"
    ok = True
    if not scan(GUARD_POS, V523):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V523):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V523):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V523)), bool(scan(GUARD_NEG, V523)),
           bool(scan(GUARD_IF, V523))))
    src = open(path).read()
    h523 = scan(src, V523)
    h522 = scan(src, V522)
    h521 = scan(src, V521)
    ...
    h518 = scan(src, V518)
    hcrew = scan(src, CREW)
    print("v523 derived defaults: %d %s" % (len(h523), h523))
    ...
    print("REAL-CASE CONTROL (FS_CREW_ON readers, the known v515 hazard): "
          "%d %s" % (len(hcrew), hcrew))
    if h523 or h522 or h521 or h520 or h519 or h518:
        ok = False
    if not hcrew:
