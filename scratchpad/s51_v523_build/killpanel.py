...
ARMS = _json.loads(os.environ["ARMS"]) if os.environ.get("ARMS") else \
    [("parent", "parent.tsv"), ("v523_FIRED", "v523.tsv"),
     ("v523_flagoff", "flagoff.tsv")]
...
for n, _ in ARMS[1:]:
    ...
    for lab, key in (("wins", "wins"), ("k<=200", "k200x"), ("k<=300", "k300")):
        ...
        d = 100.0 * (a / N - b / M)
        h = hw(a / N, N, b / M, M)
        print("%-12s vs base    %-7s %+6.2f pp (hw %.2f) %s"
