        ...
        if ok and LOKI_FS_V519 and FS_V519_MODESWITCH:
            ...
            if sig519 in FS_V519_CRIPPLE_MAPS:
                ...
                if LOKI_FS_V524:
                    grid = self.map_grid
                    try:
                        print("PREGRID", ct.get_current_round(), "id", ct.get_id(),
                              "grid_none", grid is None, "core", self.core, file=sys.stderr)
                    except Exception:
                        pass
                    if grid is None and ct is not None:
                        # The one caller with no cached grid is the v516
                        # turret beat (main.py) -- its `self.core` is None by
                        # design, so `main._core`/`_builder` never populate
                        # `self.map_grid` for it. Resolve it here, once, with
                        # the same call and cache it the same way those two
                        # sites do; a turret persists across rounds so this
                        # pays at most once per unit's lifetime.
