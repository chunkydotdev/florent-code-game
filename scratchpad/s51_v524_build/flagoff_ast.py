#!/usr/bin/env python3
"""NO NEW DERIVED DEFAULTS: AST-scan doctrine.py AND siege.py for module-level
assignments (or module-level `if`/conditional tests) whose right-hand side
reads LOKI_FS_V524 or any v524 constant, plus every inherited flag set back
through v515/CREW (the v515 finding-3 hazard: `mkarm.sh` appends arm
overrides to the end of doctrine.py, so a module-level `X = f(FLAG)` freezes
the flag value the MODULE BODY saw, not the arm's -- `LOKI_FS_V524` is read
correctly at RUN time inside `_fs_map_gated`, never at import time).

Copied and extended from scratchpad/s51_v522_build/flagoff_ast.py (same
method, same guard, same inherited flag sets) -- adds V524 and a second scan
target (siege.py now carries FS_V524_CRIPPLE_GRIDS at module scope).
...
GUARD_POS = "LOKI_FS_V524 = True\nX = LOKI_FS_V524 and 3\n"
GUARD_NEG = "LOKI_FS_V524 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V524 = True\nif LOKI_FS_V524:\n    X = 3\n"


def main():
    # ⛔ SCOPE, MATCHING THE PRECEDENT TOOL EXACTLY: the v515 append-hazard is
    # a SAME-FILE ordering hazard -- `mkarm.sh` appends an arm's flag override
    # to the END of doctrine.py, so a module-level statement EARLIER in
    # doctrine.py that reads the flag freezes the value doctrine.py's own
    # module body saw, not the arm's. A cross-file reference (e.g. siege.py's
    # `FS_V524_CRIPPLE_GRIDS = frozenset((_decode_grid(FS_V524_MIDGARD_CODE,
    # ...), ...))`, which reads doctrine.py CONSTANTS via `from doctrine
    # import *`) is safe by construction: Python fully executes doctrine.py,
    # override included, before `from doctrine import *` in ANY OTHER module
    # can see it. Scanning siege.py/main.py/eco.py for the same name set was
    # tried and produced exactly that false positive (FS_V524_CRIPPLE_GRIDS
    # reading FS_V524_MIDGARD_CODE/YULERUNE_CODE, a plain constant-to-constant
    # reference, not a flag-ordering hazard) -- reverted to doctrine.py only,
    # matching scratchpad/s51_v522_build/flagoff_ast.py's own scope.
    paths = sys.argv[1:] or ["bots/_v524exact/doctrine.py"]
    ok = True
    if not scan(GUARD_POS, V524):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V524):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V524):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V524)), bool(scan(GUARD_NEG, V524)),
           bool(scan(GUARD_IF, V524))))
    ...
    for path in paths:
        ...
        hcrew = scan(src, CREW)
        print(f"\n=== {path} ===")
        print("v524 derived defaults: %d %s" % (len(h524), h524))
        ...
        if path.endswith("doctrine.py"):
            ...
            if not hcrew:
                ...
                ok = False
        total += len(h524) + len(h522) + len(h521) + len(h520) + len(h519) + len(h518)
