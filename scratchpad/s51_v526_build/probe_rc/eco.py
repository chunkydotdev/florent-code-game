...
class EcoMixin:
    ...
    def _nav(self, ct, pave=True):
        if self.tgt is None or ct.get_move_cooldown() != 0:
            if V526RC and V526RC_WALK:
                self._v526_walk(ct, "-", "nomove" if self.tgt is not None
                                else "notgt")
            return
        desired = self._bfs_direction(ct, self.tgt)
        if desired == Direction.CENTRE:
            if V526RC and V526RC_WALK:
                self._v526_walk(ct, "CENTRE", "centre")
            return
        if self._move(ct, desired, pave):
            if V526RC and V526RC_WALK:
                self._v526_walk(ct, desired.name, "moved")
            return
        ...
        for d in (CARDINALS[(idx + 1) % 4], CARDINALS[(idx + 3) % 4], desired.opposite()):
            if self._move(ct, d, pave):
                if V526RC and V526RC_WALK:
                    self._v526_walk(ct, desired.name, "alt" + d.name)
                return
        self.stuck += 1
        if V526RC and V526RC_WALK:
            self._v526_walk(ct, desired.name, "STUCK%d" % self.stuck)

    def _v526_walk(self, ct, want, verdict):
        try:
            p = ct.get_position()
            t = self.tgt
            print("RC WALK", ct.get_current_round(), "id", ct.get_id(),
                  "role", getattr(self, "role", "?"),
                  "fs", 1 if getattr(self, "fs_raider", False) else 0,
                  "body", getattr(self, "fs_body", 1),
                  "pos", "%d,%d" % (p.x, p.y),
                  "tgt", ("%d,%d" % (t.x, t.y)) if t is not None else "-",
                  "want", want, "verdict", verdict,
