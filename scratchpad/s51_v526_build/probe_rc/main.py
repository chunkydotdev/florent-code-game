...
class Player(EcoMixin, RaidMixin, SiegeMixin):
        ...
        if V526RC:
            try:
                ...
                if _r <= V526RC_MAXRND or _r % 25 == 0:
                    _nh = 0
                    for _b in ct.get_nearby_buildings(10 ** 4):
                        try:
                            if ct.get_entity_type(_b) == EntityType.HARVESTER \
                                    and ct.get_team(_b) == self.team:
                                _nh += 1
                        except Exception:
                            pass
                    print("RC ECO", _r, "harvstore",
                          ct.read_store(SLOT_HARVESTERS),
                          "harvseen", _nh,
                          "ti", ct.get_global_resources(),
                          "units", ct.get_unit_count(),
                          "scale", ct.get_scale_percent(), file=sys.stderr)
            except Exception:
                pass
    ...
    def _builder(self, ct):
        ...
        if self.team is None:
            ...
            if V526RC:
                try:
                    print("RC SEAT", ct.get_current_round(), "id", ct.get_id(),
                          "seat", n, file=sys.stderr)
                except Exception:
                    pass
        ...
        if V526RC:
            try:
                print("RC POS", ct.get_current_round(), "id", ct.get_id(),
                      "seat", self.role_n, "role", self.role,
                      "fs", 1 if self.fs_raider else 0,
                      "body", getattr(self, "fs_body", 1),
                      "pos", "%d,%d" % (p.x, p.y),
                      "stuck", getattr(self, "stuck", 0), file=sys.stderr)
            except Exception:
                pass
