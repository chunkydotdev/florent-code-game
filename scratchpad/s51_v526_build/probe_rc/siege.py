...
class SiegeMixin:
        ...
        try:
            for eid in ct.get_nearby_units():
                ...
                if ct.get_team(eid) != self.team:
                    continue
                _d = ct.get_position(eid).distance_squared(p)
                self.v526_must_why = "near%d" % _d
                return _d <= FS_MUSTER_DSQ
        except Exception:
            self.v526_must_why = "blind"
            return True                      # blind: do not stall on a getter
        self.v526_must_why = "notseen"
        return False
    ...
    def _fs_build_ferry(self, ct, p, T, E=None):
        ...
        if V526RC:
            try:
                print("RC HOP", ct.get_current_round(), "id", ct.get_id(),
                      "body", getattr(self, "fs_body", 1),
