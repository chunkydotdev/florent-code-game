...
class SiegeMixin:
    ...
    def _v527_bunker_swap(self, ct, E, p, rnd, needed, orth_open, ti):
        ...
        if not (self._v527_on() and FS_V527_BUNKER):
            return False
        self.v527_ask_n = getattr(self, "v527_ask_n", 0) + 1
        if getattr(self, "v527_bunker_n", 0) >= FS_V527_BUNKER_MAX:
            self._v527_log("BUNKER HOLD", rnd, "why", "-", "blk", "cap")
            return False
        armed, why = self._v527_bunker_state(ct, p, orth_open)
        if not armed:
            self._v527_log("BUNKER HOLD", rnd, "why", "notarmed",
                           "blk", "armed", "orth", orth_open)
            return False
    ...
    def _fs_try_seal(self, ct, E, p, needed, ti, orth_open=0):
        ...
        if self._v527_on() and FS_V527_SEALPATH:
            ...
            self._v527_log("PICKASK", rnow, "adj",
                           len([t for t in needed
                                if abs(t.x - p.x) + abs(t.y - p.y) == 1]),
                           "pick", "-" if _pick is None
                           else (_pick.x, _pick.y),
                           "cutall", 1 if _cut_all else 0)
            if _pick is not None:
                ...
                if _first_adj is not None \
                    ...
                    self._v527_log("REORDER", rnow,
                                   "was", (_first_adj.x, _first_adj.y),
                                   "now", (_pick.x, _pick.y),
                                   "n", self.v527_selfcut_avoid)
            elif _cut_all:
        ...
        for dx, dy in CARD_DELTAS:
            ...
            for target in tiles:
                ...
                if self._v527_on() and FS_V527_DEFENDED \
                    ...
                    score += FS_V527_DEFENDED_BONUS
                    self._v527_log("DEFHIT", rnd, "at", (bx, by))
                if best_k is None or score > best_k:
                    best, best_k, best_ray = (bp, facing), score, target
        if best is None:
            return False
