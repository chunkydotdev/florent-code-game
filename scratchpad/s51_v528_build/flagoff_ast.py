...
GUARD_POS = "LOKI_FS_V528 = True\nX = LOKI_FS_V528 and 3\n"
GUARD_NEG = "LOKI_FS_V528 = True\nX = 3\n"
GUARD_IF = "LOKI_FS_V528 = True\nif LOKI_FS_V528:\n    X = 3\n"


def main():
    paths = sys.argv[1:] or ["bots/_v528eco/doctrine.py"]
    ok = True
    if not scan(GUARD_POS, V528):
        print("GUARD FAIL: positive control not reported"); ok = False
    if scan(GUARD_NEG, V528):
        print("GUARD FAIL: negative control reported"); ok = False
    if not scan(GUARD_IF, V528):
        print("GUARD FAIL: module-level `if FLAG:` not reported"); ok = False
    print("GUARD: pos=%s neg=%s if=%s" %
          (bool(scan(GUARD_POS, V528)), bool(scan(GUARD_NEG, V528)),
           bool(scan(GUARD_IF, V528))))
    ...
    for path in paths:
        ...
        hcrew = scan(src, CREW)
        print(f"\n=== {path} ===")
        print("v528 derived defaults: %d %s" % (len(h528), h528))
        ...
        if path.endswith("doctrine.py"):
            ...
            if not hcrew:
                ...
                ok = False
        total += (len(h528) + len(h527) + len(h526) + len(h525) + len(h524) + len(h522) + len(h521) + len(h520) +
                  len(h519) + len(h518))
