...
# reproduces the parent's four-map collision exactly, byte-for-byte, with no
# other code path touched. When `LOKI_FS_V524` is True, a coarse hit is only a
# CANDIDATE: it is confirmed against `self.map_grid` (or, if that is not yet
# resolved -- the one call site is the v516 turret beat, whose own `self.core`
# is None by design and never sets it -- against a fresh
# `known_map_for(mw, mh, ours, ct)` call, cached into `self.map_grid` exactly
# as the Core/builder call sites already do) before it is allowed to refuse.
...
FS_V526_TEMPO = False       # ⛔ v528 PARENT CONFIG: v526's own build report
                            # measured M6/TEMPO carrying the WHOLE v526
                            # regression alone (k<=200 -10.83pp, median kill
                            # 173 -> 237).  The shippable object in that tree is
                            # FS_V526_RDV alone, so THE PARENT OF v528 IS
                            # `_v526transit` RDV-ONLY.  Set at the DEFINITION
                            # site, not appended after the read-site helpers.
...
FS_V528_LOG = True         # stdout instrument tapes (CONN / WALK / WIRE), off
                            # in the fired build; the instrumented arm sets it.


LOKI_FS_V528 = False         # master.  False == `_v526transit` RDV-ONLY exactly.


# --- s51 v528 M-METRIC TAPE ---
V528RC = True
