#!/usr/bin/env python3
"""v530 FLAG-OFF BYTE-IDENTITY + the sub-flag standdown.

Method is v524-v529's: `NOISE_ON = False` on BOTH sides (ours and the opponent
copy `eq_opp`, which differs from `bots/_v488beltbreak2` by that ONE line),
`--tle 0`, replay BYTES compared.  `--seed` alone does not pin a game (v515
finding 1) -- NOISE_ON must also be off.

FOUR ARMS, EACH DRIVEN TO THE OTHER VERDICT:
  A1  eq_off   (master False)        vs eq_parent  -> IDENTICAL  (the known zero)
  A2  eq_v530  (as fired)            vs eq_parent  -> DIFFERS    (negative control:
                                                      without it A1 could pass
                                                      because the tree is inert)
  A3  eq_sub   (master TRUE, all three sub-flags False)
                                     vs eq_parent  -> IDENTICAL  (standdown: no
                                                      master does anything on
                                                      its own, so every
                                                      per-plank ablation in the
                                                      battery means what it says)
  A4  eq_sub                         vs eq_v530    -> DIFFERS    (negative
                                                      control for A3)
"""
import subprocess
import sys
...
D = ROOT / "scratchpad/s51_v530_build"
OUT = D / "byte_check"
OUT.mkdir(parents=True, exist_ok=True)
OPP = str(D / "eq_opp")

MAPS5 = ["atoll", "drakkarfjord", "glacierkeep", "nordkap", "yulerune"]
MAPS_WALL = ["valkyrie", "ragnarok", "midgard", "icefloe", "auroraveil"]
SEED = 530820
...
def main():
    ...
    arm("A2 ", "eq_v530", "eq_parent", MAPS5, False)
    arm("A3 ", "eq_sub", "eq_parent", MAPS5 + MAPS_WALL, True)
    arm("A4 ", "eq_sub", "eq_v530", MAPS5, False)
