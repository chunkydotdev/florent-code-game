...
def main():
    paths = sys.argv[1:] or ["bots/_v530home/doctrine.py"]
    ok = True
    ...
    for path in paths:
        ...
        hcrew = scan(src, CREW)
        print(f"\n=== {path} ===")
        print("v530 derived defaults: %d %s" % (len(h530), h530))
        ...
        if path.endswith("doctrine.py"):
            ...
            if not hcrew:
                ...
                ok = False
        total += (len(h530) + len(h528) + len(h527) + len(h526) + len(h525) + len(h524) + len(h522) + len(h521) + len(h520) +
                  len(h519) + len(h518))
