...
# =============================================================================
# v530 -- THE HOME-DEFENCE PACKAGE (s51, 2026-08-20).  Parent: `_v529merge`.
# =============================================================================
# THREE MAGNUS-DESIGNED PLANKS, ALL POINTED AT OUR OWN CORE GROUND.  The parent
# tree defends its own ring with nothing at all: the s51 replay study measured
# 565 conveyors, 20 launchers, 9 sentinels, 8 gunners and **0 barriers** on our
# own 12-tile ring across 80 modern ladder games, while we put 528 barriers on
# THEIRS.  The collar doctrine exists, is correct, and is pointed only outward.
#
# ⭐ READ-SITE ONLY.  Nothing below is folded into a module-level default; every
# flag is read inside the branch it guards, so `LOKI_FS_V530 = False` reproduces
# `_v529merge` without any other edit.  `flagoff_ast.py` asserts this with its
# FERRY_HOME_ON positive control.
#
# ⛔ DEFENCE_ADMISSION_BAR CARRIED.  All three planks are defensive by
# construction, so `PLAY_DEFENCE: not_at_the_kill_s_expense` binds: the ITT
# share of ALL games ending in a core kill by r300 must not FALL vs control,
# and the r180/r200 timely-kill shares are reported beside it.  R1000_IS_DEFEAT
# is unchanged -- none of this buys a tiebreak.
# =============================================================================

# --- P1 -- MOUTH-FIRST BELT (Magnus, coordination.md 2026-08-20T05:30:51Z) ---
#
# VERBATIM: "build the first conveyor on the edge of the core, so they need to
# peck if they want to barrier".
#
# THE DEFECT IT KILLS, named by this repo in its own words at doctrine.py:1828
# and :1866 (the LOKI-L4 block): "`_link_path` plans one route when a harvester
# is built and `_build_next_link` pops each tile as it lays it; after the queue
# drains there is no planner left" ... "the rest are DEAD HEADS -- chains this
# bot abandoned mid-walk".  `LOKI_L4_REPAIR_ON` repairs HOLES (chain on both
# sides) and by construction can never reach a HEAD, and `PAVE_TRAIL_ON` is
# False.  So a belt whose TERMINAL socket is taken while the queue drains is
# dead for the rest of the match.
#
# THE MEASURED PRICE (`docs/research/AUTOPSY-crater-vs-sweep-2026-08-20.md`,
# 900 local games `_v525flip` vs `_x3r0v165mjolnirB`): the enemy first stands on
# one of OUR 8 core-ring sockets at **r7.2** on the crater maps against our belt
# head arriving one step from home at **r80.6** -- and `titanium_collected` at
# r100 is **0 in 60 of 60 icefloe games**.  `margin = eseal1 - head2` sorts the
# 15-map pool with no exceptions: all 5 maps with margin < 0 fail the belt in
# 30-49 of 60 games; all 10 maps with margin >= 0 finish it 60/60.
#
# ⭐ THE FIX IS AN ORDERING, NOT A MECHANISM.  Same chooser (v528 CONNCOST picks
# the ore), same router (`_link_path`, unchanged), same tiles, same count of
# conveyors.  The ONLY change is that the path is laid CORE-OUTWARD instead of
# ORE-INWARD, and the harvester is built LAST at the far end:
#   link 1 = a delivery socket of OUR core, laid at r1-3 (the body spawns on the
#            ring, so it is already adjacent);
#   link i = extends toward the chosen ore, FACING BACK along the chain;
#   harvester = the final action, once the chain is whole.
# ⇒ THE CHAIN IS CORE-CONNECTED FROM TILE 1 AND THE DEAD-HEADS CLASS CANNOT
# EXIST: there is no "last step" left to lose, because it was the first one.
#
# ⭐ AND IT IS ALSO A DENIAL.  A socket we hold is a socket their barrier cannot
# take (`can_build_barrier` needs an empty tile).  Their only removal is a peck
# -- 2 dmg for 2 Ti, ten visits to break a 20 HP conveyor -- delivered inside
# our own core-heal envelope, where a builder of ours restores 4 HP for 1 Ti.
# The exchange rate on our own ring is 4:1 in our favour and it is the reason
# Magnus asked for the socket rather than a barrier.
#
# ⛔ WHAT IT COSTS, STATED BEFORE THE BATTERY: the first harvester lands LATER
# by roughly the chain length.  Delivery cannot begin before the chain is whole
# either way, so the loss is the handful of stacks that would have queued on a
# partial belt -- but on a board where the belt WOULD have finished, this is a
# strictly later harvester for the same delivery.  The DELIVERY panel and
# `harv1` are the columns to read it on.
FS_V530_MOUTH = True

V530_MOUTH_MAX_RND = 40     # opening window.  A body issued an eco seat after
                            # this round uses the parent's ore-first order --
                            # the socket race is already decided by then
                            # (measured enemy arrival r5.5-r10.3 on the craters)
                            # and front-loading a long chain mid-game is a
                            # different, unbudgeted plank.
V530_MOUTH_MAX_LINKS = 6    # chains longer than this fall back to the parent.
                            # glacierkeep's nearest ore is 11 BFS steps out and
                            # a 16-link front-loaded chain is ~32 rounds of one
                            # body's actions before the first stack exists.
                            # FINITE AND GENEROUS: the pool's deepest crater ore
                            # is 11, so this does not silently exclude the maps
                            # the plank is for.
V530_MOUTH_TTL = 60         # rounds from arming before this body abandons the
                            # mouth chain and reverts to parent behaviour.  ⭐
                            # NOT OPTIONAL: the body waits adjacent to its next
                            # link when it cannot afford one, and a permanently
                            # unaffordable link would park an eco seat forever.
                            # An abandonment leaves the belt exactly as the
                            # parent's abandonment does -- a partial chain that
                            # is CORE-CONNECTED rather than a dead head.
FS_V530_MOUTH_SEATS = True  # every "expand" seat inside the window, not just
                            # the first.  Each eco seat already lays its own
                            # chain to its own partition's ore; this reorders
                            # each of them, it does not add chains.

# --- P1b -- THE FULL CONVEYOR RING (Magnus, mid-build extension) -------------
#
# VERBATIM: "3rd builder works on connecting a harvestor and 4th builds a
# conveyor ring around our core".
#
# ⛔⛔ A CANDIDATE, NOT A COMMITMENT, AND THE FLAG STATE SAYS SO.  Magnus,
# direct: "an idea we could try, not sure its worth it".  It therefore ships
# **FALSE** and is measured as its OWN isolation arm (`ring` = the fired v530
# plus this flag alone).  ⭐ THE REASON IT IS OFF RATHER THAN ON-WITH-AN-
# ABLATION: the primary read of this build is the MOUTH, and a candidate that
# turns out to cost opening tempo would contaminate the arm the mouth is read
# on.  A default of False makes the ring an INCREMENT measured against the
# package, which is the question actually asked.  The mouth is not contingent
# on the ring's verdict in either direction.
#
# THE MECHANISM CLAIM: a conveyor accepts from THREE sides and outputs to the
# fourth (game rules).  Point all 8 of our orthogonal sockets INTO the
# footprint and every one of them is a mouth -- so any belt of ours that merely
# TOUCHES the ring anywhere, from any direction, delivers.  The single-mouth
# form (P1) has to thread a specific tile; the ring form only has to arrive.
# ⇒ THE THING TO MEASURE IS NOT "more sockets", IT IS "does connection stop
# being a threading problem": `ti_coll100 > 0` for belts that reach the ring
# without ever being routed to one nominated socket.
#
# ⛔ FOUR COSTS, ALL PRICED IN THE BATTERY, NOT ASSERTED:
#   1. ~24-30 Ti of conveyor plus ~8% cost scale (+1% per conveyor) spent in
#      the opening window, against a 500 Ti bank the raid also wants.
#   2. One body's actions for ~16 rounds -- Magnus's script gives it a
#      dedicated 4th builder; this build has no spawn-order script (see below).
#   3. It does NOT block a gunner ray unless a conveyor is an obstacle to one.
#      ⛔ UNVERIFIED AT WRITE TIME -- the barrier form (P2) is the only ray
#      block this build claims until an engine probe says otherwise.
#   4. Ring conveyors sit on the 8 HEAL SEATS.  This is measured-safe rather
#      than assumed: doctrine.py:599-616 records that conveyors and splitters
#      are BOT-PASSABLE, so a healer stands ON a paved seat and heals normally,
#      and `HS_SEAT_BAN_CONVEYORS = False` is the shipped configuration
#      precisely because banning them collapsed delivery (270 Ti vs 23,930 on
#      the paired game).  A BARRIER on a seat would be the fatal version, which
#      is why P2 is corners-only.
#
# ⛔ THE SPAWN-ORDER SCRIPT IS NOT IN THIS BUILD.  Magnus's intended sequence is
# eco-1 mouth -> launcher, raider at r2, eco-2 harvester, builder 4 ring.  This
# tree's roster is fixed elsewhere (LOKI_ECO_SEATS / LOKI_DEFEND_SEAT /
# fs_crew_seat, main.py) and re-cutting it is a separate plank with its own
# blast radius.  What ships here is the ring BEHAVIOUR on the roster that
# exists: the designated seat prioritises it, every other home body lays a
# socket opportunistically when it happens to stand beside a free one.  The
# script itself is flagged for v531 integration.
FS_V530_RING = False

V530_RING_SEAT = 3          # the "4th builder" of Magnus's script, 0-indexed.
                            # ⚠ In THIS roster that seat is not guaranteed to
                            # be a home body (it is `fs_crew_seat` when the
                            # ferry-siege crew is on, and it defects to the raid
                            # at `harv >= ECO_NEED`), so the seat is a STEER,
                            # not a gate: the ring still gets laid by whichever
                            # home body passes a free socket.  That divergence
                            # from the script is the v531 item.
V530_RING_MIN_RND = 1
V530_RING_MAX_RND = 60      # the ring is an OPENING structure or it is nothing:
                            # its whole claim is arriving before their seal
                            # (measured r5.5-r10.3 on the craters).
V530_RING_TI_FLOOR = 12     # bank kept clear of the ring's spending.
V530_RING_MAX = 8           # sockets this body will pave.  8 = the whole ring;
                            # a bound exists so a mis-cached anchor cannot turn
                            # this into an unbounded paver (the LOKI-13
                            # PAVE_TRAIL failure was 38.2 conveyors/game).

# --- P2 -- CORNER BARRIERS (Magnus, s51 home-defence package) ----------------
#
# THE MEASUREMENT (`docs/research/REPLAY-STUDY-notadgato-v23-2026-08-20.md` §2,
# 80 modern rated games, damage ledger closed to the shot classifier with ZERO
# residual on six channels): **51.4% of all core damage we take is gunner fire,
# and 26.6% of it is fired from the 12 ring tiles themselves.**  They build 88
# gunners on our ring in 50 of 80 games -- **44 on orthogonal seats, 44 on the
# CORNERS**.  A gunner ray is obstacle-blocked; a sentinel ray is not (game
# rules, not inference), and this tree's OWN doctrine states the mechanism in
# the offensive direction at doctrine.py:1232: "our own barrier collar blocks a
# gunner ray".
#
# ⭐ THE FORM IS THE 4 CORNERS ONLY, AND THAT IS A DESIGN CONSTRAINT RATHER THAN
# A BUDGET.  The 8 orthogonal seats are the only tiles a conveyor can deliver
# into our core from and the only tiles a builder can heal it from
# (`heal_seats`, eco.py:275).  A full 12-tile self-seal would strangle our own
# economy and our own core heal.  The 4 diagonals are in NEITHER set:
# `core_corners` (eco.py:291) is disjoint from `heal_seats`, from
# `_link_goals`, from `_seat_ban` and from `_free_seats`, and its own docstring
# records that a body on a corner "is orthogonally adjacent to ... no Core tile
# -- so a corner is a BUILD station ... and never a peck station".
# ⇒ ~12 Ti, no delivery seat lost, no heal seat lost, and it denies the 44 of
# their 88 ring gunners that were built on corners.
#
# ⛔ THE COST THIS DOES PAY, AND IT IS REPORTED NOT RESOLVED: the 12 ring tiles
# are also our 12 SPAWN tiles.  Four barriers remove four of them.  With two
# delivery seats paved and bodies standing on others, the spawn margin narrows.
# `V530_CORNER_KEEP` leaves one corner open as the standing hedge, and the
# battery reads our own spawn count.
FS_V530_CORNERS = True

V530_CORNER_MAX_RND = 120   # build window.  Their ring gunners land at a median
                            # r116 and their gunner siting only creeps to our
                            # doorstep from r30 (median d^2 244 for r0-30 builds,
                            # 10 thereafter), so the barrier has to be standing
                            # before that, not after.
V530_CORNER_MIN_RND = 2     # not round 0-1: the opening spawn burst needs the
                            # ring, and P1's socket is the higher-value claim.
V530_CORNER_KEEP = 1        # corners deliberately LEFT OPEN, as spawn headroom.
                            # 1 of 4 -- the hedge against the self-spawn-denial
                            # cost above.  0 seals all four.
V530_CORNER_TI_FLOOR = 24   # global titanium below which a corner barrier is
                            # not bought.  A barrier is 3 Ti at base scale; this
                            # floor keeps it from outbidding the 20 Ti harvester
                            # or the 3 Ti mouth link on a tight opening round,
                            # which is why the clause also sits BELOW both in
                            # the action ladder.

# --- P3 -- DOOR-LAUNCHER KILL (Magnus, coordination.md 2026-08-20T05:32:21Z) -
#
# VERBATIM: "we need to kill enemy launchers at our core, sentinel?"
#
# WHY THE LAUNCHER AND NOT THE GUNNER: a launcher at our door needs its target
# at d^2 <= 2 to pick it up, and it throws to 1 <= d^2 <= 26.  It costs no ammo,
# has no facing to dodge, and every one of our home builders -- the healers, the
# eco seats, the defender -- stands inside its envelope by job description.  It
# converts our home crew into their ferry.
#
# ⭐ THE SENTINEL IS THE TOOL AND THE ARITHMETIC IS THE ARGUMENT: r^2 = 32 attack
# against a pickup envelope of d^2 <= 2, dmg 18 against 30 HP -- two shots, four
# rounds, 20 ammo, fired from outside the envelope, through bodies (the sentinel
# ray ignores obstacles).  A gunner is the budget option: r^2 = 13, 7 dmg, five
# shots, and its ray is body-blockable.  ⛔ A BUILDER PECK IS FORBIDDEN: 2 dmg
# for 2 Ti at orthogonal adjacency means the pecker stands INSIDE the pickup
# envelope for fifteen rounds.  The pecker IS the farm.
#
# ⛔ THIS IS A TARGETING RULE FOR TURRETS THAT ALREADY EXIST, NOT A BUILD PLANK
# AND NOT A BUILDER RESPONSE.  `FS_HOME_TURRET_RESPONSE` stays True and
# `FS_V515_DOOR_OFF` stays True -- i.e. the v513 builder door-peck path stays
# exactly as shut as it is in the parent, and this build does not re-enable it.
# What is reused is the DETECTION SHAPE of `_door_turret` (main.py:1560-1613,
# currently dead code behind FS_V515_DOOR_OFF), rewritten to take the home
# anchor as a parameter -- because a turret's `self.core` is None by design
# (main.py:2054-2059) and the original reads it.
#
# ⛔ THE LADDER IS NOT ENOUGH ON ITS OWN.  `TURRET_PRIO` (main.py:53) ranks an
# enemy LAUNCHER 4th, below CORE/SENTINEL/GUNNER/BUILDER_BOT -- but a GUNNER
# never reaches that table: the fast path at main.py:2130-2144 fires whatever
# `get_gunner_target()` returns and RETURNS.  So the plank is two clauses, a
# ladder promotion and a gunner pre-empt, or it is a sentinel-only plank
# wearing a both-turrets label.
# ⛔ AND THE PROMOTED VALUE IS NEGATIVE, NEVER 0: main.py:2186-2191 uses
# `best_prio == 0` as its proxy for "that shot was at their core" and would
# miscount silently.
FS_V530_DOORKILL = True

V530_DOOR_DSQ = 40          # d^2 from OUR core footprint inside which an enemy
                            # launcher is a DOOR launcher.  Deliberately the
                            # same number as FS_DOOR_DSQ (doctrine.py:2654) so
                            # the two door definitions in this tree cannot drift
                            # apart; HUNT_BAND_DSQ = 41 is the neighbouring
                            # turret-side "home band" and is within a tile of it.
V530_DOOR_PRIO = -1         # ladder value for a door launcher.  NEGATIVE by
                            # necessity -- see the v517 core-shot note above.
V530_DOOR_GUNNER = True     # the gunner pre-empt clause.  False makes P3 a
                            # sentinel-only plank, which is the honest ablation
                            # for "did the gunner half do anything".

FS_V530_LOG = False         # stderr instrument tapes (MOUTH / CORNER / DOOR),
                            # off in the fired build; the instrumented arm sets
                            # it.  Gated on its OWN flag, never on the master,
                            # so the control arm emits the same tape from the
                            # same code (the v528 convention, eco.py:2240).


LOKI_FS_V530 = True         # master.  False == `_v529merge` exactly.


# =============================================================================
# v530.1 -- THE HARVESTER-BOOTSTRAP FIX (s51, 2026-08-20).  Parent: `_v530home`.
# =============================================================================
# ⛔ THE ONE DEFECT THIS TREE EXISTS TO FIX, and nothing else is touched.
# `BUILD-REPORT-v530home-2026-08-20.md` §5.3/§11: the mouth-first plank is
# DESIGN-VALIDATED (socket race `eseal1 - head1` -14.0 -> +21.1, beltfail
# 36.7% -> 9.4% on the crater panel, dead-heads 2.5% -> 0.0% on the sweep
# panel) and its FIRED CONFIG is blocked by an implementation tail:
#
#     first harvester, MEDIAN round   5 -> 8      (the +3 the design predicted)
#     first harvester, MEAN   round   5.79 -> 42.74
#     games with NO harvester EVER    0.000 -> 0.096      <- THE DEFECT
#     first harvester after r60       0.000 -> 0.154
#
# THE MECHANISM, named at §11.2 and confirmed by reading the call site
# (`eco.py::_expand`): `_v530_mouth_live()` is consulted PER BODY and the
# harvester bootstrap block is written `if not _v530_chain and (...)`.  Every
# eco seat that arms a mouth chain therefore suppresses ITS OWN bootstrap for
# as long as the chain is live -- up to `V530_MOUTH_TTL = 60` rounds when the
# body is parked beside an unaffordable or blocked link.  `LOKI_ECO_SEATS` is
# (1, 2, 3), so on a normal roster TWO bodies arm mouth chains within a couple
# of rounds of each other and the TEAM holds no harvester at all until one of
# them terminates.  When both chains are long (auroraveil `ore_d1` 8,
# glacierkeep 11) or both stall, the match can end before either does.
#
# ⭐ THE FIX IS A SEAT RULE, NOT A CHANGE TO THE PLANK.  The mouth builder
# still claims the socket at r1-3 with the same chooser, the same router, the
# same tiles and the same facing rule; what changes is that it is now the ONLY
# body allowed to be in chain phase while the team owns no harvester.  Every
# other eco seat runs the PARENT's ore-first order until a harvester exists,
# which is `_v529merge` behaviour and reads 0.000 on the defect column by
# construction.  Once the team owns a harvester the other seats may arm mouth
# chains exactly as `_v530home` does -- the plank is delayed on those seats,
# never removed.
#
# ⛔ WHY NOT §11's OTHER CHEAP TEST ("exempt the FIRST harvester of the match
# from the suppression").  It does not reach the failure: `_v530_mouth_act` is
# ranked ABOVE the bootstrap and returns True whenever it lays a link, so the
# suppression only bites in the rounds the mouth is STALLED -- and in exactly
# those rounds `_v530_mouth_move` also owns the body's move and steers it along
# the chain rather than to ore.  Un-suppressing the bootstrap alone would let
# the body build a harvester only if it happened to be standing beside one.
# The seat rule removes the body from the chain instead, which is what makes
# the parent's walk resume.
#
# ⛔ AND `FS_V530_MOUTH_SEATS` IS A DEAD FLAG IN THE PARENT.  §11.2 offers
# "`FS_V530_MOUTH_SEATS = False`, already a flag" as the cheap test; it is
# declared at doctrine.py:5373 and read in ZERO places in the whole tree
# (`grep -rn FS_V530_MOUTH_SEATS bots/_v530home` returns the definition and
# nothing else).  Setting it would have changed nothing and the test would have
# read as a null.  It is left exactly as it is here -- still dead, still
# unread -- so that this tree differs from its parent in one place only.
#
# ⭐ READ-SITE ONLY, same convention as the v530 block above: both names below
# are read inside the branch they guard, so `FS_V5301_BOOTFIX = False`
# reproduces `_v530home` byte-for-byte and `LOKI_FS_V530 = False` still
# reproduces `_v529merge`.  `flagoff_ast.py` asserts it.
FS_V5301_BOOTFIX = True

V5301_MOUTH_SEAT = 1        # the ONE seat allowed to run mouth-first before
                            # the team owns a harvester.  Seat 0 is the raid
                            # departure and seats 1-3 are `LOKI_ECO_SEATS`, so
                            # seat 1 is the FIRST body ever issued an eco seat
                            # and the earliest possible socket claim -- which
                            # is the half of the plank that wins the race
                            # (measured enemy arrival r5.5-r10.3 on the
                            # craters).  A seat number rather than a comms-store
                            # claim on purpose: store writes are buffered one
                            # round and the two eco seats are issued on
                            # CONSECUTIVE rounds, so a claim written by seat 1
                            # is not yet visible when seat 2 first runs.
V5301_MOUTH_OPEN_AFTER_HARV = True
                            # once the team owns a harvester the other eco
                            # seats arm mouth chains exactly as `_v530home`
                            # does.  False makes mouth-first a permanently
                            # single-seat plank and is the honest ablation for
                            # "was the delay enough, or is the second chain
                            # itself the cost".


# ===========================================================================
# v532 "WEAVE" (s52, 2026-08-20) -- ONE DEFECT, ONE BAN SET
# ===========================================================================
#
# THE DEFECT `_v531fix` LEFT BEHIND, AND ITS OWN REPORT NAMES IT WRONG.
# `BUILD-REPORT-v531fix-2026-08-20.md` §6 measured that on atoll v531fix wins
# 10/60 against its parent's 35/60 with `harv_wired30` 0.57 against 2.00, and
# diagnosed it as "seat 2 runs the PARENT's ore-first build order over the same
# ground and occupies a planned mouth tile first".
#
# ⛔ THAT DIAGNOSIS IS FALSIFIED ON THE ENGINE.  Instrumented v531fix, atoll,
# 30 games (15 seeds x 2 seats, opp `_v488beltbreak2`), every `_v530_mouth_act`
# occupied-pop classified by owner / type / facing:
#
#     58  OURS, CONVEYOR, already facing the way we wanted   BENIGN (adoption)
#     30  OURS, BARRIER                                      HOLE, fatal
#      0  enemy anything
#      0  wrong-facing conveyor of ours
#
# and **30 of 30** of those barrier tiles are a tile our OWN `V530 CORNER` line
# (P2) built ONE ROUND EARLIER -- corner at r3, mouth wants it at r4, in every
# game of the 30.  The collision is P1 against P2, both v530 planks, on our own
# core ring.  It is not a build-ORDER conflict at all.
#
# WHY IT IS A RACE AND WHY ONLY THE MOUTH LOSES IT.  `_link_path` already
# blocks friendly non-BELT buildings it can see (`eco.py`, the
# `et not in BELT_TYPES` arm of the `get_nearby_buildings` loop), so a chain
# planned AFTER the corner exists routes around it by construction -- measured:
# the parent's ore-first order took 52 occupied-pops over 10 atoll games and
# **not one** of them was a barrier of ours (41 our conveyor, 9 ENEMY barrier,
# 1 our gunner, 1 our sentinel).  The mouth plans at r2, BEFORE P2's r3 build,
# so it plans straight through a tile our own team is about to seal.  And the
# seat rule is what exposed it: under `_v530home` seat 2 was laying its own
# mouth chain at r2-r6 and never stood beside a corner; under `_v5301`'s seat
# rule seat 2 is free, walks the ring, and builds the corner that kills seat
# 1's chain.
#
# THE FIX, AND IT IS THE SMALLEST ONE THAT IS PROVABLE: plan as if the corners
# were already built, because they will be.  `_link_path` grows one optional
# `extra_block` argument (default None -> the parent's exact flood) and
# `_v530_mouth_arm` passes the 4 diagonal core corners when -- and only when --
# P2 is the plank that will actually take them (`FS_V530_CORNERS`).  No
# reservation, no comms slot (all 16 are already assigned), no destroy-and-
# rebuild, no re-plan, and NOTHING outside the mouth's own planner: a chain
# that can no longer be routed simply is not armed, and the body falls back to
# the parent's ore-first order, which is the behaviour the map had before v530.
#
# ⭐ WHY NOT ADOPT-OR-REFACE (the other candidate, and §11.1(a) of the v531fix
# report).  Adoption is ALREADY what the code does -- the occupied branch pops
# the tile into `mouth_prev`, so the next link is faced at it -- and the 58
# conveyor pops above show it working: every one was already facing the way we
# wanted.  The case it does not cover is a tile that cannot carry flow at all,
# and for a BARRIER "reface" means destroy-and-rebuild: 3 Ti, an action, and a
# fight with the plank that wants that tile.  Not planning through it costs
# nothing and cannot ping-pong.
#
# WHAT IT CANNOT FIX, STATED BEFORE THE BATTERY: an ENEMY building that lands
# on a planned tile after r2 still pops as a hole (0 observed on atoll, 9 enemy
# barriers observed on the PARENT-order chains in the same games), and a
# friendly turret built on a planned ring tile after r2 does too (2 observed,
# on parent-order chains).  Those are a different plank.
#
# READ-SITE ONLY, same convention as the v530 and v530.1 blocks: the name below
# is read inside the branch it guards, so `FS_V532_WEAVE = False` reproduces
# `_v531fix` byte-for-byte, `FS_V5301_BOOTFIX = False` still reproduces
# `_v530home`, and `LOKI_FS_V530 = False` still reproduces `_v529merge`.
# `flagoff_ast.py` asserts it.
FS_V532_WEAVE = True
...
# disagrees with terrain we have actually looked at.  Refutations are memoised
# per (w, h, anchor) -- terrain is static, so a disagreement can never un-happen
# -- and ONLY refutations taken on a building-free tile are stored, so nothing
# our own team builds can poison the memo.  Positive verdicts are deliberately
# NOT memoised: each call re-verifies against whatever vision the asking unit
# has now, which buys opportunistic re-verification for free.
...
#
# THE CODES BELOW ARE NOT RETYPED.  `scratchpad/s52_v534_build/
...
#
# READ-SITE ONLY, same convention as the v524/v530/v532 blocks: every name
# below is read inside the branch it guards, so `FS_V534_MAPTRUST = False`
# reproduces `_v533home` byte-for-byte.  `flagoff_ast.py` asserts it.
FS_V534_MAPTRUST = True     # master.  False == `bots/_v533home` unchanged
...
# ═════════════════════════════════════════════════════════════════════════════
# v535 -- CORNER GATE.  The home package stops being map-INVARIANT.
# ═════════════════════════════════════════════════════════════════════════════
#
# WHAT THIS FIXES, AND THE MEASUREMENT THAT NAMED IT.  s52's REFUSING
# DECOMPOSITION battery (coordination.md 2026-08-20T12:22:11Z; ws2, 6 arms,
# 240 games/arm, 0 shells, 0 tracebacks) took the home package apart ON THE
# BOARDS THE FERRY-SIEGE REFUSES and found ONE plank carrying the deficit:
#
#     arm        share%   arch/120  midg/120   d_vs_v533home (pairwise hw +-8.9)
#     parent      50.4      50        71        +20.8
#     floff       46.7      38        74        +17.1   (master-off ~ parent)
#     coff        43.8      54        51        +14.2   <-- OUTSIDE hw
#     doff        35.4      24        61         +5.8   (inside)
#     moff        31.2      30        45         +1.7   (mouth acquitted)
#     v533home    29.6      19        52          --
#
# `coff` is CORNERS OFF.  Its +14.2 [5.3, 23.1] excludes zero at pairwise
# power, and the damage is ARCHIPELAGO-CONCENTRATED (v533 19/120 = 15.8% vs
# coff 54/120 = 45.0%; midgard flat, 52 vs 51).  Mouth and door-kill are
# acquitted on this axis.  diagA2 (2026-08-20T09:18:21Z) reads the same
# direction from the other side: `nocorner` was the only ablation that
# RECOVERED wins to parent parity (-0.4) and moved the r300 bar +3.1.
#
# ⛔ WHAT THIS BLOCK DOES *NOT* CLAIM.  Both readouts are DIAGNOSTIC, self-play
# is POLICY, and the refusing battery is a TWO-MAP panel.  Nothing here is a
# currency claim; the powered confirm is the merged battery registered for
# BOTH obligations (this plank's admission bars + HOMEPOOL's F3 attribution).
# The MECHANISM below is what this build ships; the EFFECT SIZE is deferred.
#
# THE DESIGN GAP, stated as the fix.  `FS_V530_CORNERS` fires on EVERY board.
# It was designed against ONE picture -- the enemy ring-gunner creep onto our
# doorstep -- and that picture is a SIEGE-BOARD picture.  On a board the
# ferry-siege REFUSES, the bot plays the incumbent beltbreak+home game, our own
# bodies are the ones that have to move through the core ring all game, and
# four of the twelve ring tiles are also four of our twelve SPAWN tiles.  The
# v530 block already reported that cost in writing ("THE COST THIS DOES PAY,
# AND IT IS REPORTED NOT RESOLVED") and hedged it with `V530_CORNER_KEEP = 1`.
# The refusing decomposition says the hedge is not enough on those boards.
# So: keep the plank where it was designed to work, and stand it down where the
# siege itself already stands down.
#
# ⭐ THE PREDICATE IS NOT NEW AND IS NOT REIMPLEMENTED.  The verdict is
# `SiegeMixin._fs_map_gated` -- the SAME pure function `_fs_gate` calls -- read
# through `_v535_map_refuses` (siege.py).  That keeps ONE definition of "this
# board refuses" in the tree.  Consequences that follow for free and are the
# reason to reuse rather than copy:
#   * it is GEOMETRY AND GRIDS ONLY: min-dim / min-core-d^2 floors, the
#     grid-CONFIRMED `FS_MAP_SKIP` set (v534 F2) and the grid-CONFIRMED cripple
#     list (v524 change 1).  No map NAMES reach a bot, per the map-robustness
#     policy (F3); an unsurveyed board is treated as "plays normally", which is
#     the registered default on both of those lists.
#   * every future correction to the gate moves this plank with it.
#
# WHERE IT BINDS, MEASURED not assumed, over the whole 34-map local pool from
# BOTH seats (68 cells; `scratchpad/s52_v535_build/gatemap.py`, whose 8 mutants
# drive the predicate to both verdicts):
#
#   REFUSING (8 maps, 16 of 68 cells, seat-symmetric everywhere):
#     archipelago, heart, lighthouse, moonrise, saga, snowflake
#         -- `FS_MAP_SKIP`, and GRID-CONFIRMED (v534 F2)
#     midgard -- `FS_V525_CRIPPLE_MAPS`, GRID-CONFIRMED (v524 change 1)
#     inv_tiny8 -- 8x8, under `FS_V525_MIN_MAP_DIM` = 10
#   RUNNING: the other 26, including atoll, drakkarfjord, glacierkeep,
#     nordkap, ragnarok, valkyrie, yulerune.
#
# ⭐⭐ AND THE SURPRISE, WRITTEN DOWN BEFORE IT IS EXPLAINED AWAY.  This build
# went in expecting fjordgate and jackpot to be the canonical refusals -- that
# is what `_fs_gate`'s own docstring still says, in this tree, today:
# *"fjordgate ... is the game the 2174-rated version LOST ... jackpot is the
# other refusal"*.  BOTH RUN.  fjordgate runs because **v525's standdown flip
# lowered the floors to fjordgate's OWN dimensions** (`FS_V525_MIN_MAP_DIM = 10
# # fjordgate's own larger side`, `FS_V525_MIN_CORE_DSQ = 32 # fjordgate's own
# core d^2`) and both tests are STRICT `<`, so the map the floors were named
# after is now the first map that passes them.  jackpot runs because it was
# never encoded on any list at all -- the docstring records a probe finding
# that no constant ever carried.  ⇒ THE PROSE IN `_fs_gate` IS A STALE
# DESCRIPTION OF ITS OWN PREDICATE, and had this build trusted it instead of
# measuring, the gate would have been described in the report as binding on two
# maps where it does not bind and missing midgard, where it does.  Filed as a
# doc-vs-code drift item; the CODE is what shipped and the CODE is what the
# table above reports.  (Same family as the two CLAUDE.md precedents: a fact in
# a reference nobody re-derives is a fact nobody has.)
#
# ⚠ ATOLL RUNS -- which matters, because atoll is the board the v532
# mouth-weave fix was measured on, and that fix is corner-conditional (see
# below).  The v532 cell is untouched by this gate.
# ⭐ AND THE TWO BOARDS THE REFUSING DECOMPOSITION USED -- archipelago and
# midgard -- BOTH READ `refuse = 1`.  That is the fixture and the gate agreeing
# without being made to: the panel was assembled from HOMEPOOL's refusing cut,
# and this predicate, asked independently, returns the same two boards.
#
# TWO READ SITES, GATED TOGETHER ON PURPOSE.  `FS_V530_CORNERS` is read in two
# places and they are a PAIR:
#   1. `eco._v530_corner_act` -- the build itself.
#   2. `eco._v530_mouth_arm` (v532 WEAVE) -- which BANS the four corner tiles
#      from the mouth chain "because they will be built".
# Gating only (1) would leave (2) routing the opening chain around four tiles
# nothing is ever going to take -- a pure loss, and it would land on exactly
# the refusing boards this fix exists for.  So both sites read ONE predicate,
# `eco._v535_corners_on(ct)`, and the gate moves them together.  That is one
# mechanism, not two.
#
# COST.  One `_fs_map_gated` call per BODY per GAME (cached on `self`, the same
# compute-once-per-unit discipline `_fs_gate` uses), and it is usually free
# outright: on any board whose signature is not on either list the function is
# three integer comparisons and never touches a tile.  On a signature hit it
# may call `known_map_for` once -- tile-sense plus compare against a
# cache-memoised decode -- which the v524 block already prices as cheap and
# already pays on the siege path.  No comms-store slot is taken.
#
# READ-SITE ONLY, same convention as the v524/v530/v532/v534 blocks: both names
# below are read inside the branch they guard, so `FS_V535_CORNER_GATE = False`
# reproduces `bots/_v534maptrust` byte-for-byte -- and on that setting the
# siege helper is never called at all.  `test_flagoff.py` asserts both halves.
LOKI_FS_V535 = True         # master.  False == `bots/_v534maptrust` unchanged
FS_V535_CORNER_GATE = False  # corners stand down on boards the siege refuses
FS_V535_LOG = False         # stderr tape: one GATE line per body per game.
                            # LOCAL instrument only -- platform replays strip
                            # stdout (CLAUDE.md, s28), so nothing may read it
                            # off a live replay.
