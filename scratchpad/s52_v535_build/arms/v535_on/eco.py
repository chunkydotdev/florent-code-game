...
class EcoMixin:
    ...
    def _link_path(self, ct, hpos, extra_block=None):
        """v532: `extra_block` is an optional set of (x, y) this flood must
        treat as WALL on top of the pave ban.  Default None reproduces the
        parent flood exactly -- every parent call site passes nothing, and the
        only caller that passes anything is `_v530_mouth_arm` under
        `FS_V532_WEAVE`.  See the v532 block in doctrine.py: the mouth plans at
        r2 and our own corner barriers land at r3, so the mouth is the one
        planner that cannot SEE the buildings its own team is about to place.
        """
        ban = self._pave_ban()
            ...
            if ban is not None:
                for bx, by in ban:
                    if 0 <= bx < mw and 0 <= by < mh:
                        blk[(by + 1) * w2 + bx + 1] = 1
            if extra_block:
                for bx, by in extra_block:
                    # NEVER the start: re-blocking the harvester's own tile
                    # makes the flood unreachable rather than re-routed, and
                    # the caller's ban set is about tiles on the WAY.
                    if (bx, by) != start_xy and 0 <= bx < mw and 0 <= by < mh:
                        blk[(by + 1) * w2 + bx + 1] = 1
            try:
                for eid in ct.get_nearby_buildings():
        ...
        while q:
            ...
            for dx, dy in CARD_DELTAS:
                ...
                if key in prev or not (0 <= nx < mw and 0 <= ny < mh):
                    continue
                if extra_block and key in extra_block and key != start_xy:
                    continue
                n = Position(nx, ny)
    ...
    # --- v530 P1: MOUTH-FIRST BELT -----------------------------------------
    #
    # See the doctrine.py v530 block for the mechanism and the measured price.
    # Everything here is dead code with `LOKI_FS_V530 = False`, and every exit
    # sets `self.mouth_done` so the parent's ore-first order resumes rather
    # than the body stalling.
    #
    # ⛔ THE ROUTER IS NOT REPLACED.  `_link_path` is called verbatim and its
    # output is REVERSED.  Same tiles, same count, same v528 CONNCOST ore.  The
    # facing rule is the only thing that has to invert with the order: in
    # ore-first order the coreward neighbour is `link_queue[1]`, in mouth-first
    # order it is the tile laid immediately BEFORE this one, so this code
    # carries `self.mouth_prev` where `_build_next_link` reads its own queue.

    def _v530_mouth_live(self):
        """Is this body inside the CHAIN phase of a mouth-first plan?

        ⛔ THE CHAIN PHASE ONLY, AND THE NARROWNESS IS THE POINT.  Measured on
        the first dose of this build (48 games, instrumented): with the parent's
        own chain planner and harvester bootstrap left reachable while a mouth
        chain was live, the two ran CONCURRENTLY -- the parent built a harvester
        on an adjacent ore, `_wire_on_build` queued a SECOND chain into
        `link_queue`, and that queue was never walked because the mouth mover
        owned the body's move.  117 chains armed, 453 links laid, and only 22
        terminal harvesters, with 61 chains dying on the TTL.  This predicate is
        what makes the two mutually exclusive.
        """
        return not self.mouth_done and bool(self.mouth_queue)

    def _v5301_may_arm(self, ct):
        """v530.1: may THIS body be in mouth chain phase right now?

        ⛔ THE WHOLE FIX, AND IT IS ONE PREDICATE.  `_v530_mouth_live()` is a
        PER-BODY state and the harvester bootstrap is suppressed per body, so
        two eco seats in chain phase at once leave the TEAM with no harvester
        at all -- measured at 9.6% of games with no harvester EVER against the
        parent's 0.0% (`BUILD-REPORT-v530home-2026-08-20.md` §5.3).  Only the
        designated seat may hold a chain while the team owns no harvester;
        every other eco seat runs `_v529merge`'s ore-first order until one
        exists, and then arms exactly as `_v530home` does.

        Returns True for every body when `FS_V5301_BOOTFIX` is False, which is
        what makes that flag reproduce `_v530home`.
        """
        if not FS_V5301_BOOTFIX:
            return True
        if self.role_n == V5301_MOUTH_SEAT:
            return True
        if not V5301_MOUTH_OPEN_AFTER_HARV:
            return False
        try:
            return ct.read_store(SLOT_HARVESTERS) > 0
        except Exception:
            return False

    def _v530_mouth_arm(self, ct):
        """Plan this body's CORE-OUTWARD chain.  True if a chain now exists."""
        if not self._v5301_may_arm(ct):
            # ⭐ NOT `mouth_done`.  A deferred seat must stay eligible: once the
            # team owns a harvester it arms on a later round, so the plank is
            # DELAYED on that seat, never removed.  `V530_MOUTH_MAX_RND` still
            # bounds how long that can go on.
            return False
        if self.core is None or not (self.mw and self.mh):
            self.mouth_done = True
            return False
        if ct.get_current_round() > V530_MOUTH_MAX_RND:
            self.mouth_done = True
            return False
        if self.link_queue or self.wire_pending:
            # This body is already mid-chain under the parent's order; the
            # reorder is an OPENING plank and must not interleave two chains.
            self.mouth_done = True
            return False
        ore = self._pick(ct)
        if ore is None:
        ...
        try:
            if ct.get_tile_env(ore) != Environment.ORE_TITANIUM:
                # `_pick`'s wander fallback returns a bearing, not an ore.
                return False
        except Exception:
            return False
        # ⭐ v532 THE FIX.  Plan as if the corners were already built, because
        # they will be: P2 barriers a diagonal core corner at r3 and this
        # planner runs at r2, so the pre-v532 chain routed straight through a
        # tile our own team was about to seal and popped it as "occupied" one
        # round later, leaving a hole the terminal harvester could not deliver
        # through (30 of 30 atoll games, doctrine.py v532 block).  Banned only
        # when P2 is actually the plank that takes them.
        # ⭐ v535 -- `_v535_corners_on` REPLACES the bare `FS_V530_CORNERS`
        # read here, and that is not incidental: "plan as if the corners were
        # already built, because they will be" is only TRUE where the corner
        # plank actually fires.  On a refusing board the gate stands the
        # corners down, so the ban must lift with them or the chain routes
        # around four tiles that stay open all game.
        xblock = None
        if FS_V532_WEAVE and self._v535_corners_on(ct):
            xblock = self._v530_corner_key_set() or None
        plan = self._link_path(ct, ore, xblock)
        if not plan or len(plan) > V530_MOUTH_MAX_LINKS:
            # ⛔ NOT a silent loss.  A chain the ban makes unroutable leaves
            # `mouth_done` set, which is the parent's ore-first order -- the
            # behaviour this map had before v530 -- not a stalled body.
            self.mouth_done = True
            return False
        self.mouth_queue = list(reversed(plan))
        self.mouth_ore = ore
        self.mouth_prev = None
        self.mouth_since = ct.get_current_round()
        if FS_V530_LOG:
            print("V530 MOUTH arm rnd=%d seat=%d ore=%d,%d links=%d sock=%d,%d"
                  % (ct.get_current_round(), self.role_n, ore.x, ore.y,
                     len(self.mouth_queue), self.mouth_queue[0].x,
                     self.mouth_queue[0].y), file=sys.stderr)
        return True

    def _v530_mouth_face(self, tile):
        """Which way this link must OUTPUT: back along the chain, coreward."""
        if self.mouth_prev is not None:
            f = tile.cardinal_direction_to(self.mouth_prev)
            if f != Direction.CENTRE:
                return f
        f = nearest_cardinal(tile.direction_to(nearest_core_tile(tile, self.core)))
        return Direction.NORTH if f == Direction.CENTRE else f

    def _v530_mouth_act(self, ct):
        """The action half.  True when this body spent its action."""
        if self.mouth_done:
            return False
        if self.mouth_ore is None and not self.mouth_queue:
            if not self._v530_mouth_arm(ct):
                return False
        if ct.get_current_round() - self.mouth_since > V530_MOUTH_TTL:
            # ⭐ THE TTL, AND IT IS LOAD-BEARING.  This body WAITS beside its
            # next link when it cannot afford one; without a bound an eco seat
            # that never gets funded parks forever.
            if FS_V530_LOG:
                print("V530 MOUTH ttl rnd=%d seat=%d left=%d"
                      % (ct.get_current_round(), self.role_n,
                         len(self.mouth_queue)), file=sys.stderr)
            self._v530_mouth_abandon()
            return False
        p = ct.get_position()
        if self.mouth_queue:
            # The socket -- link 1, the tile beside our own footprint -- is
            # v513 change C's ESSENTIAL spend by the same definition
            # (`titanium_collected` is 0 without a conveyor beside our core).
            ess = self.mouth_prev is None
            if not self._eco_spendable(ct, ct.get_conveyor_cost(), essential=ess):
                return False
            while self.mouth_queue:
                tile = self.mouth_queue[0]
                try:
                    occupied = ct.get_tile_building_id(tile) is not None
                except Exception:
                    return False
                if occupied:
                    if FS_V530_LOG:
                        # ⛔ INSTRUMENT ONLY, and it is the instrument that
                        # falsified §6: an occupied pop is BENIGN when the
                        # occupier is a conveyor of ours already facing the way
                        # we wanted (it carries the flow), and a HOLE when it
                        # cannot carry flow at all.  The pre-v532 tape could not
                        # tell those apart and counted both as "short chain".
                        try:
                            _bid = ct.get_tile_building_id(tile)
                            _mine = 1 if ct.get_team(_bid) == self.team else 0
                            _et = ct.get_entity_type(_bid).name
                        except Exception:
                            _mine, _et = -1, "ERR"
                        try:
                            _dir = ct.get_direction(_bid).name
                        except Exception:
                            _dir = "NONE"
                        try:
                            _want = self._v530_mouth_face(tile).name
                        except Exception:
                            _want = "ERR"
                        print("V530 MOUTH occ rnd=%d seat=%d tile=%d,%d mine=%d"
                              " et=%s dir=%s want=%s left=%d"
                              % (ct.get_current_round(), self.role_n, tile.x,
                                 tile.y, _mine, _et, _dir, _want,
                                 len(self.mouth_queue)), file=sys.stderr)
                    # Someone already stands there.  Face the NEXT link at it
                    # regardless of whose it is -- ours delivers, theirs is a
                    # dead end we cannot route around without the head re-plan
                    # this build deliberately does not ship.
                    self.mouth_prev = self.mouth_queue.pop(0)
                    continue
                break
            if not self.mouth_queue:
                return False
            tile = self.mouth_queue[0]
            if abs(p.x - tile.x) + abs(p.y - tile.y) != 1:
                return False
            f = self._v530_mouth_face(tile)
            try:
                if not ct.can_build_conveyor(tile, f):
                    return False
                ct.build_conveyor(tile, f)
            except Exception:
                return False
            self.mouth_prev = self.mouth_queue.pop(0)
            self.mouth_links += 1
            if self.mouth_sock_rnd < 0:
                self.mouth_sock_rnd = ct.get_current_round()
            if FS_V530_LOG:
                print("V530 MOUTH link rnd=%d seat=%d tile=%d,%d face=%s left=%d"
                      % (ct.get_current_round(), self.role_n, tile.x, tile.y,
                         f.name, len(self.mouth_queue)), file=sys.stderr)
            return True
        # Chain whole -- the harvester is the LAST action, not the first.
        ore = self.mouth_ore
        if ore is None:
            self.mouth_done = True
            return False
        if abs(p.x - ore.x) + abs(p.y - ore.y) != 1:
            return False
        harv = ct.read_store(SLOT_HARVESTERS)
        if not (self._eco_spendable(ct, ct.get_harvester_cost())
                and harv < self._eco_cap(ct)):
            return False
        try:
            if not ct.can_build_harvester(ore):
                self._v530_mouth_abandon()
                return False
        ...
        if harv + 1 >= ECO_NEED and not (LOKI_FS_V514 and FS_V514_ECOGATE):
            ct.write_store(SLOT_ECO_READY, 1)
        if FS_V530_LOG:
            print("V530 MOUTH harv rnd=%d seat=%d ore=%d,%d sock=%d links=%d"
                  % (ct.get_current_round(), self.role_n, ore.x, ore.y,
                     self.mouth_sock_rnd, self.mouth_links), file=sys.stderr)
        # No `_wire_on_build`: the route home already stands, tile for tile.
        self.mouth_ore = None
        self.mouth_done = True
        return True

    def _v530_mouth_abandon(self):
        self.mouth_queue = []
        self.mouth_ore = None
        self.mouth_prev = None
        self.mouth_done = True

    def _v530_mouth_move(self, ct):
        """The movement half.  True when this body owns its move this round."""
        if self.mouth_done:
            return False
        if not self.mouth_queue and self.mouth_ore is None:
            return False
        p = ct.get_position()
        if self.mouth_queue:
            nxt = self.mouth_queue[0]
            if p.x == nxt.x and p.y == nxt.y:
                return self._v530_mouth_stepoff(ct, p)
            if abs(p.x - nxt.x) + abs(p.y - nxt.y) == 1:
                return True             # adjacent: hold, build next round
            self.tgt = nxt
            self._nav(ct, pave=False)
            return True
        # ⛔ THE CHAIN IS WHOLE AND THIS BODY IS NO LONGER HELD.  The first
        # version of this clause returned True while the terminal harvester was
        # pending, which parked the body beside its ore for as long as the
        # harvester cap or the collar reserve refused the spend -- up to the
        # full TTL, doing nothing.  The plank owns the CHAIN; the harvester is
        # a preference expressed at the TOP of the action ladder and nothing
        # more.  Steering only, never holding.
        ore = self.mouth_ore
        if p.x == ore.x and p.y == ore.y:
            return self._v530_mouth_stepoff(ct, p)
        self.tgt = ore
        return False

    def _v530_mouth_stepoff(self, ct, p):
        """Vacate the tile we are about to build on, moving OUTWARD.

        The mouth-first walk stands on link i+1 to lay link i, so the body is
        on its own next tile every other round.  Preference order: the tile
        after next (keeps walking the chain), then the ore (the last link's
        only outward neighbour), then any legal step.
        """
        dirs = []
        if len(self.mouth_queue) >= 2:
            dirs.append(p.cardinal_direction_to(self.mouth_queue[1]))
        if self.mouth_ore is not None:
            dirs.append(p.cardinal_direction_to(self.mouth_ore))
    ...
    # --- v530 P1b: THE FULL CONVEYOR RING (candidate, FS_V530_RING) --------
    #
    # Every one of our 8 orthogonal core-ring sockets, paved with a conveyor
    # FACING THE FOOTPRINT -- so each is a mouth and a belt that touches the
    # ring anywhere delivers.  See the doctrine.py v530 P1b block: this ships
    # OFF and is measured as its own arm.

    def _v530_ring_keys(self):
        if self._v530_ring_seats is None:
            self._v530_ring_seats = heal_seats(self.core, self.mw, self.mh)
        return self._v530_ring_seats

    def _v530_ring_gate(self, ct):
        if self.core is None or not (self.mw and self.mh):
            return False
        rnd = ct.get_current_round()
        if rnd < V530_RING_MIN_RND or rnd > V530_RING_MAX_RND:
            return False
        if self.v530_ring >= V530_RING_MAX:
            return False
        return True

    def _v530_ring_act(self, ct, p):
        """Pave a free socket this body is already standing beside."""
        if not self._v530_ring_gate(ct):
            return False
        if ct.get_global_resources() < ct.get_conveyor_cost() + V530_RING_TI_FLOOR:
            return False
        for s in self._v530_ring_keys():
            ...
            self.v530_ring += 1
            if FS_V530_LOG:
                print("V530 RING rnd=%d seat=%d tile=%d,%d face=%s n=%d"
                      % (ct.get_current_round(), self.role_n, s.x, s.y,
                         f.name, self.v530_ring), file=sys.stderr)
            return True
        return False

    def _v530_ring_move(self, ct, p):
        """Steer the DESIGNATED body to the nearest unpaved socket.

        ⛔ STEER, NOT HOLD.  Returns False the moment there is nothing left to
        pave, so this body rejoins the economy rather than parking on the ring
        -- the same mistake the first mouth-first dose measured.
        """
        if self.role_n != V530_RING_SEAT or not self._v530_ring_gate(ct):
            return False
        best = None
        for s in self._v530_ring_keys():
            try:
            ...
            if d == 0:
                continue                      # standing on it: step off below
    ...
    # --- v530 P2: CORNER BARRIERS ------------------------------------------

    def _v535_corners_on(self, ct):
        """Is the CORNER-BARRIER plank live on THIS board?

        ⭐ v535 -- THE ONE PREDICATE.  `FS_V530_CORNERS` has two read sites
        (`_v530_corner_act`, the build; and `_v530_mouth_arm`'s v532 WEAVE ban,
        which reserves those tiles "because they will be built").  They are a
        PAIR: gating the build alone would leave the mouth planner routing the
        opening chain around four tiles nothing will ever take, and that loss
        would land on precisely the boards this gate exists for.  So both sites
        call THIS, and the gate moves them together.

        The verdict is the siege's own `_fs_map_gated`, read through
        `SiegeMixin._v535_map_refuses` -- geometry and confirmed grids only, no
        map names (map-robustness policy F3).  `Player` is
        ...
        ⛔ ORDER OF THE THREE TESTS IS LOAD-BEARING.  `FS_V530_CORNERS` first,
        so a corners-off arm never reaches the gate; the v535 master second, so
        `FS_V535_CORNER_GATE = False` returns EXACTLY `FS_V530_CORNERS` and the
        siege helper is never called at all -- that is what makes flag-off
        byte-identical to `_v534maptrust` rather than merely equivalent.
        The `except` returns the PARENT behaviour (corners on): an unreadable
        gate must not silently delete a shipped plank.
        """
        if not FS_V530_CORNERS:
            return False
        ...
        try:
            ...
            return True
        if refuses:
            self.v535_corner_gated += 1
            return False
        return True

    def _v530_corner_key_set(self):
        """The 4 diagonal core-ring corners, as (x, y), cached per body.

        Factored out of `_v530_corner_act` so the v532 mouth planner can ban
        exactly the tiles P2 will take, from the same source of truth.  Empty
        when this body does not know where our core is.
        """
        if self._v530_corner_keys is None:
            if self.core is None or not (self.mw and self.mh):
                return frozenset()
            self._v530_corner_keys = frozenset(
                (c.x, c.y) for c in core_corners(self.core, self.mw, self.mh))
        return self._v530_corner_keys

    def _v530_corner_act(self, ct, p):
        """Barrier an adjacent free DIAGONAL corner of our own core ring.

        Opportunistic, exactly as the outward collar is (raid.py:301): the
        4 cardinal neighbours of where this body already stands are tested
        against the corner set and `can_build_barrier` is the whole legality
        check.  No navigation, no reservation, no store traffic -- a home body
        passes the corners constantly.
        """
        if self.core is None or not (self.mw and self.mh):
            return False
        rnd = ct.get_current_round()
        if rnd < V530_CORNER_MIN_RND or rnd > V530_CORNER_MAX_RND:
            return False
        if ct.get_global_resources() < ct.get_barrier_cost() + V530_CORNER_TI_FLOOR:
            return False
        keys = self._v530_corner_key_set()
        if not keys:
            return False
        # ⛔ THE SPAWN HEDGE.  The 12 ring tiles are also our 12 spawn tiles;
        # `V530_CORNER_KEEP` corners are left open on purpose.  Counted over
        # the LIVE ring, not over a build counter, so a corner we lost is a
        # corner that re-opens rather than one this body believes it holds.
        held = 0
        for cx, cy in keys:
            try:
                bid = ct.get_tile_building_id(Position(cx, cy))
            except Exception:
                continue
            if bid is not None:
                held += 1
        if held >= len(keys) - V530_CORNER_KEEP:
            return False
        for dx, dy in CARD_DELTAS:
            tx, ty = p.x + dx, p.y + dy
            ...
            self.v530_corners += 1
            if FS_V530_LOG:
                print("V530 CORNER rnd=%d seat=%d tile=%d,%d held=%d"
                      % (rnd, self.role_n, tx, ty, held + 1), file=sys.stderr)
            return True
        return False
    ...
    def _expand(self, ct):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            if self.samestop_pending is not None and self._samestop_fire(ct):
                return
            # ⭐ v530 P1 -- MOUTH-FIRST BELT.  Ranked directly under the armed
            # same-stop build and ABOVE the parent's ore-first chain, because
            # it IS the parent's chain with its order inverted: whichever of
            # the two owns this body, only one of them can be live (arming
            # refuses when `link_queue` is non-empty, and the queue is never
            # populated while a mouth chain is live).
            if LOKI_FS_V530 and FS_V530_MOUTH and not endgame \
                    and self._v530_mouth_act(ct):
                return
            # ⛔ `_v530_mouth_live()` -- the two chain planners are MUTUALLY
            # EXCLUSIVE, and the first dose measured what happens when they are
            # not (see the predicate's own docstring).
            _v530_chain = LOKI_FS_V530 and FS_V530_MOUTH \
                and self._v530_mouth_live()
            if not endgame and not _v530_chain and self.link_queue \
                    and self._build_next_link(ct):
                return
            # ⭐ v530 P1b -- THE RING, DESIGNATED SEAT.  Only the designated
            # body outranks the harvester bootstrap with it; that is what
            # "a builder that builds the ring" means.  Every other home body
            # gets the opportunistic clause further down instead, so the ring
            # can never starve the economy on a roster where the designated
            # seat was issued to a raider.
            if LOKI_FS_V530 and FS_V530_RING and not endgame \
                    and self.role in ("expand", "defend") \
                    and self.role_n == V530_RING_SEAT \
                    and self._v530_ring_act(ct, p):
                return
            if not _v530_chain and (
                ...
                for ddx, ddy in DIR_DELTAS_LOCAL:
                    ...
                    if ok:
                        ...
                        break
            # ⭐ v530 P1b -- THE RING, OPPORTUNISTIC.  Any other home body that
            # happens to stand beside a free socket pays 3 Ti for it, below the
            # harvester and above the corners.
            if LOKI_FS_V530 and FS_V530_RING and not endgame \
                    and self.role in ("expand", "defend") \
                    and self.role_n != V530_RING_SEAT \
                    and self._v530_ring_act(ct, p):
                return
            # ⭐ v530 P2 -- CORNER BARRIERS.  Ranked BELOW the harvester
            # bootstrap and below the chain on purpose: a 3 Ti barrier must
            # never outbid a 20 Ti harvester or the mouth socket on a tight
            # opening round.  If this body already acted above, the engine's
            # own cooldown makes `can_build_barrier` False and this is a
            # no-op -- the harvester clause `break`s rather than returning,
            # so that guard has to come from the engine, not from a flag.
            # ⭐ v535 -- gated by `_v535_corners_on(ct)` instead of the bare
            # `FS_V530_CORNERS`.  Placed LAST in this conjunction on purpose:
            # `endgame` and the role test are free, the gate is not, so a body
            # that could never build a corner anyway never pays for the verdict.
            if LOKI_FS_V530 and not endgame \
                    and self.role in ("expand", "defend") \
                    and self._v535_corners_on(ct) \
                    and self._v530_corner_act(ct, p):
                return
        ...
        if ct.get_move_cooldown() != 0:
            return
        # ⭐ v530 P1 -- the movement half of MOUTH-FIRST, and it sits ABOVE the
        # ORE STEP-OFF rather than beside the link-queue walk.  The mouth-first
        # walk deliberately STANDS ON THE ORE to lay the chain's last link (the
        # ore is the only outward neighbour that tile has), so the generic
        # step-off would evict the body one round before it could build.  This
        # clause owns the whole walk while a mouth chain is live and performs
        # its own step-off, including off the ore once the chain is whole.
        if LOKI_FS_V530 and FS_V530_MOUTH and self._v530_mouth_move(ct):
            return
        # v530 P1b: the ring steer, below the mouth walk -- a body carrying a
        # live mouth chain finishes it first.
        if LOKI_FS_V530 and FS_V530_RING \
                and self.role in ("expand", "defend") \
                and self._v530_ring_move(ct, p):
            return
