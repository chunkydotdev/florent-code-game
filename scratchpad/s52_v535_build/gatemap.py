#!/usr/bin/env python3
"""v535 build instrument #2 -- WHERE THE GATE BINDS, over the whole map pool.

For every `maps/*.map26`, from BOTH seats, this drives the shipped predicate
end-to-end on a real `Player` and prints:

    map            seat  refuse  corners_on(v535)  corners_on(parent)  delta

`corners_on(parent)` is `_v534maptrust`'s behaviour, which is the constant
`FS_V530_CORNERS` -- i.e. TRUE on every board, which is the map-INVARIANCE this
...
and two flag mutants:
  5. FS_V535_CORNER_GATE = False on a refusing board       -> corners ON
  6. FS_V530_CORNERS   = False on a running board          -> corners OFF
...
def one(mods, name, seat):
    """(refuse, corners_on) for `name` from `seat` (0 = team A anchor)."""
    ...
    on, refuse = probe_corners(mods, w, h, Position(*ours), Position(*theirs), ct)
    return refuse, on


def table(tree="_v535cornergate", partree="_v534maptrust"):
    mods = load_tree(tree)
    par = load_tree(partree)
    rows = []
    print("%-16s %4s %7s %8s %8s  %s"
          % ("map", "seat", "refuse", "v535_on", "par_on", "delta"))
    for name in maps_in_pool():
        for seat in (0, 1):
            refuse, on = one(mods, name, seat)
            par_on = par["doctrine"].FS_V530_CORNERS
...
def _flagged_tree(base, **over):
    ...
    ⛔ Legitimate here and nowhere near a shipped arm: every v535 name is a
...
def selftest():
    ok = True
    mods = load_tree("_v535cornergate")
    D = mods["doctrine"]
    ...
    on, refuse = probe_corners(mods, 8, 8, Position(1, 1), Position(6, 6), ct)
    check("mutant 1 (8x8 board, under min dim) refuses", bool(refuse), True)
    check("mutant 1 corners stand down", on, False)

    big = tuple("." * 30 for _ in range(30))
    ct = FakeCt(big, (10, 10), CORE_VIS, Position, Environment)
    on, refuse = probe_corners(mods, 30, 30, Position(10, 10), Position(11, 11), ct)
    ...
    on, refuse = probe_corners(mods, 30, 30, Position(2, 2), Position(27, 27), ct)
    check("control (30x30 open, cores far) RUNS", bool(refuse), False)
    check("control corners stay ON", on, True)
    ...
    check("mutant 3 archipelago corners stand down", on, False)
    ...
    on, refuse = probe_corners(mods, w, h, Position(*cores[0]),
                               Position(*cores[1]), ct)
    check("mutant 4 (arch signature, flat grid) RUNS", bool(refuse), False)
    check("mutant 4 corners stay ON", on, True)

    # --- 5. FS_V535_CORNER_GATE = False on a REFUSING board ---------------
    off = _flagged_tree("_v535cornergate", FS_V535_CORNER_GATE=False)
    refuse_off, on_off = one(off, "archipelago", 0)
    check("mutant 5 gate-off: corners ON on a refusing board", on_off, True)
    check("mutant 5 gate-off: refusal never computed (cache stays None)",
          refuse_off, None)

    # --- 6. FS_V530_CORNERS = False on a RUNNING board --------------------
    coff = _flagged_tree("_v535cornergate", FS_V530_CORNERS=False)
    refuse_c, on_c = one(coff, "nordkap", 0)
    check("mutant 6 corners-off: corners OFF on a running board", on_c, False)
    check("mutant 6 corners-off: gate never consulted (cache stays None)",
          refuse_c, None)

    # --- 7. the PARENT is map-invariant: corners ON everywhere ------------
    par = load_tree("_v534maptrust")
    check("parent FS_V530_CORNERS is the whole predicate",
          par["doctrine"].FS_V530_CORNERS, True)
