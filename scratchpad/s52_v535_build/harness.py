#!/usr/bin/env python3
"""v535 build instrument #1 -- THE UNIT HARNESS (no game engine, no fcode run).

Loaded from `scratchpad/s52_v534_build/harness.py` and extended for v535: it
still loads a bot TREE's modules in isolation and drives them against a FAKE
controller backed by an explicit terrain grid, but v535's predicate lives on
`Player` (the `(EcoMixin, RaidMixin, SiegeMixin)` composition in main.py), so
`load_tree` now also imports `raid` and `main`, and `probe_corners` builds a
...
local dose tape or is deferred to the powered battery.
...
def make_player(mods, mw, mh, ours, enemy=None, map_grid=None, role="expand"):
    ...
    ⛔ NOT a stand-in class: v535's predicate spans EcoMixin (`_v535_corners_on`)
...
def probe_corners(mods, mw, mh, ours, enemy, ct, map_grid=None):
    """`_v535_corners_on` on a real Player -> (corners_on, refuse_cache)."""
    o = make_player(mods, mw, mh, ours, enemy, map_grid)
    on = o._v535_corners_on(ct)
...
def selftest():
    ...
    new = load_tree("_v535cornergate")

    # 1. the two trees are DIFFERENT objects with different flags
    if hasattr(par["doctrine"], "FS_V535_CORNER_GATE"):
        ok = False
        print("[FAIL] loader: parent must not carry the v535 flag")
    else:
        print("[ok] loader: parent (_v534maptrust) has no FS_V535_CORNER_GATE")
    if new["doctrine"].FS_V535_CORNER_GATE is not True:
        ok = False
        print("[FAIL] loader: child flag is not True")
    else:
        print("[ok] loader: child (_v535cornergate) FS_V535_CORNER_GATE=True")
    ...
    if hasattr(make_player(par, w, h, Position(*cores[0])), "_v535_corners_on"):
        ok = False
        print("[FAIL] make_player: PARENT Player has _v535_corners_on")
    else:
        print("[ok] make_player: parent Player has no _v535_corners_on "
