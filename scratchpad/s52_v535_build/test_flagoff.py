#!/usr/bin/env python3
"""v535 build instrument #3 -- FLAG-OFF IS THE PARENT, PROVED FOUR WAYS.

1. BYTES. `raid.py` is untouched (md5 vs the frozen parent). The other four
   files differ only where the diff audit enumerates.
2. READ-SITE SCAN, WITH THE HOUSE POSITIVE CONTROL. Every v535 name must be
   ...
   DEAD FLAG in this lineage (doctrine.py says so in as many words: defined,
   never read). A scanner that reports "all v535 flags are read in functions"
   ...
   a live parent flag (`FS_V530_CORNERS`) and MUST return several.
...
4. BEHAVIOUR. With `FS_V535_CORNER_GATE = False` the predicate returns exactly
   `FS_V530_CORNERS` on every cell of the whole map pool -- INCLUDING the 16
...
V535_FLAGS = {"LOKI_FS_V535", "FS_V535_CORNER_GATE", "FS_V535_LOG"}
DEAD_FLAG = "FS_V530_MOUTH_SEATS"     # the house positive control
LIVE_FLAG = "FS_V530_CORNERS"         # the house negative control
...
def main():
    ...
    print("    v535 read sites: %s" % (allsites,))
    check("every v535 read site is INSIDE a function",
          all(s[3] for s in allsites), True)
    check("v535 flags are read at all (>=3 sites)", len(allsites) >= 3, True)
    got_flags = sorted({s[2] for s in allsites})
    check("all three v535 names are read somewhere",
    ...
    check("no module-level default derives from a v535 flag", derived, [])
    ...
    off = _flagged_tree(CHILD, FS_V535_CORNER_GATE=False)
    on = load_tree(CHILD)
    ...
    # FINDING ABOUT THE SHIPPED CODE, NOT A TEST DETAIL: `_v535_corners_on`
