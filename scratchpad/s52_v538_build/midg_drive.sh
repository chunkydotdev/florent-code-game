...
export PAR=4
export OUT=scratchpad/s52_v538_build/midg
...
export MAPS=midgard
echo "MIDG START $(date -u +%Y-%m-%dT%H:%M:%SZ)"
for i in 1 2 3 4 5 6; do
  ...
  echo "  MIDG BLOCK $i (seeds $a-$b) done $(date -u +%H:%M:%SZ)"
done
echo "MIDG DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
