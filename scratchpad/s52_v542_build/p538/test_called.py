#!/usr/bin/env python3
"""v538 build instrument #4 -- CALLED, NOT RE-DERIVED (D21d).

COPIED from `scratchpad/s52_v535_build/test_called.py` (never edited in place).
The v538 build PORTS `_fs_enemy_anchor` and `_v535_map_refuses` onto the
v537socket lineage, which never carried them, so the same three questions have
to be asked again OF THIS TREE -- a port inherits the original's DESIGN, not
its verification.
...
2. **THE EXTRACTION IS BEHAVIOUR-PRESERVING.** `_fs_enemy_anchor` was pulled
   ...
   is compared against the PARENT `_v537socket._fs_gate` over the whole map
