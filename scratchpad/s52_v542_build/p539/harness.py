...
def selftest():
    ...
    dec, cle, _ = detector_run("_v542wave", 200, wipe_at=120,
                               min_rnd_override=60)
    ...
    v_f = gate_probe("_v542wave", True, 8, "expand")
    v_n = gate_probe("_v542wave", False, 8, "expand")
    v_raid = gate_probe("_v542wave", True, 8, "raid")
    v_late = gate_probe("_v542wave", True, 8, "expand", rnd=200,
                        started=100)
    ...
    r_f, n_f = draft_probe("_v542wave", True, 6)
    r_n, _ = draft_probe("_v542wave", False, 6)
    r_p, _ = draft_probe("_v537socket", True, 6)
    r_eco, _ = draft_probe("_v542wave", False, 2)
    ...
    lo_on = gate_probe("_v542wave", True, 8, "expand", floor=True)
    lo_off = gate_probe("_v542wave", True, 8, "expand", floor=False)
    chk(lo_on is False,
    ...
    hi_on = gate_probe("_v542wave", True, bar + 200, "expand", floor=True)
    chk(hi_on is True,
    ...
    for cost in ("conveyor", "harvester"):
        for collar in (False, True):
            ...
            par = [gate_probe("_v537socket", True, b, "expand", **kw)
                   for b in banks]
            on = [gate_probe("_v542wave", True, b, "expand", floor=True,
                             **kw) for b in banks]
            off = [gate_probe("_v542wave", True, b, "expand", floor=False,
                              **kw) for b in banks]
    ...
    nf_on = gate_probe("_v542wave", False, bar + 200, "expand",
                       floor=True)
...
def main():
    ...
    ap.add_argument("--arms", default="_v537socket,_v542wave",
