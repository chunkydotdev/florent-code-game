# =====================================================================
# FIXTURE COPY -- "Sleipnir v2" NOISE_OFF sparring partner (F3), s57.
# In-game league fixture; nothing here runs outside the game engine.
#
# SOURCE: bots/_v488beltbreak2 (platform v159, corpus/version_trees.tsv:93),
# copied VERBATIM.  The canonical bots/ tree is never edited.
#
# DETERMINISM AUDIT (s57 F3).  The whole 4-file tree was grepped for
# `random` / `Random` / `randrange` / `randint` / `shuffle` / `choice` /
# `seed` / `time.*` / `datetime` / `urandom` / `uuid` / `hash(`.
# EXACTLY ONE randomness source exists:
#     main.py:445  self.spawn_salt = random.Random().randrange(97) if NOISE_ON else 0
# It is the s-line "PIECE G -- DECISION NOISE" spawn-dispersion salt, drawn
# once per match from OS entropy, and it is ALREADY gated on this file's
# NOISE_ON flag (main.py does `from doctrine import *` at line 35 and never
# rebinds the name).  `import random` at line 20 below is otherwise unused --
# no other call site in doctrine.py/eco.py/main.py/raid.py.
#
# THE SINGLE MODIFICATION TO THIS COPY: NOISE_ON True -> False (line ~474),
# which drives spawn_salt to the constant 0.  Identical treatment to the two
# existing fixtures (scratchpad/s54_fidtape/mkfixture.sh; the v542wave and
# Mjolnir copies differ from their bots/ trees by exactly this one line).
# Verified by IMPORT, not by reading the assignment, and then proved
# empirically by a same-cell double run through tools/rdiff.py.
# =====================================================================
