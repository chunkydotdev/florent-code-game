#!/bin/zsh
# s54 v601 fidelity tape: bots/_v602skalman vs NOISE_OFF _v542wave copy,
...
# plank tree bots/_v602skalman is on-line (_v[2-9]??* matches).
set -u
cd /Users/junghard/Projects/Work/florent-code-game
FC=.venv/bin/fcode
BASE=scratchpad/s54_fidtape
OPP=$BASE/opp_v542wave_noiseoff
OUT=$BASE/replays_tape602
mkdir -p $OUT
...
run_one() {
  ...
  if [[ $SEAT == A ]]; then
    timeout 240 $FC run bots/_v602skalman $OPP maps/$M.map26 --seed 11 --tle 10 --replay $R > $BASE/tape602_${M}_${SEAT}.log 2>&1
  else
    timeout 240 $FC run $OPP bots/_v602skalman maps/$M.map26 --seed 11 --tle 10 --replay $R > $BASE/tape602_${M}_${SEAT}.log 2>&1
  fi
