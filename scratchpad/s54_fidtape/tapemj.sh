#!/bin/zsh
# s54 v601 fidelity tape: bots/_v612skalman vs NOISE_OFF _v542wave copy,
...
# plank tree bots/_v612skalman is on-line (_v[2-9]??* matches).
set -u
cd /Users/junghard/Projects/Work/florent-code-game
FC=.venv/bin/fcode
BASE=scratchpad/s54_fidtape
OPP=$BASE/opp_mjolnir_noiseoff
OUT=$BASE/replays_tapemj
mkdir -p $OUT
...
run_one() {
  ...
  if [[ $SEAT == A ]]; then
    timeout 240 $FC run bots/_v612skalman $OPP maps/$M.map26 --seed 11 --tle 10 --replay $R > $BASE/tapemj_${M}_${SEAT}.log 2>&1
  else
    timeout 240 $FC run $OPP bots/_v612skalman maps/$M.map26 --seed 11 --tle 10 --replay $R > $BASE/tapemj_${M}_${SEAT}.log 2>&1
  fi
