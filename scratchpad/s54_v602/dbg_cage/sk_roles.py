        ...
        if (SK_CAGE_FIRST and here is None and not empty_seals
            ...
            for i in LAP_SEAL_IDX:
                ...
                if abs(q.x - p.x) + abs(q.y - p.y) != 1:
                    continue
                import sys as _s
                _r = self._clear_tile(ct, q, rnd)
                if rnd > 360 and rnd < 375:
                    try:
                        _bid = ct.get_tile_building_id(q)
                        print("DBG OFFLAP rnd=%d p=%s q=%s clear=%s bid=%s gaveup=%s enemyadj=%s res=%d melee=%s since=%d" % (
                            rnd, p, q, _r, _bid, self.gave_up(_bid, rnd),
                            self._enemy_builder_adjacent(ct, q), ct.get_global_resources(),
                            self.melee_tile, self.melee_since), file=_s.stderr)
                    except Exception as _e:
                        print("DBG OFFLAP err", _e, file=_s.stderr)
                if _r:
                    return
