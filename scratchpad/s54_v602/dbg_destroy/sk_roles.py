        ...
        for q in cheap + dear:
            try:
                if not ct.can_destroy(q):
                    continue
                import sys as _s
                print("DBG ESCAPE boxed=%s cycled=%s rnd=%d role=%s q=%s cyclen=%d cyblk=%d" % (boxed, cycled, rnd, self.role, q, self.cycle_len, self.cycle_blocked), file=_s.stderr)
                ct.destroy(q)
            except Exception:
                continue
    ...
    def _belt_evict(self, ct, p, q, bid, rnd):
        ...
        if mine:
            ...
            try:
                if ct.can_destroy(q):
                    import sys as _s
                    print("DBG BELT_EVICT destroy rnd=%d q=%s" % (rnd, q), file=_s.stderr)
                    ct.destroy(q)
                    self.belt_built.discard((q.x, q.y))
            except Exception:
                return False
