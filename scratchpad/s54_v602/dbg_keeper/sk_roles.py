...
class RolesMixin:
    ...
    def _home_keeper_move(self, ct, p, rnd):
        ...
        if shooter is not None:
            _mv = self.step_to(ct, shooter)
            if 200 <= rnd <= 206 and self.role == 0:
                import sys as _s
                print("DBG KEEP-SHOOTER rnd=%d p=%s shooter=%s moved=%s harvesc=%s beltesc=%s" % (
                    rnd, p, shooter, _mv, sorted(self.harv_escalated), sorted(self.belt_escalated)), file=_s.stderr)
            return
        self._plan_belt(ct)
        tgt = None
        best = None
        ...
        if 200 <= rnd <= 201 and self.role == 0:
            import sys as _s
            for _d in CARDINALS:
                _q = p.add(_d)
                if not self.ibp(_q):
                    continue
                try:
                    print("DBG TILE rnd=%d q=%s passable=%s bld=%s bot=%s env=%s canmove=%s" % (
                        rnd, _q, ct.is_tile_passable(_q), ct.get_tile_building_id(_q),
                        ct.get_tile_builder_bot_id(_q), ct.get_tile_env(_q), ct.can_move(_d)), file=_s.stderr)
                except Exception as _e:
                    print("DBG TILE err", _e, file=_s.stderr)
        if ((p.x, p.y) in self.belt_plan and (p.x, p.y) not in self.belt_built
            ...
            for d in CARDINALS:
                ...
                try:
                    ...
                    continue
                _mv2 = self.step_to(ct, q)
                if 200 <= rnd <= 206 and self.role == 0:
                    import sys as _s
                    print("DBG KEEP-STEPOFF rnd=%d p=%s q=%s moved=%s" % (rnd, p, q, _mv2), file=_s.stderr)
                return
        for (x, y) in self.belt_plan:
        ...
        if tgt is None:
            tgt = self.core
        _mv = self.step_to(ct, tgt)
        if 200 <= rnd <= 206 and self.role == 0:
            import sys as _s
            print("DBG KEEP rnd=%d p=%s tgt=%s moved=%s mcd=%d acd=%d plan=%d built=%d harv=%d cyc=%d blk=%d det=%d stuck=%d" % (
                rnd, p, tgt, _mv, ct.get_move_cooldown(), ct.get_action_cooldown(),
                len(self.belt_plan), len(self.belt_built), len(self.harv_tiles),
                self.cycle_len, self.cycle_blocked, self.danger_detour, self.stuck), file=_s.stderr)
    ...
    def _cage_walker(self, ct, p, rnd):
        ...
        if (SK_CAGE_FIRST and here is None and not empty_seals
