    ...
    def _core(self, ct):
        ...
        if not endgame_dumped:
            ...
            if LOKI_FERRY_SIEGE_ON:
                ...
                if FS_PH_RING <= fs_ph <= FS_PH_KILL:
                    fs_live = True
            ...
            if fs_live:
                ...
                if fs_ph == FS_PH_KILL:
                    ...
                    _need = self._fs_need(ct)
                    _n = 8 if _need is None else _need
                    if _n <= 0:
                        ti_floor = FS_AMMO_TI_FLOOR
                    else:
                        ti_floor = max(FS_AMMO_TI_FLOOR,
                                       _n * bar + FS_SEAL_MARGIN + 6)
            ...
            import sys as _sys
            try:
                _bar = ct.get_barrier_cost()
            except Exception:
                _bar = -1
            print("COREAMMO r=%d ti=%d ammo=%d fs_ph=%d fs_live=%d under=%d wtop=%d bb=%d harv=%d "
                  "ammo_target=%d ti_floor=%d chunk=%d bar=%d scale=%.0f arm=%d gate_ti=%d amt=%d"
                  % (rnd, ti, ammo, fs_ph, int(fs_live), int(under), weapons_top, int(bb_live), harv,
                     ammo_target, ti_floor, chunk, _bar, ct.get_scale_percent(),
                     int(bool(under or weapons_top or bb_live or fs_live or harv >= 2)),
                     int(ti > ti_floor),
                     min(chunk, ammo_target - ammo, ti - ti_floor)), file=_sys.stderr)
        ...
        if LOKI_FERRY_SIEGE_ON and self._fs_gate(ct):
            ...
            if ph != FS_PH_DEGRADE:
                ...
                if ph <= FS_PH_FERRY and (ct.get_max_hp() - hp) < FS_PANIC_DMG:
                    budget = min(budget, FS_OPEN_BUILDERS + self.fs_replaced)
    ...
    def _builder(self, ct):
        ...
        if LOKI_FERRY_SIEGE_ON and not self.fs_role_done:
            ...
            if self._fs_gate(ct):
                ...
                if _ph != FS_PH_DEGRADE and (
                    self.role_n == 0
                    or (rnd >= FS_BEAT_STALE and self._fs_stale(ct, rnd))
                ):
                    self.fs_raider = True
                    if self.role != "raid":
                        self.role = "raid"
