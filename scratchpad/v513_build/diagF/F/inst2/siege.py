...
class SiegeMixin:
    ...
    def _fs_publish(self, ct, rnd, phase, rid, need=None):
        try:
            nb = 0 if need is None else min(need + 1, FS_NEED_MASK)
            ct.write_store(
                SLOT_FS,
                ((rnd + 1) & FS_BEAT_MASK)
                | ((phase & FS_PHASE_MASK) << FS_PHASE_SHIFT)
                | (((rid + 1) & FS_RID_MASK) << FS_RID_SHIFT)
                | ((nb & FS_NEED_MASK) << FS_NEED_SHIFT),
            )
        except Exception:
            return

    def _fs_need(self, ct):
        """Orthogonal seats the raider still owes, or None if unpublished."""
        try:
            v = ct.read_store(SLOT_FS)
        except Exception:
            return None
        nb = (v >> FS_NEED_SHIFT) & FS_NEED_MASK
        return None if nb == 0 else nb - 1
    ...
    def _fs_turn(self, ct):
        ...
        self._fs_publish(ct, rnd, phase, ct.get_id(),
                         None if needed is None else len(needed))
