...
class SiegeMixin:
    ...
    def _fs_turn(self, ct):
        ...
        # FORCED-DEATH INSTRUMENT (scratch copy only, never the repo tree):
        # the sealer removes itself at r60 so the replacement latency can be
        # measured against a known death round.
        if rnd == 60 and not self.fs_off:
            self._fs_log("FORCEDEATH", rnd, "id", ct.get_id())
            ct.self_destruct()
