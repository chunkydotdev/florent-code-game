"""LOKI-CHAMPION (d3) -- hungrier seal/salt/guns; official WR ~53.7% vs baseline loki (n=600).
...
class Player(EcoMixin, RaidMixin):
    ...
    def _core(self, ct):
        ...
        if not hasattr(self, "spawn_salt"):
            # Keep both policy branches on the same terrain-only opening so
            # the wrapper cannot reintroduce OS-entropy noise.
            sig = 2166136261
