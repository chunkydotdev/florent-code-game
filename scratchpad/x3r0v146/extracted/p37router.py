...
def _decode_grid(w, h, code):
    cells = []
    for ch in code:
        value = p37h_doctrine.MAP_ALPHABET.index(ch)
        for _ in range(3):
...
def _weak_grids():
    grids = set()
    for key, code in p37h_doctrine.MAP_CODES.items():
        if key in WEAK_KEYS:
            grids.add(_decode_grid(key[0], key[1], code))
    for key, code in p37h_doctrine.EXTRA_MAP_CODES:
        if key in WEAK_KEYS:
            grids.add(_decode_grid(key[0], key[1], code))
    ...
    def _own_core(self, ct):
        ...
        for entity_id in ct.get_nearby_buildings():
            try:
                ...
                continue
        enemy = p37h_eco.unpack_pos(ct.read_store(p37h_doctrine.SLOT_ENEMY_CORE))
        if enemy is not None:
            return p37h_eco.enemy_core_for(ct.get_map_width(), ct.get_map_height(), enemy)
        return None
    ...
    def run(self, ct):
        if self.inner is None:
            ...
            if core is None:
                ...
                self.inner = p37h_main.Player()
            else:
                grid = p37h_eco.known_map_for(
                    ct.get_map_width(), ct.get_map_height(), core, ct
                )
                self.inner = p37i_main.Player() if grid in WEAK_GRIDS else p37h_main.Player()
        self.inner.run(ct)
