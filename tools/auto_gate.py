...
beside it. ⛔ CORRECTED SAME HOUR AS REGISTERED: the first push said
"true-neutral ~50%" and ~30/60 — a DEFINITION slip ("neutral" is ambiguous
when the floor is not at 50) that UNDERSTATED the noise output by 14 arms,
i.e. exactly the direction that makes a noisy count look like a discovery.
Caught by research pre-consumption; numbers re-derived independently here.
Survivors are enriched for 53+, not 51+: at the floor, ~3 of 4 genuinely
neutral arms are cancelled.
