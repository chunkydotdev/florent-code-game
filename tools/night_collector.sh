...
# THE BLEED BAND — ranks ~25-40, us-110..us+15. This band is -438.6 Elo across
# 58% of our match diet and it is the single biggest number on the board with
# NO MECHANISM ATTACHED. Nobody knows why we lose here. Six unattended hours
# against it produces per-opponent kill-speed, what kills us and when, on which
# maps -- exactly the input a morning plank needs.
#   kladde +3 · farming_200s -1 · Lunds -28 · Landers -32 · I Stone -53
#   gsxWins -70 · CtrlAltDefeat -72 · Team 48 -84 · Powerpuff -96
PANEL2=(c7571c87-f960-4c88-a13d-14340bb3200f 25288fdb-f3a0-4ae2-964f-2fd7eb9b11d7 eceb8455-7cb3-442b-ba40-c6597c16b446 b538e523-2250-4c1d-b718-e73b77ebad55 bfbb9a68-b37a-4a61-b0ea-d36369c8f65a ebd8d82a-7365-4ccb-af0b-defea3a1ac4d 74e43df6-bad7-474b-8e37-0ea44a2c80f1 48340ad8-701f-4a40-850d-1f3f3d56d8ca 0c1fea85-495c-41e3-bb30-f6665ddd2cd4)
...
OUT=${OUT:-scratchpad/arm_night.txt}
CYCLES=${1:-5}
...
for cycle in $(seq 1 $CYCLES); do
  ...
  if [[ "$live" != v${INCUMBENT}* ]]; then
    printf '%s night collector aborted: holder v%s expected, saw "%s"\n' "$(date -u +%H:%M:%SZ)" "$INCUMBENT" "$live" >> corpus/FANOUT_ABORT
    echo "$(date -u +%H:%M:%SZ) NIGHT: ABORT -- expected v$INCUMBENT, holder is '$live'. Firing nothing."
    exit 1
  fi
  n=0
  ...
  echo "$(date -u +%H:%M:%SZ) NIGHT cycle $cycle: fired $n/${#PANEL2} (total $(grep -c matchId $OUT))"
  ...
  if [[ $cycle -lt $CYCLES ]]; then
    ...
    [[ "$w" =~ ^[0-9]+$ ]] || w=$WINDOW
    echo "$(date -u +%H:%M:%SZ) NIGHT: meter says wait ${w}s"
    (( w > 0 )) && sleep $w
  fi
done
echo "$(date -u +%H:%M:%SZ) NIGHT: done, $(grep -c matchId $OUT) challenges banked"
