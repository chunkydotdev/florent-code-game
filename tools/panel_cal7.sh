#!/bin/zsh
# PANEL-CAL-7 runner — THE DIVERGENCE PANEL. Research fire order #1
...
PTR=${PTR:-scratchpad/panel_cal7_pointer.txt}
OUT=${OUT:-scratchpad/panel_cal7_fires.tsv}
STOP=scratchpad/PANEL_CAL7_STOP
BOUNDARY=${BOUNDARY:-60}        # accepts; 60 x 5 games = n=300, the ONE comparative look
...
for w in $(seq 1 $WINDOWS); do
  if [[ -f $STOP ]]; then
    echo "$(date -u +%H:%M:%SZ) PANEL-CAL-7: STOP file present, yielding." | tee -a $OUT
    exit 0
  fi
  n=0
  while (( n < 5 )); do
    [[ -f $STOP ]] && { echo "$(date -u +%H:%M:%SZ) PANEL-CAL-7: STOP mid-window after $n fires." | tee -a $OUT; exit 0; }
    ...
    if [[ "$live" != v${INCUMBENT}* ]]; then
      ...
      echo "$(date -u +%H:%M:%SZ) PANEL-CAL-7 ABORT: holder is '$live', not v$INCUMBENT. Firing nothing."
      exit 1
    fi
  ...
  echo "$(date -u +%H:%M:%SZ) PANEL-CAL-7: window $w complete (5 accepted), pointer at $(cat $PTR)."
